...
set_parameter_property audio_clk_freq ALLOWED_RANGES {18.432 16.9344 12.288 12.0 11.2896 0.768}
set_parameter_property audio_clk_freq VISIBLE true
set_parameter_property audio_clk_freq ENABLED true

# add_parameter audio_bclk_generation boolean false
# set_parameter_property audio_bclk_generation DISPLAY_NAME "Generate Bit Clock"
# set_parameter_property audio_bclk_generation GROUP "Settings"
# set_parameter_property audio_bclk_generation AFFECTS_ELABORATION true
# set_parameter_property audio_bclk_generation AFFECTS_GENERATION true
# set_parameter_property audio_bclk_generation VISIBLE true
# set_parameter_property audio_bclk_generation ENABLED true

# add_parameter audio_bclk_freq float 0.7680775
# set_parameter_property audio_bclk_freq DISPLAY_NAME "Audio Bit Clock Frequency"
# set_parameter_property audio_bclk_freq GROUP "Settings"
# set_parameter_property audio_bclk_freq UNITS Megahertz
# set_parameter_property audio_bclk_freq AFFECTS_GENERATION true
# set_parameter_property audio_bclk_freq DERIVED true
# set_parameter_property audio_bclk_freq VISIBLE false
# set_parameter_property audio_bclk_freq ENABLED false
...
proc compose {} {
	...
	if {  [ string match -nocase "Cyclone IV*" $device_family ]  ||
		...
		if { [ string match -nocase "Cyclone V*" $device_family ] ||
			[ string match -nocase "Max 10*" $device_family ] } {
			add_instance audio_pll altera_pll
		} else {
			add_instance audio_pll altera_iopll
		}

		set_instance_property audio_pll SUPPRESS_ALL_INFO_MESSAGES true
		set_instance_property audio_pll SUPPRESS_ALL_WARNINGS true

		if { [expr {$audio_clk_freq < 1} ] } { 
			set ref_clk_factor [expr {100.0 / $refclk}]
			set updated_audio_clk_freq [expr {$audio_clk_freq * $ref_clk_factor}]

			set_instance_parameter_value audio_pll gui_reference_clock_frequency 100
			set_instance_parameter_value audio_pll gui_output_clock_frequency0 $updated_audio_clk_freq
		} else {

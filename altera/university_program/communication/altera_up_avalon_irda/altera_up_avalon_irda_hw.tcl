...
# | parameters
# | 
add_parameter ref_clk_freq float 50000000
set_parameter_property ref_clk_freq DISPLAY_NAME "Frequency"
set_parameter_property ref_clk_freq GROUP "Reference Clock"
set_parameter_property ref_clk_freq AFFECTS_ELABORATION true
set_parameter_property ref_clk_freq DERIVED true
set_parameter_property ref_clk_freq VISIBLE false
...
# | 
proc elaborate {} {
	set sys_clk_freq [ get_interface_property clk clockRate ]
    set_parameter_value "ref_clk_freq" $sys_clk_freq
...
proc generate {} {
	...
	set data_bits [ get_parameter_value "data_bits" ]
	set stop_bits [ get_parameter_value "stop_bits" ]
	set ref_clk_freq 		    [ get_parameter_value "ref_clk_freq" ]

    # check parameters
	if { $ref_clk_freq <= 0 } {
    	send_message error "The input clock frequency must be known at generation time."
    }

    # set parameter values
	set counter_width  [ format "%.0f" [ expr ceil (log(($ref_clk_freq / $baud) + 1) / (log (2))) ] ]

	set baud_counter_width			"CW:$counter_width"
	set baud_tick_increment			"BAUD_TICK_INCREMENT:$counter_width'd1"
	set baud_tick_count				[ format "BAUD_TICK_COUNT:$counter_width'd%.0f" [ expr $ref_clk_freq / $baud ] ]
	set baud_3_16_tick_count		[ format "BAUD_3_16_TICK_COUNT:$counter_width'd%.0f" [ expr ($ref_clk_freq / $baud) * 3 / 16 ] ]
	set baud_capture_tick_count	[ format "CAPTURE_IN_TICK_COUNT:$counter_width'd%.0f" [ expr ($ref_clk_freq / $baud) * 9 / 64 ] ]
	set half_baud_tick_increment	[ format "HALF_BAUD_TICK_COUNT:$counter_width'd%.0f" [ expr $ref_clk_freq / ($baud * 2) ] ]
	...
	# set arguments
	set params "$baud_counter_width;$baud_tick_increment;$baud_tick_count;$baud_3_16_tick_count;$baud_capture_tick_count;$half_baud_tick_increment;$total_data_width;$data_width;$odd_parity"
...
add_documentation_link "User Guide" file://ip/altera/university_program/input_output/altera_up_avalon_parallel_port/doc/Parallel_Port.pdf

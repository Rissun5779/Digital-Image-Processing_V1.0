...
set_parameter_property board ALLOWED_RANGES {"DE10-Standard" "DE10-Lite" "DE0-CV" "DE1-SoC" "DE2-115" "DE2i-150"}
...
set_parameter_property device ALLOWED_RANGES {"VGA Connector" "4.3\" LCD (TRDB_LTM)" "8\" LCD on TPad/VEEK" "7\" LCD on VEEK-MT and MTL/MTL2 Modules"}
...
proc validate {} {
	...
	if { $device == "LCD Screen (TRDB_LCM)" } {
	...
	} elseif { $device == "7\" LCD on VEEK-MT and MTL/MTL2 Modules" } {
		send_message info "Video Output Stream: Format: 800 x 480 with Color: 10 (bits) x 3 (planes) converted to 8 (bits) per color plane"
	} else {
		set_parameter_property resolution ENABLED true
		...
		} elseif { ($board == "DE10-Lite") || ($board == "DE0-CV") } {
			set converted_to_msg " converted to 4 (bits) per color plane"
		}
...
#		add_interface_port external_interface VGA_SYNC export Output 1
		...
		if { ($board == "DE10-Standard") || ($board == "DE1-SoC") || ($board == "DE2-115") || ($board == "DE2i-150") } {
			add_interface_port external_interface VGA_BLANK export Output 1
			add_interface_port external_interface VGA_SYNC export Output 1
			add_interface_port external_interface VGA_R export Output 8
			add_interface_port external_interface VGA_G export Output 8
...
proc generate {} {
	...
	set use_veek_mt_screen		[ format "USE_VEEK_MT:%d" [ string match "7\" LCD on VEEK-MT and MTL/MTL2 Modules" $device ] ]

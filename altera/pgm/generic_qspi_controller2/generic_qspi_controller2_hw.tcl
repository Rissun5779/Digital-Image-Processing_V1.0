...
set_parameter_property DDASI VISIBLE true
set_parameter_property DDASI DERIVED false
set_parameter_property DDASI DISPLAY_HINT BOOLEAN

# Use GPIO
add_parameter           GUI_USE_GPIO    BOOLEAN             1
...
proc compose {} {
    ...
    set disable_asmiblock       [get_parameter_value DDASI]
    set gui_use_gpio            [get_parameter_value GUI_USE_GPIO]
    ...
    # Does not need this anymore, old controller expect epcq is using asmiblock
    # now expose the option to user.
    ...
    #Max10 does not support asmiblock
    ...
    #}
    if {!$disable_asmiblock  && [check_device_family_equivalence $DEVICE_FAMILY "MAX 10 FPGA"]} {
       send_message error "MAX 10 FPGA does not support dedicated active serial interface mode"
    }

    # check whether devices supporting multiple flash - only for Arria 10
    if {$disable_asmiblock || [check_device_family_equivalence $DEVICE_FAMILY "Arria10"] || [check_device_family_equivalence $DEVICE_FAMILY "Cyclone10GX"]} {
    ...
    if {[check_device_family_equivalence $DEVICE_FAMILY "MAX 10 FPGA"]} {
        ...
        # If C10LP, if use choose asmiblock then it only supports STANDARD, or else can be either
        if {!$disable_asmiblock} {
        ...
        if {!$disable_asmiblock} {
            send_message error "Stratix 10 does not support dedicated active serial interface mode"
        } else { 
            set_parameter_property IO_MODE ALLOWED_RANGES {"STANDARD" "QUAD"}
        }
    } else {
    ...
    # if the device support QSPI, or (18.1std) disable asmiblock, use gpio pin
    # then take IO_MODE from user.
    if {!$disable_asmiblock} {
        ...
        set_instance_parameter_value    asmi2_inst_qspi_ctrl DATA_WIDTH         $IO_MODE
        # only support STANDARD and QUAD only
        if {$IO_MODE eq "QUAD"} {
            set_parameter_value ASI_WIDTH 4
        } else { 
            set_parameter_value ASI_WIDTH 1
    ...
    if {[ get_parameter_value DDASI ] eq "1"} {
        ...
        # DDASI has higher priority, if disable then user can select either atom_port or qspi
        set_instance_parameter_value    asmi2_inst_qspi_ctrl gui_use_gpio   $gui_use_gpio
        ...
        if {$gui_use_gpio} {        
            ...
            set_interface_property atom_ports EXPORT_OF asmi2_inst_qspi_ctrl.atom_ports
        }
    } else {
        # If use asmiblock, only for A10 and C10, need to set the chip select to 3 

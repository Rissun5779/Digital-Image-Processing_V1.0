...
entity altera_pfl2_pgm_flash_sm is
	generic 
	(
...
end entity altera_pfl2_pgm_flash_sm;

architecture rtl of altera_pfl2_pgm_flash_sm is
	type state_type is (INTEL_PRE_READ_A, INTEL_PRE_READ_B, SPANSION_PRE_READ1_A, SPANSION_PRE_READ1_B, 
						SPANSION_PRE_READ2_A, SPANSION_PRE_READ2_B, SPANSION_POST_READ1, START_WRITE_PROCESS, 
						IDLE, READ_REQUEST, WRITE_ADDRESS_BUS, WRITE_DATA_BUS, WRITE_TO_FLASH, INCREASE_ADDR, 
						END_WRITE, START_CHECK_STATUS, SPANSION, INTEL, INTEL_READ_STATUS, INTEL_POST_READ, 
						SPANSION_READ_STATUS1, SPANSION_READ_STATUS2, INTEL_CLEAR_STATUS_REG, SUCCESS, 
						FAIL, FULL, END_CHECK_STATUS, WRITE_TO_FLASH_NEG, WRITE_TO_FLASH_POS);
...
begin
	...
	flash_select <= sig_flash_write or sig_flash_read or sig_flash_select;
	...
	begin
		if (sig_flash_write = '1' or present_state = WRITE_DATA_BUS or 
												present_state = WRITE_TO_FLASH_NEG or 
												present_state = WRITE_TO_FLASH or 
												present_state = WRITE_TO_FLASH_POS or 
	...
	begin
			case (present_state) is
				...
				when WRITE_DATA_BUS =>
					next_state <= WRITE_TO_FLASH_NEG;
				
				when WRITE_TO_FLASH_NEG =>
					next_state <= WRITE_TO_FLASH;
					
				when WRITE_TO_FLASH =>
					next_state <= WRITE_TO_FLASH_POS;
					
				when WRITE_TO_FLASH_POS =>

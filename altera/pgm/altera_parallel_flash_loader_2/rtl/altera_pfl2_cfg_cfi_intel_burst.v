...
module altera_pfl2_cfg_cfi_intel_burst (
	clk,
	nreset,
    ...
    wire [2:0] counter_q;
    wire access_counter_clear;
    wire access_counter_cnt_en;
    ...
    wire counter_clear;
    wire counter_cnt_en;
    wire next_state_is_addr;
    wire next_state_is_addr_low;
    wire next_state_is_addr_toggle_high;
    wire next_state_is_die;
    wire next_state_is_dummy;
    wire next_state_is_latency;
    wire next_state_is_latency_low;
    wire next_state_is_latency_toggle_high;
    wire next_state_is_rcr;
    wire next_state_is_read;
    wire next_state_is_read_low;
    wire next_state_is_read_toggle_high;
    wire next_state_is_reset;
    wire next_state_is_wait;
    wire is_addr_state;
    wire is_latency_state;
    wire is_read_state;
    ...
    reg request;
     ...
     assign data_ready = current_state_is_read_toggle_high;
	...
	// State Combination
	assign is_addr_state = current_state_is_addr_low | current_state_is_addr_toggle_high | current_state_is_addr;
	assign is_latency_state = current_state_is_latency_low | current_state_is_latency_toggle_high | current_state_is_latency;
	assign is_read_state = current_state_is_read_low | current_state_is_read_toggle_high | current_state_is_read;
								
	assign accross_die = addr_counter_q == {1'b0, {(FLASH_ADDR_WIDTH-1){1'b1}}};
	...
	assign counter_cnt_en = current_state_is_rcr ? access_counter_cycle & rcr_hold :
									current_state_is_latency ? 1'b1 :
									access_counter_out; // this is for BURST_RESET + DIE
	assign counter_clear = 	next_state_is_rcr | next_state_is_addr |
	...
	altera_pfl2_counter addr_counter
	(
		.clock(clk),
		.sload(addr_sload),
		.data(addr_in),
	...
	assign access_counter_cnt_en = current_state_is_rcr | current_state_is_reset | current_state_is_dummy | current_state_is_die;
	assign access_counter_clear = access_counter_out | next_state_is_rcr | next_state_is_reset | next_state_is_dummy | next_state_is_die ;
	...
	assign reset_state_machine = ~data_request | ~granted | ~current_state_machine_active;
	// WAIT 
	assign next_state_is_wait = current_state_is_wait? 1'b0 : reset_state_machine;
	always @ (posedge clk) begin
		current_state_is_wait = reset_state_machine;
	end
	// DIE
	assign next_state_is_die = (current_state_is_wait & addr_latched & granted & data_request) |
	...
	// RESET
	assign next_state_is_reset = current_state_is_die & access_counter_out & (counter_q == 3'd3);
	always @ (posedge clk) begin
	...
	// RCR
	assign next_state_is_rcr = current_state_is_reset & access_counter_out & (counter_q == 3'd3);
	always @ (posedge clk) begin
	...
	// DUMMY
	assign next_state_is_dummy = current_state_is_rcr & (counter_q == RCR_WORDS-1) & access_counter_cycle & rcr_hold;
	always @ (posedge clk) begin
	...
	// ADDR_LOW
	assign next_state_is_addr_low = current_state_is_dummy & access_counter_out & (FLASH_BURST_EXTRA_CYCLE != 0);
	always @ (posedge clk) begin
	...
	// ADDR_TOGGLE_HIGH
	assign next_state_is_addr_toggle_high = (current_state_is_dummy & access_counter_out & (FLASH_BURST_EXTRA_CYCLE == 0)) |
	...
	// ADDR
	assign next_state_is_addr = current_state_is_addr_toggle_high;
	always @ (posedge clk) begin
		if (reset_state_machine)
	...
	// LATENCY_LOW
	assign next_state_is_latency_low = (current_state_is_addr & (FLASH_BURST_EXTRA_CYCLE != 0)) |
	...
	// LATENCY_TOGGLE_HIGH
	assign next_state_is_latency_toggle_high = (current_state_is_addr & (FLASH_BURST_EXTRA_CYCLE == 0)) |
	...
	// LATENCY
	assign next_state_is_latency = current_state_is_latency_toggle_high;
	always @ (posedge clk) begin
		if (reset_state_machine)
	...
	// READ_LOW
	assign next_state_is_read_low = (current_state_is_latency & (counter_q == READ_LATENCY-1) & (FLASH_BURST_EXTRA_CYCLE != 0)) |
	...
	// READ_TOGGLE_HIGH
	assign next_state_is_read_toggle_high = (current_state_is_latency & (counter_q == READ_LATENCY-1) & (FLASH_BURST_EXTRA_CYCLE == 0)) |
	...
	// READ
	assign next_state_is_read = (current_state_is_read_toggle_high & ~next_state_is_die & alt_pfl_data_read);
	always @ (posedge clk) begin
		if (reset_state_machine)

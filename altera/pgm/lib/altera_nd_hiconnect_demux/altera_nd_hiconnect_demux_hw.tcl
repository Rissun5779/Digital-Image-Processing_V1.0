...
set_module_property NAME altera_nd_hiconnect_demux
set_module_property VERSION 18.1
set_module_property DISPLAY_NAME "ND Mailbox Demultiplexer"
set_module_property DESCRIPTION "Demux with one-hot channel signals"
...
proc elaborate {} {
    ...
    if { $sel_as_chan } {
        add_interface_port sink sel channel Input $st_chan_width
    }
    if { $use_ready } {
...
proc synth_callback_procedure { output_name } {
    ...
    set template_file [ file join $this_dir "altera_nd_hiconnect_demux.sv.terp" ]
    ...
    if { $sel_as_chan } {
        set params(sel_w)     [ get_parameter_value ST_CHANNEL_W ]
    } else {
        set params(sel_w)     [ get_parameter_value NUM_OUTPUTS ]
    }

...
module altera_emr_unloader (
	...
	parameter clock_cp_frequency = 50;
	parameter clock_frequency = 100;
	
	// Delay to wait for a least two core intrface cycles for Shift Register load
	localparam shiftnld_delay = ((2*clock_frequency)/clock_cp_frequency + 2);
    ...
    input [emr_reg_width-1:0] crcerror_emr_cp;
    input crcerror_cp;
	wire crcerror_wire;
	wire inject_error;
	...
	reg emr_clk;
    assign crcerror_wire = (crcerror_cp === 1'bz) ? 1'b1 : (crcerror_cp === 1'b1) ? 1'b1 : 1'b0;
    assign crcerror_pin = crcerror_cp;
	...
	begin
		...
		begin
			...
			if (emr_clk && ~inject_error)
				emr_reg[emr_reg_width-1:0]  <= crcerror_emr_cp[emr_reg_width-1:0];

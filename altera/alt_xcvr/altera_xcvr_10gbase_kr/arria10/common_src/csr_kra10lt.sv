...
`ifdef ALTERA_RESERVED_XCVR_FULL_KR_TIMERS
  ...
  localparam LT_BASE_DEFAULT  = (ES_DEVICE) ? 32'h8140_5111 : 32'he114_5111 ;     // Enable LT, step=1/1 eq=5 (8), ctle depth=00, adp=pre+post=11, manual mode
  localparam LT_CDR_TWEAKS    = (ES_DEVICE) ? 25'h00_0000 : 25'h30_0000 ;       // Enable Ipd/Rpd update 
`else // not FULL_KR_TIMERS
  localparam LT_CDR_TWEAKS    = 25'h00_0000;       // Enable Ipd/Rpd update 
  localparam LT_BASE_DEFAULT  = 32'h8148_5111;     // Enable LT, step=1/1 eq=5 (8), ctle depth=00, adp=no trig=00
...
//**************************************************************************** 
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      ...
      if (lcl_stat_sync & ~lcl_stat_dly) lcl_stat_ltch[5:0] <= lcl_txi_status[5:0];
      if (pcs_mode_rc[1])                lcl_stat_ltch[6]   <= lcl_txi_status[6];
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      ...
      ADDR_KRLT_TAPVAL: begin
        ...
        readdata[27:24] <= last_ctle_rc;
        readdata[29:28] <= last_ctle_mode;
        readdata[31:30] <= last_dfe_mode;
      end  // Tap values

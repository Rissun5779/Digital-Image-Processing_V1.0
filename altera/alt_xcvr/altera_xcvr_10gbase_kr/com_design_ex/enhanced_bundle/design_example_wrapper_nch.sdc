...
# "LOCAL_sv_rc_wrapper" is the instance name for sv_rcn_wrapper.sv in this design example.  If you 
# are changing the instance name for this when integrating into your design, set the path_project
# parameter to the new instance name you choose.
...
# input clocks
create_clock -name {pll_ref_clk[0]} -period $period_1g   [get_ports {pll_ref_clk[0]}]
...
if {$SYNTH_GIGE} {
  ...
  for {set inst 0} {$inst<$num_channels} {incr inst} {
      ...
      create_clock  -name $clock_name -period $period_1g [get_nets $clock_node] 
  }
}
...
## Following false path constrains are needed for QII - 15.0 and later 
## Clock crosser
set_false_path -from [get_registers *kr_gige_pcs_clock_crosser:*|in_data_buffer*] -to [get_registers *kr_gige_pcs_clock_crosser:*|out_data_buffer*]

## kr_gige_pcs_ph_calculator
...
##DC FIFO's false path
set_false_path -from [get_keepers {*rdptr_g*}]         -to [get_keepers {*ws_dgrp|dffpipe*:dffpipe*|dffe*}]               
set_false_path -from [get_keepers {*delayed_wrptr_g*}] -to [get_keepers {*rs_dgwp|dffpipe*:dffpipe*|dffe*}]
set_false_path -from [get_keepers {*rdptr_g*}]         -to [get_keepers {*ws_dgrp|dffpipe*:dffpipe*|dffe*}]
set_false_path -from [get_keepers {*delayed_wrptr_g*}] -to [get_keepers {*rs_dgwp|dffpipe*:dffpipe*|dffe*}]
...
## Following false path constrains are needed for QII - 15.0 and later 
set_false_path -from [get_registers *stratixv_10gbaser_1588_ppm_counter*cntr_a]           -to [get_registers *bitsync2_1588*sync_regs[*]]  
set_false_path -to   [get_registers *stratixv_10gbaser_1588_ppm_counter*sync_rst_b_n*]
set_false_path -from [get_registers *altera_10gbaser_phy_clock_crosser:*|in_data_buffer*] -to [get_registers *altera_10gbaser_phy_clock_crosser:*|out_data_buffer*]

...
module user_reconfig_access 
  #(
    ...
    parameter VER_MAJ = 16'd2,
    parameter VER_MIN = 16'd0
...
//   input 	     reconfig_rc_pending_from_arbiter, // this may be used to see if there are any pending request from the arbiter
   ...
   // continuous training CSRs
   output reg [32-1:0] ctrain_ctrl,      //0xb0
   output reg [32-1:0] ctrain_dly,       //0xb1
   input [CHANNELS-1:0] kr_done,         //0xb2
   input [32-1:0] tsd_vals,              //0xb3
   output reg [32-1:0] ctrain_setpoints, //0xb4
...
always @ (*)
  if (avmm_address[7])   // access for status
    begin
       ...
       status_avmm_address = avmm_address[6:0];
...
// checking if pcs_mode_rc bit0 or bit1 is high (for each channel)
   ...
   always @(posedge clk)
     if (reset)
	  ...
	  dfe_vref_low_reg <= 'd0;
	  dfe_ena_vref_reg <= 'd0;
	  ctrain_ctrl <= CTRAIN_CTRL_DEF;
	  ctrain_dly <= CTRAIN_DLY_DEF;
      ctrain_setpoints <= CTRAIN_SETCTLE_DEF;
       end
     ...
     else if (status_avmm_write && (status_avmm_address == 7'h30)) 
       ctrain_ctrl <= status_avmm_writedata[31:0];
     else if (status_avmm_write && (status_avmm_address == 7'h31)) 
       ctrain_dly <= status_avmm_writedata[31:0];
     else if (status_avmm_write && (status_avmm_address == 7'h34)) 
       ctrain_setpoints <= status_avmm_writedata[31:0];
     else
       begin
...
// read and waitrequest
   always @ (posedge clk)
	 ...
	 2:begin // deassert waitrequest and output readdata and wait for read low to go to state0
	   ...
	   else if (status_avmm_address =='d5)
	     status_avmm_readdata <= 31'b0 | (VER_MAJ << 16) | VER_MIN;
	   else if (status_avmm_address == 7'h30)
	     status_avmm_readdata <= ctrain_ctrl[31:0];
	   else if (status_avmm_address == 7'h31)
	     status_avmm_readdata <= ctrain_dly[31:0];
	   else if (status_avmm_address == 7'h32)
	     status_avmm_readdata <= 31'b0 | kr_done[CHANNELS-1:0];
	   else if (status_avmm_address == 7'h33)
	     status_avmm_readdata <= tsd_vals[31:0];
	   else if (status_avmm_address == 7'h34)
	     status_avmm_readdata <= ctrain_setpoints[31:0];

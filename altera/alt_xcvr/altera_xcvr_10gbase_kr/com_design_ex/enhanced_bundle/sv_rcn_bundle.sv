...
//      v1.0 3/07/2014 - Added NIOS control for temperature PI tracking
//      v1.1 3/14/2014 - Cleaned up always_ff, added EYEQ parameter, disable ADCE by default
//      v1.2 4/21/2014 - Fixed NIOS enable when non-KR lanes in use, add nios startup delay 
//      v2.0 5/16/2014 - Added CTLE temperature training and various engineering features
//      v2.1 12/31/2014 - Fix arbiter issue when channels=1
//      v2.2 04/21/2015 - Fix issue where CPU can interfere with reconfiguration into AN
...
//  parameter ONEG_TENG_MODE = 0,       // use the 1g_10g roms/mifs 
    ...
    parameter ENA_RECONFIG_CONTROLLER_CTLE_RCFG = 0,  // enable reconfig controller ADCE reconfiguration logic
    ...
    parameter ENA_RECONFIG_CONTROLLER_EYEMON_RCFG = 0,  // enable reconfig controller Eye Monitor  reconfiguration logic
    ...
    parameter CTRAIN_CTRL_DEF = 32'h000d0e88,
    parameter CTRAIN_DLY_DEF = 32'd50000,
    parameter CTRAIN_SETCTLE_DEF = 32'b0,
    parameter JTAG_UART = 0
    )
   (
    ...
    output reg [CHANNELS-1:0] kr_done // training done, ready for 10g mode
    
	 
    );
    
    localparam VER_MAJ = 16'd2;
    localparam VER_MIN = 16'd0;
...
//   wire [31:0] 		reconfig_mgmt_writedata_to_rom;
 ...
 // reconfig master busy
   ...
   wire cpu_reco_lock;
   wire [6:0]     cpu2reco_mgmt_address;
   wire           cpu2reco_mgmt_read;
   wire           cpu2reco_mgmt_write;
   wire [31:0]    cpu2reco_mgmt_writedata;
   
   wire user_request;
   
   // output the busy status of the reconfig controller
   assign reconfig_mgmt_busy = from_reconfig_mgmt_reconfig_mgmt_busy | (cpu_reco_lock & ~rc_mgmt_busy_from_reconfig_master);
...
//	.disable_ctle_dfe_before_an     (seq_pcs_mode_out == 'd1 && (disable_ctle_dfe_before_an || DISABLE_CTLE_DFE_BEFORE_AN)),
	.disable_ctle_dfe_before_an    ({(seq_pcs_mode_out == 'd1 && (disable_ctle_dfe_before_an[1] || DISABLE_CTLE_DFE_BEFORE_AN[1]) && ENA_RECONFIG_CONTROLLER_ANALOG_RCFG),
...
////////////////////////// ifdef section ///////////////////////////
   ...
   wire [31:0] ctrain_ctrl;
   wire [31:0] ctrain_dly;
   wire [31:0] tsd_vals;
   wire [31:0] ctrain_setpoints;
	...
	begin
	   ...
	   user_reconfig_access  
	     #(
	       ...
	       .CHANNELS (CHANNELS),
           .CTRAIN_CTRL_DEF(CTRAIN_CTRL_DEF),
           .CTRAIN_DLY_DEF(CTRAIN_DLY_DEF),
           .CTRAIN_SETCTLE_DEF(CTRAIN_SETCTLE_DEF),
           .VER_MAJ(VER_MAJ),
           .VER_MIN(VER_MIN)
	      ...
	      .rc_mgmt_busy_from_reconfig_master(rc_mgmt_busy_from_reconfig_master || reconfig_mgmt_busy ), //i uses this to gate grant for user access  
	      ...
	      .from_reconfig_mgmt_reconfig_mgmt_busy(reconfig_mgmt_busy),     //i

          .ctrain_ctrl(ctrain_ctrl),
          .ctrain_dly(ctrain_dly),
          .kr_done(kr_done),
          .tsd_vals(tsd_vals),
          .ctrain_setpoints(ctrain_setpoints),
...
// rewire reconfig controller busy
	   assign to_reconfig_master_reconfig_mgmt_busy = reconfig_mgmt_busy;
...
// rewire reconfig_master to arbiter
       ...
       assign user_request = 1'b0;
       
       assign ctrain_ctrl = CTRAIN_CTRL_DEF;
       assign ctrain_dly = CTRAIN_DLY_DEF;
       assign ctrain_setpoints = CTRAIN_SETCTLE_DEF;
...
// reconfig controller
//============================================================================
wire [6:0]  reconfig_mgmt_address = (cpu_reco_lock & ~rc_mgmt_busy_from_reconfig_master) ? cpu2reco_mgmt_address : to_reconfig_mgmt_address;
wire        reconfig_mgmt_read = (cpu_reco_lock & ~rc_mgmt_busy_from_reconfig_master) ? cpu2reco_mgmt_read : to_reconfig_mgmt_read;
wire        reconfig_mgmt_write = (cpu_reco_lock & ~rc_mgmt_busy_from_reconfig_master) ? cpu2reco_mgmt_write : to_reconfig_mgmt_write;
wire [31:0] reconfig_mgmt_writedata = (cpu_reco_lock & ~rc_mgmt_busy_from_reconfig_master) ? cpu2reco_mgmt_writedata : to_reconfig_mgmt_writedata;
...
alt_xcvr_reconfig #(
  ...
  .enable_eyemon                 (ENA_RECONFIG_CONTROLLER_EYEMON_RCFG),
  ...
  .reconfig_mgmt_writedata   (reconfig_mgmt_writedata),  //_to_rom
...
// nios trainer for slow adjustment of dfe 
generate
  if (KR_PHY_SYNTH_LT) begin
        
    reg [CHANNELS-1:0] ena_nios_train; // whether channel is in a state where it wants training
    reg [CHANNELS-1:0] mask_nios_train; // whether channel is in a state where it allows others to train

    // nios startup delay to ensure TSD measurement is ready
    reg [19:0] nios_rst_ctr;
    wire nios_reset = ~&nios_rst_ctr;

    always @(posedge reconfig_clk or posedge reconfig_reset) begin
       if(reconfig_reset) begin
          nios_rst_ctr <= 20'b0;
       end else begin
           if(~&nios_rst_ctr) nios_rst_ctr <= nios_rst_ctr + 1'b1;
       end
    end		

    wire [7:0] byte_to_cpu;
    wire byte_to_cpu_ack;
    wire [7:0] byte_from_cpu;
    wire byte_from_cpu_valid;
    wire from_cpu_pfull;
    wire from_jtag_pfull;
    ...
    if(JTAG_UART) begin
        jtag_bytes jterm (
            ...
            .byte_to_jtag(byte_from_cpu),
            .byte_to_jtag_valid(byte_from_cpu_valid),
            .to_jtag_pfull(from_cpu_pfull),

            .byte_from_jtag(byte_to_cpu),
            .byte_from_jtag_ack(byte_to_cpu_ack),
            .from_jtag_pfull(from_jtag_pfull)
        );
    end else begin
        assign byte_to_cpu = 8'b0;
        assign from_cpu_pfull = 1'b0;
    end

    dfe_nios_top #(
       .NUM_LN(CHANNELS),
       .LOG_NUM_LN(WIDTH_CH), // enough bits to describe 0..NUM_LN-1
       .PLL_PER_LCHAN(MAP_PLLS ? 1 : 0)
    ) nios_ctrl (
       .clk100(reconfig_clk),
       .rst100(nios_reset),
       
       // mask training state if user wants reco access, or if other channels are reconfiguring or TX training
       .ena_train(ena_nios_train & {CHANNELS{~user_request}} & {CHANNELS{&mask_nios_train}}), 
       .data_mode(ena_nios_train),
       
       // input csr
       .ctrain_ctrl(ctrain_ctrl),
       .train_dly(ctrain_dly),
       .ctrain_setpoints(ctrain_setpoints),
       
       .reco_lock(cpu_reco_lock),
       .reconfig_mgmt_address(cpu2reco_mgmt_address),
       .reconfig_mgmt_read(cpu2reco_mgmt_read),
       .reconfig_mgmt_readdata(from_reconfig_mgmt_readdata),
       .reconfig_mgmt_waitrequest(from_reconfig_mgmt_waitrequest),
       .reconfig_mgmt_write(cpu2reco_mgmt_write),
       .reconfig_mgmt_writedata(cpu2reco_mgmt_writedata),
       .reconfig_busy(from_reconfig_mgmt_reconfig_mgmt_busy),
       
       .reco_testbus(8'b0), // reco testbus unused 
       ...
       .byte_to_cpu(byte_to_cpu),
       .byte_to_cpu_ack(byte_to_cpu_ack),
       .byte_from_cpu(byte_from_cpu),
       .byte_from_cpu_valid(byte_from_cpu_valid),
       .from_cpu_pfull(from_cpu_pfull)
    );
    ...
    always @(posedge reconfig_clk or posedge reconfig_reset) begin
       if(reconfig_reset) begin
          ...
          for (i=0; i<CHANNELS; i=i+1) begin
             // only enable when going into 10G or FEC data mode
             if((seq_pcs_mode[i*6 + 2] | seq_pcs_mode[i*6 + 5]) & baser_ll_mif_done[i]) 
                 ena_nios_train[i] <= 1'b1;
             // disable when leaving
             else if(seq_start_rc[i]) 
                 ena_nios_train[i] <= 1'b0;
          end
       end
    end		
    ...
    always @(posedge reconfig_clk or posedge reconfig_reset) begin
       if(reconfig_reset) begin
          mask_nios_train <= {CHANNELS{1'b1}};
       end else begin
          for (i=0; i<CHANNELS; i=i+1) begin
             // enable when finished reconfiguring
             if(baser_ll_mif_done[i]) 
                 mask_nios_train[i] <= 1'b1;
             // disable when starting any mif streaming, or if in LT mode (AN doesn't need to mif to LT mode)
             else if(seq_start_rc[i] | seq_pcs_mode[i*6 + 1]) 
                 mask_nios_train[i] <= 1'b0;
          end
       end
    end	
    ...
    assign cpu_reco_lock = 1'b0;
    assign cpu2reco_mgmt_address = 'd0;
    assign cpu2reco_mgmt_read = 'd0;
    assign cpu2reco_mgmt_write = 'd0;
    assign cpu2reco_mgmt_writedata = 'd0;
    assign kr_done = 'd0;
    assign tsd_vals = 31'b0 | tsd_in;

...
generate
	for(ig=0;ig<NUM_RESETS;ig=ig+1) begin : g_anlg_trs_inst	
		...
		assign reset_match[ig] = (reset_out_reg[ig] == reset_sample[ig]);
		...
		// Sample the reset signal when reset_req and reset_ack are low and reset_out_reg matches the previously sampled reset value
		// i.e. When a reset change event is detected, reset signal will not be sampled until the current change is serviced by the sequencer
		...
		always @(posedge clk or negedge reset_n) begin
			if (~reset_n) begin
				...
				if ( (reset_ack[ig] && reset_timeout[ig]) || reset_match[ig]) begin									

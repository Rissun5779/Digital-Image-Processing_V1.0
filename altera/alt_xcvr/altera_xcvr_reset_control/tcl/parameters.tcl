...
namespace eval ::altera_xcvr_reset_control::parameters:: {
  ...
  set parameters {\
    ...
    {gui_tx_auto_reset      false   false         INTEGER 0             {"0:Auto" "1:Manual" "2:Expose port"}   TX_ENABLE       true    NOVAL         NOVAL         "TX Channel"      "TX digital reset mode"                   NOVAL                                                                     "In Auto mode, the associated tx_digital_reset controller automatically resets whenever the pll_locked signal is deasserted. In Manual mode, the associated tx_digital_reset controller does not reset when the pll_locked signal is deasserted. If you select \"Expose port\" - the tx_manual signal is a top-level signal of the core."}\
    {T_TX_ANALOGRESET       false   true          INTEGER 0             NOVAL                                   TX_ENABLE       true    NOVAL         ns            "TX Channel"      "tx_analogreset duration"                   ::altera_xcvr_reset_control::parameters::validate_reset_durations       "Specifies the time in ns to assert tx_analogreset after the reset input and all other reset conditions are removed. The value is rounded to the nearest clock cycle. A value of 0 indicates that no delay should be used and tx_analogreset should be driven directly from the reset input. The default value is 0 ns."}\
    {T_TX_DIGITALRESET      false   true          INTEGER 20            NOVAL                                   TX_ENABLE       true    NOVAL         ns            "TX Channel"      "tx_digitalreset duration"                  ::altera_xcvr_reset_control::parameters::validate_reset_durations       "Specifies the time in ns to assert tx_digitalreset  after the reset input and all other reset conditions are removed. The value is rounded to the nearest clock cycle."}\
    ...
    {gui_pll_cal_busy       false   false         INTEGER 0             NOVAL                                   TX_ENABLE       false   boolean       NOVAL         "TX Channel"      "Enable pll_cal_busy input port"            ::altera_xcvr_reset_control::parameters::validate_gui_pll_cal_busy      "When enabled, the Transceiver PHY Reset Controller will provide an option pll_cal_busy input port."}\
    {EN_PLL_CAL_BUSY        true    true          INTEGER 0             NOVAL                                   TX_ENABLE       false   NOVAL         NOVAL         NOVAL             NOVAL                                       ::altera_xcvr_reset_control::parameters::validate_en_pll_cal_busy       NOVAL}\
    \
    ...
    {T_RX_ANALOGRESET       false   true          INTEGER 40            NOVAL                                   RX_ENABLE       true    NOVAL         ns            "RX Channel"      "rx_analogreset duration"                   ::altera_xcvr_reset_control::parameters::validate_reset_durations       "Specifies the time in ns to assert rx_analogreset after the reset input and all other reset conditions are removed. The value is rounded to the nearest clock cycle. The default value is 40 ns."}\
...
proc ::altera_xcvr_reset_control::parameters::validate_en_pll_cal_busy { gui_pll_cal_busy device_family } {
...
proc ::altera_xcvr_reset_control::parameters::validate_gui_pll_cal_busy { device_family } {
  set visible true
  if {[::alt_xcvr::utils::device::has_s5_style_hssi   $device_family] || \
      [::alt_xcvr::utils::device::has_a5gz_style_hssi $device_family] || \
      [::alt_xcvr::utils::device::has_a5_style_hssi   $device_family] || \
...
proc ::altera_xcvr_reset_control::parameters::validate_reset_durations { PROP_NAME PROP_VALUE TX_ENABLE RX_ENABLE device_family } {
  set enabled $RX_ENABLE
  if {$PROP_NAME == "T_TX_ANALOGRESET" || $PROP_NAME == "T_TX_DIGITALRESET"} {
    set enabled $TX_ENABLE
  }

  if {$device_family == "Arria 10" && $enabled} {
    if { $PROP_VALUE < 70000 } {
      invalid_value_message warning $PROP_NAME $PROP_VALUE ">=70000" device_family
    }
  }
}

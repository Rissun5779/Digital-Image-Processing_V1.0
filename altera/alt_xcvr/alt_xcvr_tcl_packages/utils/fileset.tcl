...
# For SDC/IPRI in 16.1. 
# remove in 17.0 when all IP is opted in to the new
# assignment scheme.
...
proc alt_xcvr::utils::fileset::common_fileset_group {destDir srcDir fileType fileList tags toolFlowTags} {
	...
	# IPRI SDC project opt in check.  If the filetype is
	# SDC we'll convert it to SDC_ENTITY if and only if
	# (in 16.1) the HSSI IP is opted in for early access
	# and the user has explicitely enabled IPRI/SDC Early
	# Access functionality using a quartus.ini variable or
	# an environment variable.

	if {[string equal $fileType "SDC"] } {
		set fileType [ipri_extension_filter $fileType "HSSI"];
	}
      ...
      #puts "File: $srcPath : ${srcTags}"
		  ...
		  if {$tagMatch} {
		  	...
		  	foreach toolTag $matchingTools {
               ...
               #
               # Catch SDC_ENTITY "NO Promotion" files and convert
               # them to SDC_ENTITY files with the NO_AUTO_INSTANCE_DISCOVERY
               # and NO_SDC_PROMOTION arguments.  See Case 397203 for
               # history
               #

               if {[string equal $fileType "SDC_ENTITY_NOPROMO"]} {
		  			   add_fileset_file $destPath "SDC_ENTITY" PATH $srcPath {NO_AUTO_INSTANCE_DISCOVERY NO_SDC_PROMOTION}
               } else {

...
package provide alt_xcvr::ip_tcl::messages 18.1
...
namespace eval ::alt_xcvr::ip_tcl::messages:: {
  namespace export \
    ...
    set_auto_resolve \
    set_guided_mode \
    set_legal_values_metadata_tags \
    set_message_filter_criteria \
    set_mapping_enabled \
    set_trace_antecedentsAtErrorMessage_to_userSettableParameters_enable \
    set_deferred_messaging \
    get_invalid_current_value_string \
    get_invalid_antecedents_string \
    invalid_value_message \
    auto_message \
    auto_legal_value_warning_message \
    auto_legal_range_warning_message \
  ...
  variable l_mapping
  variable l_trace_antecedentsAtErrorMessage_to_userSettableParameters
  variable l_message_filter_criteria
  variable l_deferred_messaging
  variable l_deferred_message_list
  variable l_auto_resolve
  variable l_guided_mode
  variable l_legal_values_tag
  variable l_is_legal_values_range_tag

  # Initialize internal variables
  set package_name "alt_xcvr::ip_tcl::messages"
  set l_auto_message_level "error"
  set l_mapping 0
  set l_trace_antecedentsAtErrorMessage_to_userSettableParameters 0
  set l_message_filter_criteria {ENABLED 1 DERIVED 0}
  set l_deferred_messaging 0
  set l_deferred_message_list {}
  set l_auto_resolve 0
  set l_guided_mode 0
  set l_legal_values_tag "M_LEGAL_VALUES"
  set l_is_legal_values_range_tag "M_IS_LEGAL_VALUES_RANGE"
}

proc ::alt_xcvr::ip_tcl::messages::ip_invalid_param_message { level message_string param_name } {
    ip_message $level $message_string 	
    ::alt_xcvr::ip_tcl::ip_module::ip_log_invalid_parameter $param_name
}
...
##
# Enables auto resolution of parameters. Meaning that when an invalid value is detected,
# instead of issuing a message, a valid value will be selected
proc ::alt_xcvr::ip_tcl::messages::set_auto_resolve { auto_resolve } {
  variable l_auto_resolve
  set l_auto_resolve $auto_resolve
}

###
# During auto resolution of parameters, enables tagging of legal_values
proc ::alt_xcvr::ip_tcl::messages::set_guided_mode { guided_mode } {
  variable l_guided_mode
  set l_guided_mode $guided_mode
}

###
# During auto resolution of parameters, the information about legal values for the parameters are stored in metadata fields tagged by this function
proc ::alt_xcvr::ip_tcl::messages::set_legal_values_metadata_tags {legal_values_tag is_legal_values_range_tag} {
  variable l_legal_values_tag
  variable l_is_legal_values_range_tag

  set l_legal_values_tag $legal_values_tag
  set l_is_legal_values_range_tag $is_legal_values_range_tag
}
...
proc ::alt_xcvr::ip_tcl::messages::set_trace_antecedentsAtErrorMessage_to_userSettableParameters_enable { enabled } {
  variable package_name
  variable l_trace_antecedentsAtErrorMessage_to_userSettableParameters

  if {$enabled == "0" || $enabled == "1"} {
    set l_trace_antecedentsAtErrorMessage_to_userSettableParameters $enabled
  } else {
    internal_error "\[::${package_name}::set_trace_antecedentsAtErrorMessage_to_userSettableParameters_enable\] Illegal argument value: enabled=${enabled}"
  }
}
...
proc ::alt_xcvr::ip_tcl::messages::get_invalid_current_value_string { param_name param_value } {
  set message "The current value [get_value_display_string $param_name $param_value] for parameter [get_parameter_display_string $param_name] is invalid."
}


proc ::alt_xcvr::ip_tcl::messages::get_parameter_display_string { param_name } {
  return "<b>\"[::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${param_name}.display_name"]\"</b> (${param_name})"
}
...
proc ::alt_xcvr::ip_tcl::messages::get_mapped_display_hint_string { param_name param_value } {
  ...
  } elseif {[::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${param_name}.type"] == "STRING" } {
...
proc ::alt_xcvr::ip_tcl::messages::auto_message { level param_name message } {
	if { $level == "error" } {
		::alt_xcvr::ip_tcl::ip_module::ip_log_invalid_parameter $param_name
	} else {
		::alt_xcvr::ip_tcl::ip_module::ip_log_parameter $param_name
	}	
	ip_message $level $message
}

proc ::alt_xcvr::ip_tcl::messages::auto_legal_value_warning_message { param_name legal_values } {
	::alt_xcvr::ip_tcl::ip_module::ip_log_parameter $param_name
	ip_message warning "The value of parameter $param_name cannot be automatically resolved. Valid values are: ${legal_values}."
}

proc ::alt_xcvr::ip_tcl::messages::auto_legal_range_warning_message { param_name legal_values } {
	::alt_xcvr::ip_tcl::ip_module::ip_log_parameter $param_name
	ip_message warning "The value of parameter $param_name cannot be automatically resolved. Valid value ranges are: ${legal_values}."
}
...
proc ::alt_xcvr::ip_tcl::messages::auto_invalid_value_message_internal { message_level param_name param_value param_legal_values {antecedents {} } } {
  ...
  set cached_param_name $param_name
  ...
  if { $l_auto_resolve && [llength $param_legal_values] } {
    puts "Auto-Resolving ${param_name}"
    ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${param_name}.value" [lindex $param_legal_values 0]
    return
  }
  ...
  #trace antecedents to user settable parameters
  #making this optional so that we dont take the chance of breaking other IPs - this is currently tested only for A10 Native Phy and it was tested only when l_mapping was set to true
  set m_trace_user_param [string toupper [::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${cached_param_name}.M_TRACE_USER_PARAM"]]
  set user_settable_antecedents {}
  if {$l_trace_antecedentsAtErrorMessage_to_userSettableParameters && $l_mapping && $m_trace_user_param != "FALSE"} {
    #if there is a complex mapping for the original parameter it needs to be part of antecedents 	
    if {[::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${cached_param_name}.M_MAPS_FROM"] != "NOVAL" && [::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${cached_param_name}.M_MAP_CALLBACK"] != "NOVAL" } {
		set antecedents [linsert $antecedents end $cached_param_name]
    }

    foreach ant $antecedents {
      # every antecedent can be traced to a list of user settable GUI parameters  - this will return only non-derived parameters
      set user_settable_antecedents [concat $user_settable_antecedents [::alt_xcvr::ip_tcl::ip_module::ip_get_userSettableParameters $ant]]
    }
    
    # it is possible to have repetitions in this list - get rid of them
    set user_settable_antecedents [lsort -unique $user_settable_antecedents]
    
    # filter out parameters that we dont want user to see (either they are not useful or user can do anything about those)
    # also it is possible to have an ancedent to be same as the param_name if complex (multi) mapping is involved. we would not want to list that as an antecedent 
    set antecedents_temp {}    
    foreach ant $user_settable_antecedents {
      if {$ant != $param_name} {
        if {[::alt_xcvr::ip_tcl::ip_module::ip_get "parameter.${ant}.M_DONT_DISPLAY_IN_ERR_MSGS"] != "true"} {
          set antecedents_temp [linsert $antecedents_temp end $ant]
        }
      }
    }   
    set antecedents $antecedents_temp
  } 

  # Filter antecedents
  # We only display antecedents that the user can control (parameter is enabled)
  # [IS: if l_trace_antecedentsAtErrorMessage_to_userSettableParameters is set to true
  #      then checking for .derived is redundant. As all antecedents are already replaced with non-derived versions
  #      but checking for .enabled is not redundant as "ip_get_userSettableParameters" dont do that. and we dont want to show messages to user for parameters that are grayed out in GUI.
  # Not making this check part of "ip_get_userSettableParameters" will make it easier to print debug messages for internal use - just in case we set some parameters to disable wrongfully ]  
...
proc ::alt_xcvr::ip_tcl::messages::auto_invalid_value_message { message_level param_name param_value param_legal_values {antecedents {} } } {
  ...
  # in guided mode store the legal values to be used by the IP
  # l_is_legal_values_range_tag marking informs the consumer on what to expect regarding the structure of "legal values list"
  if {$l_guided_mode} {
    ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${param_name}.${l_legal_values_tag}" $param_legal_values
    ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${param_name}.${l_is_legal_values_range_tag}" 0
  }
...
proc ::alt_xcvr::ip_tcl::messages::auto_value_out_of_range_message { message_level param_name param_value param_legal_values {antecedents {} } } {
  ...
  if {$l_guided_mode} {
    ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${param_name}.${l_legal_values_tag}" $param_legal_values
    ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${param_name}.${l_is_legal_values_range_tag}" 1
  }
  ...
  dict for {param rules} $l_deferred_message_list {
    ...
    dict for {rule values} $rules {
      ...
      if {$index == 0} {
        ...
        # before intersecting valid values from different rules, determine if the valid values contains a range or not.
        # the function to be used to intersect the rules will be based on that
        # 1) concatenate the lists, count the colons
        set temp [concat $param_legal_values_list $legal_values]
        set numColons [expr {[string length $temp] - [string length [regsub -all ":" $temp ""]]}]

        if { $numColons==0 } {
          #if there is not any range defined in the legal values use this function
          set param_legal_values_list [::alt_xcvr::utils::common::intersect $param_legal_values_list $legal_values]
        } else {
          # there is range defined in the lagel values
          # but first test if all are numerical 
          # to do that get rid of the colons and test item by item if all are numerical
          # if one is not mark as non-numerical and break the loop
          set temp [regsub -all ":" $temp " " ]
          set are_all_values_numerical 1
          foreach value $temp {
            if { !([string is double $value]) } {
              set are_all_values_numerical 0
              break
            }
          }

          if { $are_all_values_numerical } {
            # valid values includes ranges and all values are numerical
            set param_legal_values_list [::alt_xcvr::utils::common::compare_inside $param_legal_values_list $legal_values]
          } else {
            # non-numerical values mixed with range this is not expected
            internal_error "::alt_xcvr::ip_tcl::messages::issue_deferred_messages unexpected range structure"
          }
        }
      }

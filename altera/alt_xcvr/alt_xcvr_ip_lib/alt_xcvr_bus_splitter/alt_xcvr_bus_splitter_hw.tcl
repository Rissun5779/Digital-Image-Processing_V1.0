...
set module {\
  ...
  {alt_xcvr_bus_splitter  18.1  true      alt_xcvr_bus_splitter_val_callback alt_xcvr_bus_splitter_elab_callback "Bus Splitter"      "Interface Protocols/Transceiver PHY"  "Altera Corporation"}\
}

# Declare the fileset using a nested list. First list is a header row.
set filesets {\
  {NAME            TYPE            CALLBACK               TOP_LEVEL }\
  {quartus_synth   QUARTUS_SYNTH   callback_quartus_synth alt_xcvr_bus_splitter_top }\
  {sim_verilog     SIM_VERILOG     callback_sim_verilog   alt_xcvr_bus_splitter_top }\
  {sim_vhdl        SIM_VHDL        callback_sim_vhdl      alt_xcvr_bus_splitter_top }\
}
...
set parameters {\
  ...
  {bus_port_width       INTEGER  false   true          4             NOVAL                                                                  true    true     "Port Width"                  NOVAL           NOVAL                        "all-ports - what to do about clock reset etc and split ports"}\
  \
  ...
  \
  {num_bus_splits       integer  false   false         2             NOVAL                                                                  true    true     "Number of output buses"      NOVAL           validate_num_bus_splits     "If the input interface must be broadcast to one or more output interfaces, this field specifies the number of broadcast output interfaces." }\
  {bus_split_width      integer  true    false         1             NOVAL                                                                  true    true     "Width of the split bus"      NOVAL           validate_bus_split_width    "Specifies the width of the adapter input interface"   }\
  \
...
##############################################################################################
proc alt_xcvr_bus_splitter_val_callback {} {  
  ip_validate_parameters
}

proc validate_num_bus_splits { num_bus_splits bus_port_width } {
  if { [expr $bus_port_width % $num_bus_splits] > 0 } {
    ip_message error "Must divide evenly"
  }
}

proc validate_bus_split_width { bus_port_width num_bus_splits } {
  ip_set "parameter.bus_split_width.value" [expr $bus_port_width/$num_bus_splits]
}
...
proc alt_xcvr_bus_splitter_elab_callback {} {
  add_interface       port_i          [ip_get "parameter.i_iface_type.value"] [ip_get "parameter.i_iface_direction.value"] 
  add_interface_port  port_i port_i   [ip_get "parameter.i_role.value"] input [ip_get "parameter.bus_port_width.value"]
  ...
  for {set output_index 0} {$output_index < [ip_get "parameter.num_bus_splits.value"]} {incr output_index} {
    add_interface       "port_o${output_index}"   [ip_get "parameter.o_iface_type.value"]  [ip_get "parameter.o_iface_direction.value"] 
    add_interface_port  "port_o${output_index}"   "port_o${output_index}"                  [ip_get "parameter.o_role.value"] output [ip_get "parameter.bus_split_width.value"]
    ...
    set f_list ""
    set upper_index [expr {[ip_get "parameter.bus_split_width.value"]*(${output_index}+1)-1}]
...
proc callback_quartus_synth { entity } {
  add_fileset_file ./alt_xcvr_bus_splitter_top.v VERILOG PATH ./alt_xcvr_bus_splitter_top.v
}

...
//-------------------------------------------------------------------
// Filename    : alt_xcvr_data_pattern_gen.sv
//
// Description : The Data Pattern Generator module that actually generates the output pattern. For further 
...
//
//-------------------------------------------------------------------
module alt_xcvr_data_pattern_gen #(
...
if(AVMM_EN) begin
  ...
  assign prbs_seed_mux_out         = (register_control_or_avmm[3])  ?  prbs_seed_avmm         :  prbs_seed;
  assign ext_data_pattern_mux_out  = (register_control_or_avmm[4])  ?  ext_data_pattern_avmm  :  ext_data_pattern;
  assign insert_error_mux_out      = (register_control_or_avmm[5])  ?  insert_error_avmm      :  insert_error;
end else begin
  assign reset_mux_out             =                                                             reset;
  assign enable_mux_out            =                                                             enable;
  assign pattern_mux_out           =                                                             pattern;
  assign prbs_seed_mux_out         =                                                             prbs_seed;
  assign ext_data_pattern_mux_out  =                                                             ext_data_pattern;
  assign insert_error_mux_out      =                                                             insert_error;
end
endgenerate
...
//synchronizer for reset signal	   
	alt_xcvr_resync #(
	 ...
	 alt_xcvr_data_pattern_gen_main #(
     ...
     .PRBS15               (PRBS15),
	  ...
	  .EXTERNAL             (EXTERNAL)
	) alt_xcvr_data_pattern_gen_main_inst
	(
	  .clk                  (clk),
	  .reset                (reset_synch_out),
	  .enable               (enable_synch_out),
	  .prbs_seed            (prbs_seed_mux_out),
	  .ext_data_pattern     (ext_data_pattern_mux_out),
	  .pattern              (pattern_mux_out),
	  .insert_error         (insert_error_mux_out),
	  
	  .data_out             (data_out)
	);


	 alt_xcvr_data_pattern_gen_avmm_csr #(
	  .DATA_WIDTH                 (DATA_WIDTH),
	  .PATTERN_ADDR_WIDTH         (PATTERN_ADDR_WIDTH)
	) alt_xcvr_data_pattern_gen_avmm_csr_inst
	(
	  // avmm signals
	  .avmm_clk                   (avmm_clk),
	  ...
	  .prbs_seed_avmm             (prbs_seed_avmm),
	  .ext_data_pattern_avmm      (ext_data_pattern_avmm),
	  .insert_error_avmm          (insert_error_avmm),

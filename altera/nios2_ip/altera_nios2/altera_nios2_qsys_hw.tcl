...
set_module_property DISPLAY_NAME "Nios II (Classic) Processor"
...
set_module_property SUPPORTED_DEVICE_FAMILIES {CYCLONEIVE CYCLONEIVGX CYCLONEV ARRIAIIGX ARRIAIIGZ ARRIAV ARRIAVGZ ARRIA10 STRATIXIV STRATIXV MAX10}
...
proc_set_display_group      setting_removeRAMinit                       $ADVANCED_5       1   "Remove RAM Initialization"                 "INTERNAL"
...
proc sub_elaborate_reset_req {} {
    ...
    set oci_ram_present [ expr { "${local_debug_level}" != "NoDebug" && !$setting_export_large_RAMs } ]
...
proc sub_validate_check_module {} {
    ...
    if { "$impl" == "Fast" } {
        ...
        if { "$dcache_omitDataMaster" == "1" } {
            ...
            if { $ecc_present } {
                ...
                if { $dcache_size_gui_value > 0 & !$setting_dc_ecc_present } {
                    send_message error "Data Cache does not support ECC. Please turn off Dcache by setting it to None or use Nios II Processor."
                }
        ...
        # ECC present and tightly coupled memories only for F-core
        set setting_itcm_ecc_present [ proc_get_boolean_parameter setting_itcm_ecc_present ]
        set setting_dtcm_ecc_present [ proc_get_boolean_parameter setting_dtcm_ecc_present ]
        set itcm_num [get_parameter_value icache_numTCIM]
        set dtcm_num [get_parameter_value dcache_numTCDM]
        
        if { $ecc_present } {
            # When itcm or dtcm > 0, send out error message
            # But allow overrider just in case when the itcm/dtcm_ecc_present is set
            if { $dtcm_num > 0 & !$setting_dtcm_ecc_present } {
                send_message error "Tightly Coupled Data Master (TCDM) does not support ECC. Please turn off TCDM by setting it to None or use Nios II Processor."
            }
            
            if { $itcm_num > 0 & !$setting_itcm_ecc_present } {
                send_message error "Tightly Coupled Instruction Master (TCIM) does not support ECC. Please turn off TCIM by setting it to None or use Nios II Processor."
            }
        }
...
proc validate {} {
    ...
    # Add Error For Max 10 FPGA
    set device_family                [ get_parameter_value deviceFamilyName ]

    if {[string match "MAX 10 FPGA" $device_family]} {
        send_message error "Nios II (Classic) Processor is not supported in Max 10 FPGA. Please use Nios II Processor"
    }

    send_message warning "Nios II Classic cores are no longer recommended for new projects"
...
proc proc_get_project_info_string {} {
    ...
    append return_string "\tproject_info => {\n"
        ...
        append return_string "\t\tis_hardcopy_compatible => [ proc_bool2int [ get_parameter_value setting_removeRAMinit ] ],\n"
...
add_documentation_link "User Guide" https://documentation.altera.com/#/link/ig1409256728501/iga1409257893438

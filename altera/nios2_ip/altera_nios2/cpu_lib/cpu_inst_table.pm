...
@EXPORT = qw(
    ...
    &get_inst_table_inst_code_by_name
    &get_inst_table_parent_table
    &get_inst_table_v_decode_func
    &get_inst_table_c_decode_func
...
create_inst_table
{
    ...
    validate_hash_keys("inst_table", $props, 
      ["name","inst_field","inst_type","opcodes","parent_table","v_decode_func","c_decode_func"]) ||
        return undef;
    ...
    my $v_decode_func = optional_code($props, "v_decode_func", \$err);
    if ($err) { return undef; }

    my $c_decode_func = optional_code($props, "c_decode_func", \$err);
    if ($err) { return undef; }



    my $num_opcode_entries = scalar(@$opcodes);
    if ($num_opcode_entries > 0) {
    ...
    for (my $i=0; $i < $num_opcode_entries; $i++) {
        ...
        my $opcode_type = get_inst_table_opcode_type($opcode, $inst_table) || return undef;
    ...
    for (my $i=0; $i < $num_opcode_entries; $i++) {
        for (my $j=$i+1; $j < $num_opcode_entries; $j++) {
            ...
            if ($i_opcode eq $j_opcode) {
                ...
                my $i_opcode_type = get_inst_table_opcode_type($i_opcode, $inst_table) ||
                    return undef;
                my $j_opcode_type = get_inst_table_opcode_type($j_opcode, $inst_table) ||
                    return undef;
...
get_inst_table_child_table_infos
{
    ...
    for (my $code=0; $code < scalar(@$opcodes); $code++) {
        ...
        my $child_table = get_inst_table_by_name($inst_tables, $opcode) || return undef;
...
get_inst_table_inst_code_by_name
{
    ...
    return &$error("Instruction or child table '$name' not found in table '$table_name'.");
}
...
get_inst_table_v_decode_func
{
    ...
    return $inst_table->{v_decode_func};
}
...
get_inst_table_c_decode_func
{
    ...
    return $inst_table->{c_decode_func};
}
...
convert_inst_table_to_c
{
    ...
    for (my $code=0; $code < $num_opcodes; $code++) {
        ...
        if ($h_name ne "") {
            push(@$h_lines, format_c_macro("${prefix_uc}${table_name_uc}_" . $h_name, $code));
        }
    }
    push(@$c_lines, "};");

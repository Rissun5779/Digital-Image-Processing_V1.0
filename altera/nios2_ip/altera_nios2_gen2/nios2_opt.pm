    ...
    my $nios2_infos = {
      project_info => {
        ...
        module_lib_directory              => 
          $ENV{"QUARTUS_ROOTDIR"} . "/../ip/altera/nios2_ip/altera_nios2_gen2",
        asic_add_scan_mode_input         =>  0,
      },
      misc_info => {

        cpu_arch_rev                      => 1,         # Patched by test_get_default_infos()
        core_type                         => "fast",
        cpuid_value                       => 0,
        cpu_reset                         => 0,
        register_file_ram_type            => "AUTO",
        bmx_present                       => 0,         # Patched by test_get_default_infos()
        cdx_present                       => 0,
        mpx_present                       => 0,
        register_file_por                 => 0,
      },
      ...
      multiply_info => {
        ...
        hardware_multiply_impl            => "mul_fast32",
      },
      vector_info => {
        reset_addr                        => 0x40000,
      ...
      dcache_info => {
        ...
        ioregion_dcache                    => 0,
        ioregion_base_dcache               => 4096,
        ioregion_size_dcache               => 4096,
      },
    };
...
test_get_default_infos
{
    ...
    my $infos = get_default_infos();


    $infos->{misc_info}{cpu_arch_rev} = $cpu_arch_rev;


    if ($cpu_arch_rev == 2) {
        $infos->{misc_info}{bmx_present} = 1;
    }
...
test_get_default_tiny_infos
{
    ...
    my $infos = test_get_default_infos($cpu_arch_rev);


    $infos->{misc_info}{core_type} = "tiny";
    $infos->{multiply_info}{hardware_multiply_present} = 0;
    $infos->{exception_info}{illegal_mem_exc} = 0;
    $infos->{exception_info}{extra_exc_info} = 0;
    $infos->{icache_info}{cache_has_icache} = 0;
    $infos->{dcache_info}{cache_has_dcache} = 0;
    $infos->{divide_info}{hardware_divide_present} = 0;
    delete($infos->{avalon_master_info}{avalon_masters}{tightly_coupled_data_master_0});
    delete($infos->{avalon_master_info}{avalon_masters}{tightly_coupled_instruction_master_0});
...
elaborate_infos
{
    ...
    my $fa_info = manditory_hash($infos, "fa_info") || return undef;
    ...
    if ($mmu_info->{mmu_present}) {
        ...
        my $r1 = (manditory_int($misc_info, "cpu_arch_rev") == 1);
        my $oci_present = manditory_bool($debug_info, "include_oci");
        my $allow_break_inst = manditory_bool($test_info, "allow_break_inst");
        my $oci_version1 = (manditory_int($debug_info, "oci_version") == 1);

        nios2_mmu::convert_vectors_to_kernel_region($vector_info,$r1,$oci_present,$allow_break_inst,$oci_version1);
    }
    ...
    $elaborated_infos->{exception_info} =
      elaborate_advanced_exc_info($misc_info);
...
{
    my $misc_info = shift;
    my $core_type = not_empty_scalar($misc_info, "core_type");

    my $ret = {};   # The return value



    $ret->{advanced_exc} = ($core_type ne "tiny");

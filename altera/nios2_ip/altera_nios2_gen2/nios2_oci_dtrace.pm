...
sub make_nios2_oci_dtrace
{
  ...
  $module->add_contents (
    ...
    e_signal->news (
      ["record_load_addr",    1, 0, 1], 
      ["record_store_addr",   1, 0, 1], 
      ["record_load_data",    1, 0, 1], 
      ["record_store_data",   1, 0, 1],
    ),
  ...
  if ($Opt->{oci_data_trace}) {
    ...
    e_assign->news (
      ["dummy_tie_off" => "cpu_d_wait|cpu_d_read|cpu_d_write"],
    ),
  );
  }
  return $module;

...
gen_instruction_cache
{
    ...
    if ($mmu_present && ($ic_tag_field_lsb > $mmu_addr_pfn_word_lsb)) {
    ...
    if ($ic_ecc_present) {
       ...
       e_assign->adds(
          [["D_ic_ecc_err", 1], "W_config_reg_eccen & (D_ic_tag_ecc_err | D_ic_data_ecc_err) & ~D_kill"], 
       );
       ...
       e_register->adds(
         ...
         {out => ["D_ic_data_ecc_err", 1],   in => "ic_data_ecc_err & F_sel_instruction_master & F_ic_valid",
          enable => "D_en"},
         {out => ["E_ic_ecc_err", 1,],    in => "D_ic_ecc_err",
          enable => "E_en"},
       ...
       $ecc_error = "| (M_ic_ecc_err & M_valid_ignoring_refetch & ~ic_fill_active)";
       $ecc_error_prevent_fill = "| D_ic_ecc_err | (M_ic_ecc_err & M_valid_ignoring_refetch & ~ic_fill_active)";

       push(@ecc_ic_tag_wraddress, "M_ic_ecc_err & M_valid_ignoring_refetch & ~ic_fill_active" => "M_pc[$ic_line_field_msb:$ic_line_field_lsb]");
    ...
    e_assign->adds(
      ...
      [["ic_data_wren", 1], "i_readdatavalid_d1"],
    );
    ...
    my $oci_version = not_empty_scalar($Opt, "oci_version");
    if (defined(optional($Opt, "break_addr")) && ($oci_version == 1)) {
          ...
          [["F_ic_hit", 1], 
           $fa_present ?
            "${match_mux_paddr}_got_pfn &
              ((F_ic_valid & (F_ic_desired_tag == F_ic_tag_field) & F_sel_instruction_master) | 
    ...
    if ($fa_present) {
        ...
        e_register->adds(
          ...
          {out => ["D_sel_instruction_master", 1], 
           in => "F_sel_instruction_master", 
           enable => "D_en"},
        );
    }
    
    my $want_fill_qualifier = $fa_present ? "& D_sel_instruction_master" : "";

    e_assign->adds(



      [["D_ic_want_fill_unfiltered", 1], 
        $mmu_present ? "D_refetch & D_pc_phy_pfn_valid $want_fill_qualifier" : "D_refetch $want_fill_qualifier"],
      ...
      [["ic_fill_prevent_refill_nxt", 1], 
        "D_ic_fill_starting | (ic_fill_prevent_refill & ~($cache_operation $ecc_error_prevent_fill))"],

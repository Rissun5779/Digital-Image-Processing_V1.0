...
create_altecc_decoder_instance
{
    ...
    push(@things,
      ...
      e_assign->adds(
          ...
          [["expected_msb_parity",1],"encoder_out[${code_msb}]"],
          [["calculated_parity",$ecc_bits-1],"encoder_out[${code_msb}-1:${code_lsb}]"],
      ),
    );
    ...
    for (my $count=1; $count < $this->codeword_width() ; $count++ ) {
        ...
        if ( $count == $count_powerof2) {
            if ($this->correct_parity()) {
            $result_shifted = 2 ** $parity_shifted;
            $hex_result = sprintf("%x", $result_shifted);
            push(@case_contents,
                 "${ecc_msb}'b${binarynum}" => ["corrected_bits" => "64'h$hex_result"],
            );
            }
            $parity_shifted++;
            $powerof2++;
        } else {
            $result_shifted = 2 ** $data_bit_shifted;
            $hex_result = sprintf("%x", $result_shifted);
            push(@case_contents,
                 "${ecc_msb}'b${binarynum}" => ["corrected_bits" => "64'h$hex_result"],
            );

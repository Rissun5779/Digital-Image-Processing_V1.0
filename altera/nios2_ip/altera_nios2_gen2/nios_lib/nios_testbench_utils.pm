...
create_rtl_trace_and_testend
{
    ...
    my $language = optional_scalar($args, "language");
...
create_rtl_trace_ctrl_entry
{
    ...
    foreach my $control_reg (@$control_regs) {
        ...
        e_assign_is_x->adds(
          { lhs => $prev_signal_is_x, rhs => $prev_signal, tag => "simulation", },
        );
...
create_trace_flushing
{
    ...
    e_register->adds(
      {out => ["trace_flush_counter", 7],
       in => "trace_flush_trigger ? 0 :
              trace_flush_counter + 1",
       enable => "1'b1"},
    );
    

    e_assign->add([["trace_flush_trigger", 1], "trace_flush_counter == 100"]);

    my @trace_handle = "trace_handle";

    return

      e_if->new({ condition => "trace_flush_trigger", then => [ 
          e_sim_cmd->new({ 

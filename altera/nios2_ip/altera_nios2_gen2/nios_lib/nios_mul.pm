...
initialize_config_constants
{
    ...
    if (manditory_bool($misc_info, "use_designware")) {
        ...
        $hw_mul_uses_embedded_mults = ($impl eq "mul_fast32") || ($impl eq "mul_fast64");
        $hw_mul_uses_dsp_block = ($impl eq "mul_fast64_dsp");
        $hw_mul_uses_les = ($impl eq "mul_slow32");
    }


    $hw_mul_width_b = $hw_mul_uses_les ?  4 : 32;
}
...
gen_mul
{
    ...
    if ($hw_mul_uses_dsp_block) {
        ...
        gen_pipelined_multipler_logic($Opt);
        gen_pipelined_multipler_cells($Opt);
    } elsif ( $hw_mul_uses_les ) {
...
gen_pipelined_mul_logic
{
    ...
    my $bmx_present = manditory_bool($Opt, "bmx_present");
    ...
    e_assign->adds(
      [["E_rot_n", 5], $bmx_present? "(E_ctrl_bmx ? (~{5{E_op_merge}} & E_iw_bmx_lsb) : E_src2[4:0])" : "E_src2[4:0]"],
      );
    
    e_assign->adds(
      [["E_shift_rot_col_zero", 1], "E_rot_n[$col_msb:$col_lsb] == 0"],
      );
    ...
    e_mux->add({
      ...
      selecto  => 
        "{E_ctrl_shift_rot_right, E_rot_n[$col_msb:$col_lsb]}",
      table => [
        "4'h0" => "8'h01",
        "4'h1" => "8'h02",
        "4'h2" => "8'h04",
    ...
    e_mux->add({
      ...
      selecto  => 
        "{E_ctrl_shift_rot_right, E_rot_n[$row_msb:$row_lsb],
...
gen_pipelined_multipler_logic
{
    ...
    my @mul_logic_signals = (
        { divider => "mul" },
        { radix => "x", signal => "E_src1" },
        { radix => "x", signal => "E_src2" },
        { radix => "x", signal => "E_ctrl_mul_lsw" },
        { radix => "x", signal => "M_mul_cell_p1" },
        { radix => "x", signal => "M_mul_cell_p2" },
        { radix => "x", signal => "M_mul_cell_p3" },
        { radix => "x", signal => "M_mul_s1" },
        { radix => "x", signal => "A_mul_cell_p3" },
        { radix => "x", signal => "A_mul_s1" },
        { radix => "x", signal => "A_mul_s2" },
        { radix => "x", signal => "A_mul_result" },
    );

    e_assign->adds(
      [["M_mul_s1", 17], "M_mul_cell_p2[15:0] + M_mul_cell_p1[31:16]"],
      [["A_mul_s2", 18], "A_mul_cell_p3[15:0] + A_mul_s1"],
      [["A_mul_result", 32], "{A_mul_s2[15:0], A_mul_cell_p1[15:0]}"],
    );

    e_register->adds(
      {out => ["A_mul_cell_p1", 16], 
       in =>  "M_mul_cell_p1[15:0]",            enable => "A_en"},
      {out => ["A_mul_cell_p3", 32], 
       in =>  "M_mul_cell_p3",                  enable => "A_en"},
      {out => ["A_mul_s1", 17], 
       in =>  "M_mul_s1",                       enable => "A_en"},
    );

    if (!$hw_mul_omits_msw) {
        e_register->adds(
            {out => ["A_mul_stall",1],
             in => "M_ctrl_mulx & M_valid & A_en", enable => "1'b1"},
        );

        e_assign->adds(
          [["A_mul_s3_nxt", 32], "{{16{(M_mul_cell_p3[31] & M_ctrl_mul_src1_signed)}},M_mul_cell_p3[31 : 16]} + {{16{(M_mul_cell_p2[31] & M_ctrl_mul_src2_signed)}},M_mul_cell_p2[31 : 16]}"],
          [["A_mul_s4_nxt", 32], "A_mul_cell_p4 + A_mul_s3"],
          [["A_mul_s5", 32], "A_mul_s4 + A_mul_s2_co"],
          [["A_mul_result_msw", 32], "A_mul_s5"],
        );

        e_register->adds(
          {out => ["A_mul_cell_p4", 32], 
           in =>  "M_mul_cell_p4",             enable => "A_en"},

          {out => ["A_mul_s2_co", 2], 
           in =>  "A_mul_s2[17:16]",           enable => "1'b1"},
          {out => ["A_mul_s3", 32], 
           in =>  "A_mul_s3_nxt",              enable => "1'b1"},
          {out => ["A_mul_s4", 32], 
           in =>  "A_mul_s4_nxt",              enable => "1'b1"},           
        );

        push(@mul_logic_signals,
          { radix => "x", signal => "A_mul_cell_p4" },
          { radix => "x", signal => "A_mul_s3" },
          { radix => "x", signal => "A_mul_s4" },
          { radix => "x", signal => "A_mul_s2" },
          { radix => "x", signal => "A_mul_s4" },
          { radix => "x", signal => "A_mul_result_msw" },
        );

    }

    push(@plaintext_wave_signals, @mul_logic_signals);
}
...
gen_pipelined_multipler_cells
{
    ...
    e_signal->adds(
       {name => "M_mul_cell_p1", width => 32},
       {name => "M_mul_cell_p2", width => 32},
       {name => "M_mul_cell_p3", width => 32},
    );

    if (!$hw_mul_omits_msw) {
        e_signal->adds(
           {name => "M_mul_cell_p4", width => 32},
        );

        e_assign->adds(
          [["mul_src1_signed",1], "E_ctrl_mul_src1_signed"],
          [["mul_src2_signed",1], "E_ctrl_mul_src2_signed"],
          [["mul_src1",$datapath_sz], "E_src1"],
          [["mul_src2",$datapath_sz], "E_src2"],
          );
    } else {
        e_assign->adds(
          [["mul_src1",$datapath_sz], "E_src1"],
    ...
    my %mul_parameters_p1 = (
      %mul_parameters,
    );

    my %mul_parameters_p2 = (
      %mul_parameters,
    );

    my %mul_parameters_p3 = (
      %mul_parameters,
    );
    ...
    nios_altmult_add->add({
      name              => "the_altmult_add_p1",
      in_port_map       => \%mul_inputs_p1,
      out_port_map      => { "result" => "M_mul_cell_p1" },
      parameter_map     => \%mul_parameters_p1,
      use_sim_models    => 1,
    });

    nios_altmult_add->add({
      name              => "the_altmult_add_p2",
      in_port_map       => \%mul_inputs_p2,
      out_port_map      => { "result" => "M_mul_cell_p2" },
      parameter_map     => \%mul_parameters_p2,
      use_sim_models    => 1,
    });

    nios_altmult_add->add({
      name              => "the_altmult_add_p3",
      in_port_map       => \%mul_inputs_p3,
      out_port_map      => { "result" => "M_mul_cell_p3" },
      parameter_map     => \%mul_parameters_p3,
      use_sim_models    => 1,
    });
    ...
    if (!$hw_mul_omits_msw) {
       ...
       my %mul_parameters_p4 = (
        ...
        nios_altmult_add->add({
          name              => "the_altmult_add_p4",
          in_port_map       => \%mul_inputs_p4,
          out_port_map      => { "result" => "M_mul_cell_p4" },
          parameter_map     => \%mul_parameters_p4,
          use_sim_models    => 1,
        });
    }
}
...
gen_unpipelined_mul_logic
{
    ...
    e_assign->adds(
      [["${os}_mul_cnt_nxt", $mul_cnt_sz],
        "${os}_mul_stall ?  ${os}_mul_cnt-1 : $num_mul_cycles"],
 
      [["${os}_mul_done_nxt", 1], "${os}_mul_cnt_nxt == 0"],
 
      [["${os}_mul_stall_nxt", 1], 
        "~${os}_mul_done_nxt & (${os}_mul_stall | (${is}_ctrl_mul_lsw & ${is}_valid & ${os}_en))"],
    );

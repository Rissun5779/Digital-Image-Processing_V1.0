...
sub make_testbench
{
    ...
    if ($rf_ecc_present && $ecc_test_ports_present) {
        e_assign->add([["W_ecc_test_rf",$datapath_sz], "ecc_test_rf[$datapath_sz-1:0]"]);
    ...
    my $hbreak_entry = {
        condition   => "(D_op_hbreak & W_valid)",
        pc          => $pc,
        dstRegWr    => "R_wr_dst_reg|W_rf_ecc_valid_any",
    ...
    my $inst_entry = {
        condition   => "W_valid | W_rf_ecc_unrecoverable_valid",
        exc         => "(R_ctrl_exception | R_ctrl_break | W_rf_ecc_unrecoverable_valid)",
        pc          => $pc,
...
sub make_inst_decode
{
    ...
    set_inst_ctrl_initial_stage($rd_ctl_reg_ctrl, "D");
    ...
    my $force_src2_zero_ctrl = nios2_insts::additional_inst_ctrl($Opt, {
      ...
      ctrls => ["retaddr", "jmp_indirect", "jmp_direct", "illegal","ld_st_ex"],
...
sub make_tiny_pipeline
{
    ...
    if (nios2_custom_insts::has_insts($Opt->{custom_instructions})) {
        ...
        e_assign->adds(
          ...
          [["W_ci_status", $status_reg_pie_sz], "W_status_reg[$status_reg_pie_lsb]"],
          [["W_ci_estatus", $status_reg_pie_sz],"W_estatus_reg[$status_reg_pie_lsb]"],
        ...
        push(@{$Opt->{port_list}},
          ...
          [W_ci_status          => $status_reg_pie_sz,  "out" ],
          [W_ci_estatus         => $estatus_reg_pie_sz, "out" ],
    ...
    e_assign->adds(
      [["iactive", $interrupt_sz], 
        "irq[$interrupt_sz-1:0] & " . $Opt->{internal_irq_mask_bin}],
    );
    ...
    e_assign->adds(
      [["F_pc_sel_nxt", 2],
         "(R_ctrl_exception | W_rf_ecc_unrecoverable_valid) ? 2'b00 :
          R_ctrl_break                              ? 2'b01 :
    ...
    if ($cpu_reset) {
        ...
        e_register->adds(
          {out => ["cpu_resetrequest_d1", 1], in => "cpu_resetrequest",
           enable => "1'b1"},
        );
    }
    ...
    e_assign->adds(
      ...
      [["F_pc_en", 1], "W_valid | W_rf_ecc_unrecoverable_valid"],
    ...
    e_assign->adds(
      ...
      [["F_valid", 1], ($oci_version == 2) & $oci_present ? "(i_read & ~i_waitrequest) | host_data_reg_valid" : "i_read & ~i_waitrequest"],        



      [["i_read_nxt", 1], "W_valid | W_rf_ecc_unrecoverable_valid | (i_read & i_waitrequest)"],
    ...
    e_register->adds(
      ...
      {out => ["i_read", 1], in => ($oci_version == 2) & $oci_present ? "i_read_nxt & hbreak_enabled" : "i_read_nxt",
    ...
    if ($oci_version == 2 & $oci_present) {
        e_assign->adds(
          ...
          [["oci_hbreak_req", 1], "oci_async_hbreak_req | E_oci_sync_hbreak_req"],
          [["W_op_bret", 1], "D_op_bret"],
          [["A_exc_active_no_break", 1], "(R_ctrl_exception|W_rf_ecc_unrecoverable_valid) & E_valid_from_R"],
          [["A_exc_any_active", 1], "(R_ctrl_exception|W_rf_ecc_unrecoverable_valid|R_ctrl_break) & E_valid_from_R"],
          );
    }
    ...
    my $inject_crst = 
      $cpu_reset ? 
        (($debugger_present || $hbreak_test_bench) ? 
          "(cpu_resetrequest_d1 & hbreak_enabled)" : 
          "cpu_resetrequest_d1") : 
        "1'b0";
    ...
    e_assign->adds(
      ...
      [["F_av_iw", $iw_sz], $big_endian ? "{i_readdata[7:0],i_readdata[15:8],i_readdata[23:16],i_readdata[31:24]}" : "i_readdata"],
      ...
      [["F_iw", $iw_sz], 
        ...
        "~hbreak_enabled ? W_host_data_register :
    ...
    e_register->adds(
      ...
      {out => ["D_valid", 1], in => "F_valid | W1_rf_ecc_recoverable_valid", enable => "1'b1" },
      );
    ...
    if ($rf_ecc_present) {
        ...
        $rf_a_port_map->{corrected_data_to_encoder} = "W_rf_corrected_data_to_encoder";
        $rf_a_port_map->{injs} = "W_rf_injs";
        $rf_a_port_map->{injd} = "W_rf_injd";
        $rf_a_port_map->{wrsel} = "W_rf_wrsel";
        if (manditory_bool($Opt, "activate_trace")) {
            $rf_a_port_map->{injected_data} = "W_rf_injected_wr_data";
        }
        if ($ecc_test_ports_present) {
            $rf_a_port_map->{test_invert} = "ecc_test_rf[$ecc_datapath_msb:0]";
        }

        $rf_b_port_map->{corrected_data_to_encoder} = "W_rf_corrected_data_to_encoder";
        $rf_b_port_map->{injs} = "W_rf_injs";
        $rf_b_port_map->{injd} = "W_rf_injd";
        $rf_b_port_map->{wrsel} = "W_rf_wrsel";
        if ($ecc_test_ports_present) {
            $rf_b_port_map->{test_invert} = "ecc_test_rf[$ecc_datapath_msb:0]";
        }


        $rf_a_port_map->{corrected_data_from_decoder} = "R_rf_corrected_data_a";
        $rf_a_port_map->{one_bit_err} = "R_rf_a_one_bit_err";
        $rf_a_port_map->{two_bit_err} = "R_rf_a_two_bit_err";
        $rf_a_port_map->{one_two_or_three_bit_err} = "R_rf_a_one_two_or_three_bit_err";

        $rf_b_port_map->{corrected_data_from_decoder} = "R_rf_corrected_data_b";
        $rf_b_port_map->{one_bit_err} = "R_rf_b_one_bit_err";
        $rf_b_port_map->{two_bit_err} = "R_rf_b_two_bit_err";
        $rf_b_port_map->{one_two_or_three_bit_err} = "R_rf_b_one_two_or_three_bit_err";
        
        e_signal->adds(
         { name => "R_rf_a_one_two_or_three_bit_err", width => 1, never_export => 1 },
         { name => "R_rf_b_one_two_or_three_bit_err", width => 1, never_export => 1 },
        );
        
        if ($ecc_test_ports_present) {
            e_register->adds(
              {out => ["ecc_test_rf_valid_d1", 1],    in => "ecc_test_rf_valid & W_rf_wren",
               enable => "1'b1"},
            );
            e_assign->adds(
            ...
            e_signal->adds(
                { name => "ecc_test_rf", width => 72 },
            );
        }
        

        e_signal->adds(
            {name => "ecc_event_bus", width => 30, export => $force_export },
        );
        
        e_register->adds(

           {out => ["W_status_reg_eh_d1", 1],    in => "W_status_reg_eh",
            enable => "1'b1"},
           {out => ["ecc_event_eeh", 1],    in => "1'b1",
            enable => "W_rf_ecc_unrecoverable_valid & W_status_reg_eh_d1"},
           {out => ["ecc_event_rf_re", 1],    in => "((E_rf_b_one_bit_err & ~D_ctrl_b_is_dst)|E_rf_a_one_bit_err) & ~E_rf_ecc_unrecoverable_valid & E_valid_from_R",
            enable => "1'b1"},
           {out => ["ecc_event_rf_ue", 1],    in => "((E_rf_b_two_bit_err & ~D_ctrl_b_is_dst) |E_rf_a_two_bit_err) & E_valid_from_R",
            enable => "1'b1"},
           {out => ["ecc_event_bus_reg", 30],    in => "{27'h0,ecc_event_rf_ue,ecc_event_rf_re,ecc_event_eeh}",
            enable => "1'b1"},
        );
        e_assign->adds(
           [["ecc_event_bus", 30], "ecc_event_bus_reg"],
           [["W_rf_wrsel", 1], "W_rf_ecc_recoverable_valid"],
           [["E_rf_ecc_addr_sel", 2], "{E_rf_a_one_bit_err,E_rf_b_one_bit_err}"],
           [["E_rf_ecc_addr", $regnum_sz], "E_rf_ecc_addr_sel == 2'b00 ? 5'd29 : 
                                            E_rf_ecc_addr_sel == 2'b01 ? D_iw_b :
                                            D_iw_a"],
        );
        
        e_register->adds(           

           {out => ["E_rf_corrected_data_a", $datapath_sz],    in => "R_rf_corrected_data_a",
            enable => "1'b1"},
           {out => ["E_rf_corrected_data_b", $datapath_sz],    in => "R_rf_corrected_data_b",
            enable => "1'b1"},
           {out => ["W_rf_corrected_data_to_encoder", $datapath_sz],    in => "E_rf_corrected_data_to_encoder",
            enable => "1'b1"},
           {out => ["E_rf_a_one_bit_err", 1],     in => "R_rf_a_one_bit_err",
            enable => "1'b1"},
           {out => ["E_rf_b_one_bit_err", 1],     in => "R_rf_b_one_bit_err",
            enable => "1'b1"},
           {out => ["E_rf_a_two_bit_err", 1],     in => "R_rf_a_two_bit_err",
            enable => "1'b1"},
           {out => ["E_rf_b_two_bit_err", 1],     in => "R_rf_b_two_bit_err",
            enable => "1'b1"},
        );
        
        e_assign->adds(
           [["E_rf_corrected_data_to_encoder", $datapath_sz], "E_rf_a_one_bit_err ? E_rf_corrected_data_a : E_rf_corrected_data_b"],
        );
        
    }
    
    e_assign->adds(
       [["E_rf_ecc_valid_any", 1], "E_rf_ecc_recoverable_valid|E_rf_ecc_unrecoverable_valid" ],
       [["W_rf_ecc_valid_any", 1], "W_rf_ecc_recoverable_valid|W_rf_ecc_unrecoverable_valid" ],
      
       [["E_rf_ecc_recoverable_valid", 1], $rf_ecc_present ? "W_config_reg_eccen & E_valid_from_R & (E_rf_a_one_bit_err | (E_rf_b_one_bit_err & ~D_ctrl_b_is_dst))" : "1'b0" ],
       [["E_rf_ecc_unrecoverable_valid", 1], $rf_ecc_present ? "W_config_reg_eccen & E_valid_from_R & W_config_reg_eccexc & (E_rf_a_two_bit_err | (E_rf_b_two_bit_err & ~D_ctrl_b_is_dst))" : "1'b0" ],
       
       [["W_dst_regnum", $regnum_sz], $rf_ecc_present ? "W_rf_ecc_valid_any ? E_rf_ecc_addr : R_dst_regnum" : "R_dst_regnum"],
    );
     
    e_register->adds(           

           {out => ["W_rf_ecc_recoverable_valid", 1],    in => "E_rf_ecc_recoverable_valid",
            enable => "1'b1"},
           {out => ["W1_rf_ecc_recoverable_valid", 1],    in => "W_rf_ecc_recoverable_valid",
            enable => "1'b1"},


           {out => ["W_rf_ecc_unrecoverable_valid", 1],    in => "E_rf_ecc_unrecoverable_valid & ~E_rf_ecc_recoverable_valid",
            enable => "1'b1"},
        );


    e_assign->adds(
        [["R_rf_a", $datapath_sz], "R_rf_a_q" ],
        [["R_rf_b", $datapath_sz], "R_rf_b_q" ],
    );
    ...
    e_assign->adds(
      ...
      [["W_rf_wren", 1], "(R_wr_dst_reg & W_valid) | W_rf_ecc_valid_any | ~reset_n"],



      [["W_rf_wr_data", $datapath_sz], 
         $rf_ecc_present ? "W_rf_ecc_unrecoverable_valid ? {F_pc_plus_one, 2'b00} : R_ctrl_ld ? av_ld_data_aligned_filtered : W_wr_data" :
    ...
    if (manditory_bool($Opt, "use_designware")) {
        ...
        nios_sdp_ram->add({
          ...
          provide_injected_data   => manditory_bool($Opt, "activate_trace") && $rf_ecc_present,
    ...
    lhs => ["R_src1", $datapath_sz],
      type => "priority",
      table => [
        "(R_ctrl_br & E_valid_from_R) | (R_ctrl_retaddr & R_valid)"
            => "{F_pc_plus_one, 2'b00}",
        "(R_ctrl_jmp_direct & E_valid_from_R)"   
    ...
    if ($r2) {
        e_mux->add ({
          lhs => ["R_src2_lo_mux", $datapath_sz/2],
          type => "priority",
          table => [
            "R_ctrl_force_src2_zero|R_ctrl_hi_imm16"  => "{16 {D_ctrl_set_src2_rem_imm}}",
            "R_ctrl_signed_imm12"                   => "{{4 {D_iw_imm12[11]}},D_iw_imm12}",
        ...
        e_mux->add ({
          lhs => ["R_src2_hi_mux", $datapath_sz/2],
          type => "priority",
          table => [
            "R_ctrl_force_src2_zero|R_ctrl_unsigned_lo_imm16" => "{16 {D_ctrl_set_src2_rem_imm}}",
        ...
        e_mux->add ({
          ...
          type => "priority",
          table => [
            "R_ctrl_signed_imm12"                   => "{16 {D_iw_imm12[11]}}",
            "1'b0"                                  => "R_src2_hi_mux",
            ],
          });
    } else {
    ...
    e_register->adds(
      ...
      {out => ["E_valid_from_R", 1],                   in => "R_valid | E_stall",
       enable => "1'b1"},
    ...
    e_assign->adds(
      ...
      [["E_valid", 1], 
        "E_valid_from_R & ~E_rf_ecc_valid_any"],
      [["E_stall", 1], 
        "(E_shift_rot_stall | E_ld_stall | E_st_stall | E_ci_multi_stall) & ~(E_rf_ecc_valid_any|W_rf_ecc_valid_any|W1_rf_ecc_recoverable_valid)"],
      );
    ...
    e_assign->adds(
      [["E_shift_rot_shfcnt", $datapath_log2_sz],
        "E_src2[$datapath_log2_sz-1:0]"],

      [["E_shift_rot_cnt_nxt", $datapath_log2_sz],
        "E_new_inst ? E_shift_rot_shfcnt : E_shift_rot_cnt-1"],
    ...
    if ($rf_ecc_present) {
        push(@$rd_ctl_reg_mux_table, $config_reg_regnum    => "W_config_reg");
        push(@$rd_ctl_reg_mux_table, $eccinj_reg_regnum    => "W_eccinj_reg");
        push(@$rd_ctl_reg_mux_table, $exception_reg_regnum    => "W_exception_reg");
    }
    
    push(@$rd_ctl_reg_mux_table, $cdsr_reg_regnum    => "W_cdsr_reg");


    e_mux->add ({
      lhs => ["E_control_rd_data", $max_control_reg_sz],
      selecto => "D_iw_control_regnum",
      table => $rd_ctl_reg_mux_table,
      });
    ...
    e_mux->add({
      ...
      type  => "priority",
      table => [
        "R_ctrl_br_cmp | R_ctrl_rd_ctl_reg"  => "0",
    ...
    if ($big_endian) {
      e_assign->adds(
        [["R_sth_data", 16], "{R_rf_b[7:0],R_rf_b[15:8]}"],
        [["R_stw_data", 32], "{R_rf_b[7:0],R_rf_b[15:8],
           R_rf_b[23:16],R_rf_b[31:24]}"],
        );
    } else {
      e_assign->adds(
        [["R_sth_data", 16], "R_rf_b[15:0]"],
        [["R_stw_data", 32], "R_rf_b[31:0]"],
        );
    }
    ...
    my $st_data = $rf_ecc_present ? "R_st_data" : "E_st_data"; 


    e_mux->add({
      lhs   => ["$st_data", $datapath_sz],
      type  => "priority",
      table => [
        "D_ctrl_mem8"        => "{R_stb_data, R_stb_data, R_stb_data, R_stb_data}",
    ...
    e_mux->add ({
      ...
      selecto => "{D_ctrl_mem16, D_ctrl_mem8, E_mem_baddr[1:0]}",
    ...
    e_assign->adds(
      ...
      [["d_read_nxt", 1], 
        "(R_ctrl_ld & E_new_inst & ~E_rf_ecc_valid_any) | (d_read & d_waitrequest)"],
      ...
      [["d_write_nxt", 1], 
        "((R_ctrl_st & (~R_ctrl_st_ex | W_up_ex_mon_state)) & E_new_inst & ~E_rf_ecc_valid_any) | (d_write & d_waitrequest)"],
    ...
    e_assign->adds(
      ...
      [["av_ld_align_one_more_cycle", 1], 
        "av_ld_align_cycle == (D_ctrl_mem16 ? 2 : 3)"],
      ...
      [["av_ld_done", 1], 
        "~av_ld_waiting_for_data_nxt & (D_ctrl_mem32 | ~av_ld_aligning_data_nxt)"],
    ...
    my $av_ld_byte0_endian = $big_endian ? "d_readdata[31:24]" : "d_readdata[7:0]";
    my $av_ld_byte1_endian = $big_endian ? "d_readdata[23:16]" : "d_readdata[15:8]";
    my $av_ld_byte2_endian = $big_endian ? "d_readdata[15:8]" : "d_readdata[23:16]";
    my $av_ld_byte3_endian = $big_endian ? "d_readdata[7:0]" : "d_readdata[31:24]";
    e_assign->adds(
      ...
      [["av_ld_byte1_data_en", 1], 
        "~(av_ld_extend & D_ctrl_mem16 & ~av_ld_rshift8)"],
      ...
      [["av_sign_bit", 1], 
        "D_ctrl_mem16 ? av_ld_byte1_data[7] : av_ld_byte0_data[7]"],
    ...
    e_signal->adds(
      {name => "A_valid_from_M",       never_export => 1, width => 1},
      {name => "W_valid_from_M",       never_export => 1, width => 1},
    );
    e_assign->adds(
      [["W_valid_from_M", 1], "W_valid"],
    );
    ...
    e_register->adds(
      ...
      {out => ["W_valid", 1],                       in => "E_valid & ~E_stall",
       enable => "1'b1", ip_debug_visible => 1},
      {out => ["A_valid_from_M", 1],                in => "E_valid & ~E_stall",
       enable => "1'b1", ip_debug_visible => 1},

      {out => ["W_control_rd_data", $max_control_reg_sz], 
       in => "D_ctrl_intr_inst ? W_status_reg : E_control_rd_data", enable => "1'b1"},
    ...
    if ($rf_ecc_present) {
        e_register->adds(       
          ...
          {out => ["W_config_reg_eccen", $config_reg_eccen_sz],
           in => "W_config_reg_eccen_nxt", enable => "1'b1" },
          {out => ["W_config_reg_eccexc", $config_reg_eccexc_sz],
           in => "W_config_reg_eccexc_nxt", enable => "1'b1" },
          {out => ["W_eccinj_reg_rf", 2],
           in => "W_eccinj_reg_rf_nxt", enable => "1'b1" },
          {out => ["W_exception_reg_eccftl", 1],
           in => "W_exception_reg_eccftl_nxt", enable => "1'b1" },
          {out => ["W_exception_reg_cause", 5],
           in => "W_exception_reg_cause_nxt", enable => "1'b1" },
          {out => ["W_status_reg_eh", 1],
           in => "W_status_reg_eh_nxt", enable => "1'b1" },
          );
        
        e_assign->adds(
            [["W_config_reg", 4], "{W_config_reg_eccexc,W_config_reg_eccen,2'b00}"],
            [["W_eccinj_reg", 2], "W_eccinj_reg_rf"],
            [["W_exception_reg", 32], "{W_exception_reg_eccftl,24'h0,W_exception_reg_cause,2'b00}"],
        );
    }
    
    e_register->adds(       
      {out => ["W_cdsr_reg", $cdsr_reg_status_sz],
       in => ($oci_version == 2) & $oci_present ? "{31'h0,host_present}" : "0", enable => "1'b1" },
    );

    if ($oci_version == 2) {
        e_register->adds(       
          {out => ["W_host_data_register", $cdsr_reg_status_sz],
           in => "W_host_data_register_nxt", enable => "1'b1" },
         );
    }
    ...
    if ($Opt->{export_pcbdebuginfo}) {
        ...
        e_assign->adds(
          ["pc", ($pcb_remainder_bits > 0) ? "{{${pcb_remainder_bits} {1'b0}},F_pcb}" : "F_pcb" ],
        ...
        e_assign->adds(
          ["iw", "F_iw"],
          ["iw_valid", "W_valid"],
          ["exc", "R_ctrl_exception|W_rf_ecc_unrecoverable_valid"],
          ["exc_valid", "W_valid|W_rf_ecc_unrecoverable_valid"],
        );
    }
    ...
    my $cmp_rd_ctl_reg_non_zero_sz = $max_control_reg_sz;
    my $cmp_rd_ctl_reg_non_zero_lsb = 0;
    my $cmp_rd_ctl_reg_non_zero_msb = 
      $cmp_rd_ctl_reg_non_zero_lsb + $cmp_rd_ctl_reg_non_zero_sz - 1;
    my $cmp_rd_ctl_reg_zero_sz = $datapath_sz - $cmp_rd_ctl_reg_non_zero_sz;
    my $cmp_rd_ctl_reg_zero_lsb = $cmp_rd_ctl_reg_non_zero_msb + 1;
    my $cmp_rd_ctl_reg_zero_msb = 
      $cmp_rd_ctl_reg_zero_lsb + $cmp_rd_ctl_reg_zero_sz - 1;
    ...
    e_assign->adds(
      [["W_wr_data_non_zero", $cmp_rd_ctl_reg_non_zero_sz],
        $r2 ?
        "R_ctrl_br_cmp ? W_cmp_result :
         R_ctrl_rd_ctl_reg       ? W_control_rd_data :
         D_op_stex               ? {31'b0,W_up_ex_mon_state} :
            W_alu_result[$cmp_rd_ctl_reg_non_zero_msb:$cmp_rd_ctl_reg_non_zero_lsb]" :
        "R_ctrl_br_cmp ? W_cmp_result :
         R_ctrl_rd_ctl_reg       ? W_control_rd_data :
            W_alu_result[$cmp_rd_ctl_reg_non_zero_msb:$cmp_rd_ctl_reg_non_zero_lsb]"],
      );
    ...
    if ($cmp_rd_ctl_reg_zero_sz > 0) {
        e_assign->adds(
          [["W_wr_data", $datapath_sz], 
            "{ W_alu_result[$cmp_rd_ctl_reg_zero_msb:$cmp_rd_ctl_reg_zero_lsb],
               W_wr_data_non_zero }"],
        );
    } else {
        e_assign->adds(
    ...
    e_assign->adds(
      ...
      [["W_br_taken", 1], "R_ctrl_br_uncond | (R_ctrl_br & W_cmp_result)"],
    ...
    e_assign->adds(
        [["W_status_reg", $status_reg_sz], $rf_ecc_present ? "{W_status_reg_eh,1'b0,W_status_reg_pie}" : "W_status_reg_pie"],
      );
    ...
    if ($rf_ecc_present) {
      e_assign->adds(
        [["E_wrctl_config", 1], "(R_ctrl_wrctl_inst & 
           (D_iw_control_regnum == $config_reg_regnum))"],
        [["E_wrctl_eccinj", 1], "(R_ctrl_wrctl_inst & 
           (D_iw_control_regnum == $eccinj_reg_regnum))"],
        );
    }

    if ($oci_version == 2) {
      e_assign->adds(
        [["E_wrctl_cdsr", 1], "(R_ctrl_wrctl_inst & 
           (D_iw_control_regnum == $cdsr_reg_regnum))"],
        );
    }
    ...
    e_assign->adds(
      ...
      [["W_status_reg_pie_inst_nxt", $status_reg_pie_sz],
        $r2 ?
        "(R_ctrl_exception | R_ctrl_break | R_ctrl_crst | W_rf_ecc_unrecoverable_valid) ? 1'b0 :
         (D_op_eni)                      ? 1'b1 :
         (D_op_eret)                     ? W_estatus_reg[$status_reg_pie_lsb] :
         (D_op_bret)                     ? W_bstatus_reg[$status_reg_pie_lsb] :
         (E_wrctl_status | D_op_wrpie)   ? E_src1[$status_reg_pie_lsb] :
                                           W_status_reg_pie" : 
        "(R_ctrl_exception | R_ctrl_break | R_ctrl_crst | W_rf_ecc_unrecoverable_valid) ? 1'b0 :
         (D_op_eret)                     ? W_estatus_reg[$status_reg_pie_lsb] :
         (D_op_bret)                     ? W_bstatus_reg[$status_reg_pie_lsb] :
      ...
      ($oci_version == 2) ? "iactive & W_ienable_reg & {32{~oci_idisable}} & " . 
        $Opt->{internal_irq_mask_bin} : 
    ...
    if ($rf_ecc_present) {
      e_assign->adds(
        [["W_config_reg_eccen_nxt", $config_reg_eccen_sz], 
         "(R_ctrl_crst)        ? 0 :
          (E_wrctl_config & E_valid)     ? E_src1[$config_reg_eccen_lsb] :
                                W_config_reg_eccen"],
        [["W_config_reg_eccexc_nxt", $config_reg_eccexc_sz], 
         "(R_ctrl_crst)        ? 0 :
          (E_wrctl_config & E_valid)     ? E_src1[$config_reg_eccexc_lsb] :
                                W_config_reg_eccexc"],
        [["W_eccinj_reg_rf_nxt", 2], 
         "(R_ctrl_crst)        ? 0 :
          (E_wrctl_eccinj & E_valid)     ? E_src1[$eccinj_reg_rf_msb:$eccinj_reg_rf_lsb] :
          (R_wr_dst_reg & E_valid)          ? 0 :
                                W_eccinj_reg_rf"],
        [["W_exception_reg_eccftl_nxt", 1], 
         "(R_ctrl_crst)        ? 0 :
          (E_rf_ecc_unrecoverable_valid)     ? 1'b1 :
                                W_exception_reg_eccftl"],
        [["W_exception_reg_cause_nxt", 5], 
         "(R_ctrl_crst|R_ctrl_exception)        ? 0 :
          (E_rf_ecc_unrecoverable_valid)        ? 5'd20 :
                                W_exception_reg_cause"],

        [["W_status_reg_eh_inst_nxt", $status_reg_eh_sz], 
         "(R_ctrl_crst)        ? 0 :
          (D_op_eret)                     ? W_estatus_reg[$status_reg_eh_lsb] :
          (D_op_bret)                     ? W_bstatus_reg[$status_reg_eh_lsb] :
          (R_ctrl_exception|E_rf_ecc_unrecoverable_valid|R_ctrl_break)        ? 1'b1 :
          (E_wrctl_status)     ? E_src1[$status_reg_eh_lsb] :
                                W_status_reg_eh"],
        [["W_status_reg_eh_nxt", $status_reg_eh_sz],
                                "(E_valid | (E_rf_ecc_unrecoverable_valid & ~E_rf_ecc_recoverable_valid)) ? W_status_reg_eh_inst_nxt : W_status_reg_eh"],
      );
      
      e_register->adds(
        { out => ["W_rf_injs", 1], 
          in => "(R_wr_dst_reg & E_valid) ? W_eccinj_reg_rf[0] : 
                 (W_rf_wren | R_ctrl_crst) ? 1'b0 :
                  W_rf_injs",
          enable => "1'b1", 
        },
        { out => ["W_rf_injd", 1], 
          in => "(R_wr_dst_reg & E_valid) ? W_eccinj_reg_rf[1] : 
                 (W_rf_wren | R_ctrl_crst) ? 1'b0 :
                  W_rf_injd",
          enable => "1'b1", 
        },
    ...
    if ($oci_version == 2) {
    	if ($oci_present) {
      		e_assign->adds(
      		  [["W_host_data_register_nxt", $cdsr_reg_status_sz], 
      		   "(R_ctrl_crst)          ? 0 :
      		    (host_data_reg_write)  ? host_data_reg_writedata :
      		    (E_wrctl_cdsr & E_valid)     ? E_src1 :
      		                          W_host_data_register"],
      		);
      	} else {
      		e_assign->adds(
      		  [["W_host_data_register_nxt", $cdsr_reg_status_sz], 
    ...
    if ($oci_version == 2 & $oci_present) {
        e_signal->adds({name => "oci_debug_mode_exc", never_export => 1, width => 1});
    }
...
create_ptf_signal_list
{
    ...
    my @full = (
        ...
        { radix => "x", signal => "R_ctrl_rd_ctl_reg" },
        { divider => "store" },
    ...
    my @cpu_reset = (
        ...
        { radix => "x", signal => "cpu_resetrequest_d1" },

...
sub make_nios2_oci_dbrk
{
  ...
  $module->add_contents (
    ...
    e_process->new ({
      ...
      contents  => [
        e_assign->new (["dbrk_break", "dbrk_break   ? ~debugack   
                                      : dbrk_break_pulse",
        ]),
      ],
    }),
  ...
  if ($oci_num_dbrk >= 5) {
    if (($oci_num_dbrk >= 6) && $oci_dbrk_pairs) {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit4" => "dbrk4[$dbrk_paired_bit] ?  ".
              " dbrk_hit4_match_paired : dbrk_hit4_match_single"],
          ),
       );
    } else {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit4" => "dbrk_hit4_match_single"],
          ),
       );
    }
    $module->add_contents (
       ...
       e_instance->new ({
         name    => $Opt->{name}."_nios2_oci_dbrk_hit4_match_single",
         module  => $match_single_module,
         port_map  => {
           dbrk          => "dbrk4[$dbrk_ctrl_high:0]",
           addr          => "cpu_d_address",
           data          => "dbrk_data",
           read          => "cpu_d_read_valid",
           write         => "cpu_d_write",
           match_single  => "dbrk_hit4_match_single",
         },
       }),
       e_assign->news (
          ["dbrk4_trigout"     => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_trigout_bit])"],
          ["dbrk4_break_pulse" => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_break_bit])"],
          [["dbrk4_armed", 1, 0, 1]  =>
                      "(dbrk4[$dbrk_arm0_bit] & trigger_state_0) ||
                       (dbrk4[$dbrk_arm1_bit] & trigger_state_1)",
          ],
          ["dbrk4_goto0"    => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_goto0_bit])"],
          ["dbrk4_goto1"    => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_goto1_bit])"],
       ),
    );
    if ($oci_dbrk_trace) {
       $module->add_contents (
          e_assign->news (
             ["dbrk4_traceoff" => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_traceoff_bit])"],
             ["dbrk4_traceon"  => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_traceon_bit])"],
             ["dbrk4_traceme"  => "(dbrk_hit4 & dbrk4_armed & dbrk4[$dbrk_traceme_bit])"],
          ),
       );
    } else { # ~oci_dbrk_trace 
       $module->add_contents (
  ...
  } else { #end if oci_num_dbrk >= 5
    $module->add_contents (
       e_assign->news (
          [["dbrk4_armed", 1, 0, 1]  => "1'b0"],
          ["dbrk4_trigout" => "1'b0"],
          ["dbrk4_break_pulse" => "1'b0"],
          ["dbrk4_traceoff" => "1'b0"],
          ["dbrk4_traceon"  => "1'b0"],
          ["dbrk4_traceme"  => "1'b0"],
          ["dbrk4_goto0"    => "1'b0"],
  ...
  if ($oci_num_dbrk >= 6) {
    $module->add_contents (
       ...
       e_instance->new ({
         name    => $Opt->{name}."_nios2_oci_dbrk_hit5_match_single",
         module  => $match_single_module,
         port_map  => {
           dbrk          => "dbrk5[$dbrk_ctrl_high:0]",
           addr          => "cpu_d_address",
           data          => "dbrk_data",
           read          => "cpu_d_read_valid",
           write         => "cpu_d_write",
           match_single  => "dbrk_hit5_match_single",
         },
       }),
       e_assign->news (
          ["dbrk5_trigout"     => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_trigout_bit])"],
          ["dbrk5_break_pulse" => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_break_bit])"],
          [["dbrk5_armed", 1, 0, 1]  =>
                      "(dbrk5[$dbrk_arm0_bit] & trigger_state_0) ||
                       (dbrk5[$dbrk_arm1_bit] & trigger_state_1)",
          ],
          ["dbrk5_goto0"    => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_goto0_bit])"],
          ["dbrk5_goto1"    => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_goto1_bit])"],
       ),
    );
    if ($oci_dbrk_trace) {
       $module->add_contents (
          e_assign->news (
             ["dbrk5_traceoff" => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_traceoff_bit])"],
             ["dbrk5_traceon"  => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_traceon_bit])"],
             ["dbrk5_traceme"  => "(dbrk_hit5 & dbrk5_armed & dbrk5[$dbrk_traceme_bit])"],
          ),
       );
    } else { # ~oci_dbrk_trace
       $module->add_contents (
    ...
    if ($oci_dbrk_pairs) {
       $module->add_contents (
          ...
          e_instance->new ({
            name    => $Opt->{name}."_nios2_oci_dbrk_hit4_match_paired",
            module  => $match_paired_module,
            port_map  => {
              dbrka         => "dbrk4[$dbrk_ctrl_high:0]",
              dbrkb         => "dbrk5[$dbrk_ctrl_high:0]",  
              addr          => "cpu_d_address",
              data          => "dbrk_data",
              read          => "cpu_d_read_valid",
              write         => "cpu_d_write",
              match_paired  => "dbrk_hit4_match_paired",
            },
          }),
          e_assign->new (
            ["dbrk_hit5" => "dbrk4[$dbrk_paired_bit] ? 0 : dbrk_hit5_match_single"],
          ),
       );
    } else {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit5" => "dbrk_hit5_match_single"],
          ),
       );
    }
  } else {#end if oci_num_dbrk >= 6
    $module->add_contents (
       e_assign->news (
          [["dbrk5_armed", 1, 0, 1]  => "1'b0"],
          ["dbrk5_trigout" => "1'b0"],
          ["dbrk5_break_pulse" => "1'b0"],
          ["dbrk5_traceoff" => "1'b0"],
          ["dbrk5_traceon"  => "1'b0"],
          ["dbrk5_traceme"  => "1'b0"],
          ["dbrk5_goto0"    => "1'b0"],
  ...
  if ($oci_num_dbrk >= 7) {
    if (($oci_num_dbrk >= 8) && $oci_dbrk_pairs) {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit6" => "dbrk6[$dbrk_paired_bit] ?  ".
              " dbrk_hit6_match_paired : dbrk_hit6_match_single"],
          ),
       );
    } else {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit6" => "dbrk_hit6_match_single"],
          ),
       );
    }
    $module->add_contents (
       ...
       e_instance->new ({
         name    => $Opt->{name}."_nios2_oci_dbrk_hit6_match_single",
         module  => $match_single_module,
         port_map  => {
           dbrk          => "dbrk6[$dbrk_ctrl_high:0]",
           addr          => "cpu_d_address",
           data          => "dbrk_data",
           read          => "cpu_d_read_valid",
           write         => "cpu_d_write",
           match_single  => "dbrk_hit6_match_single",
         },
       }),
       e_assign->news (
          ["dbrk6_trigout"     => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_trigout_bit])"],
          ["dbrk6_break_pulse" => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_break_bit])"],
          [["dbrk6_armed", 1, 0, 1]  =>
                      "(dbrk6[$dbrk_arm0_bit] & trigger_state_0) ||
                       (dbrk6[$dbrk_arm1_bit] & trigger_state_1)",
          ],
          ["dbrk6_goto0"    => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_goto0_bit])"],
          ["dbrk6_goto1"    => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_goto1_bit])"],
       ),
    );
    if ($oci_dbrk_trace) {
       $module->add_contents (
          e_assign->news (
             ["dbrk6_traceoff" => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_traceoff_bit])"],
             ["dbrk6_traceon"  => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_traceon_bit])"],
             ["dbrk6_traceme"  => "(dbrk_hit6 & dbrk6_armed & dbrk6[$dbrk_traceme_bit])"],
          ),
       );
    } else { # ~oci_dbrk_trace
       $module->add_contents (
    ...
    $module->add_contents (
       e_assign->news (
          [["dbrk6_armed", 1, 0, 1]  => "1'b0"],
          ["dbrk6_trigout" => "1'b0"],
          ["dbrk6_break_pulse" => "1'b0"],
          ["dbrk6_traceoff" => "1'b0"],
          ["dbrk6_traceon"  => "1'b0"],
          ["dbrk6_traceme"  => "1'b0"],
          ["dbrk6_goto0"    => "1'b0"],
  ...
  if ($oci_num_dbrk >= 8) {
    $module->add_contents (
       ...
       e_instance->new ({
         name    => $Opt->{name}."_nios2_oci_dbrk_hit7_match_single",
         module  => $match_single_module,
         port_map  => {
           dbrk          => "dbrk7[$dbrk_ctrl_high:0]",
           addr          => "cpu_d_address",
           data          => "dbrk_data",
           read          => "cpu_d_read_valid",
           write         => "cpu_d_write",
           match_single  => "dbrk_hit7_match_single",
         },
       }),
       e_assign->news (
          ["dbrk7_trigout"     => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_trigout_bit])"],
          ["dbrk7_break_pulse" => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_break_bit])"],
          [["dbrk7_armed", 1, 0, 1]  =>
                      "(dbrk7[$dbrk_arm0_bit] & trigger_state_0) ||
                       (dbrk7[$dbrk_arm1_bit] & trigger_state_1)",
          ],
          ["dbrk7_goto0"    => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_goto0_bit])"],
          ["dbrk7_goto1"    => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_goto1_bit])"],
       ),
    );
    if ($oci_dbrk_trace) {
       $module->add_contents (
          e_assign->news (
             ["dbrk7_traceoff" => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_traceoff_bit])"],
             ["dbrk7_traceon"  => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_traceon_bit])"],
             ["dbrk7_traceme"  => "(dbrk_hit7 & dbrk7_armed & dbrk7[$dbrk_traceme_bit])"],
          ),
       );
    } else { # ~oci_dbrk_trace
       $module->add_contents (
    ...
    if ($oci_dbrk_pairs) {
       $module->add_contents (
          ...
          e_instance->new ({
            name    => $Opt->{name}."_nios2_oci_dbrk_hit6_match_paired",
            module  => $match_paired_module,
            port_map  => {
              dbrka         => "dbrk6[$dbrk_ctrl_high:0]",
              dbrkb         => "dbrk7[$dbrk_ctrl_high:0]",  
              addr          => "cpu_d_address",
              data          => "dbrk_data",
              read          => "cpu_d_read_valid",
              write         => "cpu_d_write",
              match_paired  => "dbrk_hit6_match_paired",
            },
          }),
          e_assign->new (
            ["dbrk_hit7" => "dbrk6[$dbrk_paired_bit] ? 0 : dbrk_hit7_match_single"],
          ),
       );
    } else {
       $module->add_contents (
          e_assign->new (
            ["dbrk_hit7" => "dbrk_hit7_match_single"],
          ),
       );
    }
    ...
    $module->add_contents (
       e_assign->news (
          [["dbrk7_armed", 1, 0, 1]  => "1'b0"],
          ["dbrk7_trigout" => "1'b0"],
          ["dbrk7_break_pulse" => "1'b0"],
          ["dbrk7_traceoff" => "1'b0"],
          ["dbrk7_traceon"  => "1'b0"],
          ["dbrk7_traceme"  => "1'b0"],
          ["dbrk7_goto0"    => "1'b0"],
  ...
  $module->add_contents (
    ...
    e_process->new ({
      ...
      contents  => [
        e_assign->news (
          ["dbrk_trigout" =>
              "(dbrk0_trigout | dbrk1_trigout | dbrk2_trigout | dbrk3_trigout | dbrk4_trigout | dbrk5_trigout | dbrk6_trigout | dbrk7_trigout)"],
          ["dbrk_break_pulse" =>
              "(dbrk0_break_pulse | dbrk1_break_pulse | dbrk2_break_pulse | dbrk3_break_pulse | dbrk4_break_pulse | dbrk5_break_pulse | dbrk6_break_pulse | dbrk7_break_pulse)"],
          ["dbrk_traceoff" => 
              "(dbrk0_traceoff | dbrk1_traceoff | dbrk2_traceoff | dbrk3_traceoff | dbrk4_traceoff | dbrk5_traceoff | dbrk6_traceoff | dbrk7_traceoff)"],
          ["dbrk_traceon" =>
              "(dbrk0_traceon | dbrk1_traceon | dbrk2_traceon | dbrk3_traceon | dbrk4_traceon | dbrk5_traceon | dbrk6_traceon | dbrk7_traceon)"],
          ["dbrk_traceme" =>
              "(dbrk0_traceme | dbrk1_traceme | dbrk2_traceme | dbrk3_traceme | dbrk4_traceme | dbrk5_traceme | dbrk6_traceme | dbrk7_traceme)"],
          ["dbrk_goto0" =>
              "(dbrk0_goto0 | dbrk1_goto0 | dbrk2_goto0 | dbrk3_goto0 | dbrk4_goto0 | dbrk5_goto0 | dbrk6_goto0 | dbrk7_goto0)"],
          ["dbrk_goto1" => 
              "(dbrk0_goto1 | dbrk1_goto1 | dbrk2_goto1 | dbrk3_goto1 | dbrk4_goto1 | dbrk5_goto1 | dbrk6_goto1 | dbrk7_goto1)"],
        ),
      ],
    }),
  );

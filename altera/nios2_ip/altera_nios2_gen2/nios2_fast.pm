...
sub make_fast_pipeline
{
    ...
    my $oci_version_2_disable_itcm = ($oci_version == 2) ? " & ~fetch_inst_cancel" : "";
    
    if ($icache_present) {
        push(@iw_avail_expr, "F_ic_hit");
    } else {
        push(@iw_avail_expr, "F_i_hit");
    }
    
    if ($fa_present) {
        push(@iw_avail_expr, "F_fa_iw_valid");
    }
    ...
    e_assign->adds(
      ...
      [["F_sel_itcm", 1], $fa_present ? "~(F_sel_instruction_master|F_sel_flash_instruction_master)"  . $oci_version_2_disable_itcm : "~F_sel_instruction_master" . $oci_version_2_disable_itcm],
      ...
      [["F_iw_avail", 1], 
          "~W_exc_crst_active & (" . join(' | ', @iw_avail_expr) . ")"],
      ...
      [["F_older_non_sequential", 1], 
        "(((D_br_pred_taken & D_issue) |
      ...
      [["F_kill", 1], "D_refetch | F_older_non_sequential"],
      ...
      [["F_issue", 1], ($oci_version == 2) & $oci_present ? "(F_iw_valid & ~F_kill & ~W_debug_mode) | host_data_reg_valid" : "F_iw_valid & ~F_kill"],
    );
        ...
        e_assign->adds(
          ...
          [["F_iw_valid", 1], 
            "${match_mux_paddr}_got_pfn & 
              (F_iw_avail | (~${match_mux_vaddr}_bypass_tlb & F_uitlb_m))"],
        );
    } else {
        e_assign->adds(

          [["F_iw_valid", 1], "F_iw_avail"],
        );
    }
    ...
    e_assign->adds(
      ...
      [["F_br_taken_waddr_partial", $partial_br_offset_sz+1],
        "F_pc_plus_one[$partial_br_offset_sz-1:0] + 
         F_iw_imm16[$partial_br_offset_sz+1:2]"],
      );
    ...
    e_assign->adds(
      ...
      [["D_raw_refetch", 1], "~D_iw_valid & ~D_kill"],



      [["D_refetch", 1], "D_raw_refetch & ~(M_pipe_flush | E_valid_jmp_indirect)"],
    );
...
sub make_inst_decode
{
    ...
    cpu_inst_gen::gen_inst_fields($gen_info, $Opt->{inst_field_info},
      $fa_present ? ["fa", "F", "D", "E", "M", "A", "W"] : ["F", "D", "E", "M", "A", "W"]);
    ...
    my $late_result_ctrl_names = ["ld","shift_rot","rd_ctl_reg","custom_multi"];
...
make_ecc_event_bus
{
    ...
    if ($dtcm_ecc_present) {
        my $cmi = manditory_int($Opt, "num_tightly_coupled_data_masters");
        while ($cmi < 4) {
        ...
        for ($cmi = manditory_int($Opt, "num_tightly_coupled_data_masters"); 
          $cmi > 0;
          $cmi--) {
            my $cmi_minus1 = $cmi - 1;
            push(@event_bus_signals, "dtcm${cmi_minus1}_ecc_event_unrecoverable_err");   # DTCM(cmi)_UE
            push(@event_bus_signals, "dtcm${cmi_minus1}_ecc_event_recoverable_err");     # DTCM(cmi)_RE
        } 
    } else {
    ...
    if ($ecc_test_ports_present) {
       my @ecc_ram_type = ("ic_tag", "ic_data", "dc_tag", "dc_data", "dc_wb", 

...
convert_vectors_to_kernel_region
{
    ...
    my $r1 = shift;
    my $oci_enabled = shift;
    my $allow_break_inst = shift;
    my $oci_version1 = shift;
    ...
    if (($r1 || $oci_enabled || $allow_break_inst) && $oci_version1) {
...
convert_to_c
{
    ...
    format_hash_as_c_macros($elaborated_mmu_info->{mmu_constants}, $h_lines, $prefix);

    convert_mmu_addr_bit_fields_to_c(
      $elaborated_mmu_info->{mmu_addr_bit_fields}, $c_lines, $h_lines, $prefix);

    add_handy_macros($h_lines, $prefix);
...
make_tlb
{
    ...
    if ($mmu_ecc_present) {
       my $hw_read_one_bit_err      = "(~W_tlb_rd_operation & tlb_one_bit_err)";
       

       $ecc_one_bit_err_flush       = "| (~W_tlb_rd_operation & tlb_one_bit_err)";
       

       $ecc_err_match       = "| (~W_tlb_rd_operation & (tlb_two_bit_err | tlb_one_bit_err))";


       $ecc_one_bit_err_wr_op       = "$hw_read_one_bit_err |";

       $ecc_one_bit_err_get_rd_addr = "(A_wrctl_tlbacc & A_valid & W_tlbmisc_reg_we) ? { tlb_wr_line, tlb_wr_way } : 
       ...
       e_assign->adds(
        [["tlb_wrsel", 1],
        "$hw_read_one_bit_err & ~(A_wrctl_tlbacc & A_valid & W_tlbmisc_reg_we)"],
       );
       ...
       e_register->adds(
         ...
         {out => ["tlb_corrected_data_from_decoder", $tlb_data_sz],    in => "tlb_corrected_data_from_decoder_nxt",
          enable => "1'b1"},
         {out => ["tlb_one_bit_err", 1],    in => "W_config_reg_eccen & utlb_fill_match_active_nxt & tlb_one_bit_err_nxt",
          enable => "1'b1"},         
         {out => ["tlb_two_bit_err", 1],    in => "W_ecc_exc_enabled & utlb_fill_match_active_nxt & tlb_two_bit_err_nxt",
          enable => "1'b1"},
         {out => ["tlb_one_two_or_three_bit_err", 1],    in => "W_config_reg_eccen & tlb_one_two_or_three_bit_err_nxt",
          enable => "1'b1"},
    ...
    e_assign->adds(
      ...
      [["utlb_flush_all", 1], "(A_wrctl_tlbmisc & A_valid) $ecc_one_bit_err_flush"], 
    ...
    e_assign->adds(
      ...
      [["utlb_fill_tag_match_nxt", 1], 
        "(utlb_fill_tag == tlb_rd_tag) & ~W_tlb_rd_operation"], 




      [["utlb_fill_pid_match_nxt", 1], 
        "(W_tlbmisc_reg_pid == tlb_rd_pid) | tlb_rd_g"],
    ...
    e_register->adds(
      ...
      {out => ["udtlb_fill_done_d1", 1], 
       in => "(utlb_fill_done & utlb_fill_active_d) | A_refetch_required", enable => "1'b1"},
    ...
    e_assign->adds(
      ...
      [["tlb_rd_line_nxt", $tlb_line_sz],
          "A_tlb_rd_operation ?
             W_pteaddr_reg_vpn[$tlb_vpn_line_msb:$tlb_vpn_line_lsb] :
             utlb_fill_line"],
      ...
      [["tlb_wr_way", $tlb_way_sz], "W_tlbmisc_reg_way"],
      ...
      [["tlb_wr_tag", $tlb_tag_sz],
        "W_pteaddr_reg_vpn[$tlb_vpn_tag_msb:$tlb_vpn_tag_lsb]"],
      [["tlb_wr_pid", $tlb_pid_sz], "W_tlbmisc_reg_pid"],
    ...
    my @tlb_ram_wave_signals = (
      { divider => "TLB RAM" },
      { radix => "x", signal => "A_tlb_rd_operation" },
      { radix => "x", signal => "W_tlb_rd_operation" },
 ...
 */
$define ${prefix}USER_REGION_MIN_VADDR       0x00000000
$define ${prefix}USER_REGION_MAX_VADDR       0x7fffffff
$define ${prefix}KERNEL_MMU_REGION_MIN_VADDR 0x80000000
$define ${prefix}KERNEL_MMU_REGION_MAX_VADDR 0xbfffffff
$define ${prefix}KERNEL_REGION_MIN_VADDR     0xc0000000
$define ${prefix}KERNEL_REGION_MAX_VADDR     0xdfffffff
$define ${prefix}IO_REGION_MIN_VADDR         0xe0000000
$define ${prefix}IO_REGION_MAX_VADDR         0xffffffff
        
$define ${prefix}MMU_PAGE_SIZE (0x1 << (MMU_ADDR_PAGE_OFFSET_SZ))

$define is_${prefix_lc}MmuUserRegion(Vaddr)          \\
    (GET_${prefix}MMU_ADDR_USER_REGION(Vaddr) == ${prefix}MMU_ADDR_USER_REGION)
$define is_${prefix_lc}MmuKernelMmuRegion(Vaddr)     \\
    (GET_${prefix}MMU_ADDR_KERNEL_MMU_REGION(Vaddr) == ${prefix}MMU_ADDR_KERNEL_MMU_REGION)
$define is_${prefix_lc}MmuKernelRegion(Vaddr)        \\
    (GET_${prefix}MMU_ADDR_KERNEL_REGION(Vaddr) == ${prefix}MMU_ADDR_KERNEL_REGION)
$define is_${prefix_lc}MmuIORegion(Vaddr)            \\
    (GET_${prefix}MMU_ADDR_IO_REGION(Vaddr) == ${prefix}MMU_ADDR_IO_REGION)

/* Does this virtual address bypass the TLB? */
$define ${prefix_lc}vaddrBypassTlb(Vaddr)                \\
    (GET_${prefix}MMU_ADDR_BYPASS_TLB(Vaddr) == ${prefix}MMU_ADDR_BYPASS_TLB)

/* If TLB is bypassed, is the address cacheable or uncachable. */
$define ${prefix_lc}vaddrBypassTlbCacheable(Vaddr)       \\
    (GET_${prefix}MMU_ADDR_BYPASS_TLB_CACHEABLE(Vaddr) == ${prefix}MMU_ADDR_BYPASS_TLB_CACHEABLE)

/*
 * Compute physical address for regions that bypass the TLB.
 * Just need to clear some top bits.
 */
$define ${prefix_lc}bypassTlbVaddrToPaddr(Vaddr)    \\
    ((Vaddr) & (${prefix}MMU_ADDR_BYPASS_TLB_PADDR_MASK << ${prefix}MMU_ADDR_BYPASS_TLB_PADDR_LSB))

/* 
 * Will the physical address fit in the Kernel/IO region virtual address space?
 */
$define ${prefix_lc}fitsInKernelRegion(Paddr)       \\
    (GET_${prefix}MMU_ADDR_KERNEL_REGION(Paddr) == 0)
$define ${prefix_lc}fitsInIORegion(Paddr)           \\
    (GET_${prefix}MMU_ADDR_IO_REGION(Paddr) == 0)

/* Convert a physical address to a Kernel/IO region virtual address. */
$define ${prefix_lc}paddrToKernelRegionVaddr(Paddr) \\
    ((Paddr) | (${prefix}MMU_ADDR_KERNEL_REGION << ${prefix}MMU_ADDR_KERNEL_REGION_LSB))
$define ${prefix_lc}paddrToIORegionVaddr(Paddr)     \\
    ((Paddr) | (${prefix}MMU_ADDR_IO_REGION << ${prefix}MMU_ADDR_IO_REGION_LSB))

/*
 * Convert a virtual address to a Kernel/IO region virtual address.
 * Uses bypassTlbVaddrToPaddr to clear top bits.
 */
$define ${prefix_lc}vaddrToKernelRegionVaddr(Vaddr) \\
    ${prefix_lc}paddrToKernelRegionVaddr(${prefix_lc}bypassTlbVaddrToPaddr(Vaddr))
$define ${prefix_lc}vaddrToIORegionVaddr(Vaddr) \\
    ${prefix_lc}paddrToIORegionVaddr(${prefix_lc}bypassTlbVaddrToPaddr(Vaddr))

/* Convert between VPN/PFN and virtual/physical addresses. */
$define ${prefix_lc}vpnToVaddr(Vpn) ((Vpn) << ${prefix}MMU_ADDR_VPN_LSB)
$define ${prefix_lc}pfnToPaddr(Pfn) ((Pfn) << ${prefix}MMU_ADDR_PFN_LSB)
$define ${prefix_lc}vaddrToVpn(Vaddr) GET_${prefix}MMU_ADDR_VPN(Vaddr)
$define ${prefix_lc}paddrToPfn(Paddr) GET_${prefix}MMU_ADDR_PFN(Paddr)

/* Bitwise OR with a KERNEL region address to make it an IO region address */
$define ${prefix}KERNEL_TO_IO_REGION 0x20000000
EOM

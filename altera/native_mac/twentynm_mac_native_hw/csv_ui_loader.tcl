...
#| Name:    load_ports
#|
#| Purpose: Takes a csv file and defines hw.tcl ports based on the
#|          declarations made in the file.
#|
#| Params:  csv_file -- the csv file with the port declarations
#|
#| Returns: none
#|
#+------------------------------------------------------------------------------
proc load_ports {csv_file} {

    set fid [open "$csv_file" r]            ;# Open the CSV file with ports definitions

    struct::matrix mat                      ;# Create a 2d matrix to hold the data
    csv::read2matrix $fid mat "," auto      ;# Fill the matrix with the csv file data

    set ports [list]                    ;# List that holds the latest #ports data

    # Traverse each row in the csv file
    for {set r 0} {$r < [mat rows]} {incr r} {
    
        # Grab the first cell in the row and see if it defines a new #ports
        set first_cell [string trim [mat get cell 0 $r]]
        if { [string match -nocase "#PORT" $first_cell] == 1 } {
            set ports [mat get row $r]
            continue
        }

        # Add the ports if a #ports has been defined and the row has some info (not a blank line)
        set row [mat get row $r]
        if { [string match -nocase "" $ports] == 0 && [string match -nocase "" [join $row {}]] == 0 } {
            add_port_from_csv_line $ports $row
        }
    }
...
#+------------------------------------------------------------------------------
#|
#| Name:    add_port_from_csv_line
#|
#| Purpose: Takes a line from the csv file and adds a port to the IP
#|          based on its attribute
#|
#| Params:  port -- list containing the #PORT infromation
#|          row -- the csv row data that should be added as a port
#|
#| Returns: none
#|
#+------------------------------------------------------------------------------
proc add_port_from_csv_line {port row} {
    # Add Port by grabing the name, type and context 
    set name_index [lsearch -nocase $port "#PORT"]
    set type_index [lsearch -nocase $port "TYPE"]
    set context_index [lsearch -nocase $port "CONTEXT"]
	set width_index [lsearch -nocase $port "WIDTH"]
	set default_value_index [lsearch -nocase $port "DEFAULT_VALUE"]
    
	
    if { $name_index != -1 && $type_index != -1 } {
        set name [lindex $row $name_index]
        set type [lindex $row $type_index]
        set context [lindex $row $context_index]
        set width [lindex $row $width_index]
	
		# Get all the IP parameters and validate the width does not depend on the parameter
		# width_expr need to be handled during elab
		set width_expr "false"
		...
		if { $context == "" && $width_expr eq "false"} {
			if {$type eq "INPUT"} {
				add_interface $name conduit input
				set_interface_assignment $name "ui.blockdiagram.direction" Input
				if { $width == "" } {
					add_interface_port $name $name $name Input 1	
				} else {
					add_interface_port $name $name $name Input $width
				}				
			} elseif {$type eq "OUTPUT"} {
				add_interface $name conduit output
				set_interface_assignment $name "ui.blockdiagram.direction" Output
				if { $width == "" } {
					add_interface_port $name $name $name Output 1	
				} else {
					add_interface_port $name $name $name Output $width
				}					
			}
		}
    } else {

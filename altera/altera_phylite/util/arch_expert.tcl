...
# proc: get_legal_write_latencies
#
# Get string range of legal write latencies
#
proc ::altera_phylite::util::arch_expert::get_legal_write_latencies {} {
   set func_name [_get_func_name "get_legal_write_latencies"]
   return [$func_name]
}

# proc: get_max_num_groups
#
# Get the maximum number of groups supported by the architecture
#
proc ::altera_phylite::util::arch_expert::get_max_num_groups {} {
   set func_name [_get_func_name "get_max_num_groups"]
   return [$func_name]
}

# proc: get_pll_vco_to_mem_clk_freq_ratio
#
# Returns the ratio of PLL VCO frequency to the memory clock frequency. 
# This value must either be 1, 2, 4, 8.
#
# The reason why the PLL VCO can run faster than the memory clock frequency
# is due to the interpolator having a limited frequency range in which
# it has good linearity. 
#
# For example, to support a 400MHz EMIF, the PLL VCO needs to run at 800MHz
# (i.e. return value is 2). In addition, the ioereg needs to be configured
# to have a "mode_rate_out" of 1/2-rate.
#
# returns: Either 1, 2, 4, or 8
# 
proc ::altera_phylite::util::arch_expert::get_pll_vco_to_mem_clk_freq_ratio {mem_clk_freq_mhz} {
	set func_name [_get_func_name "get_pll_vco_to_mem_clk_freq_ratio"]
   return [$func_name]
}

# proc: get_pll_output_clocks_info
#
# Returns the pll output clocks that need to be configured with the given
# memory frequency and input clock frequency.  This information then feeds
# into common PLL TCL package to generate PLL related physical parameters
#
proc ::altera_phylite::util::arch_expert::get_pll_output_clocks_info {} {
   set func_name [_get_func_name "get_pll_output_clocks_info"]
   return [$func_name]
}

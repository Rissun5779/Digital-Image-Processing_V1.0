...
foreach { inst } $instances {
   ...
   foreach { usr_clock } $pins(pll_usr_clock)  {   
      ...
      if {[get_collection_size [get_nets $usr_clock -nowarn]] > 0} {
        if {[get_pin_info -name [get_net_info -pin $usr_clock]] == $pins(pll_master_usr_clk)} {
           set found_master_clock 1
           set suffix ""       
        }
      } elseif {$usr_clock == $pins(pll_master_usr_clk)} {
         set found_master_clock 1
         set suffix ""
      }
   ...
   foreach { usr_clock_a } $pins(pll_usr_clock_a)  {   
      ...
      } elseif {$usr_clock_a == $pins(pll_master_usr_clk)} {
         set found_master_clock 1
         set suffix ""
      }  
   ...
   for {set i_grp_idx 0} {$i_grp_idx < $var(PHYLITE_NUM_GROUPS)} { incr i_grp_idx} {
      ...
      set additional_input_delay [expr 0.5*($var(RD_ISI) + $var(RD_SSI) + $var(RD_JITTER_SCALED))]

      set var(WR_ISI)         $var(GROUP_${i_grp_idx}_WRITE_ISI)
      set additional_output_delay [expr 0.5*($var(WR_ISI) + $var(WR_SSO) + $var(WR_JITTER_SCALED))]
      ...
      set write_fifo_clk [get_keepers -nowarn ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst~out_phy_reg]
      set write_fifo_clk_neg [get_keepers -nowarn ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst~out_phy_reg__nff]
      ...
      if {$var(GROUP_${i_grp_idx}_GENERATE_OUTPUT_CONSTRAINT) == "true"} {
         ...
         if {[llength $write_clocks]>0} {
            ...
            set_clock_uncertainty -to [get_clocks $write_clocks] 0.0
         }
      }
      ...
      if {$var(GROUP_${i_grp_idx}_GENERATE_INPUT_CONSTRAINT) == "true"} {
         foreach { read_pin } $pins(rdata,${i_grp_idx}) {
            ...
            foreach { rclkn_pin } $pins(rclk_n,${i_grp_idx}) { 
               set_input_delay -max $in_max_delay -clock_fall -clock [get_clocks ${rclkn_pin}_IN ] [get_ports $read_pin] -add_delay
               set_input_delay -min $in_min_delay -clock_fall -clock [get_clocks ${rclkn_pin}_IN ] [get_ports $read_pin] -add_delay
               set_false_path -rise_to [get_clocks ${rclkn_pin}_IN]
            }
         }
         if {[llength $read_clocks]>0} {
 	...
 	set dll_reset [get_pins -nowarn ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst|core_dll[2]]
	if {[get_collection_size $dll_reset] > 0} {
		if {$fit_flow == 1} {
			set_multicycle_path -through ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst|core_dll[2] -setup 8 -end
			set_multicycle_path -through ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst|core_dll[2] -hold  7 -end
		} else {
			set_false_path -through ${inst}*|core|arch_inst|group_gen[$i_grp_idx].u_phylite_group_tile_14|lane_gen[*].u_lane|inst|core_dll[2]
		}
	}
	 ...
	 # -                            - #
         ...
         if {$out_clocks_pos != "" && [get_collection_size $out_clocks_pos] > 0 && [get_collection_size $out_clocks_neg] > 0} {
            set_false_path -setup -from $wf_clocks_neg -rise_to $out_clocks_pos
         ...
         if {$core_clock != ""} {
            ...
            foreach { phy_clock } $pins(pll_phy_clock_l) {
               ...
               if {$i_phy_clock > $same_tile_index} {
                  ...
                  set add_to_derived ""
                  set c2p_su         [expr {$p2c_c2p_multi_tile_clock_uncertainty + [lindex $periphery_overconstraints 0] + [lindex $periphery_clock_uncertainty 0]}]
                  set c2p_h          [expr {$p2c_c2p_multi_tile_clock_uncertainty + [lindex $periphery_overconstraints 1] + [lindex $periphery_clock_uncertainty 1]}]
                  set p2c_su         [expr {$p2c_c2p_multi_tile_clock_uncertainty + [lindex $periphery_overconstraints 2] + [lindex $periphery_clock_uncertainty 2]}]
                  set p2c_h          [expr {$p2c_c2p_multi_tile_clock_uncertainty + [lindex $periphery_overconstraints 3] + [lindex $periphery_clock_uncertainty 3]}]
               } else {
                  # C2P/P2C where the periphery tile == CPA tile

...
/////////////////////////////////////////////////////////////////////////////
// Data/Strobe Group Tile of 14nm PHYLite component.
//
/////////////////////////////////////////////////////////////////////////////
module phylite_group_tile_14 #(
	...
	parameter string  A_FILTER_CODE			= "freq_12ghz"	   ,
	...
	parameter READ_LATENCY				= 6'd4,
    	parameter [5:0]   DQS_ENABLE_DELAY              = 1                ,
	parameter [12:0]  DQS_ENABLE_PHASE_SHIFT_A      = 128              ,	
    	parameter [12:0]  DQS_ENABLE_PHASE_SHIFT_B      = 128 + 512        ,
	parameter integer USE_INTERNAL_CAPTURE_STROBE   = 0                ,
	parameter integer CAPTURE_PHASE_SHIFT           = 90               ,
	parameter [12:0]  OUTPUT_DATA_PHASE		= 256		   ,
	...
	parameter integer OUTPUT_STROBE_PHASE           = 512              ,
	...
	output [47:0] emif_phy_out_a,
	output [47:0] emif_phy_out_b,
	...
	localparam [12:0] MIN_PHASE_OFFSET = 13'h080;
	...
	localparam [12:0] ONE_EXT_MEM_PHASE_CLOCK_CYCLE = (OUT_RATE == "out_rate_full") ? 13'h0080 :
	                                                  ...
	                                                  (OUT_RATE == "out_rate_1_4")  ? 13'h0200 :
	                                                                                  13'h0400 ;
	localparam integer WRITE_LATENCY                 = 0;											  
	...
	localparam [9:0] DQS_DELAY = (DQS_DELAY_CALC > 1023) ? 1023 : DQS_DELAY_CALC; // cap DQS_DELAY at 10-bit maximum value
	...
	localparam DQSA_SELECT = (TOTAL_PIN_WIDTH <= 12)	?	
				 	((STROBE_CONFIG == "DIFFERENTIAL") ? "dqsa_diff_in_1" : (SWAP_CAPTURE_STROBE_POLARITY == 1)	? "dqsa_sstl_n_1" 	:	"dqsa_sstl_p_1") :
				 ((TOTAL_PIN_WIDTH > 12) && ((TOTAL_PIN_WIDTH <= 24))) ?
				 	((STROBE_CONFIG == "DIFFERENTIAL") ? "dqsa_diff_in_2" : (SWAP_CAPTURE_STROBE_POLARITY == 1)	? "dqsa_sstl_n_2" 	:	"dqsa_sstl_p_2") :
				((STROBE_CONFIG == "DIFFERENTIAL") ? "dqsa_diff_in_3" : (SWAP_CAPTURE_STROBE_POLARITY == 1)	? "dqsa_sstl_n_3" 	:	"dqsa_sstl_p_3");

	localparam DQS_GROUP = (TOTAL_PIN_WIDTH <= 12) ? "DQS_X8" :
			       ((TOTAL_PIN_WIDTH > 12) && ((TOTAL_PIN_WIDTH <= 24))) ? "DQS_X16" :
				"DQS_X36";

	localparam DQSB_SELECT = (STROBE_CONFIG == "SINGLE_ENDED" || STROBE_CONFIG == "DIFFERENTIAL") ? "dqsb_constant" :
				 (DQS_GROUP == "DQS_X8")	?	
				 	((SWAP_CAPTURE_STROBE_POLARITY == 1)	? 	"dqsb_sstl_p_0"	: "dqsb_sstl_n_0")	:
				 (DQS_GROUP == "DQS_X16") 	?	
				 	((SWAP_CAPTURE_STROBE_POLARITY == 1)	? 	"dqsb_sstl_p_2"	: "dqsb_sstl_n_2")	:
				((SWAP_CAPTURE_STROBE_POLARITY == 1)	? 	"dqsb_sstl_p_3"	: "dqsb_sstl_n_3")	;
	localparam DQS_MODE = (STROBE_CONFIG == "COMPLEMENTARY") ? "pin0_dqs_sampler_b_a_rise" : "dqs_sampler_a";
	// Tile Ctrl Params
	...
	fourteennm_tile_ctrl #(
		...
		.pa_feedback_divider_c0 ("div_by_1"       ),       // Core clock 0 divider (always 1)
		.pa_feedback_divider_c1 ("div_by_1"       ),       // Core clock 1 divider (always 1)
		.pa_feedback_divider_p0 ("div_by_1"       ),       // PHY clock 0 divider (always 1)
		.pa_feedback_divider_p1 ("div_by_1"       ),       // PHY clock 1 divider (always 1)
		.pa_feedback_mux_sel_0  ("fb0_p_clk"       ),       // Use phy_clk[0] as feedback
		.pa_feedback_mux_sel_1  ("fb0_p_clk"       ),       // Use phy_clk[0] as feedback
		...
		.pa_reset_n          (reset_n),                       // Not used - CPA is reset by frzreg
	...
	generate 
		...
		for(lane_num = 0; lane_num < 4; lane_num = lane_num + 1) begin : lane_gen
			...
			localparam PIN_0_PHASE_OFFSET = PIN_0_IS_STROBE ? OUTPUT_STROBE_PHASE  : OUTPUT_DATA_PHASE ;
			localparam PIN_1_PHASE_OFFSET = PIN_1_IS_STROBE ? OUTPUT_STROBE_PHASE  : OUTPUT_DATA_PHASE ;
			localparam PIN_2_PHASE_OFFSET = PIN_2_IS_STROBE ? OUTPUT_STROBE_PHASE  : OUTPUT_DATA_PHASE ;
			localparam PIN_3_PHASE_OFFSET = PIN_3_IS_STROBE ? OUTPUT_STROBE_PHASE  : OUTPUT_DATA_PHASE ;
			...
			localparam PIN_0_DATA_IN_MODE =  "ca"; 
			localparam PIN_1_DATA_IN_MODE =  "ca"; 
			localparam PIN_2_DATA_IN_MODE =  "ca"; 
			localparam PIN_3_DATA_IN_MODE =  "ca"; 
			localparam PIN_4_DATA_IN_MODE  = "ca";
			localparam PIN_5_DATA_IN_MODE  = "ca";
			localparam PIN_6_DATA_IN_MODE  = "ca";
			localparam PIN_7_DATA_IN_MODE  = "ca";
			localparam PIN_8_DATA_IN_MODE  = "ca";
			localparam PIN_9_DATA_IN_MODE  = "ca";
			localparam PIN_10_DATA_IN_MODE = "ca";
			localparam PIN_11_DATA_IN_MODE = "ca";
			...
			localparam A_FILTER_CODE = 	(PLL_VCO_FREQ_MHZ_INT >= 200 && PLL_VCO_FREQ_MHZ_INT < 600) ? "freq_08ghz" :
							(PLL_VCO_FREQ_MHZ_INT >= 600 && PLL_VCO_FREQ_MHZ_INT < 750) ? "freq_10ghz" :
							(PLL_VCO_FREQ_MHZ_INT >= 750 && PLL_VCO_FREQ_MHZ_INT < 950) ? "freq_12ghz" :
							"freq_16ghz";

			fourteennm_io_12_lane_wrapper #(
				// Top level params
				...
				.pin_10_output_phase  (OUTPUT_DATA_PHASE ),
				...
				.pin_11_output_phase  (OUTPUT_DATA_PHASE ),
				...
				.pin_0_dqs_mode (DQS_MODE),
				.pin_1_dqs_mode (DQS_MODE),
				.pin_2_dqs_mode (DQS_MODE),
				.pin_3_dqs_mode (DQS_MODE),
				.pin_4_dqs_mode (DQS_MODE),
				.pin_5_dqs_mode (DQS_MODE),
				.pin_6_dqs_mode (DQS_MODE),
				.pin_7_dqs_mode (DQS_MODE),
				.pin_8_dqs_mode (DQS_MODE),
				.pin_9_dqs_mode (DQS_MODE),
				.pin_10_dqs_mode (DQS_MODE),
				.pin_11_dqs_mode (DQS_MODE),
				...
				.dqs_lgc_dqs_a_interp_en   (DQSA_SELECT),
				.dqs_lgc_dqs_b_interp_en   (DQSB_SELECT),
				...
				.emif_phy_out_a(emif_phy_out_a   [((lane_num + 1) * 12) - 1 : lane_num * 12]),
				.emif_phy_out_b(emif_phy_out_b   [((lane_num + 1) * 12) - 1 : lane_num * 12]),

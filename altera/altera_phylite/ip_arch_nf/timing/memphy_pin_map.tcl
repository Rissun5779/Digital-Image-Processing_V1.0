...
proc memphy_get_ddr_pins { instname allpins rate var_array_name} {
   ...
   set pins(pll_c0_cpa_core_clock_id) [get_pins -nowarn [list ${instname}|core|arch_inst|group_gen\[*\].u_phylite_group_tile_20|u_twentynm_tile_ctrl|pa_core_clk_out\[0\]]]
   ...
   if {[regexp {(^.*)\|core\|arch_inst\|group_gen\[[0-9]\].u_phylite_group_tile_20\|u_twentynm_tile_ctrl(.*)\|pa_core_clk_out\[[0-9]\]$} $pins(pll_master_usr_clk) matched pins(master_instname) pins(master_instnum)] == 1} {
...
proc memphy_is_node_type_pll_clk { node_id } {
   ...
   if {$cell_id == ""} {
      ...
      if {$atom_type == "IOPLL"} {
         ...
         if {[regexp {u_twentynm_tile_ctrl\|pa_core_clk_out\[[0-9]\]$} $node_name]} {
            set result 1
         } else {
            set result 0
         }
...
proc memphy_get_p2c_c2p_clock_uncertainty { instname var_array_name } {
   ...
   if {$read_atom_netlist_error == 0} {
      ...
      if {$success == 1} {
         ...
         } elseif {($vco_period > 3572) || ([string compare -nocase $bw "HIGH"] != 0) || ($mcnt > 8) || ($mcnt < 2)} {
...
proc memphy_get_periphery_clock_uncertainty { results_array_name var_array_name } {
   ...
   set c2p_setup  [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF SETUP C2P]*1e9 ]]
   set c2p_hold   [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF HOLD C2P]*1e9  ]]
   set p2c_setup  [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF SETUP P2C]*1e9 ]]
   set p2c_hold   [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF HOLD P2C]*1e9  ]]
...
proc memphy_get_alternate_clock_uncertainty_parameters { mcnt bw cp_current vco_period } {
    ...
    if {($vco_period <= 1667) || ($vco_period > 3572) || ([string compare -nocase $bw "HIGH"] != 0) || ($mcnt > 8) || ($mcnt < 2)} {

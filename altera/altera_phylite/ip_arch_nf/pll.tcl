...
proc ::altera_phylite::ip_arch_nf::pll::get_pll_vco_to_mem_clk_freq_ratio {} {
      set rate               [get_parameter_value "PHYLITE_RATE_ENUM"]
      set user_afi_ratio [enum_data $rate AFI_RATIO]
	...
	set vco_max $pll_afi_result_array(vco_max)
         ...
         if {!$use_aggressive_vco_fmin} {
            ...
            if {$phy_hmc_clk_ratio == 1} {
               set ratio [expr {$mem_clk_freq_mhz >= 550 ? 1 : -1}]
	       set vco_min 400
            } else {
	...
	if {$mem_clk_freq_mhz >= $vco_min} {
		...
		set ratio 8
	        ...
	        emif_ie "Interface clock frequency $mem_clk_freq_mhz is too low to be supported"
	}
      }
...
proc ::altera_phylite::ip_arch_nf::pll::get_pll_vco_min {} {
      	set family             [enum_data [get_device_family_enum] MEGAFUNC_NAME]
	set speedgrade         [get_parameter_value PHYLITE_PLL_SPEED_GRADE]
	set compensation_mode  direct
	set type               IOPLL
...
proc ::altera_phylite::ip_arch_nf::pll::get_pll_vco_max {} {
      	set family             [enum_data [get_device_family_enum] MEGAFUNC_NAME]
	set speedgrade         [get_parameter_value PHYLITE_PLL_SPEED_GRADE]
	set compensation_mode  direct
	set type               IOPLL
...
proc ::altera_phylite::ip_arch_nf::pll::has_user_half_clk {config_enum} {
...
proc ::altera_phylite::ip_arch_nf::pll::_get_legal_pll_ref_clk_freqs_mhz {
   ...
   for {set i 1} {[llength $retval] < $max_entries} {incr i} {
      ...
      set divisor_is_valid [_is_pll_m_counter_valid $divisor $vco_freq_mhz $pll_compensation_mode $min_vco_div]
...
proc ::altera_phylite::ip_arch_nf::pll::_is_pll_m_counter_valid {
   pll_m \
   vco_freq_mhz \
   pll_comp_mode \
   min_vco_div
...
proc ::altera_phylite::ip_arch_nf::pll::_get_mem_clk_to_slowest_phy_clk_freq_ratio {clk_ratios} {
   return [dict get $clk_ratios C2P_P2C]
}

proc ::altera_phylite::ip_arch_nf::pll::get_pll_output_clocks_info {mem_clk_freq_mhz} {

...
proc make_var_wrapper {params_ref} {
    ...
    set ports_0  {"inaclr" "aclr" "sclr" "addressstall_a"}
    ...
    # ip_name = altsyncram or altera_syncram #
        ...
        # tri_port_list #
        set range_list_0 {"inaclr" "aclr" "sclr" "addressstall_a"}
        ...
        set ports_list_all {"aclr0" "aclr1" "sclr" "address_a" "address2_a" "byteena_a" "clock0" "clocken0" "data_a" "rden_a" "wren_a" "address_b" "address2_b" "addressstall_a" "addressstall_b" "byteena_b" "clock1" "clocken1" "clocken2" "clocken3" "data_b" "eccstatus" "q_b" "rden_b" "wren_b" "eccencbypass" "eccencparity"}
        ...
        foreach port $ports_list_all {
            if {$port eq "clocken0"} {
                ...
                } elseif {[info exists used_port_set(inaclr)]} {
                    lappend port_map_list $port "inaclr"
                } else {
                    lappend ports_not_added_list  $port "1'b0"
                }
                ...
                if {[info exists used_port_set(data)]}  {
                    lappend port_map_list $port "data"
                } else {
                    lappend ports_not_added_list $port "\{$params(width_a)\{1'b1\}\}"
                }
            } elseif {$port eq "address_a"} {
                lappend port_map_list  $port "address"
            } elseif {$port eq "wren_a"} {
                if {[info exists used_port_set(wren)]} {
        ...
        # params_list #
        lappend params_list "address_aclr_a"    ;# address_aclr_a
        if {$params(aclraddr)} {
            lappend params_list "\"CLEAR0\""    
        } else {
            lappend params_list "\"NONE\""
        }
        foreach param  {clock_enable_input_a clock_enable_output_a} {       ;# clock_enable_input_a, clock_enable_output_a
            if {$params($param)} {
        ...
        if {$params(blankmemory)} {             ;# init_file
        ...
        if {!$params(aclraddr)} {
            if {$dualclock } {
                if {!$params(regoutput) } {
                    if {$params(jtag_enabled)} {        ;# lpm_hint
        ...
        if {$params(blankmemory)==0} {              ;# power_up_uninitialized
        ...
        if {$params(ram_block_type) ne "LCs"} {     ;# ram_block_type
	...
	if {$params(force_to_zero)} {
        ...
        lappend params_list "widthad_a" $params(widthad_a)  ;# widthad_a
        lappend params_list "width_a"  $params(width_a)     ;# width_a

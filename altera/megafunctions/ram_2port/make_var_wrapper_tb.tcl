...
proc make_var_wrapper_tb {params_ref} {
        ...
        foreach port $params(port_list) {
           ...
           # switch -exact -- $port {
				...
				if {$port eq "eccencbypass"} {
					lappend module_port_list "eccencbypass"	"IN"	0	0
					lappend module_input_port_list "eccencbypass"	"IN"	0	0
					lappend module_input_port_only_list "eccencbypass"	
				}
				if {$port eq "eccstatus"} {
                                        lappend module_port_list  "eccstatus"  "OUT"   $eccstatus_width    -1  
					lappend module_output_port_only_list  "eccstatus"  
                                }
				if {$port eq "q"} {
    ...
    # ip_name = altsyncram or altera_syncram #
        ...
        # port_map_list #
        set ports_list_all_in   {"aclr1" "sclr" "address_a" "byteena_a" "clock0" "clocken0" "data_a" "rden_a" "wren_a" "aclr0" "address_b" "addressstall_a" "addressstall_b" "wr_addressstall" "rd_addressstall" "byteena_b" "clock1" "clocken1" "clocken2" "clocken3" "data_b" "rden_b" "wren_b"}
        set ports_list_all_out  {"q" "q_b" "q_a" "eccstatus" "eccencbypass" "eccencparity"}
		...
		"eccencbypass" {
			if {[info exists port_map($port)]} {
				lappend port_map_list $port "eccencbypass"
				lappend port_map_input_list $port "eccencbypass"
			} 
			#else {
			#lappend ports_not_added_list $port "1'b0"
			#}
		...
		"eccencparity" {
			if {[info exists port_map($port)]} {
			    lappend port_map_list $port "eccencparity"
			    lappend port_map_input_list $port "eccencparity"
			} 
			#else {
        ...
        if {$params(mode)==0} {         ;# clock_enable_input_a,clock_enable_input_b, clock_enable_output_b
            ...
            if {!$params(byte_enable_a) && ($params(data_width) == $params(q_width))} {
                if {($params(ram_block_type) eq "M20K")} {
                    lappend params_list "enable_ecc"        ;# enable_ecc
                    if {$params(ecc_triple)} {
                        lappend params_list "\"TRUE\""
                        lappend params_list "ecc_pipeline_stage_enabled"    ;# ecc_pipeline_stage_enabled
                        if {$params(ecc_pipeline)} {
                            lappend params_list "\"TRUE\""
                        } else {
                ...
                } elseif {($params(ram_block_type) eq "M144K") && ($params(data_width)<=64)} {
                    lappend params_list "enable_ecc"
                    if {$params(ecc_double)} {
                        lappend params_list "\"TRUE\""
                    } else {
                        lappend params_list "\"FALSE\""
                    }
        ...
        if {$params(lc_implemention_options)} {     ;#implement_in_les
	...
	if {($params(ram_block_type) eq "M20K") && ($params(ecc_double) || $params(ecc_triple))} {		;#ecc enc bypass#
		lappend params_list "enable_ecc_encoder_bypass"
		if {$params(eccencbypass)} { 
	...
	if {$params(coherent_read)} {
		lappend params_list "enable_coherent_read" "\"TRUE\""	
	}
	...
	if {$params(force_to_zero)} {
        ...
        if {$params(mode)==1}  {
            lappend params_list "\"BIDIR_DUAL_PORT\""
        } else {
        ...
        if {$params(ram_block_type) eq "M20K" && $params(ecc_triple)} { ;# width_eccstatus
            lappend params_list "width_eccstatus"  2
        } elseif {$params(ram_block_type) eq "M144K" && $params(ecc_double)} {
            lappend params_list "width_eccstatus"   3
        }
	if {$params(ram_block_type) eq "M20K" && $params(eccencbypass)} {  ;# width_eccencparity
  	    lappend params_list "width_eccencparity" $params(width_eccencparity)
	}
        if {$params(mode)==1} {

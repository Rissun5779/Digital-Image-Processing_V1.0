...
proc make_var_wrapper {params_ref} {
    ...
    foreach port $params(port_list) {
        if {$port eq "clock"} {
			lappend module_port_list [list $port "IN" -1 -1]
			lappend port_map_list $port $port "IN" -1
		} elseif {$port eq "shiftin"} {
            lappend module_port_list [list $port "IN" $top_width -1]
			lappend port_map_list $port $port "IN" $top_width
		} elseif {$port eq "shiftout"} {
            lappend module_port_list [list $port "OUT" $top_width -1]
			lappend port_map_list $port "sub_wire0" "OUT" $top_width
        } elseif {$port eq "taps"} {
			set single_tap 1
            lappend module_port_list [list $port "OUT" $all_taps_top_width -1]
			lappend port_map_list $port "sub_wire1" "OUT" $all_taps_top_width
    ...
    lappend	params_list	"intended_device_family"	"STRING"	"$params(intended_device_family)"
    lappend	params_list	"lpm_hint"					"STRING"	"$params(lpm_hint)"
	lappend	params_list	"lpm_type"					"STRING"	"$params(lpm_type)"
	lappend	params_list	"number_of_taps"			"NATURAL"	"$params(number_of_taps)"
	lappend	params_list	"tap_distance"				"NATURAL"	"$params(tap_distance)"
	lappend	params_list	"width"						"NATURAL"	"$params(width)"
    ...
    set params_terp(ip_name)					"altshift_taps"

...
# +-----------------------------------
# | 
# | $Header: //acds/rel/18.1std/ip/altera_pll/source/top/pll_hw_extra.tcl#1 $
# | 
# +-----------------------------------
...
proc elaboration_callback {} {
    ...
    set enable_phout_division [expr {$dpa_visible && $dpa_enable}]
...
proc interface_ports_and_mapping { flag } {
	...
	set use_non_generic_pll [expr {$en_dps||$en_reconf||$en_refclk_switch||$en_adv||$en_pll_cascade_out||$en_pll_cascade_in||$en_phout||$uses_counter_cascading}]
...
proc validation_param_visibility { } {
	...
	set en_pll_cascade_out [get_parameter_value gui_enable_cascade_out]
	set en_pll_cascade_in [get_parameter_value gui_enable_cascade_in]
	set use_non_generic_pll [expr {$en_dps||$en_reconf||$en_refclk_switch||$adv_params||$en_pll_cascade_out||$en_pll_cascade_in||$en_phout}]
	set op_mode [get_mapped_allowed_range_value gui_operation_mode]
...
proc get_reference_list_items { item values } {
    ...
    if { $item == "speed" } {  
	   ...
	   if {$device_part == "Unknown" || $device_part == ""} {
           set_parameter_property gui_device_speed_grade ENABLED true
           ...
           if {!($device == "StratixV" || $device == "Stratix V" || $device == "Arria V" || $device == "ArriaV" || $device == "CycloneV" || $device == "Cyclone V" || $device == "Arria V GZ")} {
                send_message error "Altera PLL only supports the 28nm device family. For other device families, use Altera IOPLL, ALTPLL or Avalon ALTPLL."
            } elseif {$speed == "1"} {
           ...
           set_parameter_property gui_device_speed_grade ENABLED false
        }
        return $device_part
...
proc my_set_rbc_check { rule_name reference_list } {
	...
	#---NEW IPTCL METHOD---
		set start_v_total [clock clicks -milliseconds]
		set result [parse_tcl_data $rule_name $reference_list]
		set end_v_total [clock clicks -milliseconds]		
	}
...
proc validation_callback {} {
	...
	#---HACK 
	# ensure the output frequency is > 0, if not, return
	if {!$en_adv} {
		for { set i 0 } {$i < $num_count} {incr i} {
			set gui_outclk_freq [get_parameter_value gui_output_clock_frequency$i]
			if {$gui_outclk_freq <= 0} {
				send_message error "Please specify a positive frequency for outclk$i"
				return 
			}
		}
	}
    ...
    if { $ref_clk < $compare_range_min || $ref_clk > $compare_range_max} {
		...
		if {!$en_adv} {
			...
			for { set i 0 } {$i < $MAX_OUTPUT_CLOCKS} {incr i} {
				...
				if {$i < $num_count} {
					...
					for { set another_val $num_count} {$another_val < 18} {incr another_val} {
                        set temp [get_parameter_value gui_actual_phase_shift$another_val]
                        set temp1 [get_mapped_allowed_range_value gui_actual_phase_shift$another_val]
                        if {$temp eq "0" && $temp1 eq "0.0 deg"} {   
                            ## FB 341680
                        } else {
					...
					# must make sure any string parameter that will receive an allowed range when enabled, also has one at start-up
                    set temp [get_parameter_value gui_actual_phase_shift$i]
                    set temp1 [get_mapped_allowed_range_value gui_actual_phase_shift$i]
                    if {$temp eq "0" && $temp1 eq "0.0 deg"} {   
                        ## FB 341680
                    } else {
		...
		if {!$en_adv} {
			...
			#set_parameter_value number_of_cascade_counters 0
		}
...
proc generate_qip {name} {
    ...
    # Case:250551 -- adding QSF assignments to prevent merging of output counters when reconfig or DPS is in use.
    set reconfig [get_parameter_value gui_en_reconf]
    set dps [get_parameter_value gui_en_dps_ports]
    if {$reconfig || $dps} {
        set no_merge_mode "ON"
        append result "set_instance_assignment -name UNFORCE_MERGE_PLL_OUTPUT_COUNTER $no_merge_mode -to \"$path\"\n"
    }
...
proc generate_simgen_model_file {file_list temp_dir family top_level_name file_type blackbox_list} {
	...
	set cmd [list [file join ${qbindir} quartus_map] "${top_level_name}.qpf" "--simgen" "--ini=disable_check_quartus_compatibility_qsys_only=on" "--simgen_parameter=${init_arg}CBX_HDL_LANGUAGE=$file_type"]
	...
	if {[string compare $bb_arg ""] == 0} {
		puts $fh "catch \{ eval \[list exec \"[file join ${qbindir} quartus_map]\" \"${top_level_name}.qpf\" \"--simgen\" \"--ini=disable_check_quartus_compatibility_qsys_only=on\" \{--simgen_parameter=${init_arg}CBX_HDL_LANGUAGE=$file_type\} \]\} temp"
	} else {
		puts $fh "catch \{ eval \[list exec \"[file join ${qbindir} quartus_map]\" \"${top_level_name}.qpf\" \"--simgen\" \"--ini=disable_check_quartus_compatibility_qsys_only=on\" \{--simgen_parameter=${init_arg}CBX_HDL_LANGUAGE=$file_type\} \{$bb_arg\} \]\} temp"
	}
	puts $fh "puts \$temp"
	close $fh
	...
	if {[regexp -nocase {Quartus.*Analysis \& Synthesis was successful} $resultout junk] == 0} {
...
#    set phys_params_file [open "phys_params_file.txt" w]
    ...
    foreach item $param_list {
        ...
        if {$name=="FRACTIONAL_PLL_PLL_M_CNT_HI_DIV"} {
        ...
        if {$name=="FRACTIONAL_PLL_PLL_M_CNT_LO_DIV"} {
            # hi_div/lo_div = 256 only when bypass, set it to 0
            # since we only have a byte
            if {$value == 256} {
                set value 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_N_CNT_HI_DIV"} {
            # hi_div/lo_div = 256 only when bypass, set it to 0
            # since we only have a byte
            if {$value == 256} {
                set value 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_N_CNT_LO_DIV"} {
            # hi_div/lo_div = 256 only when bypass, set it to 0
            # since we only have a byte
            if {$value == 256} {
                set value 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_M_CNT_BYPASS_EN"} {
            if {$value=="true"} {
                set m_bypass_en 1
            } else {
                set m_bypass_en 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_N_CNT_BYPASS_EN"} {
            if {$value=="true"} {
                set n_bypass_en 1
            } else {
                set n_bypass_en 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_M_CNT_ODD_DIV_DUTY_EN"} {
            if {$value=="true"} {
                set m_odd_div_en 1
            } else {
                set m_odd_div_en 0
        ...
        if {$name=="FRACTIONAL_PLL_PLL_N_CNT_ODD_DIV_DUTY_EN"} {
            if {$value=="true"} {
                set n_odd_div_en 1
            } else {
                set n_odd_div_en 0
        ...
        if {$name == "FRACTIONAL_PLL_PLL_CP_CURRENT"} {
            if {$value == 5} {
                set cp_bits "000"
            }
            if {$value == 10} {
        ...
        if {$name == "FRACTIONAL_PLL_PLL_BWCTRL"} {
            if {$value == 18000} {
                set bw_bits "0000"
            }
            if {$value == 16000} {
...
#        puts $phys_params_file "Item [lindex $item 2], name = [lindex $item 2], old_value=$old_value, value = $value"
    ...
    incr addr_num
	puts $mif_file "$addr_num : [string repeat "0" 28]$bw_bits;"
    incr addr_num

        ...
        foreach param $phys_params_list {
            ...
            if {![string compare $param_rbc_name $rbc_name]} {
                if {[is_legal_value $param_value]} {
					...
					} elseif {$param_name == "pll_bwctrl"} {
                        if {$advanced_param_used} {
							set m_counter [get_parameter_value gui_multiply_factor]
							set user_bw_preset [get_parameter_value gui_pll_bandwidth_preset]
                            set param_value [get_bandwidth_and_cp_current "bandwidth" $m_counter $user_bw_preset]
                        }
					} elseif {$param_name == "pll_cp_current"} {
                        if {$advanced_param_used} {
							set m_counter [get_parameter_value gui_multiply_factor]
							set user_bw_preset [get_parameter_value gui_pll_bandwidth_preset]
                            set param_value [get_bandwidth_and_cp_current "cp_current" $m_counter $user_bw_preset]
                        }
                    }
...
# ALTERA_HACK -- this lookup table should not be implemented in Tcl!
# However, given the current implementation of physical outclk mode, this was the only
# easy way to fix case 178006.
proc get_bandwidth_and_cp_current { parameter  m  user_bw_setting} {

	set bw 0
	set cp 0

	# Look up the BW and CP current settings in a lookup table
	switch $user_bw_setting {
		Low {
			if {$m <= 2} {
				set bw 2000
				set cp 5
	...
	# Return the bandwidth or CP current, depending on the parameter requested
	if {$parameter == "bandwidth"} {
		return $bw
	} elseif {$parameter == "cp_current"} {
		return $cp
	} else {
		return 0
	}

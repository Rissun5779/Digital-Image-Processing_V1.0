...
module testbench ();
  ...
  wire reg_waitrequest_embed_avalon;
  wire reg_waitrequest_extract_avalon;
  reg reg_write_extract;
  reg reg_write_embed;
  reg reg_write_cai;
  reg reg_write_cao;
  reg reg_write_embed_avalon;
  reg reg_write_extract_avalon;
  reg reg_read_extract;
  reg reg_read_embed;
  reg reg_read_cai;
  reg reg_read_cao;
  reg reg_read_embed_avalon;
  reg reg_read_extract_avalon;
  wire reg_readdatavalid_extract;
  wire reg_readdatavalid_embed;
  wire reg_readdatavalid_embed_avalon;
  ...
  wire [7:0] reg_readdata_embed_avalon;
  ...
  reg [19:0] vid_embed_out_clean;
  wire aud_extract_clk_out;
  wire aud_extract_de;
  wire aud_extract_ws;
  wire [0:0] aud_extract_data;
  reg [0:0] aud_extract_data_clean;
  wire aes_avalon_ws;
...
//------------------------------------------------------------------------------
  ...
  assign sim_vid_std_rate = (G_TEST_STD == 0 || G_TEST_STD == 3) ? 1'b0 : 1'b1;
  ...
  always @(vid_embed_out)
  begin : proc_vid_embed_out_clean
  integer B;
    for (B = 0; B < 20; B = B + 1) begin
      if (vid_embed_out[B] == 1'b1) begin
        vid_embed_out_clean[B] <= 1'b1;
      end else begin
        vid_embed_out_clean[B] <= 1'b0;
      end
    end
  end
  ...
  always @(aud_extract_data)
  begin : proc_aud_extract_data_clean
  integer B;
    for (B = 0; B < 1; B = B + 1) begin
      if (aud_extract_data[B] == 1'b1) begin
        aud_extract_data_clean[B] <= 1'b1;
      end else begin
        aud_extract_data_clean[B] <= 1'b0;
      end
    end
  end
  ...
  if (G_INCLUDE_AVALON_ST == 1) begin : g_cai_source 
    ...
    for (I = 0; I <= (2*C_AUDEMB_NUM_GROUPS)-1; I = I + 1) begin
      clocked_audio_input_top u_clocked_audio_input_0(
        ...
        .aes_data                (aud_extract_data_clean),
  ...
  if (G_INCLUDE_AVALON_ST == 1) begin : g_check_aes_avalon   
    ...
    initial
      ...
      $display("Write Embed SD EDP Register");
      avalon_write_embed(8'h03, 8'hFF);

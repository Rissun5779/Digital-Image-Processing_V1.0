    ...
    begin
      ...
      if (vid_en == 1'b1) begin
        if (enable_anc == 1'b1 && anc_insert == 1'b1 && (vid_3gb == 1'b0 || vid_ync == 1'b1) && vid_std[27:24] != `C_INTERFACE_SD_270) begin
          ...
          if (vid_std[27:24] == `C_INTERFACE_720_3G || vid_std[27:24] == `C_INTERFACE_1080_3G) begin
            if (anc_checksum_en == 1'b1) begin
          ...
          end else if (vid_std[27:24] == `C_INTERFACE_DL_3G) begin
            if (anc_checksum_en == 1'b1) begin
              vid_out_data[9:0] <= {anc_checksum[8] , (~ anc_checksum[8]) , (anc_checksum[7:0] + 8'b01000000)};
            end else if (anc_parity_en == 1'b1) begin
              if (anc_count == 9) begin
                vid_out_data[9:0] <= {anc_parity , (~ anc_parity) , (anc_data[7:0] | 8'b01000000)};
              end else begin
                vid_out_data[9:0] <= {(~ anc_parity) , anc_parity , anc_data[7:0]};
              end
            end else begin

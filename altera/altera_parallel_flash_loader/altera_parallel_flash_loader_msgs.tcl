...
# +-----------------------------------
# | 
# | $Header: //acds/rel/18.1std/ip/altera_parallel_flash_loader/altera_parallel_flash_loader_msgs.tcl#1 $
# | 
# +-----------------------------------
...
# --- Message definition --- #
set	PFL_NAND_TWO_PERCENT_NRB_ERROR_MSG			"2% of NAND flash memory density for must be reserved for reserved block"
set	PFL_NAND_ADDRESS_ERROR_MSG					"NAND flash reserved block address must fall on an 128K byte address boundary"
set	PFL_NAND_ADDRESS_RANGE_ERROR_MSG			"NAND flash reserved block address must fall within the flash address boundary"	
...
set	PFL_FAILURE_FREQ_1_ERROR_MSG				"FPGA DCLK frequency must be less than 125 MHz "
set	PFL_FAILURE_NO_FREQ_ERROR_MSG				"External clock frequency must not equal to 0 MHz "	


# +-----------------------------------
# | NAND Flash Error Checking
# +-----------------------------------
proc check_for_nand_error { nand_size } {
	global	NAND_BOUNDARY
	global	PFL_NAND_TWO_PERCENT_NRB_ERROR_MSG
	global	PFL_NAND_ADDRESS_ERROR_MSG
	global	PFL_NAND_ADDRESS_RANGE_ERROR_MSG
	
	set	status			"true"
	set	guarded_size	[calculate_nand_guarded_size	$nand_size]
	set	nrb_addr		[get_parameter_value			nflash_rb_edit]
	
	if { $nrb_addr >= $nand_size } {
		send_message ERROR $PFL_NAND_ADDRESS_RANGE_ERROR_MSG
		set	status		"false"
	}
	
	if { ($status eq "true") && ($nrb_addr >= $guarded_size) } {
		send_message ERROR $PFL_NAND_TWO_PERCENT_NRB_ERROR_MSG
		set	status		"false"
	}
	
	if { ($status eq "true") && ([expr	$nrb_addr % $NAND_BOUNDARY] != 0) } {
		send_message ERROR $PFL_NAND_ADDRESS_ERROR_MSG
		set	status		"false"
	}
}
...
proc check_for_fpga_conf_error {pfl_flash_data_width	pfl_address_width	pfl_num_flash	pfl_qflash_byte_size	operating_modes_cfg} {
	...
	set	safe_mode_revert_addr	[get_parameter_value		safe_mode_revert_edit]
	...
	if { $operating_modes_cfg == 1 } {
		# cschai: This instead of checking each DIVISOR and each PFL Freq, we calculated the final DCLK and do comparison
		if { [calculate_dclk]	> 125 } {
			send_message ERROR $PFL_FAILURE_FREQ_1_ERROR_MSG
			set	status	"false"
		}

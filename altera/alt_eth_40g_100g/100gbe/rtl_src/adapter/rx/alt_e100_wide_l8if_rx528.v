...
if (arst)
  ...
  begin
     ...
     if      ( (|rx5l_sop_q) & (~(|rx5l_eop_bm_q)))
       rxi_in_packet <= 1;
     else if      ( (|rx5l_sop_q[3:0]) & ((|rx5l_eop_bm_q[39:32])))
       rxi_in_packet <= 1;
     else if      ( (|rx5l_sop_q[2:0]) & ((|rx5l_eop_bm_q[39:24])))
       rxi_in_packet <= 1;
     else if      ( (|rx5l_sop_q[1:0]) & ((|rx5l_eop_bm_q[39:16])))
       rxi_in_packet <= 1;
     else if      ( (rx5l_sop_q[0]) & ((|rx5l_eop_bm_q[39:8])))
       rxi_in_packet <= 1;
     else if ((~(|rx5l_sop_q)) &  (|rx5l_eop_bm_q))
       rxi_in_packet   <= 0;
  end
...
generate
	for (i=0; i<WORDS; i=i+1) begin : split_fcs_and_data
		always @ (posedge clk_rxmac) begin
			...
			<= {dout [(i+1)*(WORD_LEN+2)-1], dout [(i+1)*(WORD_LEN+2)-3:i*(WORD_LEN+2)]};
		end
	end
endgenerate

always @(posedge clk_rxmac) begin
    case(num_dout_words_valid)    
    4'h0 : rx8l_fcs_error <= 1'b0;
    4'h1 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2];
    4'h2 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2];
    4'h3 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[6*(WORD_LEN+2)-2];
    4'h4 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[5*(WORD_LEN+2)-2];
    4'h5 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[5*(WORD_LEN+2)-2] || dout[4*(WORD_LEN+2)-2];
    4'h6 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[5*(WORD_LEN+2)-2] || dout[4*(WORD_LEN+2)-2] || dout[3*(WORD_LEN+2)-2];
    4'h7 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[5*(WORD_LEN+2)-2] || dout[4*(WORD_LEN+2)-2] || dout[2*(WORD_LEN+2)-2];
    4'h8 : rx8l_fcs_error <= dout[8*(WORD_LEN+2)-2] || dout[7*(WORD_LEN+2)-2] || dout[5*(WORD_LEN+2)-2] || dout[4*(WORD_LEN+2)-2] || dout[1*(WORD_LEN+2)-2];
    default : rx8l_fcs_error <= 1'b0;
    endcase
end

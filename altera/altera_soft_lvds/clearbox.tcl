...
proc do_clearbox_gen {ip_name param_file var_file} {
	...
	## TODO need figure out if in windows its bin64, linux bin
    #set cbx_exec [file join "$::env(QUARTUS_ROOTDIR)" "bin64" "clearbox"] 
	set cbx_exec [file join $::quartus(binpath) "clearbox"] 
    # Create the clearbox command as a list of arguments
    set cbx_cmd [ list $cbx_exec $ip_name -f $param_file CBX_FILE="${var_filename}" CBX_OUTPUT_DIRECTORY="${var_dir}" CBX_DECLARE_ALL_CONNECTED_PORTS="OFF"]
	#set cbx_cmd [ list $cbx_exec $ip_name NUMBER_OF_CHANNELS 1 DESERIALIZATION_FACTOR 4 DEVICE_FAMILY CYCLONE III rx_in rx_inclock rx_out rx_outclock CBX_FILE="${var_filename}" CBX_OUTPUT_DIRECTORY="${var_dir}" ]

    # Execute clearbox
    set status [catch {exec $cbx_cmd} err]
	
	########################### debug messages ##########################
	#send_message info "status $status"
	#send_message info "ip_name $ip_name"
	#send_message info "var_dir $var_dir"
	#send_message info "var_filename $var_filename"
	send_message info "err$err"
	#####################################################################

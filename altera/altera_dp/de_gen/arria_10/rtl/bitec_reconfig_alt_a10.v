...
assign COUNTER_L_PFD[3] = A10_ES2 ? 3'h3  : 3'h2;  // 8.1G
...
assign M_COUNTER_FPLL[3] = A10_ES2 ? 8'h1e : 8'h1e; // 8.1G
...
begin
  ...
  begin
    ...
    tx_xcvr_reset <= (fsm_state == FSM_END) ? 1'b0 : (tx_new_linkrate ?  1'b1 : tx_xcvr_reset);
...
#(
  ...
  parameter RX	 = 0			// Usage: 0 = Transmitter, 1 = Receiver
)
(
...
wire [31:0] xcvr_resister_pd;
wire [31:0] xcvr_chgpmp_current_up_pd;
wire [31:0] xcvr_chgpmp_current_dn_pd;

wire [1:0] LF_RESISTOR_PD[3:0];
wire [2:0] CHGPMP_CURRENT_UP_PD [3:0];
wire [2:0] CHGPMP_CURRENT_DN_PD [3:0];
assign LF_RESISTOR_PD[0]	= 2'h1;  // 1.62G
assign LF_RESISTOR_PD[1]	= 2'h0;  // 2.7G
assign LF_RESISTOR_PD[2]	= 2'h0;  // 5.4G
assign LF_RESISTOR_PD[3]	= 2'h0;  // 8.1G
assign CHGPMP_CURRENT_UP_PD[0]	= 3'h3;  // 1.62G
assign CHGPMP_CURRENT_UP_PD[1]	= 3'h4;  // 2.7G
assign CHGPMP_CURRENT_UP_PD[2]	= 3'h4;  // 5.4G
assign CHGPMP_CURRENT_UP_PD[3]	= 3'h4;  // 8.1G
assign CHGPMP_CURRENT_DN_PD[0]	= 3'h3;  // 1.62G
assign CHGPMP_CURRENT_DN_PD[1]	= 3'h4;  // 2.7G
assign CHGPMP_CURRENT_DN_PD[2]	= 3'h4;  // 5.4G
assign CHGPMP_CURRENT_DN_PD[3]	= 3'h4;  // 8.1G

assign xcvr_resister_pd = (RX) ? ((mgmt_address == ADDR_XCVR_LF_PD_PFD) ? ({mgmt_readdata[31:4],LF_RESISTOR_PD[rcnf_linkrate],mgmt_readdata[1:0]}) : mgmt_readdata) : mgmt_readdata;
assign xcvr_chgpmp_current_up_pd = (RX) ? ((mgmt_address == ADDR_XCVR_CHGPMP_PD_UP) ? ({mgmt_readdata[31:8],CHGPMP_CURRENT_UP_PD[rcnf_linkrate],mgmt_readdata[4:0]}) : mgmt_readdata) : mgmt_readdata;
assign xcvr_chgpmp_current_dn_pd = (RX) ? ((mgmt_address == ADDR_XCVR_CHGPMP_PD_DN) ? ({mgmt_readdata[31:6],CHGPMP_CURRENT_DN_PD[rcnf_linkrate],mgmt_readdata[2:0]}) : mgmt_readdata) : mgmt_readdata;
    ...
    case (state)
      ...
      FSM_SCAL_RD10:
        begin
          if(mgmt_read & !mgmt_waitrequest) 
          begin
            calib_res[rcnf_linkrate*CALIB_VALUES+rcnf_logical_ch*CALIB_RATES*CALIB_VALUES+2] <= (xcvr_chgpmp_current_up_pd[7:0] & MASK_XCVR_CHGPMP_PD_UP[7:0]);
            state <= FSM_SCAL_RD11;
          end
          else
          begin
      ...
      FSM_SCAL_RD11:
        begin
          if(mgmt_read & !mgmt_waitrequest) 
          begin
            calib_res[rcnf_linkrate*CALIB_VALUES+rcnf_logical_ch*CALIB_RATES*CALIB_VALUES+3] <= (xcvr_chgpmp_current_dn_pd[7:0] & MASK_XCVR_CHGPMP_PD_DN[7:0]);
            state <= FSM_SCAL_RD12;
          end
          else
          begin
      ...
      FSM_SCAL_RD12:
        begin
          if(mgmt_read & !mgmt_waitrequest) 
          begin
            calib_res[rcnf_linkrate*CALIB_VALUES+rcnf_logical_ch*CALIB_RATES*CALIB_VALUES+4] <= (xcvr_resister_pd[7:0] & MASK_XCVR_LF_PD_PFD[7:0]);
            state <= FSM_SCAL_RD13;
          end
          else
          begin

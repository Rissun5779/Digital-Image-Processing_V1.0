...
void bitec_dptx_init()
{
  ...
  tx_link_rate = DP_TX_DP_SOURCE_BITEC_CFG_TX_MAX_LINK_RATE;
  tx_lane_count = DP_TX_DP_SOURCE_BITEC_CFG_TX_MAX_LANE_COUNT;
  ...
  IOWR(DP_TX_XDASH_BASE,0, dash_setup);

#if BITEC_TX_CAPAB_MST
  btc_dptxll_syslib_add_tx(0,tx_link_rate,tx_lane_count,DP_TX_DP_SOURCE_BITEC_CFG_TX_MAX_NUM_OF_STREAMS,tx_edid_data);
  ...
  alt_ic_isr_register(DP_TX_DP_SOURCE_IRQ_INTERRUPT_CONTROLLER_ID,
                      DP_TX_DP_SOURCE_IRQ,
                      bitec_dptx_hpd_isr,
                      NULL,
                      0x0);
...
void bitec_dptx_linktrain()
{
  ...
  link_rate = (IORD(DP_TX_XDASH_BASE,0) >> 21) & 0xFF;
  lane_count = (IORD(DP_TX_XDASH_BASE,0) >> 5) & 0x1F;
  bpc = (IORD(DP_TX_XDASH_BASE,0) >> 2) & 0x07;
...
void bitec_dptx_hpd_isr(void* context)
{
  ...
  mask = bitec_alt_irq_interruptible(DP_TX_DP_SOURCE_IRQ);

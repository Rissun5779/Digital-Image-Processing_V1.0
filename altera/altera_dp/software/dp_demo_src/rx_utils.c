...
// Revision       : $Revision: #1 $
// URL            : $URL: svn://nas-bitec-fi/dp/trunk/software/dp_demo/rx_utils.c $
//
// Description:
//
// ********************************************************************************
...
void bitec_dprx_init()
{
...
#if 0
  // Set the capabilities as DP 1.2
  ...
  // Set the capabilities (as DP 1.2)
  data[0]  = 0x12; // DPCD_ADDR_DPCD_REV
  data[1]  = (rx_link_rate >= 0x14) ? 0x14 : rx_link_rate; // DPCD_ADDR_MAX_LINK_RATE
  data[2]  = 0x80 | (BITEC_RX_LQA_SUPPORT << 5) | rx_lane_count; // DPCD_ADDR_MAX_LANE_COUNT
  if(rx_link_rate >= 0x14)
    data[2]  |= 0x40; // Set TPS3_SUPPORTED
  data[3]  = 0x01 | (BITEC_RX_FAST_LT_SUPPORT << 6); // DPCD_ADDR_MAX_DOWNSPREAD
  ...
  data[12] = 0x00; // DPCD_ADDR_I2C_SPEED
  data[13] = 0x00; // DPCD_ADDR_EDP_CONFIGURATION_CAP
  data[14] = 0x80; // DPCD_ADDR_TRAINING_AUX_RD_INTERVAL
  data[15] = 0x00; // DPCD_ADDR_ADAPTER_CAP
  btc_dprx_dpcd_gpu_access(0,1,DPCD_ADDR_DPCD_REV,16,data);
  ...
  data[0]  = 0x00; // DPCD_ADDR_EDP_DISPLAY_CONTROL
  btc_dprx_dpcd_gpu_access(0,1,DPCD_ADDR_EDP_DISPLAY_CONTROL,1,data);

  // Init the EDID(s)
  for(i=0; i< DP_RX_MGMT_BITEC_CFG_RX_MAX_NUM_OF_STREAMS; i++)
  {
#if BITEC_EDID_800X600_AUDIO

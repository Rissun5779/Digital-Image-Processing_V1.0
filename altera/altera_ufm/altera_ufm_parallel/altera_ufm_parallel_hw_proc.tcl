...
# +-----------------------------------
# | 
# | $Header: //acds/rel/18.1std/ip/altera_ufm/altera_ufm_parallel/altera_ufm_parallel_hw_proc.tcl#1 $
# | 
# +-----------------------------------
...
# +--------------------------------------
	#addr width
	set set_addr_width [get_parameter_value WIDTH_ADDRESS]
	add_interface_port addr addr addr Input $set_addr_width

	#dataout width
	set set_databus_width [get_parameter_value WIDTH_DATA]	
	add_interface_port dataout dataout dataout Output $set_databus_width
...
# +---------------------------------------
	...
	if {$set_read_write == "READ_WRITE"} {
		add_interface datain conduit end
		add_interface_port datain datain datain Input $set_databus_width
		set_interface_assignment datain "ui.blockdiagram.direction" INPUT
		set_interface_property datain ENABLED true	
		
		add_interface nwrite conduit end
		add_interface_port nwrite nwrite nwrite Input 1
		set_interface_assignment nwrite "ui.blockdiagram.direction" INPUT
		set_interface_property nwrite ENABLED true
		
		add_interface nerase conduit end
		add_interface_port nerase nerase nerase Input 1
		set_interface_assignment nerase "ui.blockdiagram.direction" INPUT
		set_interface_property nerase ENABLED true
	} 
...
proc update_device_type_params {} {
	...
	if {!($get_device_family == "MAX V" || $get_device_family == "MAX II")} {
		send_message error "altera_ufm_parallel does not support selected device family $get_device_family"
	}

...
# +-----------------------------------
# | 
# | $Header: //acds/rel/18.1std/ip/altera_ufm/altera_ufm_spi/altera_ufm_spi_hw.tcl#1 $
# | 
# +-----------------------------------
...
# Source files
source altera_ufm_spi_hw_proc.tcl

# +-----------------------------------
# | module UFM SPI
# +-----------------------------------
set_module_property NAME altera_ufm_spi
set_module_property VERSION 18.1
set_module_property DISPLAY_NAME "User Flash Memory for SPI Interface Protocol Intel FPGA IP"
set_module_property DESCRIPTION "The Altera User Flash Memory for SPI Interface Protocol megafunction implements user \
									non-volatile memory for MAX II/Max V devices using the SPI interface protocol."
...
add_display_item "General" "Configuration mode" GROUP
...
set_parameter_property ACCESS_MODE UNITS None
...
set_parameter_property ACCESS_MODE AFFECTS_ELABORATION false
add_display_item "Access mode" ACCESS_MODE parameter

#set config mode
add_parameter CONFIG_MODE STRING 
set_parameter_property CONFIG_MODE DEFAULT_VALUE "BASE"
set_parameter_property CONFIG_MODE DISPLAY_NAME "Configuration mode"
set_parameter_property CONFIG_MODE DESCRIPTION "Specifies the configuration mode for the SPI interface protocol." 
set_parameter_property CONFIG_MODE TYPE STRING
set_parameter_property CONFIG_MODE DISPLAY_HINT "radio"
set_parameter_property CONFIG_MODE ALLOWED_RANGES {"BASE:Base mode (8 bit address and data)" "EXTENDED:Extended mode (16 bit address and data)"} 
set_parameter_property CONFIG_MODE AFFECTS_GENERATION true
set_parameter_property CONFIG_MODE AFFECTS_ELABORATION false
add_display_item "Configuration mode" CONFIG_MODE parameter
...
# Set BYTE_OF_PAGE_WRITE
add_parameter BYTE_OF_PAGE_WRITE INTEGER 
set_parameter_property BYTE_OF_PAGE_WRITE DEFAULT_VALUE 8
set_parameter_property BYTE_OF_PAGE_WRITE TYPE INTEGER
set_parameter_property BYTE_OF_PAGE_WRITE AFFECTS_GENERATION true
set_parameter_property BYTE_OF_PAGE_WRITE VISIBLE false


# +-----------------------------------
# | static connection point - input
# +-----------------------------------
add_interface si conduit end
add_interface_port si si si Input 1
set_interface_assignment si "ui.blockdiagram.direction" INPUT
set_interface_property si ENABLED true

add_interface sck conduit end
add_interface_port sck sck sck Input 1
set_interface_assignment sck "ui.blockdiagram.direction" INPUT
set_interface_property sck ENABLED true

add_interface ncs conduit end
add_interface_port ncs ncs ncs Input 1
set_interface_assignment ncs "ui.blockdiagram.direction" INPUT
...
# +----------------------------------- 
add_interface so conduit start
add_interface_port so so so Output 1
set_interface_assignment so "ui.blockdiagram.direction" OUTPUT
...
proc do_quartus_synth {output_name} {
    ...
    set status      [do_clearbox_gen altufm_spi $cbx_param_file $cbx_var_file]
    if {$status eq "false"} {

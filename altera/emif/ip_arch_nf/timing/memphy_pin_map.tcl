      ...
      if {$success == 1} {
         ...
         if {$vco_period <= 1667} {
           ...
           (($mcnt > 8) && ($vco_period > 2500))                         
            ...
            set clk_uncertainty_params [memphy_get_alternate_clock_uncertainty_parameters $mcnt $bw $cp_current $vco_period]
...
proc memphy_get_odv_reduction_factor {master_pll_location inst phyclk_name} {
...
proc memphy_get_a10_iopll_workaround_present {} {
...
proc memphy_get_periphery_clock_uncertainty { results_array_name var_array_name } {
   ...
   set c2p_setup  [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF SETUP C2P]*1e9 + ($var(PHY_REF_CLK_JITTER_PS) - 10.0)/1000.0]]
   set c2p_hold   [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF HOLD C2P]*1e9  + ($var(PHY_REF_CLK_JITTER_PS) - 10.0)/1000.0]]
   set p2c_setup  [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF SETUP P2C]*1e9 + ($var(PHY_REF_CLK_JITTER_PS) - 10.0)/1000.0]]
   set p2c_hold   [memphy_round_3dp [expr [get_clock_uncertainty_data 900MV $speed_temp_grade EMIF HOLD P2C]*1e9  + ($var(PHY_REF_CLK_JITTER_PS) - 10.0)/1000.0]]
...
proc memphy_get_alternate_clock_uncertainty_parameters { mcnt bw cp_current vco_period } {
    ...
    set SPE  [expr [memphy_get_spe_from_cp_current $cp_current] + 25e-12]

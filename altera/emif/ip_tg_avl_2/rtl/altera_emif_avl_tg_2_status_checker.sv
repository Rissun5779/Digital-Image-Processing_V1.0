...
module altera_emif_avl_tg_2_status_checker # (
   ...
   logic [ADDR_WIDTH-1:0] current_written_addr_rr;
         ...
         if (tg_restart) begin
            ...
            for (int byte_num = 0; byte_num < BE_WIDTH; byte_num++) begin
               for (int bit_num = 0; bit_num < BYTE_SIZE; bit_num++) begin
                  if (enable && rdata_valid_rr) begin
   ...
   begin
      if (tg_restart) begin
         ...
         pass <=  (&pnf_per_bit_persist) & all_tests_issued & ~(reads_in_prog);
   ...
   assign fail = ~(&pnf_per_bit_persist) & (TEST_DURATION == "INFINITE" ? 1'b1 : all_tests_issued);
   assign timeout = &timeout_count;

   //it takes 2 cycles to enable and fetch the next written data and do the comparison
   //third stage is needed to hold the data to output it if compare fails
   always_ff @(posedge clk, posedge rst)
   begin
      if (rst) begin
         rdata_valid_r             <= '0;
         ...
         current_written_addr_rr   <= current_written_addr_r;
      end
   end
   ...
   begin
      ...
      begin
         ...
         $display("     At avalon address: %h", current_written_addr_rr);
      end
   end
   ...
   begin
      if (~first_failure_occured) begin
         first_fail_addr <= current_written_addr_rr;
         first_fail_expected_data <= written_data_r & written_be_full_r;
         before_ff_expected_data <= written_data_rr & written_be_full_rr;
         after_ff_expected_data <= written_data & written_be_full;
         first_fail_read_data <= rdata_rrr & written_be_full_r;
         before_ff_read_data  <= rdata_rrrr & written_be_full_rr;
         after_ff_read_data   <= rdata_rr & written_be_full;
         first_fail_pnf <= pnf_per_bit;
         before_ff_rdata_valid <= rdata_valid_rrrr;
         after_ff_rdata_valid <= rdata_valid_rr;
      end
   end
   ...
   begin
      if (rst) begin
         ...
         if (~(&pnf_per_bit)) begin
            if (~(&failure_count)) begin

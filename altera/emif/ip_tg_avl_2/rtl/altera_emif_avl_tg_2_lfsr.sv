...
module altera_emif_avl_tg_2_lfsr # (
   ...
   output logic [WIDTH-1:0]   init_val
);
   timeunit 1ns;
   timeprecision 1ps;
   ...
   assign init_val = {SEED[WIDTH-1:0]};
   ...
   begin
      if (rst) begin
         ...
         if (tg_start) begin
            data[WIDTH-1] <= init_val[0];
            data[WIDTH-2:0] <= init_val[WIDTH-1:1] ~^ (~taps[WIDTH-2:0] | {WIDTH-1{init_val[0]}});
   ...
   initial
   begin
      assert (WIDTH >= 2 && WIDTH <= 36) else $error ("Invalid LSFR width");
   end

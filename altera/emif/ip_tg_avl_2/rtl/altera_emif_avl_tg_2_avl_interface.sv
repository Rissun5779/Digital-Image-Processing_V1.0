...
module altera_emif_avl_tg_2_avl_if # (
   ...
   // For protocols with separate read/write interfaces (QDRII)
   input [AMM_WORD_ADDRESS_WIDTH-1:0] mem_addr_w,
   input amm_ctrl_ready_w,
   output [BYTE_ADDR_WIDTH-1:0] amm_ctrl_address_w,
   output [AMM_BURSTCOUNT_WIDTH-1:0] amm_ctrl_burstcount_w,
   output controller_ready_w,
   ...
   assign controller_ready_w         = amm_ctrl_ready_w & ~read_addr_fifo_full;
   ...
   assign amm_ctrl_burstcount_w      = burstlength;

   //generate correctly aligned address
   assign amm_ctrl_address           = {mem_addr, {(BYTE_ADDR_WIDTH - AMM_WORD_ADDRESS_WIDTH){1'b0}}};
   assign amm_ctrl_address_w         = {mem_addr_w, {(BYTE_ADDR_WIDTH - AMM_WORD_ADDRESS_WIDTH){1'b0}}};

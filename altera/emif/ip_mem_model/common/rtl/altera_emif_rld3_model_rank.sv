...
//
///////////////////////////////////////////////////////////////////////////////
module altera_emif_rld3_model_rank #(
...
localparam NUM_BANKS = 2**PORT_MEM_BA_WIDTH;
...
typedef struct packed {
	bit [PORT_MEM_A_WIDTH+PORT_MEM_BA_WIDTH-1:0] address;
...
typedef struct {
	...
	bit [PORT_MEM_BA_WIDTH-1:0] bank;
} command_struct;
...
bit [PORT_MEM_A_WIDTH-1:0] address_queue[$];
bit [PORT_MEM_BA_WIDTH-1:0] bank_queue[$];
...
reg [PORT_MEM_DM_WIDTH - 1:0]	mem_dm_captured;

time mem_ck_time;
time mem_dk_time[PORT_MEM_DK_WIDTH];
...
generate
	...
	begin
		...
		for (j = 0; j < (PORT_MEM_DQ_WIDTH / PORT_MEM_DK_WIDTH / 9); j++)
		begin
			always @(posedge mem_dk[i] or posedge mem_dk_n[i])
			begin
...
task automatic write_command;
	...
	bit [PORT_MEM_BA_WIDTH-1:0] mem_ba_stripped;
	
	int num_banks_to_write;
	int bank_count;
	bit [PORT_MEM_BA_WIDTH-1:0] bank_offset;
	
	if (write_protocol == 0)
		mem_ba_stripped = mem_ba;
	else if (write_protocol == 1)
		mem_ba_stripped = {1'b0, mem_ba[PORT_MEM_BA_WIDTH-2:0]};
	else
		mem_ba_stripped = {2'b00, mem_ba[PORT_MEM_BA_WIDTH-3:0]};
...
task init_guaranteed_write;
	...
	begin
		write_memory({addr, bank}, i, {(PORT_MEM_DQ_WIDTH/9){9'b101010101}}, '0);
	end
	...
	for (i = 0; i < burst_length; i++)
	begin
		write_memory({addr, bank}, i, {(PORT_MEM_DQ_WIDTH/9){9'b010101010}}, '0);
	end
...
task automatic check_violations;
	...
	if (new_command.command != NOP_COMMAND) begin
		...
		if ((clock_cycle - banks[new_command.bank].last_write_cycle) < tRC_cycles) begin
			$display("[%0t] [DW=%0d%0d]: ERROR: Bank %0d tRC violated against last write command %0d %0d %0d", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, new_command.bank,clock_cycle,banks[new_command.bank].last_write_cycle,tRC_cycles );
			$finish(1);
		end
...
task write_memory(
	input [PORT_MEM_A_WIDTH+PORT_MEM_BA_WIDTH-1:0] mem_address, 
	input int burst_num,
	input [PORT_MEM_DQ_WIDTH-1:0] write_data,
	input [PORT_MEM_DM_WIDTH-1:0] data_mask);
	
	address_burst_type address_burst;
	bit [PORT_MEM_DQ_WIDTH - 1:0] masked_data;
	bit [PORT_MEM_DQ_WIDTH - 1:0] orig_data;
	int i;
	int j;
	int k;
	int num_banks_to_write;
	int bank_count;
	bit [PORT_MEM_BA_WIDTH-1:0] bank_offset;
	bit [PORT_MEM_BA_WIDTH-1:0] mem_bank;
	...
	if ( MEM_DM_EN == 0 ) begin
		...
		for (i = 0; i < PORT_MEM_DM_WIDTH; i++)
		begin
			for (j = 0; j < (PORT_MEM_DQ_WIDTH / PORT_MEM_DM_WIDTH / 9); j++)
			begin
	...
	if (num_banks_to_write == 1)
		mem_bank = mem_address[PORT_MEM_BA_WIDTH-1:0];
	else if (num_banks_to_write == 2)
		mem_bank = {1'b0, mem_address[PORT_MEM_BA_WIDTH-2:0]};
	else if (num_banks_to_write == 4)
		mem_bank = {2'b00, mem_address[PORT_MEM_BA_WIDTH-3:0]};
	
	for (bank_count = 0; bank_count < num_banks_to_write; bank_count++)
	begin
		bank_offset = (NUM_BANKS / num_banks_to_write) * bank_count;
		address_burst.address = {mem_address[PORT_MEM_A_WIDTH+PORT_MEM_BA_WIDTH-1:PORT_MEM_BA_WIDTH], mem_bank+bank_offset};
...
initial begin
	...
	$display("Altera Generic RLDRAM3 Memory Model");
...
always @ (mem_qk_int) begin
	mem_qk <= {PORT_MEM_QK_WIDTH{mem_qk_int}};
	mem_qk_n <= ~{PORT_MEM_QK_WIDTH{mem_qk_int}};
end

...
// This is the generic QDR II/II+/II+ Xtreme memory model for use with the UniPHY memory interface
...
module altera_emif_qdr2_model_rank # (
   ...
   parameter MEM_TRL_CYC                     = "2P5", 
   parameter MEM_TWL_CYC                     = 1,
   parameter MEM_BL                          = 4, 
   ...
   parameter MEM_VERBOSE                     = 1
	...
	input    logic [PORT_MEM_BWS_N_WIDTH-1:0] mem_bws_n,     // Memory byte write select
	output   logic [PORT_MEM_CQ_WIDTH-1:0]    mem_cq,        // Memory echo clock.
	output   logic [PORT_MEM_CQ_WIDTH-1:0]    mem_cq_n,      // Memory echo clock. Pseudo differential version of mem_cq.
	input    logic [PORT_MEM_D_WIDTH-1:0]     mem_d,         // Memory data input
	input    logic [PORT_MEM_K_WIDTH-1:0]     mem_k,         // Memory input clock.
	input    logic [PORT_MEM_K_WIDTH-1:0]     mem_k_n,       // Memory input clock. Pseudo differential version of mem_k.
	output   logic [PORT_MEM_Q_WIDTH-1:0]     mem_q,         // Memory data output
	input    logic                            mem_rps_n,     // Memory read port select
	input    logic                            mem_wps_n,     // Memory write port select
	input    logic                            mem_doff_n     // Memory DLL-off
);
timeunit 1ps;
timeprecision 1ps;
...
localparam MEM_BYTE_WIDTH = PORT_MEM_D_WIDTH/PORT_MEM_BWS_N_WIDTH;
...
logic [PORT_MEM_D_WIDTH-1:0] last_mem_d;
logic [PORT_MEM_BWS_N_WIDTH-1:0]	last_mem_bws_n;
...
// Indicates whether doff_n has ever been asserted low.
// We assert that prior to the first read/write command, doff_n must be
// asserted low at least once to reset DLL on memory device, and during
// all subsequent read/write commands, doff_n must be asserted high. This is
// not a rigorous check, but is sufficient to ensure the sequencer performs
// proper initialization per Cypress latest recommendation.
int last_doff_n_asserted_low;
...
// reference
typedef bit [PORT_MEM_A_WIDTH-1:0]	address_type;
...
task automatic read_command;
	...
	if ((clock_cycle - last_read_cycle) >= MEM_BL/2) begin
		if (MEM_VERBOSE)
...
task automatic write_command;
	...
	if (MEM_BL == 2) begin
		...
		if ((clock_cycle - last_write_cycle) >= MEM_BL/2) begin
			if (MEM_VERBOSE)
				$display("[%0t] [DW=%0d%0d]: Write Command to address %0h", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, mem_a);
...
task init_guaranteed_write;
	...
	bit [PORT_MEM_A_WIDTH-1:0] addr;
	bit [PORT_MEM_D_WIDTH-1:0] data;
	...
	for (i = 0; i < MEM_BL; i++)
	begin
		write_memory(addr, i, {(PORT_MEM_D_WIDTH/9){9'b101010101}}, '0);
	end
	...
	for (i = 0; i < MEM_BL; i++)
	begin
		write_memory(addr, i, {(PORT_MEM_D_WIDTH/9){9'b010101010}}, '0);
	end
...
task automatic check_read_violations;
	...
	if (new_read_command.command != NOP_COMMAND) begin
		...
		if (new_read_command.command == READ_COMMAND)
			if ((clock_cycle - last_read_cycle) < MEM_BL/2) begin
				if (MEM_SUPPRESS_CMD_TIMING_ERROR) begin
					if (MEM_VERBOSE)
...
// Description: Checks to make sure doff_n is HIGH and has been asserted low before
task automatic check_doff_n_violations;

	// Ensure that doff_n is high
	...
	// Ensure that doff_n was asserted low some time before
	if (last_doff_n_asserted_low < 0) begin
		if (MEM_SUPPRESS_CMD_TIMING_ERROR) begin
			if (MEM_VERBOSE)
				$display("[%0t] [DW=%0d%0d]: WARNING: mem_doff_n was never asserted low during device initialization prior to command issue", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX);
			end
		else begin
			$display("[%0t] [DW=%0d%0d]: ERROR: mem_doff_n was never asserted low during device initialization prior to command issue", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX);
			$finish(1);
		end
	end
   
	// Strictly speaking the command must happen at least 20us after doff_n going high,
	// but this requirement isn't met in skip-cal environment where we skip the wait time,
	// so we'll just perform a minimal sanity check.
	if ((clock_cycle - last_doff_n_asserted_low) < 10) begin
		if (MEM_SUPPRESS_CMD_TIMING_ERROR) begin
			if (MEM_VERBOSE)
				$display("[%0t] [DW=%0d%0d]: WARNING: Command issue follows too closely to mem_doff_n being asserted high. Must wait for device DLL to lock.", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX);
			end
		else begin
			$display("[%0t] [DW=%0d%0d]: ERROR: Command issue follows too closely to mem_doff_n being asserted high. Must wait for device DLL to lock.", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX);
			$finish(1);
		end
	end
endtask
...
task write_memory(
	...
	input int burst_num,
	input [PORT_MEM_D_WIDTH-1:0] write_data,
	input [PORT_MEM_BWS_N_WIDTH-1:0] mem_dm);
	...
	for (i = 0; i < PORT_MEM_BWS_N_WIDTH; i++) begin
...
task read_memory(
	input [PORT_MEM_A_WIDTH-1:0] mem_address,
	input int burst_num,
	output [PORT_MEM_Q_WIDTH-1:0] data_read);
	address_burst_type address_burst;
	int i;
...
initial begin
   if (MEM_TRL_CYC == "1P5") 
      tRL_cycles = 3;
   else if (MEM_TRL_CYC == "2P0")
      tRL_cycles = 4;
   else if (MEM_TRL_CYC == "2P5")
      tRL_cycles = 5;
	tWL_cycles = (2*MEM_TWL_CYC);
end
...
initial begin
	...
	$display("Altera Generic QDR II/II+/II+ Xtreme Memory Model");
...
always @ (mem_cq_int) begin
	mem_cq <= {PORT_MEM_CQ_WIDTH{mem_cq_int}};
	mem_cq_n <= ~{PORT_MEM_CQ_WIDTH{mem_cq_int}};
end

// Update last_doff_n_asserted_low
always @ (posedge mem_k_int)
	if (mem_doff_n === 1'b0) begin
		last_doff_n_asserted_low = clock_cycle;
	end
...
always @ (mem_k_int) begin
	...
	if (mem_k_int) begin
		...
		if (mem_rps_n == 1'b0) begin
			...
			// Check for doff# violations
			check_doff_n_violations();
		end
		else 
	...
	if (active_read_command.command != NOP_COMMAND)
		if (active_read_command.word_count == MEM_BL)
			active_read_command.command = NOP_COMMAND;
...
always @ (mem_k_int) begin
	...
	if (active_write_command.command != NOP_COMMAND)
		if (active_write_command.word_count == MEM_BL)
			// Command complete
			active_write_command.command = NOP_COMMAND;

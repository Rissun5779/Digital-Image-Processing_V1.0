...
module altera_emif_rld2_model_rank # (
   ...
   input logic  [MEM_ADDR_WIDTH-1:0]       mem_a,   // address
   input logic  [MEM_BANKADDR_WIDTH-1:0]   mem_ba, // bank address
   input logic                             mem_ck, // memory clock
   input logic                             mem_ck_n, // memory clock. Differential version of mem_ck.
   input logic                             mem_cs_n, // memory chip select
   input logic  [MEM_WRITE_DQS_WIDTH-1:0]  mem_dk, // memory data input clock
   input logic  [MEM_WRITE_DQS_WIDTH-1:0]  mem_dk_n, // memory data input clock. Differential version of mem_dk
   input logic  [MEM_DM_WIDTH-1:0]         mem_dm, // memory data mask.
   inout logic  [MEM_DQ_WIDTH-1:0]         mem_dq, // memory data input/output
   ...
   input logic                             mem_ref_n, // refresh command input
   input logic                             mem_we_n // write enable input
);
   timeunit 1ps;
   timeprecision 1ps;
   ...
   generate
      ...
      begin
         ...
         begin
            mem_dk_time[i] <= $time;
            mem_dq_captured[((i+1)*(MEM_DQ_WIDTH/MEM_WRITE_DQS_WIDTH))-1 : i*(MEM_DQ_WIDTH/MEM_WRITE_DQS_WIDTH)] <= 
                         mem_dq[((i+1)*(MEM_DQ_WIDTH/MEM_WRITE_DQS_WIDTH))-1 : i*(MEM_DQ_WIDTH/MEM_WRITE_DQS_WIDTH)];
            if (i == MEM_WRITE_DQS_WIDTH-1)
   ...
   task automatic set_configuration (input bit [2:0] mode);
      if (MEM_VERBOSE) 
         $display("[%0t] [DW=%0d%0d]: Setting configuration mode %0d", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, (mode+1));

      // Look up the configuration based on the input mode and set it
      // There are only 5 configuration modes but as per the Micron
      // datasheet mode 1 is duplicated for 000 and 001.
      case (mode)
         3'b000 : begin
               tRC_cycles <= 4;
               tRL_cycles <= 4;
         ...
         default : begin
            $display("Invalid mode %0d specified!", mode);
            $finish(1);
            end
      endcase
   ...
   task automatic mrs_command;
      if (MEM_VERBOSE) 
         $display("[%0t] [DW=%0d%0d]: MRS Command %18b", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, mem_a[17:0]);

      // Set the configuration mode
      set_configuration(mem_a[2:0]);
      
      // Set the burst length
      set_burst_len(mem_a[4:3]);
   ...
   task write_memory(
      ...
      if (data_mask[0] === 1'b1) begin
         if (MEM_VERBOSE) 
            $display("[%0t] [DW=%0d%0d]: Data mask enabled. Data ignored for write to address %0h.", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, mem_address);
      end
      else if (data_mask[0] === 1'b0)
         // Write the data to the array
         mem_data[address_burst] = write_data;
      else begin
         $display("[%0t] [DW=%0d%0d]: ERROR: Invalid byte mask %h!", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, data_mask[0]);
   ...
   initial begin
      ...
      $display("[%0t] [DW=%0d%0d]: Altera Generic RLDRAM II Memory Model", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX);

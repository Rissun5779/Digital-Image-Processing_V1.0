...
///////////////////////////////////////////////////////////////////////////////
// Top-level wrapper of Example LPDDR3 AFI Traffic Generator
//
...
//
///////////////////////////////////////////////////////////////////////////////
module altera_emif_tg_afi_lpddr3_top # (
   parameter PROTOCOL_ENUM                           = "",
   parameter USER_CLK_RATIO                          = 1,
   parameter MEM_DATA_MASK_EN                        = 1,
   parameter MEM_NUM_OF_LOGICAL_RANKS                = 1,
   parameter MEM_LPDDR3_TRL_CYC                      = 1,
   parameter MEM_LPDDR3_TWL_CYC                      = 1,
   parameter MEM_LPDDR3_CTRL_CFG_READ_ODT_CHIP       = 16'b0,
   parameter MEM_LPDDR3_CTRL_CFG_WRITE_ODT_CHIP      = 16'b0,
   ...
   localparam PORT_MEM_ADDR_WIDTH = PORT_AFI_ADDR_WIDTH / USER_CLK_RATIO;       // Number of CA pins
   localparam INT_MEM_ADDR_WIDTH  = 15;                                         // Internal address width
   localparam INT_MEM_BA_WIDTH    = 3;                                          // Internal bank address width
   ...
   localparam READ_TO_ODT_MEM_CYCS = MEM_LPDDR3_TRL_CYC - MEM_LPDDR3_TWL_CYC;
   ...
   logic [PORT_MEM_ADDR_WIDTH-1:0]                     afi_ca_t0;
   logic [INT_MEM_ADDR_WIDTH-1:0]                      afi_addr_t0;
   logic [INT_MEM_BA_WIDTH-1:0]                        afi_ba_t0;
   ...
   assign afi_addr_c         = { {(PORT_AFI_ADDR_WIDTH - PORT_MEM_ADDR_WIDTH){1'b0}} , afi_ca_t0   };
   ...
   assign afi_addr_t0 = (
      ...
      ) ? {{(INT_MEM_ADDR_WIDTH - LOOP_COUNTER_WIDTH - 3){1'b0}}, loop_counter, 3'b000} : '0;
   ...
   assign afi_ba_t0 = (
      ...
      state == ISSUE_READ_1 ||
      state == ISSUE_PRECHARGE
      ) ? loop_counter[INT_MEM_BA_WIDTH-1:0] : '0;
  
   // Derive CA bus signals
   assign is_read = ((state == ISSUE_READ_0) || (state == ISSUE_READ_1));
   assign afi_ca_t0 = (
      state == ISSUE_ACTIVATE ? {afi_addr_t0[14:13], afi_addr_t0[7:0], afi_ba_t0, afi_addr_t0[12:8], 2'b10} : (
      state == ISSUE_PRECHARGE ? {10'b0000000000, afi_ba_t0, 7'b00_01011} : (
      state == ISSUE_WRITE_0 ||
      state == ISSUE_READ_0 ||
      state == ISSUE_WRITE_1 ||
      state == ISSUE_READ_1 ? {afi_addr_t0[11:3], 1'b0, afi_ba_t0, afi_addr_t0[2:1], 2'b00, is_read, 2'b01} : (
                              '0 ))));   
      ...
      for (cs_n_i = 0; cs_n_i < MEM_CS_N_WIDTH; ++cs_n_i) begin: gen_cs_n_i
         ...
         begin
            ...
            begin
               ...
               cs_n_i_pin = (active_cs_n == cs_n_i) ? {{(USER_CLK_RATIO-1){1'b1}}, 1'b0} : {USER_CLK_RATIO{1'b1}};
            end else begin
               // Not issuing command.
               cs_n_i_pin = {USER_CLK_RATIO{1'b1}};
            end
         end
   ...
   generate
      ...
      for (odt_i = 0; odt_i < MEM_ODT_WIDTH; ++odt_i) begin : gen_odt_i
         ...
         begin
            ...
            begin
               odt_en = active_cs_n == 2'b00 ? MEM_LPDDR3_CTRL_CFG_WRITE_ODT_CHIP[0  + odt_i] : (
                        active_cs_n == 2'b01 ? MEM_LPDDR3_CTRL_CFG_WRITE_ODT_CHIP[4  + odt_i] : (
                        active_cs_n == 2'b10 ? MEM_LPDDR3_CTRL_CFG_WRITE_ODT_CHIP[8  + odt_i] : (
                                               MEM_LPDDR3_CTRL_CFG_WRITE_ODT_CHIP[12 + odt_i] )));
            end 
            else if (odt_state == ODT_READ_FIRST || odt_state == ODT_READ_MID || odt_state == ODT_READ_LAST) 
            begin
               odt_en = active_cs_n == 2'b00 ? MEM_LPDDR3_CTRL_CFG_READ_ODT_CHIP[0  + odt_i] : (
                        active_cs_n == 2'b01 ? MEM_LPDDR3_CTRL_CFG_READ_ODT_CHIP[4  + odt_i] : (
                        active_cs_n == 2'b10 ? MEM_LPDDR3_CTRL_CFG_READ_ODT_CHIP[8  + odt_i] : (
                                               MEM_LPDDR3_CTRL_CFG_READ_ODT_CHIP[12 + odt_i] )));               
            end
   ...
   ////////////////////////////////////////////////////////////////////
   // Terminate signals that are unused by the LPDDR3 AFI driver
   ////////////////////////////////////////////////////////////////////

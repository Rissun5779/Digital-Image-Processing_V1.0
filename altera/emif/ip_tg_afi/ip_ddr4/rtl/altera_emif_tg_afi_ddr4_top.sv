...
///////////////////////////////////////////////////////////////////////////////
// Top-level wrapper of Example DDR4 AFI Traffic Generator
//
...
//
///////////////////////////////////////////////////////////////////////////////
module altera_emif_tg_afi_ddr4_top # (
   parameter PROTOCOL_ENUM                           = "",
   parameter USER_CLK_RATIO                          = 1,
   parameter MEM_DATA_MASK_EN                        = 1,
   parameter DENY_RECAL_REQUEST                      = 0,
   parameter MEM_NUM_OF_LOGICAL_RANKS                = 1,
   parameter MEM_DDR4_TCL                            = 1,
   parameter MEM_DDR4_WTCL                           = 1,
   parameter MEM_DDR4_READ_PREAMBLE                  = 1,
   parameter MEM_DDR4_WRITE_PREAMBLE                 = 1,
   parameter MEM_DDR4_CTRL_CFG_READ_ODT_CHIP         = 16'b0,
   parameter MEM_DDR4_CTRL_CFG_WRITE_ODT_CHIP        = 16'b0,
   parameter MEM_DDR4_FORMAT_ENUM                    = "",
   ...
   localparam PORT_MEM_BG_WIDTH   = PORT_AFI_BG_WIDTH / USER_CLK_RATIO;         // Number of bank-group pins
   localparam WDATA_BITS_PER_DM   = PORT_AFI_WDATA_WIDTH / PORT_AFI_DM_N_WIDTH; // AFI symbol size
   ...
   localparam ODT_WRITE_PERIOD_MEM_CYCS = 5 + MEM_DDR4_WRITE_PREAMBLE;
   ...
   localparam ODT_READ_PERIOD_MEM_CYCS = 5 + MEM_DDR4_READ_PREAMBLE;

   // Number of memory cycles between read command and ODT assertion
   localparam READ_TO_ODT_MEM_CYCS = MEM_DDR4_TCL - MEM_DDR4_WTCL + MEM_DDR4_WRITE_PREAMBLE - MEM_DDR4_READ_PREAMBLE;
   ...
   // Add single-cycle delay to parity output
   localparam MEM_PARITY_DELAY = (((MEM_DDR4_FORMAT_ENUM == "MEM_FORMAT_RDIMM") || (MEM_DDR4_FORMAT_ENUM == "MEM_FORMAT_LRDIMM")) ? 1 : 0);
   ...
   logic [PORT_AFI_BG_WIDTH-1:0]                       afi_bg_c;
   logic [PORT_AFI_CS_N_WIDTH-1:0]                     afi_cs_n_c;
   logic [PORT_AFI_ODT_WIDTH-1:0]                      afi_odt_c;
   logic [PORT_AFI_ACT_N_WIDTH-1:0]                    afi_act_n_c;
   logic [PORT_AFI_DM_N_WIDTH-1:0]                     afi_dm_n_c;
   ...
   logic [PORT_AFI_DM_N_WIDTH-1:0]                     asserted_afi_dm_n;
   ...
   assign afi_addr_c         = { {(PORT_AFI_ADDR_WIDTH - PORT_MEM_ADDR_WIDTH){1'b1}} , afi_addr_t0 };
   ...
   // when preamble length is 1CK, and both last and second last time slot when
   // preamble length is 2CK.
   assign preamble_afi_dqs_burst = (MEM_DDR4_WRITE_PREAMBLE == 1) ? { 1'b1 , {(USER_CLK_RATIO-1){1'b0}} } :
                                                                    { 2'b11, {(USER_CLK_RATIO-2){1'b0}} };
   assign preamble_afi_wrank     = (MEM_DDR4_WRITE_PREAMBLE == 1) ? { active_logical_rank_one_hot     , {(PORT_AFI_WRANK_WIDTH - 1*MEM_NUM_OF_LOGICAL_RANKS){1'b0}} } :
                                                                    { {2{active_logical_rank_one_hot}}, {(PORT_AFI_WRANK_WIDTH - 2*MEM_NUM_OF_LOGICAL_RANKS){1'b0}} };
   ...
   generate
      ...
      if (MEM_DATA_MASK_EN) begin : use_dm
         for (i = 0; i < PORT_AFI_DM_N_WIDTH; ++i) begin : afi_dm_n_idx
            // Generate data mask
            assign asserted_afi_dm_n[i] = loop_counter[0] ^ i[0];
            ...
            for (j = i * WDATA_BITS_PER_DM; j < (i + 1) * WDATA_BITS_PER_DM; ++j) begin : wdata_idx
               assign write_1_pattern[j] = asserted_afi_dm_n[i] ? ~write_0_pattern[j] : write_0_pattern[j];
            end
         end
      end else begin
         assign asserted_afi_dm_n = '1;
   ...
   assign afi_addr_t0 =
      (state == ISSUE_ACTIVATE)                          ? {{(PORT_MEM_ADDR_WIDTH - LOOP_COUNTER_WIDTH){1'b0}}, loop_counter} : (
      (state == ISSUE_WRITE_0 || state == ISSUE_WRITE_1) ? {{(PORT_MEM_ADDR_WIDTH - 17){1'b0}}, 1'b1, 1'b0, 1'b0, {(11 - LOOP_COUNTER_WIDTH){1'b0}}, loop_counter, 3'b000} : (
      (state == ISSUE_READ_0  || state == ISSUE_READ_1)  ? {{(PORT_MEM_ADDR_WIDTH - 17){1'b0}}, 1'b1, 1'b0, 1'b1, {(11 - LOOP_COUNTER_WIDTH){1'b0}}, loop_counter, 3'b000} : (
      (state == ISSUE_PRECHARGE)                         ? {{(PORT_MEM_ADDR_WIDTH - 17){1'b0}}, 1'b0, 1'b1, 1'b0, {14{1'b0}}} : (
                                                           {{(PORT_MEM_ADDR_WIDTH - 17){1'b0}}, 1'b1, 1'b1, 1'b1, {14{1'b0}}} ))));
   ...
   // The following are unused by example AFI driver
   assign afi_bg_c = '0;
   assign afi_c = '0;
   ...
   generate
      ...
      for (odt_i = 0; odt_i < MEM_ODT_WIDTH; ++odt_i) begin : gen_odt_i
         ...
         begin
            ...
            begin
               odt_en = active_cs_n == 2'b00 ? MEM_DDR4_CTRL_CFG_WRITE_ODT_CHIP[0  + odt_i] : (
                        active_cs_n == 2'b01 ? MEM_DDR4_CTRL_CFG_WRITE_ODT_CHIP[4  + odt_i] : (
                        active_cs_n == 2'b10 ? MEM_DDR4_CTRL_CFG_WRITE_ODT_CHIP[8  + odt_i] : (
                                               MEM_DDR4_CTRL_CFG_WRITE_ODT_CHIP[12 + odt_i] )));
            end
            else if (odt_state == ODT_READ_FIRST || odt_state == ODT_READ_MID || odt_state == ODT_READ_LAST)
            begin
               odt_en = active_cs_n == 2'b00 ? MEM_DDR4_CTRL_CFG_READ_ODT_CHIP[0  + odt_i] : (
                        active_cs_n == 2'b01 ? MEM_DDR4_CTRL_CFG_READ_ODT_CHIP[4  + odt_i] : (
                        active_cs_n == 2'b10 ? MEM_DDR4_CTRL_CFG_READ_ODT_CHIP[8  + odt_i] : (
                                               MEM_DDR4_CTRL_CFG_READ_ODT_CHIP[12 + odt_i] )));
            end
   ...
   assign afi_dm_n_c = (
      state == WRITE_DATA_0 ? '1 : (
      state == WRITE_DATA_1 ? asserted_afi_dm_n :
                              '1 ));
   ...
   begin
      ...
      afi_par_dly_t0    <= afi_par_c[USER_CLK_RATIO-1];
      afi_par           <= (   (MEM_PARITY_DELAY == 1)
                             ? {afi_par_c[USER_CLK_RATIO-2:0], afi_par_dly_t0} : afi_par_c);
   end
      ...
      if (PORT_AFI_PAR_WIDTH != USER_CLK_RATIO) begin : no_ac_par
         ...
         for (t = 0; t < USER_CLK_RATIO; ++t) begin : timeslot
            assign afi_par_c[t] = ^{afi_addr_c[PORT_MEM_ADDR_WIDTH * t +: PORT_MEM_ADDR_WIDTH],
                                    ...
                                    afi_bg_c[PORT_MEM_BG_WIDTH * t +: PORT_MEM_BG_WIDTH],
                                    afi_act_n_c[t]};
         end
      end
   endgenerate
   ...
   ////////////////////////////////////////////////////////////////////
   // Terminate signals that are unused by the DDR4 AFI driver
   ////////////////////////////////////////////////////////////////////

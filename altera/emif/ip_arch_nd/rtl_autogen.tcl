...
# package: altera_emif::ip_arch_nd::rtl_autogen
#
# Provides the utility functions for the arch_nd component
#
package provide altera_emif::ip_arch_nd::rtl_autogen 0.1
...
###                          TCL NAMESPACE                                   ###
################################################################################
namespace eval ::altera_emif::ip_arch_nd::rtl_autogen:: {
   # Namespace Variables
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::update_rtl {} {
...
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_update_segment {original new begin_marker end_marker} {
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_update_long_bitvec_params {} {

   set rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_top.sv"
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_update_hmc_params {} {

   set top_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_top.sv"
   set io_tiles_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_io_tiles.sv"
   ...
   foreach prefix [list "PRI" "SEC"] {
      foreach hmc_cfg_enum [enums_of_type HMC_CFG] {
         set rtl_name     "${prefix}_${hmc_cfg_enum}"
         ...
         if {$data_type == "string"} {
            set default_val "\"\""
            lappend param_defs      "parameter           [format "%-40s" $rtl_name]= $default_val"
         } else {
            set default_val 0
            lappend param_defs      "parameter \[[format "%3d" [expr {$width-1}]]:  0\] [format "%-40s" $rtl_name]= $default_val"
         }
         lappend io_tiles_asgmts ".[format "%-36s" $rtl_name] ($rtl_name)"
         
         if {[string first "*" $wysiwyg_name] == -1} {
            lappend wysiwyg_asgmts  ".[format "%-32s" $wysiwyg_name] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
         } else {
            # Expand RTL parameters to the full set that overs the controller and the dbc's
            lappend wysiwyg_asgmts  ".[format "%-32s" [string map {"*" "ctrl"} $wysiwyg_name]] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
            lappend wysiwyg_asgmts  ".[format "%-32s" [string map {"*" "dbc0"} $wysiwyg_name]] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
            lappend wysiwyg_asgmts  ".[format "%-32s" [string map {"*" "dbc1"} $wysiwyg_name]] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
            lappend wysiwyg_asgmts  ".[format "%-32s" [string map {"*" "dbc2"} $wysiwyg_name]] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
            lappend wysiwyg_asgmts  ".[format "%-32s" [string map {"*" "dbc3"} $wysiwyg_name]] [format "%-38s" "($rtl_name),"] //__ACDS_USER_COMMENT__ $comment"
         }
      }
   }
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_update_mem_ports {} {

   set top_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_top.sv"
   set bufs_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_bufs.sv"
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_update_afi_ports {} {

   set top_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_top.sv"
   set afi_if_rtl_path "$::env(ACDS_SRC_ROOT)/ip/emif/ip_arch_nd/rtl/altera_emif_arch_nd_afi_if.sv"
...
# returns:
#
proc ::altera_emif::ip_arch_nd::rtl_autogen::_init {} {
}
...
::altera_emif::ip_arch_nd::rtl_autogen::_init

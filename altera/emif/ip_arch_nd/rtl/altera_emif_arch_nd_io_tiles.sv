...
`define _get_vfifo_burst_length                          ( PROTOCOL_ENUM == "PROTOCOL_RLD3" ? "vburst_length_2" : ( \
                                                           PROTOCOL_ENUM == "PROTOCOL_RLD2" ? "vburst_length_2" : ( \
...
`define _get_pin_pair_mode                               ( PROTOCOL_ENUM == "PROTOCOL_DDR4"   ? "ddr4_idle" : ( \
                                                           ...
                                                           PROTOCOL_ENUM == "PROTOCOL_RLD2"   ? "ddr3_idle" : ( \
...
`define _get_pst_en_shrink                               ( PROTOCOL_ENUM == "PROTOCOL_DDR4"   ? "shrink_1_1" : ( \
...
`define _get_pa_filter_code                              ( PLL_VCO_FREQ_MHZ_INT > 1200   ? 1600 : ( \
                                                           PLL_VCO_FREQ_MHZ_INT > 1000   ? 1200 : ( \
                                                           PLL_VCO_FREQ_MHZ_INT > 800    ? 1000 : ( \
                                                                                           800 ))))

`define _get_a_filter_code                               ( PLL_VCO_FREQ_MHZ_INT > 1200   ? "freq_16ghz" : ( \
                                                           PLL_VCO_FREQ_MHZ_INT > 1000   ? "freq_12ghz" : ( \
                                                           PLL_VCO_FREQ_MHZ_INT > 800    ? "freq_10ghz" : ( \
                                                                                           "freq_08ghz"))))
...
module altera_emif_arch_nd_io_tiles #(
   ...
   assign phy_reset_n = t2c_seq2core_reset_n[PRI_AC_TILE_INDEX];
   ...
   assign core_clks_from_cpa_pri = all_tiles_core_clks_out[PRI_AC_TILE_INDEX];
   assign core_clks_locked_cpa_pri = all_tiles_core_clks_locked[PRI_AC_TILE_INDEX];
   assign all_tiles_core_clks_fb_in[PRI_AC_TILE_INDEX] = core_clks_fb_to_cpa_pri;

   assign core_clks_from_cpa_sec = PHY_PING_PONG_EN ? all_tiles_core_clks_out[SEC_AC_TILE_INDEX] : '0;
   assign core_clks_locked_cpa_sec = PHY_PING_PONG_EN ? all_tiles_core_clks_locked[SEC_AC_TILE_INDEX] : '0;
   generate
   ...
   assign phy_fb_clk_to_pll = all_tiles_phy_fb_clk_to_pll[PRI_AC_TILE_INDEX];
   ...
   assign ctl2core_avl_cmd_ready_0 = all_tiles_ctl2core_avl_cmd_ready[PRI_AC_TILE_INDEX];
   assign ctl2core_avl_rdata_id_0  = all_tiles_ctl2core_avl_rdata_id[PRI_AC_TILE_INDEX];

   assign ctl2core_avl_cmd_ready_1 = all_tiles_ctl2core_avl_cmd_ready[SEC_AC_TILE_INDEX];
   assign ctl2core_avl_rdata_id_1  = all_tiles_ctl2core_avl_rdata_id[SEC_AC_TILE_INDEX];
   ...
   assign t2c_afi = all_tiles_t2c_afi[PRI_AC_TILE_INDEX];

   // Sideband signals between HMC and core
   // In the following arrays, only elements at tile index *_AC_TILE_INDEX, corresponding to the addr/cmd tile, are used
   logic [NUM_OF_RTL_TILES-1:0][13:0] all_tiles_ctl2core_sideband;

   assign ctl2core_sideband_0 = all_tiles_ctl2core_sideband[PRI_AC_TILE_INDEX];
   assign ctl2core_sideband_1 = all_tiles_ctl2core_sideband[SEC_AC_TILE_INDEX];

   // MMR signals between HMC and core
   // In the following arrays, only elements at tile index *_AC_TILE_INDEX, corresponding to the addr/cmd tile, are used
   logic [NUM_OF_RTL_TILES-1:0][33:0] all_tiles_ctl2core_mmr;

   assign ctl2core_mmr_0 = all_tiles_ctl2core_mmr[PRI_AC_TILE_INDEX];
   assign ctl2core_mmr_1 = all_tiles_ctl2core_mmr[SEC_AC_TILE_INDEX];
   ...
   assign pa_dprio_readdata = all_tiles_pa_dprio_readdata[PRI_AC_TILE_INDEX];
   assign pa_dprio_block_select = all_tiles_pa_dprio_block_select[PRI_AC_TILE_INDEX];
   ...
   generate
      ...
      begin: tile_gen
         ...
         fourteennm_tile_ctrl # (
            ...
            .dbc2core_wr_data_rdy0            (l2core_wr_data_rdy_ast[tile_i][0]),
            .dbc2core_wr_data_rdy1            (l2core_wr_data_rdy_ast[tile_i][1]),
            .dbc2core_wr_data_rdy2            (l2core_wr_data_rdy_ast[tile_i][2]),
            .dbc2core_wr_data_rdy3            (l2core_wr_data_rdy_ast[tile_i][3]),
         ...
         begin: lane_gen
            ...
            altera_emif_arch_nd_io_lane_remap #(
               ...
               .oct_enable                               (l2b_dtc [tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),
               .data_oe                                  (l2b_oe  [tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),
               .data_out                                 (l2b_data[tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),
               .data_in                                  (b2l_data[tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),
               .emif_phy_out_a                           (l2b_e_a[tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),
               .emif_phy_out_b                           (l2b_e_b[tile_i * PINS_PER_LANE * LANES_PER_TILE + lane_i * PINS_PER_LANE +: PINS_PER_LANE]),

               // Interface to core
               .data_from_core                           (core2l_data[tile_i][lane_i]),
               .data_to_core                             (l2core_data[tile_i][lane_i]),
               ...
               .rdata_valid_core                         (l2core_rdata_valid[tile_i][lane_i]),
               .afi_wlat_core                            (l2core_afi_wlat[tile_i][lane_i]),
               .afi_rlat_core                            (l2core_afi_rlat[tile_i][lane_i]),

               // Data Buffer Interface to Core
               .dbc2core_rd_data_vld0                    (l2core_rd_data_vld_avl0[tile_i][lane_i]),
               .dbc2core_rd_data_vld1                    (),
               .core2dbc_wr_data_vld0                    (`_get_core2dbc_wr_data_vld(tile_i, lane_i)),
               .core2dbc_wr_data_vld1                    (1'b0),
               .dbc2core_wr_data_rdy                     (l2core_wr_data_rdy_ast [tile_i][lane_i]),
               .core2dbc_rd_data_rdy                     (`_get_core2dbc_rd_data_rdy(tile_i, lane_i)),
               .dbc2core_wb_pointer                      (l2core_wb_pointer_for_ecc[tile_i][lane_i]),

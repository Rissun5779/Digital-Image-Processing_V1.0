...
output                                           idle;
    ...
    reg  [32                                             - 1 : 0] error_cmd_addr_padded;
    ...
    assign int_slave_cmd_size = int_slave_cmd_data[(34+CFG_LOCAL_SIZE_WIDTH-1):34];
    ...
    assign initial_pointer_fifo_wr      = (cfg_wrpath_pipeline_en == 1'b1) ? push_to_initial_pointer_fifo_r7 : (CFG_REGISTER_WDATA_PATH == 1) ? push_to_initial_pointer_fifo_r3 : push_to_initial_pointer_fifo_r1; 
    assign initial_pointer_fifo_wr_data = {((cfg_wrpath_pipeline_en == 1'b1) ? master_wr_data_info_r8 [1 : 0] : (CFG_REGISTER_WDATA_PATH == 1) ? master_wr_data_info_r4 [1 : 0] : master_wr_data_info_r2 [1 : 0]), int_master_wr_data_ptr_in}; 
    ...
    generate
        ...
        begin : encoder_instance_loop
            ...
            begin
                assign int_encoder_input_data_combi           [i] = {{(CFG_ENCODER_DATA_WIDTH - CFG_LOCAL_DATA_PER_WORD_WIDTH){1'b0}}, encoder_input_data [(i + 1) * CFG_LOCAL_DATA_PER_WORD_WIDTH - 1 : i * CFG_LOCAL_DATA_PER_WORD_WIDTH]};
            end
            else
            begin
        ...
        begin : decoder_instance_loop
            ...
            begin
                ...
                begin
                    ...
                    begin
                        if (int_decoder_output_err_corrected [i] | int_decoder_output_err_sbe [i])
                        begin
                            decoder_output_sbe_combi [i] = 1'b1;
                            decoder_output_dbe_combi [i] = 1'b0;
                        end
                        else if (int_decoder_output_err_fatal [i] | int_decoder_output_err_addr_detected [i])
                        begin
    ...
    begin
        if (
                ...
                !error_cmd_valid &&                                                     
                !error_wr_data_valid &&                                                 
                 pending_write_data_counter_is_zero &&                                  
    ...
    begin
        ...
        begin
            ...
            pending_write_data_counter_is_zero   <= 1'b1;
        end
        else
        begin
            ...
            begin
                ...
                begin
                    ...
                    if (int_slave_cmd_size == {{(CFG_LOCAL_SIZE_WIDTH-1){1'b0}}, 1'b1} && pending_write_data_counter_is_zero) begin
                       pending_write_data_counter_is_zero <= 1'b1;
                    end else begin
                       pending_write_data_counter_is_zero <= 1'b0;
                    end
                end
                else
                begin
            ...
            begin
                if (pending_write_data_counter_is_zero)
                begin
                ...
                begin
                    pending_write_data_counter_underflow <= 1'b0;
                    pending_write_data_counter_is_zero <= (pending_write_data_counter[CFG_PENDING_WRITE_DATA_WIDTH-1:1] == {(CFG_PENDING_WRITE_DATA_WIDTH-1){1'b0}} ? 1'b1 : 1'b0);
                end
    ...
    begin
        if (is_ast)
        begin
            if (pending_write_data_counter_is_zero && slave_wr_data_ready && int_slave_wr_data_valid) 
            begin
    ...
    begin
        ...
        if (CFG_LOCAL_ADDR_WIDTH >= 32) begin
            error_cmd_addr_padded = error_cmd_addr;
        end else begin
            error_cmd_addr_padded = {{(32 - CFG_LOCAL_ADDR_WIDTH){1'b0}}, error_cmd_addr};
        end          

...
module altera_emif_ctrl_rld2_controller # (
   ...
   // In half rate (USER_CLK_RATIO=2), these values are rounded up to the next odd number
   parameter   MEM_BUS_TURNAROUND_CYCLES_RD_TO_WR   = 0,
   parameter   MEM_BUS_TURNAROUND_CYCLES_WR_TO_RD   = 0,

   // CONTROLLER PARAMETERS
   // For a half rate controller (USER_CLK_RATIO=2), the burst length in the controller - equals memory burst length / 4
   // For a full rate controller (USER_CLK_RATIO=1), the burst length in the controller - equals memory burst length / 2
   ...
   input logic   [PORT_CTRL_AMM_WDATA_WIDTH-1:0]      avl_wdata,
   output logic                                       avl_rdata_valid,
   output logic  [PORT_CTRL_AMM_RDATA_WIDTH-1:0]      avl_rdata,
   ...
   localparam CTL_T_WL = (USER_CLK_RATIO == 2) ? CTL_T_WL_HR : CTL_T_WL_FR;
   localparam CTL_T_RC = (USER_CLK_RATIO == 2) ? ((MEM_T_RC + 1) / 2) : MEM_T_RC;
   ...
   wire                                          cmd0_write_req;
   wire                                          cmd0_read_req;
   wire   [PORT_CTRL_AMM_ADDRESS_WIDTH-1:0]      cmd0_avl_addr;
   wire                                          cmd0_addr_can_merge;
   wire                                          cmd1_write_req;
   wire                                          cmd1_read_req;
   wire   [PORT_CTRL_AMM_ADDRESS_WIDTH-1:0]      cmd1_avl_addr;
   ...
   wire   [PORT_AFI_WDATA_WIDTH-1:0]             data_logic_wdata_in;
   wire   [PORT_AFI_RDATA_WIDTH-1:0]             data_logic_rdata_out;

   // PHY interface module signals
   wire                                          wdata_req;
   wire   [PORT_AFI_WDATA_WIDTH-1:0]             wdata;
   wire                                          wdata_valid;
   wire                                          rdata_valid;
   wire   [PORT_AFI_RDATA_WIDTH-1:0]             rdata;
   ...
   if (CTL_BURST_LENGTH == 1) begin
      ...
      begin
         cmd0_beat_addr <= '0;
         cmd0_beat_addr[cmd0_avl_addr[CTL_BEATADDR_INDEX +: CTL_BEATADDR_WIDTH]] <= 1'b1;
      end
   end
   endgenerate
   ...
   defparam data_if.AVL_ADDR_WIDTH      = PORT_CTRL_AMM_ADDRESS_WIDTH;
   defparam data_if.AVL_SIZE_WIDTH      = PORT_CTRL_AMM_BCOUNT_WIDTH;
   defparam data_if.AVL_DWIDTH          = PORT_CTRL_AMM_RDATA_WIDTH;
   defparam data_if.BEATADDR_WIDTH      = CTL_BEATADDR_WIDTH;
   defparam data_if.CTL_BURST_LENGTH    = CTL_BURST_LENGTH;
   ...
   defparam memctl_wdata_rdata_logic_inst.AFI_DWIDTH                    = PORT_AFI_RDATA_WIDTH;
   defparam memctl_wdata_rdata_logic_inst.AFI_RATE_RATIO                = USER_CLK_RATIO;
   ...
   defparam fsm.ADDR_WIDTH                           = PORT_CTRL_AMM_ADDRESS_WIDTH;
   ...
   defparam fsm.AFI_RATE_RATIO                       = USER_CLK_RATIO;


   // DQ bus and bank timers
   altera_emif_ctrl_rld2_timers timers (
      .clk            (afi_clk),
   ...
   altera_emif_ctrl_rld2_refresh refresh (
      .clk                  (afi_clk),
   ...
   defparam afi.PORT_AFI_ADDR_WIDTH          = PORT_AFI_ADDR_WIDTH;
   defparam afi.PORT_AFI_BA_WIDTH            = PORT_AFI_BA_WIDTH;
   defparam afi.PORT_AFI_WE_N_WIDTH          = PORT_AFI_WE_N_WIDTH;
   defparam afi.PORT_AFI_REF_N_WIDTH         = PORT_AFI_REF_N_WIDTH;
   defparam afi.PORT_AFI_CS_N_WIDTH          = PORT_AFI_CS_N_WIDTH;
   defparam afi.PORT_AFI_DM_WIDTH            = PORT_AFI_DM_WIDTH;
   defparam afi.PORT_AFI_RDATA_WIDTH         = PORT_AFI_RDATA_WIDTH;
   defparam afi.PORT_AFI_RDATA_VALID_WIDTH   = PORT_AFI_RDATA_VALID_WIDTH;
   defparam afi.PORT_AFI_RDATA_EN_FULL_WIDTH = PORT_AFI_RDATA_EN_FULL_WIDTH;
   defparam afi.PORT_AFI_RDATA_EN_WIDTH      = PORT_AFI_RDATA_EN_WIDTH;
   defparam afi.PORT_AFI_WDATA_WIDTH         = PORT_AFI_WDATA_WIDTH;
   defparam afi.PORT_AFI_WDATA_VALID_WIDTH   = PORT_AFI_WDATA_VALID_WIDTH;
   defparam afi.AFI_WDATA_REQ_DELAY          = AFI_WDATA_REQ_DELAY;
   defparam afi.AFI_RATE_RATIO               = USER_CLK_RATIO;
   ...
   initial
   begin
      assert (CTL_ADDR_WIDTH + CTL_CHIPADDR_WIDTH + CTL_BANKADDR_WIDTH + CTL_BEATADDR_WIDTH == PORT_CTRL_AMM_ADDRESS_WIDTH)
      ...
      assert (PORT_CTRL_AMM_RDATA_WIDTH == PORT_AFI_RDATA_WIDTH) else $error ("Avalon and AFI data width mismatch");
   end

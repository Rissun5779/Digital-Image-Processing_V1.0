...
package provide altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports 0.1
...
namespace eval ::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports:: {
...
proc ::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports::get_num_of_interfaces_used {if_enum} {
   switch $if_enum {
      IF_AFI -
      IF_AFI_RESET -
      IF_AFI_CLK -
...
proc ::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports::get_interface_ports {if_enum} {
   set ports [list]
   switch $if_enum {
      IF_AFI {
      ...
      IF_CTRL_AMM {
         ...
         lappend ports {*}[create_port  $use_be     PORT_CTRL_AMM_BYTEEN        1                ]
      }
      default {
...
proc ::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports::get_interface_properties {if_enum} {
   ...
   switch $if_enum {
      IF_CTRL_AMM {
         ...
         set use_byte_enable         false
         set be_width                0

         if {[expr {$dq_width % 8}] == 0} {
            set symbol_width 8
         } else {
            set symbol_width 9
            emif_assert {[expr {$dq_width % 9}] == 0}
         }
            
         set ctl_burst_length        [expr {$burst_length / (2 * $clk_ratio)}]
         
         set data_width              [expr {$dq_width * 2 * $clk_ratio}]
                 
         set symbols_per_word        [expr {$data_width / $symbol_width}]
         set symbol_address_width    [expr {$addr_width + $bank_addr_width + int(ceil([log2 $cs_width])) + int(ceil([log2 $ctl_burst_length])) + int(ceil([log2 $symbols_per_word]))}]
         set word_address_width      [expr {$addr_width + $bank_addr_width + int(ceil([log2 $cs_width])) + int(ceil([log2 $ctl_burst_length]))}]
         ...
         dict set props USE_BYTE_ENABLE                $use_byte_enable
...
proc ::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports::_init {} {
}

::altera_emif::ip_ctrl::ip_rld2::ctrl_expert_exports::_init

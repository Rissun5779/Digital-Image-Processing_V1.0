...
///////////////////////////////////////////////////////////////////////////////
// Top-level wrapper of QDR2 soft memory controller
//
...
//
///////////////////////////////////////////////////////////////////////////////
module altera_emif_ctrl_qdr2_top # (
   ...
   // Ports for "ctrl_amm" interface 0 -- read port
   ...
   // Ports for "ctrl_amm" interface 1 -- write port
   ...
   // For QDRII/II+/Xtreme, AFI wlat can only go as high as 2 and
   // sequencer can only make it go lower than 2, but not higher.
   // Setting the max to 3 gives us some safety margin without
   // making timing closure difficult.
   localparam MAX_AFI_WLAT = 3;
   
   // Expose user clocks
   assign emif_usr_reset_n = afi_reset_n;
   assign emif_usr_clk     = (USER_CLK_RATIO == 8) ? afi_half_clk : afi_clk;

   altera_emif_ctrl_qdr2 # (
      ...
      .afi_wlat                                 (afi_wlat),
      .avl_w_ready                              (amm_ready_1),
      .avl_w_write_req                          (amm_write_1),
      .avl_w_addr                               (amm_address_1),
      .avl_w_burstcount                         (amm_burstcount_1),
      .avl_w_be                                 (amm_byteenable_1),
      .avl_w_wdata                              (amm_writedata_1),
      .avl_r_ready                              (amm_ready_0),
      .avl_r_read_req                           (amm_read_0),
      .avl_r_addr                               (amm_address_0),
      .avl_r_burstcount                         (amm_burstcount_0),
      .avl_r_rdata_valid                        (amm_readdatavalid_0),
      .avl_r_rdata                              (amm_readdata_0),
      .local_init_done                          ()
   );
   
   // This soft controller never tries to turn off DLL.
   assign afi_doff_n             = {PORT_AFI_DOFF_N_WIDTH{1'b1}};
   
   // This soft controller never requests recalibration
   assign afi_cal_req            = '0;
   
   // This soft controller never performs refresh
   assign afi_ctl_refresh_done   = '1;
   assign afi_ctl_long_idle      = '0;
   
   // This soft controller only exposes one AMM interface
   ...
   // The following AFI signals aren't applicable to QDRII/II+
   assign afi_ba                 = '0;
   assign afi_bg                 = '0;
   assign afi_c                  = '0;
   assign afi_cke                = '0;

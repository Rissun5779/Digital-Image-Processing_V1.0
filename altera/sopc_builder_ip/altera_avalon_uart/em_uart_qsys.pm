...
#  my $int_in_section = 
#$project->spaceless_module_ptf($module_name)->{SIMULATION}{INTERACTIVE_IN};
#  my $int_out_section=
#$project->spaceless_module_ptf($module_name)->{SIMULATION}{INTERACTIVE_OUT};
...
#      # print "$module_name: key = $int_key, enable = $interactive_in\n";
#  }
#
#  ## use foreach loop to avoid knowing the name of the INTERACTIVE section:
...
#      # print "$module_name: key = $int_key, enable = $interactive_out\n";
#  }
                   
  #$interactive_in = $interactive_in;
  ...
  #$log_module_name = $module_name . "_log";
...
sub make_uart_rxd_source
{
  ...
  #    rom_size       => $size,
  #    dat_name	    => $char_data_file,
  #    mutex_name	    => $char_mutex_file,
  ...
  #    port_map	    => {"q"         => "stim_data",
	 ...
	 #	    }
  # });
  # tie-off output signal(q, new_rom, safe) from e_drom 
  e_assign->add({
      tag => "simulation",
      lhs => "stim_data",
      rhs => "8'b0",   

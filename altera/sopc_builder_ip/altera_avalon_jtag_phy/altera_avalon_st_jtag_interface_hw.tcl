...
# | files
# |
add_fileset synthesis_fileset QUARTUS_SYNTH synth_callback
set_fileset_property synthesis_fileset TOP_LEVEL altera_avalon_st_jtag_interface

set verilog_sim_fileset {verilog_fileset}
add_fileset $verilog_sim_fileset SIM_VERILOG sim_callback_verilog
set_fileset_property $verilog_sim_fileset TOP_LEVEL altera_avalon_st_jtag_interface

set vhdl_sim_fileset {vhdl_fileset}
add_fileset $vhdl_sim_fileset SIM_VHDL sim_callback_vhdl
set_fileset_property $vhdl_sim_fileset TOP_LEVEL altera_avalon_st_jtag_interface
...
set system_verilog_files [ list \
     altera_avalon_st_pipeline_stage.sv \
]

set sdc_files [ list altera_avalon_st_jtag_interface.sdc ]

proc set_sim_properties { fileset } {
    if { [ get_parameter_value USE_PLI] } {
        set qdir {$QUARTUS_INSTALL_DIR}

        set_fileset_sim_properties $fileset LINUX64 OPT_MENTOR_PLI "$qdir/linux64/libsld_sysconsim.so"
        set_fileset_sim_properties $fileset LINUX64 ENV_MENTOR_LD_LIBRARY_PATH "$qdir/linux64"

        set_fileset_sim_properties $fileset WINDOWS64 OPT_MENTOR_PLI "$qdir/bin64/sld_sysconsim.dll"
        set_fileset_sim_properties $fileset WINDOWS64 ENV_MENTOR_LD_LIBRARY_PATH "$qdir/bin64"
    }
}

proc synth_callback { entity_name } {
    global verilog_files
    global system_verilog_files
    global sdc_files
    global verilog_sim_fileset
    
    foreach file $verilog_files {
        add_fileset_file $file VERILOG PATH $file
    }

    foreach file $system_verilog_files {
        add_fileset_file $file SYSTEM_VERILOG PATH $file
    }

    foreach file $sdc_files {
        add_fileset_file $file SDC PATH $file
    }
}

proc sim_callback_verilog { entity_name } {
    global verilog_sim_fileset
    synth_callback $entity_name
    set_sim_properties $verilog_sim_fileset
}

proc sim_callback_vhdl { entity_name } {
    global vhdl_sim_fileset
    synth_callback $entity_name
    set_sim_properties $vhdl_sim_fileset
}
...
add_parameter EXPORT_JTAG INTEGER 0
set_parameter_property EXPORT_JTAG DISPLAY_HINT boolean
set_parameter_property EXPORT_JTAG VISIBLE false
set_parameter_property EXPORT_JTAG HDL_PARAMETER true
...
proc elaborate {} {
    ...
    set export_jtag [get_parameter_value EXPORT_JTAG]

	if {$use_pli && $export_jtag} {
		send_message Error "USE_PLI = 1, EXPORT_JTAG = 1 case is not valid. Please change one or both of these parameters to continue."
	}
	
    add_interface tck clock end
    add_interface_port tck jtag_tck clk Input 1

    add_interface node conduit end
    set_interface_property node associatedClock tck
    add_interface_port node jtag_tms tms Input 1
    add_interface_port node jtag_tdi tdi Input 1
    add_interface_port node jtag_tdo tdo Output 1
    add_interface_port node jtag_ena ena Input 1
    add_interface_port node jtag_usr1 usr1 Input 1
    add_interface_port node jtag_clr clr Input 1
    add_interface_port node jtag_clrn clrn Input 1
    ...
    add_interface_port node jtag_ir_in ir_in Input 3
    add_interface_port node jtag_irq irq Output 1
    add_interface_port node jtag_ir_out ir_out Output 3
    
    # Settings provided here so we can cross check it with the sld endpoint
    set_interface_assignment node debug.settings {fabric sld dir agent mfr_code 110 type_code 132 version 1 ir_width 3}

    set_interface_property node ENABLED $export_jtag
    set_interface_property tck ENABLED $export_jtag
    ...
    if {$use_pli != 1 && $use_downstream_ready != 1} {
        # If the source_ready input is unused in this parameterization, 
        # terminate it.
        set_port_property source_ready TERMINATION true
        set_port_property source_ready TERMINATION_VALUE 1
    }
    # |
    # +-----------------------------------
    ...
    set hdl_mgmt_channel_width [ expr {$mgmt_channel_width > 0 ?  $mgmt_channel_width : 1} ]
    ...
    set_interface_property mgmt maxChannel [expr (1<<$hdl_mgmt_channel_width) - 1]
    set_interface_property mgmt readyLatency 0
    add_interface_port mgmt mgmt_valid valid Output 1
    add_interface_port mgmt mgmt_channel channel Output $hdl_mgmt_channel_width
    ...
    if {$mgmt_channel_width > 0} {
        ...
        # If the mgmt_* output signals are unused in this parameterization, 
        # terminate them.
    ...
    set_port_property mgmt_channel VHDL_TYPE STD_LOGIC_VECTOR

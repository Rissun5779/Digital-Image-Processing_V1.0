...
set_module_property DESCRIPTION "Avalon-compatible Intel PLL module. For Stratix V and newer families, use Intel FPGA PLL IP"
...
set_module_property DISPLAY_NAME "ALTPLL Intel FPGA IP"
set_module_property INSTANTIATE_IN_SYSTEM_MODULE true
set_module_property EDITABLE true
set_module_property SIMULATION_MODEL_IN_VHDL true
set_module_property SIMULATION_MODEL_IN_VERILOG true  
set_module_property SUPPORTED_DEVICE_FAMILIES {"Arria II GX" "Arria II GZ" "Stratix IV" "Cyclone IV E" "Cyclone IV GX" "Max 10" "Cyclone 10 LP"}
# | 
# +---------------------------------------------------------
...
# This is the system clock interface
...
add_interface_port 	"pll_slave" "address" "address" "Input" 2
add_interface_port 	"pll_slave" "readdata" "readdata" "Output" 32
add_interface_port 	"pll_slave" "writedata" "writedata" "Input" 32

add_interface scandone conduit end
add_interface_port scandone scandone export Output 1

add_interface scandataout conduit end
add_interface_port scandataout scandataout export Output 1
...
proc altpll_avalon_post_edit {} {
	...
	set param_name "AVALON_USE_SEPARATE_SYSCLK"
	set param_val [get_private_parameter $param_name]

	set_constant_parameter $param_name $param_val

	if { $param_val == "YES" } {
		set inclk0_port_used [get_parameter_value PORT_INCLK0]
		if { $inclk0_port_used == "PORT_USED" } {
			set_ifport_parameter inclk0 {input 0}
		}
	}
}
...
proc altpll_avalon_elaboration {} {
	...
	if { $device_family == "MAX10" || $device_family == "MAX 10"} {
	...
	# Add inclk0 interface if required
	set separate_sysclk [get_parameter_value AVALON_USE_SEPARATE_SYSCLK]
	if { $separate_sysclk == "YES" } {
		# interface and clk are named inclk0
		add_interface inclk0 clock sink
		add_interface_port inclk0 inclk0 clk input 1
	}
	...
	foreach {clk_name clk_connection_param	clk_freq_private } $OUTCLK_LIST {
		...
		if { $use_clock == "PORT_USED" } {
			...
			if { $device_family == "MAX10" || $device_family == "MAX 10"} {
				add_interface $av_interface_name clock source 
				add_interface_port $av_interface_name $clk_name "clk" output 1
				set_interface_property $av_interface_name ENABLED false
			}
		}
	}

	set param_port_areset [get_parameter_value PORT_ARESET]
	if { $param_port_areset == "PORT_UNUSED" } {
             	add_interface areset conduit end
		add_interface_port areset areset export Input 1
		set_interface_property areset ENABLED false
	}

	set param_port_locked [get_parameter_value PORT_LOCKED]
	if { $param_port_locked == "PORT_UNUSED" } {
             	add_interface locked conduit end
		add_interface_port locked locked export Output 1
		set_interface_property locked ENABLED false
	}

	set param_port_phasedone [get_parameter_value PORT_PHASEDONE]
	if { $param_port_phasedone == "PORT_UNUSED" } {
             	add_interface phasedone conduit end
		add_interface_port phasedone phasedone export Output 1
		set_interface_property phasedone ENABLED false
	}

	set param_port_phasecounterselect [get_parameter_value PORT_PHASECOUNTERSELECT]
	if { $param_port_phasecounterselect == "PORT_UNUSED" } {
             	add_interface phasecounterselect conduit end
		add_interface_port phasecounterselect phasecounterselect export Input $phasectrsel_width
		set_interface_property phasecounterselect ENABLED false
	}

	set param_port_phaseupdown [get_parameter_value PORT_PHASEUPDOWN]
	if { $param_port_phaseupdown == "PORT_UNUSED" } {
             	add_interface phaseupdown conduit end
		add_interface_port phaseupdown phaseupdown export Input 1
		set_interface_property phaseupdown ENABLED false
	}

	set param_port_phasestep [get_parameter_value PORT_PHASESTEP]
	if { $param_port_phasestep == "PORT_UNUSED" } {
             	add_interface phasestep conduit end
		add_interface_port phasestep phasestep export Input 1
		set_interface_property phasestep ENABLED false
	}

	set param_port_scanclk [get_parameter_value PORT_SCANCLK]
	if { $param_port_scanclk == "PORT_USED" } {
             	add_interface scanclk clock sink
		add_interface_port scanclk scanclk "clk" Input 1
		set_interface_property scanclk ENABLED true
	} 
        if { $param_port_scanclk == "PORT_UNUSED" } {
		add_interface scanclk conduit end
		add_interface_port scanclk scanclk export Input 1
		set_interface_property scanclk ENABLED false
	}

	set param_port_scanclkena [get_parameter_value PORT_SCANCLKENA]
	if { $param_port_scanclkena == "PORT_UNUSED" } {
             	add_interface scanclkena conduit end
		add_interface_port scanclkena scanclkena export Input 1
		set_interface_property scanclkena ENABLED false
	}

        set param_port_scandata [get_parameter_value PORT_SCANDATA]
	if { $param_port_scandata == "PORT_UNUSED" } {
             	add_interface scandata conduit end
		add_interface_port scandata scandata export Input 1
		set_interface_property scandata ENABLED false
	}

        set param_port_configupdate [get_parameter_value PORT_CONFIGUPDATE]
	if { $param_port_configupdate == "PORT_UNUSED" } {
             	add_interface configupdate conduit end
		add_interface_port configupdate configupdate export Input 1
		set_interface_property configupdate ENABLED false
	}

	set param_port_scandone [get_parameter_value PORT_SCANDONE]
	if { $param_port_scandone == "PORT_UNUSED" } {
		set_interface_property scandone ENABLED false
	} else {
		set_interface_property scandone ENABLED true
	}

	set param_port_scandataout [get_parameter_value PORT_SCANDATAOUT]
	if { $param_port_scandataout == "PORT_UNUSED" } {
		set_interface_property scandataout ENABLED false
	} else {
		set_interface_property scandataout ENABLED true
	}

	#send_message info "param_port_configupdate=$param_port_configupdate\n"
}

## Add documentation links for user guide and/or release notes
add_documentation_link "User Guide" https://documentation.altera.com/#/link/sam1412657969844/sam1412657677094

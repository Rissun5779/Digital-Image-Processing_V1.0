# $File: //acds/rel/18.1std/ip/sopc/components/altera_avalon_dc_fifo/altera_avalon_dc_fifo_hw.tcl $
# $Revision: #1 $
# $Date: 2018/07/18 $
# $Author: psgswbuild $
#--------------------------------------------------------------------------
...
set_fileset_property synth   TOP_LEVEL altera_avalon_dc_fifo

add_fileset          sim     SIM_VERILOG   {generate sim}
set_fileset_property sim     TOP_LEVEL altera_avalon_dc_fifo

add_fileset          simvhdl SIM_VHDL      {generate sim}
set_fileset_property simvhdl TOP_LEVEL altera_avalon_dc_fifo

proc generate {intent output_name} {
    add_fileset_file altera_avalon_dc_fifo.v VERILOG PATH altera_avalon_dc_fifo.v
    add_fileset_file altera_dcfifo_synchronizer_bundle.v VERILOG PATH altera_dcfifo_synchronizer_bundle.v
    ...
    if {$intent eq "synth"} {
	add_fileset_file altera_avalon_dc_fifo.sdc SDC PATH altera_avalon_dc_fifo.sdc
    }
}
...
add_parameter WR_SYNC_DEPTH             INTEGER 3  ""
add_parameter RD_SYNC_DEPTH             INTEGER 3  ""
...
set_parameter_property CHANNEL_WIDTH      DESCRIPTION  "Channel width. If 0, the channel signal is omitted"
set_parameter_property ERROR_WIDTH        DISPLAY_NAME "Error width"
set_parameter_property ERROR_WIDTH        DESCRIPTION  "Error width. If 0, the error signal is omitted"
set_parameter_property USE_PACKETS        DISPLAY_NAME "Use packets"
set_parameter_property USE_PACKETS        DESCRIPTION  "Use packet signals"
set_parameter_property USE_IN_FILL_LEVEL  DISPLAY_NAME "Use sink fill level"
set_parameter_property USE_IN_FILL_LEVEL  DESCRIPTION  "Enables fill level calculations for the sink side"
set_parameter_property USE_OUT_FILL_LEVEL DISPLAY_NAME "Use source fill level"
set_parameter_property USE_OUT_FILL_LEVEL DESCRIPTION  "Enables fill level calculations for the source side"
set_parameter_property WR_SYNC_DEPTH      DISPLAY_NAME "Write pointer synchronizer length"
set_parameter_property WR_SYNC_DEPTH      DESCRIPTION  "Write pointer synchronizer depth"
set_parameter_property RD_SYNC_DEPTH      DISPLAY_NAME "Read pointer synchronizer length"
set_parameter_property RD_SYNC_DEPTH      DESCRIPTION  "Read pointer synchronizer depth"
...
set_parameter_property USE_IN_FILL_LEVEL  AFFECTS_GENERATION false
set_parameter_property USE_OUT_FILL_LEVEL AFFECTS_GENERATION false 
set_parameter_property WR_SYNC_DEPTH      AFFECTS_GENERATION false 
set_parameter_property RD_SYNC_DEPTH      AFFECTS_GENERATION false
...
#--------------------------------------------------------------------------------------
    ...
    if {$channel_width > 0} {
        set_port_property in_channel WIDTH_EXPR $channel_width
        set_port_property out_channel WIDTH_EXPR $channel_width
    } else {
...
#--------------------------------------------------------------------------------------
    ...
    set space_width [ expr [ log2ceil $depth ] + 1 ]
    add_interface space_avail conduit end
    add_interface_port space_avail space_avail_data data Output $space_width
    set_port_property space_avail_data termination true
}


## Add documentation links for user guide and/or release notes
add_documentation_link "User Guide" https://documentation.altera.com/#/link/sfo1400787952932/iga1401395563779 

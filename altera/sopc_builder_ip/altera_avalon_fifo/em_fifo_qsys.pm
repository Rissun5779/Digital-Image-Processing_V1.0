...
# Parameters: 
#             project
#             device_family   : e.g. $Device_Family
#             fifo_name       : e.g. $module_name."_single_clock_fifo"
#             fifo_depth      : e.g. $FIFO_Depth
#             fifo_width      : e.g. $FIFO_Width
#             use_register    : e.g. $Use_Register
...
sub define_dcfifo
{
    ...
    my $mod = e_module->new({name => $fifo_name});
    $mod->add_attribute ( ALTERA_ATTRIBUTE => qq(SUPPRESS_DA_RULE_INTERNAL=R101));
    $proj->add_module($mod);
    ...
    (
         ...
         lpm_showahead => qq("OFF"),
         lpm_type => qq("dcfifo"),
         lpm_hint => qq("DISABLE_DCFIFO_EMBEDDED_TIMING_CONSTRAINT"),
         lpm_width => $fifo_width,
         lpm_widthu => $fifo_widthu,
         overflow_checking => qq("ON"),
         underflow_checking => qq("ON"),
		 write_aclr_synch => qq("ON"),
		 read_aclr_synch => qq("ON"),
		 clocks_are_synchronized => qq("FALSE"),
         rdsync_delaypipe => 4,
         wrsync_delaypipe => 4,
         use_eab => $use_eab,
    );

    if ( ($device_family =~ m/stratix\s*ii/i) || ($device_family =~ m/cyclone\s*ii/i) )
    {
...
sub define_dcfifo_with_controls
{
    ...
    # combine both rdclk_reset_n and wrclk_reset_n as asynchronous clear signal for the DC FIFO
    my %port_map_hash = 
    (
        aclr => "~($rdclk_reset_n && $wrclk_reset_n)",
        data => 'data',
        wrreq => 'wrreq',
        wrclk => 'wrclk',
        rdreq => 'rdreq',
    ...
    ########################################################
    #
    # Synchronize the rdreq and wrreq to the crossing domain
    # This is to pass the latest ever status, not 'flag'
    #
    ########################################################
    if($Use_Write_Control)
    {  
    my $rdsync_in_port_map   = { reset_n => 'wrreset_n',
                                 clk     => 'wrclk',
                                 din     => 'rdreq' };
    my $rdsync_out_port_map  = { dout    => 'rdreq_sync' };
    my $rdsync_parameter_map = { depth   => 4 };
  ...
  #    tag           => 'synthesis',
      ...
      in_port_map   => $rdsync_in_port_map,
      out_port_map  => $rdsync_out_port_map,
      parameter_map => $rdsync_parameter_map,
      })
      );
    }
    ...
    if($Use_Read_Control)
    {  
    my $wrsync_in_port_map   = { reset_n => 'rdreset_n',
                                 clk     => 'rdclk',
                                 din     => 'wrreq' };
    my $wrsync_out_port_map  = { dout    => 'wrreq_sync' };
    my $wrsync_parameter_map = { depth   => 4 };

    $mod->add_contents(
    e_blind_instance->new({
      name          => 'wrdreq_sync_i',
      module        => 'altera_std_synchronizer',
      in_port_map   => $wrsync_in_port_map,
      out_port_map  => $wrsync_out_port_map,
      parameter_map => $wrsync_parameter_map,
      })
      );
    }
...
sub make_avalonst_avalonst_interface
{
    ...
    if($Single_Clock_Mode)
    {
        ...
        e_assign->adds(
			...
			rhs => "($wrclk_reset_n == 0) ? 1\'b0 : no_stop_write"},   
        );
		}
...
sub create_avalonst_to_avalonmm_with_one_read_per_beat
{
	...
	e_assign->adds(
        ...
        else
        {
			if ($need_other_info_fifo)
			{
                e_assign->adds(
                    {lhs => "avalonmm_read_slave_waitrequest",
                     rhs => "!deassert_waitrequest & rdempty & rdempty_i"},
                );
		    }
			else
			{
	...
	else
	{
	    ...
	    {
	        e_assign->adds(
	        	{lhs => e_signal->add(["no_stop_write_i" =>1]),
	        	 rhs => "(ready_selector & ready_1_i)"},
                    );
	    }
    ...
    if($Use_Backpressure)
    {
		if($Single_Clock_Mode)
        {
            ...
            e_assign->adds(
		    {lhs => "avalonst_sink_ready",
		     rhs => "($wrclk_reset_n == 0) ? 1\'b0 : (no_stop_write & no_stop_write_d1)"}, 
                );
		}
		else 
		{
		    if ($need_other_info_fifo)
	        {
                ...
                e_assign->adds(
		        {lhs => "avalonst_sink_ready",
		         rhs => "no_stop_write & no_stop_write_d1 & no_stop_write_i & no_stop_write_i_d1"}, 
                    );
		    }
		    else
		    {
        ...
        else
        {
            ...
            ({
                ...
                port_map =>
                    {
                        data => &concatenate(@avalonst_other_info_signals),
                        wrreq => 'avalonst_sink_valid & no_stop_write_i_d1', # FIFO take care of overflow checking  
                        rdreq => $rdreq_driver,   
						rdempty => 'rdempty_i',
                        wrclk => 'wrclk',
						wrfull => 'wrfull_i',
                        rdclk => 'rdclk',
                        aclr => "~($wrclk_reset_n && $rdclk_reset_n)",
        ...
        else
        {
            ...
            ({
                ...
                port_map =>
                    {
                        avalonmm_other_info => "avalonmm_write_slave_writedata",
                        auto_clr => "wrreq_driver & ~wrfull_i", # Only clear sop and eop when a valid write occurs
                        enable => $enable_other_info_register,
                        clock => "wrclk",
                        reset_n => "$wrclk_reset_n",
    ...
    if($Use_Backpressure)
    {
        ...
        else
        {
			if ($need_other_info_fifo)
			{
                e_assign->adds(
                    {lhs => "rdreq",
                     rhs => "~rdempty & ~rdempty_i & avalonst_source_ready"},
                );
				
                e_register->add({
                    out => "avalonst_source_valid",
                    in  => "~rdempty & ~rdempty_i & avalonst_source_ready",
                    clock => "rdclk",
                    enable => 1,
                    reset => "$rdclk_reset_n",
                });
    ...
    else
    {
        ...
        else
        {
			if ($need_other_info_fifo)
			{
                e_assign->adds(
                    {lhs => "rdreq",
                     rhs => "~rdempty & ~rdempty_i"},
                );
                
                e_register->add({
                    out => "avalonst_source_valid",
                    in  => "~rdempty & ~rdempty_i",
                    clock => "rdclk",
                    enable => 1,
                    reset => "$rdclk_reset_n",
                });

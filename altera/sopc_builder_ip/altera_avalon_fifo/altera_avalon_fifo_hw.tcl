...
set_module_property DISPLAY_NAME {Avalon FIFO Memory Intel FPGA IP}
...
set_parameter_property avalonMMAvalonMMDataWidth DESCRIPTION {Data width}
...
set_parameter_property avalonMMAvalonSTDataWidth DESCRIPTION {avalonMMAvalonSTDataWidth}
...
set_parameter_property bitsPerSymbol DESCRIPTION {These parameters determine the width of the FIFO.}
...
set_parameter_property channelWidth DESCRIPTION {The width of the channel signal.}
...
set_parameter_property errorWidth DESCRIPTION {The width of the error signal.}
...
set_parameter_property fifoDepth DESCRIPTION {An output pipeline stage is added to the FIFO to increase performance, which increases the FIFO depth by one.}
...
set_parameter_property fifoInputInterfaceOptions DESCRIPTION {Input type}
...
set_parameter_property fifoOutputInterfaceOptions DESCRIPTION {Output type}
...
set_parameter_property singleClockMode DESCRIPTION {Toggle for single clock or dual clock mode}
...
set_parameter_property singleResetMode VISIBLE {0}
...
set_parameter_property symbolsPerBeat DESCRIPTION  {Number of symbols transferred in a beat.}
...
set_parameter_property useBackpressure DESCRIPTION {For backpressure}
...
set_parameter_property useIRQ DESCRIPTION {Enable IRQ for status ports}
...
set_parameter_property usePacket DESCRIPTION {Enable packet data}
...
set_parameter_property useReadControl DESCRIPTION {Create status interface for output}
...
set_parameter_property useRegister DESCRIPTION {FIFO implementation from registers or embedded memory blocks.}
...
set_parameter_property useWriteControl DESCRIPTION {Status interface for input}
...
# ---- part 2 validate GUI input parameter ----
  ...
  if { $derived_sink_source_avalonST_width > 256 } {
    send_message error "The Avalon-ST data width of $derived_sink_source_avalonST_width bits is too large. Valid Avalon-ST data width is 1~256 bits."   
  }
	...
	# interfaces
	  ...
	  set_interface_assignment out_csr embeddedsw.configuration.isPrintableDevice {0} 
	    ...
	    if { $useIRQ } {
	      ...
	      if { $singleClockMode } {
	          set_interface_property out_irq associatedClock {clk_in}
	          set_interface_property out_irq associatedReset {reset_in}
	      } else {
	          set_interface_property out_irq associatedClock {clk_out}
	          set_interface_property out_irq associatedReset {reset_out}	  
	      }       
	    }
        }
	  ...
	  set_interface_assignment in_csr embeddedsw.configuration.isPrintableDevice {0} 
	    ...
	    if { $useIRQ } {
          ...
          set_interface_property in_irq associatedClock {clk_in}
          set_interface_property in_irq associatedReset {reset_in}
	    }
        }
...
proc generate {output_name output_directory rtl_ext simgen} {
    ...
    if {!($singleClockMode)} {
      set pre_out [create_temp_file temp_fifo.sdc]
      set pre_in "constraint_sample.sdc"   
      ...
      close $in
      close $out
      add_fileset_file ${output_name}.sdc SDC PATH $pre_out
    }
...
add_documentation_link "User Guide" https://documentation.altera.com/#/link/sfo1400787952932/iga1401396003807 

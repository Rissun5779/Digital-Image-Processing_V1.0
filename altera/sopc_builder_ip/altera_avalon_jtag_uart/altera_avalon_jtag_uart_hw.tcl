...
set_module_property DISPLAY_NAME {JTAG UART Intel FPGA IP}
...
set_parameter_property allowMultipleConnections VISIBLE {false}
...
set_parameter_property readBufferDepth DESCRIPTION {Read FIFO depth (bytes).}
...
set_parameter_property readIRQThreshold DESCRIPTION {IRQ threshold governs how the core asserts its IRQ in response to the FIFO filling up.}
...
set_parameter_property useRegistersForReadBuffer DESCRIPTION {Construct using registers instead of memory blocks.}
...
set_parameter_property useRegistersForWriteBuffer DESCRIPTION {Construct using registers instead of memory blocks.}
...
set_parameter_property writeBufferDepth DESCRIPTION {Write buffer depth (bytes)}
...
set_parameter_property writeIRQThreshold DESCRIPTION {The write IRQ threshold governs how the core asserts its IRQ in response to the FIFO emptying.}
...
set_parameter_property clkFreq SYSTEM_INFO_ARG {clk}
...
proc validate {} {
	...
	if { $clkFreq < 48000000 } {
		send_message warning "JTAG UART IP input clock need to be at least double (2x) the operating frequency of JTAG TCK on board"
	} else {
		send_message info "JTAG UART IP input clock need to be at least double (2x) the operating frequency of JTAG TCK on board"
	}
...
add_documentation_link "User Guide" https://documentation.altera.com/#/link/sfo1400787952932/iga1401317112138 

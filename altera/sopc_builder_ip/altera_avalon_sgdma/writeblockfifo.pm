...
sub get_writeblock_internal_fifo_depth {
  ...
  # this also takes ($WRITE_BLOCK_DATA_WIDTH + $IN_ERROR_WIDTH + $EMPTY_WIDTH + SOP + EOP) bits.
  # we make sure that the fifo takes only one single M4K block
...
sub make_sop_eop_detector {
  ...
  $mod->add_contents(
    ...
    # Set register when EOP is found and hold that value until the sink acknowledges it 
...
sub make_internal_m_write_fifo {
  ...
  $mod->add_contents(
    ...
    # If a packet is received, stop receiving until an EOP is popped off the FIFO.
    ...
    #taking this one out as valid now depends on not_emtpy in showahead mode
    ...
    # Avoid putting out unknown values

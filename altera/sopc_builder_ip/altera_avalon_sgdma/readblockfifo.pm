...
sub make_read_block_fifo {
  ...
  # The supported latency count is half of the FIFO depth.
  ...
  if ($is_mtm) {
  # the fifo includes these signals: tx_shift_in, data, sop, eop, empty and error
  ...
  # the fifo includes these signals: generate_eop, generate_sop, tx_shift_in, data, sop, eop, empty and error
  ...
  # the fifo includes these signals : data, sop, eop, empty and error
...
sub get_readblock_internal_fifo_depth {
  ...
  # my internal fifo data width is ($READ_BLOCK_DATA_WIDTH + SOP + EOP) bits 
  # What will happen is that we will always waste memory space because of the two extra bits.
  # in fact, we are wasting ($READ_BLOCK_DATA_WIDTH - 2), the larger the data width, the more is wasted.
  # let's take the next power of two.
  # my $internal_fifo_depth = 128 * 32 / ($READ_BLOCK_DATA_WIDTH + 2);
...
sub make_internal_m_read_fifo {
  ...
  $mod->add_contents(
    ...
    # allow subsequent reads from memory if fifo is less than half full
  ...
  if ($UNALIGNED_TRANSFER) {
    ...
    # If unaligned transfer, create tx_shift_out
    $mod->add_contents(
	...
	# If unaligned transfer & non-MTM, create generate_sop_out/generate_eop_out
  	if (!$is_mtm) {
    	$mod->add_contents(
    ...
    # Common signal in fifo, regardless of unalign/align/MTM 
    $mod->add_contents(

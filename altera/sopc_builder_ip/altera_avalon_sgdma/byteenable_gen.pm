...
sub byteenable_gen
{
    ...
    # Create internal-used variables
    ...
    # byteenable_gen_module ports declaration
    my @ports = (
    ["clk"=>1=>"input"],
    ["reset_n"=>1=>"input"],
    ["write_in"=>1=>"input"],
    ...
    # Create ports for byteenable_gen_module
    $mod->add_contents(
   	e_port->news(@ports),
    );

 if ($BYTEENABLE_WIDTH == 1) {
 # 8 bits write block data width
 # Existing sgdma do not generate byteenable when there is only 1 byte bus width, but for completeness
 # leave it here. Might be useful in future
      $mod->add_contents(
 ...
 # 16 bits write block data width
 ...
 # 32 bits write block data width
 ...
 # 64 bits write block data width
 ...
 # 128 bits write block data width
 # Existing SGDMA do not support 128 bits yet, but put as placeholder
 ...
 # 256 bits write block data width
 # Existing SGDMA do not support 256 bits yet, but put as placeholder

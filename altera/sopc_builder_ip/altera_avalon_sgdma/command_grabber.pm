...
sub add_command_grabber_ports {
  ...
  # command_data to each of the read and write block
  if ($HAS_READ_BLOCK) {
...
sub make_command_grabber_assignments {
  ...
  $mod->add_contents(
    ...
    # don't let these signals go up the top, we only want to use it internally
...
sub make_command_grabber_registers {
  ...
  # command_valid signal for both the read and write block
  $mod->add_contents(  

...
# TODO : trim down this command_fifo width
...
sub make_descriptor_read_assignments {
  ...
  $mod->add_contents(
    ...
    # give the signals above some widths, as it does not infer from the rhs
  ...
  if ($DESCRIPTOR_READ_BURST) {
    ...
    $mod->add_contents(
      ...
      # here we hardcode 4 because the master always give out byte addresses
...
sub make_descriptor_read_descriptor_registers {
  ...
  $mod->add_contents(
    ...
    # temporary register to gather the incoming descriptor in 32bit chunks, and assemble into 256bit

...
sub make_sgdma {
  ...
  # DATA_WIDTH is only used for M-T-M
  # add 2 bits for sop and eop and some bits for the empty signal.
  # thank god we do not have error signals on M-T-M
  ...
  # if this is a Memory-To-Memory configuration, create an internal FIFO 
  # between the Read_Block and the Write_block
  print "Internal FIFO data_width : $DATA_WIDTH\n" if $debug;
  if ($HAS_READ_BLOCK && $HAS_WRITE_BLOCK) {
    if ($BURST_TRANSFER) {
      # Just wire up the readfifo to the writefifo
  ...
  # make barrel shifters for unaligned transfer
...
sub make_barrel_shifters {
  ...
  # no sop on the m_write for mem-to-mem
...
sub make_reset_n_mapping {
	...
	$mod->add_contents(
		...
		e_register->new({
           ...
           # SPR 249022 : Explicitly tie the reset signal from FIFOs to reset_n
...
sub make_stream_to_mem {
  ...
  # Atlantic is big endian, so let's just swap the bytes around
  if ($SYMBOLS_PER_BEAT > 1) {
...
sub instantiate_sgdma_sub_modules {
  ...
  if ($HAS_READ_BLOCK) {
    ...
    # make m_read_fifo for avalon latency awareness
  ...
  # for all of these modules, make wires out of their ports
...
sub get_sgdma_external_port_list {
  ...
  # If descriptor_read_burst is turned on, then we want to include the burstcount signal as well
  if ($DESCRIPTOR_READ_BURST) {
    push (@ports, "descriptor_read_burstcount");
  }

  # If we're not M-T-M mode, add the atlantic error signals
  # TODO : Determine if we want to allow optional usage of these Atlantic error signals
  if (!($HAS_READ_BLOCK && $HAS_WRITE_BLOCK)) {
  	push (@ports, "e_00"); 
    push (@ports, "e_05");
    push (@ports, "e_06");
...
sub create_internal_fifo {
  ...
  # wire up the fifo the the Read and Write blocks
  $mod->add_contents(
    ...
    # ensure that source_stream_valid pops out the top
    ...
    #The sink_stream_endofpacket_sig will not set correctly if the following 2 conditions happens,
    #1. when we perform multiple SGDMA transfer without reset SGDMA code between each SGDMA transfer; 2. when SOP & EOP happen at same time. 
    #This is because transmitted_eop will only reset to zero one clock cycle after SOP=1, which prevent sink_stream_endofpacket_sig set to 1 together with EOP=1. 
    ...
    # assume data is out on the one cycle later
...
sub make_sgdma_fifos {
  ...
  # for some reasons, the core stop working if we put desc_address_fifo in memories
  # TODO : check da waveforms
...
########################################################################################
  # Function : do_device_family_name_mapping
  # input    : Device family name w/o spaces
  # returns  : Device family name with spaces

...
sub make_read_block {
  ...
  # the fifo includes these signals : data, sop, eop, empty
  my $internal_fifo_width = $READ_BLOCK_DATA_WIDTH + 2 + $bits_to_encode_symbols_per_beat;

  # Credit Interface - not used right now as we instantiate an internal readblockfifo
  ...
  # Add the code that calculates and posts transactions - request path
...
sub make_read_block_request_path {
  ...
  # Remember to not post more than we can handle, taking into account the burstcount
  ...
  # Add Avalon signals
  # We already defined the port widths for these signals, so we do not need to restate them here.
  $mod->add_contents(
...
sub make_read_block_command_input {
  ...
  $mod->add_contents(
    # First create the command_reg itself
...
sub add_read_block_ports {
  ...
  my @ports = (
    ...
    # read_go bit to indicate operation is busy
    [read_go=>1=>"output"],
  ...
  if ($OUT_ERROR_WIDTH > 0) {
    ...
    # regardless, we do not generate errors
    $mod->add_contents(
...
sub make_m_read_status_token {
  ...
  $mod->add_contents(
    ...
    # Note : References the atlantic_error_signals located within write_block.pm
    e_port->news(&get_atlantic_error_signals()),
    
    # ports for the status_token_fifo
    # Note : References the status_token_fifo_ports located within write_block.pm

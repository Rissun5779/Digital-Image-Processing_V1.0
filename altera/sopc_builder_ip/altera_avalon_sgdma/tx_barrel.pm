...
sub make_tx_barrel_logic {
  ...
  # Generate the different cases for out_data
  my %out_data_content;
...
if (!$is_mtm) {
  ...
  $mod->add_contents(
    ...
    # again, explicitly export the port. 
    ...
    # Internal SOP and EOP generation
    e_comment->new({comment=>"Now we generate the outputs based on the given shift input"}),
    ...
    # Assumes that the sink only samples empty at the right time. Hah!
    ...
    # because this signal is used internally, we need to tell europa to explicitly export it.

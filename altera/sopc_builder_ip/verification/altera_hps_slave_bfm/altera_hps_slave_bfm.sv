...
module altera_hps_slave_bfm (
   ...
   localparam ADDR_CH_DATA_W     = ID_W + ADDR_W + ADDR_USER_W + 18;
   ...
   localparam MAX_CH_W           = (WDATA_CH_DATA_W > ADDR_CH_DATA_W)? WDATA_CH_DATA_W : ADDR_CH_DATA_W;
   ...
   typedef logic [ADDR_USER_W-1 : 0]                                 AxiUser_t;
   ...
   function automatic AxiUser_t get_command_user(AxiCommandType_t command_type); // public
   ...
   function AxiCommandTrans_t __get_transaction (
      ...
      if (AxiCommandTrans_type == CMD_WADDRESS || AxiCommandTrans_type == CMD_RADDRESS) begin
         ...
         // decode the data signal -- adjust the user signal
         temp_command.id            = temp_data[lindex(ID_W)+lindex(ADDR_W)+lindex(ADDR_USER_W)+20 : lindex(ADDR_W)+lindex(ADDR_USER_W)+20];
         temp_command.address       = temp_data[lindex(ADDR_W)+lindex(ADDR_USER_W)+19 : lindex(ADDR_USER_W)+19];
         temp_command.user          = temp_data[lindex(ADDR_USER_W)+18         : 18];

...
set_module_property DISPLAY_NAME {DMA Controller Intel FPGA IP}
...
set_parameter_property allowByteTransactions DESCRIPTION {Status of transfer width in Byte.}
...
set_parameter_property allowDoubleWordTransactions DESCRIPTION {Status of transfer width in doubleword.}
...
set_parameter_property allowHalfWordTransactions DESCRIPTION {Status of transfer width in halfword.}
...
set_parameter_property allowQuadWordTransactions DESCRIPTION {Status of transfer width in quadword.}
...
set_parameter_property allowWordTransactions DESCRIPTION {Status of transfer width in word.}
...
set_parameter_property burstEnable DESCRIPTION {Enable burst transfers.}
set_parameter_property burstEnable AFFECTS_GENERATION {1}
set_parameter_property burstEnable HDL_PARAMETER {0}

add_parameter softresetEnable BOOLEAN
set_parameter_property softresetEnable DEFAULT_VALUE {true}
set_parameter_property softresetEnable DISPLAY_NAME {Enable soft reset}
set_parameter_property softresetEnable DESCRIPTION {Enable soft reset.}
set_parameter_property softresetEnable AFFECTS_GENERATION {1}
set_parameter_property softresetEnable HDL_PARAMETER {0}
...
set_parameter_property fifoDepth DESCRIPTION {The depth of the FIFO buffer used for data transfers.}
...
set_parameter_property maxBurstSize DESCRIPTION {Maximum burst size
...
set_parameter_property minimumDmaTransactionRegisterWidth DESCRIPTION {Width of the DMA length register (1-32) (bits).}
...
set_parameter_property useRegistersForFIFO DESCRIPTION {Construct FIFO from registers.}
...
add_display_item {DMA Parameters} {Soft reset} GROUP
add_display_item {Soft reset} softresetEnable PARAMETER
...
proc validate {} {
	...
	set writeSlaveDataWidthMax [ get_parameter_value writeSlaveDataWidthMax ]
        ...
        send_message info "DMA Controller will only be supported in Quartus Prime Standard Edition in the future release."
	...
	# validate DMA slave address width max
	if {$readSlaveAddressWidthMax > 32 || $writeSlaveAddressWidthMax > 32} {
		send_message error "DMA does not support more than 32 bits slave address width"
	}
...
The maximum may be automatically increased to encompass the slave span.</html>"
	...
	if { $readSlaveAddressWidthMax > $writeSlaveAddressWidthMax } {
	    set maxspan $readSlaveAddressWidthMax
	} else {
	    set maxspan $writeSlaveAddressWidthMax
	}
...
proc elaborate {} {
	...
	# control register has 13 bit, hence need at least 13 bit wide.
...
proc generate {output_name output_directory rtl_ext simulation} {
	...
	set bigEndian [ proc_get_boolean_parameter bigEndian ]
	set burstEnable [ proc_get_boolean_parameter burstEnable ]
	set softresetEnable [ proc_get_boolean_parameter softresetEnable ]
	set fifoDepth [ get_parameter_value fifoDepth ]
	set maxBurstSize [ get_parameter_value maxBurstSize ]
	set minimumDmaTransactionRegisterWidth [ get_parameter_value minimumDmaTransactionRegisterWidth ]
	set minimumNumberOfByteTransfers [ get_parameter_value minimumNumberOfByteTransfers ]
	...
	send_message Info "softresetEnable = $softresetEnable"
	...
	puts $component_config "return {"
		...
		puts $component_config "\tsoftreset_enable					  => $softresetEnable,"

...
set_module_property DISPLAY_NAME {PIO (Parallel I/O) Intel FPGA IP}
...
set_parameter_property bitClearingEdgeCapReg DESCRIPTION {Enable bit-clearing for edge capture register to clear individual bit in the edge capture register.}
...
set_parameter_property bitModifyingOutReg DESCRIPTION {Enable individual bit setting/clearing}
...
set_parameter_property captureEdge DESCRIPTION {Synchronously capture low-to-high transitions,high-to-low transitions, or both.}
...
set_parameter_property direction DESCRIPTION {Direction can be set to Bidir, Input, InOut or Output}
...
set_parameter_property edgeType DESCRIPTION {Select edge type}
...
set_parameter_property generateIRQ DESCRIPTION {To assert an IRQ output when a specified event occurs on input ports.}
...
set_parameter_property irqType DESCRIPTION {Select an IRQ Type}
...
set_parameter_property resetValue DESCRIPTION {Output Port Reset Value}
...
set_parameter_property simDoTestBenchWiring DESCRIPTION {Hardwire PIO inputs in test bench}
...
set_parameter_property simDrivenValue DISPLAY_NAME {Drive inputs to field.}
set_parameter_property simDrivenValue DESCRIPTION {Hardwire PIO inputs in test bench to set the PIO input ports to a certain value in the testbench, and specify the value in Drive inputs to field.}
...
set_parameter_property width DESCRIPTION {Width of the I/O ports}
...
# ---- part 3 update software assignment with validation value ----	
  ...
  switch $direction {
	...
	set_module_assignment embeddedsw.dts.params.altr,gpio-bank-width $width
  ...
  set_parameter_value derived_has_irq $derived_has_irq
	...
	if { $derived_has_irq } {
		...
		set interrupt_type 0
		if {$irqType == "LEVEL"} {
			set interrupt_type 4
		}
		if {$irqType == "EDGE"} {
			if {$edgeType == "RISING"} {
				set interrupt_type 1
			}
			if {$edgeType == "FALLING"} {
				set interrupt_type 2
			}
			if {$edgeType == "ANY"} { 
				set interrupt_type 3
			}
		}
		if {$interrupt_type != 0} {
			set_module_assignment embeddedsw.dts.params.altr,interrupt_type $interrupt_type
			set_module_assignment {embeddedsw.dts.params.altr,interrupt-type} $interrupt_type
		}
	}
}
...
add_documentation_link "User Guide" https://documentation.altera.com/#/link/sfo1400787952932/iga1401394825911 

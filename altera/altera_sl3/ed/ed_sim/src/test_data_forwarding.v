...
// agreement for further details.
      ...
      if (test_name == "data_forwarding") begin
         ...
         // Wait for the deassertion of rx_user_clock_reset
         while (test_env.rx_user_clock_reset == 1'b1) @(posedge test_env.rx_user_clock);

         // Wait for the assertion of rx_link_up
         while (test_env.rx_link_up == 1'b0) @(posedge test_env.rx_user_clock);
         ...
         if (source_burst_gap_error_count != 0) begin

            test_abort = 1;
            $write("   \n");
            $write("   *** ERROR *** %0d source burst gap errors detected\n", source_burst_gap_error_count);

         end

         if (source_err_interrupt_count != 0) begin

            test_abort = 1;
            $write("   \n");
            $write("   *** ERROR *** %0d source error interrupts detected\n", source_err_interrupt_count);
         ...
         if (sink_err_interrupt_count != 0) begin

            test_abort = 1;
            $write("   \n");
            $write("   *** ERROR *** %0d sink error interrupts detected\n", sink_err_interrupt_count);

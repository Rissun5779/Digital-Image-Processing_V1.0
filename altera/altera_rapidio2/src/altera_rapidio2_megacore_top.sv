...
//##############################################################################
  #( //----------------------------------------------------
     ...
     parameter HEADER_BUF_DEPTH                = 32,
     parameter PAYLOAD_BUF_DEPTH               = 256,
     //----------------------------------------------------
...
//##############################################################################
   ...
   input  logic        external_io_error_response_set,
...
localparam int MAINTENANCE_SRC_DEST_ID_PORTWIDTH = (TRANSPORT_LARGE == 1'b0)? 8 : 16;
...
logic [(MAINTENANCE_SRC_DEST_ID_PORTWIDTH - 1):0]       mnt_s_err_dest_id;                  
logic [(MAINTENANCE_SRC_DEST_ID_PORTWIDTH - 1):0]       mnt_s_err_src_id;                     
logic [3:0]       mnt_s_err_ftype;                      
logic [3:0]       mnt_s_err_ttype;
logic [28:0]      mnt_s_err_address;
logic             mnt_tx_error;                 
...
logic [(8*(TRANSPORT_LARGE+1))-1:0]       drbell_s_err_source_id;
logic [(8*(TRANSPORT_LARGE+1))-1:0]       drbell_s_err_destination_id;
    ...
    begin
      ...
      transport_top
        (
         ...
         .mnt_packet_size                      ((MAINTENANCE_MASTER | MAINTENANCE_SLAVE)? mnt_tx_packet_size : 9'd0),
         .io_m_packet_size                     ((IO_MASTER)?iom_tx_packet_size:9'd0),
         .io_s_packet_size                     ((IO_SLAVE)? ios_tx_packet_size:9'd0),
         .drb_packet_size                      ((DOORBELL)? drb_tx_packet_size:9'd0),
         .gen_packet_size                      ((PASS_THROUGH)? ext_tx_packet_size: 9'd0),
         // Outputs
  ...
  if(MAINTENANCE_MASTER == 1'b1 || MAINTENANCE_SLAVE == 1'b1)
    begin
        ...
        begin
          ...
          maintenance_dev8
            ...
            // Outputs
             ...
             .mnt_s_err_address                    (mnt_s_err_address)
          ...
          mnt_adaptor
            ( 
             ...
             .clk                                  (clk),
             .rst_n                                (clk_rst_n),
             .transport_type                       (TRANSPORT_LARGE),
             // Outputs                                            
      ...
      begin
          ...
          assign mnt_tx.channel = 1'b0;  //signal is 1 bit only
          assign mnt_tx.error = 1'b0;    //signal is 1 bit only
...
generate
  if(IO_MASTER == 1'b1) begin
      ...
      assign iom_tx.channel[0] = 1'b0;
      assign iom_tx.error[0] = 1'b0;
...
generate
  if(IO_SLAVE == 1'b1) begin
      ...
      assign ios_tx.channel[0] = 1'b0;
      assign ios_tx.error[0] = 1'b0;
...
generate
  if(DOORBELL == 1'b1) begin
    ...
    drbell_adaptor
     ...
     assign drb_tx.channel[0] = 1'b0;
     assign drb_tx.error[0] = 1'b0;
...
//##############################################################################
  ...
  assign err_mgmt_intf.emef_logical_trans_err_detect_csr.external_io_error_response_set                =  external_io_error_response_set;

...
#(
     ...
     parameter IO_SLAVE_OUTSTANDING_NWRITE_RS  = 32,
     ...
     parameter XCVR_RECONFIG                   = 1,//not in use
     parameter XCVR_RCFG_SHARED                = 0,//not in use
...
(
  ...
  //------------------------
   ...
   output logic             pll_locked,  //export. needed by reset controller. 1-bit only.
  ...
  //---------------------------------------------------------------------
   generate
        ...
        begin : elecidle_sync
            altera_std_synchronizer_nocut #(.depth(SYNC_DEPTH)) 
                    tx_elecidle_inst (
                    .clk(tx_std_clkout[0]),
                    .reset_n(1'b1),
                    .din(force_electrical_idle[j]),
                    .dout(tx_std_elecidle[j])
            );

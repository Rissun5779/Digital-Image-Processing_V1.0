...
set_module_property DISPLAY_NAME "RapidIO II (IDLE2 up to 6.25 Gbaud) Intel FPGA IP"
set_module_property INSTANTIATE_IN_SYSTEM_MODULE true
set_module_property EDITABLE true
set_module_property HIDE_FROM_SOPC true
set_module_property DATASHEET_URL "http://www.altera.com/literature/ug/ug_rapidio2.pdf"
set_module_property SUPPORTED_DEVICE_FAMILIES {{Stratix 10} {Arria 10} {Stratix V} {Arria V} {Arria V GZ} {Cyclone V}}
########################
...
#------------------------------------------------------------------------
    ...
    set_parameter_property "DEVICE_FAMILY" ALLOWED_RANGES { "Stratix 10" "Arria 10" "Stratix V" "Arria V" "Arria V GZ" "Cyclone V" }
    ...
    set_parameter_property "RSIZE" VISIBLE false
    set_parameter_property "RSIZE" DERIVED true

    add_parameter "ASIZE" integer 0
    set_parameter_property "ASIZE" VISIBLE false
    set_parameter_property "ASIZE" DERIVED true
 
    add_parameter "CONST_1" integer 1
    set_parameter_property "CONST_1" VISIBLE false
    set_parameter_property "CONST_1" DERIVED true
    ...
    add_parameter "XCVR_ADME" boolean false
    set_parameter_property "XCVR_ADME" HDL_PARAMETER false
    set_parameter_property "XCVR_ADME" DISPLAY_NAME "Enable transceiver Altera Debug Master Endpoint"
    set_parameter_property "XCVR_ADME" VISIBLE true 
    set_parameter_property "XCVR_ADME" DESCRIPTION "When enabled, the PHY IP includes an embedded Altera Debug Master Endpoint that connects internally Avalon-MM slave interface. The ADME can access the reconfiguration space of the transceiver. It can perform certain test and debug functions via JTAG using the System Console. This option changes the reconfig interface to sharing between channels for configurations using more than 1 channel and may also require that a jtag_debug link be included in the system."
    add_display_item "Transceiver Settings" "XCVR_ADME" PARAMETER 

    add_display_item "Physical Layer" "Transceiver Settings" GROUP
    add_parameter "GUI_XCVR_RCFG_SHARED" boolean false
    set_parameter_property "GUI_XCVR_RCFG_SHARED" HDL_PARAMETER false
    set_parameter_property "GUI_XCVR_RCFG_SHARED" DISPLAY_NAME "Transceiver Share reconfiguration interface"
    set_parameter_property "GUI_XCVR_RCFG_SHARED" VISIBLE true
    set_parameter_property "GUI_XCVR_RCFG_SHARED" DERIVED false
    set_parameter_property "GUI_XCVR_RCFG_SHARED" DESCRIPTION "When enabled, the Native PHY presents a single Avalon-MM slave interface for dynamic reconfiguration of all channels. In this configuration the upper \[n:9\] address bits of the reconfiguration address bus specify the selected channel. Address bits \[8:0\] provide the register offset address within the reconfiguration space of the selected channel. This option is unavailable in 1x mode."
    add_display_item "Transceiver Settings" "GUI_XCVR_RCFG_SHARED" PARAMETER
    ...
    #Case:199987 remove MAINTENANCE_ADDRESS_WIDTH from GUI.
    add_parameter "MAINTENANCE_ADDRESS_WIDTH" integer 26
    set_parameter_property "MAINTENANCE_ADDRESS_WIDTH" HDL_PARAMETER true 
    set_parameter_property "MAINTENANCE_ADDRESS_WIDTH" VISIBLE false
    set_parameter_property "MAINTENANCE_ADDRESS_WIDTH" ENABLED 0
    ...
    #For PLL Arria 10 - derived from MAX_BAUD_RATE
    add_parameter "PLL_OUTCLK_FREQ" string "1562.5"
    set_parameter_property "PLL_OUTCLK_FREQ" VISIBLE false 
    set_parameter_property "PLL_OUTCLK_FREQ" DERIVED true
    set_parameter_property "PLL_OUTCLK_FREQ" HDL_PARAMETER false

    add_parameter "XCVR_RCFG_SHARED" boolean false
    set_parameter_property "XCVR_RCFG_SHARED" HDL_PARAMETER true
    set_parameter_property "XCVR_RCFG_SHARED" VISIBLE false
    set_parameter_property "XCVR_RCFG_SHARED" DERIVED true
...
proc my_validation_callback {} {
    ...
    if {[param_matches DEVICE_FAMILY "Arria 10" ] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
    ...
    if {[param_is_true ENABLE_TRANSPORT_LAYER]} {
		...
		param_enable "PROMISCUOUS"
                ...
                ## FB Case: 258138
                if {[param_is_false MAINTENANCE_GUI] && [param_is_false DOORBELL_GUI] && [param_is_false IO_MASTER_GUI] && [param_is_false IO_SLAVE_GUI] && [param_is_false PASS_THROUGH]} {
                    send_message error "At least one of the logical layer modules or the pass-through interface must be enabled"
        }
    } else {
    ...
    if {[param_matches DEVICE_FAMILY "Arria 10" ] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
        ...
        if {[param_is_true XCVR_RECONFIG]} {
            ...
            if {[param_is_true XCVR_ADME]} {
                param_disable "GUI_XCVR_RCFG_SHARED"
            } elseif { [param_is_true SUPPORT_4X] || [param_is_true SUPPORT_2X] } {
                param_enable "GUI_XCVR_RCFG_SHARED"
            } else {
                param_disable "GUI_XCVR_RCFG_SHARED"
            }

        } else {
            param_disable "XCVR_ADME"
            param_disable "GUI_XCVR_RCFG_SHARED"
        ...
        param_disable "XCVR_PRBS_SOFT_LOG_EN"
        param_disable "GUI_XCVR_RCFG_SHARED"
        param_disable "XCVR_RCFG_SHARED"
        set_parameter_property "GUI_XCVR_RCFG_SHARED" VISIBLE false
        set_parameter_property "XCVR_ADME" VISIBLE false
    }
...
proc my_elaboration_callback {} {
    my_xcvr_elaboration_callback
    #PLL generated for demo testbench usage, so not to include into QIP file. 
    #Note: If PLL is instantiated, need to remove the duplicated alt_xcvr_resync.sv from QIP because it is already declared by transceiver. 
    my_pll_elaboration_callback
    my_xcvr_rst_ctrl_elaboration_callback 
    ...
    if { [param_matches SUPPORTED_MODES "4x,2x,1x"] } {
       ...
       if { [param_is_true XCVR_RCFG_SHARED] } {
           set_parameter_value RSIZE 1
           set_parameter_value ASIZE 2
       } else {
           set_parameter_value RSIZE 4
           set_parameter_value ASIZE 0
       }
       ...
       if { [param_is_true XCVR_RCFG_SHARED] } {
           set_parameter_value RSIZE 1
           set_parameter_value ASIZE 1
       } else {
           set_parameter_value RSIZE 2
           set_parameter_value ASIZE 0
       }
    } else {
       set_parameter_value "SUPPORT_4X" false
   ...
   if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
    ...
    # For Arria 10 and Stratix 10. Minimum ref clk freq for ATX PLL is 100 MHz. 
    if { [param_matches REF_CLK_FREQ "62.5"] } {
        send_message error  "Reference clock frequency 62.5 MHz is not supported in this device family"
    } elseif { [param_matches REF_CLK_FREQ "78.125"] } {
        send_message error  "Reference clock frequency 78.125 MHz is not supported in this device family"
    }
   }
    ...
    if { [param_matches MAX_BAUD_RATE 1250] } {
       ...
       set_parameter_value "PLL_OUTCLK_FREQ" "625.0" 
       ...
       set_parameter_value "PLL_OUTCLK_FREQ" "1250.0" 
       ...
       set_parameter_value "PLL_OUTCLK_FREQ" "1562.5" 
       ...
       set_parameter_value "PLL_OUTCLK_FREQ" "2500.0" 
       ...
       set_parameter_value "PLL_OUTCLK_FREQ" "3125.0" 
    }
    ...
    # Derive shared reconfig based on ADME and channels 
    if { [param_matches DEVICE_FAMILY "Stratix 10"] || [param_matches DEVICE_FAMILY "Arria 10"] } {
        if { [param_is_true XCVR_RECONFIG] && ([param_is_true XCVR_ADME] || [param_is_true GUI_XCVR_RCFG_SHARED]) } {
            if { [param_is_true SUPPORT_4X ] } {
                set_parameter_value "XCVR_RCFG_SHARED" true
            } elseif { [param_is_true SUPPORT_2X ] } {
                set_parameter_value "XCVR_RCFG_SHARED" true
            } else {
                set_parameter_value "XCVR_RCFG_SHARED" false
            }
        } else {
            set_parameter_value "XCVR_RCFG_SHARED" false
        }
...
#------------------------------------------------------------------------
   ...
   #TODO     add_interface_port mnt_slave mnt_s_readerror    readerror output 1 
    ...
    add_interface_port mnt_slave_readerror mnt_s_readerror     mnt_s_readerror output 1
    ...
    if {[string compare -nocase [get_parameter_value MAINTENANCE] true] != 0 } { 
        ...
        set_port_property   mnt_mnt_s_irq     TERMINATION   true
    }
    ...
    add_interface_port io_slave_readerror ios_rd_wr_readresponse  ios_rd_wr_readresponse output 1
    ...
    add_interface_port io_slave ios_wr_address ios_wr_address input $io_slave_addr_width
    add_interface_port io_slave ios_wr_write ios_wr_write input 1    
    add_interface_port io_slave ios_wr_writedata ios_wr_writedata input 128
    add_interface_port io_slave ios_wr_byteenable ios_wr_byteenable input 16
    add_interface_port io_slave ios_wr_burstcount ios_wr_burstcount input 5
    #add_interface_port io_slave ios_wr_waitrequest ios_wr_waitrequest output 1
    ...
    add_interface_port io_slave ios_rd_address ios_rd_address input $io_slave_addr_width
    add_interface_port io_slave ios_rd_read ios_rd_read input 1        
    add_interface_port io_slave ios_rd_burstcount ios_rd_burstcount input 5 
    add_interface_port io_slave ios_rd_byteenable ios_rd_byteenable input 16 
    #add_interface_port io_slave ios_rd_waitrequest ios_rd_waitrequest output 1
    #add_interface_port io_slave ios_rd_readdata ios_rd_readdata output 128   
    #add_interface_port io_slave ios_rd_readresponse ios_rd_readresponse output 1  
    #add_interface_port io_slave ios_rd_readdatavalid ios_rd_readdatavalid output 1
    ...
    if {[string compare -nocase [get_parameter_value IO_SLAVE] true] != 0 } { 
        set_port_property ios_rd_wr_address TERMINATION TRUE
        set_port_property ios_rd_wr_address TERMINATION_VALUE 0
        set_port_property ios_rd_wr_write TERMINATION TRUE
        set_port_property ios_rd_wr_write TERMINATION_VALUE 0
        set_port_property ios_rd_wr_read  TERMINATION TRUE
        set_port_property ios_rd_wr_read  TERMINATION_VALUE 0
        set_port_property ios_rd_wr_writedata TERMINATION TRUE
        set_port_property ios_rd_wr_writedata TERMINATION_VALUE 0
        set_port_property ios_rd_wr_byteenable TERMINATION TRUE
        set_port_property ios_rd_wr_byteenable TERMINATION_VALUE 0
        set_port_property ios_rd_wr_burstcount TERMINATION TRUE
        set_port_property ios_rd_wr_burstcount TERMINATION_VALUE 0

        set_port_property ios_rd_wr_waitrequest   TERMINATION TRUE
        set_port_property ios_rd_wr_readdata      TERMINATION TRUE
        set_port_property ios_rd_wr_readdatavalid TERMINATION TRUE
        set_port_property ios_rd_wr_readresponse  TERMINATION TRUE
        set_port_property io_s_mnt_irq            TERMINATION TRUE
    }
    ...
    add_interface_port register_slave_readerror ext_mnt_readresponse  ext_mnt_readresponse output 1
    #Case:80128 writeresponse type is not supported in Qsys and this signal has little value for Register Access interface.
    #add_interface_port register_slave_readerror ext_mnt_writeresponse ext_mnt_writeresponse output 1
    ...
    add_interface_port io_master_readerror iom_rd_wr_readresponse  iom_rd_wr_readresponse input 1  
    ...
    if {[string compare -nocase [get_parameter_value IO_MASTER] true] != 0 } {
        set_port_property iom_rd_wr_readdatavalid  TERMINATION TRUE
        set_port_property iom_rd_wr_readdatavalid  TERMINATION_VALUE 0
        set_port_property iom_rd_wr_readdata       TERMINATION TRUE
        set_port_property iom_rd_wr_readdata       TERMINATION_VALUE 0
        set_port_property iom_rd_wr_waitrequest    TERMINATION TRUE
        set_port_property iom_rd_wr_waitrequest    TERMINATION_VALUE 0
        set_port_property iom_rd_wr_readresponse   TERMINATION TRUE
        set_port_property iom_rd_wr_readresponse   TERMINATION_VALUE 0
        set_port_property iom_rd_wr_read           TERMINATION TRUE 
        set_port_property iom_rd_wr_write          TERMINATION TRUE 
        set_port_property iom_rd_wr_writedata      TERMINATION TRUE
        set_port_property iom_rd_wr_burstcount     TERMINATION TRUE
        set_port_property iom_rd_wr_byteenable     TERMINATION TRUE 
        set_port_property iom_rd_wr_address        TERMINATION TRUE 
    }
    ...
    if {[string compare -nocase [get_parameter_value PASS_THROUGH] true] == 0 } {
        ...
        set_port_property gen_rx_pd_ready         TERMINATION TRUE
        set_port_property gen_rx_pd_ready         TERMINATION_VALUE 0
        set_port_property gen_rx_hd_ready         TERMINATION TRUE
        set_port_property gen_rx_hd_ready         TERMINATION_VALUE 0
        ...
        set_port_property gen_rx_pd_data          TERMINATION TRUE
        set_port_property gen_rx_pd_valid         TERMINATION TRUE
        set_port_property gen_rx_pd_startofpacket TERMINATION TRUE
        set_port_property gen_rx_pd_endofpacket   TERMINATION TRUE
        set_port_property gen_rx_pd_empty         TERMINATION TRUE
        set_port_property gen_rx_hd_data          TERMINATION TRUE
        set_port_property gen_rx_hd_valid         TERMINATION TRUE
        set_port_property gen_tx_ready            TERMINATION TRUE

        add_interface ext_tx_packet_size avalon_streaming sink sys_clk
        add_interface_port ext_tx_packet_size ext_tx_packet_size data input 9
        set_interface_property ext_tx_packet_size dataBitsPerSymbol 9
        set_port_property ext_tx_packet_size    TERMINATION TRUE
        set_port_property ext_tx_packet_size    TERMINATION_VALUE 0
    }
    ...
    if {[param_is_true ENABLE_TRANSPORT_LAYER]} {
        ...
        add_interface_port transport_rx_packet_dropped transport_rx_packet_dropped transport_rx_packet_dropped output 1
    }
    ...
    add_interface_port base_device_id base_device_id base output 8
    add_interface_port base_device_id large_base_device_id large_base output 16
    ####################################
    # CAR Interface
    ####################################
    add_interface car_csr_intf conduit end

    add_interface_port car_csr_intf car_device_id car_device_id input 16
    ...
    add_interface_port car_csr_intf car_device_vendor_id car_device_vendor_id input 16
    ...
    add_interface_port car_csr_intf car_device_revision_id car_device_revision_id input 32
    ...
    add_interface_port car_csr_intf car_assey_id car_assey_id input 16
    ...
    add_interface_port car_csr_intf car_assey_vendor_id car_assey_vendor_id input 16
    ...
    add_interface_port car_csr_intf car_revision_id car_revision_id input 16
    ...
    add_interface_port car_csr_intf car_bridge car_bridge input 1
    ...
    add_interface_port car_csr_intf car_memory car_memory input 1
    ...
    add_interface_port car_csr_intf car_processor car_processor input 1
    ...
    add_interface_port car_csr_intf car_switch car_switch input 1
    ...
    add_interface_port car_csr_intf car_num_of_ports car_num_of_ports input 8
    ...
    add_interface_port car_csr_intf car_port_num car_port_num input 8
    ...
    add_interface_port car_csr_intf car_extended_route_table car_extended_route_table input 1
    ...
    add_interface_port car_csr_intf car_standard_route_table car_standard_route_table input 1
    ...
    add_interface_port car_csr_intf car_flow_arbitration car_flow_arbitration input 1
    ...
    add_interface_port car_csr_intf car_flow_control car_flow_control input 1
    ...
    add_interface_port car_csr_intf car_max_pdu_size car_max_pdu_size input 16
    ...
    add_interface_port car_csr_intf car_segmentation_contexts car_segmentation_contexts input 16
    ...
    add_interface_port car_csr_intf car_max_destid car_max_destid input 16
    ...
    add_interface_port car_csr_intf car_source_operations car_source_operations input 32
    ...
    add_interface_port car_csr_intf car_destination_operations car_destination_operations input 32
    ...
    # CSR Interface
    ####################################
    add_interface_port car_csr_intf ef_ptr_reset_value ef_ptr_reset_value input 16
    ...
    add_interface_port car_csr_intf csr_tm_types_reset_value csr_tm_types_reset_value input 4 
    ...
    add_interface_port car_csr_intf csr_tm_mode_reset_value csr_tm_mode_reset_value input 4  
    ...
    add_interface_port car_csr_intf csr_mtu_reset_value csr_mtu_reset_value input 8  
    ...
    add_interface_port car_csr_intf tm_mode_wr tm_mode_wr input 1 
    set_port_property tm_mode_wr fragment_list "csr_external_tm_mode_wr"
 
    add_interface_port car_csr_intf mtu_wr mtu_wr input 1   
    set_port_property mtu_wr fragment_list "csr_external_mtu_wr"

    add_interface_port car_csr_intf tm_mode_in tm_mode_in input 4  
    set_port_property tm_mode_in fragment_list "csr_external_tm_mode_in(3:0)"

    add_interface_port car_csr_intf mtu_in mtu_in input 8
    set_port_property mtu_in fragment_list "csr_external_mtu_in(7:0)"

    add_interface_port car_csr_intf tm_types tm_types output 4
    add_interface_port car_csr_intf tm_mode tm_mode output 4
    add_interface_port car_csr_intf mtu mtu output 8
    if {[string compare -nocase [get_parameter_value ERROR_MANAGEMENT_EXTENSION] true] == 0 } {  
        add_interface error_management conduit end
        add_interface_port error_management message_error_response_set message_error_response_set input 1
        add_interface_port error_management gsm_error_response_set gsm_error_response_set input 1 
        add_interface_port error_management message_format_error_response_set message_format_error_response_set input 1  

        add_interface_port error_management illegal_transaction_decode_set illegal_transaction_decode_set input 1   
        set_port_property illegal_transaction_decode_set fragment_list "external_illegal_transaction_decode_set"

        add_interface_port error_management io_error_response_set io_error_response_set input 1    
        set_port_property io_error_response_set fragment_list "external_io_error_response_set"

        add_interface_port error_management message_request_timeout_set message_request_timeout_set input 1    
        set_port_property message_request_timeout_set fragment_list "external_message_request_timeout_set"

        add_interface_port error_management slave_packet_response_timeout_set slave_packet_response_timeout_set input 1   
        set_port_property slave_packet_response_timeout_set fragment_list "external_slave_packet_response_timeout_set"

        add_interface_port error_management unsolicited_response_set unsolicited_response_set input 1  
        set_port_property unsolicited_response_set fragment_list "external_unsolicited_response_set"

        add_interface_port error_management unsupported_transaction_set unsupported_transaction_set input 1  
        set_port_property unsupported_transaction_set fragment_list "external_unsupported_transaction_set"

        add_interface_port error_management illegal_transaction_target_error_set illegal_transaction_target_error_set input 1    
        set_port_property illegal_transaction_target_error_set fragment_list "external_illegal_transaction_target_error_set"

        add_interface_port error_management missing_data_streaming_context_set missing_data_streaming_context_set input 1   
        set_port_property missing_data_streaming_context_set fragment_list "external_missing_data_streaming_context_set"

        add_interface_port error_management open_existing_data_streaming_context_set open_existing_data_streaming_context_set input 1   
        ...
        add_interface_port error_management long_data_streaming_segment_set long_data_streaming_segment_set input 1   
        set_port_property long_data_streaming_segment_set fragment_list "external_long_data_streaming_segment_set"

        add_interface_port error_management short_data_streaming_segment_set short_data_streaming_segment_set input 1   
        set_port_property short_data_streaming_segment_set fragment_list "external_short_data_streaming_segment_set"

        add_interface_port error_management data_streaming_pdu_length_error_set data_streaming_pdu_length_error_set input 1  
        set_port_property data_streaming_pdu_length_error_set fragment_list "external_data_streaming_pdu_length_error_set"

        add_interface_port error_management external_capture_destinationID_wr external_capture_destinationID_wr input 1   
        ...
        add_interface_port error_management external_capture_destinationID_in external_capture_destinationID_in input 16  
        ...
        add_interface_port error_management external_capture_sourceID_wr external_capture_sourceID_wr input 1   
        ...
        add_interface_port error_management external_capture_sourceID_in external_capture_sourceID_in input 16   
        ...
        add_interface_port error_management capture_ftype_wr capture_ftype_wr input 1   
        set_port_property capture_ftype_wr fragment_list "external_capture_ftype_wr"

        add_interface_port error_management capture_ftype_in capture_ftype_in input 4  
        set_port_property capture_ftype_in fragment_list "external_capture_ftype_in(3:0)"

        add_interface_port error_management capture_ttype_wr capture_ttype_wr input 1   
        set_port_property capture_ttype_wr fragment_list "external_capture_ttype_wr"

        add_interface_port error_management capture_ttype_in capture_ttype_in input 4   
        set_port_property capture_ttype_in fragment_list "external_capture_ttype_in(3:0)"

        add_interface_port error_management letter_wr letter_wr input 1    
        set_port_property letter_wr fragment_list "external_letter_wr"

        add_interface_port error_management letter_in letter_in input 2   
        set_port_property letter_in fragment_list "external_letter_in(1:0)"

        add_interface_port error_management mbox_wr mbox_wr input 1
        set_port_property mbox_wr fragment_list "external_mbox_wr"

        add_interface_port error_management mbox_in mbox_in input 2
        set_port_property mbox_in fragment_list "external_mbox_in(1:0)"

        add_interface_port error_management msgseg_wr msgseg_wr input 1
        set_port_property msgseg_wr fragment_list "external_msgseg_wr"

        add_interface_port error_management msgseg_in msgseg_in input 4
        set_port_property msgseg_in fragment_list "external_msgseg_in(3:0)"

        add_interface_port error_management xmbox_wr xmbox_wr input 1
        set_port_property xmbox_wr fragment_list "external_xmbox_wr"

        add_interface_port error_management xmbox_in xmbox_in input 4
        set_port_property xmbox_in fragment_list "external_xmbox_in(3:0)"

        add_interface_port error_management port_degraded port_degraded output 1 
        add_interface_port error_management port_failed port_failed output 1
        add_interface_port error_management logical_transport_error logical_transport_error output 1
        add_interface_port error_management time_to_live time_to_live output 16
    } else {
        add_interface error_management conduit end
        add_interface_port error_management message_error_response_set message_error_response_set input 1
        set_port_property message_error_response_set TERMINATION TRUE
        set_port_property message_error_response_set TERMINATION_VALUE 0

        add_interface_port error_management gsm_error_response_set gsm_error_response_set input 1 
        set_port_property gsm_error_response_set TERMINATION TRUE
        set_port_property gsm_error_response_set TERMINATION_VALUE 0

        add_interface_port error_management message_format_error_response_set message_format_error_response_set input 1  
        set_port_property message_format_error_response_set TERMINATION TRUE
        set_port_property message_format_error_response_set TERMINATION_VALUE 0

        add_interface_port error_management external_illegal_transaction_decode_set external_illegal_transaction_decode_set input 1   
        set_port_property external_illegal_transaction_decode_set TERMINATION TRUE
        set_port_property external_illegal_transaction_decode_set TERMINATION_VALUE 0

        add_interface_port error_management external_io_error_response_set external_io_error_response_set input 1    
        set_port_property external_io_error_response_set TERMINATION TRUE
        set_port_property external_io_error_response_set TERMINATION_VALUE 0

        add_interface_port error_management external_message_request_timeout_set external_message_request_timeout_set input 1    
        set_port_property external_message_request_timeout_set TERMINATION TRUE
        set_port_property external_message_request_timeout_set TERMINATION_VALUE 0

        add_interface_port error_management external_slave_packet_response_timeout_set external_slave_packet_response_timeout_set input 1   
        set_port_property external_slave_packet_response_timeout_set TERMINATION TRUE
        set_port_property external_slave_packet_response_timeout_set TERMINATION_VALUE 0

        add_interface_port error_management external_unsolicited_response_set external_unsolicited_response_set input 1  
        set_port_property external_unsolicited_response_set TERMINATION TRUE
        set_port_property external_unsolicited_response_set TERMINATION_VALUE 0

        add_interface_port error_management external_unsupported_transaction_set external_unsupported_transaction_set input 1  
        set_port_property external_unsupported_transaction_set TERMINATION TRUE
        set_port_property external_unsupported_transaction_set TERMINATION_VALUE 0

        add_interface_port error_management external_illegal_transaction_target_error_set external_illegal_transaction_target_error_set input 1    
        set_port_property external_illegal_transaction_target_error_set TERMINATION TRUE
        set_port_property external_illegal_transaction_target_error_set TERMINATION_VALUE 0

        add_interface_port error_management external_missing_data_streaming_context_set external_missing_data_streaming_context_set input 1   
        set_port_property external_missing_data_streaming_context_set TERMINATION TRUE
        set_port_property external_missing_data_streaming_context_set TERMINATION_VALUE 0

        add_interface_port error_management external_open_existing_data_streaming_context_set external_open_existing_data_streaming_context_set input 1   
        set_port_property external_open_existing_data_streaming_context_set TERMINATION TRUE
        set_port_property external_open_existing_data_streaming_context_set TERMINATION_VALUE 0

        add_interface_port error_management external_long_data_streaming_segment_set external_long_data_streaming_segment_set input 1   
        set_port_property external_long_data_streaming_segment_set TERMINATION TRUE
        set_port_property external_long_data_streaming_segment_set TERMINATION_VALUE 0

        add_interface_port error_management external_short_data_streaming_segment_set external_short_data_streaming_segment_set input 1   
        set_port_property external_short_data_streaming_segment_set TERMINATION TRUE
        set_port_property external_short_data_streaming_segment_set TERMINATION_VALUE 0

        add_interface_port error_management external_data_streaming_pdu_length_error_set external_data_streaming_pdu_length_error_set input 1  
        set_port_property external_data_streaming_pdu_length_error_set TERMINATION TRUE
        set_port_property external_data_streaming_pdu_length_error_set TERMINATION_VALUE 0

        add_interface_port error_management external_capture_destinationID_wr external_capture_destinationID_wr input 1   
        set_port_property external_capture_destinationID_wr TERMINATION TRUE
        set_port_property external_capture_destinationID_wr TERMINATION_VALUE 0
        #add_interface_port error_management capture_destinationID_wr export input 1   
        #set_port_property capture_destinationID_wr fragment_list "external_capture_destinationID_wr"

        add_interface_port error_management external_capture_destinationID_in external_capture_destinationID_in input 16  
        set_port_property external_capture_destinationID_in TERMINATION TRUE
        set_port_property external_capture_destinationID_in TERMINATION_VALUE 0
        #add_interface_port error_management capture_destinationID_in export input 16   
        #set_port_property capture_destinationID_in fragment_list "external_capture_destinationID_in(15:0)"

        add_interface_port error_management external_capture_sourceID_wr external_capture_sourceID_wr input 1   
        set_port_property external_capture_sourceID_wr  TERMINATION TRUE
        set_port_property external_capture_sourceID_wr TERMINATION_VALUE 0
        #add_interface_port error_management capture_sourceID_wr export input 1    
        #set_port_property capture_sourceID_wr fragment_list "external_capture_sourceID_wr"

        add_interface_port error_management external_capture_sourceID_in external_capture_sourceID_in input 16   
        set_port_property external_capture_sourceID_in TERMINATION TRUE
        set_port_property external_capture_sourceID_in TERMINATION_VALUE 0
        #add_interface_port error_management capture_sourceID_in export input 16   
        #set_port_property capture_sourceID_in fragment_list "external_capture_sourceID_in(15:0)"

        add_interface_port error_management external_capture_ftype_wr external_capture_ftype_wr input 1   
        set_port_property external_capture_ftype_wr TERMINATION TRUE
        set_port_property external_capture_ftype_wr TERMINATION_VALUE 0

        add_interface_port error_management external_capture_ftype_in external_capture_ftype_in input 4  
        set_port_property external_capture_ftype_in TERMINATION TRUE
        set_port_property external_capture_ftype_in TERMINATION_VALUE 0

        add_interface_port error_management external_capture_ttype_wr external_capture_ttype_wr input 1   
        set_port_property external_capture_ttype_wr TERMINATION TRUE
        set_port_property external_capture_ttype_wr TERMINATION_VALUE 0

        add_interface_port error_management external_capture_ttype_in external_capture_ttype_in input 4   
        set_port_property external_capture_ttype_in TERMINATION TRUE
        set_port_property external_capture_ttype_in TERMINATION_VALUE 0

        add_interface_port error_management external_letter_wr external_letter_wr input 1    
        set_port_property external_letter_wr TERMINATION TRUE
        set_port_property external_letter_wr TERMINATION_VALUE 0

        add_interface_port error_management external_letter_in external_letter_in input 2   
        set_port_property external_letter_in TERMINATION TRUE
        set_port_property external_letter_in TERMINATION_VALUE 0

        add_interface_port error_management external_mbox_wr external_mbox_wr input 1
        set_port_property external_mbox_wr TERMINATION TRUE
        set_port_property external_mbox_wr TERMINATION_VALUE 0

        add_interface_port error_management external_mbox_in external_mbox_in input 2
        set_port_property external_mbox_in TERMINATION TRUE
        set_port_property external_mbox_in TERMINATION_VALUE 0

        add_interface_port error_management external_msgseg_wr external_msgseg_wr input 1
        set_port_property external_msgseg_wr TERMINATION TRUE
        set_port_property external_msgseg_wr TERMINATION_VALUE 0

        add_interface_port error_management external_msgseg_in external_msgseg_in input 4
        set_port_property external_msgseg_in TERMINATION TRUE
        set_port_property external_msgseg_in TERMINATION_VALUE 0

        add_interface_port error_management external_xmbox_wr external_xmbox_wr input 1
        set_port_property external_xmbox_wr TERMINATION TRUE
        set_port_property external_xmbox_wr TERMINATION_VALUE 0

        add_interface_port error_management external_xmbox_in external_xmbox_in input 4
        set_port_property external_xmbox_in  TERMINATION TRUE
        set_port_property external_xmbox_in TERMINATION_VALUE 0

        add_interface_port error_management port_degraded port_degraded output 1 
        set_port_property port_degraded TERMINATION TRUE

        add_interface_port error_management port_failed port_failed output 1
        set_port_property port_failed TERMINATION TRUE

        add_interface_port error_management logical_transport_error logical_transport_error output 1
        set_port_property logical_transport_error TERMINATION TRUE

        add_interface_port error_management time_to_live time_to_live output 16
        set_port_property time_to_live TERMINATION TRUE
    }
    ...
    add_interface_port phy_register_in host_reset_value host_reset_value input 1 
    ...
    add_interface_port phy_register_in master_enable_reset_value master_enable_reset_value input 1
    ...
    add_interface_port phy_register_in discovered_reset_value discovered_reset_value input 1
    ...
    add_interface_port phy_register_in flow_control_participant_reset_value flow_control_participant_reset_value input 1
    ...
    add_interface_port phy_register_in enumeration_boundary_reset_value enumeration_boundary_reset_value input 1
    ...
    add_interface_port phy_register_in flow_arbitration_participant_reset_value flow_arbitration_participant_reset_value input 1
    ...
    add_interface_port phy_register_in transmitter_type_reset_value transmitter_type_reset_value input 1
    ...
    add_interface_port phy_register_in receiver_type_reset_value receiver_type_reset_value input 2
    ...
    add_interface_port phy_register_in disable_destination_id_checking_reset_value disable_destination_id_checking_reset_value input 1
    ...
    add_interface_port control_symbol_rx_tx send_multicast_event send_multicast_event input 1
    add_interface_port control_symbol_rx_tx send_link_request_reset_device send_link_request_reset_device input 1
    add_interface_port control_symbol_rx_tx sent_link_request_reset_device sent_link_request_reset_device output 1
    add_interface_port control_symbol_rx_tx multicast_event_rx multicast_event_rx output 1 
    add_interface_port control_symbol_rx_tx link_req_reset_device_received link_req_reset_device_received output 1 
    add_interface_port control_symbol_rx_tx sent_multicast_event sent_multicast_event output 1

    add_interface physical_layer_status conduit end
    add_interface_port physical_layer_status port_initialized port_initialized output 1              
    add_interface_port physical_layer_status link_initialized link_initialized output 1   
    add_interface_port physical_layer_status port_ok port_ok output 1                   
    add_interface_port physical_layer_status port_error port_error output 1                    
    add_interface_port physical_layer_status packet_transmitted packet_transmitted output 1            
    add_interface_port physical_layer_status packet_cancelled packet_cancelled output 1             
    add_interface_port physical_layer_status packet_accepted_cs_sent packet_accepted_cs_sent output 1        
    add_interface_port physical_layer_status packet_retry_cs_sent packet_retry_cs_sent output 1          
    add_interface_port physical_layer_status packet_not_accepted_cs_sent packet_not_accepted_cs_sent output 1  
    add_interface_port physical_layer_status packet_accepted_cs_received packet_accepted_cs_received output 1  
    add_interface_port physical_layer_status packet_retry_cs_received packet_retry_cs_received output 1      
    add_interface_port physical_layer_status packet_not_accepted_cs_received packet_not_accepted_cs_received output 1 
    add_interface_port physical_layer_status packet_crc_error packet_crc_error output 1   
    add_interface_port physical_layer_status control_symbol_error control_symbol_error output 1
     # Master enable signal
    add_interface_port physical_layer_status master_enable master_enable output 1 
    if {[param_is_false XCVR_RESET_CTRL] } {
       #export these signals only when external reset controller is used.
       if {[param_is_true SUPPORT_4X] } {
          add_interface_port physical_layer_status four_lanes_aligned four_lanes_aligned output 1 
       }
       if {[param_is_true SUPPORT_2X] } {
          add_interface_port physical_layer_status two_lanes_aligned two_lanes_aligned output 1 
       }
    }
    ...
    add_interface_port serial_data rd rd input LSIZE
    add_interface_port serial_data td td output LSIZE
    if {[param_is_true SUPPORT_4X] } {
   ...
   if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
    add_interface_port transceiver pll_locked pll_locked output 1 
   }
    add_interface_port transceiver rx_is_lockedtoref rx_is_lockedtoref output LSIZE
    add_interface_port transceiver rx_is_lockedtodata rx_is_lockedtodata output LSIZE
    add_interface_port transceiver rx_syncstatus rx_syncstatus output LSIZE
    add_interface_port transceiver rx_signaldetect rx_signaldetect output LSIZE
    if {[param_is_false XCVR_RESET_CTRL] } {
       #export these signals only when external reset controller is used.
       add_interface_port transceiver tx_cal_busy tx_cal_busy output LSIZE
       add_interface_port transceiver rx_cal_busy rx_cal_busy output LSIZE
    }
   ...
   if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
    ...
    if { [param_matches SUPPORT_4X true]} {
        add_interface_port reconfig_to_xcvr reconfig_to_xcvr reconfig_to_xcvr input 350
        add_interface_port reconfig_from_xcvr reconfig_from_xcvr reconfig_from_xcvr output 230
    } elseif {[param_matches SUPPORT_2X true]} {
        add_interface_port reconfig_to_xcvr reconfig_to_xcvr reconfig_to_xcvr input 210
        add_interface_port reconfig_from_xcvr reconfig_from_xcvr reconfig_from_xcvr output 138
    } else {
    ...
    add_interface_port xcvr_rst_ctrl tx_ready tx_ready output LSIZE
    add_interface_port xcvr_rst_ctrl rx_ready rx_ready output LSIZE
    } else {
    # External
    add_interface xcvr_rst_ctrl conduit end
    add_interface_port xcvr_rst_ctrl tx_ready tx_ready input LSIZE
    add_interface_port xcvr_rst_ctrl rx_ready rx_ready input LSIZE
      # Not for Arria 10; pll_powerdown is from external reset controller to external PLL. 
      if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
      add_interface_port xcvr_rst_ctrl pll_powerdown pll_powerdown input 1
      }
    add_interface_port xcvr_rst_ctrl tx_analogreset tx_analogreset input LSIZE
    add_interface_port xcvr_rst_ctrl tx_digitalreset tx_digitalreset input LSIZE
    add_interface_port xcvr_rst_ctrl rx_analogreset rx_analogreset input LSIZE
    add_interface_port xcvr_rst_ctrl rx_digitalreset rx_digitalreset input LSIZE

    if { [param_matches DEVICE_FAMILY "Stratix 10"] } {
        add_interface_port xcvr_rst_ctrl tx_analogreset_stat    tx_analogreset_stat     output LSIZE
        add_interface_port xcvr_rst_ctrl tx_digitalreset_stat   tx_digitalreset_stat    output LSIZE
        add_interface_port xcvr_rst_ctrl rx_analogreset_stat    rx_analogreset_stat     output LSIZE
        add_interface_port xcvr_rst_ctrl rx_digitalreset_stat   rx_digitalreset_stat    output LSIZE
        }
    }
    ...
    if { [param_matches DEVICE_FAMILY "Arria 10"] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
        ...
        #separate the tx_bonding_clocks to per channel to enable connection in Qsys. 
            ...
            #add_interface_port tx_bonding_clocks tx_bonding_clocks clk input LSIZE*6
        }
        # External reconfig signals
        if { [param_is_true XCVR_RECONFIG] } {
            if { [param_is_true XCVR_ADME] || [param_is_true XCVR_RCFG_SHARED] } {
                if {[param_is_true SUPPORT_4X] || [param_is_true SUPPORT_2X] } {
                    add_interface reconfig_clk clock sink
                    add_interface_port reconfig_clk reconfig_clk clk input 1
                    set_port_property reconfig_clk VHDL_TYPE STD_LOGIC_VECTOR
                    ...
                    set_port_property reconfig_write VHDL_TYPE STD_LOGIC_VECTOR
                    add_interface_port reconfig_avmm reconfig_read        read        input  1
                    set_port_property reconfig_read VHDL_TYPE STD_LOGIC_VECTOR

                    if {[param_is_true SUPPORT_4X] } {
                        if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                            add_interface_port reconfig_avmm reconfig_address     address     input  12
                        } elseif { [param_matches DEVICE_FAMILY "Stratix 10" ]} {
                            add_interface_port reconfig_avmm reconfig_address     address     input  13
                        }
                    } elseif {[param_is_true SUPPORT_2X] } {
                        if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                            add_interface_port reconfig_avmm reconfig_address     address     input  11
                        } elseif { [param_matches DEVICE_FAMILY "Stratix 10" ]} {
                            add_interface_port reconfig_avmm reconfig_address     address     input  12
                        }
                    } 
                    ...
                    set_port_property reconfig_waitrequest VHDL_TYPE STD_LOGIC_VECTOR
                } else {
                    ...
                    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                        ...
                        add_interface_port reconfig_avmm_ch0 reconfig_address_ch0     address     input  11
                        set_port_property reconfig_address_ch0 fragment_list "reconfig_address(10:0)"
                    }
                ...
                if {[param_is_true SUPPORT_4X] } {
                    ...
                    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                        ...
                        add_interface_port reconfig_avmm_ch1 reconfig_address_ch1     address     input  11
                        set_port_property reconfig_address_ch1 fragment_list "reconfig_address(21:11)"
                    }
                    ...
                    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                        ...
                        add_interface_port reconfig_avmm_ch2 reconfig_address_ch2     address     input  11
                        set_port_property reconfig_address_ch2 fragment_list "reconfig_address(32:22)"
                    }
                    ...
                    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                        ...
                        add_interface_port reconfig_avmm_ch3 reconfig_address_ch3     address     input  11
                        set_port_property reconfig_address_ch3 fragment_list "reconfig_address(43:33)"
                    }
            ...
            if {[param_is_true SUPPORT_4X] } {
                ...
                if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                    ...
                    add_interface_port reconfig_avmm reconfig_address     address     input  44
                    set_port_property reconfig_address TERMINATION TRUE
                    set_port_property reconfig_address TERMINATION_VALUE 0
                }
                ...
                if { [param_matches DEVICE_FAMILY "Arria 10"] } {
                    ...
                    add_interface_port reconfig_avmm reconfig_address     address     input  22
                    set_port_property reconfig_address TERMINATION TRUE
                    set_port_property reconfig_address TERMINATION_VALUE 0
                }
    ...
    #############################################################
    #          add_hdl_instance for Arria 10 and Stratix 10
    #############################################################
proc my_xcvr_elaboration_callback {} {
    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
      add_hdl_instance altera_rapidio2_native_phy altera_xcvr_native_a10 18.1
      set_instance_property altera_rapidio2_native_phy HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value  altera_rapidio2_native_phy  base_device [get_parameter_value "BASE_DEVICE"]
      set_instance_parameter_value  altera_rapidio2_native_phy  duplex_mode "duplex"
      set_instance_parameter_value  altera_rapidio2_native_phy  protocol_mode "basic_std"
      set_instance_parameter_value  altera_rapidio2_native_phy  channels [get_parameter_value "LSIZE"]
      set_instance_parameter_value  altera_rapidio2_native_phy  bonded_mode "pma_only"
      set_instance_parameter_value  altera_rapidio2_native_phy  set_data_rate [get_parameter_value "MAX_BAUD_RATE"]
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_simple_interface 1
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_tx_pma_elecidle 1
      set_instance_parameter_value  altera_rapidio2_native_phy  set_cdr_refclk_freq [get_parameter_value "REF_CLK_FREQ"]
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_pma_clkout 0
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_is_lockedtodata 1
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_is_lockedtoref 1 
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_ports_rx_manual_cdr_mode 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_pcs_pma_width 20
      set_instance_parameter_value  altera_rapidio2_native_phy  std_tx_byte_ser_mode "Serialize x2"
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_byte_deser_mode "Deserialize x2"
      set_instance_parameter_value  altera_rapidio2_native_phy  std_tx_8b10b_enable 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_8b10b_enable 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_word_aligner_mode "synchronous state machine"
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_word_aligner_pattern 124
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_word_aligner_rknumber 127
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_word_aligner_renumber 3
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_word_aligner_rgnumber 255
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_std_wa_patternalign 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_tx_bitrev_enable 0
      set_instance_parameter_value  altera_rapidio2_native_phy  std_tx_byterev_enable 0
      set_instance_parameter_value  altera_rapidio2_native_phy  std_tx_polinv_enable 1	
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_tx_polinv 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_bitrev_enable 1 
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_std_bitrev_ena 1
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_byterev_enable 0 
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_std_byterev_ena 0
      set_instance_parameter_value  altera_rapidio2_native_phy  std_rx_polinv_enable 1
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_polinv 1
      set_instance_parameter_value  altera_rapidio2_native_phy  enable_port_rx_std_signaldetect 1
      set_instance_parameter_value  altera_rapidio2_native_phy  rcfg_enable [get_parameter_value "XCVR_RECONFIG"]
      set_instance_parameter_value  altera_rapidio2_native_phy  rcfg_jtag_enable [get_parameter_value "XCVR_ADME"]
      set_instance_parameter_value  altera_rapidio2_native_phy  rcfg_shared [get_parameter_value "XCVR_RCFG_SHARED"]
      set_instance_parameter_value  altera_rapidio2_native_phy  set_capability_reg_enable [get_parameter_value "XCVR_CAPABILITY_REG_EN"]
      set_instance_parameter_value  altera_rapidio2_native_phy  set_user_identifier [get_parameter_value "XCVR_SET_USER_IDENTIFIER"]
      set_instance_parameter_value  altera_rapidio2_native_phy  set_csr_soft_logic_enable [get_parameter_value "XCVR_CSR_SOFT_LOG_EN"]
      set_instance_parameter_value  altera_rapidio2_native_phy  set_prbs_soft_logic_enable [get_parameter_value "XCVR_PRBS_SOFT_LOG_EN"]
    } elseif { [param_matches DEVICE_FAMILY "Stratix 10"] } {
      add_hdl_instance altera_rapidio2_native_phy altera_xcvr_native_s10 18.1
      set_instance_property altera_rapidio2_native_phy HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value  altera_rapidio2_native_phy  base_device [get_parameter_value "BASE_DEVICE"]
      set_instance_parameter_value  altera_rapidio2_native_phy  duplex_mode "duplex"
      set_instance_parameter_value  altera_rapidio2_native_phy  protocol_mode "basic_std"
      ...
      set_instance_parameter_value  altera_rapidio2_native_phy  set_csr_soft_logic_enable [get_parameter_value "XCVR_CSR_SOFT_LOG_EN"]
      set_instance_parameter_value  altera_rapidio2_native_phy  set_prbs_soft_logic_enable [get_parameter_value "XCVR_PRBS_SOFT_LOG_EN"]
      set_instance_parameter_value  altera_rapidio2_native_phy  tx_fifo_mode "Phase compensation"
      set_instance_parameter_value  altera_rapidio2_native_phy  rx_fifo_mode "Phase compensation"
      set_instance_parameter_value  altera_rapidio2_native_phy  tx_clkout_sel "pcs_clkout"
      set_instance_parameter_value  altera_rapidio2_native_phy  rx_clkout_sel "pcs_clkout"
      set_instance_parameter_value  altera_rapidio2_native_phy  tx_coreclkin_clock_network "dedicated"
      set_instance_parameter_value  altera_rapidio2_native_phy  rx_coreclkin_clock_network "dedicated"
      set_instance_parameter_value  altera_rapidio2_native_phy  pcs_reset_sequencing_mode "not_bonded"
      ## TODO: http://fogbugz.altera.com/default.asp?384029
      # workaround due to default value from native PHY is invalid when parameterized to suit our core
      set_instance_parameter_value  altera_rapidio2_native_phy  tx_fifo_pfull 10
      set_instance_parameter_value  altera_rapidio2_native_phy  rx_fifo_pfull 10
    }
}

#For demo testbench usage
proc my_pll_elaboration_callback {} {
    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
      add_hdl_instance altera_rapidio2_pll altera_xcvr_atx_pll_a10 18.1
      set_instance_property altera_rapidio2_pll HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value  altera_rapidio2_pll  base_device [get_parameter_value "BASE_DEVICE"]
      set_instance_parameter_value  altera_rapidio2_pll  enable_8G_path "0"
      set_instance_parameter_value  altera_rapidio2_pll  set_output_clock_frequency [get_parameter_value "PLL_OUTCLK_FREQ"]
      set_instance_parameter_value  altera_rapidio2_pll  set_auto_reference_clock_frequency [get_parameter_value "REF_CLK_FREQ"]
      set_instance_parameter_value  altera_rapidio2_pll  enable_mcgb "1"
      set_instance_parameter_value  altera_rapidio2_pll  enable_bonding_clks "1"
      set_instance_parameter_value  altera_rapidio2_pll  pma_width "20"
    } elseif { [param_matches DEVICE_FAMILY "Stratix 10"] } {
      add_hdl_instance altera_rapidio2_pll altera_xcvr_atx_pll_s10 18.1
      set_instance_property altera_rapidio2_pll HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value  altera_rapidio2_pll  base_device [get_parameter_value "BASE_DEVICE"]
      set_instance_parameter_value  altera_rapidio2_pll  enable_8G_path "0"
      set_instance_parameter_value  altera_rapidio2_pll  set_output_clock_frequency [get_parameter_value "PLL_OUTCLK_FREQ"]
...
proc my_xcvr_rst_ctrl_elaboration_callback {} {
    if { [param_matches DEVICE_FAMILY "Arria 10"] } {
      add_hdl_instance xcvr_rst_ctrl altera_xcvr_reset_control 18.1
      set_instance_property xcvr_rst_ctrl HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value xcvr_rst_ctrl CHANNELS [get_parameter_value "LSIZE"]
      set_instance_parameter_value xcvr_rst_ctrl PLLS 1
      set_instance_parameter_value xcvr_rst_ctrl SYS_CLK_IN_MHZ 250
      set_instance_parameter_value xcvr_rst_ctrl SYNCHRONIZE_RESET 0
      set_instance_parameter_value xcvr_rst_ctrl REDUCED_SIM_TIME 1
      set_instance_parameter_value xcvr_rst_ctrl TX_PLL_ENABLE 1
      set_instance_parameter_value xcvr_rst_ctrl T_PLL_POWERDOWN 1000
      set_instance_parameter_value xcvr_rst_ctrl SYNCHRONIZE_PLL_RESET 0
      set_instance_parameter_value xcvr_rst_ctrl TX_ENABLE 1
      set_instance_parameter_value xcvr_rst_ctrl TX_PER_CHANNEL 0
      set_instance_parameter_value xcvr_rst_ctrl T_TX_ANALOGRESET 70000
      set_instance_parameter_value xcvr_rst_ctrl T_TX_DIGITALRESET 70000
      set_instance_parameter_value xcvr_rst_ctrl T_PLL_LOCK_HYST 0
      set_instance_parameter_value xcvr_rst_ctrl RX_ENABLE 1
      set_instance_parameter_value xcvr_rst_ctrl RX_PER_CHANNEL 1
      set_instance_parameter_value xcvr_rst_ctrl T_RX_ANALOGRESET 70000
      set_instance_parameter_value xcvr_rst_ctrl T_RX_DIGITALRESET 4000
    } elseif { [param_matches DEVICE_FAMILY "Stratix 10"] } {
      add_hdl_instance xcvr_rst_ctrl altera_xcvr_reset_control_s10 18.1
      set_instance_property xcvr_rst_ctrl HDLINSTANCE_USE_GENERATED_NAME 1
      set_instance_parameter_value xcvr_rst_ctrl CHANNELS [get_parameter_value "LSIZE"]
      set_instance_parameter_value xcvr_rst_ctrl PLLS 1
      set_instance_parameter_value xcvr_rst_ctrl SYS_CLK_IN_MHZ 250
      ...
      set_instance_parameter_value xcvr_rst_ctrl TX_ENABLE 1
      set_instance_parameter_value xcvr_rst_ctrl TX_PER_CHANNEL 0
      set_instance_parameter_value xcvr_rst_ctrl T_TX_ANALOGRESET 0
      set_instance_parameter_value xcvr_rst_ctrl T_TX_DIGITALRESET 20
      set_instance_parameter_value xcvr_rst_ctrl T_PLL_LOCK_HYST 0
      set_instance_parameter_value xcvr_rst_ctrl RX_ENABLE 1
      set_instance_parameter_value xcvr_rst_ctrl RX_PER_CHANNEL 1
      set_instance_parameter_value xcvr_rst_ctrl T_RX_ANALOGRESET 40
      set_instance_parameter_value xcvr_rst_ctrl T_RX_DIGITALRESET 4000
    }
}
...
# 3. Fileset
#------------------------------------------------------------------------
proc common_fileset {name toplevel_name language gensim simulator} {
    global env
    set qdir $env(QUARTUS_ROOTDIR)
   ...
   send_message INFO "top_rio2_rtl_module: $toplevel_name"

   ## IPD cross-clock synchronizer fix
   set ipd_synch_path $env(QUARTUS_ROOTDIR)/../ip/altera/primitives/altera_std_synchronizer
   add_fileset_file ${simulator}/altera_std_synchronizer_nocut.v $filekind PATH ${ipd_synch_path}/altera_std_synchronizer_nocut.v $simulator_specific

    # Top level - System Verilog files
    #-----------------------------------
    # Terp for TOP LEVEL RTL
    #-----------------------------------
    if {[param_matches DEVICE_FAMILY "Arria 10"] || [param_matches DEVICE_FAMILY "Stratix 10"] } {
       set native_phy_generated_fixed_name [get_instance_property altera_rapidio2_native_phy HDLINSTANCE_GET_GENERATED_NAME]
       set atx_pll_generated_fixed_name [get_instance_property altera_rapidio2_pll HDLINSTANCE_GET_GENERATED_NAME]
       set xcvr_rst_ctrl_generated_fixed_name [get_instance_property xcvr_rst_ctrl HDLINSTANCE_GET_GENERATED_NAME]
       send_message INFO "native_phy_wrapper_name: $native_phy_generated_fixed_name"
       send_message INFO "atx_pll_wrapper_name: $atx_pll_generated_fixed_name"
       send_message INFO "phy_reset_controller_wrapper_name: $xcvr_rst_ctrl_generated_fixed_name"
       ...
       set SYS_CLK_PERIOD  [ get_parameter_value SYS_CLK_PERIOD ]
       ...
       set ENABLE_TRANSPORT_LAYER      [ get_parameter_value ENABLE_TRANSPORT_LAYER ]
       set MAINTENANCE_SLAVE           [ get_parameter_value MAINTENANCE_SLAVE ]
       set MAINTENANCE_MASTER          [ get_parameter_value MAINTENANCE_MASTER ]
       set IO_SLAVE                    [ get_parameter_value IO_SLAVE ]
       set IO_MASTER                   [ get_parameter_value IO_MASTER ]
       set DOORBELL                    [ get_parameter_value DOORBELL ]
       set ERROR_MANAGEMENT_EXTENSION  [ get_parameter_value ERROR_MANAGEMENT_EXTENSION ]
       set TRANSPORT_LARGE             [ get_parameter_value TRANSPORT_LARGE ]
       set PASS_THROUGH                [ get_parameter_value PASS_THROUGH ]
       set PROMISCUOUS                 [ get_parameter_value PROMISCUOUS ]
       set MAINTENANCE_ADDRESS_WIDTH   [ get_parameter_value MAINTENANCE_ADDRESS_WIDTH ]
       set PORT_WRITE_TX               [ get_parameter_value PORT_WRITE_TX ]
       set PORT_WRITE_RX               [ get_parameter_value PORT_WRITE_RX ]
       set DOORBELL_TX_ENABLE          [ get_parameter_value DOORBELL_TX_ENABLE ]
       set DOORBELL_RX_ENABLE          [ get_parameter_value DOORBELL_RX_ENABLE ]
       set IO_SLAVE_DOORBELL_WRITES_ORDER_PRESERVATION    [ get_parameter_value IO_SLAVE_DOORBELL_WRITES_ORDER_PRESERVATION ]
       set IO_MASTER_WINDOWS           [ get_parameter_value IO_MASTER_WINDOWS ]
       set IO_SLAVE_WINDOWS            [ get_parameter_value IO_SLAVE_WINDOWS ]
       set IO_MASTER_ADDRESS_WIDTH     [ get_parameter_value IO_MASTER_ADDRESS_WIDTH ]
       set IO_SLAVE_ADDRESS_WIDTH      [ get_parameter_value IO_SLAVE_ADDRESS_WIDTH ]
       set MAX_BAUD_RATE_WITH_UNIT     [ get_parameter_value MAX_BAUD_RATE_WITH_UNIT ]
       set REF_CLK_FREQ_WITH_UNIT      [ get_parameter_value REF_CLK_FREQ_WITH_UNIT ]
       set XCVR_RECONFIG               [ get_parameter_value XCVR_RECONFIG ]



       #Do Terp
       if {[param_matches DEVICE_FAMILY "Arria 10"]} {
           set top_template_file [ file join $tmpdir "altera_rapidio2_top_a10.sv.terp" ]  
           set params_ip(module_name) ${toplevel_name}
           set params_ip(instance_name) ${native_phy_generated_fixed_name}
       } elseif { [param_matches DEVICE_FAMILY "Stratix 10"]} {
           set top_template_file [ file join $tmpdir "altera_rapidio2_top_s10.sv.terp" ]  
           set params_ip(module_name) ${toplevel_name}
           set params_ip(instance_name) ${native_phy_generated_fixed_name}
       } 
       set template_top   [ read [ open $top_template_file r ] ]  
       set params_ip(dev_family) $DEVICE_FAMILY
       set params_ip(ref_clk_period_ip) $REF_CLK_PERIOD
       set params_ip(sys_clk_period_ip) $SYS_CLK_PERIOD
       if {$SUPPORT_4X == "false"} {
         set params_ip(support_4x) "0"
       } else {
         set params_ip(support_4x) "1"
       }
       if {$SUPPORT_2X == "false"} {
         set params_ip(support_2x) "0"
       } else {
         set params_ip(support_2x) "1"
       }
       if {$ENABLE_TRANSPORT_LAYER == "false"} {
         set params_ip(en_transport) "0"
       } else {
         set params_ip(en_transport) "1"
       }
       if {$MAINTENANCE_SLAVE == "false"} {
         set params_ip(maintenance_slave) "0"
       } else {
         set params_ip(maintenance_slave) "1"
       }
       if {$MAINTENANCE_MASTER == "false"} {
         set params_ip(maintenance_master) "0"
       } else {
         set params_ip(maintenance_master) "1"
       }
       if {$IO_SLAVE == "false"} {
         set params_ip(io_slave_gui) "0"
       } else {
         set params_ip(io_slave_gui) "1"
       }
       if {$IO_MASTER == "false"} {
         set params_ip(io_master_gui) "0"
       } else {
         set params_ip(io_master_gui) "1"
       }
       if {$DOORBELL == "false"} {
         set params_ip(doorbell_gui) "0"
       } else {
         set params_ip(doorbell_gui) "1"
       }
       if {$ERROR_MANAGEMENT_EXTENSION == "false"} {
         set params_ip(err_mgmnt_ext) "0"
       } else {
         set params_ip(err_mgmnt_ext) "1"
       }
       if {$TRANSPORT_LARGE == "false"} {
         set params_ip(transport_large) "0"
       } else {
         set params_ip(transport_large) "1"
       }
       if {$PASS_THROUGH == "false"} {
         set params_ip(pass_through) "0"
       } else {
         set params_ip(pass_through) "1"
       }
       if {$PROMISCUOUS == "false"} {
         set params_ip(promiscuous) "0"
       } else {
         set params_ip(promiscuous) "1"
       }
       if {$PORT_WRITE_TX == "false"} {
         set params_ip(port_wr_tx) "0"
       } else {
         set params_ip(port_wr_tx) "1"
       }
       if {$PORT_WRITE_RX == "false"} {
         set params_ip(port_wr_rx) "0"
       } else {
         set params_ip(port_wr_rx) "1"
       }
       if {$DOORBELL_TX_ENABLE == "false"} {
         set params_ip(drbell_tx_en) "0"
       } else {
         set params_ip(drbell_tx_en) "1"
       }
       if {$DOORBELL_RX_ENABLE == "false"} {
         set params_ip(drbell_rx_en) "0"
       } else {
         set params_ip(drbell_rx_en) "1"
       }
       if {$IO_SLAVE_DOORBELL_WRITES_ORDER_PRESERVATION == "false"} {
         set params_ip(ios_drbll_wr_order_preserve) "0"
       } else {
         set params_ip(ios_drbll_wr_order_preserve) "1"
       }
       if {$XCVR_RECONFIG == "false"} {
         set params_ip(xcvr_reconfig) "0"
       } else {
         set params_ip(xcvr_reconfig) "1"
       }

       set params_ip(mnt_add_width) $MAINTENANCE_ADDRESS_WIDTH
       set params_ip(sys_clk_freq) $SYS_CLK_FREQ
       set params_ip(max_baud_rate) $MAX_BAUD_RATE
       set params_ip(iom_window) $IO_MASTER_WINDOWS
       set params_ip(ios_window) $IO_SLAVE_WINDOWS
       set params_ip(iom_add_width) $IO_MASTER_ADDRESS_WIDTH
       set params_ip(ios_add_width) $IO_SLAVE_ADDRESS_WIDTH
       set params_ip(max_baud_rate_unit) $MAX_BAUD_RATE_WITH_UNIT
       set params_ip(ref_clk_freq_unit) $REF_CLK_FREQ_WITH_UNIT

       #Derive sys_clk frequency = MAX_BAUD_RATE/40
       if { [param_matches MAX_BAUD_RATE 1250] } {
           set params_ip(sys_clk_period_ip) 32000
       } elseif { [param_matches MAX_BAUD_RATE 2500] } {
           set params_ip(sys_clk_period_ip) 16000
       } elseif { [param_matches MAX_BAUD_RATE 3125] } { 
           set params_ip(sys_clk_period_ip) 12800
       } elseif { [param_matches MAX_BAUD_RATE 5000] } { 
           set params_ip(sys_clk_period_ip) 8000
       } elseif { [param_matches MAX_BAUD_RATE 6250] } { 
           set params_ip(sys_clk_period_ip) 6400
       }
   
       #Derive PLL refclk frequency in nano-second. 
       if { [param_matches REF_CLK_FREQ "50"] } {
          set params_ip(ref_clk_period_ns) 20000
       } elseif { [param_matches REF_CLK_FREQ "62.5"] } {
          set params_ip(ref_clk_period_ns) 16000
       } elseif { [param_matches REF_CLK_FREQ "78.125"] } {
          set params_ip(ref_clk_period_ns) 12800
       } elseif { [param_matches REF_CLK_FREQ "100"] } {
          set params_ip(ref_clk_period_ns) 10000
       } elseif { [param_matches REF_CLK_FREQ "125"] } {
          set params_ip(ref_clk_period_ns) 8000
       } elseif { [param_matches REF_CLK_FREQ "156.25"] } {
          set params_ip(ref_clk_period_ns) 6400
       } elseif { [param_matches REF_CLK_FREQ "195.3125"] } {
          set params_ip(ref_clk_period_ns) 5120
       } elseif { [param_matches REF_CLK_FREQ "200"] } {
          set params_ip(ref_clk_period_ns) 5000
       } elseif { [param_matches REF_CLK_FREQ "250"] } {
          set params_ip(ref_clk_period_ns) 4000
       } elseif { [param_matches REF_CLK_FREQ "312.5"] } {
          set params_ip(ref_clk_period_ns) 3200
       } elseif { [param_matches REF_CLK_FREQ "390.625"] } {
          set params_ip(ref_clk_period_ns) 2560
       } elseif { [param_matches REF_CLK_FREQ "400"] } {
          set params_ip(ref_clk_period_ns) 2500
       } elseif { [param_matches REF_CLK_FREQ "500"] } {
          set params_ip(ref_clk_period_ns) 2000
       } elseif { [param_matches REF_CLK_FREQ "625"] } {
          set params_ip(ref_clk_period_ns) 1600
       }


       set rio2top_inwd_file   [ altera_terp $template_top params_ip ]
   }  else {  
       set top_template_file [ file join $tmpdir "altera_rapidio2_top.sv" ]
       set fileID [open $top_template_file r]
       set temp_topfile ""
       # read the contents of the file
       while {[eof $fileID] != 1} {
           gets $fileID lineInfo
           # replace the top level entity name with the name provided during generation
           regsub -all "altera_rapidio2_top" $lineInfo "${toplevel_name}" lineInfo
           append temp_topfile "${lineInfo}\n"
       }
   }

   #
    if {[param_matches DEVICE_FAMILY "Arria 10"]} {
        set xcvr_common_path $env(QUARTUS_ROOTDIR)/../ip/altera/altera_xcvr_generic
        add_fileset_file ${simulator}/altera_xcvr_functions.sv $filekind_systemverilog PATH ${xcvr_common_path}/altera_xcvr_functions.sv $simulator_specific   
        add_fileset_file ${simulator}/${toplevel_name}.sv      $filekind_systemverilog TEXT $rio2top_inwd_file  $simulator_specific
    } elseif { [param_matches DEVICE_FAMILY "Stratix 10"]} {
        set xcvr_common_path $env(QUARTUS_ROOTDIR)/../ip/altera/altera_xcvr_generic
        add_fileset_file ${simulator}/altera_xcvr_functions.sv $filekind_systemverilog PATH ${xcvr_common_path}/altera_xcvr_functions.sv $simulator_specific   
        add_fileset_file ${simulator}/${toplevel_name}.sv      $filekind_systemverilog TEXT $rio2top_inwd_file  $simulator_specific
    } else { 
        add_fileset_file ${simulator}/${toplevel_name}.sv      $filekind_systemverilog TEXT $temp_topfile  $simulator_specific
    }
    ...
    if {[param_matches DEVICE_FAMILY "Arria 10"] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
    add_fileset_file ${simulator}/${name}_isg.sv  $filekind_systemverilog PATH ${tmpdir}/${simulator_path}/physical/phy2/altera_rapidio2_isg_a10.sv $simulator_specific
 } else {
    ...
    if {[param_matches DEVICE_FAMILY "Arria 10"] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
    add_fileset_file ${simulator}/${name}_rx_hd_pd_separator.sv  $filekind_systemverilog PATH ${tmpdir}/${simulator_path}/transport/altera_rapidio2_rx_hd_pd_separator_a10.sv $simulator_specific
 } else {
    ...
    if {[param_matches DEVICE_FAMILY "Arria 10"] || [param_matches DEVICE_FAMILY "Stratix 10" ]} {
       add_fileset_file ${simulator}/${name}_native_phy_ip_soft_pcs.sv $filekind_systemverilog PATH ${tmpdir}/${simulator_path}/altera_rapidio2_native_phy_ip/soft_logic/altera_rapidio2_soft_pcs_a10.sv $simulator_specific
    } else { 
   ...
   if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
    ...
    set params(module_name) ${toplevel_name}
    ...
    if {[string compare -nocase ${simulator} synthesis] == 0} {
        add_fileset_file altera_rapidio2_${toplevel_name}.sdc SDC TEXT $result $simulator_specific 
...
proc cust_demo_tb {name language gensim} {
    ...
    if { [expr $gensim == 1] } {
        ...
        if { [param_matches DEVICE_FAMILY "Stratix V"] || [param_matches DEVICE_FAMILY "Arria V"] || [param_matches DEVICE_FAMILY "Arria V GZ"] || [param_matches DEVICE_FAMILY "Cyclone V"] } {
            set simtop_template_file [ file join $demo_tb_lib "altera_rapidio2_top_with_reset_ctrl.sv.terp" ]  
        } elseif { [param_matches DEVICE_FAMILY "Arria 10"] } {
            set simtop_template_file [ file join $demo_tb_lib "altera_rapidio2_top_with_reset_ctrl_pll.sv.terp" ]  
            set params_sim(atx_pll_module) ${atx_pll_generated_fixed_name}
            set params_sim(rst_ctrl_module) ${xcvr_rst_ctrl_generated_fixed_name}
        } elseif { [param_matches DEVICE_FAMILY "Stratix 10"] } { 
            set simtop_template_file [ file join $demo_tb_lib "altera_rapidio2_top_with_reset_ctrl_pll_s10.sv.terp" ]  
            set params_sim(atx_pll_module) ${atx_pll_generated_fixed_name}
            set params_sim(rst_ctrl_module) ${xcvr_rst_ctrl_generated_fixed_name}
        }
        set params_sim(module_name) ${name}
        set template_simtop   [ read [ open $simtop_template_file r ] ]
        set rio2top_sim_file   [ altera_terp $template_simtop params_sim ]
 ...
 #           add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/altera_rapidio2_top_with_reset_ctrl.sv
            add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl.sv SYSTEM_VERILOG TEXT $rio2top_sim_file
            add_fileset_file tb/tb_rio.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/tb_rio.sv
        } elseif { [param_matches DEVICE_FAMILY "Arria 10"] } {
            #Arria 10
  #          add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl_pll.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/altera_rapidio2_top_with_reset_ctrl_pll.sv
            add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl_pll.sv SYSTEM_VERILOG TEXT $rio2top_sim_file
            add_fileset_file tb/tb_rio.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/tb_rio_a10.sv
        } elseif { [param_matches DEVICE_FAMILY "Stratix 10"] } {
#            add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl_pll.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/altera_rapidio2_top_with_reset_ctrl_pll_s10.sv
            add_fileset_file tb/altera_rapidio2_top_with_reset_ctrl_pll.sv SYSTEM_VERILOG TEXT $rio2top_sim_file
            add_fileset_file tb/tb_rio.sv SYSTEM_VERILOG PATH ${demo_tb_lib}/tb_rio_s10.sv
        }
    }
...
proc synthproc {name} {
    ...
    common_fileset altera_rapidio2 ${name} verilog 0 synthesis
...
proc verilogsimproc {name} {
    ...
    if {1} {
    common_fileset altera_rapidio2 ${name} "verilog" 1 mentor
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "verilog" 1 synopsys
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "verilog" 1 cadence
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "verilog" 1 aldec
    }
...
proc vhdlsimproc {name} {
    ...
    if {1} {
    common_fileset altera_rapidio2 ${name} "vhdl" 1 mentor
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "vhdl" 1 synopsys
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "vhdl" 1 cadence
    }
    if {1} {
    common_fileset altera_rapidio2 ${name} "vhdl" 1 aldec
    }
...
add_documentation_link "User Guide" http://www.altera.com/literature/ug/ug_rapidio2.pdf
add_documentation_link "Release Notes" https://documentation.altera.com/#/link/hco1421698042087/hco1421697912137

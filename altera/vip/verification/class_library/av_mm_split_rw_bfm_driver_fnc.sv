...
reg [INTERNAL_MEM_DWIDTH*8-1:0] `INTERNAL_MEM [0:(MEM_SIZE*8/INTERNAL_MEM_DWIDTH)];


reg [INTERNAL_MEM_DWIDTH-1:0] internal_mem [0:80000000]; // 256 bytes = 1 32beat burst
...
begin
   ...
   waitlen = {$random()} % READ_MAX_WAIT_TIME;
   `MM_SINK_RD.set_interface_wait_time(waitlen,0);
  ...
  internal_mem[address] = data;
   ...
   if ((`SEQ_CTR % 3) == 0) begin
      for (z=0;z<burst;z++) begin
         resp_time += {$random()} % (WRITE_MAX_WAIT_TIME_INC);

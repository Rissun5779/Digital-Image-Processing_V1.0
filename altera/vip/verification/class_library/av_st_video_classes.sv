...
class c_pixel #(parameter BITS_PER_CHANNEL = 8, CHANNELS_PER_PIXEL = 3) ;
    ...
    constraint c0 { foreach (channel[i]) channel[i] != 0 && channel[i] != '1; }; //'
...
class c_av_st_video_data #(parameter BITS_PER_CHANNEL = 8, CHANNELS_PER_PIXEL = 3) extends c_av_st_video_item ;
    ...
    rand int       video_line_length; // used to split the frame into lines for transmission 
    ...
    constraint c1 { video_length == height*width; }
    constraint c2 { video_length >= video_min_length; video_length <= video_max_length; video_length % pixels_in_parallel == 0; }
    constraint c3 { video_line_length >= 32; video_line_length <= 15360; video_line_length <= video_max_length; }
endclass
...
class c_av_st_video_control #(parameter BITS_PER_CHANNEL = 8, CHANNELS_PER_PIXEL = 3) extends c_av_st_video_item ;
    ...
    constraint c1 {  width >=  min_width; width  <= max_width; }
    constraint c2 { height >= min_height; height <= max_height; }

    //  [3][2][1][0]
    //   0  0  x  x    Progressive
    //   1  0  x  x    Interlaced F0 (Odd  lines)
    //   1  1  x  x    Interlaced F1 (Even lines)        
    constraint c3 {
        interlace dist {
            0 := 10,    
            8 := 5,    
...
class c_av_st_video_user_packet #(parameter DATA_WIDTH = 24) extends c_av_st_video_item;
    ...
    constraint c1 { data.size() inside {[min_length:max_length]}; data.size() % pixels_in_parallel == 0; } 
    constraint c2 { identifier inside {[1:9]};} 

...
task send_control;
    ...
    begin
       ...
       // $display("%t    CONTROL : %s Sending command, write = %0d, address = %0d data = %0h use_event = %0d ",$time, name, register_item.write, register_item.address, register_item.value, register_item.use_event);
        ...
        /*if ($urandom_range(100, 1) > idleness_probability)
            `MASTER_HIERARCHY_NAME.set_command_init_latency(0);
        else
            if($urandom_range(100, 1) < 50)
                `MASTER_HIERARCHY_NAME.set_command_init_latency(1);
            else
                `MASTER_HIERARCHY_NAME.set_command_init_latency(2);
        if(register_item.write)
            `MASTER_HIERARCHY_NAME.set_command_init_latency(2);
        else
            `MASTER_HIERARCHY_NAME.set_command_init_latency(0);*/

...
`define CLASSNAME c_av_mm_control_bfm_`SOURCE

class `CLASSNAME extends c_av_mm_control_base;

 bit [`BITS_PER_CHANNEL*`CHANNELS_PER_PIXEL-1:0] garbage_data; 
...
function new(mailbox #(c_av_mm_control_item) m_control);
    super.new(m_control);
endfunction

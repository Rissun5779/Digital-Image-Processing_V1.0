...
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_step_coeff.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_step_line.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_realign.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_controller.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_nn_channel.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_horizontal_channel.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_vertical_channel.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_bilinear_channel.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core_bilinear_coeffs.sv
add_static_sv_file src_hdl/alt_vip_scaler_alg_core.sv

add_static_misc_file src_hdl/alt_vip_scaler_alg_core.ocp
...
add_parameter H_KERNEL_BITS INTEGER 12
...
set_parameter_property V_COEFF_FILE DISPLAY_NAME "Vertical coefficients CSV file"
...
set_parameter_property H_COEFF_FILE DISPLAY_NAME "Horizontal coefficients CSV file"
...
add_parameter SHIFTED_MIRROR INTEGER 0
set_parameter_property SHIFTED_MIRROR DISPLAY_NAME "Shift mirroring to remove replication"
set_parameter_property SHIFTED_MIRROR ALLOWED_RANGES 0:1
set_parameter_property SHIFTED_MIRROR DISPLAY_HINT boolean
set_parameter_property SHIFTED_MIRROR HDL_PARAMETER false
set_parameter_property SHIFTED_MIRROR VISIBLE false
...
proc scl_alg_core_elaboration_callback {} {
    ...
    if {[string compare $algo_name NEAREST_NEIGHBOUR] == 0 } {
       set   v_taps_internal   1
    } else {
       if {[string compare $algo_name BILINEAR] == 0 } {
          set   v_taps_internal   2
       } else {
          set   v_taps_internal   [get_parameter_value V_TAPS]
       }
    }
    ...
    set din_width                   [expr $data_width * $v_taps_internal ]
...
proc scl_alg_core_validation_callback {} {
    ...
    if { [string compare $alg_name NEAREST_NEIGHBOUR] != 0 && [string compare $alg_name BILINEAR] != 0 } {
       set shifted_mirror [get_parameter_value SHIFTED_MIRROR]
       if { $shifted_mirror > 0 } {
          set v_taps [get_parameter_value V_TAPS]
          if { $v_taps % 2 == 0 } {
             send_message Error "Shifted mirroring is only supported for odd numbers of vertical taps"
          }
       }
    }
    ...
    if { $runtime_load == 0 && [string compare $algo_name POLYPHASE] == 0 } {
        ...
        if { [get_parameter_value ARE_IDENTICAL] > 0 } {
           ...
           if { $coeff_width > 16 } {
              send_message error   "Total coefficient width (fraction bits + integer bits + optional sign bit) must not exceed 16 bits with fixed coefficients"
           }
        } else {
    ...
    if { [get_parameter_value DST_WIDTH] > 8 } {
       send_message ERROR "Destination width must not exceed 8 bits"
    }
}

# -- Generation callback --
proc generate_cb {output_name} {
    set template_file "alt_vip_scaler_alg_core.v.terp"
    ...
    if {[string compare $alg_name NEAREST_NEIGHBOUR] == 0 } {
       set   v_taps_internal   1
    } else {
   ...
   if { $check == 1 } {
    ...
    set params(shifted_mirror)      $shifted_mirror
    ...
    if { $runtime_load == 0 } {
        if { [string compare $algo_name POLYPHASE] == 0 || [string compare $algo_name BICUBIC] == 0 } {
            ...
            set coeff_mif_content    [call_helper generateCoeff $jar_arg]

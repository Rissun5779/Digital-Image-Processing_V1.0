...
set_module_property NAME alt_vip_tpg_bars_alg_core
set_module_property DISPLAY_NAME "Test Pattern Generator Core (Colour Bars)"
...
add_static_sv_file src_hdl/alt_vip_tpg_bars_alg_core.sv
add_static_sv_file src_hdl/alt_vip_tpg_bars_alg_core_par_lut.sv
add_static_sv_file src_hdl/alt_vip_tpg_bars_alg_core_seq_lut.sv

add_static_misc_file  src_hdl/alt_vip_tpg_bars_alg_core.ocp

setup_filesets alt_vip_tpg_bars_alg_core
...
add_parameter           PATTERN        STRING               COLOUR_BARS
set_parameter_property  PATTERN        DISPLAY_NAME         "Test pattern"
set_parameter_property  PATTERN        ALLOWED_RANGES       {COLOUR_BARS GREYSCALE_BARS BW_BARS}
set_parameter_property  PATTERN        AFFECTS_ELABORATION  true
set_parameter_property  PATTERN        HDL_PARAMETER        false
...
add_parameter           GREYSCALE      INTEGER              0
set_parameter_property  GREYSCALE      HDL_PARAMETER        true
set_parameter_property  GREYSCALE      VISIBLE              false
set_parameter_property  GREYSCALE      DERIVED              true

add_parameter           BW_ONLY        INTEGER              0
set_parameter_property  BW_ONLY        HDL_PARAMETER        true
set_parameter_property  BW_ONLY        VISIBLE              false
set_parameter_property  BW_ONLY        DERIVED              true
...
proc tpg_elaboration_callback {} {
   ...
   if { [string compare $pattern GREYSCALE_BARS] == 0 } {
      set_parameter_value  GREYSCALE   1
   } else {
      set_parameter_value  GREYSCALE   0
   }
   if { [string compare $pattern BW_BARS] == 0 } {
      set_parameter_value  BW_ONLY   1
   } else {
      set_parameter_value  BW_ONLY   0
   }
...
proc tpg_validation_callback {} {
   ...
   if {[string compare $colour_space MONO] == 0} {
      ...
      if { [string compare $pattern GREYSCALE_BARS] != 0 } {
         if { [string compare $pattern BW_BARS] != 0 } {
            send_message Error "The COLOUR_BARS option cannot be selected for the MONO colour space"
         }
      }
      if { $num_colours != 1 } {

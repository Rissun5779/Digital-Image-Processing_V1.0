...
set_module_property  NAME           alt_vip_cl_crs_var_to_444
set_module_property  DISPLAY_NAME   "Chroma Resampler (variable to 4:4:4)"
...
proc composition_cb {} {
   ...
   if { $enable_444_or_user_or_422 > 0 } {
      ...
      if { $enable_444_or_user_or_422_and_420 > 0 } {
         ...
         if { $enable_vib_cmd > 0 } {
            add_connection video_in_cmd.av_st_dout    pip_other.av_st_din
         } else {
            set_interface_property  av_st_din   export_of   pip_other.av_st_din
         }
      }
   }
   ...
   if { $enable_444_or_user_and_422 > 0 } {
      ...
      set_instance_parameter_value   middle_demux     PIXELS_IN_PARALLEL            $pixels_in_parallel_out
      ...
      add_connection    pip_other.av_st_dout             middle_demux.av_st_din
   }
   ...
   if { $enable_422 > 0 } {
      ...
      set_instance_parameter_value  drop_422    PIXELS_IN_PARALLEL            $pixels_in_parallel_out
      ...
      if { $enable_444_or_user > 0 } {
         ...
         add_connection    pip_other.av_st_dout          drop_422.av_st_din
      }
   }
   ...
   if { $enable_422_and_420 > 0 } {
      ...
      set_instance_parameter_value   middle_mux    PIXELS_IN_PARALLEL            $pixels_in_parallel_out
      ...
      add_connection    pip_420_out.av_st_dout           middle_mux.av_st_din_0
   }
   
   add_instance      horiz_upsample    alt_vip_crs_h_up_core            $isVersion
   
   set_instance_parameter_value  horiz_upsample    BITS_PER_SYMBOL               $bits_per_symbol
   set_instance_parameter_value  horiz_upsample    ALGORITHM                     [get_parameter_value HORIZ_ALGORITHM]
   set_instance_parameter_value  horiz_upsample    CO_SITING                     [get_parameter_value HORIZ_CO_SITING]
   set_instance_parameter_value  horiz_upsample    ENABLE_LUMA_ADAPT             [get_parameter_value HORIZ_ENABLE_LUMA_ADAPT]
   set_instance_parameter_value  horiz_upsample    COLOR_PLANES_ARE_IN_PARALLEL  $color_planes_are_in_parallel
   set_instance_parameter_value  horiz_upsample    PIXELS_IN_PARALLEL            $pixels_in_parallel_out
   set_instance_parameter_value  horiz_upsample    PIPELINE_READY                $pipeline_ready
   set_instance_parameter_value  horiz_upsample    SRC_WIDTH                     8
   set_instance_parameter_value  horiz_upsample    DST_WIDTH                     8
   set_instance_parameter_value  horiz_upsample    CONTEXT_WIDTH                 8
   set_instance_parameter_value  horiz_upsample    TASK_WIDTH                    8
   
   add_connection    av_st_clk_bridge.out_clk         horiz_upsample.main_clock
   add_connection    av_st_reset_bridge.out_reset     horiz_upsample.main_reset
   
   if { $enable_422_and_420 > 0 } {
      add_connection    middle_mux.av_st_dout            horiz_upsample.av_st_din
   } else {
      if { $enable_420 > 0 } {
         add_connection    pip_420_out.av_st_dout        horiz_upsample.av_st_din
      } else {
         add_connection    drop_422.av_st_dout           horiz_upsample.av_st_din
      }
   }
   ...
   if { $enable_444_or_user > 0 } {
      ...
      if { $enable_422 > 0 } {
         add_connection    middle_demux.av_st_dout_1     output_mux.av_st_din_1 
      } else {
         add_connection    pip_other.av_st_dout          output_mux.av_st_din_1 
      }         
      add_connection    horiz_upsample.av_st_dout        output_mux.av_st_din_0           
   } else {
      set_interface_property  av_st_dout  export_of   horiz_upsample.av_st_dout
   }

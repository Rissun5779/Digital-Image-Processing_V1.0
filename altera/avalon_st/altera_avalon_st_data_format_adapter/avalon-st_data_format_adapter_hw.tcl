...
set_module_property NAME data_format_adapter
set_module_property DISPLAY_NAME "Avalon-ST Data Format Adapter"
        ...
        for {set dest 0} {$dest < $stopLimit} {incr dest} {
            ...
            if {$usedMem==false} {
                if {$indataTmp < $inSymbolsPerBeat} {
                    lset dataSource $dest "a_data${indataTmp}"
            ...
            if {$usedMem==false} {
                ...
                if {($outputFilledByMemoryAlone==false) || ($dest <= $outSymbolsPerBeat)} {
                    if {$indata < $inSymbolsPerBeat} {
                        lset dataSource $dest "a_data${indata}"
                        incr indata
                        set filled true
                    }
                }
...
proc createSRAMVerilogFile {name dataWidth depth numReadPorts clrOnRST} {
    ...
    set sramTemplate_file [ file join $this_dir "../AtlanticComponentsShared/lookaheadmultiportmemory.sv.terp" ] 
...
proc createVerilogAdapterFile {output_name} {
    set this_dir [ get_module_property MODULE_DIRECTORY ]
    set template_file [ file join $this_dir "avalon-st_data_format_adapter.sv.terp" ]        

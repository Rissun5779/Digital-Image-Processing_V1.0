...
module altdq_dqs2_arriaiigz_ddio_3reg_use_offset_ctrl (
...
begin
	arriaiigz_io_config dqs_io_config_1 (
	...
	arriaiigz_io_config dqsn_io_config_1 (
		    .datain(config_data_in),          
			.clk(config_clock_in),
			.ena(config_dqs_io_ena),
			.update(config_update),       
...
begin
	...
	begin :dqs_config_gen
			arriaiigz_dqs_config   dqs_config_inst
		( 
		.clk(config_clock_in),
		.datain(config_data_in),
		.dataout(),
...
begin
	...
	begin
		...
		arriaiigz_delay_chain dqs_in_delay_1(
			.datain             (dqsbusout_predelay),
			.delayctrlin        (dqsbusoutdelaysetting_dlc[0]),
			.dataout            (dqsbusout_preena)
		);

		arriaiigz_delay_chain dqs_ena_delay_1(
	...
	begin
		...
		arriaiigz_dqs_enable dqs_enable_block (
	...
	begin
		...
		begin
			...
			arriaiigz_delay_chain dqs_delay_1(
		...
		begin
			...
			arriaiigz_dqs_enable dqs_enablen_block (
...
begin
	...
	begin
		...
		arriaiigz_delay_chain oct_delay_1(
			.datain             (aligned_os_oct),
			.delayctrlin        (octdelaysetting1_dlc [0]),
			.dataout            (delayed_os_oct_1)
		);
		
		arriaiigz_delay_chain oct_delay_2(
	...
	begin
		...
		if (USE_DYNAMIC_CONFIG == "true")
		begin
			arriaiigz_delay_chain oct_delay_1_bar(
				.datain             (aligned_os_oct_bar),
				.delayctrlin        (octdelaysetting1_dlc [0]),
				.dataout            (delayed_os_bar_oct_1)
			);

			arriaiigz_delay_chain oct_delay_2_bar(
		...
		if (USE_BIDIR_STROBE == "true")
		begin
			arriaiigz_pseudo_diff_out   pseudo_diffa_0
			( 
				.i(os_delayed2),
				.o(os),
				.obar(os_bar)
...
generate
	...
	begin :pad_gen
		...
		begin
			...
			if (USE_DYNAMIC_CONFIG == "true")
			begin
				arriaiigz_delay_chain in_delay_1(
					.datain             (raw_input),

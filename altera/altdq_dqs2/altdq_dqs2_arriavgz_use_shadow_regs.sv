...
module altdq_dqs2_arriavgz_use_shadow_regs (
...
begin
	arriavgz_io_config dqs_io_config_1 (
	...
	arriavgz_io_config dqsn_io_config_1 (
		    .datain(config_data_in),          
			.clk(config_clock_in),
			.ena(config_dqs_io_ena),
			.update(config_update),       
...
begin
	...
	begin :dqs_config_gen
		...
		assign octdelaysetting2_dlc[c_num] = octdelaysetting2[c_num];
			...
			arriavgz_delay_chain rankselectread_delay(
...
generate
	...
	begin
		if (USE_DYNAMIC_CONFIG == "false") begin
			arriavgz_dqs_config dqs_config_placeholder_inst
			( 
...
generate
	...
	begin
		...
		arriavgz_delay_chain oct_delay_1
		(
		...
		arriavgz_delay_chain oct_delay_2
		(
...
begin
	...
	begin
		...
		if (USE_BIDIR_STROBE == "true")
		begin
			arriavgz_pseudo_diff_out   pseudo_diffa_0
			( 
				.oein(delayed_os_oe),
				.dtcin(delayed_oct),
				.oeout(diff_oe),
	...
	begin
        ...
        .devclrn(),
          ...
          .dffphasetransfer(),
				...
				arriavgz_ddio_out the_ddio_data (
          .clkhi(),
          .clklo(),
          .muxsel(),
          .areset(),
...
begin
	...
	begin	
			arriavgz_read_fifo_read_enable fifo_enable(
			.re(lfifo_rden),
			.rclk(),
			.plus2(1'b0), 
			.areset(),
	...
	.clkn(),
		...
		begin
			arriavgz_input_phase_alignment data_alignment_lo (
				.datain(sdr_data[0]),
			...
			arriavgz_input_phase_alignment data_alignment_hi (
				.datain(sdr_data[1]),
				.levelingclk(dq_shifted_clock),
				.zerophaseclk(zero_phase_clock),
	...
	begin :pad_gen
		...
		begin
			...
			begin
				...
				arriavgz_delay_chain in_delay_1(			
					.datain             (raw_input),
					.delayctrlin        (dq_inputdelaysetting_dlc),
					.dataout            (raw_input_delay)
				);

				arriavgz_delay_chain in_delay_2(			

...
set_parameter_property CAPTURE_STROBE_TYPE DESCRIPTION "The capture strobe can be single, differential or complementary."
set_parameter_property CAPTURE_STROBE_TYPE DISPLAY_HINT ""
set_parameter_property CAPTURE_STROBE_TYPE ALLOWED_RANGES {Single:Single Differential:Differential Complimentary:Complementary}
...
set_parameter_property DIFFERENTIAL_OUTPUT_STROBE DISPLAY_NAME "Differential/complementary output strobe"
set_parameter_property DIFFERENTIAL_OUTPUT_STROBE DESCRIPTION "Output strobe can be differential/complementary or not."
...
add_parameter EMIF_BYPASS_OCT_DDIO BOOLEAN false ""
set_parameter_property EMIF_BYPASS_OCT_DDIO DISPLAY_NAME "EMIF-only: Bypass OCT DDIO register"
set_parameter_property EMIF_BYPASS_OCT_DDIO DESCRIPTION "EMIF-only parameter: Bypasses DDIO register on OCT output path for HPHC designs"
set_parameter_property EMIF_BYPASS_OCT_DDIO DISPLAY_HINT ""
set_parameter_property EMIF_BYPASS_OCT_DDIO VISIBLE false
...
proc my_validate {} {
	...
	if {[string compare -nocase $dev_fam "arriav"] == 0 || [string compare -nocase $dev_fam "cyclonev"] == 0} {
		set_parameter_property DQS_PHASE_SETTING ALLOWED_RANGES {"0:0 degrees" "2:90 degrees"}
	}
	...
	if { [string compare -nocase $dev_fam "cyclonev"] == 0 } {
		set num_strobes 0
		set num_in_strobes 0
		set num_out_strobes 0

		if { [string compare -nocase [get_parameter_value PIN_TYPE] "input"] == 0 || [string compare -nocase [get_parameter_value PIN_TYPE] "bidir"] == 0} {
			if {[string compare -nocase [get_parameter_value CAPTURE_STROBE_TYPE] "single"]==0 } {
				set num_in_strobes 1
			} else {
				set num_in_strobes 2
			}
			set num_strobes $num_in_strobes
		}
		if { [string compare -nocase [get_parameter_value PIN_TYPE] "output"] == 0 || [string compare -nocase [get_parameter_value PIN_TYPE] "bidir"] == 0} {
			if {[string compare -nocase [get_parameter_value USE_OUTPUT_STROBE] "true"] == 0 } {
				if {[string compare -nocase [get_parameter_value DIFFERENTIAL_OUTPUT_STROBE] "true"] == 0} {
					set num_out_strobes 2
				} else {
					set num_out_strobes 1
				}
			}
			set num_strobes $num_out_strobes
		} 
		if {[string compare -nocase [get_parameter_value PIN_TYPE] "bidir"] == 0} {
			if  {[string compare -nocase [get_parameter_value USE_BIDIR_STROBE] "true"] == 0} {
				set num_strobes ${num_in_strobes}
			} else {
				set num_strobes [expr ${num_in_strobes} + ${num_out_strobes}]
			}
		}

		set max_num_pins [expr 12 - ${num_strobes}]
		set_parameter_property PIN_WIDTH ALLOWED_RANGES "1:${max_num_pins}"
	}
...
proc my_elaborate {} {
	...
	if {[string compare -nocase [get_parameter_value USE_DQS_ENABLE] "true"]==0} {
		...
		} elseif {[string compare -nocase [get_parameter_value USE_HARD_FIFOS_INTERNAL] "true"] != 0} {
			add_interface_port data_interface capture_strobe_ena export Input $dqs_ena_width
		}
...
proc generate_fileset { outputname fileset_type } {
	if {[string compare -nocase [get_parameter_value MODULE_VALID] false] == 0} {
		...
		append param_str ",EMIF_BYPASS_OCT_DDIO=[get_parameter_value EMIF_BYPASS_OCT_DDIO]"

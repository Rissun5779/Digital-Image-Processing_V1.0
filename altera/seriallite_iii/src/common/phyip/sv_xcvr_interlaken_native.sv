...
`define data_rate_int (str2hz(DATA_RATE)/1000000)       // data rate in Hz.  Must use time unit since its a 64-bit unsigned int
   ...
   wire [LINKWIDTH-1:0]     rx_fifopfull_raw; 
   wire [LINKWIDTH-1:0]     r_csr_rx_digitalreset;
...
//			       .pcs10g_rx_control_del("control_del_all"),
	      ...
	      interlaken_inst (
			       ...
			       .out_pld_10g_rx_pfull(rx_fifopfull_raw[ig +: num_bonded]),
   ...
   generate
	 ...
	 // Synchronize the tx_digitalreset
	   ...
	   alt_xcvr_resync #(
         ...
         //synchronize the csr_rx_digitalreset
		alt_xcvr_resync #(
    	   .INIT_VALUE (1)
	     ) inst_reset_sync_rx (
                 .clk    (rx_pld_clk       ),
                 .reset  (csr_rx_digitalreset[numlane]),
                 .d      (1'b0               ),
                 .q      (r_csr_rx_digitalreset[numlane]   )
             );
	 ...
	 //pfull is async so synchronize it with
	 altera_std_synchronizer #(
				   .depth (2)
				   ) stdsync_rxpfull ( 
							.clk(rx_pld_clk),
							.din(rx_fifopfull_raw[numlane]),
							.dout(rx_fifopfull[numlane]),
							.reset_n(~r_csr_rx_digitalreset[numlane])
							);

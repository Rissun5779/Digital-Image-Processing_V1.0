...
(
   ...
   parameter                     min_inter_burst_gap           = 2
)
(
   // Clocks and reset
   input                         user_clock,
...
`endif
   ...
   // Status interface
   output reg  [26:0]            tg_burst_count,
...
`endif
   ...
   reg  [26:0]            tg_burst_state_count;
   reg  [26:0]            next_tg_burst_state_count;
   ...
   always @(posedge user_clock or posedge user_clock_reset) begin
      ...
      if (user_clock_reset == 1'b1) begin
         ...
         if (stall_tg == 1'b0) begin
            ...
            sync                 <= next_sync;
            tg_burst_state_count <= next_tg_burst_state_count;
            ...
            sync                 <= sync;
            tg_burst_state_count <= tg_burst_state_count;
   ...
   always @* begin
      ...
      case (tg_state)
         ...
         TG_INIT: begin
            ...
            next_sync                  = 8'd0;
             next_tg_burst_state_count = 27'd0;
            next_burst_length          = 64'd0;
            next_tg_words_transferred  = 64'd0;

`ifdef SIMULATION
            next_inter_burst_count     = $urandom_range(max_inter_burst_gap, min_inter_burst_gap) - 8'd1;
`else
            next_inter_burst_count     = (prbs[15:8] == 0) ? 8'd0 : prbs[15:8] - 8'd1;
`endif
...
`ifdef SIMULATION
               next_burst_length          = $urandom_range(max_burst_size, 1);
`else
               next_burst_length          = prbs[15:0];
`endif

              if (next_burst_length == 1'd1 ) begin //single burst 
               next_tg_state              = TG_ACTIVE_COMPLETE;
               next_end_of_burst          = 1'b1;
              end else begin //next_burst_length > 1
...
`endif
	    ...
	    next_tg_burst_state_count  = tg_burst_state_count + 27'd1;
            next_burst_length          = burst_length;
            next_inter_burst_count     = 8'd0;
            next_tg_words_transferred  = tg_words_transferred + 64'd1;

             if ((tg_burst_state_count       == burst_length - 1)              ||
...
`endif
	...
	//
	//*********************************************************************
	wire [26:0] tg_burst_count_inc;
	assign tg_burst_count_inc = tg_burst_count + 27'd1;

	 always @(posedge user_clock or posedge user_clock_reset) begin

	   if (user_clock_reset == 1'b1) begin
		 tg_burst_count <= 27'd0;
	   end else begin
		  tg_burst_count   <= (start_of_burst == 1'b1) ? tg_burst_count_inc:tg_burst_count;
		 end
	end
	 //////////////////////////////////////////
...
`ifdef SIMULATION
   ...
   always @* begin
      ...
      if ((start_of_burst == 1'b1) && (end_of_burst == 1'b1) && (valid == 1'b1)) begin
         ...
         next_unbuf_burst_descr     = {`BURST_DESCR_START_END, next_inter_burst_count, sync, 8'd0, tg_words_transferred[31:0]};
         next_unbuf_burst_descr_wr  = 1'b1;

      end else if ((start_of_burst == 1'b1) && (valid == 1'b1)) begin

         next_unbuf_burst_descr     = {`BURST_DESCR_START, 8'd0, sync, 8'd0, tg_words_transferred[31:0]};
         next_unbuf_burst_descr_wr  = 1'b1;

      end else if ((end_of_burst == 1'b1) && (valid == 1'b1)) begin

         next_unbuf_burst_descr     = {`BURST_DESCR_END, next_inter_burst_count, sync, 8'd0, tg_words_transferred[31:0]};
         next_unbuf_burst_descr_wr  = 1'b1;

...
module test_env #
(
   parameter                  user_clock_frequency       = `USER_CLK_FREQ,
   parameter                  lanes                      = `XCVR_LANES,
   parameter                  pll_ref_freq               = `XCVR_REF_CLK_FREQ,
   parameter                  pll_ref_var                = `XCVR_REF_CLK_VAR,
   parameter                  data_rate                  = `XCVR_DATA_RATE,
   parameter                  meta_frame_length          = `META_FRAME_LEN,
   parameter                  ecc_enable                 = `ECC_ENABLE_VAL,
   parameter                  burst_gap                  = `BURST_GAP,
   parameter                  ADDRESS_WIDTH              = `ADDRESS_WIDTH,
   parameter                  reference_clock_frequency  = `REFCLK_FREQ,
   parameter                  coreclkin_frequency        = `CORECLKIN_FREQ,
   parameter                  reconfig_clk_period        = 10.000e3,        // mgmt_clk = 100 MHz
   ...
   wire   [ADDRESS_WIDTH-1:0]               src_phy_mgmt_address;
   ...
   wire   [ADDRESS_WIDTH-1:0]               snk_phy_mgmt_address;
  ...
  `ifndef SERIALLITE_III_FOR_A10
     //
     // Source Interlaken PHY IP reconfiguration controller only for SV/AVGZ
     //
     source_reconfig #
     (
        .lanes(lanes)
  ...
  `else
    assign src_reconfig_busy = 1'b0;
  `endif


   // Determine the total number of PLLS 
   // on per Six Pack
   ...
   // Instantiate TX - PLL and reset-ctrl for A10
   ...
   altera_xcvr_reset_control #( 
      ...
      .clock                   (src_phy_mgmt_clk),
      .reset                   (src_phy_mgmt_clk_reset),
...
// Decide device-family    
  `ifdef SERIALLITE_III_FOR_A10
   ...
   // Duplex Core
   //
   seriallite_iii_streaming duplex_inst
   (
      ...
      //.tx_serial_clk (tx_bonding_clocks),
   ...
   // Source core
   //
   seriallite_iii_streaming_source source_inst
   (
   ...
   // Sink Core
   //
   seriallite_iii_streaming_sink sink_inst
   (
      // Clocks and reset
   ...
   (
      ...
      .reconfig_clk_period(reconfig_clk_period)
   )
   testbench
   (
      .reconfig_clk(reconfig_clk),

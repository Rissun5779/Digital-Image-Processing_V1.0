...
`endif	 
   ...
   always @(posedge test_env.src_user_clock) begin
      ...
      if (test_env.src_error[0] == 1'b1) source_overflow_error_count = source_overflow_error_count + 1;
   ...
   initial begin
     ...
     `ifndef ADVANCED_CLOCKING  
        `ifndef DUPLEX_MODE 
            force test_env.sink_inst.seriallite_iii_streaming_sink.sink_clock_gen.user_clock   = test_env.source_inst.seriallite_iii_streaming_source.source_clock_gen.user_clock;
            force test_env.sink_inst.seriallite_iii_streaming_sink.sink_clock_gen.coreclkin = test_env.source_inst.seriallite_iii_streaming_source.source_clock_gen.coreclkin;
         `else 
            force test_env.duplex_inst.seriallite_iii_streaming.sink_clock_gen.user_clock   = test_env.duplex_inst.seriallite_iii_streaming.source_clock_gen.user_clock;
            force test_env.duplex_inst.seriallite_iii_streaming.sink_clock_gen.coreclkin = test_env.duplex_inst.seriallite_iii_streaming.source_clock_gen.coreclkin;
         `endif
     `endif	 
      timeout_count     = 0;
      test_timeout      = 0;
     ...
     //    $write("      Lane Rate  %0d                      = %0d \n", lane, test_env.skew_insertion_inst.lane_rate);
      ...
      `ifdef SERIALLITE_III_FOR_A10	
         while(test_env.pll_locked != 1) @(posedge test_env.src_user_clock);
      `endif
      //
      ...
      `ifdef DUPLEX_MODE
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.tx_clkout[0]); start_time   = $time;
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.tx_clkout[0]); end_time     = $time;
      `else
      @(posedge test_env.source_inst.seriallite_iii_streaming_source.tx_clkout[0]); start_time   = $time;
      @(posedge test_env.source_inst.seriallite_iii_streaming_source.tx_clkout[0]); end_time     = $time;
      `endif
      ...
      `ifdef ADVANCED_CLOCKING
      `ifdef DUPLEX_MODE
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.interface_clock_tx); start_time   = $time;
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.interface_clock_tx); end_time     = $time;      
      `else
      @(posedge test_env.source_inst.seriallite_iii_streaming_source.interface_clock); start_time   = $time;
      @(posedge test_env.source_inst.seriallite_iii_streaming_source.interface_clock); end_time     = $time;
      `endif
	  

      clock_period = end_time - start_time;

      $write("      interface_clock_tx frequency            = %0e MHz\n", 10e5/clock_period);
      `endif
      ...
      `ifdef DUPLEX_MODE
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.rx_clkout[0]); start_time  = $time;
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.rx_clkout[0]); end_time    = $time;
      `else
      @(posedge test_env.sink_inst.seriallite_iii_streaming_sink.rx_clkout[0]); start_time  = $time;
      @(posedge test_env.sink_inst.seriallite_iii_streaming_sink.rx_clkout[0]); end_time    = $time;
      `endif
      ...
      `ifdef ADVANCED_CLOCKING
      `ifdef DUPLEX_MODE
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.interface_clock_rx); start_time  = $time;
      @(posedge test_env.duplex_inst.seriallite_iii_streaming.interface_clock_rx); end_time    = $time;
      `else
      @(posedge test_env.sink_inst.seriallite_iii_streaming_sink.interface_clock); start_time  = $time;
      @(posedge test_env.sink_inst.seriallite_iii_streaming_sink.interface_clock); end_time    = $time;
      `endif

      clock_period = end_time - start_time;

      $write("      interface_clock_rx frequency            = %0e MHz\n", 10e5/clock_period);
      `endif

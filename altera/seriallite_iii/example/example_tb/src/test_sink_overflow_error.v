...
// agreement for further details.
      ...
      if (test_name == "sink_overflow_error") begin
         ...
         `ifdef DUPLEX_MODE
         force test_env.duplex_inst.seriallite_iii_streaming.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rdreq = {lanes{1'b0}};
         `else
         force test_env.sink_inst.seriallite_iii_streaming_sink.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rdreq = {lanes{1'b0}};
         `endif
         
         @(posedge test_env.snk_user_clock);
         `ifdef DUPLEX_MODE
         while (test_env.duplex_inst.seriallite_iii_streaming.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rx_adapt_fifo_full == {lanes{1'b0}}) @(posedge test_env.snk_user_clock);
         release test_env.duplex_inst.seriallite_iii_streaming.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rdreq;
         
         `else
         while (test_env.sink_inst.seriallite_iii_streaming_sink.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rx_adapt_fifo_full == {lanes{1'b0}}) @(posedge test_env.snk_user_clock);
         release test_env.sink_inst.seriallite_iii_streaming_sink.altera_sl3_sink_mac_inst.scm_sink.altera_sl3_sink_adaptation.rdreq;
         `endif
         
         // Wait for the source error to register, a test error, or a test timeout.
         while ((test_env.snk_error[lanes+2] == 1'b0) && (test_abort == 0)) begin
         ...
         if (test_env.snk_error[lanes+2] == 1'b1) begin

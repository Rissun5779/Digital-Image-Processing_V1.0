...
int mif_init() 
{
    ...
    *reg_ptr_status = 4;
    //Set up Offset User to pcs10g_tx_ctrl register_13 which is 0x138 (1 lane), 0x390 (2 lanes), 0x5E8 (3 lanes)
    *reg_ptr_offset = 0x390;

    //Ctrl write, mif_mode = 01b, enable rd, as its read modify register before writing read the contents of the register_13
    *reg_ptr_status = 6;
    //wait for busy to clear
    while((*(reg_ptr_status)) != 4);

   // setup data, field bit offset is 15 so set that bit to enable the error injection.
    mif = *reg_ptr_data;
    mif = mif | 0x00008000 ;
    *reg_ptr_data = mif;

README for Downloading the SerialLite III Example Design onto the Board
...
3. Change into the ../ed_hwtest/software and run the batch_script.sh
...
1. Compile the Seriallite III Example Design to generate the seriallite_streaming_demo.sof file 
   under the /ed_synth/ directory.
2. Change into the ../ed_hwtest/software and run the batch_script.sh:
   >./batch_script.sh
...
5. Download the seriallite_streaming_demo.sof file under the /ed_synth/ directory onto the FPGA board by:
     a. Using Programmer in Quartus
     b. In the Nios Command Shell, change into the /ed_synth/ directory. Specify the cable number of
        the USB cable by substituting it for $CABLE_NUMBER:
        >nios2-configure-sof $CABLE_NUMBER seriallite_streaming_demo.sof

6. Download the demo_control.elf file onto the board while still in the /ed_synth/ directory. In the            Nios Command Shell specify the cable number of the USB cable by substituting for $CABLE_NUMBER:
   > nios2-download -g -r $CABLE_NUMBER ../ed_hwtest/software/app/demo_control.elf

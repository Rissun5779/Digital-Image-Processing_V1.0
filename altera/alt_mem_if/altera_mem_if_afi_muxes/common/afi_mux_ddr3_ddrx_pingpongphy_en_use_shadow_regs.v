...
module afi_mux_ddr3_ddrx_pingpongphy_en_use_shadow_regs (
	clk,
	mux_sel,
	phy_mux_sel, 
	afi_addr,
...
localparam SEQ_MUX_CS_WIDTH = MRS_MIRROR_PING_PONG_ATSO ? AFI_CS_WIDTH : (AFI_CS_WIDTH / 2);
...
input           [SEQ_MUX_CS_WIDTH-1:0]  seq_mux_cs_n;
input       [AFI_CLK_EN_WIDTH/2-1:0]  seq_mux_cke;
input	       [AFI_ODT_WIDTH/2-1:0]  seq_mux_odt;
...
reg [AFI_CLK_EN_WIDTH/2-1:0] seq_mux_cke_r;
reg	[SEQ_MUX_CS_WIDTH-1:0] seq_mux_cs_n_r;
reg [AFI_ODT_WIDTH/2-1:0] seq_mux_odt_r;
...
reg	[AFI_CS_WIDTH-1:0] pp_seq_mux_cs_n_r;
reg [AFI_CLK_EN_WIDTH-1:0] pp_seq_mux_cke_r;
reg [AFI_ODT_WIDTH-1:0] pp_seq_mux_odt_r;

always @(*) begin
	for (beat=0; beat<AFI_RATE_RATIO; beat=beat+1) begin 
		if(MRS_MIRROR_PING_PONG_ATSO) begin
		pp_seq_mux_cs_n_r[(2 * CS_WIDTH_PER_SIDE * beat) +: CS_WIDTH_PER_SIDE] 
			<= seq_mux_cs_n_r[2 * CS_WIDTH_PER_SIDE * beat +: CS_WIDTH_PER_SIDE];
		end else begin
			pp_seq_mux_cs_n_r[(2 * CS_WIDTH_PER_SIDE * beat) +: CS_WIDTH_PER_SIDE] 
			<= seq_mux_cs_n_r[CS_WIDTH_PER_SIDE * beat +: CS_WIDTH_PER_SIDE];
		end
		
		pp_seq_mux_cke_r[(2 * CLK_EN_WIDTH_PER_SIDE * beat) +: CLK_EN_WIDTH_PER_SIDE] 
			<= seq_mux_cke_r[CLK_EN_WIDTH_PER_SIDE * beat +: CLK_EN_WIDTH_PER_SIDE];
		
		pp_seq_mux_odt_r[(2 * ODT_WIDTH_PER_SIDE * beat) +: ODT_WIDTH_PER_SIDE] 
			<= seq_mux_odt_r[ODT_WIDTH_PER_SIDE * beat +: ODT_WIDTH_PER_SIDE];
	end

	for (beat=0; beat<AFI_RATE_RATIO; beat=beat+1) begin
		if(MRS_MIRROR_PING_PONG_ATSO) begin
			pp_seq_mux_cs_n_r[(2 * CS_WIDTH_PER_SIDE * beat) + CS_WIDTH_PER_SIDE +: CS_WIDTH_PER_SIDE] <= seq_mux_cs_n_r[(2 * CS_WIDTH_PER_SIDE * beat) + CS_WIDTH_PER_SIDE +: CS_WIDTH_PER_SIDE];
		end else begin
		pp_seq_mux_cs_n_r[(2 * CS_WIDTH_PER_SIDE * beat) + CS_WIDTH_PER_SIDE +: CS_WIDTH_PER_SIDE]
			<= seq_mux_cs_n_r[CS_WIDTH_PER_SIDE * beat +: CS_WIDTH_PER_SIDE];
		end
		
		pp_seq_mux_cke_r[(2 * CLK_EN_WIDTH_PER_SIDE * beat) + CLK_EN_WIDTH_PER_SIDE +: CLK_EN_WIDTH_PER_SIDE]
			<= seq_mux_cke_r[CLK_EN_WIDTH_PER_SIDE * beat +: CLK_EN_WIDTH_PER_SIDE];
		
		pp_seq_mux_odt_r[(2 * ODT_WIDTH_PER_SIDE * beat) + ODT_WIDTH_PER_SIDE +: ODT_WIDTH_PER_SIDE]
			<= seq_mux_odt_r[ODT_WIDTH_PER_SIDE * beat +: ODT_WIDTH_PER_SIDE];
	end
end

assign phy_mux_sel	= mux_sel;
...
assign phy_mux_cs_n  = mux_sel ? pp_seq_mux_cs_n_r  : afi_cs_n_r;
assign phy_mux_cke   = mux_sel ? pp_seq_mux_cke_r   : afi_cke_r;
assign phy_mux_odt   = mux_sel ? pp_seq_mux_odt_r   : afi_odt_r;

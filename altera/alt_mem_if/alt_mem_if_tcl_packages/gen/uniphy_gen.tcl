...
proc ::alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 {prepend_str protocol tmpdir fileset} {
	...
	if {[string compare -nocase $protocol "DDR2"] == 0 || [string compare -nocase $protocol "DDR3"] == 0} {
		if {[string compare -nocase $device_family "ARRIAV"] == 0 || [string compare -nocase $device_family "CYCLONEV"] == 0} {
			if {[string compare -nocase [get_parameter_value HARD_EMIF] "true"] == 0} {
				if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_bypass_oct_ddio] == 1} {
					lappend param_list "EMIF_BYPASS_OCT_DDIO=true"
				} else {
					lappend param_list "EMIF_BYPASS_OCT_DDIO=false"
				}
			} else {
				lappend param_list "EMIF_BYPASS_OCT_DDIO=false"
			}
...
proc ::alt_mem_if::gen::uniphy_gen::generate_altgpio {tmpdir fileset} {
	...
	alt_mem_if::gen::uniphy_gen::generate_ip_from_quartus_sh altera_gpio_lite $arg_list $tmpdir "generate_altgpio.tcl"

	lappend return_files [file join $tmpdir submodules altera_gpio_lite.sv]
...
proc ::alt_mem_if::gen::uniphy_gen::derive_delay_params {protocol} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on bypass_dqs_delay_chain] == 1 } {
		_iprint "The DQS delay chain is being bypassed due to the ini bypass_dqs_delay_chain"
		...
		if {$is_ddrx_protocol == 1 && [string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "ARRIAV"] == 0} {
			...
			if {[::alt_mem_if::util::qini::cfg_is_on do_not_bypass_dqs_delay_chain] == 1 } {
				if { $hps_mode } {
	...
	if {[string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "STRATIXV"] == 0 || [string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "ARRIAVGZ"] == 0} {
		...
		if {[string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "ARRIAV"] == 0 &&
			...
			[string compare -nocase $protocol "QDRII"] == 0} {
			set_parameter_value IO_DQS_OUT_RESERVE 18
...
proc  ::alt_mem_if::gen::uniphy_gen::generate_sdc_file { core_name protocol intcl_dir tmpdir } {
	...
	if { [ string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "MAX10" ] == 0} {
		lappend core_params "IPTCL_MAX10"
	}
	...
	if {[string compare -nocase [get_parameter_value DUPLICATE_AC] "true"] == 0} {
		lappend core_params "IPTCL_DUPLICATE_AC"
	}
...
proc ::alt_mem_if::gen::uniphy_gen::generate_report_timing_files { core_name protocol intcl_dir tmpdir } {
	...
	if {[get_parameter_value MEM_CLK_FREQ] >= 200} {
		lappend core_params "IPTCL_200MHZ_AND_ABOVE"
	}
...
proc ::alt_mem_if::gen::uniphy_gen::generate_qsys_sequencer {prepend_str protocol tmpdir fileset inhdl_dir} {
		...
		if {[string compare -nocase $protocol "LPDDR2"] == 0} {
			...
			if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
				if {[string compare -nocase $protocol "DDR3"] == 0} {
					lappend param_list "MR1_RTT_RANK0=[get_parameter_value MR1_RTT_RANK0]"
					lappend param_list "MR1_RTT_RANK1=[get_parameter_value MR1_RTT_RANK1]"
					lappend param_list "MR1_RTT_RANK2=[get_parameter_value MR1_RTT_RANK2]"
					lappend param_list "MR1_RTT_RANK3=[get_parameter_value MR1_RTT_RANK3]"

					lappend param_list "MR1_RTT_DRV_RANK0=[get_parameter_value MR1_RTT_DRV_RANK0]"
					lappend param_list "MR1_RTT_DRV_RANK1=[get_parameter_value MR1_RTT_DRV_RANK1]"
					lappend param_list "MR1_RTT_DRV_RANK2=[get_parameter_value MR1_RTT_DRV_RANK2]"
					lappend param_list "MR1_RTT_DRV_RANK3=[get_parameter_value MR1_RTT_DRV_RANK3]"
				}
			}
			...
			if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
				if {[string compare -nocase $protocol "DDR3"] == 0} {
					lappend param_list "MR2_RTT_WR_RANK0=[get_parameter_value MR2_RTT_WR_RANK0]"
					lappend param_list "MR2_RTT_WR_RANK1=[get_parameter_value MR2_RTT_WR_RANK1]"
					lappend param_list "MR2_RTT_WR_RANK2=[get_parameter_value MR2_RTT_WR_RANK2]"
					lappend param_list "MR2_RTT_WR_RANK3=[get_parameter_value MR2_RTT_WR_RANK3]"
					lappend param_list "ADVANCED_ODT_CONTROL=1"
					lappend param_list "DDR3_MR1_ODT_MASK=[expr {0xFFFFFD99}]]"
					lappend param_list "DDR3_MR2_ODT_MASK=[expr {0xFFFFF9FF}]]"
					lappend param_list "CFG_READ_ODT_CHIP=[get_parameter_value RD_ODT_PATTERN]"
					lappend param_list "CFG_WRITE_ODT_CHIP=[get_parameter_value WR_ODT_PATTERN]"
				}
			}
	...
	lappend param_list "ENABLE_NON_DES_CAL=[get_parameter_value ENABLE_NON_DES_CAL]"	
	lappend param_list "ENABLE_NON_DES_CAL_TEST=[get_parameter_value ENABLE_NON_DES_CAL_TEST]"	

	if {[string compare -nocase $protocol "DDR3"] == 0} {
		lappend param_list "AP_MODE_EN=[get_parameter_value AP_MODE_EN]"
	}

	lappend param_list "USE_USER_RDIMM_VALUE==[get_parameter_value USE_USER_RDIMM_VALUE]";

	lappend param_list "MRS_MIRROR_PING_PONG_ATSO=[get_parameter_value MRS_MIRROR_PING_PONG_ATSO]"
	...
	lappend param_list "HHP_HPS=[get_parameter_value HHP_HPS]"
	lappend param_list "MAX10_RTL_SEQ=[get_parameter_value MAX10_RTL_SEQ]"
	...
	if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
	    set avl_clk_frequency [get_parameter_value PLL_AFI_CLK_FREQ]
	} else {
	    set avl_clk_frequency [get_parameter_value PLL_NIOS_CLK_FREQ]
	}
	set avl_clk_period [ expr {1000000.0 / $avl_clk_frequency} ]
	set avl_clk_ps [ expr {round($avl_clk_period)} ]
	lappend param_list "AVL_CLK_PS=$avl_clk_ps"

	set afi_clk_frequency [get_parameter_value PLL_AFI_CLK_FREQ]
	set afi_clk_period [ expr {1000000.0 / $afi_clk_frequency} ]
	set afi_clk_ps [ expr {round($afi_clk_period)} ]
	lappend param_list "AFI_CLK_PS=$afi_clk_ps"

	lappend param_list "TREFI=[get_parameter_value TREFI]"
	lappend param_list "TRFC=[get_parameter_value TRFC]"
	lappend param_list "REFRESH_INTERVAL=[get_parameter_value REFRESH_INTERVAL]"

	if {[string compare -nocase [get_parameter_value USER_DEBUG_LEVEL] "4"] == 0 ||
            [::alt_mem_if::util::qini::cfg_is_on alt_mem_if_en_inst_rom_write] == 1 ||
	...
	foreach file_name $return_files {
		if {[regexp -nocase {^[ ]*\.do[ ]*$} [file extension $file_name] match] == 0 &&
			...
			if {([string compare -nocase [get_parameter_value USER_DEBUG_LEVEL] "0"] != 0 && [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] != 0) &&
	...
	if {[string compare -nocase [get_parameter_value MAX10_RTL_SEQ] "true"] == 0} {
	    set return_files_sw [generate_qsys_sequencer_sw $prepend_str $protocol $qsysdir $fileset $inhdl_dir $rdimm $lrdimm $seq_mem_size $mem_size_aligned \
			     $nios_hex_file_name $ac_rom_init_file_name $inst_rom_init_file_name 1]
	} else {
	    set return_files_sw [generate_qsys_sequencer_sw $prepend_str $protocol $qsysdir $fileset $inhdl_dir $rdimm $lrdimm $seq_mem_size $mem_size_aligned \
			     $nios_hex_file_name $ac_rom_init_file_name $inst_rom_init_file_name]
	}
	
	if {[::alt_mem_if::util::hwtcl_utils::param_compare PARSE_FRIENDLY_DEVICE_FAMILY "MAX10"]} {
	    lappend return_files_sw [hex_to_rtl $qsysdir $ac_rom_init_file_name]
	    lappend return_files_sw [hex_to_rtl $qsysdir $inst_rom_init_file_name]
	}
...
proc ::alt_mem_if::gen::uniphy_gen::hex_to_rtl {tmpdir hex_file} {
    
    set hex_file_name [file join $tmpdir $hex_file]
    set fp [open "$hex_file_name" r]
    set file_data [read $fp]
    close $fp
    
    if {[string match {*inst*} $hex_file]} {
        set base_name "rw_manager_m10_inst_ROM"
    } else {
        set base_name "rw_manager_m10_ac_ROM"
    }
	set base_file_name "${base_name}.v"
	set output_file_name [file join $tmpdir $base_file_name]
	set FH [open "$output_file_name" w]
	
	puts $FH "`timescale 1 ps / 1 ps"
    puts $FH "module $base_name ("
    puts $FH "	clock,"
    puts $FH "	rdaddress,"
    puts $FH "	q);"
    puts $FH "	"
    puts $FH "	input	  clock;"
    if {[string match {*inst*} $hex_file]} {
    puts $FH "	input	\[6:0\]  rdaddress;"
    puts $FH "	output	\[19:0\]  q;"
    puts $FH "	"
    puts $FH "	reg	\[19:0\]  q;"
    puts $FH "	"
    puts $FH "	wire	\[6:0\]  rdaddress_r;"
    puts $FH "  assign  rdaddress_r =   rdaddress;"
    } else {
    puts $FH "	input	\[5:0\]  rdaddress;"
    puts $FH "	output	\[31:0\]  q;"
    puts $FH "	"
    puts $FH "	reg	\[31:0\]  q;"
    puts $FH "	"
    puts $FH "	reg	\[5:0\]  rdaddress_r;"
    puts $FH "always @ (posedge clock)"
    puts $FH "    rdaddress_r <=  rdaddress;"
    }
    puts $FH "	"
    puts $FH "always @ (posedge clock)"
    puts $FH "    case(rdaddress_r)"
	
    set data [split $file_data "\n"]
    foreach line $data {
         set address [string range $line 5 6]
         set data [string range $line 9 [expr [string length $line]-3]]
         if {[string length $data] != 0} {
             puts $FH "'h$address : q <= 'h$data;"
         }
    }
    puts $FH "        default : q <= 0;"
    puts $FH "    endcase"
...
proc ::alt_mem_if::gen::uniphy_gen::generate_qsys_sequencer_sw {prepend_str protocol tmpdir fileset inhdl_dir rdimm lrdimm requested_mem_size mem_size_aligned
	...
	set qwrapdir [alt_mem_if::util::iptclgen::get_quartus_wrapper_dir]
	set curdir [pwd]
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
		if {[string compare -nocase $protocol "DDR3"] == 0} {
			array set param_array [list MR1_RTT_RANK0 [get_parameter_value MR1_RTT_RANK0]]
			array set param_array [list MR1_RTT_RANK1 [get_parameter_value MR1_RTT_RANK1]]
			array set param_array [list MR1_RTT_RANK2 [get_parameter_value MR1_RTT_RANK2]]
			array set param_array [list MR1_RTT_RANK3 [get_parameter_value MR1_RTT_RANK3]]

			array set param_array [list MR1_RTT_DRV_RANK0 [get_parameter_value MR1_RTT_DRV_RANK0]]
			array set param_array [list MR1_RTT_DRV_RANK1 [get_parameter_value MR1_RTT_DRV_RANK1]]
			array set param_array [list MR1_RTT_DRV_RANK2 [get_parameter_value MR1_RTT_DRV_RANK2]]
			array set param_array [list MR1_RTT_DRV_RANK3 [get_parameter_value MR1_RTT_DRV_RANK3]]

			array set param_array [list MR2_RTT_WR_RANK0 [get_parameter_value MR2_RTT_WR_RANK0]]
			array set param_array [list MR2_RTT_WR_RANK1 [get_parameter_value MR2_RTT_WR_RANK1]]
			array set param_array [list MR2_RTT_WR_RANK2 [get_parameter_value MR2_RTT_WR_RANK2]]
			array set param_array [list MR2_RTT_WR_RANK3 [get_parameter_value MR2_RTT_WR_RANK3]]

			array set param_array [list ADVANCED_ODT_CONTROL 1]

			array set param_array [list DDR3_MR1_ODT_MASK [expr {0xFFFFFD99}]]
			array set param_array [list DDR3_MR2_ODT_MASK [expr {0xFFFFF9FF}]]

			array set param_array [list CFG_WRITE_ODT_CHIP [get_parameter_value WR_ODT_PATTERN]]
			array set param_array [list CFG_READ_ODT_CHIP  [get_parameter_value RD_ODT_PATTERN]]
		}
	}   
	...
	set NON_DES_CAL 0
	if {[string compare -nocase [get_parameter_value ENABLE_NON_DES_CAL] "TRUE"] == 0} {
		set NON_DES_CAL 1
	}

	array set param_array [list NON_DES_CAL $NON_DES_CAL]
	lappend mc_supported_param_list NON_DES_CAL
	...
	if {[string compare -nocase $protocol "DDR3"] == 0} {
		...
		array set param_array [list AP_MODE $AP_MODE]
		lappend mc_supported_param_list AP_MODE
	}
   ...
   if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_set_fixed_read_latency]} {
      array set param_array [list SET_FIX_READ_LATENCY_ENABLE 1]
   } else {
      array set param_array [list SET_FIX_READ_LATENCY_ENABLE 0]
   }
   lappend simple_param_list FIX_READ_LATENCY
   
	
	set device_family [::alt_mem_if::gui::system_info::get_device_family]
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_no_skip_ptap_0]} { 
		set skip_ptap_0_dqs_en_cal 0
	} elseif {[string compare -nocase $device_family "ARRIAV"] == 0 || [string compare -nocase $device_family "CYCLONEV"] == 0} {
		set skip_ptap_0_dqs_en_cal 1
	} else {
		set skip_ptap_0_dqs_en_cal 0
	}
	
	array set param_array [list SKIP_PTAP_0_DQS_EN_CAL $skip_ptap_0_dqs_en_cal]
	...
	if {$DDR3 && ($rdimm || $lrdimm)} {
		...
		if {$lrdimm} {
			...
			set lrdimm_rank_multiplication_factor [get_parameter_value MEM_IF_LRDIMM_RM]
			_dprint 1 "LRDIMM Calculated Rank Multiplication Factor = ${lrdimm_rank_multiplication_factor}"
			array set param_array [ list\
	...
	if {[string compare -nocase $fileset "QUARTUS_SYNTH"] == 0} {
		...
		if {[::alt_mem_if::util::hwtcl_utils::param_compare CALIBRATION_MODE "FULL"]} {
			...
			if {([::alt_mem_if::util::hwtcl_utils::param_is_on ENABLE_EMIT_JTAG_MASTER] && !($bfm_mode || $hps_mode)) ||
             ...
             [::alt_mem_if::util::hwtcl_utils::param_is_on ENABLE_EXPORT_SEQ_DEBUG_BRIDGE]} {
				set tcl_debug 1
			}
	...
	if {[::alt_mem_if::util::hwtcl_utils::param_compare ENABLE_DELAY_CHAIN_WRITE "TRUE"]} { 
		array set param_array [list ENABLE_DELAY_CHAIN_WRITE 1]
	} else {
		array set param_array [list ENABLE_DELAY_CHAIN_WRITE 0]
	}
   ...
   if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_fake_cal_fail] == 1} {
      array set param_array [list FAKE_CAL_FAIL 1]
   } else {
      array set param_array [list FAKE_CAL_FAIL 0]
   }
	...
	lappend boolean_param_list ENABLE_NON_DES_CAL
	lappend boolean_param_list ENABLE_NON_DES_CAL_TEST

	if {[string compare -nocase $protocol "DDR3"] == 0} {
		lappend boolean_param_list AP_MODE
	}

	lappend boolean_param_list USE_USER_RDIMM_VALUE
	lappend boolean_param_list MRS_MIRROR_PING_PONG_ATSO
	...
	if {$DDRX} {
		set tinit_cntr0 [expr {int(ceil(0.5*((0.5 * [get_parameter_value PLL_AFI_CLK_FREQ])-1))-1)}]
		set tinit_cntr1 [expr {int(ceil(sqrt([get_parameter_value MEM_TINIT_US] * 2)))}]
		set tinit_cntr2 $tinit_cntr1

		array set param_array [list TINIT_CNTR0_VAL $tinit_cntr0]
		array set param_array [list TINIT_CNTR1_VAL $tinit_cntr1]
		array set param_array [list TINIT_CNTR2_VAL $tinit_cntr2]

		set treset_cntr0  $tinit_cntr0
		set treset_cntr1  99
		set treset_cntr2  10

		array set param_array [list TRESET_CNTR0_VAL $treset_cntr0]
		array set param_array [list TRESET_CNTR1_VAL $treset_cntr1]
		array set param_array [list TRESET_CNTR2_VAL $treset_cntr2]
	}
   ...
   puts $fh "/*"
   puts $fh "Copyright (C) 2016 Intel Corporation"
   puts $fh "All rights reserved."
   puts $fh ""
   puts $fh "SPDX-License-Identifier:    BSD-3-Clause"
   puts $fh ""
   puts $fh "Redistribution and use in source and binary forms, with or without"
   puts $fh "modification, are permitted provided that the following conditions are met:"
   puts $fh "    * Redistributions of source code must retain the above copyright"
   puts $fh "      notice, this list of conditions and the following disclaimer."
   puts $fh "    * Redistributions in binary form must reproduce the above copyright"
   puts $fh "      notice, this list of conditions and the following disclaimer in the"
   puts $fh "      documentation and/or other materials provided with the distribution."
   puts $fh "    * Neither the name of Altera Corporation nor the"
   puts $fh "      names of its contributors may be used to endorse or promote products"
   puts $fh "      derived from this software without specific prior written permission."
   puts $fh ""
   puts $fh "THIS SOFTWARE IS PROVIDED BY THE COPYRIGHT HOLDERS AND CONTRIBUTORS \"AS IS\" AND"
   puts $fh "ANY EXPRESS OR IMPLIED WARRANTIES, INCLUDING, BUT NOT LIMITED TO, THE IMPLIED"
   puts $fh "WARRANTIES OF MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE ARE"
   puts $fh "DISCLAIMED. IN NO EVENT SHALL ALTERA CORPORATION BE LIABLE FOR ANY"
   puts $fh "DIRECT, INDIRECT, INCIDENTAL, SPECIAL, EXEMPLARY, OR CONSEQUENTIAL DAMAGES"
   puts $fh "(INCLUDING, BUT NOT LIMITED TO, PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES;"
   puts $fh "LOSS OF USE, DATA, OR PROFITS; OR BUSINESS INTERRUPTION) HOWEVER CAUSED AND"
   puts $fh "ON ANY THEORY OF LIABILITY, WHETHER IN CONTRACT, STRICT LIABILITY, OR TORT"
   puts $fh "(INCLUDING NEGLIGENCE OR OTHERWISE) ARISING IN ANY WAY OUT OF THE USE OF THIS"
   puts $fh "SOFTWARE, EVEN IF ADVISED OF THE POSSIBILITY OF SUCH DAMAGE."
   puts $fh "*/"
	...
	puts $fh "\t$qwrapdir/uniphy_mcc -ac_code sequencer_mc/ac_rom.s -inst_code sequencer_mc/inst_rom.s -ac_rom ../$ac_rom_init_file_name -inst_rom ../$inst_rom_init_file_name -header sequencer/sequencer_auto.h -vheader ../sequencer_auto_h.sv -ac_rom_init sequencer/sequencer_auto_ac_init.c -inst_rom_init sequencer/sequencer_auto_inst_init.c \$(MC_MACROS)"
	puts $fh "endif"
	puts $fh ""
	puts $fh ".PHONY : gui"
	puts $fh "gui : setup"
	...
	if { !$do_only_rw_mgr_mc && [string compare -nocase [get_parameter_value ENABLE_EXPORT_SEQ_DEBUG_BRIDGE] "true"] == 0 && $tcl_debug} {
		...
		if {[file exists $objdump_file]} {
			...
			puts $fh "#define INDIRECT_SEQ_CORE_DEBUG_BASE 0x70004"
			foreach key [lsort -dictionary [array names param_array]] {
				puts $fh "#define $key $param_array($key)"
			}
			puts $fh ""
...
proc ::alt_mem_if::gen::uniphy_gen::instantiate_abstract_ram { CONTROLLER exported_avl_if afi_clk_source afi_reset_source {rename_interfaces 1}} {
	...
	set_instance_parameter $ABSTRACT_RAM USE_MEM_VERBOSE [get_parameter_value MEM_VERBOSE]
...
proc ::alt_mem_if::gen::uniphy_gen::instantiate_efficiency_monitor {protocol CONTROLLER exported_avl_if controller_avl_if afi_clk_source afi_reset_source {rename_interfaces 1}} {
   ...
   if {[string compare -nocase $protocol "RLDRAMII"] == 0} {
	...
	set_instance_parameter $EFF_MON EMPC_MAX_READ_TRANSACTIONS [get_instance_interface_property ${CONTROLLER} $controller_avl_if maximumPendingReadTransactions]
	...
	set_instance_parameter $BRIDGE USE_BYTEENABLE $byteen
	...
	set_instance_parameter $BRIDGE MAX_PENDING_READ_TRANSACTIONS [get_instance_interface_property ${CONTROLLER} $controller_avl_if maximumPendingReadTransactions]
	...
	add_connection "${EFF_MON}.avl_out/${CONTROLLER}.${controller_avl_if}"
...
proc ::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller {protocol CONTROLLER afi_clk_source afi_reset_source {rename_interfaces 1}} {
	...
	if {[string compare -nocase $protocol "RLDRAMII"] == 0 && [string compare -nocase [get_parameter_value ENABLE_ABSTRACT_RAM] "true"] == 0} {
		...
		if {[string compare -nocase [get_parameter_value ADD_EFFICIENCY_MONITOR] "true"] == 0} {
			...
			::alt_mem_if::gen::uniphy_gen::instantiate_efficiency_monitor $protocol $CONTROLLER avl avl $afi_clk_source $afi_reset_source $rename_interfaces
			...
			if {$use_hard_emif} {
				...
				if {[string compare -nocase [get_parameter_value ENABLE_CTRL_AVALON_INTERFACE] "true"] == 0} {
					if { [string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0 } {
						...
						if {[string compare -nocase [get_parameter_value ADD_EFFICIENCY_MONITOR] "true"] == 0} {
							::alt_mem_if::gen::uniphy_gen::instantiate_efficiency_monitor $protocol $CONTROLLER "avl_${CONTROLLER}" avl $afi_clk_source $afi_reset_source $rename_interfaces
						}
					} else { 
						add_interface avl avalon end
						if {[string compare -nocase [get_parameter_value ADD_EFFICIENCY_MONITOR] "true"] == 0} {

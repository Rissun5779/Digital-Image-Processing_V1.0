...
proc ::alt_mem_if::gen::uniphy_interfaces::pll_sharing {direction {suffix {}}} {
	...
	if {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0 && [string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "max10"] != 0} {
...
proc ::alt_mem_if::gen::uniphy_interfaces::tracking_strobe {component} {
	...
	if {[string compare -nocase $component "phy"] == 0} {
		...
		_error "Invalid component specification to determine tracking strobe interface"
	}
	...
	add_interface_port tracking_strobe capture_strobe_tracking capture_strobe_tracking $output_phy [get_parameter_value MEM_IF_READ_DQS_WIDTH]
...
proc ::alt_mem_if::gen::uniphy_interfaces::afi {protocol component {name {afi}} {complete_interface {1}} {add_status {1}} {full_controller_if 0} {pingpongphy_if_flag {0}} } {
	...
	if {[string compare -nocase $protocol "DDR2"] == 0 || [string compare -nocase $protocol "DDR3"] == 0 || [string compare -nocase $protocol "LPDDR1"] == 0} {
		...
		if {[string compare -nocase $protocol "DDR2"] == 0 &&
			...
			[string compare -nocase $name "afi"] == 0 && $full_controller_if} {
			add_interface_port ${name} ${name}_seq_busy afi_seq_busy $output_phy [get_parameter_value MEM_IF_NUMBER_OF_RANKS]
			set_port_property ${name}_seq_busy VHDL_TYPE STD_LOGIC_VECTOR
			if {[string compare -nocase [get_parameter_value USE_DQS_TRACKING] "true"] == 0} {
				set_port_property ${name}_seq_busy termination false
...
proc ::alt_mem_if::gen::uniphy_interfaces::max10_csr_connection {controller} {
	
    if {[string compare -nocase [get_parameter_value CTL_CSR_ENABLED] "true"] == 0} {
	    add_interface csr avalon end
	    set_interface_property csr export_of ${controller}.csr
...
proc ::alt_mem_if::gen::uniphy_interfaces::phase_detector_interface {component} {
    
    if {[string compare -nocase $component "phy"] == 0} {
	    set input_pd_phy "Input"
	    set output_pd_phy "Output"
    } elseif {[string compare -nocase $component "sequencer"] == 0} {
	    set input_pd_phy "Output"
	    set output_pd_phy "Input"
    } else {
	    _error "Invalid component specification to determine interface"
    }

    add_interface phase_detector conduit end
    set_interface_property phase_detector ENABLED true
    add_interface_port phase_detector pd_reset_n pd_reset_n $input_pd_phy 1
    add_interface_port phase_detector pd_ack pd_ack $input_pd_phy 1
    add_interface_port phase_detector pd_up pd_up $output_pd_phy 1
    add_interface_port phase_detector pd_down pd_down $output_pd_phy 1
}

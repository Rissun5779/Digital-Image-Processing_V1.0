...
proc ::alt_mem_if::gui::common_ddr_mem_model::_family_supports_non_leveling {} {
	if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 ||
	    ...
	    [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
		return 0
	} else {
		return 1
	}
...
proc ::alt_mem_if::gui::common_ddr_mem_model::validate_component {} {
	...
	if {[regexp {^DDR2$} $protocol] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			set_parameter_property MEM_RTT_DRV_RANK0 VISIBLE true
			set_parameter_property MEM_RTT_DRV_RANK0 ALLOWED_RANGES {"RZQ/6" "RZQ/7"}

			set_parameter_property MEM_RTT_DRV_RANK1 VISIBLE true
			set_parameter_property MEM_RTT_DRV_RANK1 ALLOWED_RANGES {"RZQ/6" "RZQ/7"}

			set_parameter_property MEM_RTT_DRV_RANK2 VISIBLE true
			set_parameter_property MEM_RTT_DRV_RANK2 ALLOWED_RANGES {"RZQ/6" "RZQ/7"}

			set_parameter_property MEM_RTT_DRV_RANK3 VISIBLE true
			set_parameter_property MEM_RTT_DRV_RANK3 ALLOWED_RANGES {"RZQ/6" "RZQ/7"}
		} else {
			set_parameter_property MEM_DRV_STR VISIBLE true
	...
	if {[regexp {^DDR2$} $protocol] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			set_parameter_property MEM_RTT_NOM_RANK0 VISIBLE true
			set_parameter_property MEM_RTT_NOM_RANK0 ALLOWED_RANGES {"ODT Disabled" "RZQ/4" "RZQ/2" "RZQ/6"}
			set_parameter_property MEM_RTT_NOM_RANK1 VISIBLE true
			set_parameter_property MEM_RTT_NOM_RANK1 ALLOWED_RANGES {"ODT Disabled" "RZQ/4" "RZQ/2" "RZQ/6"}
			set_parameter_property MEM_RTT_NOM_RANK2 VISIBLE true
			set_parameter_property MEM_RTT_NOM_RANK2 ALLOWED_RANGES {"ODT Disabled" "RZQ/4" "RZQ/2" "RZQ/6"}
			set_parameter_property MEM_RTT_NOM_RANK3 VISIBLE true
			set_parameter_property MEM_RTT_NOM_RANK3 ALLOWED_RANGES {"ODT Disabled" "RZQ/4" "RZQ/2" "RZQ/6"}
		} else {
			set_parameter_property MEM_RTT_NOM VISIBLE true
	...
	if {[regexp {^DDR3$} $protocol] == 1} {
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			set_parameter_property MEM_RTT_WR_RANK0 VISIBLE true
			set_parameter_property MEM_RTT_WR_RANK0 ALLOWED_RANGES {"Dynamic ODT off" "RZQ/4" "RZQ/2"}

			set_parameter_property MEM_RTT_WR_RANK1 VISIBLE true
			set_parameter_property MEM_RTT_WR_RANK1 ALLOWED_RANGES {"Dynamic ODT off" "RZQ/4" "RZQ/2"}

			set_parameter_property MEM_RTT_WR_RANK2 VISIBLE true
			set_parameter_property MEM_RTT_WR_RANK2 ALLOWED_RANGES {"Dynamic ODT off" "RZQ/4" "RZQ/2"}

			set_parameter_property MEM_RTT_WR_RANK3 VISIBLE true
			set_parameter_property MEM_RTT_WR_RANK3 ALLOWED_RANGES {"Dynamic ODT off" "RZQ/4" "RZQ/2"}
		} else {
	...
	if {[regexp {^DDR2$|^DDR3$|^LPDDR2$} $protocol] == 1} {
	...
	if {[regexp {^DDR3$} $protocol] == 1} {
		...
		} elseif {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
			set_parameter_property MEM_FORMAT ALLOWED_RANGES {"DISCRETE:Discrete Device"}
			set_parameter_property MEM_FORMAT ENABLED false
		} else {
			set_parameter_property MEM_FORMAT ALLOWED_RANGES {"DISCRETE:Discrete Device" "UNBUFFERED:Unbuffered DIMM" "REGISTERED:Registered DIMM" "LOADREDUCED:Load-Reduced DIMM"}
	...
	if {[regexp {^LPDDR2$} $protocol] == 1 || [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
	...
	if {[string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0} {
		_iprint "For more information regarding the configuration of LRDIMMs, refer to KDB Article rd11122013_19 or select an LRDIMM preset from the library."
		if {[get_parameter_value MEM_IF_NUMBER_OF_RANKS] != [expr {2*[get_parameter_value MEM_NUMBER_OF_DIMMS]}]} {
			_eprint "Total number of ranks ([get_parameter_value MEM_IF_NUMBER_OF_RANKS]) out of range. Only two logical ranks per LRDIMM are supported; please update the Number of Ranks per Slot parameter to 2."
			set validation_pass 0
		}
		set rmfactor [get_parameter_value MEM_IF_LRDIMM_RM]
		switch -exact $rmfactor {
			1 { set log2rm 0 }
			2 { set log2rm 1 }
			4 { set log2rm 2 }
			default { set validation_pass 0}
		}
		
		if { $validation_pass != 0 } {
			set num_dimms         [get_parameter_value MEM_NUMBER_OF_DIMMS]
			set num_cs_per_dimm   [get_parameter_value MEM_CS_WIDTH]
			set num_expected_cs   [expr {(2 + $log2rm) * $num_dimms}]
			set num_expected_addr [expr {(16 + $log2rm)}]

			set num_actual_cs     [expr {$num_dimms * $num_cs_per_dimm}]
			set num_actual_addr   [get_parameter_value MEM_ROW_ADDR_WIDTH]
			
			_dprint 1 "LRDIMM: RM Factor = $rmfactor (log2 = $log2rm)"

			if {($num_expected_cs != $num_actual_cs)} {
				_eprint "Invalid LRDIMM configuration - Please modify the number of chip selects per DIMM or select an LRDIMM preset."
				_dprint 1 "LRDIMM: num_expected_cs = $num_expected_cs, num_actual_cs = $num_actual_cs"
				set validation_pass 0
			} elseif {($num_expected_addr != $num_actual_addr)} {
				_eprint "Invalid LRDIMM configuration - Please modify the row address width or select an LRDIMM preset."
				_dprint 1 "LRDIMM: num_expected_addr = $num_expected_addr, num_actual_cs = $num_actual_addr"
				set validation_pass 0
			} else {
				set num_physranks [expr {2 * $rmfactor}]
				_iprint "LRDIMM Configuration Info: "
				_iprint "- Calculated Rank Multiplication Factor: $rmfactor"
				_iprint "- Number of physical ranks per DIMM: $num_physranks"
				_iprint "- Number of physical chip select signals per DIMM: $num_cs_per_dimm"
				_iprint "- Number of physical address signal per DIMM: 16 (fixed)"

				if {$num_dimms == 1} {
					if {$rmfactor == 2} {
						_iprint "- Controller A\[16\] output is mapped to DIMM CS\[2\] input"
					} elseif {$rmfactor == 4} {
						_iprint "- Controller A\[17:16\] outputs are mapped to DIMM CS\[3:2\] inputs"
					} else {
						set validation_pass 0
					}
				} elseif {$num_dimms == 2} {
					if {$rmfactor == 2} {
						_iprint "- Controller A\[16\] output is mapped to FPGA CS\[2\] and CS\[5\] outputs (CS\[2\] input of both DIMMs)"
					} elseif {$rmfactor == 4} {
						_iprint "- Controller A\[17:16\] outputs are mapped to FPGA CS\[3:2\] and CS\[7:6\] outputs (CS\[3:2\] inputs of both DIMMs)"
					} else {
						set validation_pass 0
					}
				} else {
	...
	if {[regexp {^DDR3$} $protocol] == 1} {
		if {[string compare -nocase [get_parameter_value MEM_FORMAT] "DISCRETE"] == 0 && 
			...
			[string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "cyclonev"] != 0 &&
			[string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "max10"] != 0} {
	...
	if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
	    set_parameter_property MEM_DQ_WIDTH ALLOWED_RANGES {8:24}
	}
	...
	if {([regexp {^DDR3$} $protocol] == 1 && [string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0)} {
		set_parameter_property MEM_ROW_ADDR_WIDTH ALLOWED_RANGES {16:18}
		set_parameter_property MEM_NUMBER_OF_RANKS_PER_DIMM ALLOWED_RANGES {2}
		if {([get_parameter_value MEM_ROW_ADDR_WIDTH] < 16) || ([get_parameter_value MEM_ROW_ADDR_WIDTH] > 18)} {
			_eprint "LRDIMMs require either 16, 17 or 18 row adress bits generated by the controller."
			set validation_pass 0
		}
		if {[get_parameter_value MEM_NUMBER_OF_RANKS_PER_DIMM] != 2} {
			_eprint "UniPHY only supports two logical ranks per LRDIMM DIMM slot."
			set validation_pass 0
		}
	} else {
		set_parameter_property MEM_ROW_ADDR_WIDTH ALLOWED_RANGES {12:16}
	...
	if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
	    if {[get_parameter_value MEM_CK_WIDTH] > 1} {
	        _eprint "mem_ck width is limited to 1 on Max 10"
	        set validation_pass 0
	    }
	}

	if {[regexp {^DDR3$} $protocol] == 1} {
	    if {[get_parameter_value MR2_ASR] == 1 && [get_parameter_value MR2_SRT] == 1} {
	        _eprint "Mode Register 2: automatic selfrefresh method with extended selfrefresh temperature is an illegal selfrefresh operation for DDR3."
	        set validation_pass 0
	    }
	}

	if {    ([regexp {^DDR3$} $protocol] == 1) \
        && ([string compare -nocase [get_parameter_value HARD_EMIF] "true"] == 0) \
        && (    ([string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "arriav"] == 0) \
      		 ||  [string compare -nocase [::alt_mem_if::gui::system_info::get_device_family] "cyclonev"] == 0) \
        && ([get_parameter_value MEM_ROW_ADDR_WIDTH] == 16) \
        && ([get_parameter_value MEM_BANKADDR_WIDTH] == 3)} {

      if {[get_parameter_value MEM_COL_ADDR_WIDTH] > 10} {
         _eprint "Memory column address width must be less than or equal to 10 for the Arria V / Cyclone V Hard Memory Controller configuration."
         set validation_pass 0
      }
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_derived_mode_register_parameters {} {
	...
	if {[regexp {^DDR2$|^DDR3$|^HPS$} $ddr_mode] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_DRV_RANK0 Integer 1
			set_parameter_property MR1_RTT_DRV_RANK0 VISIBLE FALSE
			set_parameter_property MR1_RTT_DRV_RANK0 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_DRV_RANK1 Integer 1
			set_parameter_property MR1_RTT_DRV_RANK1 VISIBLE FALSE
			set_parameter_property MR1_RTT_DRV_RANK1 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_DRV_RANK2 Integer 1
			set_parameter_property MR1_RTT_DRV_RANK2 VISIBLE FALSE
			set_parameter_property MR1_RTT_DRV_RANK2 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_DRV_RANK3 Integer 1
			set_parameter_property MR1_RTT_DRV_RANK3 VISIBLE FALSE
			set_parameter_property MR1_RTT_DRV_RANK3 DERIVED TRUE
		}
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_RANK0 Integer 1
			set_parameter_property MR1_RTT_RANK0 VISIBLE FALSE
			set_parameter_property MR1_RTT_RANK0 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_RANK1 Integer 1
			set_parameter_property MR1_RTT_RANK1 VISIBLE FALSE
			set_parameter_property MR1_RTT_RANK1 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_RANK2 Integer 1
			set_parameter_property MR1_RTT_RANK2 VISIBLE FALSE
			set_parameter_property MR1_RTT_RANK2 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR1_RTT_RANK3 Integer 1
			set_parameter_property MR1_RTT_RANK3 VISIBLE FALSE
			set_parameter_property MR1_RTT_RANK3 DERIVED TRUE
		}
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MR2_RTT_WR_RANK0 Integer 1
			set_parameter_property MR2_RTT_WR_RANK0 VISIBLE FALSE
			set_parameter_property MR2_RTT_WR_RANK0 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR2_RTT_WR_RANK1 Integer 1
			set_parameter_property MR2_RTT_WR_RANK1 VISIBLE FALSE
			set_parameter_property MR2_RTT_WR_RANK1 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR2_RTT_WR_RANK2 Integer 1
			set_parameter_property MR2_RTT_WR_RANK2 VISIBLE FALSE
			set_parameter_property MR2_RTT_WR_RANK2 DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter MR2_RTT_WR_RANK3 Integer 1
			set_parameter_property MR2_RTT_WR_RANK3 VISIBLE FALSE
			set_parameter_property MR2_RTT_WR_RANK3 DERIVED TRUE
		}
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter RD_ODT_PATTERN Integer 0
			set_parameter_property RD_ODT_PATTERN VISIBLE FALSE
			set_parameter_property RD_ODT_PATTERN DERIVED TRUE

			::alt_mem_if::util::hwtcl_utils::_add_parameter WR_ODT_PATTERN Integer 0			
			set_parameter_property WR_ODT_PATTERN VISIBLE FALSE
			set_parameter_property WR_ODT_PATTERN DERIVED TRUE
		}
	}
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_derived_parameters {} {
	...
	::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_IF_LRDIMM_RM Integer 0
	set_parameter_property MEM_IF_LRDIMM_RM VISIBLE false
	set_parameter_property MEM_IF_LRDIMM_RM DERIVED true
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_memory_initialization_parameters {} {
	...
	if {[regexp {^DDR2$} $ddr_mode] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_DRV_RANK0 String "RZQ/6"
			set_parameter_property MEM_RTT_DRV_RANK0 Display_Name "Output drive strength setting for Rank 0"
			set_parameter_property MEM_RTT_DRV_RANK0 DESCRIPTION "Specify the output driver impedance setting at the memory device for Rank 0"

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_DRV_RANK1 String "RZQ/6"
			set_parameter_property MEM_RTT_DRV_RANK1 Display_Name "Output drive strength setting for Rank 1"
			set_parameter_property MEM_RTT_DRV_RANK1 DESCRIPTION "Specify the output driver impedance setting at the memory device for Rank 1"

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_DRV_RANK2 String "RZQ/6"
			set_parameter_property MEM_RTT_DRV_RANK2 Display_Name "Output drive strength setting for Rank 2"
			set_parameter_property MEM_RTT_DRV_RANK2 DESCRIPTION "Specify the output driver impedance setting at the memory device for Rank 2"

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_DRV_RANK3 String "RZQ/6"
			set_parameter_property MEM_RTT_DRV_RANK3 Display_Name "Output drive strength setting for Rank 3"
			set_parameter_property MEM_RTT_DRV_RANK3 DESCRIPTION "Specify the output driver impedance setting at the memory device for Rank 3"
		} else {
	...
	if {[regexp {^DDR2$} $ddr_mode] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_NOM_RANK0 String "ODT Disabled"
			set_parameter_property MEM_RTT_NOM_RANK0 Display_Name "ODT Rtt nominal value for Rank 0"
			set_parameter_property MEM_RTT_NOM_RANK0 DESCRIPTION "Determines the ODT resistance at the memory device for Rank 0."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_NOM_RANK1 String "ODT Disabled"
			set_parameter_property MEM_RTT_NOM_RANK1 Display_Name "ODT Rtt nominal value for Rank 1"
			set_parameter_property MEM_RTT_NOM_RANK1 DESCRIPTION "Determines the ODT resistance at the memory device for Rank 1."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_NOM_RANK2 String "ODT Disabled"
			set_parameter_property MEM_RTT_NOM_RANK2 Display_Name "ODT Rtt nominal value for Rank 2"
			set_parameter_property MEM_RTT_NOM_RANK2 DESCRIPTION "Determines the ODT resistance at the memory device for Rank 2."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_NOM_RANK3 String "ODT Disabled"
			set_parameter_property MEM_RTT_NOM_RANK3 Display_Name "ODT Rtt nominal value for Rank 3"
			set_parameter_property MEM_RTT_NOM_RANK3 DESCRIPTION "Determines the ODT resistance at the memory device for Rank 3."
		} else {
	...
	if {[regexp {^DDR3$|^HPS$} $ddr_mode] == 1} {
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_WR_RANK0 String "Dynamic ODT off"
			set_parameter_property MEM_RTT_WR_RANK0 Display_Name "Dynamic ODT (Rtt_WR) value for Rank 0"
			set_parameter_property MEM_RTT_WR_RANK0 DESCRIPTION "Specify the mode of the dynamic ODT feature of the memory device for Rank 0."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_WR_RANK1 String "Dynamic ODT off"
			set_parameter_property MEM_RTT_WR_RANK1 Display_Name "Dynamic ODT (Rtt_WR) value for Rank 1"
			set_parameter_property MEM_RTT_WR_RANK1 DESCRIPTION "Specify the mode of the dynamic ODT feature of the memory device for Rank 1."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_WR_RANK2 String "Dynamic ODT off"
			set_parameter_property MEM_RTT_WR_RANK2 Display_Name "Dynamic ODT (Rtt_WR) value for Rank 2"
			set_parameter_property MEM_RTT_WR_RANK2 DESCRIPTION "Specify the mode of the dynamic ODT feature of the memory device for Rank 2."

			::alt_mem_if::util::hwtcl_utils::_add_parameter MEM_RTT_WR_RANK3 String "Dynamic ODT off"
			set_parameter_property MEM_RTT_WR_RANK3 Display_Name "Dynamic ODT (Rtt_WR) value for Rank 3"
			set_parameter_property MEM_RTT_WR_RANK3 DESCRIPTION "Specify the mode of the dynamic ODT feature of the memory device for Rank 3."
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
		if {[regexp {^DDR3$} $ddr_mode] == 1} {
			for { set dir 0 } { $dir < 2 } { incr dir } {
				for { set rank 0 } { $rank < 4 } { incr rank } {
					if {$dir == 0} {
						set dir_text "RD"
					} else {
						set dir_text "WR"
					}
					for { set pin 0 } { $pin < 4 } { incr pin } {
						set param_name "MEM_ADV_ODT_${dir_text}_R${rank}_O${pin}"
						_dprint 1 "parameter name = $param_name"

						::alt_mem_if::util::hwtcl_utils::_add_parameter $param_name boolean false
						set_parameter_property $param_name DISPLAY_NAME "Assert ODT${pin} pin during ${dir_text} commands targeting rank ${rank}"
						set_parameter_property $param_name DESCRIPTION "Assert ODT${pin} pin during ${dir_text} commands targeting rank ${rank}"
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_memory_timing_parameters {} {
	...
	if {[regexp {^LPDDR2$} $ddr_mode] == 1} {
	    set_parameter_property TIMING_TDQSCK DISPLAY_NAME "tDQSCK (max)"
	} else {
	...
	if {[regexp {^HPS$} $ddr_mode] == 1} {
		set_parameter_property MEM_TMRD_CK Description "Load mode register command period (tMRD for DDR2/DDR3; tMRW for LPDDR2)"
	} else {
	...
	if {[regexp {^LPDDR2$} $ddr_mode] == 1} {
		...
		set_parameter_property MEM_TMRD_CK Display_Name "tMRD (tMRW)"
	} else {
	...
	if {[regexp {^LPDDR2$} $ddr_mode] == 1} {
		...
		set_parameter_property MEM_TREFI_US Description "Refresh command interval (tREFI for DDR2/DDR3; tREFIab for LPDDR2)"
	        set_parameter_property MEM_TREFI_US Display_Name "tREFI (tREFIab)"
	} else {
	...
	if {[regexp {^LPDDR2$} $ddr_mode] == 1} {
	        ...
	        set_parameter_property MEM_TRFC_NS Description "Auto-refresh command interval (tRFC for DDR2/DDR3; tRFCab for LPDDR2)"
	        set_parameter_property MEM_TRFC_NS Display_Name "tRFC (tRFCab)"		
	} else {
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_memory_parameters_parameters {} {
	...
	set_parameter_property MEM_RANK_MULTIPLICATION_FACTOR DISPLAY_NAME "(Deprecated) LRDIMM Rank Multiplication Factor"
	set_parameter_property MEM_RANK_MULTIPLICATION_FACTOR DESCRIPTION "(Deprecated) Specifies the multiplication factor for LRDIMM chip select encoding."
	...
	::alt_mem_if::util::hwtcl_utils::_add_parameter DUPLICATE_AC BOOLEAN false
	set_parameter_property DUPLICATE_AC DISPLAY_NAME "Duplicate Address and Command Signals"
	set_parameter_property DUPLICATE_AC DESCRIPTION "Duplicate CK, CK#, Reset#, CAS#, RAS#, WE#, CKE, CS#, ODT, Bank and Address signals."
	set_parameter_property DUPLICATE_AC VISIBLE false
	...
	if {[::alt_mem_if::util::qini::cfg_is_on micron_ap_mode]} {

		::alt_mem_if::util::hwtcl_utils::_add_parameter AP_MODE boolean true

		::alt_mem_if::util::hwtcl_utils::_add_parameter AP_MODE_EN Std_logic_vector 3
		set_parameter_property AP_MODE_EN WIDTH 2

		set_parameter_property AP_MODE_EN VISIBLE true
		set_parameter_property AP_MODE_EN DISPLAY_NAME "Mixed rank Automatta selection"
		set_parameter_property AP_MODE_EN DESCRIPTION "Choose a 2-bit vector to configure which ranks are DDR3 and which are Automatta processors.  The bit position corresponds to the rank and the value sets the type where 0 == DDR3 and 1 == AP mode"
		set_parameter_property AP_MODE_EN ALLOWED_RANGES {"0:Rank 00" "1: Rank 01" "2:Rank 10" "3:Rank 11"}

	} else {
		::alt_mem_if::util::hwtcl_utils::_add_parameter AP_MODE_EN  Std_logic_vector 0
		set_parameter_property AP_MODE_EN WIDTH 2

		::alt_mem_if::util::hwtcl_utils::_add_parameter AP_MODE boolean false

		set_parameter_property AP_MODE_EN VISIBLE false
	}

	set_parameter_property AP_MODE VISIBLE false
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_create_memory_parameters_gui {} {
	...
	add_display_item "Memory Parameters" AP_MODE_EN PARAMETER
	...
	if {[regexp {^DDR2$|^DDR3$|^HPS$} $ddr_mode] == 1} {
		if {[regexp {^DDR2$|^DDR3$} $ddr_mode] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			add_display_item "Memory Initialization Options" MEM_RTT_DRV_RANK0 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_DRV_RANK1 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_DRV_RANK2 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_DRV_RANK3 parameter
		} else {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			add_display_item "Memory Initialization Options" MEM_RTT_NOM_RANK0 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_NOM_RANK1 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_NOM_RANK2 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_NOM_RANK3 parameter
		} else {
	...
	if {[regexp {^DDR3$|^HPS$} $ddr_mode] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			add_display_item "Memory Initialization Options" MEM_RTT_WR_RANK0 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_WR_RANK1 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_WR_RANK2 parameter
			add_display_item "Memory Initialization Options" MEM_RTT_WR_RANK3 parameter
		} else {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
		if {[regexp {^DDR3$} $ddr_mode] == 1} {
			add_display_item "Memory Initialization Options" mi3 TEXT "<html><b>ODT Output Assertion Table</b><br>"
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R0_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R0_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R0_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R0_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R1_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R1_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R1_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R1_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R2_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R2_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R2_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R2_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R3_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R3_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R3_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_WR_R3_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R0_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R0_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R0_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R0_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R1_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R1_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R1_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R1_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R2_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R2_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R2_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R2_O3 parameter

			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R3_O0 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R3_O1 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R3_O2 parameter
			add_display_item "Memory Initialization Options" MEM_ADV_ODT_RD_R3_O3 parameter
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_derive_mode_register_parameters {} {
	...
	if {[regexp {^DDR2$} $protocol] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			switch [string tolower [get_parameter_value MEM_RTT_DRV_RANK0]] {
				"rzq/6" { set_parameter_value MR1_RTT_DRV_RANK0 [expr {0x00000}] }
				"rzq/7" { set_parameter_value MR1_RTT_DRV_RANK0 [expr {0x00002}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_DRV_RANK1]] {
				"rzq/6" { set_parameter_value MR1_RTT_DRV_RANK1 [expr {0x00000}] }
				"rzq/7" { set_parameter_value MR1_RTT_DRV_RANK1 [expr {0x00002}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_DRV_RANK2]] {
				"rzq/6" { set_parameter_value MR1_RTT_DRV_RANK2 [expr {0x00000}] }
				"rzq/7" { set_parameter_value MR1_RTT_DRV_RANK2 [expr {0x00002}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_DRV_RANK3]] {
				"rzq/6" { set_parameter_value MR1_RTT_DRV_RANK3 [expr {0x00000}] }
				"rzq/7" { set_parameter_value MR1_RTT_DRV_RANK3 [expr {0x00002}] }
			}
	...
	if {[regexp {^DDR2$} $protocol] == 1} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			...
			switch [string tolower [get_parameter_value MEM_RTT_NOM_RANK0]] {
				"odt disabled" { set_parameter_value MR1_RTT_RANK0 [expr {0x00000}] }
				"rzq/4"        { set_parameter_value MR1_RTT_RANK0 [expr {0x00004}] }
				"rzq/2"        { set_parameter_value MR1_RTT_RANK0 [expr {0x00040}] }
				"rzq/6"        { set_parameter_value MR1_RTT_RANK0 [expr {0x00044}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_NOM_RANK1]] {
				"odt disabled" { set_parameter_value MR1_RTT_RANK1 [expr {0x00000}] }
				"rzq/4"        { set_parameter_value MR1_RTT_RANK1 [expr {0x00004}] }
				"rzq/2"        { set_parameter_value MR1_RTT_RANK1 [expr {0x00040}] }
				"rzq/6"        { set_parameter_value MR1_RTT_RANK1 [expr {0x00044}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_NOM_RANK2]] {
				"odt disabled" { set_parameter_value MR1_RTT_RANK2 [expr {0x00000}] }
				"rzq/4"        { set_parameter_value MR1_RTT_RANK2 [expr {0x00004}] }
				"rzq/2"        { set_parameter_value MR1_RTT_RANK2 [expr {0x00040}] }
				"rzq/6"        { set_parameter_value MR1_RTT_RANK2 [expr {0x00044}] }
			}
			switch [string tolower [get_parameter_value MEM_RTT_NOM_RANK3]] {
				"odt disabled" { set_parameter_value MR1_RTT_RANK3 [expr {0x00000}] }
				"rzq/4"        { set_parameter_value MR1_RTT_RANK3 [expr {0x00004}] }
				"rzq/2"        { set_parameter_value MR1_RTT_RANK3 [expr {0x00040}] }
				"rzq/6"        { set_parameter_value MR1_RTT_RANK3 [expr {0x00044}] }
			}
		} else {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			switch [string tolower [get_parameter_value MEM_RTT_WR_RANK0]] {
				"dynamic odt off" { set_parameter_value MR2_RTT_WR_RANK0 [expr {0x00000}] }
				"rzq/4"           { set_parameter_value MR2_RTT_WR_RANK0 [expr {0x00200}] }
				"rzq/2"           { set_parameter_value MR2_RTT_WR_RANK0 [expr {0x00400}] }
			}

			switch [string tolower [get_parameter_value MEM_RTT_WR_RANK1]] {
				"dynamic odt off" { set_parameter_value MR2_RTT_WR_RANK1 [expr {0x00000}] }
				"rzq/4"           { set_parameter_value MR2_RTT_WR_RANK1 [expr {0x00200}] }
				"rzq/2"           { set_parameter_value MR2_RTT_WR_RANK1 [expr {0x00400}] }
			}

			switch [string tolower [get_parameter_value MEM_RTT_WR_RANK2]] {
				"dynamic odt off" { set_parameter_value MR2_RTT_WR_RANK2 [expr {0x00000}] }
				"rzq/4"           { set_parameter_value MR2_RTT_WR_RANK2 [expr {0x00200}] }
				"rzq/2"           { set_parameter_value MR2_RTT_WR_RANK2 [expr {0x00400}] }
			}

			switch [string tolower [get_parameter_value MEM_RTT_WR_RANK3]] {
				"dynamic odt off" { set_parameter_value MR2_RTT_WR_RANK3 [expr {0x00000}] }
				"rzq/4"           { set_parameter_value MR2_RTT_WR_RANK3 [expr {0x00200}] }
				"rzq/2"           { set_parameter_value MR2_RTT_WR_RANK3 [expr {0x00400}] }
			}
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
		if {[regexp {^DDR3$} $protocol] == 1} {
			...
			for { set i 0 } { $i < 2 } { incr i } {
				...
				set bit_vec 0
				for { set rank [expr {$key_mem_if_cs_width - 1}] } { $rank >= 0 } { incr rank -1 } {
					for { set pin [expr {[get_parameter_value MEM_IF_ODT_WIDTH] - 1}] } { $pin >= 0 } { incr pin -1 } {
						set param_name "MEM_ADV_ODT_${dir_text}_R${rank}_O${pin}"
						...
						set bit_vec [expr {$param_val | ($bit_vec << 1)}]
					}
				}
				set param_name "${dir_text}_ODT_PATTERN"
				set param_val [format "0x%08x" $bit_vec]
				set_parameter_value $param_name $param_val

				_iprint "Setting ${dir_text}_ODT_PATTERN using advanced ODT (#Ranks:$key_mem_if_cs_width, #ODT:[get_parameter_value MEM_IF_ODT_WIDTH]) to [get_parameter_value $param_name]"
			}
		}
	}
}
...
proc ::alt_mem_if::gui::common_ddr_mem_model::_derive_parameters {} {
	...
	_dprint 1 "Preparing to derive parameters for common_ddr_mem_model"
	...
	if {[string compare -nocase [ get_parameter_value MEM_FORMAT ] "DISCRETE" ] == 0 } {
		...
		set lrdimm_rm [expr {pow(2, ([get_parameter_value MEM_ROW_ADDR_WIDTH] - 16))}]
		set_parameter_value MEM_IF_LRDIMM_RM $lrdimm_rm
		_dprint 1 "Computed LRDIMM Rank Multiplication factor of [get_parameter_value MEM_IF_LRDIMM_RM]"
    ...
    if {$mem_tras_ck > $mem_tras_ck_max} {
		_wprint "tRAS exceed limit of $mem_tras_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }
	...
	set_parameter_value MEM_TRCD [_within $mem_trcd_ck $mem_trcd_ck_min $mem_trcd_ck_max]
        if {$mem_trcd_ck > $mem_trcd_ck_max} {
		_wprint "tRCD exceed limit of $mem_trcd_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
        }
    ...
    if {$mem_trp_ck > $mem_trp_ck_max} {
		_wprint "tRP exceed limit of $mem_trp_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }

    set mem_trefi_ck_min 780
    set mem_trefi_ck_max 8316
    ...
    if {$mem_trefi_ck > $mem_trefi_ck_max} {
		_wprint "tREFI exceed limit of $mem_trefi_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }

    set mem_trfc_ck_min 12
    set mem_trfc_ck_max 375
    ...
    if {$mem_trfc_ck > $mem_trfc_ck_max} {
		_wprint "tRFC exceed limit of $mem_trfc_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }
    ...
    if {[regexp {^LPDDR2$} $ddr_mode] == 1} {
        ...
        if {$cfg_tccd_ck > $cfg_tccd_ck_max} {
		_wprint "tCCD exceed limit of $cfg_tccd_ck_max mem clocks."
        }
    }
    ...
    if {$mem_twr_ck > $mem_twr_ck_max} {
		_wprint "tWR exceed limit of $mem_twr_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }
    ...
    if {$mem_tfaw_ck > $mem_tfaw_ck_max} {
		_wprint "tFAW exceed limit of $mem_tfaw_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }
    ...
    if {$mem_trrd_ck > $mem_trrd_ck_max} {
		_wprint "tRRD exceed limit of $mem_trrd_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
    }
	...
	set_parameter_value MEM_TRTP [_within $mem_trtp_ck $mem_trtp_ck_min $mem_trtp_ck_max]
        if {$mem_trtp_ck > $mem_trtp_ck_max} {
		_wprint "tRTP exceed limit of $mem_trtp_ck_max mem clocks. Please update the value in \"Memory Timing\" tab."
        }

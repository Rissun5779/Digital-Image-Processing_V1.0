...
proc ::alt_mem_if::gui::common_ddrx_phy::validate_component {} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on uniphy_display_duplicate_ac_gui] 
		&& [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 
		&& [regexp {^DDR3$} $protocol] == 1 } {
		set_parameter_property DUPLICATE_AC VISIBLE true
	} 
...
proc ::alt_mem_if::gui::common_ddrx_phy::_create_derived_parameters {} {
	...
	::alt_mem_if::util::hwtcl_utils::_add_parameter MAX10_RTL_SEQ BOOLEAN false
	set_parameter_property MAX10_RTL_SEQ VISIBLE false
	set_parameter_property MAX10_RTL_SEQ DERIVED true
...
proc ::alt_mem_if::gui::common_ddrx_phy::_create_phy_parameters {} {
	...
	if {[regexp {^DDR3$|^HPS$} $ddr_mode ] == 1} {
		...
		Note DDR3L is currently supported only on Stratix V, Arria V GZ, Arria V, Cyclone V and MAX 10 ."
	}
...
proc ::alt_mem_if::gui::common_ddrx_phy::_create_board_settings_parameters {} {
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_SU DESCRIPTION "The reduction in the eye diagram on the setup side (or left side of the eye) on the address and command signals compared to compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_H DESCRIPTION "The reduction in the eye diagram on the hold side (or right side of the eye) on the address and command signals compared to compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_DQ_EYE_REDUCTION DESCRIPTION "The total reduction in the write eye diagram due to SI effects such as ISI/cross talk on DQ signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
	...
	set_parameter_property TIMING_BOARD_READ_DQ_EYE_REDUCTION DESCRIPTION "The total reduction in the read eye diagram due to SI effects such as ISI/cross talk on DQ signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_SU_APPLIED DESCRIPTION "The reduction in the eye diagram on the setup side (or left side of the eye) due to channel loss on the address and command signals compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_H_APPLIED DESCRIPTION "The reduction in the eye diagram on the hold side (or right side of the eye) due to channel loss on the address and command signals compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_DQ_EYE_REDUCTION_APPLIED DESCRIPTION "The total reduction in the write eye diagram due to SI effects such as ISI/cross talk on DQ signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
	...
	set_parameter_property TIMING_BOARD_READ_DQ_EYE_REDUCTION_APPLIED DESCRIPTION "The total reduction in the read eye diagram due to SI effects such as ISI/cross talk on DQ signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
	...
	The Quartus Prime software uses this skew to optimize the delay of the address and command signals to have appropriate setup and hold margins for DDR2 and DDR3 interfaces."
...
proc ::alt_mem_if::gui::common_ddrx_phy::_create_phy_parameters_gui {} {
	...
	add_display_item "PHY Settings" "Non-Destructive Calibration Settings" GROUP 
	add_display_item "Non-Destructive Calibration Settings" ENABLE_NON_DES_CAL PARAMETER
	add_display_item "Non-Destructive Calibration Settings" TREFI PARAMETER
	add_display_item "Non-Destructive Calibration Settings" REFRESH_INTERVAL PARAMETER
	add_display_item "Non-Destructive Calibration Settings" TRFC PARAMETER
	add_display_item "Non-Destructive Calibration Settings" AC_ROM_USER_ADD_0 PARAMETER
	add_display_item "Non-Destructive Calibration Settings" AC_ROM_USER_ADD_1 PARAMETER
	...
	add_display_item "Advanced PHY Settings" DUPLICATE_AC PARAMETER 
...
proc ::alt_mem_if::gui::common_ddrx_phy::_create_board_settings_parameters_gui {} {
	...
	add_display_item "Channel Signal Integrity" bs21 TEXT "<html>Channel Signal Integrity is a measure of the distortion of the eye due to intersymbol interference<br>or crosstalk or other effects. Typically when going from a single-rank configuration to a multi-rank<br>configuration there is an increase in the channel loss as there are multiple stubs causing<br>reflections. Please perform your channel signal integrity simulations and enter the extra channel<br>uncertainty as compared to Altera's reference eye diagram."
...
proc ::alt_mem_if::gui::common_ddrx_phy::_max { a b } {
	if { $a == "" } { 
		return $b
	} elseif { $a > $b } {
		return $a
...
proc ::alt_mem_if::gui::common_ddrx_phy::_derive_board_settings_parameters {} {
	...
	if {[string compare -nocase [get_parameter_value TIMING_BOARD_ISI_METHOD] MANUAL] == 0} {
		...
		set_parameter_value TIMING_BOARD_AC_EYE_REDUCTION_SU_APPLIED [_max $ac_eye_reduction_su 0]
		set_parameter_value TIMING_BOARD_AC_EYE_REDUCTION_H_APPLIED [_max $ac_eye_reduction_h 0]
		set_parameter_value TIMING_BOARD_DQ_EYE_REDUCTION_APPLIED [_max $dq_eye_reduction 0]
		set_parameter_value TIMING_BOARD_DELTA_DQS_ARRIVAL_TIME_APPLIED [_max $delta_dqs_arrival_time 0]
		set_parameter_value TIMING_BOARD_READ_DQ_EYE_REDUCTION_APPLIED [_max $read_dq_eye_reduction 0]
		set_parameter_value TIMING_BOARD_DELTA_READ_DQS_ARRIVAL_TIME_APPLIED [_max $delta_read_dqs_arrival_time 0]
...
proc ::alt_mem_if::gui::common_ddrx_phy::_derive_parameters {} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on use_2x_dll_clock] == 1 } {
		...
		set_parameter_value USE_DR_CLK true
		set_parameter_value DLL_USE_DR_CLK true
	} elseif {[::alt_mem_if::util::qini::cfg_is_on do_not_use_2x_dll_clock] == 1 } {
		_iprint "The DLL will be configured to not use a 2x clock due to the ini do_not_use_2x_dll_clock"
		set_parameter_value USE_DR_CLK false
		set_parameter_value DLL_USE_DR_CLK false
	} elseif {([regexp {^LPDDR2$} $protocol] == 1)} {
		set_parameter_value USE_DR_CLK false
	...
	if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0} {
		set_parameter_value MAX10_RTL_SEQ true
	} else {
		set_parameter_value MAX10_RTL_SEQ false
	}
}

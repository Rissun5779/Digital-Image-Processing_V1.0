...
proc ::alt_mem_if::gui::qdrii_phy::_create_phy_parameters {} {
	...
	set_parameter_property MAX10_RTL_SEQ DISPLAY_NAME "Enable RTL sequencer for Max10"
...
proc ::alt_mem_if::gui::qdrii_phy::_create_board_settings_parameters {} {
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_SU DESCRIPTION "The reduction in the eye diagram on the setup side (or left side of the eye) due to channel loss on the address/command signals compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_AC_EYE_REDUCTION_H DESCRIPTION "The reduction in the eye diagram on the hold side (or right side of the eye) due to channel loss on the address/command signals compared to the reference eye diagram."
	...
	set_parameter_property TIMING_BOARD_DQ_EYE_REDUCTION DESCRIPTION "The total reduction in the write eye diagram due to SI effects such as ISI/cross talk on D signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
	...
	set_parameter_property TIMING_BOARD_READ_DQ_EYE_REDUCTION DESCRIPTION "The total reduction in the read eye diagram due to SI effects such as ISI/cross talk on Q signals compared to the reference eye diagram.  Note it is assumed that the channel loss reduces the eye width symmetrically on the left and right side of the eye."
...
proc ::alt_mem_if::gui::qdrii_phy::_create_phy_parameters_gui {} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on uniphy_display_extra_parameters_gui]} {
		...
		set_parameter_property FIX_READ_LATENCY VISIBLE true
		add_display_item "Advanced PHY Settings" FIX_READ_LATENCY PARAMETER
	}
...
proc ::alt_mem_if::gui::qdrii_phy::_create_board_settings_parameters_gui {} {
	...
	add_display_item "Channel Signal Integrity" bs2 TEXT "<html>Channel Signal Integrity is a measure of the distortion of the eye due to intersymbol interference<br>or crosstalk or other effects. Typically when using a device depth greater than 1<br>there is an increase in the channel loss as there are multiple stubs causing<br>reflections. Please perform you channel signal integrity simulations and enter the extra channel<br>uncertainty as compared to Altera's reference eye diagram."	

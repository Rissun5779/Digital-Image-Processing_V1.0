...
proc ip_upgrade {ip_core_type version parameters} {
	...
	foreach { name value } $parameters {
		...
		if {[string compare -nocase $name SYS_INFO_DEVICE_FAMILY] == 0 || [string compare -nocase $name DEFAULT_FAST_SIM_MODEL] == 0} {
			continue
		}
      ...
      if { [string compare -nocase $version "13.1"] == 0 &&
         [string compare -nocase $name AUTO_DESIGN_ENVIRONMENT] == 0 } {
         continue;
      }

      if { [string compare -nocase $version "14.1"] == 0 &&
         [string compare -nocase $name AUTO_BASE_DEVICE] == 0 } {
         continue;
      }
...
proc ::alt_mem_if::gui::ddrx_controller::_create_derived_parameters {} {
	...
	::alt_mem_if::util::hwtcl_utils::_add_parameter CONTINUE_AFTER_CAL_FAIL Boolean false
	set_parameter_property CONTINUE_AFTER_CAL_FAIL VISIBLE false
	set_parameter_property CONTINUE_AFTER_CAL_FAIL DERIVED true

	::alt_mem_if::util::hwtcl_utils::_add_parameter MAX10_CFG Boolean false
	set_parameter_property MAX10_CFG VISIBLE false
	set_parameter_property MAX10_CFG DERIVED true
}
...
proc ::alt_mem_if::gui::ddrx_controller::_derive_parameters {} {
	...
	if {(([::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]) && ([regexp {^DDR3$} $protocol] == 1))} {
    ...
    if {[string compare -nocase [get_parameter_value NEXTGEN] "true"] == 0} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_advanced_odt_control]} {
			if {([regexp {^DDR3$} $protocol] == 1)} {
				set_parameter_value CFG_WRITE_ODT_CHIP [get_parameter_value WR_ODT_PATTERN]
				set_parameter_value CFG_READ_ODT_CHIP  [get_parameter_value RD_ODT_PATTERN]
				_dprint 1 "Advanced ODT: Overriding controller-computed ODT tables"
				_dprint 1 " CFG_WRITE_ODT_CHIP = WR_ODT_PATTERN => [get_parameter_value CFG_WRITE_ODT_CHIP]"
				_dprint 1 " CFG_READ_ODT_CHIP  = RD_ODT_PATTERN => [get_parameter_value CFG_READ_ODT_CHIP]"
			}
		}
   ...
   if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_continue_after_cal_fail]} {
       set_parameter_value CONTINUE_AFTER_CAL_FAIL true
   }
   
   if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "max10"] == 0} {
       set_parameter_value MAX10_CFG true
   }
...
proc ::alt_mem_if::gui::ddrx_controller::_compute_odt_chip {write} {
        ...
        if {[regexp {^DDR2$} $protocol] == 1 && ([array names cfg_write_ddr2_table $key_name] != "") && $write} {
                ...
                if {[info exists cfg_write_ddr3_lrdimm_table($key_name)] == 1} {
                        if {$write} {
...
set_parameter_property ENUM_MEM_IF_TRAS ALLOWED_RANGES {"TRAS_0" "TRAS_1" "TRAS_2" "TRAS_3" "TRAS_4" "TRAS_5" "TRAS_6" "TRAS_7" "TRAS_8" "TRAS_9" "TRAS_10" "TRAS_11" "TRAS_12" "TRAS_13" "TRAS_14" "TRAS_15" "TRAS_16" "TRAS_17" "TRAS_18" "TRAS_19" "TRAS_20" "TRAS_21" "TRAS_22" "TRAS_23" "TRAS_24" "TRAS_25" "TRAS_26" "TRAS_27" "TRAS_28" "TRAS_29" "TRAS_30" "TRAS_31" "TRAS_32" "TRAS_33" "TRAS_34" "TRAS_35" "TRAS_36"}
...
set_parameter_property ENUM_MEM_IF_TWR ALLOWED_RANGES {"TWR_1" "TWR_2" "TWR_3" "TWR_4" "TWR_5" "TWR_6" "TWR_7" "TWR_8" "TWR_9" "TWR_10" "TWR_11" "TWR_12" "TWR_13" "TWR_14" "TWR_15" "TWR_16"}
...
set_parameter_property INTG_MEM_IF_TREFI ALLOWED_RANGES 0:8316
...
set_parameter_property INTG_MEM_IF_TRFC ALLOWED_RANGES 0:375

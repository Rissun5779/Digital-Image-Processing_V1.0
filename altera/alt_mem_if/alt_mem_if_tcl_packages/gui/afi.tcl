...
proc ::alt_mem_if::gui::afi::validate_component {} {
	...
	if {[string compare -nocase [get_parameter_value FORCE_DQS_TRACKING] "Enabled"] == 0 &&
	    ([string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] != 0 &&
	     ...
	     [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "ARRIAVGZ"] != 0 &&
	     [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] != 0)} {
		_eprint "The option '[get_parameter_property FORCE_DQS_TRACKING DISPLAY_NAME]' is only applicable to Stratix V, Arria V GZ, Arria V, Cyclone V and Max 10 devices."
...
proc ::alt_mem_if::gui::afi::_derive_parameters {} {
	...
	if {[string compare -nocase [get_parameter_value USE_DQS_TRACKING] "true"] == 0 && [string compare -nocase [get_parameter_value USE_SHADOW_REGS] "true"] == 0} {
		_eprint "The options '[get_parameter_property FORCE_DQS_TRACKING DISPLAY_NAME]' and '[get_parameter_property FORCE_SHADOW_REGS DISPLAY_NAME]' cannot be enabled simultaneously."
	}
	...
	[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "MAX10"] == 0 &&
	[get_parameter_value MEM_CLK_FREQ] > 200} {
		_eprint "The specified Memory clock frequency [get_parameter_value MEM_CLK_FREQ] MHz is not supported. Maximum Memory clock frequency for Max 10 DDR2 is 200.0 MHz."
	}

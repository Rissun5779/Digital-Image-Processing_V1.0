...
(* altera_attribute = "-name IP_TOOL_NAME altera_mem_if_qdrii_phy_core; -name IP_TOOL_VERSION 18.1; -name FITTER_ADJUST_HC_SHORT_PATH_GUARDBAND 100" *)
...
wire	[MEM_IF_DQ_WIDTH-1:0]		mem_d_abst;
wire	[MEM_IF_CONTROL_WIDTH-1:0] 	mem_wps_n_abst;
wire	[MEM_IF_DM_WIDTH-1:0] 		mem_bws_n_abst;
wire	[MEM_IF_ADDR_WIDTH-1:0] 	mem_a_abst;
wire	[MEM_IF_CONTROL_WIDTH-1:0] 	mem_rps_n_abst;
wire	[MEM_IF_WRITE_DQS_WIDTH-1:0] 	mem_k_abst;
wire	[MEM_IF_WRITE_DQS_WIDTH-1:0] 	mem_k_n_abst;
...
`endif
	...
	.mem_d(mem_d_abst),
	.mem_wps_n(mem_wps_n_abst),
	.mem_bws_n(mem_bws_n_abst),
	.mem_a(mem_a_abst),
	.mem_rps_n(mem_rps_n_abst),
	.mem_doff_n(mem_doff_n),
	.mem_k(mem_k_abst),
	.mem_k_n(mem_k_n_abst),
	.mem_cq(mem_cq),
	.mem_cq_n(mem_cq_n),
	.mem_q(mem_q),
	.pll_afi_clk(afi_clk),
...
assign mem_k_cmp = mem_k[0] & mem_k_abst[0];
assign mem_k_n_cmp = mem_k_n[0] & mem_k_n_abst[0];
...
cmp_signals #(.NAME("ADDR_HI"), .WIDTH(MEM_IF_ADDR_WIDTH)) cmp_addr_hi (
	 .clk(mem_k_cmp),
	 .valid(ready),
	 .a(mem_a),
	 .b(mem_a_abst)
...
cmp_signals #(.NAME("ADDR_LO"), .WIDTH(MEM_IF_ADDR_WIDTH)) cmp_addr_lo (
	 .clk(mem_k_n_cmp),
	 .valid(ready),
	 .a(mem_a),
	 .b(mem_a_abst)
...
cmp_signals #(.NAME("WPS_HI"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_wps_n_hi (
	 .clk(mem_k_cmp),
	 .valid(ready),
	 .a(mem_wps_n),
	 .b(mem_wps_n_abst)
...
cmp_signals #(.NAME("WPS_LO"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_wps_n_lo (
	 .clk(mem_k_n_cmp),
	 .valid(ready),
	 .a(mem_wps_n),
	 .b(mem_wps_n_abst)
...
cmp_signals #(.NAME("RPS_HI"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_rps_n_hi (
	 .clk(mem_k_cmp),
	 .valid(ready),
	 .a(mem_rps_n),
	 .b(mem_rps_n_abst)
...
cmp_signals #(.NAME("RPS_LO"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_rps_n_lo (
	 .clk(mem_k_n_cmp),
	 .valid(ready),
	 .a(mem_rps_n),
	 .b(mem_rps_n_abst)
...
cmp_signals #(.NAME("BWS_HI"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_bws_n_hi (
	 .clk(mem_k_cmp),
	 .valid(ready),
	 .a(mem_bws_n),
	 .b(mem_bws_n_abst)
...
cmp_signals #(.NAME("BWS_LO"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_bws_n_lo (
	 .clk(mem_k_n_cmp),
	 .valid(ready),
	 .a(mem_bws_n),
	 .b(mem_bws_n_abst)
...
cmp_signals #(.NAME("D_HI"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_d_hi (
	.clk(mem_k_cmp),
...
cmp_signals #(.NAME("D_LO"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_d_lo (
	.clk(mem_k_n_cmp),
	.valid(afi_cal_success_cmp),
	.a(mem_d),
	.b(mem_d_abst)

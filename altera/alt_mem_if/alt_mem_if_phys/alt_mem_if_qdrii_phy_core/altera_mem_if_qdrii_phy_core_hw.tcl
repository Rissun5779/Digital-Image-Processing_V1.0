...
set_module_property DESCRIPTION "QDR II/II+ External Memory PHY"
set_module_property NAME altera_mem_if_qdrii_phy_core
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_phys_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "QDR II/II+ External Memory PHY"
...
proc generate_verilog_fileset {name tmpdir fileset {include_sim_vhdl_files 0}} {
	...
	set new_inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_qdrii_phy_core"
	...
	if {[string compare -nocase [get_parameter_value ABSTRACT_REAL_COMPARE_TEST] "true"] == 0} {
		set toplevel_inhdl_files_list [list altera_mem_if_qdrii_phy_core_abs_cmp_top.sv]
	} else {
		set toplevel_inhdl_files_list [list altera_mem_if_qdrii_phy_core_top.sv]
	}
	...
	} elseif {[string compare -nocase [get_parameter_value CALIBRATION_MODE] "Skip"] == 0} {
	...
	alt_mem_if::util::iptclgen::sub_strings_params [file join $tmpdir [lindex $parsed_toplevel_file_list 0]] [file join $tmpdir $toplevel_file_name] \
		[list VARIANT_OUTPUT_NAME $variant_name SIM_FILESET $simulation_fileset "FAST_SIM_MODEL_DEFAULT" $fast_sim_model_default TB_PROTOCOL "QDRII"]
...
proc generate_vhdl_sim {name} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_altdq_dqs2]} {
		set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "QDRII" $tmpdir SIM_VERILOG]
		lappend bb_top_files "${name}_altdqdqs.v" "${name}_altdqdqs_in.v"
	} else {
		set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "QDRII" $tmpdir SIM_VHDL]
		lappend bb_top_files "${name}_altdqdqs.vhd" "${name}_altdqdqs_in.vhd"
	}
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_phy]} {
		...
		if {[string compare -nocase [get_parameter_value USE_HARD_READ_FIFO] "true"] == 0} {
			...
			add_fileset_file $generated_filename [::alt_mem_if::util::hwtcl_utils::get_file_type $generated_filename 1 0] \
					PATH [file join $new_inhdl_common_dir "abstract" $generated_filename] $non_encryp_simulators
		}
	}
	...
	if {[string compare [get_parameter_value ENABLE_EXTRA_REPORTING] "true"] == 0} {
		set param_files [alt_mem_if::gen::uniphy_gen::generate_parameters_txt_file "${name}_" "QDRII" $tmpdir]
...
proc generate_verilog_sim {name} {
	...
	foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "QDRII" $tmpdir SIM_VERILOG] {
		set file_name [file tail $generated_file]
		_dprint 1 "Preparing to add $generated_file as $file_name"
		add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name] PATH $generated_file
	}
...
proc generate_synth {name} {
	...
	foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "QDRII" $tmpdir QUARTUS_SYNTH] {
		set file_name [file tail $generated_file]
		_dprint 1 "Preparing to add $generated_file as $file_name"
		add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name 0] PATH $generated_file
	}
	...
	set qdir $::env(QUARTUS_ROOTDIR)
	set inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_qdrii_phy_core"
	...
	foreach sdc_file [::alt_mem_if::gen::uniphy_gen::generate_sdc_file $name "QDRII" $constraints_dir $tmpdir] {
	...
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_report_timing_files $name "QDRII" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}
	
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_map_file $name "QDRII" [file join $common_constraints_dir memphy_common_pin_map.tcl] [file join $constraints_dir memphy_pin_map.tcl] $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_assignments_file $name "QDRII" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_parameters_file $name "QDRII" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	::alt_mem_if::gen::uniphy_gen::print_generation_done_message $name "QDRII"
...
proc ip_elaborate {} {
	...
	if {[string compare -nocase [get_parameter_value ENABLE_CSR_SOFT_RESET_REQ] "false"] == 0 ||
		[string compare -nocase [get_parameter_value ENABLE_EMIT_BFM_MASTER] "true"] == 0} {
	...
	::alt_mem_if::gen::uniphy_interfaces::afi "QDRII" "phy" "afi" 0


	if {[string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "QDRII" "phy" 1
	} else {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "QDRII" "phy" 0
	}
	...
	add_interface_port memory mem_d mem_d Output [get_parameter_value MEM_IF_DQ_WIDTH]
	add_interface_port memory mem_wps_n mem_wps_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]
	add_interface_port memory mem_bws_n mem_bws_n Output [get_parameter_value MEM_IF_DM_WIDTH]
	add_interface_port memory mem_a mem_a Output [get_parameter_value MEM_IF_ADDR_WIDTH]
	add_interface_port memory mem_q mem_q Input [get_parameter_value MEM_IF_DQ_WIDTH]
	add_interface_port memory mem_rps_n mem_rps_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]
	add_interface_port memory mem_k mem_k Output [get_parameter_value MEM_IF_WRITE_DQS_WIDTH]
	add_interface_port memory mem_k_n mem_k_n Output [get_parameter_value MEM_IF_WRITE_DQS_WIDTH]

	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_rtl_calib]} {
		if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 && [string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] != 0} { 

			add_interface_port memory mem_config_clk mem_config_clk Input 1
			add_interface_port memory mem_config_datain mem_config_datain Input 1
			add_interface_port memory mem_config_update mem_config_update Input 1
			add_interface_port memory mem_config_cqdqs_ena mem_config_cqdqs_ena Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
			add_interface_port memory mem_config_cq_io_ena mem_config_cq_io_ena Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
			add_interface_port memory mem_config_rio_ena mem_config_rio_ena	Input [get_parameter_value MEM_IF_DQ_WIDTH]
			add_interface_port memory mem_config_kdqs_ena mem_config_kdqs_ena Input [get_parameter_value MEM_IF_WRITE_DQS_WIDTH]
			add_interface_port memory mem_config_k_io_ena mem_config_k_io_ena Input [get_parameter_value MEM_IF_WRITE_DQS_WIDTH]
			add_interface_port memory mem_config_wio_ena mem_config_wio_ena Input [get_parameter_value MEM_IF_DQ_WIDTH]
			add_interface_port memory mem_config_dm_ena mem_config_dm_ena Input [get_parameter_value MEM_IF_DM_WIDTH]

			add_interface rtl_dll_sharing conduit end
			set_interface_property rtl_dll_sharing ENABLED true
			add_interface_port rtl_dll_sharing dll_offsetctrlout dll_offset_ctrl_offsetctrlout INPUT [get_parameter_value DLL_DELAY_CTRL_WIDTH]
		}
	}
	...
	if {[string compare -nocase [get_parameter_value NO_COMPLIMENTARY_STROBE] "true"] == 0} {
		if {[get_parameter_value MEM_T_RL] == 2} {
			add_interface_port memory mem_cq_n mem_cq_n Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
		} else {
			add_interface_port memory mem_cq mem_cq Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
		}
	} else {
		add_interface_port memory mem_cq mem_cq Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
		add_interface_port memory mem_cq_n mem_cq_n Input [get_parameter_value MEM_IF_READ_DQS_WIDTH]
	}
	add_interface_port memory mem_doff_n mem_doff_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]

...
proc memphy_perform_flexible_write_launch_timing_analysis {opcs opcname inst family scale_factors_name interface_type max_package_skew dll_length period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name} {
	...
	set msg_list [ list ]
	set d_pll_clock_id [memphy_get_output_clock_id [ join [ join $pins(d_groups) ]] "DQ output" msg_list]
	if {$d_pll_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "$msg"
		}
		...
		set dclksource [get_node_info -name $d_pll_clock_id]
	}
	...
	set pll_clock $pins(pll_k_clock)
...
`endif
			...
			set msg_list [list]
			set leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $kpin msg_list]

			set dqs_periphery_node ${leveling_delay_chain_name}|clkin
			
			set cps_name [memphy_traverse_to_clock_phase_select $kpin msg_list]
			set dqs_clk_phase_select_node ${cps_name}|clkout

			# Find paths from PLL to DQS clock periphery node
			set DQSpaths_max [get_path -rise_from $kclksource -rise_to $dqs_clk_phase_select_node -nworst 1]
			set DQSpaths_min [get_path -rise_from $kclksource -rise_to $dqs_clk_phase_select_node -nworst 1 -min_path]
			...
			set DQSperiphery_min [get_path -rise_from $dqs_clk_phase_select_node -rise_to $kpin -min_path -nworst 1]
			set DQSperiphery_max [get_path -rise_from $dqs_clk_phase_select_node -rise_to $kpin -nworst 1]
			...
			set aiot_delay [memphy_round_3dp [expr [memphy_get_rise_aiot_delay $kpin] * 1e9]]
			...
			foreach dpin $dpins {
				...
				set msg_list [list]
				set leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $dpin msg_list]
				
				set dq_periphery_node ${leveling_delay_chain_name}|clkin

				set cps_name [memphy_traverse_to_clock_phase_select $dpin msg_list]
				set dq_clk_phase_select_node ${cps_name}|clkout

				# Find paths from PLL to DQ clock periphery node
				set DQpaths_max [get_path -rise_from $dclksource -rise_to $dq_clk_phase_select_node -nworst 1]
				set DQpaths_min [get_path -rise_from $dclksource -rise_to $dq_clk_phase_select_node -nworst 1 -min_path]
				...
				set DQperiphery_min [get_path -rise_from $dq_clk_phase_select_node -rise_to $dpin -min_path -nworst 1]
				set DQperiphery_max [get_path -rise_from $dq_clk_phase_select_node -rise_to $dpin -nworst 1]				
				...
				set aiot_delay [memphy_round_3dp [expr [memphy_get_rise_aiot_delay $dpin] * 1e9]]
...
#############################################################
proc memphy_perform_flexible_read_capture_timing_analysis {opcs opcname inst family scale_factors_name io_std interface_type max_package_skew dqs_phase period all_q_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
	...
	set pins(cq_pins) [ concat $pins(cq_pins) $pins(cq_n_pins) ]
	set before_calibration_reporting [get_ini_var -name "qsta_enable_before_calibration_ddr_reporting"]
	if {![string equal -nocase $before_calibration_reporting off]} {
		...
		foreach qpins $pins(q_groups) {
			...
			foreach qpin $qpins {
				...
				if {$IP(read_deskew_mode) == "dynamic"} {
					...
					if {$dqs_width <= 36} {
						set pin_setup_slack [expr $pin_setup_slack + $total_DQ_DQS_pessimism]
						set pin_hold_slack  [expr $pin_hold_slack  + $total_DQ_DQS_pessimism]
					} else {
						set pin_setup_slack [expr $pin_setup_slack + $DQpath_pessimism_only_eol]
						set pin_hold_slack  [expr $pin_hold_slack  + $DQSpath_pessimism_only_eol]
					}
				...
				set pin_setup_slack [expr {$pin_setup_slack + 0.05}]
	...
	if {$IP(read_deskew_mode) == "dynamic"} {
		...
		# Consider variation in the delay chains used during dynamic deksew
		set dqs_period [ memphy_get_dqs_period $pins(cq_pins) ]
...
`ifdef IPTCL_READ_FIFO_HALF_RATE
	set dqs_pins ${prefix}*${fbasename}_read_datapath:uread_datapath|read_capture_clk_div2[*]
`else

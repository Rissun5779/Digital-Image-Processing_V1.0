...
`ifdef IPTCL_USE_2X_FF
		set pin_to_cps_path [list {IO_PAD fanin PADIN} {IO_OBUF fanin I} {PSEUDO_DIFF_OUT fanin I -optional} {DELAY_CHAIN fanin DATAIN -optional} {DELAY_CHAIN fanin DATAIN -optional} {FF fanin CLK -optional} {CLK_PHASE_SELECT fanin CLKOUT} ]
`else
...
`ifdef IPTCL_USE_LDC_AS_LOW_SKEW_CLOCK
		...
		set patterns "${instname}|p0|umemphy|uio_pads|dq_ddio[$i].uwrite|altdq_dqs2_inst|dqs_select*|clkout"
		lappend k_leveling_pins [ memphy_get_names_in_collection [ get_pins $patterns ] ]
`endif
...
`ifdef IPTCL_USE_LDC_AS_LOW_SKEW_CLOCK
	set patterns "${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|*|*cps|clkout"	
	lappend ac_leveling_pins [ memphy_get_names_in_collection [ get_pins $patterns ] ]
`endif	
...
`endif
	...
	lappend patterns cmd_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|udoff_n_pad|${addr_cmd_postfix}
...
`endif
	set pins(afi_phy_ck_pins) ${instname}|p0|umemphy|uio_pads|dq_ddio[0].uwrite|altdq_dqs2_inst|output_path_gen[0].hr_to_fr_lo~DFFLO
...
`ifdef IPTCL_CORE_PERIPHERY_DUAL_CLOCK
	set pins(p2c_read_ck_pins) ${instname}|p0|umemphy|p2c_read_clk_reg
	`ifdef IPTCL_FULL_RATE
...
`endif
	...
	# AFI PHY CLOCK PLL
	set pll_afi_phy_clock_id [memphy_get_output_clock_id $pins(afi_phy_ck_pins) "AFI CK" msg_list]
	if {$pll_afi_phy_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "memphy_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(afi_phy_ck_pins)]"
	} else {
		set pll_afi_phy_clock [memphy_get_pll_clock_name $pll_afi_phy_clock_id]
	}
...
`else
	set pll_k_clock_id [memphy_get_output_clock_id $pins(k_pins) "K Output" msg_list]
`endif
	if {$pll_k_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "memphy_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(k_pins)]"
	} else {
	...
	if {$pll_d_clock_id == -1} {
		...
		post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [ join [ join $pins(d_groups) ]]"
	} else {
...
`ifdef IPTCL_CORE_PERIPHERY_DUAL_CLOCK
	# CLOCK FOR THE HARD READ-FIFO READ-CLOCK PORT, FOR THE P2C TRANSFER
	...
	# CLOCK FOR THE HALF-RATE REGISTER (i.e. "half-rate" ddio_out), FOR THE C2P TRANSFER
 `ifdef IPTCL_FULL_RATE
...
`endif
	...
	set fifo_wraddress_reg $prefix|*:p0|*:umemphy|*:uio_pads|*:read_capture[*].uread|*:altdq_dqs2_inst|input_path_gen[*].read_fifo~WRITE_ADDRESS_DFF
	if { ! $entity_names_on } {
		set fifo_wraddress_reg $instname|p0|umemphy|uio_pads|read_capture[*].uread|altdq_dqs2_inst|input_path_gen[*].read_fifo~WRITE_ADDRESS_DFF
	}
	set pins(fifo_wraddress_reg) $fifo_wraddress_reg
	
	set fifo_rdaddress_reg $prefix|*:p0|*:umemphy|*:uio_pads|*:read_capture[*].uread|*:altdq_dqs2_inst|input_path_gen[*].read_fifo~READ_ADDRESS_DFF
	if { ! $entity_names_on } {
		set fifo_rdaddress_reg $instname|p0|umemphy|uio_pads|read_capture[*].uread|altdq_dqs2_inst|input_path_gen[*].read_fifo~READ_ADDRESS_DFF
	}
    ...
    set valid_fifo_wrdata_reg $prefix|*:p0|*:umemphy|*:uio_pads|*:read_capture[*].uread|*:altdq_dqs2_inst|vfifo~*QVLD_IN_DFF
    if { ! $entity_names_on } {
    	set valid_fifo_wrdata_reg $instname|p0|umemphy|uio_pads|read_capture[*].uread|altdq_dqs2_inst|vfifo~*QVLD_IN_DFF
    }
    set pins(valid_fifo_wrdata_reg) $valid_fifo_wrdata_reg

    set valid_fifo_rddata_reg $prefix|*:p0|*:umemphy|*:uio_pads|*:read_capture[*].uread|*:altdq_dqs2_inst|vfifo~*QVLD_OUT_DFF
    if { ! $entity_names_on } {
    	set valid_fifo_rddata_reg $instname|p0|umemphy|uio_pads|read_capture[*].uread|altdq_dqs2_inst|vfifo~*QVLD_OUT_DFF
    }
...
proc memphy_dump_static_pin_map { ddr_db_par filename } {
	...
	foreach instname $instnames {
		...
		memphy_static_map_expand_string $FH pins pll_afi_phy_clock

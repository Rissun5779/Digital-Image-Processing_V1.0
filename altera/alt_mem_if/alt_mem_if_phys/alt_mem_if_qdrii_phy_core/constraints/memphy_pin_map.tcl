...
`endif
	...
	if {$pll_afi_clock_id == -1} {
		...
		post_message -type critical_warning "memphy.sdc: Failed to find PLL clock for pins [join $pins(afi_ck_pins)]"
	} else {
...
`else 
	...
	set read_capture_ddio_capture ""
	
	`else
	set read_capture_ddio "${read_capture_ddio_prefix}read_data_out[*]*"
	set read_capture_ddio_capture [list	"${read_capture_ddio_prefix}read_data_out[*]*" \
										"${read_capture_ddio_prefix}input_path_gen[*].capture_reg~DFFLO" ]
	`endif
`endif

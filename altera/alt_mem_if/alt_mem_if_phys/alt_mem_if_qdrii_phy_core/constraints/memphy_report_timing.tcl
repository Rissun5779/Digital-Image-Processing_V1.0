...
`else
	set clk_to_write_d [memphy_traverse_to_ddio_out_pll_clock [lindex $all_d_pins 0] msg_list]
...
`endif
	...
	# DQS_phase offset
	set dqs_phase [ memphy_get_dqs_phase $cq_pins ]

	# Get the interface type (HPAD or VPAD)
	set interface_type [memphy_get_io_interface_type $all_q_pins]
	...
	set io_std [memphy_get_io_standard [lindex $all_q_pins 0]]
...
`ifdef IPTCL_PRINT_MACRO_TIMING
				set assumptions_valid [memphy_verify_high_performance_timing_assumptions $inst pins IP $mem_if_memtype]
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
			}
`endif	
...
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_TIMING
			memphy_perform_flexible_read_capture_timing_analysis $opcs $opcname $inst $family $DQS_min_max $io_std $interface_type $max_package_skew $dqs_phase $period $all_q_pins pins t summary MP IP board fpga
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		} else {
`endif
`ifdef IPTCL_PRINT_MACRO_TIMING
			memphy_perform_macro_read_capture_timing_analysis $opcs $opcname $inst $dqs_phase $all_q_pins pins t summary IP board
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		}
`endif

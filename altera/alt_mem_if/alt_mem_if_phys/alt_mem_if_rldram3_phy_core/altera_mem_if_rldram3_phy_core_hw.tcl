...
set_module_property DESCRIPTION "RLDRAM 3 External Memory PHY"
set_module_property NAME altera_mem_if_rldram3_phy_core
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_phys_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "RLDRAM 3 External Memory PHY"
...
proc generate_verilog_fileset {name tmpdir fileset {include_sim_vhdl_files 0}} {
	...
	set new_inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_rldram3_phy_core"
	...
	if {[string compare -nocase [get_parameter_value ABSTRACT_REAL_COMPARE_TEST] "true"] == 0} {
		set toplevel_inhdl_files_list [list altera_mem_if_rldram3_phy_core_abs_cmp_top.sv]
	} else {
		set toplevel_inhdl_files_list [list altera_mem_if_rldram3_phy_core_top.sv]
	}
	...
	alt_mem_if::util::iptclgen::sub_strings_params [file join $tmpdir [lindex $parsed_toplevel_file_list 0]] [file join $tmpdir $toplevel_file_name] \
		[list VARIANT_OUTPUT_NAME $variant_name SIM_FILESET $simulation_fileset "FAST_SIM_MODEL_DEFAULT" $fast_sim_model_default TB_PROTOCOL "RLDRAM3"]
...
proc generate_vhdl_sim {name} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_altdq_dqs2]} {
		set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "RLDRAM3" $tmpdir SIM_VERILOG]
		...
		set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "RLDRAM3" $tmpdir SIM_VHDL]
	...
	if {[string compare [get_parameter_value ENABLE_EXTRA_REPORTING] "true"] == 0} {
		set param_files [alt_mem_if::gen::uniphy_gen::generate_parameters_txt_file "${name}_" "RLDRAM3" $tmpdir]
...
proc generate_verilog_sim {name} {
	...
	foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "RLDRAM3" $tmpdir SIM_VERILOG] {
		set file_name [file tail $generated_file]
		_dprint 1 "Preparing to add $generated_file as $file_name"
		add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name] PATH $generated_file
	}
...
proc generate_synth {name} {
	...
	foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "RLDRAM3" $tmpdir QUARTUS_SYNTH] {
		set file_name [file tail $generated_file]
		_dprint 1 "Preparing to add $generated_file as $file_name"
		add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name 0] PATH $generated_file
	}
	...
	set qdir $::env(QUARTUS_ROOTDIR)
	set inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_rldram3_phy_core"
	...
	foreach sdc_file [::alt_mem_if::gen::uniphy_gen::generate_sdc_file $name "RLDRAM3" $constraints_dir $tmpdir] {
	...
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_report_timing_files $name "RLDRAM3" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}
	
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_map_file $name "RLDRAM3" [file join $common_constraints_dir memphy_common_pin_map.tcl] [file join $constraints_dir memphy_pin_map.tcl] $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_assignments_file $name "RLDRAM3" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_parameters_file $name "RLDRAM3" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	::alt_mem_if::gen::uniphy_gen::print_generation_done_message $name "RLDRAM3"
...
proc ip_elaborate {} {
	...
	::alt_mem_if::gen::uniphy_interfaces::afi "RLDRAM3" "phy" "afi" 0


	if {[string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "RLDRAM3" "phy" 1
	} else {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "RLDRAM3" "phy" 0
	}

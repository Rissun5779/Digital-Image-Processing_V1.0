...
#    t(CS) = tAS
#    t(CH) = tAH
set t(AS) [expr { IPTCL_TIMING_BOARD_TIS_APPLIED / 1000.0 }]
set t(AH) [expr { IPTCL_TIMING_BOARD_TIH_APPLIED / 1000.0 }]

# Data Setup/Hold
set t(DS) [expr { IPTCL_TIMING_BOARD_TDS_APPLIED / 1000.0 }]
set t(DH) [expr { IPTCL_TIMING_BOARD_TDH_APPLIED / 1000.0 }]

# DQ output hold time from QK
set t(QH) [expr { 1.0 * IPTCL_TIMING_TQH * $t(CK)}]

# QK clock edge to DQ data edge (in same group)
set t(QKQ_max) [expr { IPTCL_TIMING_TQKQ_MAX / 1000.0 }]
set t(QKQ_min) [expr { -1.0 * ($t(CK) / 2.0 - $t(QH))}]

# Clock to Input Clock
set t(CKDK_max) [expr { 1.0 * IPTCL_TIMING_TCKDK_MAX * $t(CK) }]
set t(CKDK_min) [expr { 1.0 * IPTCL_TIMING_TCKDK_MIN * $t(CK) }]
...
set t(rd_to_wr_nop_commands) IPTCL_TIMING_CONTROLLER_RD_TO_WR_NOP_COMMANDS
set t(wr_to_rd_nop_commands) IPTCL_TIMING_CONTROLLER_WR_TO_RD_NOP_COMMANDS
...
set ISI(READ_DQ) [expr { IPTCL_TIMING_BOARD_READ_DQ_EYE_REDUCTION / 1000.0 }]
set ISI(READ_DQS) [expr { IPTCL_TIMING_BOARD_DELTA_READ_DQS_ARRIVAL_TIME / 1000.0 }]

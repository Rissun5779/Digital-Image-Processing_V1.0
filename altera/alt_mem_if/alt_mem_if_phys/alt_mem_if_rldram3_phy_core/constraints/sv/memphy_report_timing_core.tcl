...
`ifdef REMOVE_ME_WHEN_WE_HAVE_RLDRAM3_CONTROLLER
...
proc memphy_perform_ac_analyses {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {
	...
	foreach ac_pin $ac_pins {
		...
		set matched [regexp {addr_cmd} $ac_clk_phase_select_node matched]
		if {$matched == 1} {
			regsub {[^\.]+$} $ac_clk_phase_select_node "" ac_clk_phase_select_node
...
proc memphy_perform_dk_vs_ck_timing_analysis {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {

	################################################################################
	##The DK/CK analysis concerns the timing relationship between CK and DK. 
	################################################################################
	...
	# DK vs CK
	set panel_name_setup  "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst DK vs CK (setup)"
	set panel_name_hold   "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst DK vs CK (hold)"
	
	######################################################################
	##Default DK vs CK Analysis
	set res_0 [report_timing -detail full_path -to [get_ports $pins(dk_pins)] \
		-npaths $num_failing_path -panel_name $panel_name_setup -setup -disable_panel_color -quiet]
	set res_1 [report_timing -detail full_path -to [get_ports $pins(dk_pins)] \
		-npaths $num_failing_path -panel_name $panel_name_hold -hold -disable_panel_color -quiet]
	set default_setup_slack [lindex $res_0 1]
	set default_hold_slack [lindex $res_1 1]
	
	# Perform the default timing analysis to get required and arrival times
	set paths_setup [get_timing_paths -to [get_ports $pins(dk_pins)] -npaths 100 -setup -nworst 1]
	set paths_hold  [get_timing_paths -to [get_ports $pins(dk_pins)] -npaths 100 -hold  -nworst 1]
	...
	foreach dk_pin $pins(dk_pins) {
		...
		set pin_setup_slack [memphy_min_in_collection_to_name $paths_setup "slack" $dk_pin]
		set pin_hold_slack  [memphy_min_in_collection_to_name $paths_hold  "slack" $dk_pin]
		set current_ccpr    [memphy_min_in_collection_to_name $paths_hold  "ccpp" $dk_pin]
		
		set dk_clk_phase_select_node [memphy_traverse_to_clock_phase_select $dk_pin msg_list]
		regsub {^[^\:]+\:} $dk_clk_phase_select_node "" dk_clk_phase_select_node
		regsub -all {\|[^\:]+\:} $dk_clk_phase_select_node "|" dk_clk_phase_select_node
		
		set matched [regexp {addr_cmd} $dk_clk_phase_select_node matched]
		if {$matched == 1} {
			regsub {[^\.]+$} $dk_clk_phase_select_node "" dk_clk_phase_select_node
			set dk_periphery_node ${dk_clk_phase_select_node}cps|clkout
		} else {
			set dk_periphery_node ${dk_clk_phase_select_node}|clkout		
		}

		set dk_max [memphy_get_max_clock_path_delay_through_clock_node \
			${pll_interface}uphy_clkbuf_memphy|inclk[0] \
			${pll_interface}uphy_clkbuf_memphy|outclk[0] \
			 $dk_periphery_node]

		set dk_min [memphy_get_min_clock_path_delay_through_clock_node \
			${pll_interface}uphy_clkbuf_memphy|inclk[0] \
			${pll_interface}uphy_clkbuf_memphy|outclk[0] \
			 $dk_periphery_node]
		
		set phy_clock_ccpr [expr ($clk_max - $clk_min + $dk_max - $dk_min)/2.0]
		...
		set DKperiphery_min [get_path -rise_from $dk_periphery_node -fall_to $pins(dk_pins) -min_path -nworst 1]
		set DKperiphery_max [get_path -rise_from $dk_periphery_node -fall_to $pins(dk_pins) -nworst 1]
		set DKperiphery_min_delay [memphy_min_in_collection $DKperiphery_min "arrival_time"]
		set DKperiphery_max_delay [memphy_max_in_collection $DKperiphery_max "arrival_time"]
		set aiot_delay [memphy_round_3dp [expr [memphy_get_rise_aiot_delay $dk_pin] * 1e9]]
		set DKperiphery_min_delay [expr $DKperiphery_min_delay - $aiot_delay]
		set DKperiphery_max_delay [expr $DKperiphery_max_delay - $aiot_delay]
		set DKpath_pessimism  [expr $DKperiphery_min_delay*($scale_factors(eol) - $scale_factors(eol)/$eol_reduction_factor)]
		
		set pin_setup_slack [expr $pin_setup_slack + $DKpath_pessimism]
		set pin_hold_slack [expr $pin_hold_slack + $CKpath_pessimism]
	...
	set dk_summary [list]
	lappend dk_summary [list "  Standard DK vs CK" [memphy_format_3dp $default_setup_slack] [memphy_format_3dp $default_hold_slack]]
	lappend dk_summary [list "  Extra common clock pessimism removal" [memphy_format_3dp [expr $setup_slack - $default_setup_slack]] [memphy_format_3dp [expr $hold_slack - $default_hold_slack]]]

	set positive_fcolour [list "black" "blue" "blue"]
	set negative_fcolour [list "black" "red"  "red"]
	set panel_name "$inst DK vs CK"
	set root_folder_name [get_current_timequest_report_folder]
	...
	set fcolour [memphy_get_colours $setup_slack $hold_slack] 
	add_row_to_table -id $panel_id [list "DK vs CK" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]] -fcolor $fcolour
	lappend summary [list $opcname 0 "DK vs CK ($opcname)" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]]
...
proc memphy_perform_flexible_bus_turnaround_time_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD outputDQSpathjitter outputDQSpathjitter_setup_prop pll_steps pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	# (WL - RL), which is always 1 for RLDRAM2/3
	...
	# 1 if read and write commands are on consecutive CK cycles.
	# 2 if there's 1 NOP command between read and write.
	# 3 if there are 2 NOP commands, etc.
	set min_dly [expr $min_dly + ($t(rd_to_wr_nop_commands) + 1)*$t(CK)]

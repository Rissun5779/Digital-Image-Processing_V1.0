...
proc memphy_traverse_to_clock_phase_select {pin msg_list_name} {
	...
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		set pin_to_cps_path [list {IO_PAD fanin PADIN} {IO_OBUF fanin I} {PSEUDO_DIFF_OUT fanin I -optional} {DELAY_CHAIN fanin DATAIN -optional} {DELAY_CHAIN fanin DATAIN -optional} {FF fanin CLK -optional} {DDIO_OUT fanin CLK -optional} {DDIO_OUT fanin CLKHI -optional} {CLK_PHASE_SELECT end CLKOUT} ]
...
`ifdef IPTCL_QUARTER_RATE			
			set dqs_in_clock(div4_name) "${instname}|div4_clock_$i"
			set dqs_in_clock(div4_pin) "${instname}|p0|umemphy|uread_datapath|read_capture_clk_div4[$i]"
			set dqs_in_clock(div4_source_pin) "${instname}|p0|umemphy|uread_datapath|clock_div2[$i].uread_clock_divider|clkout"
`endif			
...
`endif
	...
	for { set i 0 } { $i < $::GLOBAL_number_of_dm_pins } { incr i } {
		...
		set dm_cps_pattern ${instname}|p0|umemphy|uio_pads|dq_ddio[$dq_ddio_index].ubidir_dq_dqs|altdq_dqs2_inst|dm_select*|clkout
		lappend dm_leveling_pins [ memphy_get_names_in_collection [ get_pins $dm_cps_pattern ] ]
	}
...
`ifdef IPTCL_USE_LDC_AS_LOW_SKEW_CLOCK	
	...
	for { set i 0 } { $i < [ llength $pins(ac_wo_reset_pins) ] } { incr i } {
		set ac_pin [ lindex $pins(ac_wo_reset_pins) $i ]		
...
`ifdef IPTCL_USE_LDC_AS_LOW_SKEW_CLOCK
	...
	set pins(dm_leveling_pins) [ join $dm_leveling_pins ]
...
`ifdef IPTCL_USE_LDC_AS_LOW_SKEW_CLOCK
		...
		puts $FH "DM LEVELING PINS: $pins(dm_leveling_pins)"

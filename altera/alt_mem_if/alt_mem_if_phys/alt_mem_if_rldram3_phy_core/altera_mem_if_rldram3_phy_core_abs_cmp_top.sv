...
(* altera_attribute = "-name IP_TOOL_NAME altera_mem_if_rldram3_phy_core; -name IP_TOOL_VERSION 18.1; -name FITTER_ADJUST_HC_SHORT_PATH_GUARDBAND 100" *)
...
parameter MEM_IF_RESERVED_PINS_WIDTH = IPTCL_MEM_IF_RESERVED_PINS_WIDTH;
...
wire  				   mem_we_n_abst;
wire  				   mem_ref_n_abst;
wire  				   mem_reset_n_abst;
`ifdef MEM_IF_DM_PINS_EN
wire  [MEM_IF_DM_WIDTH-1:0]  	   mem_dm_abst;
`endif
wire  				   mem_ck_abst;
wire  				   mem_ck_n_abst;
wire  [MEM_IF_WRITE_DQS_WIDTH-1:0] mem_dk_abst;
wire  [MEM_IF_WRITE_DQS_WIDTH-1:0] mem_dk_n_abst;
...
`ifdef QUARTER_RATE
integer MEM_T_WL_int = ((MEM_T_WL-1)/4);
`endif
`ifdef HALF_RATE
integer MEM_T_WL_int = (MEM_T_WL/2-1);
`endif
...
`endif	
	.mem_we_n(mem_we_n_abst),
   	.mem_ref_n(mem_ref_n_abst),
	.mem_reset_n(mem_reset_n_abst),
   	.mem_ck(mem_ck_abst),
   	.mem_ck_n(mem_ck_n_abst),
  	.mem_dk(mem_dk_abst),
   	.mem_dk_n(mem_dk_n_abst),
   	.mem_qk(mem_qk),
   	.mem_qk_n(mem_qk_n),
   	.mem_dq(mem_dq_abst),
	.pll_afi_clk(afi_clk),
...
always @(posedge mem_dk[0] or posedge mem_dk_n[0])
begin
...
always @(posedge mem_dk_abst[0] or posedge mem_dk_n_abst[0])
begin
...
wire [AFI_RATE_RATIO-1:0] afi_rdata_valid_cmp;

assign mem_dk_cmp = mem_dk[0] & mem_dk_abst[0];
assign mem_dk_n_cmp = mem_dk_n[0] & mem_dk_n_abst[0];
assign mem_ck_cmp = mem_ck & mem_ck_abst;
assign afi_cal_success_cmp = afi_cal_success & afi_cal_success_abst;
assign afi_rdata_valid_cmp = afi_rdata_valid & afi_rdata_valid_abst;

wire [AFI_DQ_WIDTH-1:0] afi_rdata_masked; 
wire [AFI_DQ_WIDTH-1:0] afi_rdata_abst_masked; 

generate
	genvar t;
	for (t = 0; t < AFI_RATE_RATIO; ++t)
	begin : timeslot
		assign afi_rdata_masked     [DQ_DDR_WIDTH * t +: DQ_DDR_WIDTH] = 
			afi_rdata_valid[t] ? afi_rdata[DQ_DDR_WIDTH * t +: DQ_DDR_WIDTH] : '0;
         
		assign afi_rdata_abst_masked[DQ_DDR_WIDTH * t +: DQ_DDR_WIDTH] = 
			afi_rdata_valid_abst[t] ? afi_rdata_abst[DQ_DDR_WIDTH * t +: DQ_DDR_WIDTH] : '0;
	end
endgenerate
...
cmp_signals #(.NAME("REF"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_ref_n (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_ref_n),
	 .b(mem_ref_n_abst)
...
cmp_signals #(.NAME("REF"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_reset_n (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_reset_n),
	 .b(mem_reset_n_abst)

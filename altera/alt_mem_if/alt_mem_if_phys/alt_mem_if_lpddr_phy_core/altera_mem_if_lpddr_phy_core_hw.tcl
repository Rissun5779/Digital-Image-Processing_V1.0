...
set_module_property DESCRIPTION "LPDDR SDRAM External Memory PHY"
set_module_property NAME altera_mem_if_lpddr_phy_core
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_phys_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "LPDDR SDRAM External Memory PHY"
...
proc generate_verilog_fileset {name tmpdir fileset {include_sim_vhdl_files 0}} {
	...
	set new_inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_lpddr_phy_core"
	...
	if {[string compare -nocase [get_parameter_value USE_HARD_READ_FIFO] "true"] == 0} {
		...
		lappend inhdl_files_list "simple_ddio_out.sv"
	}
	

	if {[string compare -nocase [get_parameter_value ABSTRACT_REAL_COMPARE_TEST] "true"] == 0} {
		set toplevel_inhdl_files_list [list altera_mem_if_lpddr_phy_core_abs_cmp_top.sv]
	} else {
		set toplevel_inhdl_files_list [list altera_mem_if_lpddr_phy_core_top.sv]
	}
	...
	alt_mem_if::util::iptclgen::sub_strings_params [file join $tmpdir [lindex $parsed_toplevel_file_list 0]] [file join $tmpdir $toplevel_file_name] \
		[list VARIANT_OUTPUT_NAME $variant_name SIM_FILESET $simulation_fileset "FAST_SIM_MODEL_DEFAULT" $fast_sim_model_default TB_PROTOCOL "LPDDR1"]
...
proc generate_synth {name} {
	...
	set qdir $::env(QUARTUS_ROOTDIR)
	set inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_lpddr_phy_core"
...
proc ip_elaborate {} {
	...
	::alt_mem_if::gen::uniphy_interfaces::afi "LPDDR1" "phy" "afi" 0


	if {[string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "LPDDR1" "phy" 1
	} else {
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "LPDDR1" "phy" 0
	}

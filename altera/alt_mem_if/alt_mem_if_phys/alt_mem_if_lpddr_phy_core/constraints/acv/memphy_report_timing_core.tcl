...
proc memphy_perform_flexible_write_launch_timing_analysis {opcs opcname inst family scale_factors_name interface_type max_package_skew dll_length period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name} {
	...
	set eol_reduction_factor $IP(eol_reduction_factor)
	...
	set pll_clock $pins(pll_afi_clock)
	regsub {\|divclk$} $pll_clock "" pll_clock
	regsub {pll[0-9]~PLL_OUTPUT_COUNTER} $pll_clock "" pll_interface
	regsub {pll[0-9]_phy~PLL_OUTPUT_COUNTER} $pll_interface "" pll_interface
	set pll_max [memphy_min_in_collection [get_path -rise_from ${pll_interface}pll*~PLL_OUTPUT_COUNTER|vco*ph[0] -rise_to ${pll_interface}pll*~PLL_OUTPUT_COUNTER|divclk] "arrival_time"]
	set pll_min [memphy_min_in_collection [get_path -rise_from ${pll_interface}pll*~PLL_OUTPUT_COUNTER|vco*ph[0] -rise_to ${pll_interface}pll*~PLL_OUTPUT_COUNTER|divclk -min_path] "arrival_time"]
	...
	if {($result == 1)} {
		...
		foreach dqpins $pins(q_groups) {
			...
			if {$matched == 1} {
				regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
				set dqs_periphery_node ${leveling_delay_chain_name}leveling_delay_chain_dqs|clkin					
			} else {
				regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
				set dqs_periphery_node ${leveling_delay_chain_name}leveling_delay_chain_dqs|clkin
			}
			set dqs_clk_phase_select_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|clk_phase_select_dqs|clkout
...
`ifdef IPTCL_USE_2X_FF
			...
			set DQSperiphery_min1 [get_path -rise_from [get_pins -compatibility_mode $dqs_periphery_node] -rise_to $twox_reg_node -min_path -nworst 1]
			set DQSperiphery_min2 [get_path -rise_from $twox_reg_node -rise_to $dqspin -min_path -nworst 1]
			set DQSperiphery_min_delay [expr [memphy_min_in_collection $DQSperiphery_min1 "arrival_time"] + [memphy_min_in_collection $DQSperiphery_min2 "arrival_time"]]
			set DQSperiphery_max1 [get_path -rise_from [get_pins -compatibility_mode $dqs_periphery_node] -rise_to $twox_reg_node -nworst 1]
			set DQSperiphery_max2 [get_path -rise_from $twox_reg_node -rise_to $dqspin -nworst 1]
			set DQSperiphery_min_delay [expr [memphy_max_in_collection $DQSperiphery_max1 "arrival_time"] + [memphy_max_in_collection $DQSperiphery_max2 "arrival_time"]]
`else
...
`endif
			set aiot_delay [memphy_round_3dp [expr [memphy_get_rise_aiot_delay $dqspin] * 1e9]]
			...
			set DQSpath_pessimism [expr ($DQSperiphery_max_delay - $DQSperiphery_min_delay)/$scale_factors(total)*($scale_factors(emif)+(1-1.0/$eol_reduction_factor)*$scale_factors(eol))]
			...
			foreach dqpin $dqdmpins {
				...
				if {$matched == 1} {
					regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
					set dq_periphery_node ${leveling_delay_chain_name}leveling_delay_chain_dq|clkin					
				} else {
					regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
					set dq_periphery_node ${leveling_delay_chain_name}leveling_delay_chain_dq|clkin
				}
				set dq_clk_phase_select_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|clk_phase_select_dq|clkout
...
`ifdef IPTCL_USE_2X_FF
				set DQperiphery_min1 [get_path -rise_from [get_pins -compatibility_mode $dq_periphery_node] -rise_to $twox_reg_node_dq -min_path -nworst 1]
				set DQperiphery_min2 [get_path -rise_from $twox_reg_node_dq -to $dqpin -min_path -nworst 1]
				set DQperiphery_max1 [get_path -rise_from [get_pins -compatibility_mode $dq_periphery_node] -rise_to $twox_reg_node_dq -nworst 1]
				set DQperiphery_max2 [get_path -rise_from $twox_reg_node_dq -to $dqpin -nworst 1]
				set DQperiphery_min_delay [expr [memphy_min_in_collection $DQperiphery_min1 "arrival_time"] + [memphy_min_in_collection $DQperiphery_min2 "arrival_time"]]
				set DQperiphery_max_delay [expr [memphy_max_in_collection $DQperiphery_max1 "arrival_time"] + [memphy_max_in_collection $DQperiphery_max2 "arrival_time"]]
`else
...
`endif
				...
				set DQpath_pessimism [expr ($DQperiphery_max_delay - $DQperiphery_min_delay)/$scale_factors(total)*($scale_factors(emif)+(1-1.0/$eol_reduction_factor)*$scale_factors(eol))]
...
`ifdef IPTCL_ENABLE_EXTRA_REPORTING
	...
	`endif
		set phy_regs [remove_from_collection [get_registers $inst|*] [get_registers $pins(read_capture_ddio)]]
	} else {
	`ifdef IPTCL_HARD_PHY
	`else
		set controller_regs [get_registers $prefix_controller|*:*alt_*controller_*inst|*]
	`endif
		set phy_regs [remove_from_collection [get_registers $prefix|*] [get_registers $pins(read_capture_ddio)]]
	}
...
`ifdef IPTCL_HARD_PHY
	...
	set output_counter_portion_ccpr 0
	...
	foreach ck_pin $pins(ck_pins) {
		...
		regsub {[^\|]+$} $clk_leveling_delay_chain_name "" clk_leveling_delay_chain_name
		set clk_periphery_node ${clk_leveling_delay_chain_name}leveling_delay_chain_dqs|clkin
	...
	foreach ac_pin $ac_pins {
		if {![string equal "mem_reset_n" $ac_pin]} {
			...
			regsub {[^\|]+$} $ac_leveling_delay_chain_name "" ac_leveling_delay_chain_name
			set ac_periphery_node ${ac_leveling_delay_chain_name}leveling_delay_chain_dqs|clkin
...
proc memphy_perform_flexible_postamble_timing_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD DQSpathjitter pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set dqsenableextend_regs [list *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_ctrl~IN0_DFF]
	if {[get_collection_size [get_registers -nowarn $dqsenableextend_regs]] == 0} {
		set dqsenableextend_regs [list *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_ctrl~DQS_ENABLE_OUT_DFF]
	}

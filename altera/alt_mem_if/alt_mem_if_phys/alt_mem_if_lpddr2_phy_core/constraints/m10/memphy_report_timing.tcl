...
set tDCD 0.02
	...
	foreach opc $opcs {
		...
		memphy_perform_flexible_write_launch_timing_analysis $opcs $opcname $inst $family scale_factors $interface_type $max_package_skew $period pins t summary MP IP board

		#######################################
		# Read Analysis

		memphy_perform_flexible_read_capture_non_dqs_timing_analysis $opcs $opcname $inst $family $period $all_dq_pins pins t summary MP IP board
		...
		memphy_perform_flexible_bus_turnaround_time_analysis $opcs $opcname $inst $family $period $interface_type $tJITper $tDCD $pll_steps pins t summary MP IP SSN board ISI
...
`ifdef IPTCL_ENABLE_EXTRA_REPORTING
		...
		foreach clock_fmax_line $clock_fmax {
			...
			if {[string compare -nocase $clock_name "${inst}|pll0|upll_memphy|auto_generated|pll1|clk\[4\]_afi_clk"] == 0} {

...
set outputDQSpathjitter 0.116
set outputDQSpathjitter_setup_prop 0.5
set tJITper 0.253 
set tDH_tDS [expr $t(DH) + $t(DS)]
set tDH_tDS_setup_prop 0.3
...
set t(wru_output_min_delay_external) [expr $tDH_tDS*(1.0-$tDH_tDS_setup_prop) + $board(intra_DQS_group_skew)/2 + $ISI(DQ)/2 + $ISI(DQS)/2 - $board(DQ_DQS_skew)]
...
set t(wru_output_max_delay_external) [expr $tDH_tDS*$tDH_tDS_setup_prop + $board(intra_DQS_group_skew)/2 + $ISI(DQ)/2 + $ISI(DQS)/2 + $board(DQ_DQS_skew)]
...
foreach { inst } $instances {
	...
	set resync_clock ${inst}|pll0|upll_memphy|auto_generated|pll1|clk\[2\]
        ...
        set local_resync_clk [ memphy_get_or_add_clock_vseries \
  		-target $resync_clock \
  		-suffix "resync_clk" \
  		-source $pll_ref_clock \
	...
	set tracking_clock ${inst}|pll0|upll_memphy|auto_generated|pll1|clk\[3\]
        ...
        set local_tracking_clk [ memphy_get_or_add_clock_vseries \
...
#########################
        ...
        if ($fit_flow) {
        set_min_delay -from [get_registers *ddr_input[*]] -to [get_registers *ddio_phy_dq_r[*]] 0.1
        }

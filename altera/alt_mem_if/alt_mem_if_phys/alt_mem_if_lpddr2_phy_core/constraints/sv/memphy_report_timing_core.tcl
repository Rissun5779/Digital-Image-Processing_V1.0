...
proc memphy_perform_flexible_read_capture_timing_analysis {opcs opcname inst family scale_factors_name io_std interface_type max_package_skew dqs_phase period all_dq_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
	...
	if {$IP(read_deskew_mode) == "dynamic"} {
		...
		# Consider variation in the delay chains used during dynamic deksew
		set dqs_period [memphy_get_dqs_period $pins(dqs_pins) ]
		set offset_from_90 [expr abs(90/360.0*$period - $dqs_phase/360.0*$dqs_period)]
		if {$IP(num_ranks) <= $IP(num_shadow_registers)} {
			set t1_variation [expr [min [expr $offset_from_90 + [max [expr $MP(DQSQ)*$t(DQSQ)] [expr $MP(QHS)*$t(QHS)]] + 2*$board(intra_DQS_group_skew) + $max_package_skew + $fpga(tDQS_PSERR)] [max $max_read_deskew_setup $max_read_deskew_hold]]*2*$t1_vt_variation_percent*0.75]
		} else {
...
proc memphy_perform_ac_analyses {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {
	...
	set clk_max [memphy_max_in_collection [get_path -rise_from ${pll_interface}uphy_clkbuf_memphy|inclk[0] -rise_to $clk_periphery_nodes] "arrival_time"]
	set clk_min [memphy_max_in_collection [get_path -rise_from ${pll_interface}uphy_clkbuf_memphy|inclk[0] -rise_to $clk_periphery_nodes -min_path] "arrival_time"]
	...
	foreach ac_pin $ac_pins {
		...
		set ac_max  [memphy_max_in_collection [get_path -rise_from ${pll_interface}uphy_clkbuf_memphy|inclk[0] -rise_to $ac_periphery_node] "arrival_time"]
		set ac_min  [memphy_max_in_collection [get_path -rise_from ${pll_interface}uphy_clkbuf_memphy|inclk[0] -rise_to $ac_periphery_node -min_path] "arrival_time"]
		...
		set ACperiphery_min_delay [expr [memphy_min_in_collection $ACperiphery_min "arrival_time"] + [memphy_min_in_collection $ACperiphery_min2 "arrival_time"]]
		set ACperiphery_max_delay [expr [memphy_max_in_collection $ACperiphery_max "arrival_time"] + [memphy_min_in_collection $ACperiphery_max2 "arrival_time"]]
		set aiot_delay [memphy_round_3dp [expr [memphy_get_rise_aiot_delay $ac_pin] * 1e9]]
	...
	lappend ac_summary [list "  Extra common clock pessimism removal" [memphy_format_3dp [expr $setup_slack - $default_setup_slack]] [memphy_format_3dp [expr $hold_slack - $default_hold_slack]]]
...
proc memphy_perform_flexible_postamble_timing_analysis {opcs opcname instname scale_factors_name family period dll_length interface_type tJITper tJITdty tDCD DQSpathjitter pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set mem_clock_delay_max [memphy_max_in_collection [get_path -rise_from $pins(pll_write_clock) -fall_to $pins(ck_pins)] "arrival_time"]
	set mem_clock_delay_min [memphy_min_in_collection [get_path -rise_from $pins(pll_write_clock) -fall_to $pins(ck_pins) -min_path] "arrival_time"]
	set dqs_delay_max  [memphy_max_in_collection [get_path -rise_from $pins(dqs_pins) -rise_to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_delay_chain|dqsin]] "arrival_time"]
	set dqs_delay_min  [memphy_min_in_collection [get_path -rise_from $pins(dqs_pins) -rise_to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_delay_chain|dqsin] -min_path] "arrival_time"]	
...
`ifdef IPTCL_FULL_RATE
	set clock_delay_max [memphy_max_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $dqsenableextend_regs] "arrival_time"]
	set clock_delay_min [memphy_min_in_collection [get_path -rise_from $pins(pll_write_clock) -rise_to $dqsenableextend_regs -min_path] "arrival_time"]
`else
	set clock_delay_max [memphy_max_in_collection [get_path -rise_from [list $pins(pll_write_clock) $pins(pll_c2p_write_clock)] -rise_to $dqsenableextend_regs] "arrival_time"]
	set clock_delay_min [memphy_min_in_collection [get_path -rise_from [list $pins(pll_write_clock) $pins(pll_c2p_write_clock)] -rise_to $dqsenableextend_regs -min_path] "arrival_time"]
`endif

...
`endif
	...
	if {($result == 1)} {
		...
		foreach dqpins $pins(q_groups) {
			...
			set cps_name [memphy_traverse_to_clock_phase_select $dqspin msg_list]
...
proc memphy_perform_flexible_read_capture_timing_analysis {opcs opcname inst family scale_factors_name io_std interface_type max_package_skew dqs_phase period all_dq_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
	...
	if {$IP(read_deskew_mode) == "dynamic"} {
		...
		set offset_from_90 [memphy_get_acv_read_offset $period $dqs_phase $dqs_period]
		if {$IP(num_ranks) == 1} {
...
proc memphy_perform_ac_analyses {opcs opcname inst family scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name} {
	...
	foreach ck_pin $pins(ck_pins) {
		set leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $ck_pin msg_list]
...
`ifdef IPTCL_HHP_HPS
	set clk_max [memphy_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $clk_ldc_periphery_nodes] "arrival_time"]
`else
	set clk_max [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $clk_ldc_periphery_nodes] "arrival_time"]
`endif
	if {$clk_max <= 0} {set clk_max -9999}
`ifdef IPTCL_HHP_HPS
	set clk_min [memphy_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $clk_ldc_periphery_nodes -min_path] "arrival_time"]
`else
	set clk_min [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $clk_ldc_periphery_nodes -min_path] "arrival_time"]
`endif
	...
	foreach ac_pin $ac_pins {
		...
		set leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $ac_pin msg_list]
...
`ifdef IPTCL_HHP_HPS
		set ac_max  [memphy_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $ac_ldc_periphery_node] "arrival_time"]
`else
		set ac_max  [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $ac_ldc_periphery_node] "arrival_time"]
`endif
		if {$ac_max <= 0} {set ac_max -9999}
`ifdef IPTCL_HHP_HPS
		set ac_min  [memphy_max_in_collection [get_path -rise_from ${pll_clock} -rise_to $ac_ldc_periphery_node -min_path] "arrival_time"]
`else
		set ac_min  [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $ac_ldc_periphery_node -min_path] "arrival_time"]
`endif
		if {$ac_min <= 0} {set ac_min 9999}
		set extra_ccpr [memphy_round_3dp [expr {$clk_max - $clk_min + $ac_max - $ac_min}]]
...
`ifdef IPTCL_HHP_HPS
        # #####################################################################
        # ##DQS vs CK
        set panel_name_setup  "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst DQS vs CK(setup)"
	set panel_name_hold   "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst DQS vs CK (hold)"

        # ######################################################################
	# ##Default DQS vs CK
	set res_0 [report_timing -detail full_path -to [get_ports $pins(dqs_pins)] -npaths $num_failing_path -panel_name $panel_name_setup -setup -disable_panel_color -quiet]
	set res_1 [report_timing -detail full_path -to [get_ports $pins(dqs_pins)] -npaths $num_failing_path -panel_name $panel_name_hold -hold -disable_panel_color -quiet]
        
        if {$family == "arria v"} {
		set hps_phy_clock_path_compensation 0.100
	} else {
		set hps_phy_clock_path_compensation 0.200
	}
        set default_setup_slack [lindex $res_0 1]
	set default_hold_slack [lindex $res_1 1]
        set ccpr_dqs_ck [expr $clk_max - $clk_min  - $hps_phy_clock_path_compensation]
        set setup_slack [expr $default_setup_slack  + $ccpr_dqs_ck]
        set hold_slack [expr $default_hold_slack  + $ccpr_dqs_ck]	
   
        # ########################################
	# ##Create the dqs vs ck analysis panel	
	set dqs_vs_ck_summary [list]
	lappend dqs_vs_ck_summary [list "  Standard DQS vs CK" [memphy_format_3dp $default_setup_slack] [memphy_format_3dp $default_hold_slack]]
	lappend dqs_vs_ck_summary [list "  Extra common clock pessimism removal" [memphy_format_3dp [expr $setup_slack - $default_setup_slack]] [memphy_format_3dp [expr $hold_slack - $default_hold_slack]]]

	set positive_fcolour [list "black" "blue" "blue"]
	set negative_fcolour [list "black" "red"  "red"]
	set panel_name "$inst DQS vs CK"
	set root_folder_name [get_current_timequest_report_folder]
	...
	set fcolour [memphy_get_colours $setup_slack $hold_slack] 
	add_row_to_table -id $panel_id [list "DQS vs CK" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]] -fcolor $fcolour
	lappend summary [list $opcname 0 "DQS vs CK ($opcname)" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]]

	foreach summary_line $dqs_vs_ck_summary {
		add_row_to_table -id $panel_id $summary_line -fcolors $positive_fcolour
	}
`else
        # #####################################################################
...
`ifdef IPTCL_CUT_TIMING
	lappend summary [list $opcname 0 "Postamble ($opcname)" 1.000 1.000]
...
`ifdef IPTCL_HHP_HPS
	if {$family == "arria v"} {
		set hps_compensation [expr 0.130 - [memphy_max_in_collection [get_path -rise_to ${instname}|pll|pll|clk_out[0]] "arrival_time"]]
	} else {
		set hps_compensation [expr 0.350 - [memphy_max_in_collection [get_path -rise_to ${instname}|pll|pll|clk_out[0]] "arrival_time"]]
	}
`endif
...
`endif
	set dqs_delay_max  [memphy_max_in_collection [get_path -rise_from $pins(dqs_pins) -fall_to *POSTAMBLE_DFF] "arrival_time"]
	set dqs_delay_min  [memphy_min_in_collection [get_path -rise_from $pins(dqs_pins) -fall_to *POSTAMBLE_DFF -min_path] "arrival_time"]	
	set ck_pin_buffer_delay [memphy_round_3dp [expr [memphy_get_min_aiot_delay [lindex $pins(ck_pins) 0]] * 1e9]]
...
`endif
	set t11_delay_max [memphy_max_in_collection [get_path -rise_from $dqsenableextend_regs -fall_to *POSTAMBLE_DFF] "arrival_time"]
	set t11_delay_min [memphy_min_in_collection [get_path -rise_from $dqsenableextend_regs -fall_to *POSTAMBLE_DFF -min_path] "arrival_time"]

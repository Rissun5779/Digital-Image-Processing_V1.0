...
set_module_property DESCRIPTION "DDR3 SDRAM External Memory PHY"
set_module_property NAME altera_mem_if_ddr3_phy_core
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_phys_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "DDR3 SDRAM External Memory PHY"
...
proc generate_verilog_fileset {name tmpdir fileset {include_sim_vhdl_files 0}} {
	...
	set new_inhdl_dir "${qdir}/../ip/altera/alt_mem_if/alt_mem_if_phys/alt_mem_if_ddr3_phy_core"
	...
	if {[check_device_family_equivalence $device_family {"MAX10"}]} {
        	set inhdl_files_list [list \
        		...
        		reset_sync.v \
        		fr_cycle_shifter.v \
        		read_datapath_m10.sv \
        		write_datapath_m10.v \
        		simple_ddio_out_m10.sv \
        		max10emif_dcfifo.sv \
        		iss_probe.v
        	]
        } else {
	...
	if {$include_sim_vhdl_files} {
        ...
        if {[check_device_family_equivalence $device_family {"MAX10"}]} {
       		lappend inhdl_files_list "addr_cmd_pads_m10.v"
        } else {
	...
	if {[string compare -nocase [get_parameter_value ABSTRACT_REAL_COMPARE_TEST] "true"] == 0} {
		set toplevel_inhdl_files_list [list altera_mem_if_ddr3_phy_core_abs_cmp_top.sv]
	} else {
		set toplevel_inhdl_files_list [list altera_mem_if_ddr3_phy_core_top.sv]
	}
    ...
    if {([check_device_family_equivalence $device_family {"MAX10"}] != 1)} {
	...
	alt_mem_if::util::iptclgen::sub_strings_params [file join $tmpdir [lindex $parsed_toplevel_file_list 0]] [file join $tmpdir $toplevel_file_name] \
		[list VARIANT_OUTPUT_NAME $variant_name SIM_FILESET $simulation_fileset "FAST_SIM_MODEL_DEFAULT" $fast_sim_model_default TB_PROTOCOL "DDR3"]
...
proc generate_vhdl_sim {name} {
	...
	if {[check_device_family_equivalence $device_family {"MAX10"}]} {
	    ...
	    if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_altdq_dqs2]} {
	    	set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "DDR3" $tmpdir SIM_VERILOG]
	    	lappend bb_top_files "${name}_altdqdqs.v"
	    } else {
	    	set altdq_dqs_gen_files [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "DDR3" $tmpdir SIM_VHDL]
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_phy]} {
		...
		if {[check_device_family_equivalence $device_family {"MAX10"}]} {
		    set generated_files [concat $generated_files $altgpio_gen_files]
		}
...
proc generate_verilog_sim {name} {
	...
	if {[check_device_family_equivalence $device_family {"MAX10"}]} {
	    ...
	    foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "DDR3" $tmpdir SIM_VERILOG] {
	    	set file_name [file tail $generated_file]
	    	_dprint 1 "Preparing to add $generated_file as $file_name"
	    	add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name] PATH $generated_file
	    }
...
proc generate_synth {name} {
	...
	if {[check_device_family_equivalence $device_family {"MAX10"}]} {
	    ...
	    foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_altdq_dqs2 "${name}_" "DDR3" $tmpdir QUARTUS_SYNTH] {
	    	set file_name [file tail $generated_file]
	    	_dprint 1 "Preparing to add $generated_file as $file_name"
	    	add_fileset_file $file_name [::alt_mem_if::util::hwtcl_utils::get_file_type $file_name 0] PATH $generated_file
	    }
	...
	} elseif {[check_device_family_equivalence $device_family {"MAX10"}]} {
	...
	foreach sdc_file [::alt_mem_if::gen::uniphy_gen::generate_sdc_file $name "DDR3" $constraints_dir $tmpdir] {
	...
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_report_timing_files $name "DDR3" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}
	
	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_map_file $name "DDR3" [file join $common_constraints_dir memphy_common_pin_map.tcl] [file join $constraints_dir memphy_pin_map.tcl] $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}

	foreach tcl_file [::alt_mem_if::gen::uniphy_gen::generate_pin_assignments_file $name "DDR3" $constraints_dir $tmpdir] {
		set file_name [file tail $tcl_file]
		add_fileset_file $file_name OTHER PATH $tcl_file
	}
	...
	::alt_mem_if::gen::uniphy_gen::print_generation_done_message $name "DDR3"
...
proc ip_elaborate {} {
	...
	if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true" ] == 0} {
		::alt_mem_if::gen::uniphy_interfaces::pingpong_1t "phy"
	...
	if {[string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0 && [check_device_family_equivalence $device_family {"MAX10"}] != 1} {
		::alt_mem_if::gen::uniphy_interfaces::scc_manager "phy"
	} else {
	    ::alt_mem_if::gen::uniphy_interfaces::phase_detector_interface "phy"
	}
	...
	if {[check_device_family_equivalence $device_family {"MAX10"}]} {
	    add_interface_port memory mem_ck mem_ck Bidir [get_parameter_value MEM_IF_CK_WIDTH]
	    add_interface_port memory mem_ck_n mem_ck_n Bidir [get_parameter_value MEM_IF_CK_WIDTH]
	} else {
	...
	if {[string compare -nocase [get_parameter_value DUPLICATE_AC] "true"] == 0 && [string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0} {
		add_interface_port memory mem_1_a mem_1_a Output [get_parameter_value MEM_IF_ADDR_WIDTH]
		add_interface_port memory mem_1_ba mem_1_ba Output [get_parameter_value MEM_IF_BANKADDR_WIDTH]
		add_interface_port memory mem_1_ck mem_1_ck Output [get_parameter_value MEM_IF_CK_WIDTH]
		add_interface_port memory mem_1_ck_n mem_1_ck_n Output [get_parameter_value MEM_IF_CK_WIDTH]
		add_interface_port memory mem_1_cke mem_1_cke Output [get_parameter_value MEM_IF_CLK_EN_WIDTH]
		add_interface_port memory mem_1_cs_n mem_1_cs_n Output [get_parameter_value MEM_IF_CS_WIDTH]
		add_interface_port memory mem_1_ras_n mem_1_ras_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]
		add_interface_port memory mem_1_cas_n mem_1_cas_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]
		add_interface_port memory mem_1_we_n mem_1_we_n Output [get_parameter_value MEM_IF_CONTROL_WIDTH]
		add_interface_port memory mem_1_reset_n mem_1_reset_n Output 1
		add_interface_port memory mem_1_odt mem_1_odt Output [get_parameter_value MEM_IF_ODT_WIDTH]
	}
	...
	foreach port_name [get_interface_ports memory] {
		if {[regexp -nocase {mem_ac_parity|mem_err_out_n|mem_parity_error_n|mem_reset_n|mem_1_reset_n} $port_name match] == 0} {

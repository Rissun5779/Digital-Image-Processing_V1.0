...
parameter MEM_IF_NUMBER_OF_RANKS	= IPTCL_MEM_IF_NUMBER_OF_RANKS;
...
parameter AFI_CLK_EN_WIDTH			= IPTCL_AFI_CLK_EN_WIDTH;
...
parameter AFI_RRANK_WIDTH           = IPTCL_AFI_RRANK_WIDTH;
parameter AFI_WRANK_WIDTH           = IPTCL_AFI_WRANK_WIDTH;
...
parameter MR1_ODS								= IPTCL_MR1_ODS;
parameter MR1_RTT								= IPTCL_MR1_RTT;
parameter MR2_RTT_WR							= IPTCL_MR2_RTT_WR;
...
parameter MEM_IF_DM_LOWER_WIDTH     = (MEM_IF_DM_WIDTH == 1) ? 1 : (MEM_IF_DM_WIDTH / 2);
...
input   afi_cal_req;		// calibration reset request
...
input 	                              mem_err_out_n;
output                                mem_parity_error_n;
`endif
output	                              mem_reset_n;

// Duplicated AC Signals
`ifdef STRATIXV
`ifdef DUPLICATE_AC
output  [MEM_IF_ADDR_WIDTH-1:0]       mem_1_a;        // address
output  [MEM_IF_BANKADDR_WIDTH-1:0]   mem_1_ba;       // bank
output  [MEM_IF_CK_WIDTH-1:0]         mem_1_ck;       // differential address and command clock
output  [MEM_IF_CK_WIDTH-1:0]         mem_1_ck_n;
output  [MEM_IF_CLK_EN_WIDTH-1:0]     mem_1_cke;      // clock enable
output  [MEM_IF_CS_WIDTH-1:0]         mem_1_cs_n;     // chip select
output  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_ras_n;		
output  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_cas_n;		
output  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_we_n;		
output  [MEM_IF_ODT_WIDTH-1:0]        mem_1_odt;
...
wire  [AFI_RATE_RATIO-1:0]	    afi_rdata_valid_1t_abst;
wire  [AFI_RATE_RATIO-1:0]	    afi_rdata_valid_abst;
wire  				    afi_cal_success_abst = phy_cal_success;
wire  				    afi_cal_fail_abst = phy_cal_fail;

wire  [MEM_IF_ADDR_WIDTH-1:0]       mem_a_abst;
wire  [MEM_IF_BANKADDR_WIDTH-1:0]   mem_ba_abst;
...
`endif
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_ras_n_abst;		
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_cas_n_abst;		
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_we_n_abst;

// Duplicated AC Signals
`ifdef STRATIXV
`ifdef DUPLICATE_AC
wire  [MEM_IF_ADDR_WIDTH-1:0]       mem_1_a_abst;
wire  [MEM_IF_BANKADDR_WIDTH-1:0]   mem_1_ba_abst;
wire  [MEM_IF_CK_WIDTH-1:0]         mem_1_ck_abst;
wire  [MEM_IF_CLK_EN_WIDTH-1:0]     mem_1_cke_abst;
wire  [MEM_IF_CS_WIDTH-1:0]         mem_1_cs_n_abst;
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_ras_n_abst;		
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_cas_n_abst;		
wire  [MEM_IF_CONTROL_WIDTH-1:0]    mem_1_we_n_abst;
...
`ifdef HCX_COMPAT_MODE
assign pll_dqs_ena_clk = pll_mem_clk;
`else
...
`ifdef AC_PARITY
	...
	.parity_error_n(mem_parity_error_n),
`endif
	.mem_odt(mem_odt),
`ifdef STRATIXV
`ifdef DUPLICATE_AC
...
`endif
	`ifdef PINGPONGPHY_EN
		...
		.afi_rdata_en_full_1t			(afi_rdata_en_full_1t),
		.mux_sel						(mux_sel),
		.afi_rdata_valid_1t				(afi_rdata_valid_1t_abst),
	`endif
	.afi_addr(afi_addr),
	.afi_ba(afi_ba),
	.afi_cke(afi_cke),
...
`ifdef MEM_IF_DM_PINS_EN
	.mem_dm(mem_dm_abst),
`endif
	.mem_ras_n(mem_ras_n_abst),
	.mem_cas_n(mem_cas_n_abst),
	.mem_we_n(mem_we_n_abst),
	.mem_reset_n(),
	.mem_dq(mem_dq_abst),
...
`ifdef STRATIXV
`ifdef DUPLICATE_AC
	.mem_1_a(mem_1_a_abst),
	.mem_1_ba(mem_1_ba_abst),
	.mem_1_ck(mem_1_ck_abst),
	.mem_1_ck_n(),
	.mem_1_cke(mem_1_cke_abst),
	.mem_1_cs_n(mem_1_cs_n_abst),
	.mem_1_ras_n(mem_1_ras_n_abst),
	.mem_1_cas_n(mem_1_cas_n_abst),
	.mem_1_we_n(mem_1_we_n_abst),
	.mem_1_reset_n(),
	.mem_1_odt(),
`endif
`endif
...
reg [MEM_IF_DQ_WIDTH/2-1:0] dq_capture_real_upper;
reg [MEM_IF_DQ_WIDTH/2-1:0] dq_capture_abst_upper;
reg [MEM_IF_DQ_WIDTH/2-1:0] dq_capture_real_lower;
reg [MEM_IF_DQ_WIDTH/2-1:0] dq_capture_abst_lower;
`ifdef MEM_IF_DM_PINS_EN
reg [MEM_IF_DM_WIDTH/2-1:0] dm_capture_real_upper;
reg [MEM_IF_DM_WIDTH/2-1:0] dm_capture_abst_upper;
reg [MEM_IF_DM_LOWER_WIDTH-1:0] dm_capture_real_lower;
reg [MEM_IF_DM_LOWER_WIDTH-1:0] dm_capture_abst_lower;
`endif

wire mem_dqs_real_upper = mem_dqs_real[MEM_IF_DQS_WIDTH / 2];
wire mem_dqs_real_lower = mem_dqs_real[0];
wire mem_dqs_abst_upper = mem_dqs_abst[MEM_IF_DQS_WIDTH / 2];
wire mem_dqs_abst_lower = mem_dqs_abst[0];


always @(posedge mem_dqs_real_upper or negedge mem_dqs_real_upper)
begin
	dq_capture_real_upper <= mem_dq_real[MEM_IF_DQ_WIDTH-1 : (MEM_IF_DQ_WIDTH/2)];
`ifdef MEM_IF_DM_PINS_EN
	dm_capture_real_upper <= mem_dm[MEM_IF_DM_WIDTH-1 : (MEM_IF_DM_WIDTH/2)];
`endif
end

always @(posedge mem_dqs_real_lower or negedge mem_dqs_real_lower)
begin
	dq_capture_real_lower <= mem_dq_real[(MEM_IF_DQ_WIDTH/2) - 1 : 0];
`ifdef MEM_IF_DM_PINS_EN
	dm_capture_real_lower <= mem_dm[MEM_IF_DM_LOWER_WIDTH - 1 : 0];
`endif
end

always @(posedge mem_dqs_abst_upper or negedge mem_dqs_abst_upper)
begin
	dq_capture_abst_upper <= mem_dq_abst[MEM_IF_DQ_WIDTH-1 : (MEM_IF_DQ_WIDTH/2)];
`ifdef MEM_IF_DM_PINS_EN
	dm_capture_abst_upper <= mem_dm_abst[MEM_IF_DM_WIDTH-1 : (MEM_IF_DM_WIDTH/2)];
`endif
end

always @(posedge mem_dqs_abst_lower or negedge mem_dqs_abst_lower)
begin
	dq_capture_abst_lower <= mem_dq_abst[(MEM_IF_DQ_WIDTH/2) - 1 : 0];
`ifdef MEM_IF_DM_PINS_EN
	dm_capture_abst_lower <= mem_dm_abst[MEM_IF_DM_LOWER_WIDTH - 1 : 0];
`endif
end
...
assign mem_dqs_cmp_upper = filter_xz_to_zero(mem_dqs_real_upper) & filter_xz_to_zero(mem_dqs_abst_upper);
assign mem_dqs_n_cmp_upper = ~filter_xz_to_one(mem_dqs_real_upper) & ~filter_xz_to_one(mem_dqs_abst_upper); 
assign mem_dqs_cmp_lower = filter_xz_to_zero(mem_dqs_real_lower) & filter_xz_to_zero(mem_dqs_abst_lower);
assign mem_dqs_n_cmp_lower = ~filter_xz_to_one(mem_dqs_real_lower) & ~filter_xz_to_one(mem_dqs_abst_lower); 
assign mem_ck_cmp = mem_ck & mem_ck_abst;
assign mem_cs_n_cmp = & (~(~mem_cs_n & ~mem_cs_n_abst));
`ifdef STRATIXV
`ifdef DUPLICATE_AC
assign mem_1_ck_cmp = mem_1_ck & mem_1_ck_abst;
assign mem_1_cs_n_cmp = & (~(~mem_1_cs_n & ~mem_1_cs_n_abst));
`endif
`endif
...
cmp_signals #(.NAME("ADDR"), .WIDTH(MEM_IF_ADDR_WIDTH)) cmp_addr (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_a),
	 .b(mem_a_abst)
...
cmp_signals #(.NAME("BADDR"), .WIDTH(MEM_IF_BANKADDR_WIDTH)) cmp_baddr (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_ba),
	 .b(mem_ba_abst)
...
cmp_signals #(.NAME("RAS"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_ras_n (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_ras_n),
	 .b(mem_ras_n_abst)
...
cmp_signals #(.NAME("CAS"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_cas_n (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_cas_n),
	 .b(mem_cas_n_abst)
...
cmp_signals #(.NAME("WE"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_we_n (
	 .clk(mem_ck_cmp),
	 .valid(~mem_cs_n_cmp),
	 .a(mem_we_n),
	 .b(mem_we_n_abst)
...
cmp_signals #(.NAME("ADDR_1"), .WIDTH(MEM_IF_ADDR_WIDTH)) cmp_1_addr (
	 .clk(mem_1_ck_cmp),
	 .valid(~mem_1_cs_n_cmp),
	 .a(mem_1_a),
	 .b(mem_1_a_abst)
);

cmp_signals #(.NAME("BADDR_1"), .WIDTH(MEM_IF_BANKADDR_WIDTH)) cmp_1_baddr (
	 .clk(mem_1_ck_cmp),
	 .valid(~mem_1_cs_n_cmp),
	 .a(mem_1_ba),
	 .b(mem_1_ba_abst)
...
cmp_signals #(.NAME("RAS_1"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_1_ras_n (
	 .clk(mem_1_ck_cmp),
	 .valid(~mem_1_cs_n_cmp),
	 .a(mem_1_ras_n),
	 .b(mem_1_ras_n_abst)
...
cmp_signals #(.NAME("CAS_1"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_1_cas_n (
	 .clk(mem_1_ck_cmp),
	 .valid(~mem_1_cs_n_cmp),
	 .a(mem_1_cas_n),
	 .b(mem_1_cas_n_abst)
...
cmp_signals #(.NAME("WE_1"), .WIDTH(MEM_IF_CONTROL_WIDTH)) cmp_1_we_n (
	 .clk(mem_1_ck_cmp),
	 .valid(~mem_1_cs_n_cmp),
	 .a(mem_1_we_n),
	 .b(mem_1_we_n_abst)
...
`endif
`endif
cmp_signals #(.NAME("DQ_UPPER_HI"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_dq_upper_hi (
	.clk(mem_dqs_cmp_upper),
	.valid(afi_cal_success_cmp),
	.a(dq_capture_real_upper),
	.b(dq_capture_abst_upper)
	);

cmp_signals #(.NAME("DQ_UPPER_LO"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_dq_upper_lo (
	.clk(mem_dqs_n_cmp_upper),
	.valid(afi_cal_success_cmp),
	.a(dq_capture_real_upper),
	.b(dq_capture_abst_upper)
	);
...
`ifdef MEM_IF_DM_PINS_EN
cmp_signals #(.NAME("DM_UPPER_HI"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_dm_upper_hi (
	.clk(mem_dqs_cmp_upper),
	.valid(afi_cal_success_cmp),
	.a(dm_capture_real_upper),
	.b(dm_capture_abst_upper)
	);

cmp_signals #(.NAME("DM_UPPER_LO"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_dm_upper_lo (
	.clk(mem_dqs_n_cmp_upper),
	.valid(afi_cal_success_cmp),
	.a(dm_capture_real_upper),
	.b(dm_capture_abst_upper)
...
cmp_signals #(.NAME("DQ_LOWER_HI"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_dq_lower_hi (
	.clk(mem_dqs_cmp_lower),
	.valid(afi_cal_success_cmp),
	.a(dq_capture_real_lower),
	.b(dq_capture_abst_lower)
	);

cmp_signals #(.NAME("DQ_LOWER_LO"), .WIDTH(MEM_IF_DQ_WIDTH)) cmp_dq_lower_lo (
	.clk(mem_dqs_n_cmp_lower),
	.valid(afi_cal_success_cmp),
	.a(dq_capture_real_lower),
	.b(dq_capture_abst_lower)
	);
...
`ifdef MEM_IF_DM_PINS_EN
cmp_signals #(.NAME("DM_LOWER_HI"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_dm_lower_hi (
	.clk(mem_dqs_cmp_lower),
	.valid(afi_cal_success_cmp),
	.a(dm_capture_real_lower),
	.b(dm_capture_abst_lower)
	);

cmp_signals #(.NAME("DM_LOWER_LO"), .WIDTH(MEM_IF_DM_WIDTH)) cmp_dm_lower_lo (
	.clk(mem_dqs_n_cmp_lower),
	.valid(afi_cal_success_cmp),
	.a(dm_capture_real_lower),
	.b(dm_capture_abst_lower)
...
cmp_signals #(.NAME("RDATA_VALID"), .WIDTH(AFI_RATE_RATIO)) cmp_rdata_valid (
	.clk(afi_clk),
	.valid(1'b1),
	.a(afi_rdata_valid),
	.b(afi_rdata_valid_abst)
	);
cmp_signals #(.NAME("RDATA_VALID"), .WIDTH(AFI_RATE_RATIO)) cmp_rdata_valid_1t (
	.clk(afi_clk),
	.valid(1'b1),
	.a(afi_rdata_valid_1t),
	.b(afi_rdata_valid_1t_abst)
	);

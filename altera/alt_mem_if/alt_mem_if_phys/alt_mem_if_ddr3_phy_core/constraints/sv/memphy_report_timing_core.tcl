...
proc memphy_perform_ac_analyses {opcs opcname inst scale_factors_name pin_array_name timing_parameters_array_name summary_name IP_name {bus_number 0}} {
...
`ifdef IPTCL_DUPLICATE_AC
	...
	if {$bus_number == 0} {
		...
		set panel_name_setup  "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst Address Command $bus_number (setup)"
		set panel_name_hold   "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst Address Command $bus_number (hold)"
	}
	`else
	set panel_name_setup  "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst Address Command (setup)"
	set panel_name_hold   "Before Extra Common Clock Pessimism Removal \u0028Negative slacks are OK\u0029||$inst Address Command (hold)"
...
`endif
	...
	set clock_path_max [memphy_get_max_clock_path_delay_through_clock_node \
		$pins(pll_ref_clock) \
		${pll_interface}pll*~FRACTIONAL_PLL|vcoph[*] \
		${pll_clock}|divclk]

	set clock_path_min [memphy_get_min_clock_path_delay_through_clock_node \
		$pins(pll_ref_clock) \
		${pll_interface}pll*~FRACTIONAL_PLL|vcoph[*] \
		${pll_clock}|divclk]
...
`ifdef IPTCL_DUPLICATE_AC
    `ifdef IPTCL_STRATIXV
		if [
			regexp {clock_1_} ${clk_clk_phase_select_node} 
		] then {
			set clk_periphery_node ${clk_clk_phase_select_node}cps_1|clkout
		} else {
...
`endif
	...
	set clk_max [memphy_get_max_clock_path_delay_through_clock_node \
		${pll_interface}uphy_clkbuf_memphy|inclk[0] \
		${pll_interface}uphy_clkbuf_memphy|outclk[0] \
		 $clk_periphery_nodes]

	set clk_min [memphy_get_min_clock_path_delay_through_clock_node \
		${pll_interface}uphy_clkbuf_memphy|inclk[0] \
		${pll_interface}uphy_clkbuf_memphy|outclk[0] \
		 $clk_periphery_nodes]	
...
`ifdef IPTCL_DUPLICATE_AC
    `ifdef IPTCL_STRATIXV
		if [
			regexp {clock_1_} ${ac_clk_phase_select_node} 
		] then {
			set ac_periphery_node ${ac_clk_phase_select_node}cps_1|clkout
		} else {
...
`endif
		...
		set ac_max [memphy_get_max_clock_path_delay_through_clock_node \
			${pll_interface}uphy_clkbuf_memphy|inclk[0] \
			${pll_interface}uphy_clkbuf_memphy|outclk[0] \
			 $ac_periphery_node]

		set ac_min [memphy_get_min_clock_path_delay_through_clock_node \
			${pll_interface}uphy_clkbuf_memphy|inclk[0] \
			${pll_interface}uphy_clkbuf_memphy|outclk[0] \
			 $ac_periphery_node]
...
`ifdef IPTCL_DUPLICATE_AC
	...
	if {$bus_number == 0} {
		...
		set panel_name "$inst Address Command $bus_number"
	}
    `else
	set panel_name "$inst Address Command"
    `endif
...
`ifdef IPTCL_DUPLICATE_AC
	...
	if {$bus_number == 0} {
		...
		lappend summary [list $opcname 0 "Address Command $bus_number ($opcname)" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]]
	}
    `else
	lappend summary [list $opcname 0 "Address Command ($opcname)" [memphy_format_3dp $setup_slack] [memphy_format_3dp $hold_slack]]
    `endif
...
`ifdef IPTCL_NUM_WRITE_FR_CYCLE_SHIFTS
`ifdef IPTCL_FULL_RATE
	set possible_phase_increase [expr $possible_phase_increase + 360]
`else
`ifdef IPTCL_QUARTER_RATE
	set possible_phase_increase [expr $possible_phase_increase + 3*360]
`else
	set possible_phase_increase [expr $possible_phase_increase + 2*360]
`endif
`endif
`endif

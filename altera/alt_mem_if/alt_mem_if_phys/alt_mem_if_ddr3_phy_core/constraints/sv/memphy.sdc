...
`ifdef IPTCL_STRATIXV
	set ck_1_pins $pins(ck_1_pins)
	set ckn_1_pins $pins(ckn_1_pins)
	set add_1_pins $pins(add_1_pins)
	set ba_1_pins $pins(ba_1_pins)
	set cmd_1_pins $pins(cmd_1_pins)
	set reset_1_pins $pins(reset_1_pins)
	set ac_1_pins [ concat $add_1_pins $ba_1_pins $cmd_1_pins ]
`endif
`endif
...
`else
	set pll_c2p_core_clk $pll_afi_clock
	set local_pll_c2p_core_clk $local_pll_afi_clk
`endif
...
`ifdef IPTCL_STRATIXV
	...
	# This is the CK_1 clock
	foreach { ck_1_pin } $ck_1_pins {
		create_generated_clock -multiply_by 1 -invert -source $pll_write_clock -master_clock "$local_pll_write_clk" $ck_1_pin -name $ck_1_pin
		set_clock_uncertainty -to [get_clocks $ck_1_pin] 0.025
	}

	# This is the CK#_1 clock
	foreach { ckn_1_pin } $ckn_1_pins {
		create_generated_clock -multiply_by 1 -source $pll_write_clock -master_clock "$local_pll_write_clk" $ckn_1_pin -name $ckn_1_pin
		set_clock_uncertainty -to [get_clocks $ckn_1_pin] 0.025
	}
`endif
`endif
...
`ifdef IPTCL_STRATIXV
	...
	foreach { ck_1_pin } $ck_1_pins {
		# ac_pins can contain input ports such as mem_err_out_n
		# Loop through each ac pin to make sure we only apply set_output_delay to output ports
		foreach { ac_1_pin } $ac_1_pins {
			set ac_1_port [ get_ports $ac_1_pin ]
			if {[get_collection_size $ac_1_port] > 0} {
				if [ get_port_info -is_output_port $ac_1_port ] {
					# Specifies the minimum delay difference between the DQS pin and the address/control pins:
					set_output_delay -min $ac_min_delay -clock [get_clocks $ck_1_pin] $ac_1_port -add_delay

					# Specifies the maximum delay difference between the DQS pin and the address/control pins:
					set_output_delay -max $ac_max_delay -clock [get_clocks $ck_1_pin] $ac_1_port -add_delay
				}
			}
		}
	}

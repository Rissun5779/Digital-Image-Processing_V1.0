...
proc memphy_get_dqs_phase_setting { dqs_pins } {
	...
	if {$dqs_phase_setting == -1} {
		set dqs_phase_setting $::GLOBAL_dqs_delay_chain_length
...
`ifdef IPTCL_STRATIXV
	set pins(ck_1_pins) [ list ]
	set pins(ckn_1_pins) [ list ]
	set pins(add_1_pins) [ list ]
	set pins(ba_1_pins) [ list ]
	set pins(cmd_1_pins) [ list ]
	set pins(reset_1_pins) [ list ]
`endif
`endif
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	lappend patterns ck_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|clock_1_gen[*].uclk_1_generator|pseudo_diffa_0|o
	lappend patterns ckn_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|clock_1_gen[*].uclk_1_generator|pseudo_diffa_0|obar
`endif
`endif
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	lappend patterns add_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|uaddress_1_pad|${addr_cmd_postfix}
	lappend patterns reset_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ureset_1_n_pad|*fr_ddio_out|*ddio_o|dataout
	lappend patterns ba_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ubank_1_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ucs_1_n_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|uwe_1_n_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|uras_1_n_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ucas_1_n_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ucke_1_pad|${addr_cmd_postfix}
	lappend patterns cmd_1_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|uodt_1_pad|${addr_cmd_postfix}
`endif
`endif
	`ifdef IPTCL_AC_PARITY
	lappend patterns cmd_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|uac_par_pad|${addr_cmd_postfix}
	`endif
	...
	`ifdef IPTCL_AC_PARITY
	set err_out_n_capture_reg "${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|parity_error_n"
	if {[get_collection_size [get_nodes -nowarn $err_out_n_capture_reg]] != 0} {
		foreach_in_collection node [get_fanins -synch $err_out_n_capture_reg] {
			if {[get_node_info -type $node] == "port"} {
				set pin [get_node_info -name $node]
				lappend pins(cmd_pins) $pin
			}
		}
	}
	`endif	
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	set pins(ac_1_pins) [ concat $pins(add_1_pins) $pins(ba_1_pins) $pins(cmd_1_pins) $pins(reset_1_pins)]
	set pins(ac_1_wo_reset_pins) [ concat $pins(add_1_pins) $pins(ba_1_pins) $pins(cmd_1_pins)]
`endif
`endif
...
`endif
`ifdef IPTCL_GRCIB
	set pins(avl_ck_pins) ${instname}|p0|umemphy|usequencer|sequencer_rw_mgr_inst|rw_mgr_inst|cmd_exec
`endif
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	set pll_ac_clock_1 "_UNDEFINED_PIN_"
`endif
`endif
`ifdef IPTCL_EXPORT_AFI_HALF_CLK
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	set pll_ac_clock_id_1 [memphy_get_output_clock_id $pins(add_1_pins) "Address/Command 1 output" msg_list]
	if {$pll_ac_clock_id_1 == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "memphy_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(add_1_pins)]"
	} else {
		set pll_ac_clock_1 [memphy_get_pll_clock_name $pll_ac_clock_id_1]
	}
	set pins(pll_ac_clock_1) $pll_ac_clock_1
`endif
`endif
...
`ifdef IPTCL_QUARTER_RATE
	set pll_hr_clock_id [memphy_get_output_clock_id $pins(hr_ck_pins) "Half-Rate CK" msg_list]
	if {$pll_hr_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "memphy_pin_map.tcl: $msg"
		}
		post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(hr_ck_pins)]"
	} else {
		set pll_hr_clock [memphy_get_pll_clock_name $pll_hr_clock_id]
	}
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
	if { [ llength $pins(add_1_pins) ] == 0 } {
		post_message -type critical_warning "Address_1 pins of size 0"
	}
	if { [ llength $pins(cmd_1_pins) ] == 0 } {
		post_message -type critical_warning "Command_1 pins of size 0"
	}
	if { [ llength $pins(ba_1_pins) ] == 0 } {
		post_message -type critical_warning "BA_1 pins of size 0"
	}
	if { [ llength $pins(reset_1_pins) ] == 0 } {
		post_message -type critical_warning "Reset_1 pins of size 0"
	}	
`endif
`endif
}
...
`ifdef IPTCL_STRATIXV
		...
		puts $FH "CK_1: $pins(ck_1_pins)"
		puts $FH "CKn_1: $pins(ckn_1_pins)"

		puts $FH "ADD_1: $pins(add_1_pins)"
		puts $FH "CMD_1: $pins(cmd_1_pins)"
		puts $FH "RESET_1: $pins(reset_1_pins)"
		puts $FH "BA_1: $pins(ba_1_pins)"	
`endif
`endif
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
		puts $FH "PLL AC 1: $pins(pll_ac_clock_1)"
`endif
`endif
`ifdef IPTCL_EXPORT_AFI_HALF_CLK
...
proc memphy_dump_static_pin_map { ddr_db_par filename } {
	...
	foreach instname $instnames {
		...
		memphy_static_map_expand_list $FH pins reset_pins
...
`ifdef IPTCL_STRATIXV
		...
		memphy_static_map_expand_list $FH pins ck_1_pins
		memphy_static_map_expand_list $FH pins ckn_1_pins

		memphy_static_map_expand_list $FH pins add_1_pins
		memphy_static_map_expand_list $FH pins cmd_1_pins
		memphy_static_map_expand_list $FH pins reset_1_pins
		memphy_static_map_expand_list $FH pins ba_1_pins

		puts $FH ""
		puts $FH "   set pins(ac_1_pins) \[ concat \$pins(add_1_pins) \$pins(ba_1_pins) \$pins(cmd_1_pins) \$pins(reset_1_pins)\]"
`endif
`endif
...
`ifdef IPTCL_DUPLICATE_AC
`ifdef IPTCL_STRATIXV
		memphy_static_map_expand_string $FH pins pll_ac_clock_1
`endif
`endif
`ifdef IPTCL_EXPORT_AFI_HALF_CLK

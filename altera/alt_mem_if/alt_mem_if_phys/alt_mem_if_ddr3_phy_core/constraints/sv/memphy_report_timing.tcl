...
`endif
	...
	set fitter_run [memphy_get_io_interface_type [lindex [lindex $pins(q_groups) 0] 0]]
	if {$fitter_run == ""} {
	...
	foreach opc $opcs {
		...
		memphy_perform_resync_timing_analysis $opcs $opcname $inst ${::GLOBAL_corename} $family scale_factors $io_std $interface_type $period pins t summary MP IP board fpga SSN
...
`ifdef IPTCL_DUPLICATE_AC
    `ifdef IPTCL_STRATIXV
		array set pins_1 [array get pins]
		set pins_1(ck_pins) $pins(ck_1_pins)
		set pins_1(ckn_pins) $pins(ckn_1_pins)
		set pins_1(add_pins) $pins(add_1_pins)
		set pins_1(pll_ac_clock) $pins(pll_ac_clock_1)
		set pins_1(ba_pins) $pins(ba_1_pins)
		set pins_1(cmd_pins) $pins(cmd_1_pins)
		set pins_1(reset_pins) $pins(reset_1_pins)
		set pins_1(ac_pins) $pins(ac_1_pins)
		set pins_1(ac_wo_reset_pins) $pins(ac_1_wo_reset_pins)

		memphy_perform_ac_analyses  $opcs $opcname $inst scale_factors pins t summary IP 0
		memphy_perform_ac_analyses  $opcs $opcname $inst scale_factors pins_1 t summary IP 1	
    `else
...
`endif		
		...
		memphy_perform_flexible_write_levelling_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
...
`else
			...
			set dqs_div_clocks [concat [memphy_get_all_instances_dqs_pins ddr_db] [memphy_get_all_instances_div_names ddr_db]]

			foreach_in_collection path [get_timing_paths -from_clock [get_clocks $dqs_div_clocks] -to_clock [get_clocks $dqs_div_clocks] -from [get_registers *] -to [get_registers *] -setup -npaths $num_failing_path] {
				set path_from [get_node_info [get_path_info $path -from] -name]
				set path_to [get_node_info [get_path_info $path -to] -name]
				set path_slack [get_path_info $path -slack]
				set path_from_loc [get_node_info [get_path_info $path -from] -location]
			...
			foreach_in_collection path [get_timing_paths -from_clock [get_clocks $dqs_div_clocks] -to_clock [get_clocks $dqs_div_clocks] -from [get_registers *] -to [get_registers *] -hold -npaths $num_failing_path] {
				set path_from [get_node_info [get_path_info $path -from] -name]
				set path_to [get_node_info [get_path_info $path -to] -name]
				set path_slack [get_path_info $path -slack]
				set path_from_loc [get_node_info [get_path_info $path -from] -location]

...
proc memphy_perform_macro_write_launch_timing_analysis {opcs opcname inst dll_length interface_type pin_array_name timing_parameters_array_name summary_name IP_name ISI_name board_name} {
	...
	set tccs [memphy_get_tccs $IP(mem_if_memtype) $pins(dqs_pins) $t(CK) -write_deskew $IP(write_deskew_mode) -dll_length $dll_length -config_period $t(CK) -ddr3_discrete $IP(discrete_device)] 			
...
proc memphy_perform_macro_read_capture_timing_analysis {opcs opcname inst dqs_phase dll_length interface_type all_dq_pins pin_array_name timing_parameters_array_name summary_name IP_name board_name} {
	...
	# Read capture
	#Macro parameters support dynamic deskew mode only in C2
	set speedgrade [ get_speedgrade_string ]
	if { $speedgrade == "2" } {
		set tsw [memphy_get_tsw $IP(mem_if_memtype) $pins(dqs_pins) $t(CK) -read_deskew $IP(read_deskew_mode) -dll_length $dll_length -config_period $t(CK)]			
	} else {

...
`ifdef IPTCL_HARD_PHY
					set_output_delay -max [memphy_round_3dp [expr {$ac_max_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] $ac_port -add_delay
`else
...
`ifdef IPTCL_QUARTER_RATE
	if {$::GLOBAL_pll_phase(PLL_HR_CLK) >= 180} {
		set_multicycle_path -from [get_clocks $local_pll_hr_clock] -to [get_clocks $local_pll_afi_phy_clk] 2
	}

	set hr_clk_addr_clk_diff [expr $::GLOBAL_pll_phase(PLL_HR_CLK) - $::GLOBAL_pll_phase(PLL_ADDR_CMD_CLK)]
	if {($hr_clk_addr_clk_diff < 0) && ($hr_clk_addr_clk_diff > -225)} {
		set_multicycle_path -from [get_clocks $local_pll_hr_clock] -to [get_clocks $local_pll_addr_cmd_clk] 2
	}
`endif
...
`ifdef IPTCL_ARRIAV
`ifdef IPTCL_QUARTER_RATE
                 set_clock_uncertainty -from [get_clocks $local_pll_afi_clk] -to [get_clocks $local_pll_hr_clock] -add -hold 0.000
`endif

...
proc memphy_traverse_to_clkbuf {pin msg_list_name} {
	upvar 1 $msg_list_name msg_list
	set result ""
	if {$pin != ""} {
		set pin_id [get_atom_node_by_name -name $pin]
		...
		set clkbuf_id_list [memphy_traverse_atom_path $pin_id -1 $pin_to_clkbuf_path]
...
`ifdef IPTCL_USE_LDC_FOR_ADDR_CMD
	...
	`endif
	lappend patterns reset_pins ${instname}|p0|umemphy|uio_pads|uaddr_cmd_pads|ureset_n_pad|*fr_ddio_out|*ddio_o*	
`else
...
`ifdef IPTCL_HARD_PHY
	...
	if {[string compare -nocase $pins(driver_core_ck_pins) ""] != 0 && [get_collection_size [get_registers -nowarn $pins(driver_core_ck_pins)]] > 0} {
		set pll_driver_core_clock_id [memphy_get_output_clock_id $pins(driver_core_ck_pins) "DRIVER CORE CK" msg_list]
		if {$pll_driver_core_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "memphy_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(driver_core_ck_pins)]"
		} else {
			set pll_driver_core_clock [memphy_get_pll_clock_name $pll_driver_core_clock_id]
		}
	}
...
`else
	# AFI PHY CLOCK PLL
	set pll_afi_phy_clock_id [memphy_get_output_clock_id $pins(afi_phy_ck_pins) "AFI PHY CK" msg_list]
...
`else
	...
	if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
		set pll_avl_phy_clock_id [memphy_get_output_clock_id $pins(avl_phy_ck_pins) "Avalon PHY Bus CK" msg_list]
		if {$pll_avl_phy_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "memphy_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "memphy_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_phy_ck_pins)]"
		} else {
			set pll_avl_phy_clock [memphy_get_pll_clock_name $pll_avl_phy_clock_id]
		}
	}
...
`else
	...
	`ifdef IPTCL_HARD_PHY
		memphy_static_map_expand_string $FH pins pll_avl_phy_clock
	`endif
		memphy_static_map_expand_string $FH pins pll_config_clock
`endif
`ifdef IPTCL_HARD_PHY
		memphy_static_map_expand_string $FH pins pll_driver_core_clock
`endif

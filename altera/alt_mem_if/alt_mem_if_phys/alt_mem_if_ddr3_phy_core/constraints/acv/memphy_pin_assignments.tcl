...
`endif
	...
	`ifdef IPTCL_HHP_HPS
				if { ! [ string compare $::GLOBAL_io_standard "SSTL-135" ] } {
					...
					set_instance_assignment -name IO_STANDARD "$::GLOBAL_io_standard CLASS I" -to $rzq_pin -tag __$::GLOBAL_corename
				}
	`else
				set_instance_assignment -name IO_STANDARD "$::GLOBAL_io_standard" -to $rzq_pin -tag __$::GLOBAL_corename
	`endif
...
`ifdef IPTCL_NIOS_SEQUENCER
		set_instance_assignment -name D5_DELAY IPTCL_IO_DQS_OUT_RESERVE -to $dqs_pin -tag __$::GLOBAL_corename
		set_instance_assignment -name D6_DELAY 0 -to $dqs_pin -tag __$::GLOBAL_corename
`endif
`endif
    }
...
`ifdef IPTCL_HHP_HPS
      set_instance_assignment -name D5_DELAY 2 -to $ck_pin -tag __$::GLOBAL_corename
`else
	  if {$is_av_soc_device} {
        if {$is_speedgrade_i5} {
            set_instance_assignment -name D5_DELAY 5 -to $ck_pin -tag __$::GLOBAL_corename
         } elseif {$is_speedgrade_c5} {
            set_instance_assignment -name D5_DELAY 10 -to $ck_pin -tag __$::GLOBAL_corename
         } else {
            set_instance_assignment -name D5_DELAY 8 -to $ck_pin -tag __$::GLOBAL_corename
         }
	  } else {
	     set_instance_assignment -name D5_DELAY 2 -to $ck_pin -tag __$::GLOBAL_corename
	  } 
...
`endif
    ...
    foreach reset_pin $pins(reset_pins) {
`ifdef IPTCL_HHP_HPS
      set_instance_assignment -name IO_STANDARD "$::GLOBAL_io_standard CLASS I" -to $reset_pin -tag __$::GLOBAL_corename
`else
...
`endif
	...
	if { $::GLOBAL_num_pll_clock == [ expr 5 + $seq_clks + $dr_clk + $qr_related_clks ] } {
`ifdef IPTCL_HALF_RATE
...
`ifdef IPTCL_QUARTER_RATE
		set_instance_assignment -name GLOBAL_SIGNAL "GLOBAL CLOCK" -to $pll_afi_clock -tag __$::GLOBAL_corename
		set_instance_assignment -name GLOBAL_SIGNAL "GLOBAL CLOCK" -to $pll_hr_clock -tag __$::GLOBAL_corename
`endif			
		...
		post_message -type critical_warning "Expected [ expr 5 + $seq_clks + $dr_clk + $qr_related_clks ] PLL clocks but found $::GLOBAL_num_pll_clock!"
	}
`endif // IPTCL_HHP_HPS

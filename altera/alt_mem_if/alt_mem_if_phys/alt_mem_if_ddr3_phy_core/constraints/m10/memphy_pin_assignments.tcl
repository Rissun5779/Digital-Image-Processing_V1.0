...
foreach inst $instances {
	...
	if { ! [ string compare $::GLOBAL_io_standard "SSTL-135" ] } {
		...
		foreach reset_pin $pins(reset_pins) {
			...
			set_instance_assignment -name OUTPUT_TERMINATION "SERIES 40 OHM WITHOUT CALIBRATION" -to $reset_pin -tag __$::GLOBAL_corename
		} 
	...
	foreach { ckn_pin } $pins(ckn_pins) { ck_pin } $pins(ck_pins) {
		if { $ck_pin != "" } {
                        set_instance_assignment -name CKN_CK_PAIR ON -from $ckn_pin -to $ck_pin -tag __$::GLOBAL_corename
                }
        }        
	...
	foreach { dqs_pin } $pins(dqs_pins) { dm_pin } $pins(dm_pins) {
		...
		if { $dm_pin != "" } {
			set_instance_assignment -name DM_PIN ON -to $dm_pin -tag __$::GLOBAL_corename 
		}
	}
...
`endif
	...
	#set_instance_assignment -name PLL_COMPENSATION_MODE DIRECT -to "${inst}|pll0|fbout" -tag __$::GLOBAL_corename
        ...
        set_instance_assignment -name GLOBAL_SIGNAL OFF -to "dut_example_if0:if0|dut_example_if0_p0:p0|dut_example_if0_p0_memphy_m10:umemphy|dut_example_if0_p0_dqdqs_pads_m10:dq_ddio[*].ubidir_dq_dqs|altera_gpio_lite:dq_ddio_io|altgpio_one_bit:gpio_one_bit.i_loop[*].altgpio_bit_i|fr_clock"

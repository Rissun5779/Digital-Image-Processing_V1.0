...
`endif
	...
	if {($result == 1)} {
		...
		foreach dqpins $pins(d_groups) {
			...
			set dqs_periphery_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${dq_ddio_index}].ubidir_dq_dqs|altdq_dqs2_inst|phase_align_os|muxsel
			set output_buffer_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${dq_ddio_index}].ubidir_dq_dqs|altdq_dqs2_inst|obuf_os_0|i
			
			set DQSperiphery_min [get_path -from $dqs_periphery_node -to $dkpin -min_path -nworst 100]
			set DQSperiphery_delay [memphy_min_in_collection $DQSperiphery_min "arrival_time"]
			set aiot_delay [memphy_round_3dp [expr [memphy_get_min_aiot_delay $dkpin] * 1e9]]
			...
			foreach dqpin $dqdmpins {
				...
				set DQperiphery_min [get_path -from $dq_periphery_node -to $dqpin -min_path -nworst 100]
...
proc memphy_perform_macro_write_launch_timing_analysis {opcs opcname inst pin_array_name timing_parameters_array_name summary_name IP_name ISI_name board_name} {
	...
	set local_pll_write_clk [memphy_get_clock_name_from_pin_name $pins(pll_dk_clock)]
	set local_pll_mem_clk [memphy_get_clock_name_from_pin_name $pins(pll_ck_clock)]
	...
	set tccs [memphy_get_tccs $IP(mem_if_memtype) $pins(dk_pins) $t(CK)]
	...
	set su   [memphy_round_3dp [expr {$t(CK)*$dq2dqs_output_phase_offset/360.0 - $write_board_skew - [lindex $tccs 0]/1000.0 - $t(DS)}]]
	set hold [memphy_round_3dp [expr {$t(CK)*(0.5 - $dq2dqs_output_phase_offset/360.0) - $write_board_skew - [lindex $tccs 1]/1000.0 - $t(DH)}]]
...
`ifdef IPTCL_STRATIXV
			set DQScapture_node [list "${prefix}|*dq_ddio[$group_number].ubidir_dq_dqs|*input_path_gen[*].capture_reg~HIGH_DFF" \
									  "${prefix}|*dq_ddio[$group_number].ubidir_dq_dqs|*input_path_gen[*].capture_reg~FF" ]
`else
	`ifdef IPTCL_ACV_FAMILY
...
`ifdef IPTCL_STRATIXV
				set DQcapture_node [list "${prefix}|*dq_ddio[$group_number].ubidir_dq_dqs|*input_path_gen[$q_index].capture_reg~HIGH_DFF" \
										 "${prefix}|*dq_ddio[$group_number].ubidir_dq_dqs|*input_path_gen[$q_index].capture_reg~FF" ]
`else
	`ifdef IPTCL_ACV_FAMILY
...
`endif
	...
	if {$IP(read_deskew_mode) == "dynamic"} {
		...
		if {$IP(num_ranks) == 1} {
			set t1_variation [expr [min [expr $offset_from_90 + [max [expr $MP(QKQ_max)*$t(QKQ_max)] [expr -$MP(QKQ_min)*$t(QKQ_min)]] + 2*$board(intra_DQS_group_skew) + $max_package_skew + $fpga(tDQS_PSERR)] [max $max_read_deskew_setup $max_read_deskew_hold]]*2*$t1_vt_variation_percent]
		} else {
...
`ifdef IPTCL_PRINT_MACRO_TIMING
proc memphy_perform_macro_read_capture_timing_analysis {opcs opcname inst dqs_phase all_dq_pins pin_array_name timing_parameters_array_name summary_name IP_name board_name} {
	...
	# Read capture QK
	set tsw  [memphy_get_tsw $IP(mem_if_memtype) $pins(qk_pins) $t(CK)]
	set su   [memphy_round_3dp [expr {$t(CK)*$dqs_phase/360.0 - [lindex $tsw 0]/1000.0 - $t(QKQ_max) - $board_skew}]]
	set hold [memphy_round_3dp [expr {$t(CK)*($t(CKH) * $t(QKH) - $t(DCD)) + $t(QKQ_min) - $t(CK)*$dqs_phase/360.0 - [lindex $tsw 1]/1000.0 - $board_skew}]]
...
`ifdef IPTCL_ACV_FAMILY
		lappend summary [list $opcname 0 "DK vs CK ($opcname)" 1.000 1.000 1.000 1.000]	
`else

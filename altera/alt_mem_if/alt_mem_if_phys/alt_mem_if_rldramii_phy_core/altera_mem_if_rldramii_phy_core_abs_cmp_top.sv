...
(* altera_attribute = "-name IP_TOOL_NAME altera_mem_if_rldramii_phy_core; -name IP_TOOL_VERSION 18.1; -name FITTER_ADJUST_HC_SHORT_PATH_GUARDBAND 100" *)
...
`endif
	...
	mem_dk,
	mem_dk_n,
	`ifdef RESERVED_PINS_FOR_DK_GROUP
	// The DK pins must be placed in a DQ group. However, in a x9 or x18 RLDRAMII, there aren't enough pins in the group for these pins,
	// so they must be placed in an adjacent group, along with these reserved pins.
	mem_reserved_pins_for_dk_group,
	`endif
	mem_qk,
	mem_qk_n,
	mem_dq,
...
`ifdef RESERVED_PINS_FOR_DK_GROUP
output [MEM_IF_RESERVED_PINS_WIDTH-1:0] mem_reserved_pins_for_dk_group;
reg [MEM_IF_RESERVED_PINS_WIDTH-1:0] mem_reserved_reg /* synthesis syn_preserve = 1 */;
always @(posedge afi_clk or negedge global_reset_n)
begin
	if (~global_reset_n)
		mem_reserved_reg <= {MEM_IF_RESERVED_PINS_WIDTH{1'b1}};
	else
		mem_reserved_reg <= {MEM_IF_RESERVED_PINS_WIDTH{1'b0}};
end
assign mem_reserved_pins_for_dk_group = mem_reserved_reg;
`endif

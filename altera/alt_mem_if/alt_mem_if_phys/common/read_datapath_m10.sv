...
module read_datapath_m10(
    pll_afi_clk,
    reset_n_afi_clk,
    read_capture_clk_hr_dq,
    rdata_hr,
        `ifdef USE_DQS_TRACKING
    ddio_phy_dqs_tracking,
    capture_strobe_tracking,
    read_capture_clk_hr_dqs,
        `endif
    afi_rdata_en,
    afi_rdata,
    afi_rdata_valid,
...
`ifdef USE_DQS_TRACKING
input           [MEM_READ_DQS_WIDTH-1:0]    read_capture_clk_hr_dqs;
input           [MEM_READ_DQS_WIDTH-1:0]    ddio_phy_dqs_tracking;
output logic    [MEM_READ_DQS_WIDTH-1:0]    capture_strobe_tracking;
`endif
...
`ifdef USE_DQS_TRACKING
// detect rising edge of rdata valid to latch strobe tracking output
logic vfifo_output_r1;
logic rvalid_edge_detect;
always_ff @ (posedge pll_afi_clk, negedge reset_n_afi_clk)
    begin
        if (!reset_n_afi_clk)
            vfifo_output_r1 <= 0;
        else
            vfifo_output_r1 <= vfifo_output;
    end

always_comb rvalid_edge_detect <= vfifo_output && !vfifo_output_r1;

// propagate the strobe tracking output by 2 afi_clk cycles to align with the rdata and rdata_valid to allow suffuicient capture window 
generate
for (i=0; i<NUM_OF_DQDQS; i=i+1)
begin: dqs_tracking

logic ddio_phy_dqs_tracking_r1_wrclk;
logic ddio_phy_dqs_tracking_r2_wrclk;
logic ddio_phy_dqs_tracking_r1_rdclk;
logic ddio_phy_dqs_tracking_r2_rdclk;

always_ff @ (posedge read_capture_clk_hr_dqs[i], negedge reset_n_afi_clk)
    begin
        if (!reset_n_afi_clk)
            begin
                ddio_phy_dqs_tracking_r1_wrclk <= 0;
                ddio_phy_dqs_tracking_r2_wrclk <= 0;
            end
        else
            begin 
                ddio_phy_dqs_tracking_r1_wrclk <= ddio_phy_dqs_tracking[i];
                ddio_phy_dqs_tracking_r2_wrclk <= ddio_phy_dqs_tracking_r1_wrclk;
            end
    end
...
always_ff @ (posedge pll_afi_clk, negedge reset_n_afi_clk)
    begin
        if (!reset_n_afi_clk)
            begin
                ddio_phy_dqs_tracking_r1_rdclk <= 0;
                ddio_phy_dqs_tracking_r2_rdclk <= 0;
            end
        else
            begin
                ddio_phy_dqs_tracking_r1_rdclk <= ddio_phy_dqs_tracking_r2_wrclk;
                ddio_phy_dqs_tracking_r2_rdclk <= ddio_phy_dqs_tracking_r1_rdclk;
            end
    end

always_ff @ (negedge pll_afi_clk)
    begin
        if (rvalid_edge_detect)
            capture_strobe_tracking[i] <= ddio_phy_dqs_tracking_r2_rdclk;
    end

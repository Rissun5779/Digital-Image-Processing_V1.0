...
`ifdef USE_DQS_TRACKING
        read_capture_clk_hr_dqs, // Div2 HR dropback clock 
        dqs_capture_clk,
        ddio_phy_dqs_tracking,  // FR dqs out for tracking
`endif
...
`ifdef USE_DQS_TRACKING
input                           dqs_capture_clk;
output                          read_capture_clk_hr_dqs;
output logic                    ddio_phy_dqs_tracking;
`endif
...
`ifdef USE_DQS_TRACKING
wire                            dqs_tracking_hi;
wire                            dqs_tracking_lo;
logic                           ddio_phy_dqs_tracking_fr;
logic                           read_capture_clk_fr_dqs;
logic                           read_capture_clk_hr_dqs;
`endif
...
`ifdef USE_DQS_TRACKING
    .inclock                   (dqs_capture_clk),
    .fr_clock                  (read_capture_clk_fr_dqs),
    .hr_clock                  (read_capture_clk_hr_dqs),
    .invert_hr_clock           (invert_hr_clock),
    .dout                      ({dqs_tracking_hi, dqs_tracking_lo}),
    .pad_io                    (mem_dqs),
`ifdef MEM_IF_DQSN_EN
    .pad_io_b                  (mem_dqs_n),
`else        
...
`endif
    ...
    `ifdef USE_DQS_TRACKING
    always_ff @ (posedge read_capture_clk_fr_dqs) ddio_phy_dqs_tracking_fr <= dqs_tracking_lo;

    always_ff @ (posedge read_capture_clk_hr_dqs) ddio_phy_dqs_tracking <= ddio_phy_dqs_tracking_fr;
    `endif

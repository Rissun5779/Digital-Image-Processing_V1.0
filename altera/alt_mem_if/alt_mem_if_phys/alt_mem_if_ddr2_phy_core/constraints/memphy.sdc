...
`else
		create_generated_clock -multiply_by 1 -master_clock [get_clocks $local_pll_mem_clk] -source $dqs_out_clock(src) $dqs_out_clock(dst) -name $dqs_out_clock(dst)_OUT -add
`endif
`endif
`endif
...
`else
		create_generated_clock -multiply_by 1 -master_clock [get_clocks $local_pll_mem_clk] -source $dqsn_out_clock(src) $dqsn_out_clock(dst) -name $dqsn_out_clock(dst)_OUT -add
`endif
`endif
`endif
...
`else
		set_clock_groups -physically_exclusive	-group "$dqs_in_clock(dqs_pin)_IN $dqs_in_clock(div_name)" -group "$dqs_in_clock(dqs_pin)_OUT"
`endif
`endif
...
`ifdef IPTCL_READ_FIFO_HALF_RATE	
		foreach dqs_in_clock_struct $dqs_in_clocks {
			array set dqs_in_clock $dqs_in_clock_struct
			set_max_delay -from $dqs_in_clock(div_pin) -to $dqs_in_clock(div_pin) [expr $t(CK) -0.200]
...
`ifdef IPTCL_NIOS_SEQUENCER
		...
		set_min_delay -from *|avl_wdata[*] -to *datain_reg0 [expr 0 + 0.200]
...
`ifdef IPTCL_ACV_FAMILY
	...
	set_false_path -from *alt_ddrx_afi_block*
	set_false_path -to *altdq_dqs2_acv*output_path_gen[*].oe_reg
	set_false_path -to *altdq_dqs2_acv*output_*_gen[*]*
	set_false_path -to *altdq_dqs2_acv*phase_align_os*
	set_false_path -to *altdq_dqs2_acv*os_oe_reg*
	set_false_path -to *altdq_dqs2_acv*oct_oe_reg*
	set_false_path -to *afi_mux_ddrx*
	set_false_path -to *rw_manager_jumplogic*
	set_false_path -from *rw_manager_jumplogic*
	set_false_path -from *if0_p0_read_datapath*latency_shifter*
	set_false_path -from *alt_mem_ddrx_burst_gen*
	set_false_path -from *alt_mem_ddrx_rdwr_data_tmg*
	...
	set ac_pins [ concat $pins(add_pins) $pins(ba_pins) $pins(cmd_pins) ]
	foreach pin $ac_pins {
		set_multicycle_path -from *|*pll0|pll4~PLL_OUTPUT_COUNTER|divclk -to $pin -hold -end 2
	}
`ifdef IPTCL_HARD_PHY
	set_false_path -from *if0_p0_acv_hard_memphy*afi_clk_reg
	set_false_path -from *if0_p0_acv_hard_memphy*if0_p0_reset_sync*reset_reg[*]

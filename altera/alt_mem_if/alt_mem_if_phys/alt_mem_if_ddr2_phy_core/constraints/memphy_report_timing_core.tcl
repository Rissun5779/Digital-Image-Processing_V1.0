...
proc memphy_perform_flexible_write_launch_timing_analysis {opcs opcname inst family DQS_min_max interface_type max_package_skew dll_length period pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name} {
	...
	set msg_list [ list ]
	set dqs_pll_clock_id [memphy_get_output_clock_id $pins(dqs_pins) "DQS output" msg_list]
...
`ifdef IPTCL_DUAL_WRITE_CLOCK	
	set msg_list [ list ]
	set dq_pll_clock_id [memphy_get_output_clock_id [ join [ join $pins(q_groups) ]] "DQ output" msg_list]
...
`ifdef IPTCL_MEM_LEVELING	
	...
	set ref_pll_min_of_max [memphy_min_in_collection $ref_pll_paths_max "arrival_time"]
	set ref_pll_min_of_min [memphy_min_in_collection $ref_pll_paths_min "arrival_time"]	
...
`ifdef IPTCL_ACV_FAMILY
	set max_write_deskew_setup 6
	set max_write_deskew_hold  6
`else
...
`ifdef IPTCL_STRATIXV
			set msg_list [list]
			set leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $dqspin msg_list]
...
`else			
				regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
				set dqs_periphery_node ${leveling_delay_chain_name}thechain|clkin
`endif				
			} else {
				regsub {[^\.]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
...
`else
	...
	`else
			set dqs_periphery_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|phase_align_os|muxsel
	`endif
`endif	
`ifdef IPTCL_DUAL_WRITE_CLOCK
...
`else
`ifdef IPTCL_STRATIXV
			set DQSpaths_max [get_path -rise_from $dqsclksource -rise_to [get_pins -compatibility_mode $dqs_periphery_node] -nworst 1]
			set DQSpaths_min [get_path -rise_from $dqsclksource -rise_to [get_pins -compatibility_mode $dqs_periphery_node] -nworst 1 -min_path]
`else
...
`ifdef IPTCL_USE_2X_FF
			set twox_reg_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|dd_os			
			set DQSperiphery_min1 [get_path -from [get_pins -compatibility_mode $dqs_periphery_node] -to $twox_reg_node -min_path -nworst 100]
			set DQSperiphery_min2 [get_path -from $twox_reg_node -to $dqspin -min_path -nworst 100]
			set DQSperiphery_delay [expr [memphy_min_in_collection $DQSperiphery_min1 "arrival_time"] + [memphy_min_in_collection $DQSperiphery_min2 "arrival_time"]]
`else
`ifdef IPTCL_DUAL_WRITE_CLOCK
			set DQSperiphery_min [get_path -from $dqs_clk_phase_select_node -to $dqspin -min_path -nworst 100]
`else
...
`endif
			set aiot_delay [memphy_round_3dp [expr [memphy_get_min_aiot_delay $dqspin] * 1e9]]
...
`else
					regsub {[^\|]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
					set dq_periphery_node ${leveling_delay_chain_name}thechain|clkin
`endif
				} else {
					regsub {[^\.]+$} $leveling_delay_chain_name "" leveling_delay_chain_name
...
`else
					set dq_periphery_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|phase_align_os|muxsel
`endif						
				} else {
`ifdef IPTCL_MEM_LEVELING
...
`else
					set dq_periphery_node ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|output_path_gen[${dq_index}].ddio_out|muxsel
`endif
				}
`endif
`ifdef IPTCL_DUAL_WRITE_CLOCK
...
`ifdef IPTCL_DUAL_WRITE_CLOCK
				if {([expr abs($DQSmin_of_max - $DQmin_of_max)] < 0.100) && ([expr abs($DQSmax_of_min - $DQmax_of_min)] < 0.100)} {
`else
...
`ifdef IPTCL_STRATIXV
					set extra_ccpp [expr [min $extra_ccpp_max $extra_ccpp_min] + $pll_ccpp]
`else
...
`endif
				...
				if {$isdmpin == 1} {
`ifdef IPTCL_USE_2X_FF
					set twox_reg_node_dq ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|extra_output_pad_gen[0].dd_data
`endif
...
`ifdef IPTCL_USE_2X_FF
					set twox_reg_node_dq ${inst}|p0|umemphy|uio_pads|dq_ddio[${group_number}].ubidir_dq_dqs|altdq_dqs2_inst|pad_gen[${dq_index}].dd_data
`endif
...
`ifdef IPTCL_USE_2X_FF
				set DQperiphery_min1 [get_path -from [get_pins -compatibility_mode $dq_periphery_node] -to $twox_reg_node_dq -min_path -nworst 100]
				set DQperiphery_min2 [get_path -from $twox_reg_node_dq -to $dqpin -min_path -nworst 100]
				set DQperiphery_delay [expr [memphy_min_in_collection $DQperiphery_min1 "arrival_time"] + [memphy_min_in_collection $DQperiphery_min2 "arrival_time"]]
`else
`ifdef IPTCL_DUAL_WRITE_CLOCK
				set DQperiphery_min [get_path -from $dq_clk_phase_select_node -to $dqpin -min_path -nworst 100]
`else
...
`endif
				set aiot_delay [memphy_round_3dp [expr [memphy_get_min_aiot_delay $dqpin] * 1e9]]
...
`ifdef IPTCL_ACV_FAMILY
		set extra_setup_pessimism 0.300
		set extra_hold_pessimism 0.300
		set setup_slack [expr $setup_slack + $extra_setup_pessimism]
		set hold_slack [expr $hold_slack + $extra_hold_pessimism]
`endif
...
proc memphy_perform_macro_write_launch_timing_analysis {opcs opcname inst dll_length interface_type pin_array_name timing_parameters_array_name summary_name IP_name ISI_name board_name} {
...
`endif	
	...
	set tccs [memphy_get_tccs $IP(mem_if_memtype) $pins(dqs_pins) $t(CK) -dll_length $dll_length -config_period $t(CK)] 		
	...
	set su   [memphy_round_3dp [expr {$t(CK)*$dq2dqs_output_phase_offset/360.0 - $write_board_skew - [lindex $tccs 0]/1000.0 - $t(DS) - $t9_variation}]]
	set hold [memphy_round_3dp [expr {$t(CK)*(0.5 - $dq2dqs_output_phase_offset/360.0) - $write_board_skew - [lindex $tccs 1]/1000.0 - $t(DH) - $t9_variation}]]
	if {$IP(write_deskew_mode) == "dynamic"} {
...
#############################################################
`ifdef IPTCL_PRINT_FLEXIBLE_TIMING
proc memphy_perform_flexible_read_capture_timing_analysis {opcs opcname inst family DQS_min_max io_std interface_type max_package_skew dqs_phase period all_dq_pins pin_array_name timing_parameters_array_name summary_name MP_name IP_name board_name fpga_name} {
...
`ifdef IPTCL_ACV_FAMILY
			set DQScapture_node [list "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[*].capture_reg*" ]
`else
`ifdef IPTCL_STRATIXV
			set DQScapture_node [list "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[*].capture_reg~HIGH_DFF" \
			                          "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[*].capture_reg~FF" ]
`else
...
`ifdef IPTCL_ACV_FAMILY
				set DQcapture_node [list "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[${q_index}].capture_reg*" ]
`else
`ifdef IPTCL_STRATIXV
				set DQcapture_node [list "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[${q_index}].capture_reg~HIGH_DFF" \
				                         "${prefix}*dq_ddio[${group_number}].ubidir_dq_dqs|*altdq_dqs2_inst|*input_path_gen[${q_index}].capture_reg~FF" ]
`else
...
`ifdef IPTCL_PRINT_MACRO_TIMING
proc memphy_perform_macro_read_capture_timing_analysis {opcs opcname inst dqs_phase dll_length interface_type all_dq_pins pin_array_name timing_parameters_array_name summary_name IP_name board_name} {
	...
	# Read capture
	set tsw [memphy_get_tsw $IP(mem_if_memtype) $pins(dqs_pins) $t(CK) -dll_length $dll_length -config_period $t(CK)]  
	set su   [memphy_round_3dp [expr {$t(CK)*$dqs_phase/360.0 - [lindex $tsw 0]/1000.0 - $t(DQSQ) - $read_board_skew - $t1_variation}]]
	set hold [memphy_round_3dp [expr {$t(QH_time) - $dqs_phase/360.0 * $t(CK) - [lindex $tsw 1]/1000.0 - $read_board_skew - $t1_variation}]]
...
proc memphy_perform_ac_analyses {opcs opcname inst DQS_min_max pin_array_name timing_parameters_array_name summary_name IP_name} {
...
`ifdef IPTCL_STRATIXV
	...
	set paths_setup [get_timing_paths -to $ac_pins]
	foreach_in_collection path $paths_setup {
		set current_ccpr [get_path_info $path -ccpp]
	}
	...
	set clock_path_max [memphy_max_in_collection [get_path -rise_from $pins(pll_ref_clock) -rise_through [list ${pll_clock}|vco*ph[*] ${pll_interface}pll*~FRACTIONAL_PLL|refclkin] -rise_to ${pll_clock}|divclk] "arrival_time"]
	set clock_path_min [memphy_min_in_collection [get_path -rise_from $pins(pll_ref_clock) -rise_through [list ${pll_clock}|vco*ph[*] ${pll_interface}pll*~FRACTIONAL_PLL|refclkin] -rise_to ${pll_clock}|divclk -min_path] "arrival_time"]
	set clock_portion_ccpr [expr $clock_path_max - $clock_path_min]

	set output_counter_max [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to ${pll_interface}uphy_clkbuf_memphy|inclk[0]] "arrival_time"]
	set output_counter_min [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to ${pll_interface}uphy_clkbuf_memphy|inclk[0] -min_path] "arrival_time"]
	...
	foreach ck_pin $pins(ck_pins) {
		set clk_leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $ck_pin msg_list]
		regsub {^[^\:]+\:} $clk_leveling_delay_chain_name "" clk_leveling_delay_chain_name
		regsub -all {\|[^\:]+\:} $clk_leveling_delay_chain_name "|" clk_leveling_delay_chain_name
		regsub {[^\.]+$} $clk_leveling_delay_chain_name "" clk_leveling_delay_chain_name
		set clk_periphery_node ${clk_leveling_delay_chain_name}ldc|clkin
		lappend clk_periphery_nodes $clk_periphery_node
	}
	set clk_periphery_nodes [lsort -unique $clk_periphery_nodes]

	set ac_periphery_nodes [list]
	foreach ac_pin $ac_pins {
		if {![string equal "mem_reset_n" $ac_pin]} {
			set ac_leveling_delay_chain_name [memphy_traverse_to_leveling_delay_chain $ac_pin msg_list]
			regsub {^[^\:]+\:} $ac_leveling_delay_chain_name "" ac_leveling_delay_chain_name
			regsub -all {\|[^\:]+\:} $ac_leveling_delay_chain_name "|" ac_leveling_delay_chain_name
			regsub {[^\.]+$} $ac_leveling_delay_chain_name "" ac_leveling_delay_chain_name
			set ac_periphery_node ${ac_leveling_delay_chain_name}ldc|clkin
			lappend ac_periphery_nodes $ac_periphery_node
		}
	}
	set ac_periphery_nodes [lsort -unique $ac_periphery_nodes]
	
	set clk_max [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $clk_periphery_nodes] "arrival_time"]
	set ac_max  [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $ac_periphery_nodes] "arrival_time"]
	set clk_min [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $clk_periphery_nodes -min_path] "arrival_time"]
	set ac_min  [memphy_max_in_collection [get_path -rise_from ${pll_clock}|divclk -rise_to $ac_periphery_nodes -min_path] "arrival_time"]
	...
	set extra_ccpr [memphy_round_3dp [expr $total_ccpr - $current_ccpr]]
	...
	lappend ac_summary [list "  Extra common clock pessimism removal" [memphy_format_3dp $extra_ccpr] [memphy_format_3dp $extra_ccpr]]
	set setup_slack [expr $default_setup_slack + $extra_ccpr]
	set hold_slack  [expr $default_hold_slack  + $extra_ccpr]
...
`ifdef IPTCL_PRINT_FLEXIBLE_TIMING
proc memphy_perform_flexible_write_levelling_timing_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD pll_steps pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set dqs_launch_time [memphy_max_in_collection $data_hold "latch_time"]
	set clk_launch_time [memphy_min_in_collection $ac_setup "latch_time"]
...
`endif
`else
	set dqs_launch_time [expr $dqs_launch_time - $t(CK)]
`endif	
	
	# See if we can level to the first device

	# Get the min CLK delay
	set clk_min [memphy_min_in_collection [get_path -from $pins(pll_ck_clock) -to $pins(ck_pins) -min_path] "arrival_time"]

`ifdef IPTCL_STRATIXV
	# Get the max DQS delay
	set dqs_max [memphy_max_in_collection [get_path -from $pins(pll_write_clock)  -to $pins(dqs_pins)] "arrival_time"]
`else	
	...
	set dqs_max [expr [memphy_max_in_collection $dqs_path_part1 "arrival_time"] + [memphy_max_in_collection $dqs_path_part2 "arrival_time"]]
`endif	
	...
	# Get the min CLK delay
	set clk_max [memphy_max_in_collection [get_path -from $pins(pll_ck_clock) -to $pins(ck_pins)] "arrival_time"]

`ifdef IPTCL_STRATIXV
	set dqs_min [memphy_min_in_collection [get_path -from $pins(pll_write_clock) -to $pins(dqs_pins) -min_path] "arrival_time"]
`else
...
`else
	set dqs_path_part1 [get_path -from [get_pins -compatibility_mode *clk] -to $pins(dqs_pins) -min_path]
`endif	
	foreach_in_collection path $dqs_path_part1 { set tempstartnode [get_path_info $path -from] }
	set tempstart [get_node_info $tempstartnode -name]
	set dqs_path_part2 [get_path -from $pins(pll_write_clock) -to $tempstart -min_path]
	set dqs_min [expr [memphy_min_in_collection $dqs_path_part1 "arrival_time"] + [memphy_min_in_collection $dqs_path_part2 "arrival_time"]]
`endif
...
`else
	# Determine the maximum possible delay increase (and convert to phase)
	set possible_phase_increase [expr 7*0.050/$t(CK)*360]
`endif
`endif
...
`ifdef IPTCL_PRINT_MACRO_TIMING
proc memphy_perform_macro_write_levelling_timing_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD pll_steps pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set min_period [memphy_round_3dp [expr 1000.0/$mem_fmax]]
	...
	set su [memphy_round_3dp [expr $tDQSS - $min_write_leveling_inaccuracy ]]
	set hold [memphy_round_3dp [expr $tDQSS - $max_write_leveling_inaccuracy ]]
	...
	set su [memphy_round_3dp [expr $min_DQS_low - $max_write_leveling_inaccuracy - $CK_period_jitter/2 - $tDSS ]]
	set hold [memphy_round_3dp [expr $min_DQS_high - $min_write_leveling_inaccuracy - $tDSH ]]
...
proc memphy_perform_flexible_bus_turnaround_time_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD pll_steps pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	set max_dly [expr [memphy_max_in_collection $ac_hold "clock_skew"]]
	...
	set min_dly [expr [memphy_min_in_collection $ac_setup "clock_skew"]]
...
`ifdef IPTCL_QUARTER_RATE
	if {[expr $num_clocks_read_to_write % 4] != 0} {
		set num_clocks_read_to_write [expr $num_clocks_read_to_write + ($num_clocks_read_to_write % 4)]
	}
`else
...
`ifdef IPTCL_PRINT_FLEXIBLE_TIMING
proc memphy_perform_flexible_postamble_timing_analysis {opcs opcname instname family period dll_length interface_type tJITper tJITdty tDCD DQSpathjitter pin_array_name timing_parameters_array_name summary_name  MP_name IP_name SSN_name board_name ISI_parameters_name} {
	...
	# Remove the quantization error
	set quantization_step $IP(quantization_WL)
	...
	# Remove VT variations
`ifdef IPTCL_STRATIXV
	set dqsenableextend_regs [list *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_ctrl~DFFEXTENDDQSENABLE *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_ctrl~DQS_ENABLE_OUT_DFF]
`else	
	set dqsenableextend_regs [list *dq_ddio[*].ubidir_dq_dqs|*|enable_ctrl~DFFEXTENDDQSENABLE *dq_ddio[*].ubidir_dq_dqs|*|enable_ctrl~DQS_ENABLE_OUT_DFF]
`endif	
`ifdef IPTCL_ACV_FAMILY
	set clock_delay_max 0
	set clock_delay_min 0
`else
	set clock_delay_max [memphy_max_in_collection [get_path -from $pins(pll_write_clock) -to $dqsenableextend_regs] "arrival_time"]
	set clock_delay_min [memphy_min_in_collection [get_path -from $pins(pll_write_clock) -to $dqsenableextend_regs -min_path] "arrival_time"]
`endif
`ifdef IPTCL_ACV_FAMILY
...
`else
`ifdef IPTCL_STRATIXV
	set data_delay_max  [memphy_max_in_collection [get_path -from $pins(dqs_pins) -to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_delay_chain|dqsin]] "arrival_time"]
	set data_delay_min  [memphy_min_in_collection [get_path -from $pins(dqs_pins) -to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_delay_chain|dqsin] -min_path] "arrival_time"]
`else	
	set data_delay_max  [memphy_max_in_collection [get_path -from $pins(dqs_pins) -to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_block|dqsin]] "arrival_time"]
	set data_delay_min  [memphy_min_in_collection [get_path -from $pins(dqs_pins) -to [get_pins -compatibility_mode *dq_ddio[*].ubidir_dq_dqs|*|dqs_enable_block|dqsin] -min_path] "arrival_time"]
`endif
`endif
	set vt_variation_percent [get_float_table_node_delay -src {DELAYCHAIN_DUTY_CYCLE} -dst {VTVARIATION} -parameters [list IO $interface_type]]
	set t11_vt_variation_percent [get_float_table_node_delay -src {DELAYCHAIN_T1} -dst {VTVARIATION} -parameters [list IO $interface_type]]
	# Add the delay of the maximum T11 delay
	set t11_delay_max [expr $t(CK)/$dll_length]
	set vt_variation [expr $data_delay_max*$vt_variation_percent*1.5 + $t11_delay_max*$t11_vt_variation_percent*1.5]
	set vt_miscorrelation [expr $clock_delay_max*0.01]
	set setup_slack [expr $setup_slack - $vt_variation - $vt_miscorrelation]
	set hold_slack  [expr $hold_slack  - $vt_variation - $vt_miscorrelation]

...
`ifdef IPTCL_HARD_PHY
		set_output_delay -min [memphy_round_3dp [expr {$ac_min_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] [ get_ports $ac_pins ] -add_delay
`else
...
`ifdef IPTCL_HARD_PHY
		set_output_delay -max [memphy_round_3dp [expr {$ac_max_delay + $t(CK)/2}]] -clock [get_clocks $ck_pin] [ get_ports $ac_pins ] -add_delay
`else
...
`else
		if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
			set_clock_uncertainty -from [get_clocks $local_pll_avl_phy_clk] -to [get_clocks $local_pll_avl_clock] -add -hold 0.200
			set_clock_uncertainty -from [get_clocks $local_pll_avl_clock] -to [get_clocks $local_pll_avl_phy_clk] -add -hold 0.200
		}

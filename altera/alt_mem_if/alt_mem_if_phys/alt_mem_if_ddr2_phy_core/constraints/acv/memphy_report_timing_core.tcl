...
`endif
	...
	if {($IP(write_deskew_mode) == "dynamic") && ($IP(mp_calibration) == 1) && ($IP(num_ranks) == 1)} {
		...
		if {([regexp "cyclone" $family] == 1) && ($IP(num_ranks) == 2) && ($t(CK) <= 2.5)} {
			set mp_setup_slack [expr 0.02]
			set mp_hold_slack [expr 0.02]
		}
	}
`ifdef IPTCL_HHP_HPS
	set pll_ccpp 0
...
`endif
	...
	foreach dqs_pin $pins(dqs_pins) {
		...
		set dqs_clk_phase_select_node [memphy_traverse_to_clock_phase_select $dqs_pin msg_list]
...
`ifdef IPTCL_HHP_HPS
	if {$family == "arria v"} {
		set hps_compensation [expr 0.130 - [hps_sdram_p0_max_in_collection [get_path -rise_to ${instname}|pll|pll|clk_out[0]] "arrival_time"]]
	} else {
...
`ifdef IPTCL_USE_DR_CLK
	...
	set delay_gap [max 0 [expr $delay_180_ps - (3*$phase_step_ps + $t11_max_delay_pvt_min) - 0.017]]

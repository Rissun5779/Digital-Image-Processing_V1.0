...
`endif
`ifdef IPTCL_PRINT_MACRO_TIMING
			memphy_perform_macro_write_launch_timing_analysis $opcs $opcname $inst $dll_length $interface_type pins t summary IP ISI board
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		}
`endif
...
`endif
`ifdef IPTCL_PRINT_MACRO_TIMING
			memphy_perform_macro_read_capture_timing_analysis $opcs $opcname $inst $dqs_phase $dll_length $interface_type $all_dq_pins pins t summary IP board
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		}
`endif
		...
		memphy_perform_ac_analyses  $opcs $opcname $inst $DQS_min_max pins t summary IP		
...
`endif
`ifdef IPTCL_PRINT_MACRO_TIMING
			memphy_perform_macro_write_levelling_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $pll_steps pins t summary MP IP SSN board ISI
`endif			
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		}
`endif
...
`endif
`ifdef IPTCL_PRINT_FLEXIBLE_TIMING
			memphy_perform_flexible_postamble_timing_analysis $opcs $opcname $inst $family $period $dll_length $interface_type $tJITper $tJITdty $tDCD $DQSpathjitter pins t summary MP IP SSN board ISI
`endif			
`ifdef IPTCL_PRINT_FLEXIBLE_MACRO_SWITCH
		}
`endif

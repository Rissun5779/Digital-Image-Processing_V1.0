...
assign addr = {'0, addr0};
`else
// Walking '1' for second write / read
int addr_bit_count;
always_ff @(posedge clk or negedge reset_n)
begin
	if (!reset_n)
		addr_bit_count <= ADDR_WIDTH;
	else if (enable && addr0 == 0 && addr_bit_count > 0)
        addr_bit_count <= addr_bit_count - 1'b1;
end
...
always_comb
begin
    for (loc = 0; loc < ADDR_WIDTH; loc = loc + 1'b1)
    begin : afi_dm_hip_to_sip
        if (loc == addr_bit_count)
            addr_logic[loc] = addr0;
        else
            addr_logic[loc] = 1'b0;
        end
end

assign addr = (addr_bit_count > 0) ? addr_logic : {'0, addr0};
`endif

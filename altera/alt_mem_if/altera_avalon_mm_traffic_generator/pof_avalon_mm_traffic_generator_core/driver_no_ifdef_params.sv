...
parameter TG_CPORT_TYPE_PORT                        = 0;

parameter TG_USER_ADDR_ENABLED                      = 0;
parameter TG_USER_ADDR_BINARY                       = 1;
parameter TG_USER_ADDR_DEPTH                        = 300;
parameter TG_USER_ADDR_FILE                         = "user_input_addr.hex";

parameter TG_USER_DATA_ENABLED                      = 0;
parameter TG_USER_DATA_BINARY                       = 1;
parameter TG_USER_DATA_DEPTH                        = 300;
parameter TG_USER_DATA_FILE                         = "user_input_data.hex";

parameter TG_COORDINATOR_IS_ENABLED                         = 0;
parameter TG_ENABLE_READ_ONLY_COMPARE               = 0;
parameter TG_USE_SYNC_READY                         = 0;
...
// Export the individual ready signals to be sync
assign unique_ready = avl_ready;
assign unique_done = test_complete;
...
begin
	...
	begin
		...
		can_write <= traffic_gen_ready & addr_gen_ready & data_gen_ready;
		can_read <= traffic_gen_ready & ~addr_burstcount_fifo_empty & data_gen_ready;
	end
	else
	begin
		...
		can_write <= traffic_gen_ready & addr_gen_ready & data_gen_ready;
		can_read <= traffic_gen_ready & addr_gen_ready & data_gen_ready;
	end
end
...
defparam addr_gen_inst.USER_ADDR_ENABLED                     = TG_USER_ADDR_ENABLED;
defparam addr_gen_inst.USER_ADDR_BINARY                      = TG_USER_ADDR_BINARY ;
defparam addr_gen_inst.USER_ADDR_DEPTH                       = TG_USER_ADDR_DEPTH  ;
defparam addr_gen_inst.USER_ADDR_FILE                        = TG_USER_ADDR_FILE   ;


// Pseudo-random data generator
generate
if (TG_USER_DATA_ENABLED == 0)
begin : data_gen
    assign data_gen_ready = 1'b1;
...
begin
    ...
    user_input user_data_gen_inst (
	    .clk		(clk),
        .reset_n	(resync_reset_n[2]),
        .enable		(wdata_req),
        .ready		(data_gen_ready),
        .data		({wdata,be}));
    defparam user_data_gen_inst.DATA_WIDTH            = TG_AVL_DATA_WIDTH + TG_AVL_BE_WIDTH;
    defparam user_data_gen_inst.USER_INPUT_ENABLED    = TG_USER_DATA_ENABLED;
    defparam user_data_gen_inst.USER_INPUT_BINARY     = TG_USER_DATA_BINARY;
    defparam user_data_gen_inst.USER_INPUT_DEPTH      = TG_USER_DATA_DEPTH;
    defparam user_data_gen_inst.USER_INPUT_FILE       = TG_USER_DATA_FILE;
    defparam user_data_gen_inst.SHOW_AHEAD            = 1;
...
driver_fsm_no_ifdef_params driver_fsm_inst (
	.clk						(clk),
...
defparam driver_fsm_inst.TG_CPORT_TYPE_PORT		= TG_CPORT_TYPE_PORT;
defparam driver_fsm_inst.USER_ADDR_ENABLED              = TG_USER_ADDR_ENABLED;

// The Avalon traffic generator translates the commands
// issued by the state machine into Avalon signals
avalon_traffic_gen_no_ifdef_params avalon_traffic_gen_inst (
	.clk				(clk),
...
defparam avalon_traffic_gen_inst.COORDINATOR_IS_ENABLED     = TG_COORDINATOR_IS_ENABLED;
// Read compare module
//NO_IFDEF_PARAMS
read_compare_no_ifdef_params read_compare_inst (
	.clk						(clk),
...
defparam read_compare_inst.TG_USER_DATA_ENABLED     = TG_USER_DATA_ENABLED;
defparam read_compare_inst.TG_USER_DATA_BINARY      = TG_USER_DATA_BINARY;
defparam read_compare_inst.TG_USER_DATA_DEPTH       = TG_USER_DATA_DEPTH;
defparam read_compare_inst.TG_USER_DATA_FILE        = TG_USER_DATA_FILE;
defparam read_compare_inst.TG_ENABLE_READ_COMPARE   = TG_ENABLE_READ_COMPARE;
defparam read_compare_inst.TG_ENABLE_READ_ONLY_COMPARE = TG_ENABLE_READ_ONLY_COMPARE;

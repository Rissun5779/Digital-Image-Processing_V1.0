...
module driver_fsm_no_ifdef_params (
	clk,
	reset_n,
	can_write,
	can_read,
...
addr_gen_select_t				pre_addr_gen_select;
...
assign addr_gen_select = (USER_ADDR_ENABLED == 1) ? USER_ADDR_GEN : pre_addr_gen_select;
...
begin
	case (stage)
		SINGLE_RW:
		begin
			pre_addr_gen_select <= single_rw_addr_gen_select;
		...
		BLOCK_RW:
		begin
			pre_addr_gen_select <= block_rw_addr_gen_select;
		...
		TEMPLATE_STAGE:
		begin
			pre_addr_gen_select <= template_stage_addr_gen_select;
		...
		default:
		begin
			pre_addr_gen_select <= addr_gen_select.first();
			do_write <= 1'b0;
			do_read <= 1'b0;
		end
	endcase
...
//NO_IFDEF_PARAMS
single_rw_stage_no_ifdef_params single_rw_stage_inst (
.clk						(clk),
	.reset_n					(reset_n),
	.can_write					(can_write),
	.can_read					(can_read),
...
defparam single_rw_stage_inst.TG_CPORT_TYPE_PORT	= TG_CPORT_TYPE_PORT;

// Block write/read test stage
//NO_IFDEF_PARAMS
block_rw_stage_no_ifdef_params block_rw_stage_inst (
	.clk						(clk),
	.reset_n					(reset_n),
	.can_write					(can_write),
	.can_read					(can_read),
...
defparam block_rw_stage_inst.TG_CPORT_TYPE_PORT			= TG_CPORT_TYPE_PORT;
...
defparam template_stage_inst.TG_CPORT_TYPE_PORT			= TG_CPORT_TYPE_PORT;

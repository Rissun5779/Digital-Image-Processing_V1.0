...
`ifdef QDRII
`else
wire                                    user_addr_gen_enable;
wire                                    user_addr_gen_ready;
wire    [ADDR_GEN_ADDR_WIDTH-1:0]       user_addr_gen_addr;
wire    [BURSTCOUNT_WIDTH-1:0]          user_addr_gen_burstcount;
`endif
...
`else
		USER_ADDR_GEN:
		begin
			ready <= user_addr_gen_ready;
			word_addr <= user_addr_gen_addr;
			burstcount <= user_addr_gen_burstcount;
		end
`endif
	endcase
end
...
`ifdef QDRII
`else
assign user_addr_gen_enable = (addr_gen_select == USER_ADDR_GEN) & enable;
`endif
...
`ifdef QDRII
`else
// User input address generator
user_addr_gen user_addr_gen_inst (
	.clk		(clk),
	.reset_n	(reset_n),
	.enable		(user_addr_gen_enable),
	.ready		(user_addr_gen_ready),
	.addr		(user_addr_gen_addr),
	.burstcount	(user_addr_gen_burstcount));
defparam user_addr_gen_inst.ADDR_WIDTH			= ADDR_GEN_ADDR_WIDTH;
defparam user_addr_gen_inst.BURSTCOUNT_WIDTH	= BURSTCOUNT_WIDTH;
defparam user_addr_gen_inst.USER_ADDR_ENABLED   = USER_ADDR_ENABLED;
defparam user_addr_gen_inst.USER_ADDR_BINARY    = USER_ADDR_BINARY;
defparam user_addr_gen_inst.USER_ADDR_DEPTH     = USER_ADDR_DEPTH;
defparam user_addr_gen_inst.USER_ADDR_FILE      = USER_ADDR_FILE;
`endif
endmodule

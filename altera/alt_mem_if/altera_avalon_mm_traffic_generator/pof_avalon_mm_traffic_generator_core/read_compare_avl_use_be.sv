...
parameter TG_USER_DATA_FILE        = "user_input.hex";
...
reg     enable_int;
assign  enable_int = ((TG_ENABLE_READ_COMPARE == 1) || (TG_ENABLE_READ_ONLY_COMPARE == 1)) ? enable :  1'b0;
...
begin
	...
	begin
		...
		begin
			...
			begin
				if (enable_int && rdata_valid_reg && written_be[byte_num])
...
generate
if ((TG_ENABLE_READ_COMPARE == 1) || (TG_ENABLE_READ_ONLY_COMPARE == 1)) 
begin : force_error_on_compare
    iss_source #(
    	.WIDTH(1)
...
begin : expected_data_gen

// Write data FIFO
    scfifo_wrapper written_data_fifo(
	    .clk		(clk),
	    .reset_n	(reset_n),
	    .write_req	(enable_int & wdata_req_reg),
	    .read_req	(enable_int & rdata_valid),
...
begin
    ...
    assign read_compare_fifo_full = 1'b0;
    assign read_compare_fifo_empty = 1'b1;

    user_input user_data_gen_inst (
	    .clk		(clk),
        .reset_n	(reset_n),
        .enable		(enable_int & rdata_valid),
        .data		({written_data_fifo_out,written_be}));
    defparam user_data_gen_inst.DATA_WIDTH            = DATA_WIDTH + BE_WIDTH;
    ...
    defparam user_data_gen_inst.SHOW_AHEAD		      = 0;

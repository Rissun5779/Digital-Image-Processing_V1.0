...
parameter COORDINATOR_IS_ENABLED    = 0;
...
assign can_issue_avl_cmd = (COORDINATOR_IS_ENABLED == 1) ? coordinator_ready : avl_ready | (~avl_write_req & ~avl_read_req);
assign empty_traffic = fifo_empty;
...
generate
if (COORDINATOR_IS_ENABLED == 1)
begin
    wire permitted;
    assign permitted = coordinator_ready && ~fifo_empty;

    always_comb
    begin
        avl_write_req <= (permitted == 1'b1) ? fifo_write_req_out : '0;
        avl_read_req <= (permitted == 1'b1) ? fifo_read_req_out : '0;
        avl_burstbegin <= (permitted == 1'b1) ? fifo_burstbegin_out : '0;
        avl_addr <= (permitted == 1'b1) ? fifo_addr_out : '0;
        avl_size <= (permitted == 1'b1) ? fifo_size_out : '0;
        avl_wdata <= (permitted == 1'b1) ? fifo_wdata_out : '0;
        avl_be <= (permitted == 1'b1) ? fifo_be_out : '0;
    end
end
else
begin
...
begin
	...
	begin
		...
		begin
			...
			avl_be <= fifo_write_req_out ? fifo_be_out : 'b1;
		end
	end
	else
	begin

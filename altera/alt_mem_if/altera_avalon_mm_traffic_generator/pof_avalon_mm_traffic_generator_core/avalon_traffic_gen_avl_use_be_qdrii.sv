...
module avalon_traffic_gen_avl_use_be_qdrii (
	clk,
	reset_n,
	avl_ready,
	avl_ready_w,
...
scfifo_wrapper avalon_traffic_fifo_w (
	...
	.data_in	({fifo_write_req_in,fifo_addr_w_in,fifo_size_w_in,fifo_wdata_in}),
	.data_out	({fifo_write_req_out,fifo_addr_w_out,fifo_size_w_out,fifo_wdata_out}),
	.full		(fifo_w_full),
	.empty		(fifo_w_empty)
);
defparam avalon_traffic_fifo_w.DEVICE_FAMILY	= DEVICE_FAMILY;
defparam avalon_traffic_fifo_w.FIFO_WIDTH		= 1 + ADDR_WIDTH + BURSTCOUNT_WIDTH + DATA_WIDTH;
...
defparam avalon_traffic_fifo_r.FIFO_WIDTH		= 1 + ADDR_WIDTH + BURSTCOUNT_WIDTH;

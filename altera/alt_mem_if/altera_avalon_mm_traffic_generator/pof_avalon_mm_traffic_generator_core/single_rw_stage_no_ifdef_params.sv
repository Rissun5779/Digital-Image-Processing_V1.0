...
module single_rw_stage_no_ifdef_params (
	clk,
	reset_n,
	can_write,
	can_read,
		...
		case (state)
			INIT:
				...
				if (NUM_SINGLE_WRITES <= 0)
					...
					if ((TG_CPORT_TYPE_PORT == 1)||(TG_CPORT_TYPE_PORT == 3))
						state <= SINGLE_WRITE;
					else if (TG_CPORT_TYPE_PORT == 2)
						state <= SINGLE_READ;
					else
						state <= DONE;
			...
			WAIT:
			begin
				if ((read_compare_fifo_empty) || (TG_CPORT_TYPE_PORT == 2))

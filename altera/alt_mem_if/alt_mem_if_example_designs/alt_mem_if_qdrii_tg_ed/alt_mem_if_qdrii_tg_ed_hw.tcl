...
set_module_property DESCRIPTION "Altera QDR II/II+ External Memory Interface Example Design"
set_module_property NAME alt_mem_if_qdrii_tg_ed
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::example_designs_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Altera QDR II/II+ External Memory Interface Example Design"
...
proc ip_compose {} {
	...
	for {set ii 0} {$ii < $num_emif} {incr ii} {
		set EMIF "if${ii}"
		add_instance $EMIF altera_mem_if_qdrii_emif
	...
	if {[string compare -nocase [get_parameter_value PHY_ONLY] "true"] == 0} {
		...
		if {[string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] != 0} {
			::alt_mem_if::gen::uniphy_gen::instantiate_qdrii_controller "${CONTROLLER0}" "${EMIF0}.afi_clk" "${EMIF0}.afi_reset"
		} else {
			::alt_mem_if::gen::uniphy_gen::instantiate_qdrii_controller "${CONTROLLER0}" "${EMIF1}.afi_clk" "${EMIF1}.afi_reset"
		}
		...
		if {$num_emif == 2} {
			...
			if {[string compare -nocase [get_parameter_value PLL_SHARING_MODE] "master"] != 0} {
				::alt_mem_if::gen::uniphy_gen::instantiate_qdrii_controller "${CONTROLLER1}" "${EMIF1}.afi_clk" "${EMIF1}.afi_reset" 0
			} else {
				::alt_mem_if::gen::uniphy_gen::instantiate_qdrii_controller "${CONTROLLER1}" "${EMIF0}.afi_clk" "${EMIF0}.afi_reset" 0
			}
	...
	for {set ii 0} {$ii < $num_emif} {incr ii} {
		...
		set_instance_parameter $TRAFFIC_GEN TG_TWO_AVL_INTERFACES "true"
		...
		if {[string compare -nocase [get_parameter_value PHY_ONLY] "true"] == 0} {
			add_connection "${TRAFFIC_GEN}.avl_w/${CONTROLLER}.avl_w"
			add_connection "${TRAFFIC_GEN}.avl_r/${CONTROLLER}.avl_r"
		} else {
			add_connection "${TRAFFIC_GEN}.avl_w/${EMIF}.avl_w"
			add_connection "${TRAFFIC_GEN}.avl_r/${EMIF}.avl_r"
		}

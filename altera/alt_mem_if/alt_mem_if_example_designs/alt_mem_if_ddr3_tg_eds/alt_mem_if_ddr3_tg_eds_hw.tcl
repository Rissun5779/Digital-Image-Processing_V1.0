...
set_module_property DESCRIPTION "Altera DDR3 SDRAM External Memory Interface Example Design Simulation"
set_module_property NAME alt_mem_if_ddr3_tg_eds
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::example_designs_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Altera DDR3 SDRAM External Memory Interface Example Design Simulation"
...
if {[::alt_mem_if::util::qini::cfg_is_on micron_ap_mode]} {
	...
	set_parameter_property TG_NUM_DRIVER_LOOP ALLOWED_RANGES {-1:-1}
} else {
	add_parameter TG_NUM_DRIVER_LOOP integer 1
	set_parameter_property TG_NUM_DRIVER_LOOP ALLOWED_RANGES {-1:1000000}
}

set_parameter_property TG_NUM_DRIVER_LOOP DISPLAY_NAME "Number of loops through patterns"
set_parameter_property TG_NUM_DRIVER_LOOP DESCRIPTION "Specifies the number of times the driver will loop through all patterns before asserting test complete. A value of 0 specifies that the driver should infinitely loop. A value of -1 will make the driver skip all operations."
...
proc ip_compose {} {
	...
	set ED e0
	add_instance $ED alt_mem_if_ddr3_tg_ed
	...
	if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
		set num_checker_ports [expr {$num_mpfe_ports * $num_emif * 2}]
	} else {
			...
			foreach jj $mpfe_suffixes {
				if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
					for {set kk 0} {$kk < 2} {incr kk} {
						add_interface "drv_pnf${ii}${jj}_pp${kk}" conduit end
						set_interface_property "drv_pnf${ii}${jj}_pp${kk}" ENABLED true
						set_interface_property "drv_pnf${ii}${jj}_pp${kk}" export_of "${ED}.drv_pnf${ii}${jj}_pp${kk}"
					}
				} else {
	...
	if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
		set_instance_parameter $TRAFFIC_GEN_CHECKER NUM_STATUS_INTERFACES [expr {2*$num_emif}]
	} else {
		...
		foreach emif_suffix $interface_suffixes {
			foreach mpfe_suffix $mpfe_suffixes {
				for {set kk 0} {$kk < 2} {incr kk} {
					...
					add_connection "${ED}.drv_status${emif_suffix}${mpfe_suffix}_pp${kk}/${TRAFFIC_GEN_CHECKER}.drv_status${checker_suffix}"
					incr count
				}
			}

			if {$count_emif == 0} {
				add_connection "${ED}.emif_status_c0${emif_suffix}/${TRAFFIC_GEN_CHECKER}.emif_status"
				incr count_emif
				add_connection "${ED}.emif_status_c1${emif_suffix}/${TRAFFIC_GEN_CHECKER}.emif_status_${count_emif}"
			} else {
				add_connection "${ED}.emif_status_c0${emif_suffix}/${TRAFFIC_GEN_CHECKER}.emif_status_${count_emif}"
				incr count_emif
				add_connection "${ED}.emif_status_c1${emif_suffix}/${TRAFFIC_GEN_CHECKER}.emif_status_${count_emif}"
			}
			incr count_emif
	...
	set override_mem_cs_width [get_parameter_value MEM_CS_WIDTH]
	set override_mem_cke_width [get_parameter_value MEM_CLK_EN_WIDTH]
	if {!([string compare -nocase [ get_parameter_value MEM_FORMAT ] "REGISTERED" ] == 0 || [string compare -nocase [ get_parameter_value MEM_FORMAT ] "LOADREDUCED" ] == 0)} {
		set override_mem_cs_width 1
		set override_mem_cke_width 1
	}

	if {[string compare -nocase [get_parameter_value INCLUDE_MULTIRANK_BOARD_DELAY_MODEL] "true"] == 0} {
		for {set i 0} {$i < $num_emif} {incr i} {
			if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
				set PP_MEM_MODEL_BRIDGE "pp_mem_br${i}"
				add_instance $PP_MEM_MODEL_BRIDGE altera_mem_if_pp_mem_model_bridge
        	    foreach param_name [get_instance_parameters $PP_MEM_MODEL_BRIDGE] {
    		            set_instance_parameter $PP_MEM_MODEL_BRIDGE $param_name [get_parameter_value $param_name]
        		}
    	    	set_instance_parameter $PP_MEM_MODEL_BRIDGE DISABLE_CHILD_MESSAGING true
	        	
				set suffix [lindex $interface_suffixes $i]
				
				add_connection "${ED}.memory${suffix}/${PP_MEM_MODEL_BRIDGE}.mem_in"


				set num_rank [expr [get_parameter_value MEM_IF_CS_WIDTH] / [get_parameter_value MEM_IF_CS_PER_RANK] / 2]

				for {set pp 0} {$pp < 2} {incr pp} {
					set BOARD_DELAY_MODEL_BRIDGE "dly_brdg${i}_pp${pp}"
					add_instance $BOARD_DELAY_MODEL_BRIDGE altera_ddr3_board_delay_model_bridge
					foreach param_name [get_instance_parameters $BOARD_DELAY_MODEL_BRIDGE] {
						set_instance_parameter $BOARD_DELAY_MODEL_BRIDGE $param_name [get_parameter_value $param_name]
					}
					
					set_instance_parameter $BOARD_DELAY_MODEL_BRIDGE PINGPONGPHY_EN false

					set suffix [lindex $interface_suffixes $i]
					
					if { $pp == 0 } {
						add_connection "${PP_MEM_MODEL_BRIDGE}.mem_lhs/${BOARD_DELAY_MODEL_BRIDGE}.memory_in"
					} elseif { $pp == 1 } {
						add_connection "${PP_MEM_MODEL_BRIDGE}.mem_rhs/${BOARD_DELAY_MODEL_BRIDGE}.memory_in"
					}

					for {set rank_i 0} {$rank_i < $num_rank} {incr rank_i} {
						set MEM_MODEL "m${i}_pp${pp}_r${rank_i}"
						add_instance $MEM_MODEL altera_mem_if_ddr3_mem_model
						foreach param_name [get_instance_parameters $MEM_MODEL] {
							if {[string compare -nocase $param_name "MEM_NUMBER_OF_DIMMS"] == 0 ||
								[string compare -nocase $param_name "MEM_NUMBER_OF_RANKS_PER_DIMM"] == 0 ||
								[string compare -nocase $param_name "MEM_NUMBER_OF_RANKS_PER_DEVICE"] == 0 ||
								[string compare -nocase $param_name "DEVICE_DEPTH"] == 0} {
								set_instance_parameter $MEM_MODEL $param_name 1
							} elseif {[string compare -nocase $param_name "MEM_CS_WIDTH"] == 0} {
								set_instance_parameter $MEM_MODEL $param_name $override_mem_cs_width
							} elseif {[string compare -nocase $param_name "MEM_CLK_EN_WIDTH"] == 0} {
								set_instance_parameter $MEM_MODEL $param_name $override_mem_cke_width
							} else {
						...
						set_instance_parameter $MEM_MODEL PINGPONGPHY_EN false
			
						set BOARD_DELAY_MODEL "dly${i}_pp${pp}_r${rank_i}"
						add_instance $BOARD_DELAY_MODEL altera_ddr3_board_delay_model
						foreach param_name [get_instance_parameters $BOARD_DELAY_MODEL] {
							if {[string compare -nocase $param_name "SHADOW_REG_IDX"] == 0} {
								if {$rank_i < $num_rank/2} {
									set_instance_parameter $BOARD_DELAY_MODEL SHADOW_REG_IDX 0
								} else {
									set_instance_parameter $BOARD_DELAY_MODEL SHADOW_REG_IDX 1
								}
							} else {
								if {[string compare -nocase $param_name "MEM_NUMBER_OF_DIMMS"] == 0 ||
									[string compare -nocase $param_name "MEM_NUMBER_OF_RANKS_PER_DIMM"] == 0 ||
									[string compare -nocase $param_name "MEM_NUMBER_OF_RANKS_PER_DEVICE"] == 0 ||
									[string compare -nocase $param_name "DEVICE_DEPTH"] == 0} {
										set_instance_parameter $BOARD_DELAY_MODEL $param_name 1
								} elseif {[string compare -nocase $param_name "MEM_CS_WIDTH"] == 0} {
									set_instance_parameter $BOARD_DELAY_MODEL $param_name $override_mem_cs_width
								} elseif {[string compare -nocase $param_name "MEM_CLK_EN_WIDTH"] == 0} {
									set_instance_parameter $BOARD_DELAY_MODEL $param_name $override_mem_cke_width
								} else {
						...
						set_instance_parameter $BOARD_DELAY_MODEL PINGPONGPHY_EN false

						add_connection "${BOARD_DELAY_MODEL_BRIDGE}.memory_${rank_i}/${BOARD_DELAY_MODEL}.board_to_phy"
			
						add_connection "${BOARD_DELAY_MODEL}.board_to_mem/${MEM_MODEL}.memory"
					}
				} 
			} else {
				set num_rank [expr [get_parameter_value MEM_IF_CS_WIDTH] / [get_parameter_value MEM_IF_CS_PER_RANK]]
				set BOARD_DELAY_MODEL_BRIDGE "dly_brdg${i}"
				add_instance $BOARD_DELAY_MODEL_BRIDGE altera_ddr3_board_delay_model_bridge
				foreach param_name [get_instance_parameters $BOARD_DELAY_MODEL_BRIDGE] {
					set_instance_parameter $BOARD_DELAY_MODEL_BRIDGE $param_name [get_parameter_value $param_name]
				}
				...
				add_connection "${ED}.memory${suffix}/${BOARD_DELAY_MODEL_BRIDGE}.memory_in"
				
				for {set rank_i 0} {$rank_i < $num_rank} {incr rank_i} {
					set MEM_MODEL "m${i}_r${rank_i}"
					add_instance $MEM_MODEL altera_mem_if_ddr3_mem_model
					foreach param_name [get_instance_parameters $MEM_MODEL] {
						if {[string compare -nocase $param_name "MEM_NUMBER_OF_DIMMS"] == 0 ||
							[string compare -nocase $param_name "MEM_NUMBER_OF_RANKS_PER_DIMM"] == 0 ||
					...
					set BOARD_DELAY_MODEL "dly${i}_r${rank_i}"
					add_instance $BOARD_DELAY_MODEL altera_ddr3_board_delay_model
					foreach param_name [get_instance_parameters $BOARD_DELAY_MODEL] {
						if {[string compare -nocase $param_name "SHADOW_REG_IDX"] == 0} {
							if {$rank_i < $num_rank/2} {
	...
	} elseif {[string compare -nocase [get_parameter_value INCLUDE_BOARD_DELAY_MODEL] "true"] == 0} {
		for {set i 0} {$i < $num_emif} {incr i} {
			if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
				set PP_MEM_MODEL_BRIDGE "pp_mem_br${i}"
				add_instance $PP_MEM_MODEL_BRIDGE altera_mem_if_pp_mem_model_bridge
				...
				for {set pp 0} {$pp < 2} {incr pp} {
					set BOARD_DELAY_MODEL "dly${i}_pp${pp}"
					add_instance $BOARD_DELAY_MODEL altera_ddr3_board_delay_model
					foreach param_name [get_instance_parameters $BOARD_DELAY_MODEL] {
						if {[string compare -nocase $param_name "SHADOW_REG_IDX"] == 0} {
							set_instance_parameter $BOARD_DELAY_MODEL SHADOW_REG_IDX 0
					...
					if { $pp == 0 } {
						add_connection "${PP_MEM_MODEL_BRIDGE}.mem_lhs/${BOARD_DELAY_MODEL}.board_to_phy"
					} elseif { $pp == 1 } {
						add_connection "${PP_MEM_MODEL_BRIDGE}.mem_rhs/${BOARD_DELAY_MODEL}.board_to_phy"
					}
					add_connection "${BOARD_DELAY_MODEL}.board_to_mem/${MEM_MODEL}.memory"
				}
                ...
                for {set i 0} {$i < $num_emif} {incr i} {
					if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
						...
						for {set pp 0} {$pp < 2} {incr pp} {
							...
							if { $pp == 0 } {
								add_connection "${PP_MEM_MODEL_BRIDGE}.mem_lhs/${MEM_MODEL}.memory"	
							} elseif { $pp == 1 } {
								add_connection "${PP_MEM_MODEL_BRIDGE}.mem_rhs/${MEM_MODEL}.memory"	
							}
						}
	...
	if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
		set num_sbt_per_emif 2
	} else {
		set num_sbt_per_emif 1
	}
	...
	foreach ii $interface_suffixes {
			...
			if {[string compare -nocase [get_parameter_value MULTICAST_EN] "true"] == 0 ||
			    ([string compare -nocase [get_parameter_value CTL_AUTOPCH_EN] "true"] == 0 &&
			     [string compare -nocase [get_parameter_value HARD_EMIF] "true"] == 1) || 
				...
				for {set kk 0} {$kk < $num_sbt_per_emif} {incr kk} {
					if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
						set pp_suffix "_c${kk}"
					} else {
						set pp_suffix ""
					...
					set SBTERM "sbt${count}${pp_suffix}"
					add_instance $SBTERM altera_mem_if_sideband_terminator
					if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
						set_instance_parameter $SBTERM CTL_CS_WIDTH [expr {[get_parameter_value CTL_CS_WIDTH]/2}]
					} else {
					...
					if {[string compare -nocase [get_parameter_value MULTICAST_EN] "true"] == 0} {
						add_connection "${SBTERM}.avl_multicast_write/${ED}.avl_multicast_write${pp_suffix}${ii}"
					}
					if {[string compare -nocase [get_parameter_value HARD_EMIF] "true"] == 0} {
						...
						if {[string compare -nocase [get_parameter_value CTL_AUTOPCH_EN] "true"] == 0} {
							add_connection "${SBTERM}.autoprecharge_req/${ED}.autoprecharge_req${pp_suffix}${ii}"
						}
					}
					...
					if {[string compare -nocase [get_parameter_value CTL_USR_REFRESH_EN] "true"] == 0} {
						add_connection "${SBTERM}.user_refresh/${ED}.user_refresh${pp_suffix}${ii}"
					}
					...
					if {[string compare -nocase [get_parameter_value CTL_SELF_REFRESH_EN] "true"] == 0} {
						add_connection "${SBTERM}.self_refresh/${ED}.self_refresh${pp_suffix}${ii}"
					}
					...
					if {[string compare -nocase [get_parameter_value CTL_DEEP_POWERDN_EN] "true"] == 0} {
						add_connection "${SBTERM}.deep_powerdn/${ED}.deep_powerdn${pp_suffix}${ii}"
					}
				}
			}
			incr count

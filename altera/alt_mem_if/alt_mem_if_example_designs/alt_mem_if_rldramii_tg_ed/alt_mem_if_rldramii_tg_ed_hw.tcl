...
set_module_property DESCRIPTION "Altera RLDRAM II External Memory Interface Example Design"
set_module_property NAME alt_mem_if_rldramii_tg_ed
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::example_designs_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Altera RLDRAM II External Memory Interface Example Design"
...
proc ip_compose {} {
	...
	for {set ii 0} {$ii < $num_emif} {incr ii} {
		set EMIF "if${ii}"
		add_instance $EMIF altera_mem_if_rldramii_emif
		...
		if {[string compare -nocase [get_parameter_value RESERVED_PINS_FOR_DK_GROUP] "true"] == 0} {
			add_interface "reserved${suffix}" conduit end
			set_interface_property "reserved${suffix}" export_of "${EMIF}.reserved"
			if {$ii == 0} {
				::alt_mem_if::util::hwtcl_utils::rename_exported_interface_ports "${EMIF}" "reserved" "reserved${suffix}"
			}
		}
	...
	if {[string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] == 0} {
		if {[string compare -nocase [get_parameter_value EXPORT_AFI_CLK_RESET] "true"] == 0 || [string compare -nocase [get_parameter_value ENABLE_EXPORT_SEQ_DEBUG_BRIDGE] "true"] == 0} {
			...
			add_connection "${EMIF1}.afi_reset_export/afi_reset.in_reset"
		}
	...
	if {[string compare -nocase [get_parameter_value PHY_ONLY] "true"] == 0} {
		...
		if {[string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] != 0} {
			::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller "RLDRAMII" "${CONTROLLER0}" "${EMIF0}.afi_clk" "${EMIF0}.afi_reset"
		} else {
			::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller "RLDRAMII" "${CONTROLLER0}" "${EMIF1}.afi_clk" "${EMIF1}.afi_reset"
		}
		...
		if {$num_emif == 2} {
			...
			if {[string compare -nocase [get_parameter_value PLL_SHARING_MODE] "master"] != 0} {
				::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller "RLDRAMII" "${CONTROLLER1}" "${EMIF1}.afi_clk" "${EMIF1}.afi_reset" 0
			} else {
				::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller "RLDRAMII" "${CONTROLLER1}" "${EMIF0}.afi_clk" "${EMIF0}.afi_reset" 0
			}
		...
		::alt_mem_if::gen::uniphy_interfaces::export_rldramx_controller_sideband_interfaces "RLDRAMII" "${EMIF0}" {} {}
		if {$num_emif == 2} {
			::alt_mem_if::gen::uniphy_interfaces::export_controller_status_interface "${EMIF1}" "status" "emif_status_1" 0
			::alt_mem_if::gen::uniphy_interfaces::export_rldramx_controller_sideband_interfaces "RLDRAMII" "${EMIF1}" {} {_1} 0
		}
	...
	for {set ii 0} {$ii < $num_emif} {incr ii} {
		...
		set_instance_parameter $TRAFFIC_GEN TG_BYTE_ENABLE [get_parameter_value BYTE_ENABLE]
		...
		set_interface_property "drv_status${suffix}" export_of "${TRAFFIC_GEN}.status"

		if {[string compare -nocase [get_parameter_value TG_PNF_ENABLE] "true"] == 0} {
			add_interface "drv_pnf${suffix}" conduit end
			set_interface_property "drv_pnf${suffix}" export_of "${TRAFFIC_GEN}.pnf"
			if {$ii == 0} {
				::alt_mem_if::util::hwtcl_utils::rename_exported_interface_ports "${TRAFFIC_GEN}" "pnf" "drv_pnf${suffix}"
			}
		}
	}
	...
	if {[string compare -nocase [get_parameter_value USER_REFRESH] "true"] == 0} {
		...
		for {set ii 0} {$ii < $num_emif} {incr ii} {
			set EMIF "if${ii}"
			set CONTROLLER "c${ii}"
			set REFRESH_GEN "r${ii}"

			add_instance $REFRESH_GEN altera_mem_if_rld_user_refresh_gen
			
			set_instance_parameter $REFRESH_GEN CTL_BANKADDR_WIDTH [get_parameter_value CTL_BANKADDR_WIDTH]
			set_instance_parameter $REFRESH_GEN CTL_T_REFI [get_parameter_value CTL_T_REFI]
			...
			add_connection "${pll_master_instance}.afi_clk/${REFRESH_GEN}.avl_clock"
			add_connection "${pll_master_instance}.afi_reset/${REFRESH_GEN}.avl_reset"


			if {[string compare -nocase [get_parameter_value PHY_ONLY] "true"] == 0} {
				add_connection "${REFRESH_GEN}.user_refresh/${CONTROLLER}.user_refresh"
			} else {
				add_connection "${REFRESH_GEN}.user_refresh/${EMIF}.user_refresh"
			}
		}
	}

...
set_module_property DESCRIPTION "Altera DDR3 AFI Gasket"
set_module_property NAME altera_mem_if_ddr3_afi_gasket
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_afi_gasket_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Altera DDR3 AFI Gasket"
...
proc solve_core_params {} {
	...
	if {[string compare -nocase [get_parameter_value USE_DQS_TRACKING] "true"] == 0} {
		lappend core_params_list "USE_DQS_TRACKING"
	}

	if {[string compare -nocase [get_parameter_value PINGPONGPHY_EN] "true"] == 0} {
		lappend core_params_list "PINGPONGPHY_EN"
	}
...
proc create_hdl_parameters {} {
	...
	::alt_mem_if::util::hwtcl_utils::_add_parameter DDR3_RDIMM                      INTEGER 0
	set_parameter_property DDR3_RDIMM                      DERIVED true
	set_parameter_property DDR3_RDIMM                      VISIBLE false
	set_parameter_property DDR3_RDIMM                      HDL_PARAMETER true

	::alt_mem_if::util::hwtcl_utils::_add_parameter DDR3_LRDIMM                     INTEGER 0
	set_parameter_property DDR3_LRDIMM                     DERIVED true
	set_parameter_property DDR3_LRDIMM                     VISIBLE false
	set_parameter_property DDR3_LRDIMM                     HDL_PARAMETER true
...
proc ip_elaborate {} {
	...
	::alt_mem_if::gen::uniphy_interfaces::afi "DDR3" "phy" "afi_ctlr" 1 1
	_dprint 1 "adding ctlr (output) interface 'afi' to gasket"
	::alt_mem_if::gen::uniphy_interfaces::afi "DDR3" "controller" "afi" 1 1
	...
	set afi_mux_addr_width [get_parameter_value AFI_ADDR_WIDTH]
	...
	if {[get_parameter_value MEM_IF_ROW_ADDR_WIDTH] > 16} {
		set afi_mux_addr_width [expr {16 * [get_parameter_value AFI_RATE_RATIO]}]
		_dprint 1 "AFI RDIMM/LRDIMM Gasket - Overriding afi_addr port width from logical with of [get_parameter_value AFI_ADDR_WIDTH] to physical mapping of $afi_mux_addr_width" 

		add_interface_port afi afi_addr afi_addr output $afi_mux_addr_width
	}
	...
	if {[string compare -nocase [get_parameter_value USE_DQS_TRACKING] "true"] == 0} {
		...
		set_port_property afi_ctlr_seq_busy VHDL_TYPE STD_LOGIC_VECTOR
		set_port_property afi_ctlr_ctl_refresh_done VHDL_TYPE STD_LOGIC_VECTOR
		set_port_property afi_ctlr_ctl_long_idle VHDL_TYPE STD_LOGIC_VECTOR
	...
	set_parameter_value AFI_MUX_ADDR_WIDTH             $afi_mux_addr_width
	...
	if {([string compare -nocase [get_parameter_value MEM_FORMAT] "REGISTERED"] == 0)} {
		set_parameter_value DDR3_RDIMM 1
	} else {
		set_parameter_value DDR3_RDIMM 0
	}

	if {([string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0)} {
		set_parameter_value DDR3_LRDIMM 1
	} else {
		set_parameter_value DDR3_LRDIMM 0
	}
}

...
set_module_property DESCRIPTION "DDR2 SDRAM Controller with UniPHY Intel FPGA IP"
set_module_property NAME altera_mem_if_ddr2_emif
set_module_property VERSION 18.1
set_module_property SUPPORTED_DEVICE_FAMILIES {"STRATIX V" "STRATIX IV" "STRATIX III" "ARRIA II GZ" "ARRIA V" "ARRIA V GZ" "CYCLONE V" "MAX 10"}
set_module_property INTERNAL false
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP "[::alt_mem_if::util::hwtcl_utils::mem_ifs_group_name]"
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "DDR2 SDRAM Controller with UniPHY Intel FPGA IP"
...
set_module_assignment "testbench.partner.mem_model.class" "altera_mem_if_ddr2_mem_model"
...
proc ip_compose {} {
	...
	if { [string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] != 0} {
		...
		set PLL pll0
		add_instance $PLL altera_mem_if_ddr2_pll
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		add_instance $PHY altera_mem_if_fake_ddr2_phy
	} else {
		if {[string compare -nocase [get_parameter_value HARD_PHY] "true"] == 0} {
			add_instance $PHY altera_mem_if_ddr2_hard_phy_core
		} else {
			add_instance $PHY altera_mem_if_ddr2_phy_core
		}
	}
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		set AFI_MUX m0
		add_instance $AFI_MUX altera_mem_if_ddr2_afi_mux
	...
	add_instance $AFI_SPLITTER altera_mem_if_ddr2_afi_splitter
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		if {[get_parameter_value SEQ_MODE] > 0} {
			...
			set SEQ_MUX sb0
			add_instance $SEQ_MUX altera_mem_if_ddr2_seq_mux_bridge
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		set SEQ s0
		add_instance $SEQ altera_mem_if_ddr2_qseq
		...
		if {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
			...
			if {[check_device_family_equivalence $device_family {"MAX10"}]} {
			    ...
			    add_connection "${SEQ}.phase_detector/${PHY}.phase_detector"
			} else {
	...
	if {[string compare -nocase [get_parameter_value MEM_FORMAT] "REGISTERED"] == 0} {
		...
		add_instance $AFI_GASKET altera_mem_if_ddr2_afi_gasket
	...
	if {[string compare -nocase [get_parameter_value PHY_ONLY] "false"] == 0} {
		...
		set CONTROLLER c0
		::alt_mem_if::gen::uniphy_gen::instantiate_ddrx_controller "ddr2" $CONTROLLER $afi_clk_source $afi_half_clk_source $afi_reset_source

		if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		    [string compare -nocase [get_parameter_value USE_FAKE_PHY_INTERNAL] "true"] == 0} {
			if {[info exists AFI_GASKET]} {
				add_connection "${CONTROLLER}.afi/${AFI_GASKET}.afi"
			} else {
			...
			if {[info exists AFI_GASKET]} {
				...
				if {[string compare -nocase [get_parameter_value HARD_EMIF] "true"] == 0} {
					add_connection "${CONTROLLER}.hard_phy_cfg/${PHY}.hard_phy_cfg"
							...
							::alt_mem_if::gen::uniphy_gen::instantiate_efficiency_monitor "DDR2" $CONTROLLER "avl_${port_id}" "avl_${port_id}" "mp_cmd_clk_${port_id}.out_clk" "mp_cmd_reset_n_${port_id}.out_reset" 1
	...
	} elseif { [string compare -nocase [get_parameter_value PLL_SHARING_MODE] "master"] == 0} {
	...
	if {([check_device_family_equivalence $device_family {"MAX10"}] != 1)} {
	    ...
	    if {[string compare -nocase [get_parameter_value PHY_ONLY] "false"] == 0} {
	        alt_mem_if::gen::uniphy_interfaces::max10_csr_connection $CONTROLLER
	    }
	}
...
proc generate_example {name} {
	...
	alt_mem_if::gen::uniphy_gen::generate_example_design_fileset "DDR2" $name
...
add_documentation_link "Release Notes" "https://documentation.altera.com/#/link/hco1421698042087/hco1421697797999"

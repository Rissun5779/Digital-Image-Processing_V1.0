...
set_module_property DESCRIPTION "RLDRAM II Controller with UniPHY Intel FPGA IP"
set_module_property NAME altera_mem_if_rldramii_emif
set_module_property VERSION 18.1
set_module_property SUPPORTED_DEVICE_FAMILIES {"STRATIX V" "STRATIX IV" "STRATIX III" "ARRIA II GZ" "ARRIA V" "ARRIA V GZ"}
set_module_property INTERNAL false
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP "[::alt_mem_if::util::hwtcl_utils::mem_ifs_group_name]"
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "RLDRAM II Controller with UniPHY Intel FPGA IP"
...
set_module_assignment "testbench.partner.mem_model.class" "altera_mem_if_rldramii_mem_model"
...
proc ip_compose {} {
	...
	if { [string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] != 0} {
		...
		set PLL pll0
		add_instance $PLL altera_mem_if_rldramii_pll
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		if {[string compare -nocase [get_parameter_value HARD_PHY] "true"] == 0} {
			add_instance $PHY altera_mem_if_rldramii_hard_phy_core
		} else {
			add_instance $PHY altera_mem_if_rldramii_phy_core
		}
	}
	...
	if {[string compare -nocase [get_parameter_value RESERVED_PINS_FOR_DK_GROUP] "true"] == 0} {
		...
		set_interface_property reserved export_of "${PHY}.reserved"
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		set AFI_MUX m0
		add_instance $AFI_MUX altera_mem_if_rldramii_afi_mux
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		if {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
			add_instance $SEQ altera_mem_if_rldramii_qseq
		} elseif {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "RTL"] == 0} {
			add_instance $SEQ altera_mem_if_rldramii_rseq
		} else {
	...
	if {[string compare -nocase [get_parameter_value PHY_ONLY] "false"] == 0} {
		...
		set CONTROLLER c0
		::alt_mem_if::gen::uniphy_gen::instantiate_rldramx_controller "RLDRAMII" $CONTROLLER $afi_clk_source $afi_reset_source
		...
		::alt_mem_if::gen::uniphy_interfaces::export_rldramx_controller_sideband_interfaces "RLDRAMII" $CONTROLLER
...
proc generate_example {name} {
	...
	alt_mem_if::gen::uniphy_gen::generate_example_design_fileset "RLDRAMII" $name
...
add_documentation_link "Release Notes" "https://documentation.altera.com/#/link/hco1421698042087/hco1421697928551"

...
set_module_property DESCRIPTION "QDR II and QDR II+ SRAM Controller with UniPHY Intel FPGA IP"
set_module_property NAME altera_mem_if_qdrii_emif
set_module_property VERSION 18.1
set_module_property SUPPORTED_DEVICE_FAMILIES {"STRATIX V" "STRATIX IV" "STRATIX III" "ARRIA II GZ" "ARRIA II GX" "ARRIA V" "ARRIA V GZ"}
set_module_property INTERNAL false
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP "[::alt_mem_if::util::hwtcl_utils::mem_ifs_group_name]"
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "QDR II and QDR II+ SRAM Controller with UniPHY Intel FPGA IP"
...
set_module_assignment "testbench.partner.mem_model.class" "altera_mem_if_qdrii_mem_model"
...
alt_mem_if::gui::qdrii_controller::create_gui
...
proc ip_compose {} {
	...
	set num_afi_clk_outputs [expr $num_afi_clk_outputs + $seq_debug_num_clks] 
	set num_afi_reset_outputs [expr $num_afi_reset_outputs + $seq_debug_num_resets] 
	...
	if { [string compare -nocase [get_parameter_value PLL_SHARING_MODE] "slave"] != 0} {
		...
		set PLL pll0
		add_instance $PLL altera_mem_if_qdrii_pll
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		_error "There is no fake PHY support for RLDRAMII"
	} else {
		if {[string compare -nocase [get_parameter_value HARD_PHY] "true"] == 0} {
			add_instance $PHY altera_mem_if_qdrii_hard_phy_core
		} else {
			add_instance $PHY altera_mem_if_qdrii_phy_core
		}
	}
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		set AFI_MUX m0
		add_instance $AFI_MUX altera_mem_if_qdrii_afi_mux
	...
	if {[string compare -nocase [get_parameter_value USE_FAKE_PHY] "true"] == 0 ||
		...
		if {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "NIOS"] == 0} {
			add_instance $SEQ altera_mem_if_qdrii_qseq
		} elseif {[string compare -nocase [get_parameter_value SEQUENCER_TYPE] "RTL"] == 0} {
			add_instance $SEQ altera_mem_if_qdrii_rseq
		} else {
	...
	if {[string compare -nocase [get_parameter_value PHY_ONLY] "false"] == 0} {
		...
		set CONTROLLER c0
		::alt_mem_if::gen::uniphy_gen::instantiate_qdrii_controller $CONTROLLER $afi_clk_source $afi_reset_source
	...
	if { [string compare -nocase [get_parameter_value DLL_SHARING_MODE] "none"] == 0} {
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_rtl_calib]} {
			if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 && [string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] != 0} { 
				add_connection "${DLL}.rtl_dll_sharing/${PHY}.rtl_dll_sharing"

				add_interface rtl_dll_reconfig conduit end
				set_interface_property rtl_dll_reconfig export_of "${DLL}.rtl_dll_reconfig"
				alt_mem_if::util::hwtcl_utils::rename_exported_interface_ports $DLL "rtl_dll_reconfig" "rtl_dll_reconfig"
			}
		}
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_rtl_calib]} {
			if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 && [string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] != 0} { 
				add_connection "${DLL}.rtl_dll_sharing/${DLL_BRIDGE}.rtl_dll_sharing_in"
				add_connection "${DLL_BRIDGE}.rtl_dll_sharing_x/${PHY}.rtl_dll_sharing"

				add_interface rtl_dll_sharing conduit end
				set_interface_property rtl_dll_sharing export_of "${DLL}.rtl_dll_sharing"
				alt_mem_if::util::hwtcl_utils::rename_exported_interface_ports $DLL "rtl_dll_sharing" "rtl_dll_sharing"
		...
		if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_rtl_calib]} {
			if {[string compare -nocase [get_parameter_value PARSE_FRIENDLY_DEVICE_FAMILY] "STRATIXV"] == 0 && [string compare [get_parameter_value SEQUENCER_TYPE] "NIOS"] != 0} { 
				add_interface rtl_dll_sharing conduit end
				set_interface_property rtl_dll_sharing export_of "${PHY}.rtl_dll_sharing"
				alt_mem_if::util::hwtcl_utils::rename_exported_interface_ports $PHY "rtl_dll_sharing" "rtl_dll_sharing"
			}
		}
	}
...
proc generate_example {name} {
	...
	alt_mem_if::gen::uniphy_gen::generate_example_design_fileset "QDRII" $name

}


add_documentation_link "User Guide" "https://www.altera.com/content/dam/altera-www/global/en_US/pdfs/literature/hb/external-memory/emi.pdf"
add_documentation_link "Release Notes" "https://documentation.altera.com/#/link/hco1421698042087/hco1421697899587"

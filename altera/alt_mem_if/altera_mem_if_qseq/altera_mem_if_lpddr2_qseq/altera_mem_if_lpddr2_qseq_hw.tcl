...
set_module_property DESCRIPTION "LPDDR2 SDRAM Qsys Sequencer"
set_module_property NAME altera_mem_if_lpddr2_qseq
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_sequencers_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "LPDDR2 SDRAM Qsys Sequencer"
...
proc generate_vhdl_sim {name} {
	...
	if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_use_verilog_sequencer]} {
		set seq_gen_files [alt_mem_if::gen::uniphy_gen::generate_sequencer_files $name "LPDDR2" $tmpdir SIM_VERILOG]
	} else {
		set seq_gen_files [alt_mem_if::gen::uniphy_gen::generate_sequencer_files $name "LPDDR2" $tmpdir SIM_VHDL]
	}
	...
	foreach generated_file $seq_gen_files {
		...
		if {[string compare -nocase [file tail $file_dirname] "mentor"] == 0} {
			...
			_dprint 1 "Preparing to add $generated_file of type $file_type"
			if {[string compare -nocase $file_type "VHDL"] == 0} {
				...
				add_fileset_file $file_name $file_type PATH $generated_file
			} elseif {([string compare -nocase $file_type "SYSTEM_VERILOG"] == 0) || ([string compare -nocase $file_type "VERILOG"] == 0)} {
				add_fileset_file $file_name $file_type PATH $generated_file
			} else {
				set device_family [::alt_mem_if::gui::system_info::get_device_family]
				if {[check_device_family_equivalence $device_family {"MAX10"}]} {
			                if {[string compare -nocase $file_name "rw_manager_m10_ac_ROM.v"] == 0} {
						_dprint 1 "Preparing to add $generated_file"
						add_fileset_file $file_name $file_type PATH $generated_file
					} elseif {[string compare -nocase $file_name "rw_manager_m10_inst_ROM.v"] == 0} {
						_dprint 1 "Preparing to add $generated_file"
						add_fileset_file $file_name $file_type PATH $generated_file
					} else {
...
proc generate_verilog_sim {name} {
	...
	foreach generated_file [alt_mem_if::gen::uniphy_gen::generate_sequencer_files $name "LPDDR2" $tmpdir SIM_VERILOG] {
		set file_name [file tail $generated_file]
...
proc ip_elaborate {} {
	...
	if {[string compare -nocase [get_parameter_value HARD_PHY] "false"] == 0} {
		...
		::alt_mem_if::gen::uniphy_interfaces::phy_manager "LPDDR2" "sequencer" 1 "disable_fr_shift_calibration"
		...
		::alt_mem_if::gen::uniphy_interfaces::afi "LPDDR2" "controller" "afi" 0 0
	...
	::alt_mem_if::gen::uniphy_interfaces::tracking_interface "phy"

...
set_module_property DESCRIPTION "Altera DDR3 Board Delay Model Bridge"
set_module_property NAME altera_ddr3_board_delay_model_bridge
set_module_property VERSION 18.1
::alt_mem_if::util::hwtcl_utils::set_module_internal_mode
set_module_property OPAQUE_ADDRESS_MAP true
set_module_property GROUP [::alt_mem_if::util::hwtcl_utils::memory_phys_group_name]
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Altera DDR3 Board Delay Model Bridge"
...
proc generate_verilog_sim {name} {
        ...
        set generated_file [alt_mem_if::util::iptclgen::generate_outfile_name "altera_ddr3_board_delay_model_bridge_top.sv" [solve_core_params]]
...
proc create_hdl_parameters {} {
        ...
        _dprint 1 "Defining HDL parameters for altera_ddr3_board_delay_model_bridge"
...
proc create_memory_interface {type {suffix {}}} {
        ...
        if {[string compare -nocase $type "output"] == 0} {
            if {([string compare -nocase [get_parameter_value MEM_FORMAT] "REGISTERED"] == 0 || [string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0)} {
                set denom [get_parameter_value MEM_NUMBER_OF_DIMMS]
            } else {
                set denom [expr [get_parameter_value MEM_IF_CS_WIDTH] / [get_parameter_value MEM_IF_CS_PER_RANK]]
            }
        }
        ...
        add_interface_port memory_${suffix} memory_${suffix}_mem_ck mem_ck $type [get_parameter_value MEM_IF_CK_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_ck_n mem_ck_n $type [get_parameter_value MEM_IF_CK_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_cke mem_cke $type [expr [get_parameter_value MEM_IF_CLK_EN_WIDTH] / $denom]
        ...
        add_interface_port memory_${suffix} memory_${suffix}_mem_ras_n mem_ras_n $type [get_parameter_value MEM_IF_CONTROL_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_cas_n mem_cas_n $type [get_parameter_value MEM_IF_CONTROL_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_we_n mem_we_n $type [get_parameter_value MEM_IF_CONTROL_WIDTH]
        ...
        add_interface_port memory_${suffix} memory_${suffix}_mem_dqs mem_dqs Bidir [get_parameter_value MEM_IF_DQS_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_dqs_n mem_dqs_n Bidir [get_parameter_value MEM_IF_DQS_WIDTH]
        add_interface_port memory_${suffix} memory_${suffix}_mem_odt mem_odt $type [expr [get_parameter_value MEM_IF_ODT_WIDTH] / $denom]

        if {([string compare -nocase [get_parameter_value MEM_FORMAT] "REGISTERED"] == 0 || [string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0)} {
                if {[string compare -nocase [get_parameter_value AC_PARITY] "true"] == 0} {
                        add_interface_port memory_${suffix} memory_${suffix}_mem_ac_parity mem_ac_parity $type 1
                        add_interface_port memory_${suffix} memory_${suffix}_mem_err_out_n mem_err_out_n [expr {[string compare -nocase $type "input"] == 0 ? "Output" : "Input"}] 1 
                        add_interface_port memory_${suffix} memory_${suffix}_mem_parity_error_n mem_parity_error_n $type 1
                }
        }
}
...
proc ip_elaborate {} {
	...
	if {([string compare -nocase [get_parameter_value MEM_FORMAT] "REGISTERED"] == 0 || [string compare -nocase [get_parameter_value MEM_FORMAT] "LOADREDUCED"] == 0)} {
		set num_rank  [expr [get_parameter_value MEM_NUMBER_OF_DIMMS] * [get_parameter_value MEM_NUMBER_OF_RANKS_PER_DIMM]]
	} else {
	...
	for {set ii 0} {$ii < $num_rank} {incr ii} {
        ...
        set_fileset_property [string tolower SIM_VERILOG] TOP_LEVEL \
            [alt_mem_if::util::iptclgen::generate_outfile_name "altera_ddr3_board_delay_model_bridge_top.sv" [solve_core_params] 1]

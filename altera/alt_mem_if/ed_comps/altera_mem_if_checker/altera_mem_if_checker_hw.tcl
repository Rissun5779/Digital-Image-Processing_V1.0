...
if {[::alt_mem_if::util::qini::cfg_is_on alt_mem_if_fake_cal_fail] == 1} {
   add_parameter CONTINUE_AFTER_CAL_FAIL BOOLEAN true
} else {
   add_parameter CONTINUE_AFTER_CAL_FAIL BOOLEAN false
}
set_parameter_property CONTINUE_AFTER_CAL_FAIL DISPLAY_NAME "Continue simulation even after calibration failure"
set_parameter_property CONTINUE_AFTER_CAL_FAIL DESCRIPTION "Use this parameter to allow the simulation to continue even if calibration fails"
set_parameter_property CONTINUE_AFTER_CAL_FAIL HDL_PARAMETER true

...
module alt_mem_if_ddr3_mem_model_top_ac_parity_ddr3_duplicate_ac_mem_if_dm_pins_en_mem_if_dqsn_en_rdimm 
...
input	[MEM_IF_ADDR_WIDTH - 1:0]	mem_1_a;
input	[MEM_IF_BANKADDR_WIDTH - 1:0]	mem_1_ba;
input	[MEM_IF_CK_WIDTH - 1:0] mem_1_ck;
input	[MEM_IF_CK_WIDTH - 1:0] mem_1_ck_n;
input	[MEM_IF_CLK_EN_WIDTH - 1:0] mem_1_cke;
input	[MEM_IF_CS_WIDTH - 1:0] mem_1_cs_n;
input	[MEM_IF_CONTROL_WIDTH - 1:0] mem_1_ras_n;
input	[MEM_IF_CONTROL_WIDTH - 1:0] mem_1_cas_n;
input	[MEM_IF_CONTROL_WIDTH - 1:0] mem_1_we_n;
input 	[MEM_IF_ODT_WIDTH - 1:0] mem_1_odt;
input	mem_1_reset_n;

generate
always @ (posedge mem_ck or posedge mem_ck_n)
begin
	if (mem_a !== mem_1_a
		|| mem_ba !== mem_1_ba
		|| mem_ck !== mem_1_ck
		...
		|| mem_cke !== mem_1_cke
		|| mem_cs_n !== mem_1_cs_n
		|| mem_ras_n !== mem_1_ras_n
		|| mem_cas_n !== mem_1_cas_n
		|| mem_we_n !== mem_1_we_n
		|| mem_odt !== mem_1_odt
		|| mem_reset_n !== mem_1_reset_n
	)
	begin
		$display("Duplicated (mem_1) AC signals do not match original (mem) AC signals");
		$finish;
	end
end
endgenerate
...
begin : depth_gen
	...
	begin : width_gen
			...
			alt_mem_if_common_ddr_mem_model_ac_parity_ddr3_duplicate_ac_mem_if_dm_pins_en_mem_if_dqsn_en_rdimm #(
			...
			.mem_parity_error_n(mem_parity_error_n),
			.mem_odt	(mem_odt[0])
		);
	end
end

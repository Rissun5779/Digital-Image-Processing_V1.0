...
module alt_mem_if_common_ddr_mem_model_lpddr2_max10_mem_if_dm_pins_en_mem_if_dqsn_en  
    # (parameter	 
        MEM_CLK_EN_WIDTH = 1,
        MEM_IF_BA_WIDTH = 2,
...
reg [9:0] 	mem_ca_pos, mem_ca_neg;

typedef enum logic[5:0] {
	OPCODE_MRW = 6'b100000,
	OPCODE_REFRESH = 6'b100011,
	OPCODE_ACTIVATE = 6'b1001xx,
	OPCODE_WRITE = 6'b10100x,
	OPCODE_READ = 6'b10101x,
	OPCODE_PRECHARGE = 6'b101101,
	OPCODE_BST = 6'b101100,
	OPCODE_DPD = 6'b00110x,	
	OPCODE_ERROR = 6'b10xxxx,
	OPCODE_NOP = 6'b10111x
} OPCODE_TYPE;
...
int tDQSCK_div = TIMING_TDQSCK/5;
...
typedef struct {
	...
	bit [MEM_IF_ROW_ADDR_WIDTH - 1:0] address;
	bit [5:0] opcode;
} command_struct;
...
bit [MEM_IF_COL_ADDR_WIDTH - 1:0] write_address_queue[$];
...
bit [MEM_IF_COL_ADDR_WIDTH - 1:0] read_address_queue[$];
...
bit [MEM_IF_ROW_ADDR_WIDTH-1:0] activate_row_queue[$];
...
task automatic set_cas_latency (input bit [3:0] code);
	case(code)
                ...
                4'b0010 : cas_latency = 4;
                4'b0011 : cas_latency = 5;
                4'b0100 : cas_latency = 6;
                4'b0101 : cas_latency = 7;
...
function automatic int get_write_latency;
	int write_latency;
	case(cas_latency)
	4'd3 : 	write_latency = 1;
	4'd4 : 	write_latency = 2;
	4'd5 : 	write_latency = 2;
	4'd6 : 	write_latency = 3;
	4'd7 : 	write_latency = 4;
	4'd8 : 	write_latency = 4;
	endcase	
...
endtask
    ...
    task automatic set_burst_type (input bit [2:0] burst_mode);
        case (burst_mode)
            ...
            3'b100 : begin
                    if (MEM_VERBOSE)
                        $display("   Setting burst length Fixed BL16");
                    burst_type = DDR_BURST_TYPE_BL16;
                    end         
            default : begin
...
task automatic cmd_bst;
	if (MEM_VERBOSE)
		$display("[%0t] [DWR=%0d%0d%0d]:  BST Command", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, MEM_RANK_IDX);
endtask

task automatic cmd_dpd;
	if (MEM_VERBOSE)
		$display("[%0t] [DWR=%0d%0d%0d]:  DPD Command", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, MEM_RANK_IDX);
endtask
...
task automatic cmd_activate(bit [MEM_IF_BA_WIDTH-1:0] bank, bit [MEM_IF_ROW_ADDR_WIDTH-1:0] address);
	if (MEM_VERBOSE)
...
task automatic cmd_mrw;
	if (MEM_VERBOSE)
		$display("[%0t] [DWR=%0d%0d%0d]:  MRW Command - MRW [ %0d ] -> %0h", $time, MEM_DEPTH_IDX, MEM_WIDTH_IDX, MEM_RANK_IDX, mode_reg, new_command.address);
	case(mode_reg)
		8'h01 : set_burst_type(new_command.address[2:0]); 
		8'h02 : set_cas_latency(new_command.address[3:0]);
	endcase
endtask
...
assign mem_ba_wire = mem_ca_pos[9:7];
...
initial begin
	...
	$display("Altera Generic LPDDR2 Memory Model");
	if (MEM_VERBOSE) begin
...
always @ (posedge mem_ck_diff or negedge mem_ck_diff) begin
	...
	bst_command_pipeline = bst_command_pipeline >> 1;
        ...
        if(mem_ck_diff && clock_stable) begin
                ...
                DDR_CMD_TYPE_ACTIVATE : begin
                	new_command.bank = mem_ca_pos[9:7];
                	new_command.address = {mem_ca_neg[9:8],mem_ca_pos[6:2],mem_ca_neg[7:0]};
                	cmd_set_activate();
                	end
                DDR_CMD_TYPE_PRECHARGE : begin 
                	new_command.bank = mem_ca_pos[9:7];
			cmd_precharge(new_command.bank, mem_ca_pos[4]);
                	end
                DDR_CMD_TYPE_WRITE : begin
                	new_command.bank = mem_ca_pos[9:7];
                	new_command.address = {mem_ca_neg[9:1],mem_ca_pos[6:5],1'b0};
                	cmd_write();
                	if(CHECK_VIOLATIONS)
                		check_violations();
                	end
                ...
                DDR_CMD_TYPE_MRW : begin
			mode_reg = {mem_ca_neg[1:0],mem_ca_pos[9:4]};
			new_command.address = {mem_ca_neg[9:2]};
			cmd_mrw();
			end
		DDR_CMD_TYPE_BST : cmd_bst();
		DDR_CMD_TYPE_DPD : cmd_dpd();
                DDR_CMD_TYPE_REFRESH : cmd_refresh();
                default: cmd_nop();
                endcase
            
        	new_command.word_count = 0;
        	mem_ca_pos[9:0] = mem_ca[9:0];
              	new_command.opcode = {mem_cke,mem_cs_n,mem_ca[0],mem_ca[1],mem_ca[2],mem_ca[3]};
		...
		OPCODE_MRW : new_command.cmd_type = DDR_CMD_TYPE_MRW;
		OPCODE_REFRESH : new_command.cmd_type = DDR_CMD_TYPE_REFRESH;
		OPCODE_BST : new_command.cmd_type = DDR_CMD_TYPE_BST;
		OPCODE_DPD : new_command.cmd_type = DDR_CMD_TYPE_DPD;
                OPCODE_NOP : new_command.cmd_type = DDR_CMD_TYPE_NOP;
                OPCODE_ERROR : new_command.cmd_type = DDR_CMD_TYPE_ERROR;
		default: new_command.cmd_type = DDR_CMD_TYPE_NOP;	
            endcase
            	                

            case (burst_type)
            	DDR_BURST_TYPE_BL16 : new_command.burst_length = 16;
        ...
        if (!mem_ck_diff && clock_stable && !mem_cs_n) begin
        	mem_ca_neg[9:0] = mem_ca[9:0];
        end
  ...
  if (new_command.cmd_type == DDR_CMD_TYPE_BST) begin
	bst_clock_cycle = clock_cycle + 1;        
	if (last_all_read_cmd_cycle > last_all_write_cmd_cycle) begin
		compare_latency = bst_clock_cycle - last_all_read_cmd_cycle;
		if (compare_latency > get_read_latency()) begin
			active_command.burst_length = 2 * compare_latency;
		end
		else begin
			if (mem_ck_diff) begin
			bst_command_pipeline[2 * (get_read_latency() - compare_latency)] = 1;
			compare_read_latency.push_back(compare_latency);
			end
			if (read_command_pipeline[0]) begin
				if (bst_command_pipeline[0]==0) begin
		...
		compare_latency = bst_clock_cycle - last_all_write_cmd_cycle;
		if (compare_latency > get_write_latency()) begin
			active_command.burst_length = 2 * compare_latency;
		end
		else begin
			if (mem_ck_diff) begin
			bst_command_pipeline[2 * (get_write_latency() - compare_latency + 1)] = 1;
			compare_write_latency.push_back(compare_latency);
			end
			if (write_command_pipeline[0]) begin
				if (bst_command_pipeline[0]==0) begin
...
wire [MEM_DQ_WIDTH - 1:0] mem_dq_int_delay1, mem_dq_int_delay2, mem_dq_int_delay3, mem_dq_int_delay4, mem_dq_int_delay5; 
assign #tDQSCK_div mem_dq_int_delay1 = mem_dq_int;
assign #tDQSCK_div mem_dq_int_delay2 = mem_dq_int_delay1;
assign #tDQSCK_div mem_dq_int_delay3 = mem_dq_int_delay2;
assign #tDQSCK_div mem_dq_int_delay4 = mem_dq_int_delay3;
assign #tDQSCK_div mem_dq_int_delay5 = mem_dq_int_delay4;

wire mem_dq_en_delay1, mem_dq_en_delay2, mem_dq_en_delay3, mem_dq_en_delay4, mem_dq_en_delay5; 
assign #tDQSCK_div mem_dq_en_delay1 = mem_dq_en;
assign #tDQSCK_div mem_dq_en_delay2 = mem_dq_en_delay1;
assign #tDQSCK_div mem_dq_en_delay3 = mem_dq_en_delay2;
assign #tDQSCK_div mem_dq_en_delay4 = mem_dq_en_delay3;
assign #tDQSCK_div mem_dq_en_delay5 = mem_dq_en_delay4;

wire mem_dqs_en_delay1, mem_dqs_en_delay2, mem_dqs_en_delay3, mem_dqs_en_delay4, mem_dqs_en_delay5; 
assign #tDQSCK_div mem_dqs_en_delay1 = mem_dqs_en;
assign #tDQSCK_div mem_dqs_en_delay2 = mem_dqs_en_delay1;
assign #tDQSCK_div mem_dqs_en_delay3 = mem_dqs_en_delay2;
assign #tDQSCK_div mem_dqs_en_delay4 = mem_dqs_en_delay3;
assign #tDQSCK_div mem_dqs_en_delay5 = mem_dqs_en_delay4;

wire mem_dqs_preamble_delay1, mem_dqs_preamble_delay2, mem_dqs_preamble_delay3, mem_dqs_preamble_delay4, mem_dqs_preamble_delay5; 
assign #tDQSCK_div mem_dqs_preamble_delay1 = mem_dqs_preamble;
assign #tDQSCK_div mem_dqs_preamble_delay2 = mem_dqs_preamble_delay1;
assign #tDQSCK_div mem_dqs_preamble_delay3 = mem_dqs_preamble_delay2;
assign #tDQSCK_div mem_dqs_preamble_delay4 = mem_dqs_preamble_delay3;
assign #tDQSCK_div mem_dqs_preamble_delay5 = mem_dqs_preamble_delay4;

wire mem_ck_diff_delay1, mem_ck_diff_delay2, mem_ck_diff_delay3, mem_ck_diff_delay4, mem_ck_diff_delay5; 
assign #tDQSCK_div mem_ck_diff_delay1 = mem_ck_diff;
assign #tDQSCK_div mem_ck_diff_delay2 = mem_ck_diff_delay1;
assign #tDQSCK_div mem_ck_diff_delay3 = mem_ck_diff_delay2;
assign #tDQSCK_div mem_ck_diff_delay4 = mem_ck_diff_delay3;
assign #tDQSCK_div mem_ck_diff_delay5 = mem_ck_diff_delay4;

assign mem_dq = (mem_dq_en_delay5) ? mem_dq_int_delay5 : 'z;
assign mem_dqs =
	(mem_dqs_en_delay5) 
		?  (mem_dqs_preamble_delay5) 
			     ? '0 : {MEM_DQS_WIDTH{mem_ck_diff_delay5}} 
		              : 'z;
        
assign mem_dqs_n =
	(mem_dqs_en_delay5) 
		?  (mem_dqs_preamble_delay5) 
			? '1 : {MEM_DQS_WIDTH{~mem_ck_diff_delay5}} 
		: 'z;

...
module alt_mem_if_common_rldram_mem_model_mem_if_dm_pins_en_rldram3 (
	mem_a,
	mem_ba,
	mem_ck,
	mem_ck_n,
...
always @(posedge mem_ck_int) begin
   if (mem_reset_n !== 1'b1) begin
      reset_asserted <= 1'b1;
      reset_deasserted <= 1'b0;
   end
   else if (mem_reset_n == 1'b1 && reset_asserted == 1'b1) begin
      reset_deasserted <= 1'b1;
   end	
end

always @(posedge mem_ck_int) begin
   if ( reset_deasserted == 'b1 && MEM_IF_CS_WIDTH > 1 ) begin
      if ( mem_cs_n == '1 && mem_ref_n == 'b1 ) begin
         $display("NOP For Multi Rank RLDRAM3 should have REF# set to low");
         $finish(1);
      end
   end
end   

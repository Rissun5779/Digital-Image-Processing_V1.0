...
module altera_gpio_driver(
	...
	assign #1 sclr = sclr_w;
	assign #1 sset = sset_w;
	assign #1 cke = cke_w;
	...
	generate
		if (PIN_TYPE == "input")
		begin
			assign #1 pad_in = pad_io;
			assign #1 pad_in_b = pad_io_b;
		end
		...
		begin
			...
			begin
				genvar i;
				for (i = 0; i < SIZE; i++) begin : pullup_loop
					pullup(pad_out[i]);
				end
			end
	...
	generate
		if (PIN_TYPE == "output" && ENABLE_OE != "true")
		begin
			assign oe = 1'b1;
		end
		else
		begin
			assign #1 oe = data_oe_out;
		end
	endgenerate
	assign #1 din = data_out;
	assign data_in = dout;

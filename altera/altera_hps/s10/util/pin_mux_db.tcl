...
namespace eval ::pin_mux_db {
    source "$::env(QUARTUS_ROOTDIR)/../ip/altera/altera_hps/s10/util/procedures.tcl"

    variable cache_base_device
    variable cache_base_device_result
        
    proc verify_soc_device {device} {
        if {[string match   "1SX*" $device ]} { 
            return 1
        } else {
            return 0
        }
    ...
    proc verify_base_device {device} {
        ...
        if { [string compare -nocase $cache_base_device $device] == 0} {
            ...
            set quartus_script_path "$::env(QUARTUS_ROOTDIR)/../ip/altera/altera_hps/s10/util/quartus_pin_mux_db_part_query.tcl"
        ...
        #send_message info "SANTI 10: got $cache_base_device_result"
    ...
    proc get_pinmux_csv {} {
        set csv_file [file join $::env(QUARTUS_ROOTDIR) .. ip altera altera_hps s10 altera_hps_stratix_10 "pinmux.csv"]
        return $csv_file
    }
    proc load_pin_mux_table {device} {
        ...
        if { $send_error } {
            ...
            send_message warning "Unsupported device $device"
        }
    ...
    proc get_csv_data {cols base_device index_ref pad_name_ref valid_pins_ref pin_name_ref send_error_ref} {
        ...
        set pad_name     [string trim [lindex $cols 1]]
        set valid_pins   [string trim [lindex $cols 2]]

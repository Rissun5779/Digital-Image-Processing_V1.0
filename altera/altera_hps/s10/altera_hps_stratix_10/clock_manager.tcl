...
proc get_mpu_freq { max_vco_freq_ref l3_main_free_clk_max_freq_ref cv_freq_ref} {
    ...
    if {[ regexp {^1SX.+I1.+} $device match ] || [ regexp {^1SX.+E1.+} $device match ] } {
        send_message debug "MATCH fast speed grade ${device}"
        set recomended_mpu_freq 1500
        set max_vco_freq        3000 
        set l3_main_free_clk_max_freq 500
        set cv_freq             100
    } elseif {[ regexp {^1SX.+I2.+} $device match ] || [ regexp {^1SX.+E2.+} $device match ]} {
        send_message debug "MATCH nom speed grade ${device}"
        set recomended_mpu_freq 1200
        set max_vco_freq        2400 
        set l3_main_free_clk_max_freq 400
        set cv_freq              60
    } elseif {[ regexp {^1SX.+I3.+} $device match ] || [ regexp {^1SX.+E3.+} $device match ] } {
        send_message debug "MATCH slow speed grade ${device}"
        set recomended_mpu_freq 800
        set max_vco_freq        1600 
        set l3_main_free_clk_max_freq 400
        set cv_freq             30
    } else {
        send_message error "Unsupported device ${device} (this is a bug)"
    }
...
proc update_hps_to_fpga_clock_frequency_parameters {} {
    ...
    update_pll_out_debug
}

proc update_pll_out_debug {} {
    set clk0 [get_parameter_value PLL_CLK0] 
    set clk1 [get_parameter_value PLL_CLK1] 
    set clk2 [get_parameter_value PLL_CLK2] 
    set clk3 [get_parameter_value PLL_CLK3] 
    set clk4 [get_parameter_value PLL_CLK4] 

    if { $clk0 == [IO_MUX_VALUE] || $clk1 == [IO_MUX_VALUE]} {
        set_parameter_property   TESTIOCTRL_MAINCLKSEL  enabled        true
    } else {
        set_parameter_property   TESTIOCTRL_MAINCLKSEL  enabled        false
    }
    
    if { $clk2 == [IO_MUX_VALUE] || $clk3 == [IO_MUX_VALUE]} {
        set_parameter_property   TESTIOCTRL_PERICLKSEL  enabled        true
    } else {
        set_parameter_property   TESTIOCTRL_PERICLKSEL  enabled        false
    }
    
    if { $clk4 == [IO_MUX_VALUE] } {
        set_parameter_property   TESTIOCTRL_DEBUGCLKSEL  enabled        true
    } else {
    ...
    for {set i 0} {$i < 3} {incr i} {
        ...
        if {$emac_ena} {
            ...
            if {$emac_clk == 250} {
                ...
                set emaca_ena 1
                if {$is_rmii} {
                    send_message warning "EMAC${i} is in RMII mode but has a 250MHz clock. 50MHz Clock is recomened"
                }
            } else {
                set_parameter_value EMAC${i}SEL [MUXB]
                set emacb_ena 1
                if {!$is_rmii} {
                    send_message warning "EMAC${i} is in GRMII mode but has a 50MHz clock. 250MHz Clock is recomened"
                }
            }
        }
...
proc add_clock_tab { parent_tab } {
    ...
    add_parameter2 $clock_output_main_pll_group USE_DEFAULT_MPU_CLK "Overide default MPU clock frequency" boolean false
    ...
    add_parameter2 $perip_group OVERIDE_PERI_PLL "Overide peripherial PLL VCO frequency" boolean false
    ...
    set_parameter_property        CLK_SDMMC_SOURCE  description "Secure Digital/Multimedia Card (SD/MMC) controller clock sourc"
    ...
    set resets_tab  "Resets and PLL Debug"
    ...
    set_parameter_property   TESTIOCTRL_MAINCLKSEL  allowed_ranges {"0:C0 (MPU) & C8 (User 1 Clk)" "1:C1 (NOC) & C9 (HMC Ref)" "2:C2 (EMAC Mux A) & C10 (Unused)" "3:C3 (EMAC Mux B) & C11 (Unused)" "4:C4 (EMAC PTP) & C13 (Unused)" "5:C5 (GPIO DB) & C14 (Unused)" "6:C6 (SDMMC) & C15 (Unused)" "7:C7 (User 1 Clk) & C16 (Unused)" "8:Disabled (VSS)"}
    ...
    set_parameter_property   TESTIOCTRL_MAINCLKSEL  description    "Selects the source of PLL_CLK0 and PLL_CLK1 dedicated IO outputs from the Main PLL."
    set_parameter_property   TESTIOCTRL_MAINCLKSEL  enabled        false

    add_parameter            TESTIOCTRL_PERICLKSEL  integer 8
    set_parameter_property   TESTIOCTRL_PERICLKSEL  allowed_ranges {"0:C0 (MPU) & C8 (User 1 Clk)" "1:C1 (NOC) & C9 (HMC Ref)" "2:C2 (EMAC Mux A) & C10 (Unused)" "3:C3 (EMAC Mux B) & C11 (Unused)" "4:C4 (EMAC PTP) & C13 (Unused)" "5:C5 (GPIO DB) & C14 (Unused)" "6:C6 (SDMMC) & C15 (Unused)" "7:C7 (User 1 Clk) & C16 (Unused)" "8:Disabled (VSS)"}
    ...
    set_parameter_property   TESTIOCTRL_PERICLKSEL  description    "Selects the source of PLL_CLK2 and PLL_CLK3 dedicated IO outputs from the Peripherial PLL."
    set_parameter_property   TESTIOCTRL_PERICLKSEL  enabled        false
    ...
    set_parameter_property   TESTIOCTRL_DEBUGCLKSEL description    "Selects the source of PLL_CLK4 for miscellaneous PLL signals."
    set_parameter_property   TESTIOCTRL_DEBUGCLKSEL enabled        false

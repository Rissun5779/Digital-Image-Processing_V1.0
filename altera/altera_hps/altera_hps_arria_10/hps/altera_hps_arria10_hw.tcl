...
set_module_property NAME altera_arria10_hps
...
set_module_property DESCRIPTION "The HPS contains a microprocessor unit (MPU) subsystem with dual-core ARM Cortex-A9 MPCore processors, flash memory controllers, dedicatedSDRAMcontroller interconnect, on-chip memories, support peripherals, interface peripherals, debug capabilities, and phase-locked loops (PLLs)."
set_module_property DISPLAY_NAME "Arria 10 Hard Processor System"
set_module_property INSTANTIATE_IN_SYSTEM_MODULE true
set_module_property EDITABLE false
set_module_property HIDE_FROM_SOPC true
set_module_property HIDE_FROM_QUARTUS true
add_documentation_link "HPS User Guide for Arria 10"   "http://www.altera.com/literature/lit-arria-10.jsp"
...
proc load_modes_table { modes_table_ref } {
	...
	array set modes_table {
		...
		"NAND"  { "8-bit" "16-bit" }
		"QSPI"  { "1ss" "2ss" "3ss" "4ss" }
		"SDMMC" { "1-bit" "1-bit_PWREN" "4-bit" "4-bit_PWREN" "8-bit" "8-bit_PWREN" }
		"SDIO"  { "1-bit" "1-bit_PWREN" "4-bit" "4-bit_PWREN" "8-bit" "8-bit_PWREN" }
		"USB"   { "default" }
...
proc add_fpga_emac_switch_parameters {} {
	...
	set_parameter_property   EMAC0_SWITCH_Enable   enabled        false
	...
	set_parameter_property   EMAC1_SWITCH_Enable   enabled        false
	...
	set_parameter_property   EMAC2_SWITCH_Enable   enabled        false
...
set_parameter_property   BOOT_FROM_FPGA_Enable   description    "Enables an input to the HPS indicating it is safe for the processor to boot from memory located at offset 0x0 of the HPS-to-FPGA bridge (absolute address 0xTBD). This ensures that a valid bootloader image is available before the bootROM code attempts to jump to the location in the FPGA. In addition of its boot operation, f2h_boot_from_fpga_ready is used when the BootRom wants to boot with the public key stored in the FPGA. The f2h_boot_from_fpga_ready signal must be asserted to boot with public key stored in the FPGA."
...
add_parameter            F2S_Width  integer 5
...
add_parameter            S2F_Width  integer 5
...
add_parameter            LWH2F_Enable integer 2
...
add_display_item $group_name F2SDRAM_TABLE TEXT "<html><table border=\"1\" width=\"100%\">
		                                    <tr><th>Port Configuration</th><td>F2SDRAM 0</td><td>F2SDRAM 1</td><td>F2SDRAM 2</td></tr>
		                                    <tr><td>1</td><td>AXI-32</td><td>AXI-32</td><td>AXI-32</td></tr>
		                                    <tr><td>2</td><td>AXI-64</td><td>AXI-64</td><td>AXI-64</td></tr>
		                                    <tr><td>3</td><td>AXI-128</td><td>unused</td><td>AXI-128</td></tr>
		                                    <tr><td>4</td><td>AXI-128</td><td>AXI-32</td><td>AXI-64</td></tr>
		                                    </table></html>"
...
set_parameter_property   F2SDRAM_PORT_CONFIG  hdl_parameter  false
set_parameter_property   F2SDRAM_PORT_CONFIG  group          $group_name
set_parameter_property   F2SDRAM_PORT_CONFIG  description    "You can add one or more SDRAM ports that make the HPS SDRAM accessible to the FPGA fabric. You can configure the slave interface to a data width of 32, 64, or 128 bits by using the port configuration value from the table above."

add_parameter            F2SDRAM0_ENABLED  boolean false
set_parameter_property   F2SDRAM0_ENABLED  display_name   "F2SDRAM0 Enabled"
set_parameter_property   F2SDRAM0_ENABLED  hdl_parameter  false
set_parameter_property   F2SDRAM0_ENABLED  group          $group_name
set_parameter_property   F2SDRAM0_ENABLED  description    "F2SDRAM port 0"

add_parameter            F2SDRAM1_ENABLED  boolean false
set_parameter_property   F2SDRAM1_ENABLED  display_name   "F2SDRAM1 Enabled"
set_parameter_property   F2SDRAM1_ENABLED  hdl_parameter  false
set_parameter_property   F2SDRAM1_ENABLED  group          $group_name
set_parameter_property   F2SDRAM1_ENABLED  description    "F2SDRAM port 1"


add_parameter            F2SDRAM2_ENABLED  boolean false
set_parameter_property   F2SDRAM2_ENABLED  display_name   "F2SDRAM2 Enabled"
set_parameter_property   F2SDRAM2_ENABLED  hdl_parameter  false
set_parameter_property   F2SDRAM2_ENABLED  group          $group_name
set_parameter_property   F2SDRAM2_ENABLED  description    "F2SDRAM port 2"

add_parameter            F2SDRAM_READY_LATENCY  boolean true
set_parameter_property   F2SDRAM_READY_LATENCY  display_name   "Enable Ready Latency for F2SDRAM"
set_parameter_property   F2SDRAM_READY_LATENCY  hdl_parameter  false
set_parameter_property   F2SDRAM_READY_LATENCY  group          $group_name
set_parameter_property   F2SDRAM_READY_LATENCY  description    "Enables the Ready Latency Interface for all of the F2SDRAM AXI bridgese"


add_parameter            F2SDRAM2_DELAY  integer        4
set_parameter_property   F2SDRAM2_DELAY  display_name   "F2SDRAM Fix Hold"
set_parameter_property   F2SDRAM2_DELAY  hdl_parameter  false
set_parameter_property   F2SDRAM2_DELAY  group          $group_name
set_parameter_property   F2SDRAM2_DELAY  description    "Advanced Setting to play with the delay used to fix Hold time"
set_parameter_property   F2SDRAM2_DELAY  visible        false
set_parameter_property   F2SDRAM2_DELAY  allowed_ranges {0:100}

add_display_item $group_name F2SDRAMInitMessage TEXT ""


add_parameter            F2SDRAM_ADDRESS_WIDTH  integer 32
set_parameter_property   F2SDRAM_ADDRESS_WIDTH  display_name   "F2SDRAM bridge address width"
...
proc add_peripheral_pin_muxing_parameters {} {
	...
	set max_possible_hps_io_options 62
...
proc add_pin_mux_gui {} {
    ...
    add_parameter           Quad_1_Save      string_list {}
    set_parameter_property  Quad_1_Save      derived true
    set_parameter_property  Quad_1_Save      visible false

    add_parameter           Quad_2_Save      string_list {}
    set_parameter_property  Quad_2_Save      derived true
    set_parameter_property  Quad_2_Save      visible false

    add_parameter           Quad_3_Save      string_list {}
    set_parameter_property  Quad_3_Save      derived true
    set_parameter_property  Quad_3_Save      visible false

    add_parameter           Quad_4_Save      string_list {}
    set_parameter_property  Quad_4_Save      derived true
    set_parameter_property  Quad_4_Save      visible false
	...
	set widget_parameters {
        ...
        Quad_1_Save             Quad_1_Save
        Quad_2_Save             Quad_2_Save
        Quad_3_Save             Quad_3_Save
        Quad_4_Save             Quad_4_Save
    }

    add_display_item $group_name the_widget "group"
    set_display_item_property the_widget widget [list ../pin_mux_widget/pin_mux_arria10.jar pin_mux_arria10]
    ...
    add_parameter            EMAC2_PTP  boolean        false
    set_parameter_property   EMAC2_PTP  display_name   "Enable EMAC2 Precision Time Protocol (PTP) FPGA Interface"
    set_parameter_property   EMAC2_PTP  hdl_parameter  false
    set_parameter_property   EMAC2_PTP  enabled        false
    set_parameter_property   EMAC2_PTP  group          $group_name
    set_parameter_property   EMAC2_PTP  description    "When the EMAC is connected to the HPS IO via the Pinmux, the IEEE 1588 Precision Time Protocol (PTP) interface can be accessed through the FPGA. When the EMAC connects to the FPGA, the PTP signals are always available."
}
...
proc add_sdram_parameters {} {
	...
	set_parameter_property   EMIF_CONDUIT_Enable  display_name   "Enable the conduit to connect to the Arria 10 External Memory Interface"
	set_parameter_property   EMIF_CONDUIT_Enable  description    "Enables the HPS dedicated conduit to the Arria 10 External Memory Interface for HPS. This conduit cannot connect to any other External Memory Interface (EMIF). Only IP generated by the Arria 10 External Memory Interface for HPS Qsys library should be used."
	set_parameter_property   EMIF_CONDUIT_Enable  group          "EMIF Conduit"


	add_parameter            EMIF_BYPASS_CHECK  boolean        false
	set_parameter_property   EMIF_BYPASS_CHECK  display_name   "Disable EMIF F2SDRAM legality"
	set_parameter_property   EMIF_BYPASS_CHECK  description    "Internal Parameter"
	set_parameter_property   EMIF_BYPASS_CHECK  group          "EMIF Conduit"
	set_parameter_property   EMIF_BYPASS_CHECK  visible false
...
add_parameter          quartus_ini_hps_ip_enable_sdmmc_clk_in boolean "" ""
set_parameter_property quartus_ini_hps_ip_enable_sdmmc_clk_in system_info_type quartus_ini
set_parameter_property quartus_ini_hps_ip_enable_sdmmc_clk_in system_info_arg  hps_ip_enable_sdmmc_clk_in
set_parameter_property quartus_ini_hps_ip_enable_sdmmc_clk_in visible false
...
add_parameter          quartus_ini_hps_ip_overide_f2sdram_delay boolean "" ""
set_parameter_property quartus_ini_hps_ip_overide_f2sdram_delay system_info_type quartus_ini
set_parameter_property quartus_ini_hps_ip_overide_f2sdram_delay system_info_arg  hps_ip_overide_f2sdram_delay
set_parameter_property quartus_ini_hps_ip_overide_f2sdram_delay visible false
...
proc compose {} {
	...
	add_instance fpga_interfaces altera_arria10_interface_generator
	...
	add_instance hps_io altera_arria10_hps_io
...
proc logicalview_dtg {} {
	...
	source ../hps_periphs/hps_baum_periphs.tcl
	...
	set device_family [get_parameter_value hps_device_family]
        set F2SDRAM_PORT_CONFIG [get_parameter_value F2SDRAM_PORT_CONFIG]
        set F2SDRAM0_ENABLED [get_parameter_value F2SDRAM0_ENABLED]
        set F2SDRAM1_ENABLED [get_parameter_value F2SDRAM1_ENABLED]
        set F2SDRAM2_ENABLED [get_parameter_value F2SDRAM2_ENABLED]
        set F2SDRAM_ready_latency_enabled [get_parameter_value F2SDRAM_READY_LATENCY]
	...
	if { $base_device == "NIGHTFURY4ES" } {
		set_parameter_property F2SDRAM0_ENABLED ENABLED $F2SDRAM0_ENABLED
		set_parameter_property F2SDRAM2_ENABLED ENABLED $F2SDRAM2_ENABLED
		
	} else {
		set_parameter_property F2SDRAM0_ENABLED ENABLED true
		set_parameter_property F2SDRAM2_ENABLED ENABLED true
	}

	
	if {$F2SDRAM_PORT_CONFIG == 6 } {
		set F2SDRAM1_ENABLED false
		set_parameter_property F2SDRAM1_ENABLED ENABLED false
	}

	set number_of_a9 2

	add_instance clk_0 hps_clk_src
	hps_utils_add_instance_clk_reset clk_0 bridges arria10_hps_bridge_avalon
	...
	set_instance_parameter_value bridges F2SDRAM_PORT_CONFIG $F2SDRAM_PORT_CONFIG
	set_instance_parameter_value bridges F2SDRAM0_ENABLED $F2SDRAM0_ENABLED
	set_instance_parameter_value bridges F2SDRAM1_ENABLED $F2SDRAM1_ENABLED
	set_instance_parameter_value bridges F2SDRAM2_ENABLED $F2SDRAM2_ENABLED
	set_instance_parameter_value bridges F2SDRAM_READY_LATENCY $F2SDRAM_ready_latency_enabled
    ...
    set svd_path [file join $::env(QUARTUS_ROOTDIR) .. ip altera altera_hps altera_hps_arria_10 hps altera_hps.svd]
	if { $h2f_present } {
        ...
        set_interface_property h2f_axi_master SVD_ADDRESS_OFFSET [hps_i_fpga_bridge_soc2fpga128_base]
	...
	if { $h2f_lw_present } {
        ...
        set_interface_property h2f_lw_axi_master SVD_ADDRESS_OFFSET [hps_i_fpga_bridge_lwsoc2fpga_base]
        if {!$declared_svd_file} {
	...
	if { $f2h_present } {
		hps_utils_add_slave_interface bridges.axi_f2h baum_clkmgr.axi_slave0 [hps_i_clk_mgr_clkmgr_base]
	}
    ...
    hps_instantiate_baum_timer $number_of_a9
    ...
    hps_instantiate_baum_scu $number_of_a9
...
proc compose_logicalview {} {
	...
	set interface_names $temp_array([ORDERED_NAMES])

	if { $interface_names == "" } {
		send_message info "No dedicated HPS pins have been selected, and the hps_io interface is empty."
	}
	...
	#send_message info "instance= hps_io  param= border_description  value = [array get hps_io_border]"
...
proc validate {} {
	...
	#fix out of range case 282593
	foreach pll_counet_parm {MAINPLLGRP_VCO_DENOM MAINPLLGRP_VCO_NUMER 
		MAINPLLGRP_MPU_CNT MAINPLLGRP_NOC_CNT MAINPLLGRP_EMACA_CNT MAINPLLGRP_EMACB_CNT 
		MAINPLLGRP_EMAC_PTP_CNT MAINPLLGRP_GPIO_DB_CNT MAINPLLGRP_SDMMC_CNT MAINPLLGRP_S2F_USER0_CNT 
		MAINPLLGRP_S2F_USER1_CNT MAINPLLGRP_HMC_PLL_REF_CNT MAINPLLGRP_PERIPH_REF_CNT 
		PERPLLGRP_VCO_DENOM PERPLLGRP_VCO_NUMER PERPLLGRP_MPU_CNT PERPLLGRP_NOC_CNT 
		PERPLLGRP_EMACA_CNT PERPLLGRP_EMACB_CNT PERPLLGRP_EMAC_PTP_CNT PERPLLGRP_GPIO_DB_CNT 
		PERPLLGRP_SDMMC_CNT PERPLLGRP_S2F_USER0_CNT PERPLLGRP_S2F_USER1_CNT PERPLLGRP_HMC_PLL_REF_CNT } {

		if { [get_parameter_value $pll_counet_parm ] == 2048 } {
			set_parameter_value $pll_counet_parm  2047
			send_message info "Unused Value for $pll_counet_parm was fixed from the illigal value 2048 to 2074"
		}		
	}
	...
	set l4_free     [get_parameter_value L4_SYS_FREE_CLK]
	if {$l4_free == 0} {
	    send_message error "HPS will not work properly if L4 free clock is set as divide by 2. Change the L4 free clock frequency to divide by 4 from the L3 clock."
	}
...
proc validate_sdmmc {} {
    set base_device  [::pin_mux_db::verify_base_device [get_device] ]
    if { $base_device == "NIGHTFURY4ES" } {
        if { ! [get_parameter_value quartus_ini_hps_ip_override_sdmmc_4bit] } {
            
            set pin_muxing_value [get_parameter_value SDMMC_PinMuxing ]
            if {[string compare "IO" $pin_muxing_value] == 0} {
                
                set mode_value [get_parameter_value SDMMC_Mode ]
                if {[string match  "4-bit*" $mode_value ]} {
                    send_message error "The SDMMC 4 bit mode has issues in the ES device. Please enable the 8-bit interface mode for the SD/MMC instead and ensure that the upper four data bits are pulled high.  You can ensure that they are pulled high by tying them high on the board or adding weak pullup settings for those pins in the Quartus assignment editor."
                }
            }
        }
    }
    # Case 297453: power enbale pin does not work as expected
    set pin_muxing_value [get_parameter_value SDMMC_PinMuxing ]
    if {[string compare "IO" $pin_muxing_value] == 0} {
        
        set mode_value [get_parameter_value SDMMC_Mode ]
        if {[string match  "*PWREN" $mode_value ]} {
            send_message warning "Altera does not recommend enabling the power enable pin since it operates in an active low state when enabled while flash devices expect an active high power enable.  You can use a GPIO to control the power enable independently."
        }
    }
...
proc validate_F2SDRAM {} {
	#--------------------------------------------------------------------------#
	# validating FPGA-to-SDRAM interfaces
    ...
    set EMIF_ENABLED [get_parameter_value EMIF_CONDUIT_Enable ]
    set EMIF_BYPASS  [get_parameter_value EMIF_BYPASS_CHECK ]
	
    
    set show_delay [get_parameter_value quartus_ini_hps_ip_overide_f2sdram_delay]
    
    set_parameter_property   F2SDRAM2_DELAY  visible  $show_delay
    
    if { !$EMIF_ENABLED } {
        if { $f2sdram0_enabled || $f2sdram1_enabled || $f2sdram2_enabled } {
			if {$EMIF_BYPASS} {
            	send_message warning "The F2SDRAM Bridge is not available if there is no HPS EMIF in the design. Enable the EMIF Conduit on the SDRAM tab or disable the F2SDRAM Interface."
			} else {
				send_message error "The F2SDRAM Bridge is not available if there is no HPS EMIF in the design. Enable the EMIF Conduit on the SDRAM tab or disable the F2SDRAM Interface."
			}
        } 
    }
	...
	if { $base_device == "NIGHTFURY4ES" } {
		if {$f2sdram2_enabled} {
			send_message error "The F2SDRAM 2 port is not available on ES devices."
		}
		if {$f2sdram0_enabled} {
			send_message error "The F2SDRAM 0 port is not available on ES devices."
		}
		
		set f2sdram2_enabled false
		set f2sdram0_enabled false
	} 

	set indexs [list]
	set widths [list]

	set F2SDRAMInitMessage "<b>F2SDRAM 0 port is"
	if {$f2sdram0_enabled} {
		 set F2SDRAMInitMessage "$F2SDRAMInitMessage enabled"
		 if {$configuration_list == 4} {
			 set F2SDRAMInitMessage "$F2SDRAMInitMessage with 32-bit AXI</b><br>"
			 lappend widths "32"
		 } elseif {$configuration_list == 5} {
			 set F2SDRAMInitMessage "$F2SDRAMInitMessage with 64-bit AXI</b><br>"
			 lappend widths "64"
		 } elseif {$configuration_list == 6} {
			 set F2SDRAMInitMessage "$F2SDRAMInitMessage with 128-bit AXI</b><br>"
			 lappend widths "128"
		 ...
		 set F2SDRAMInitMessage "$F2SDRAMInitMessage disabled</b><br>"
	}

	set F2SDRAMInitMessage "$F2SDRAMInitMessage<b>F2SDRAM 1 port is"
	if {$configuration_list == 6} {
		set_parameter_property F2SDRAM1_ENABLED ENABLED false
		set F2SDRAMInitMessage "$F2SDRAMInitMessage disabled</b><br>"
	} else {
		set_parameter_property F2SDRAM1_ENABLED ENABLED true
		if {$f2sdram1_enabled} {
			 set F2SDRAMInitMessage "$F2SDRAMInitMessage enabled"
			 if {$configuration_list == 4} {
				 set F2SDRAMInitMessage "$F2SDRAMInitMessage with 32-bit AXI</b><br>"
	...
	set F2SDRAMInitMessage "$F2SDRAMInitMessage<b>F2SDRAM 2 port is"
	if {$f2sdram2_enabled} {
		 set F2SDRAMInitMessage "$F2SDRAMInitMessage enabled"
		 if {$configuration_list == 4} {
			 set F2SDRAMInitMessage "$F2SDRAMInitMessage with 32-bit AXI</b><br>"
	...
	set htmlF2SDRAMInitMessage "<html><table border=\"0\" width=\"100%\"><tr><td valign=\"top\"><font size=3>$F2SDRAMInitMessage</td></tr></table></html>"
	set_display_item_property F2SDRAMInitMessage TEXT $htmlF2SDRAMInitMessage

	#--------------------------------------------------------------------------#

	set rows [llength $widths]
	for {set index 0} {${index} < ${rows}} {incr index} {
		# check for invalid combinations of type/width
		set mywidth [lindex $widths $index]
...
proc peripheral_to_wys_atom_name { peripheral} {
	...
	set wys_atom_name "twentynm_${generic_atom_name}"
...
proc elab {logical_view} {
	...
	elab_BOOT_FROM_FPGA           $device_family
	...
	if {$logical_view == 0} {
		...
		set EMIF_ENABLED [get_parameter_value EMIF_CONDUIT_Enable ]
		set EMIF_BYPASS  [get_parameter_value EMIF_BYPASS_CHECK ]
		#If the EMIF is not used, do not instanciate the F2SDRAM interface
		if {$EMIF_ENABLED || $EMIF_BYPASS } {
	...
	elab_SECURITY_MODULE          $device_family
	...
	for {set i 0} {$i < 3} {incr i} {
		...
		if {[string compare $emac_pin_mux_value [IO_MUX_VALUE]]   == 0} {
		    set  emac_ptp_interface_enabled   [get_parameter_value EMAC${i}_PTP]
		    if {$emac_ptp_interface_enabled} {
                set emac_fpga_enabled   true
            }
        }
	}
...
proc elab_EMAC_ptp {device_family} {
    ...
    for {set i 0} {$i < 3} {incr i} {
        ...
        if {$emac_io_enabled && $emac_ptp } {
            ...
            set wys_atom_name  twentynm_hps_interface_peripheral_emac
            ...
            fpga_interfaces::add_interface_port  $iface_name emac${i}_ptp_tstmp_data     ptp_tstmp_data     Output 1  $instance_name ptp_tstmp_data
            fpga_interfaces::add_interface_port  $iface_name emac${i}_ptp_tstmp_en       ptp_tstmp_en       Output 1  $instance_name ptp_tstmp_en
...
proc elab_MPU_EVENTS_STANDBY {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_mpu_event_standby
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_DEBUG_APB {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_dbg_apb
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_STM {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_stm_event
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_CTI {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_cross_trigger
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_JTAG {device_family} {
	
	set ini_string            [get_parameter_value quartus_ini_hps_ip_enable_jtag]
	set ini_enabled           [expr { [string compare $ini_string "true" ] == 0 } ]	
	set_parameter_property JTAG_Enable visible $ini_enabled
	
	if [param_bool_is_enabled JTAG_Enable] {
		set instance_name  jtag_interface
		set atom_name      hps_interface_jtag
		set wys_atom_name  twentynm_hps_interface_jtag
		set location       [locations::get_fpga_location $instance_name $atom_name]

		set iface_name "h2f_jtag"
		set z          "h2f_jtag_"
		fpga_interfaces::add_interface          $iface_name conduit Output
		fpga_interfaces::add_interface_port     $iface_name ${z}tdi    s2f_fpga_scanman_tdi  Output  1  $instance_name  s2f_fpga_scanman_tdi
		fpga_interfaces::add_interface_port     $iface_name ${z}tms    s2f_fpga_scanman_tms  Output  1  $instance_name  s2f_fpga_scanman_tms
...
proc elab_EMIF_INTERFACE {device_family} {
    if [param_bool_is_enabled EMIF_CONDUIT_Enable] {
        ...
        if { $base_device == "NIGHTFURY4ES" } {
            ...
            fpga_interfaces::add_interface_port $iface_name ${z}emif_to_gp      emif_to_gp      Input   1 "h2f_gp"  f2s_gp
            fpga_interfaces::set_port_fragments $iface_name ${z}emif_to_gp "h2f_gp:f2s_gp(31:31)"
                
            fpga_interfaces::add_interface_port $iface_name ${z}gp_to_emif      gp_to_emif      Output  2 "h2f_gp"  s2f_gp
            fpga_interfaces::set_port_fragments $iface_name ${z}gp_to_emif "h2f_gp:s2f_gp(31:30)"
        } else {
            #someting
...
proc elab_GP {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_mpu_general_purpose
	set location       [locations::get_fpga_location $instance_name $atom_name]
	...
	if [param_bool_is_enabled GP_Enable] {
        ...
        if { $base_device == "NIGHTFURY4ES" } {
		    fpga_interfaces::add_interface_port  $iface_name  ${z}in   gp_in  Input  30  $instance_name f2s_gp
		    fpga_interfaces::set_port_fragments  $iface_name  ${z}in   "$instance_name:f2s_gp(29:0)"

		    fpga_interfaces::add_interface_port  $iface_name  ${z}out  gp_out Output 30  $instance_name s2f_gp
		    fpga_interfaces::set_port_fragments  $iface_name  ${z}out  "$instance_name:s2f_gp(29:0)"
	    ...
	    if { $base_device == "NIGHTFURY4ES" && [param_bool_is_enabled EMIF_CONDUIT_Enable] } {
	        fpga_interfaces::add_module_instance $instance_name $wys_atom_name $location
	    }
	}
}

proc elab_BOOT_FROM_FPGA {device_family} {
	set instance_name  boot_from_fpga
	set atom_name      hps_interface_boot_from_fpga
	set wys_atom_name  twentynm_hps_interface_boot_from_fpga
	set location       [locations::get_fpga_location $instance_name $atom_name]
	...
	if {$bsel_en} {
		fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_bsel_en" 1 0 0:0 1
		fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_bsel"    3 0 2:0 $bsel
		set_parameter_property BSEL enabled true
		
	} else {
		fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_bsel_en" 1 0 0:0 0
		fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_bsel"    3 0 2:0 1
		set_parameter_property BSEL enabled false
	}

	set boot_from_fpga_enable [expr { [string compare [get_parameter_value BOOT_FROM_FPGA_Enable] "true" ] == 0 } ]
	if {$boot_from_fpga_enable} {
		set iface_name "f2h_boot_from_fpga"
		set z          "f2h_boot_from_fpga_"
		fpga_interfaces::add_interface       $iface_name  conduit Input
		fpga_interfaces::add_interface_port  $iface_name  "${z}ready"       boot_from_fpga_ready       Input 1  $instance_name f2s_boot_from_fpga_ready
		fpga_interfaces::add_interface_port  $iface_name  "${z}on_failure"  boot_from_fpga_on_failure  Input 1  $instance_name f2s_boot_from_fpga_on_failure
	} else {
	    set boot_ini_string    [get_parameter_value quartus_ini_hps_ip_boot_from_fpga_ready]
	    if {$boot_ini_string } {
		    fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_boot_from_fpga_ready"      1 0 0:0 1
		    fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_boot_from_fpga_on_failure" 1 0 0:0 1
	    } else {
		    fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_boot_from_fpga_ready"      1 0 0:0 0
		    fpga_interfaces::set_instance_port_termination ${instance_name} "f2s_boot_from_fpga_on_failure" 1 0 0:0 0
		}
	}

	if {$boot_from_fpga_enable} {
		send_message info "Valid A9 code must be available to the h2f_axi_master interface in a memory component at address 0x0."
	}
...
proc elab_CLOCKS_RESETS {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_clocks_resets
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_DMA {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_dma
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc elab_INTERRUPTS {device_family logical_view} {
	...
	set wys_atom_name  twentynm_hps_interface_interrupts
	set location       [locations::get_fpga_location $instance_name $atom_name]
	...
	set interrupts_to_hide [list "EMAC0" "EMAC1" "EMAC2"  "NAND" "QSPI" \
				"I2CEMAC0" "I2CEMAC1" "I2CEMAC2" "USB0" "USB1" "UART0" "UART1" "SDMMC"\
...
proc elab_SECURITY_MODULE {device_family} {
	set instance_name  security
	set atom_name      hps_interface_security_module
	set wys_atom_name  twentynm_hps_interface_security_module
	set location       [locations::get_fpga_location $instance_name $atom_name]

	if [param_bool_is_enabled SECURITY_MODULE_Enable] {
		set iface_name "h2f_security"
		fpga_interfaces::add_interface       $iface_name  conduit Input
		fpga_interfaces::add_interface_port  $iface_name  f2h_security_anti_tamper_in  anti_tamper_in  Input  1  $instance_name anti_tamper_in
		fpga_interfaces::add_interface_port  $iface_name  h2f_security_anti_temper_out anti_tamper_out Output 1  $instance_name anti_tamper_out
...
proc elab_EMAC_SWITCH {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_emac_switch
...
proc elab_TEST {device_family} {
	...
	set wys_atom_name  twentynm_hps_interface_test
	set location       [locations::get_fpga_location $instance_name $atom_name]
...
proc get_pin_location {peripheral_and_pin} {
	...
	send_message error "The pin $peripheral_and_pin Needs to be connected to a pin, but has no pin asigment Yet "
...
proc elab_io_interface_signals {peripheral_name mode_value peripheral_ports_arg} {
	...
	if {[lsearch $values $mode_value] == -1} {
		send_message error " The $peripheral_name has an invalid mode: \"$mode_value\". Valid modes are: $values"
	}
...
proc elab_GPIO {device_family} {
	...
	if {$gpio_used} {
		set wys_atom_name  twentynm_hps_peripheral_gpio
...
proc elab_FPGA_Peripheral_Signals {device_family} {
	...
	set enable_sdmmc_clk_in  [expr {[string compare [get_parameter_value quartus_ini_hps_ip_enable_sdmmc_clk_in] "true" ] == 0}]
	set EMAC_TO_FPGA_USED 0
	set I2C_TO_FPGA_USED  0
	...
	foreach peripheral_name $fpga_ifaces([ORDERED_NAMES]) { ;# Peripherals
		...
		if {[string compare $pin_mux_value [FPGA_MUX_VALUE]] == 0 && [lsearch $allowed_ranges [FPGA_MUX_VALUE]] != -1} {
            ...
            #####################
            set pn $peripheral_name
            if {$pn == "EMAC0" || $pn == "EMAC1" || $pn == "EMAC2" } {
                set EMAC_TO_FPGA_USED 1
            }
            if {$pn == "SDMMC" } {
                fpga_interfaces::add_raw_sdc_constraint "set_false_path -from \[get_registers {*sdmmc_clk.reg__nff}\]"
            }
            if {$pn == "I2C0" || $pn == "I2C1" || $pn == "I2CEMAC0" || $pn == "I2CEMAC1" || $pn == "I2CEMAC2" } {
                set I2C_TO_FPGA_USED 1
            }
            ##########################
			foreach interface_name $interfaces([ORDERED_NAMES]) { ;# Interfaces
			    if {$interface_name == "sdmmc_clk_in" && !$enable_sdmmc_clk_in } { 
			        #case 321937 sdmmc_clk_in should not be exposed.
			        continue 
			    }
				...
				if { [ interface_is_vaild_in_this_mode  $peripheral_name  $interface_name ]} {
					...
					if {[string compare -nocase $interface(type) "clock"] == 0 && $once_per_clock && $frequency != 0} {
						set once_per_clock 0
						add_clock_constraint [frequency_to_period $frequency]  $port(atom_signal_name) $peripheral_name
						set add_false_paths 1
					}
			...
			set wys_atom_name twentynm_${atom_name}
    ...
    if {$add_false_paths} {
        ...
        if {$I2C_TO_FPGA_USED} {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_i2c_internal -period 2500.000 \[get_registers {*~l4_sp_clk.reg}\]"
        }
        
        if {$EMAC_TO_FPGA_USED} {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac_ptp_ref_clk -period 10 \[get_ports \{emac_ptp_ref_clock_clk\}\]"
            fpga_interfaces::add_raw_sdc_constraint "set_false_path -to \[get_registers {*_clk_rx_i.reg__nff}\]"
        }
		
		set base_device  [::pin_mux_db::verify_base_device [get_device]]
		if { $base_device == "NIGHTFURY4ES" } {
		    fpga_interfaces::add_raw_sdc_constraint "#This is only used for ES Silicon. "
			fpga_interfaces::add_raw_sdc_constraint "set_false_path -through \[ get_pins -compatibility_mode *fpga_interfaces|h2f_gp|f2s_gp\[31\]\]"
			fpga_interfaces::add_raw_sdc_constraint "set_false_path -through \[ get_pins -compatibility_mode *fpga_interfaces|h2f_gp|f2s_gp\[30\]\]"
			fpga_interfaces::add_raw_sdc_constraint "set_false_path -through \[ get_pins -compatibility_mode *fpga_interfaces|h2f_gp|s2f_gp\[31\]\]"
			fpga_interfaces::add_raw_sdc_constraint "set_false_path -through \[ get_pins -compatibility_mode *fpga_interfaces|h2f_gp|s2f_gp\[30\]\]"
		}
    }
}

proc add_clock_constraint { frequency  signal_name pn } {

    if {$pn == "TRACE" } {
        fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_trace_s2f_clk -period $frequency \[get_nodes *trace_clk\]"
    }
    if {$pn == "SPIM0" } {
        fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_spim0_sclk_out -period $frequency \[get_nodes *spim_0_clk\]"
    }
    if {$pn == "SPIM1" } {
        fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_spim1_sclk_out -period $frequency \[get_nodes *spim_1_clk\]"
    }
    if {$pn == "QSPI" } {
        fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_qspi_s2f_clk -period $frequency \[get_nodes *qspi_out_clk\]"
    }
    if {$pn == "SDMMC" } {
        fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_sdmmc_cclk -period $frequency \[get_nodes *sdmmc_clk\]"
    }
    
    #if {$pn == "SPIS0" } {
    #    fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_spis0_sclk_in -period $frequency \[get_ports \{spis0_sclk_in_clk\}\]"
    #}
    #if {$pn == "SPIS1" } {
    #    fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_spis1_sclk_in -period $frequency \[get_ports \{spis1_sclk_in_clk\}\]"
    #}
    if {$signal_name == "s2f_clk" } {
        if {$pn == "EMAC0"   } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac0_gtx_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac0|s2f_clk\]"
        }
        if {$pn == "EMAC1"  } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac1_gtx_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac1|s2f_clk\]"
        }
        if {$pn == "EMAC2"  } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac2_gtx_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac2|s2f_clk\]"
        }
    }
    if {$signal_name == "gmii_mdc_o" } {
        if {$pn == "EMAC0"  } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac0_md_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac0|gmii_mdc_o\]"
        }
        if {$pn == "EMAC1"  } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac1_md_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac1|gmii_mdc_o\]"
        }
        if {$pn == "EMAC2"  } {
            fpga_interfaces::add_raw_sdc_constraint "create_clock -name hps_emac2_md_clk -period $frequency \[get_pins -compatibility_mode *|fpga_interfaces|peripheral_emac2|gmii_mdc_o\]"
        }
    }
...
proc ensure_device_data {device_family} {
	...
	if {![is_soc_device $device]} {
		send_message error "Selected device '${device}' is not an SoC device supported by this Qsys Library. Please choose a valid Arria 10 device to uses the Hard Processor System (HPS)."
	}
}
...
add_documentation_link "Technical Reference Manual" https://documentation.altera.com/#/link/sfo1410070178831/sfo1410070064660
add_documentation_link "Release Notes" https://www.altera.com/content/dam/altera-www/global/en_US/pdfs/literature/rn/a10_hps_rn.pdf

...
// $Id: //acds/prototype/mm_s10/ip/merlin/altera_merlin_slave_agent/altera_merlin_burst_uncompressor.sv#2 $
// $Revision: #2 $
// $Date: 2015/05/15 $
// $Author: nkrueger $
...
module alt_hiconnect_burst_uncompressor
#(
    parameter ADDR_W      = 16,
...
(
    ...
    input sink_single_cycle_transfer,
...
function reg[63:0] bytes_in_transfer;
   ...
   wire [32:0] int_num_symbols_times_2 = PKT_SYMBOLS * 2;
   wire [BYTE_CNT_W-1:0] num_symbols = int_num_symbols[BYTE_CNT_W-1:0];
   wire [BYTE_CNT_W-1:0] num_symbols_times_2 = int_num_symbols_times_2[BYTE_CNT_W-1:0];
   ...
   wire burst_uncompress_byte_counter_lint_eq;
   reg burst_uncompress_byte_counter_lint_eq_reg;
   ...
   assign last_packet_beat = ~sink_is_compressed |
     (
     burst_uncompress_busy ? burst_uncompress_byte_counter_lint_eq_reg : sink_single_cycle_transfer
     );
   ...
   // Pre-calculate when the byte_counter is num_symbols << 1, i.e. one beat before last_packet_beat should be asserted
   // Source ready is needed to gate burst_uncompress_byte_counter_lint expression since the source might be backpressured with byte_cnt == (num_symbols << 1)
   // -- we only want last_packet_beat asserted when byte_cnt == num_symbols on the next cycle.  Sink_valid is needed to gate the sink_byte_cnt comparison
   // to ignore garbage on the sink interface per the Avalon ST spec.
   assign burst_uncompress_byte_counter_lint_eq = (((burst_uncompress_byte_counter_lint == num_symbols_times_2) & source_ready) | ((sink_byte_cnt == num_symbols_times_2) & sink_valid));
   
   // Register byte_counter equal to num_symbols << 1 to reduce the logic in the critical path.  Enable with
   // sink_valid & source_ready so we only update 'burst_uncompress_byte_counter_lint_eq_reg' when:
   // a) The byte counter is decrementing from num_symbols * 2 to num_symbols, i.e. we aren't back-pressured
   //    with byte_counter == num_symbols * 2. 
   // b) The byte counter is changing from num_symbols to the next "total byte count" of the next incoming burst.
   //    Put another way, we want to keep 'burst_uncompress_byte_counter_lint_eq_reg' asserted until the uncompressed
   //    beat with source_byte_cnt == num_symbols is accepted.  This accounts for backpressuring the last beat in an
   //    uncompressed packet.
   always @(posedge clk) begin
     if (sink_valid & source_ready) begin
        burst_uncompress_byte_counter_lint_eq_reg <= burst_uncompress_byte_counter_lint_eq;
     end
   end
     
   // Burst_uncompress_busy logic
   always @(posedge clk) begin
   ...
   // Burst_uncompress_byte_counter logic
   always @(posedge clk) begin
     if (internal_sclr) begin
   ...
   // Setup the starting address for the first beat
   always @(posedge clk) begin
   ...
   assign source_endofpacket   = sink_endofpacket & last_packet_beat & sink_valid;

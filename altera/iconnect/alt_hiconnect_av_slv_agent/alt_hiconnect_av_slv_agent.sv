...
// $Id: //acds/prototype/mm_s10/ip/merlin/altera_merlin_slave_agent/altera_merlin_slave_agent.sv#2 $
// $Revision: #2 $
// $Date: 2015/05/15 $
// $Author: nkrueger $
...
module alt_hiconnect_av_slv_agent
#(
   // Packet parameters
   ...
   // Derived FIFO width
   parameter FIFO_DATA_W = ST_DATA_W + 2,
   parameter PKT_FIFO_GENERATE_RESP = FIFO_DATA_W - 1,
   parameter PKT_FIFO_SINGLE_CYCLE_TRANS = FIFO_DATA_W - 2,
   ...
   wire rf_single_cycle_trans;
   wire single_cycle_trans;
   assign single_cycle_trans = (cmd_byte_cnt == num_symbols);
   ...
   always_comb begin
      ...
      rf_source_data[PKT_FIFO_GENERATE_RESP]             = needs_response_synthesis;
      ...
      rf_source_data[PKT_FIFO_SINGLE_CYCLE_TRANS]        = single_cycle_trans;
   end
   ...
   assign generate_response = rf_sink_data[PKT_FIFO_GENERATE_RESP];
   assign rf_single_cycle_trans = rf_sink_data[PKT_FIFO_SINGLE_CYCLE_TRANS];
   ...
   alt_hiconnect_burst_uncompressor #(
      ...
      // This violates the Avalon-ST spec which requires valid to be held high once asserted
      // until ready is asserted completing the transfer.
      ...
      .sink_single_cycle_transfer   (rf_single_cycle_trans),

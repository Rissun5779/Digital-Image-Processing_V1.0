...
// $Id: //acds/prototype/mm_s10/ip/merlin/altera_merlin_burst_adapter/new_source/altera_incr_burst_converter.sv#2 $
// $Revision: #2 $
// $Date: 2015/06/11 $
// $Author: nkrueger $
...
module alt_incr_burst_converter
#(
  parameter
    // ----------------------------------------
...
(
    ...
    input wire                               is_uncompressed,
    input wire [LEN_WIDTH       - 1 : 0]     in_len,
    input wire                               in_sop,

    input wire [ADDR_WIDTH      - 1 : 0]     in_addr,
    input wire [ADDR_WIDTH      - 1 : 0]     in_addr_reg,
    input wire [BURSTSIZE_WIDTH - 1 : 0]     in_size_t,
    input wire [BURSTSIZE_WIDTH - 1 : 0]     in_size_reg,
    ...
    always_comb begin : proc_uncompressed_remaining_len
        if ((in_len <= max_out_length) && is_uncompressed) begin
    ...
    always_ff @(posedge clk) begin
        if (enable && is_uncompressed) begin
    ...
    // Address Calculation
    // --------------------------------------------------
    reg [ADDR_LSBS_W - 1 : 0]        addr_incr_sel;
    reg [ADDR_LSBS_W - 1 : 0]        addr_incr_sel_reg;
    reg [ADDR_LSBS_W - 1 : 0]        addr_incr_full_size;

    localparam [ADDR_LSBS_W - 1 : 0] ADDR_INCR = MAX_OUT_LEN << LOG2_NUMSYMBOLS;

    generate
        if (IN_NARROW_SIZE) begin : narrow_addr_incr
            reg [ADDR_LSBS_W - 1 : 0]    addr_incr_variable_size;
            reg [ADDR_LSBS_W - 1 : 0]    addr_incr_variable_size_reg;
            ...
            assign addr_incr_full_size = ADDR_INCR[ADDR_LSBS_W - 1 : 0];

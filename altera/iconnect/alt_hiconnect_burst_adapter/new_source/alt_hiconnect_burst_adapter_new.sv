...
// $Id: //acds/prototype/mm_s10/ip/merlin/altera_merlin_burst_adapter/new_source/altera_merlin_burst_adapter_new.sv#2 $
// $Revision: #2 $
// $Date: 2015/06/11 $
// $Author: nkrueger $
...
// -------------------------------------------------------------------
// Burst Adapter: converts incoming burst packets to fit slave burst
// capabilities. 
// -------------------------------------------------------------------
module alt_hiconnect_burst_adapter_new
#(
    parameter 
    ...
    // System-type parameters which determines the protocol from the features that
    // are enabled or disabled.  Should be moved to the code that parameterizes this adapter.
    AXI_SLAVE                     = 0,
    IS_WRAP_AVALON_SLAVE          = 0,
    IS_INCR_SLAVE                 = 0,
    NON_BURSTING_SLAVE            = 0,
    INCR_ONLY_SYS                 = 1,
    INCR_AVALON_SYS               = 1,

    // An optimization to only use 12 bit address incrementers when
    // the slave is AXI, or all masters connected to the slave are AXI.
    // Simply put, this parameter will be set when we can assume no incoming
    // transaction will cross a 4KB address boundary.
    ...
    INSTANTIATE_DEFAULT_CONVERTER = 1,
    INSTANTIATE_WRAP_CONVERTER    = 1,
    INSTANTIATE_INCR_CONVERTER    = 1
)
(
    input wire                           clk,
    input wire                           reset,
    ...
    input wire [ST_CHANNEL_W - 1 : 0]    sink0_channel,
    ...
    alt_hiconnect_address_alignment
    #(
        .ADDR_W             (PKT_ADDR_W),
    ...
    alt_hiconnect_burst_adapter_burstwrap_increment 
    #(
    ...
    generate
        if (PIPE_INTERNAL == 0) begin : no_internal_pipeline
            ...
            // These signals don't need reset... state is reset to IDLE, and source0_valid should be de-asserted during reset.
            always_ff @(posedge clk) begin
    ...
    wire                    first_beat;
    wire                    subsequent_compressed_beat;
    wire                    enable_incr_converter;
    wire                    enable_fixed_converter;

    // The wrap converter is different from other converters because it has seperate read / write enables, and related internal logic for adapting bursts.
    // The in_sop' signal prevents the wrap converter from progressing past the first beat of an uncompressed write burst while a compressed read burst is currently being adapted.
    // The other direction, a compressed read coming in while an uncompressed write burst is undergoing adaptation shouldn't be an issue since it won't happen in practice if
    // the backpressure logic of the burst adapter is functioning properly.
    wire                    enable_uncompressed_wrap_converter;
    wire                    enable_compressed_wrap_converter;
    ...
    // -----------------------------------------------------------------------
    assign first_beat = d0_in_valid && (source0_ready || !source0_valid);
    assign subsequent_compressed_beat = (state == ST_COMP_TRANS) && source0_ready && !nb;
    
    // Uncompressed reads will go to the default converter if present, then the incr, then the wrap in that order of priority
    localparam UNCMP_READ_ROUTE_TO_DEF_CONVERTER = INSTANTIATE_DEFAULT_CONVERTER ? 1'b1 : 1'b0;
    localparam UNCMP_READ_ROUTE_TO_INCR_CONVERTER = INSTANTIATE_DEFAULT_CONVERTER ? 1'b0 : (INSTANTIATE_INCR_CONVERTER ? 1'b1 : 1'b0);
    localparam UNCMP_READ_ROUTE_TO_WRAP_CONVERTER = ((INSTANTIATE_DEFAULT_CONVERTER == 0) && (INSTANTIATE_INCR_CONVERTER == 0)) ? 1'b1 : 1'b0;
    ...
    generate
        if (NON_BURSTING_SLAVE) begin : non_bursting_converter_control
            ...
            // -----------------------------------------------------------------------
            // I. Converter enable signals: Turn on/off each converter accordingly
            // -----------------------------------------------------------------------
            assign enable_fixed_converter = (first_beat || (source0_endofpacket ?  1'b0 :(state == ST_COMP_TRANS) && (!source0_valid | source0_ready)));
        ...
        else if (INCR_ONLY_SYS) begin : incr_only_converter_control
            ...
            // I. Converter enable signals: Turn on/off each converter accordingly
            // -----------------------------------------------------------------------
            assign enable_incr_converter = (first_beat || (source0_endofpacket ?  1'b0 :(state == ST_COMP_TRANS) && (!source0_valid | source0_ready)));
            ...
            if (IS_WRAP_AVALON_SLAVE) begin
                ...
                assign in_read_but_not_fixed_or_narrow  = INSTANTIATE_WRAP_CONVERTER ? (in_compressed_read && !in_narrow_or_fixed) : 1'b0;
                assign in_write_but_not_fixed_or_narrow = INSTANTIATE_WRAP_CONVERTER ? (!in_narrow_or_fixed && (in_write || (UNCMP_READ_ROUTE_TO_WRAP_CONVERTER && in_uncompressed_read))) : 1'b0;
                assign in_narrow_or_fixed               = INSTANTIATE_DEFAULT_CONVERTER ? (in_narrow || in_fixed) : 1'b0;
                assign in_default_converter             = INSTANTIATE_DEFAULT_CONVERTER ? (in_fixed || in_narrow || (UNCMP_READ_ROUTE_TO_DEF_CONVERTER && in_uncompressed_read)) : 1'b0;
                
                // nb: new burst
                assign nb                               = (d1_in_narrow_or_fixed ? fixed_new_burst : wrap_new_burst);
                
                wire [PKT_BYTE_CNT_W - 1 : 0] incr_out_byte_cnt  = (d1_in_compressed_read ? wrap_out_len : wrap_uncompr_out_len) << log2_numsymbols;
                wire [PKT_BYTE_CNT_W - 1 : 0] fixed_out_byte_cnt = fixed_out_len << log2_numsymbols;
                ...
                // send first sub_burst length is aligned to slave boundary,
                ...
                // This works same way for INCOMPLETE wrap as well, so cannot make separate
                // optimization when dont support INCOMPLETE wrap
                // --------------------------------------------------------------------------
                if (INSTANTIATE_WRAP_CONVERTER == 1) begin : first_len_avalon_wrapping_slave
                ...
                end // INSTANTIATE_WRAP_CONVERTER

                // -----------------------------------------------------------------------
                // III. Converter enable signals: Turn on/off each converter accordingly
                // -----------------------------------------------------------------------
                // WRAPPING AVALON: two converters:
                //  1. wrap_burst_converter    -> handle full_size INCR, WRAP
                //  2. default_burst_converter -> handle narrow_size burst
                //  opt, separate enable for write and reach
                // -----------------------------------------------------------------------
                assign enable_uncompressed_wrap_converter = INSTANTIATE_WRAP_CONVERTER ? (first_beat && d0_in_write_but_not_fixed_or_narrow && fixed_new_burst) : 1'b0;
                assign enable_compressed_wrap_converter   = INSTANTIATE_WRAP_CONVERTER ? ((first_beat && d0_in_read_but_not_fixed_or_narrow && fixed_new_burst) || (subsequent_compressed_beat && d1_in_read_but_not_fixed_or_narrow)) : 1'b0;
                assign enable_fixed_converter             = INSTANTIATE_DEFAULT_CONVERTER ? ((first_beat && d0_in_default_converter && wrap_new_burst) || (subsequent_compressed_beat && d1_in_default_converter)) : 1'b0;

                // -----------------------------------------------------------------------
                // IV. Packet signals
                // -----------------------------------------------------------------------
                assign next_out_sop  = ((state == ST_COMP_TRANS) & source0_ready & !(d1_in_default_converter ? fixed_new_burst : wrap_new_burst)) ? 1'b0 : d0_in_sop;
                assign next_out_eop  = (state == ST_COMP_TRANS) ?  (d1_in_default_converter ? fixed_new_burst : wrap_new_burst)  : d1_in_eop;
                
                // -----------------------------------------------------------------------
                // V. Output select
                // -----------------------------------------------------------------------
                assign out_byte_cnt  = d1_in_default_converter ? fixed_out_byte_cnt : incr_out_byte_cnt;
                assign out_addr      = d1_in_default_converter ? fixed_out_addr : wrap_out_addr;
            ...
            if (AXI_SLAVE) begin
                ...
                reg                      d1_in_incr;
                reg                      in_incr_reg;
                wire                     in_read_wrap_converter;
                wire                     in_write_wrap_converter;
                reg                      in_read_wrap_converter_reg;
                reg                      in_write_wrap_converter_reg;

                reg                      d0_in_read_wrap_converter;
                reg                      d0_in_write_wrap_converter;
                reg                      d0_in_default_converter;
                reg                      d1_in_read_wrap_converter;
                reg                      d1_in_write_wrap_converter;
                reg                      d1_in_default_converter;

                wire                     in_incr                          = ((in_bursttype == 2'b01) && !in_uncompressed_read) || (UNCMP_READ_ROUTE_TO_INCR_CONVERTER && in_uncompressed_read);
                wire                     in_wrap                          = (in_bursttype == 2'b10);
                wire                     in_fixed                         = (in_bursttype == 2'b00) || (in_bursttype == 2'b11);
                wire                     in_narrow_read_wrap_smaller_16   = in_narrow && in_wrap && is_passthru && in_compressed_read;
                wire                     in_narrow_write_wrap_smaller_16  = in_narrow && in_wrap && is_passthru && (in_write || (UNCMP_READ_ROUTE_TO_WRAP_CONVERTER && in_uncompressed_read));
                wire                     in_narrow_wrap_larger_16         = in_narrow && in_wrap && !is_passthru;

                assign in_full_size_read_wrap   = INSTANTIATE_WRAP_CONVERTER ? (in_compressed_read && in_wrap && !in_narrow) : 1'b0;
                assign in_full_size_write_wrap  = INSTANTIATE_WRAP_CONVERTER ? ((in_wrap && !in_narrow) && (in_write || (UNCMP_READ_ROUTE_TO_WRAP_CONVERTER && in_uncompressed_read))) : 1'b0;
                assign in_read_wrap_converter   = INSTANTIATE_WRAP_CONVERTER ? (in_full_size_read_wrap  || in_narrow_read_wrap_smaller_16) : 1'b0;
                assign in_write_wrap_converter  = INSTANTIATE_WRAP_CONVERTER ? (in_full_size_write_wrap || in_narrow_write_wrap_smaller_16) : 1'b0;
                assign in_default_converter     = INSTANTIATE_DEFAULT_CONVERTER ? (in_narrow_wrap_larger_16 || in_fixed || (UNCMP_READ_ROUTE_TO_DEF_CONVERTER && in_uncompressed_read)) : 1'b0;
                ...
                wire [PKT_BYTE_CNT_W  - 1 : 0] incr_out_byte_cnt  = (d1_in_compressed_read ? incr_out_len : incr_uncompr_out_len) << log2_numsymbols;
                wire [PKT_BYTE_CNT_W  - 1 : 0] wrap_out_byte_cnt  = (d1_in_compressed_read ? wrap_out_len : wrap_uncompr_out_len) << log2_numsymbols;
                wire [PKT_BYTE_CNT_W  - 1 : 0] fixed_out_byte_cnt = fixed_out_len << log2_numsymbols;
                ...
                if(PIPE_INTERNAL == 0) begin : NO_PIPELINE_INPUT
                    always_ff @(posedge clk) begin
                        if (internal_sclr) begin
                            in_write_wrap_converter_reg <= '0;
                            in_read_wrap_converter_reg  <= '0;
                            in_default_converter_reg   <= '0;
                            in_incr_reg                <= '0;
                        end else begin
                            if (sink0_pipe_ready & sink0_pipe_valid) begin
                                in_write_wrap_converter_reg <= in_write_wrap_converter;
                                in_read_wrap_converter_reg  <= in_read_wrap_converter;
                                in_default_converter_reg   <= in_default_converter;
                                in_incr_reg                <= in_incr;
                            end
                        end // else: !if(internal_sclr)
                    ...
                    always_comb begin
                        d0_in_incr                 = in_incr;
                        d0_in_default_converter    = in_default_converter;
                        d0_in_read_wrap_converter   = in_read_wrap_converter;
                        d0_in_write_wrap_converter  = in_write_wrap_converter;
                        d0_first_len               = first_len;
                        d1_in_default_converter    = in_default_converter_reg;
                        d1_in_read_wrap_converter   = in_read_wrap_converter_reg;
                        d1_in_write_wrap_converter  = in_write_wrap_converter_reg;
                    ...
                    always_comb begin
                        ...
                        d0_in_default_converter    = in_default_converter_reg;
                        d0_in_read_wrap_converter   = in_read_wrap_converter_reg;
                        d0_in_write_wrap_converter  = in_write_wrap_converter_reg;
                        d0_first_len               = first_len_reg;
                        d0_in_bursttype            = nb ? in_bursttype_reg : d0_in_bursttype_dl;
                    end
                end
                ...
                // -------------------------------------------------------------------------
                if (INSTANTIATE_WRAP_CONVERTER == 1) begin : first_len_axi_slave
                    always_comb begin
                ...
                // III. Converter enable signals: Turn on/off each converter accordingly
                // -----------------------------------------------------------------------
                // AXI slave: three converters:
                //  1. wrap_burst_converter    -> handle WRAP
                ...
                //  3. default_burst_converter -> handle FIXED
                // -----------------------------------------------------------------------
                ...
                // -----------------------------------------------------------------------
                assign enable_incr_converter              = INSTANTIATE_INCR_CONVERTER ? (first_beat && d0_in_incr && fixed_new_burst && wrap_new_burst) || (subsequent_compressed_beat && d1_in_incr) : 1'b0;
                assign enable_uncompressed_wrap_converter = INSTANTIATE_WRAP_CONVERTER ? (first_beat && d0_in_write_wrap_converter && fixed_new_burst && incr_new_burst) : 1'b0;
                assign enable_compressed_wrap_converter   = INSTANTIATE_WRAP_CONVERTER ? (first_beat && d0_in_read_wrap_converter && fixed_new_burst && incr_new_burst) || (subsequent_compressed_beat && d1_in_read_wrap_converter) : 1'b0;
                assign enable_fixed_converter             = INSTANTIATE_DEFAULT_CONVERTER ? (first_beat && d0_in_default_converter && wrap_new_burst && incr_new_burst) || (subsequent_compressed_beat && d1_in_default_converter) : 1'b0;
            ...
            if (IS_INCR_SLAVE) begin
                ...
                wire                     in_incr        = ((in_bursttype == 2'b01) && !in_uncompressed_read) || (UNCMP_READ_ROUTE_TO_INCR_CONVERTER && in_uncompressed_read);
                wire                     in_narrow_incr = in_incr && in_narrow;
                wire                     in_wrap        = (in_bursttype == 2'b10);
                wire                     in_fixed       = (in_bursttype == 2'b00) || (in_bursttype == 2'b11);
   
                assign in_full_size_read_wrap    = INSTANTIATE_WRAP_CONVERTER ? (in_compressed_read && in_wrap && !in_narrow) : 1'b0;
                assign in_full_size_write_wrap   = INSTANTIATE_WRAP_CONVERTER ? ((in_wrap && !in_narrow) && (in_write || (UNCMP_READ_ROUTE_TO_WRAP_CONVERTER && in_uncompressed_read))) : 1'b0;
                assign in_full_size_incr         = INSTANTIATE_INCR_CONVERTER ? (in_incr && !in_narrow) : 1'b0;
                assign in_default_converter      = INSTANTIATE_DEFAULT_CONVERTER ? (in_fixed || in_narrow || in_narrow_incr || (UNCMP_READ_ROUTE_TO_DEF_CONVERTER && in_uncompressed_read)) : 1'b0;
                ...
                // II. First length calculation
                // --------------------------------------------------------------------------------------
                // Note: the slave is INCR slave, in practical is has no boundary so if in burst is wrap
                ...
                if (INSTANTIATE_WRAP_CONVERTER == 1) begin : first_len_incr_slave
                    ...
                    end // INCOMPLETE_WRAP_SUPPORT
                end // INSTANTIATE_WRAP_CONVERTER

                // -----------------------------------------------------------------------
                // III. Converter enable signals: Turn on/off each converter accordingly
                // -----------------------------------------------------------------------
                // INCR slave: three converters:
                //  1. wrap_burst_converter    -> handle WRAP
                //  2. incr_burst_convter     -> handle INCR
                //  2. default_burst_converter -> handle narrow burst
                // -----------------------------------------------------------------------

                assign enable_incr_converter               = INSTANTIATE_INCR_CONVERTER ? (first_beat && d0_in_full_size_incr && fixed_new_burst && wrap_new_burst) || (subsequent_compressed_beat && d1_in_full_size_incr) : 1'b0;
                assign enable_fixed_converter              = INSTANTIATE_DEFAULT_CONVERTER ? (first_beat && d0_in_default_converter && incr_new_burst && wrap_new_burst) || (subsequent_compressed_beat && d1_in_default_converter) : 1'b0;
                assign enable_uncompressed_wrap_converter  = INSTANTIATE_WRAP_CONVERTER ? (first_beat && d0_in_full_size_write_wrap && fixed_new_burst && incr_new_burst) : 1'b0;
                assign enable_compressed_wrap_converter    = INSTANTIATE_WRAP_CONVERTER ? (first_beat && d0_in_full_size_read_wrap  && fixed_new_burst && incr_new_burst) || (subsequent_compressed_beat && d1_in_full_size_read_wrap) : 1'b0;
    ...
    wire is_write                     = nb ? (d0_in_write) : d1_in_write;
    wire is_uncompressed_read         = nb ? (d0_in_uncompressed_read) : d1_in_uncompressed_read;
    wire incr_converter_is_uncompressed = UNCMP_READ_ROUTE_TO_INCR_CONVERTER ? (is_write || is_uncompressed_read) : is_write;
    ...
    // --------------------------------------------------
    generate
        if(INSTANTIATE_DEFAULT_CONVERTER) begin : def_burst_converter_gen
            alt_default_burst_converter
            #(
        ...
        end else begin : no_def_burst_converter_gen
            assign fixed_new_burst = 1'b1;
        end
        if(INSTANTIATE_INCR_CONVERTER) begin : incr_burst_converter_gen
            alt_incr_burst_converter
            #(
            ...
            (
                ...
                .in_addr                (d0_in_aligned_addr),
                .in_addr_reg            (d1_in_aligned_addr),
                .is_uncompressed        (incr_converter_is_uncompressed),
                .out_len                (incr_out_len),
                .uncompr_out_len        (incr_uncompr_out_len),
                .out_addr               (incr_out_addr),
                .new_burst_export       (incr_new_burst)
            );
        end else begin : no_incr_burst_converter_gen
            assign incr_new_burst = 1'b1;
        end
        if (INSTANTIATE_WRAP_CONVERTER) begin : wrap_burst_converter_gen
            alt_wrap_burst_converter
            #(
                .MAX_IN_LEN             (MAX_IN_LEN),
                .MAX_OUT_LEN            (MAX_OUT_LEN),
                .ADDR_WIDTH             (PKT_ADDR_W),
            ...
            (
                .clk                    (clk),
                .reset                  (reset),
                .enable_compressed      (enable_compressed_wrap_converter),
                .enable_uncompressed    (enable_uncompressed_wrap_converter),
                .in_len                 (d0_in_len),
        ...
        end else begin : no_wrap_burst_converter_gen
            assign wrap_new_burst = 1'b1;
        end
    endgenerate
    ...
    // There can be no NARROW transfers when the default converter isn't around! However the default converter may be present when
    // no narrow transfers are possible due to FIXED bursts, so IN_NARROW_SIZE == 1 is required to shut off BES in this case if the
    // transforms don't set BYTEENABLE_SYNTHESIS optimally. Better to avoid relying on external code which can change to be optimal.
    localparam BE_SYN_NEEDED = ((BYTEENABLE_SYNTHESIS == 1) && (INSTANTIATE_DEFAULT_CONVERTER == 1) && (IN_NARROW_SIZE == 1));
    
    always_comb begin
    if (BE_SYN_NEEDED && d1_in_narrow == 1 && (state == ST_COMP_TRANS))
...
module alt_hiconnect_burst_adapter_burstwrap_increment #(parameter WIDTH = 8)
  (
    input wire  [WIDTH - 1:0] mask,
    output wire [WIDTH - 1:0] inc
  );
    assign inc[0] = ~mask[0];

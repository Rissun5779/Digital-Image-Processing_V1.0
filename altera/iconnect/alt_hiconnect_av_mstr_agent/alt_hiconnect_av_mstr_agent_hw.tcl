...
# $Id: //acds/rel/18.1std/ip/iconnect/alt_hiconnect_av_mstr_agent/alt_hiconnect_av_mstr_agent_hw.tcl#1 $
# $Revision: #1 $
# $Date: 2018/07/18 $
# $Author: psgswbuild $
...
set_module_property NAME alt_hiconnect_av_mstr_agent
set_module_property DISPLAY_NAME "Avalon-MM Master Agent"
set_module_property DESCRIPTION "Translates Avalon-MM master transactions into Qsys command packets and translates the Qsys Avalon-MM slave response packets into Avalon-MM responses."
...
set_fileset_property synthesis_fileset TOP_LEVEL alt_hiconnect_av_mstr_agent

add_fileset simulation_fileset SIM_VERILOG synth_callback_procedure
set_fileset_property simulation_fileset TOP_LEVEL alt_hiconnect_av_mstr_agent

add_fileset vhdl_fileset SIM_VHDL synth_callback_procedure
set_fileset_property vhdl_fileset TOP_LEVEL alt_hiconnect_av_mstr_agent

proc synth_callback_procedure { entity_name } {
   add_fileset_file alt_hiconnect_av_mstr_agent.sv SYSTEM_VERILOG PATH "alt_hiconnect_av_mstr_agent.sv"
}
...
# | packet parameters
# |
add_packet_parameter PKT_ORI_BURST_SIZE         {original burst size}         114   3
add_packet_parameter PKT_RESPONSE_STATUS        {response status}             112   2
add_packet_parameter PKT_QOS                    {qos sideband}                108   4
add_packet_parameter PKT_DATA_SIDEBAND          {data sideband}               100   8
add_packet_parameter PKT_ADDR_SIDEBAND          {address sideband}            95    5
add_packet_parameter PKT_BURST_TYPE             {bursttype}                   93    2
add_packet_parameter PKT_CACHE                  {cache}                       89    4
add_packet_parameter PKT_THREAD_ID              {thread id}                   87    2
add_packet_parameter PKT_BURST_SIZE             {burstsize}                   84    3
add_packet_parameter PKT_TRANS_EXCLUSIVE        {exclusive transaction}       83    0
...
# |
add_interface req avalon_streaming start
set_interface_property req dataBitsPerSymbol 71
...
set_interface_property req symbolsPerBeat 1

set_interface_property req ASSOCIATED_CLOCK clk
set_interface_property req associatedReset clk_reset

add_interface_port req req_valid valid Output 1
add_interface_port req req_data data Output 71
add_interface_port req req_startofpacket startofpacket Output 1
add_interface_port req req_endofpacket endofpacket Output 1
# | 
# +-----------------------------------
...
proc elaborate {} {
   ...
   set_interface_property req dataBitsPerSymbol $st_data_width
   ...
   set_port_property req_data WIDTH_EXPR $st_data_width
   set_port_property req_data VHDL_TYPE std_logic_vector
   ...
   set_interface_assignment req merlin.packet_format $field_info

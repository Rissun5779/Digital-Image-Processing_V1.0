...
# $Id: //acds/rel/18.1std/ip/iconnect/alt_hiconnect_traffic_limiter/alt_hiconnect_traffic_limiter_hw.tcl#1 $
# $Revision: #1 $
# $Date: 2018/07/18 $
# $Author: psgswbuild $
...
source ../shared/iconnect_shared.tcl

init_mm_iconnect_module_property
set_module_property NAME alt_hiconnect_traffic_limiter
...
set_fileset_property synthesis_fileset TOP_LEVEL alt_hiconnect_traffic_limiter

add_fileset simulation_fileset SIM_VERILOG synth_callback_procedure
set_fileset_property simulation_fileset TOP_LEVEL alt_hiconnect_traffic_limiter

add_fileset vhdl_fileset SIM_VHDL synth_callback_procedure
set_fileset_property vhdl_fileset TOP_LEVEL alt_hiconnect_traffic_limiter

proc synth_callback_procedure { entity_name } {
   add_fileset_file alt_hiconnect_traffic_limiter.sv SYSTEM_VERILOG PATH "alt_hiconnect_traffic_limiter.sv"
   add_fileset_file alt_hiconnect_reorder_memory.sv SYSTEM_VERILOG PATH "alt_hiconnect_reorder_memory.sv"
   add_fileset_file alt_hiconnect_sc_fifo.sv SYSTEM_VERILOG PATH "../alt_hiconnect_sc_fifo/alt_hiconnect_sc_fifo.sv"
...
# | parameters
# | 
add_packet_parameter PKT_DEST_ID       {destination id}     6 1
add_packet_parameter PKT_SRC_ID        {source id}          5 1
add_packet_parameter PKT_BYTE_CNT      {byte count}         4 1
add_packet_parameter PKT_BYTEEN        {byteenable}         3 1
add_packet_parameter PKT_TRANS_POSTED  {posted transaction} 1 0
add_packet_parameter PKT_TRANS_WRITE   {write transaction}  0 0
...
add_parameter SUPPORTS_POSTED_WRITES INTEGER 1 0
set_parameter_property SUPPORTS_POSTED_WRITES DISPLAY_NAME {Hazard prevention: posted write support}
set_parameter_property SUPPORTS_POSTED_WRITES AFFECTS_ELABORATION false 
set_parameter_property SUPPORTS_POSTED_WRITES HDL_PARAMETER true
set_parameter_property SUPPORTS_POSTED_WRITES DISPLAY_HINT boolean
set_parameter_property SUPPORTS_POSTED_WRITES ALLOWED_RANGES 0:1
set_parameter_property SUPPORTS_POSTED_WRITES DESCRIPTION {Determines if the limiter will need to support posted write transactions. Mutually exclusive with SUPPORTS_NONPOSTED_WRITES. Only used if PREVENT_HAZARDS is on.}

add_parameter SUPPORTS_NONPOSTED_WRITES INTEGER 0 0
set_parameter_property SUPPORTS_NONPOSTED_WRITES DISPLAY_NAME {Hazard prevention: non-posted write support}
set_parameter_property SUPPORTS_NONPOSTED_WRITES AFFECTS_ELABORATION false 
set_parameter_property SUPPORTS_NONPOSTED_WRITES HDL_PARAMETER true
set_parameter_property SUPPORTS_NONPOSTED_WRITES DISPLAY_HINT boolean
set_parameter_property SUPPORTS_NONPOSTED_WRITES ALLOWED_RANGES 0:1
set_parameter_property SUPPORTS_NONPOSTED_WRITES DESCRIPTION {Determines if the limiter will need to support non-posted write transactions. Mutually exclusive with SUPPORTS_POSTED_WRITES. Only used if PREVENT_HAZARDS is on.}

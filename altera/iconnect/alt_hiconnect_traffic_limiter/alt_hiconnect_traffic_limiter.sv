...
// $Id: //acds/prototype/mm_s10/ip/merlin/altera_merlin_traffic_limiter/altera_merlin_traffic_limiter.sv#4 $
// $Revision: #4 $
// $Date: 2015/06/10 $
// $Author: jyeap $
...
module alt_hiconnect_traffic_limiter
#(
   parameter
      PKT_TRANS_POSTED           = 1,
      PKT_DEST_ID_H              = 0,
...
(
   ...
   input wire [ST_DATA_W-1 : 0]     cmd_sink_data,
   input wire [ST_CHANNEL_W-1 : 0]  cmd_sink_channel,
   ...
   input wire [ST_DATA_W-1 : 0]     rsp_sink_data,
   input wire [ST_CHANNEL_W-1 : 0]  rsp_sink_channel,

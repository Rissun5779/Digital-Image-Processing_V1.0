...
// *****************************************************************************
//
// Title: axi4stream_monitor_module
//

// import package for the axi4stream interface
import mgc_axi4stream_pkg::*;

interface mgc_axi4stream_monitor #(int AXI4_ID_WIDTH = 8, int AXI4_USER_WIDTH = 8, int AXI4_DEST_WIDTH = 18, int AXI4_DATA_WIDTH = 1024, int index = 0)
(
    input ACLK,
    input ARESETn,
    input TVALID,
...
`ifdef MODEL_TECH
  `ifdef _MGC_VIP_VHDL_INTERFACE
    `include "mgc_axi4stream_monitor.mti.svp"
  `endif
`endif
...
`ifdef _MGC_VIP_VHDL_INTERFACE
   ...
   generate
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4STREAM_VHD_CREATE_MONITOR_TRANSACTION]);
        ack_p[gg][AXI4STREAM_VHD_CREATE_MONITOR_TRANSACTION] = 1;
        ...
        ++circular_id;
        ack_p[gg][AXI4STREAM_VHD_CREATE_MONITOR_TRANSACTION] <= 0;
    end
...
`endif    // _MGC_VIP_VHDL_INTERFACE
    ...
    task automatic get_transfer(axi4stream_transaction trans, int index = 0, output bit last);
      ...
      `call_for_axi4stream_bfm(dvc_get_transfer)(
                ...
                `call_for_axi4stream_bfm(get_axi4stream__monitor_end)(),
                data,
                byte_type,
                last,
                trans.id,

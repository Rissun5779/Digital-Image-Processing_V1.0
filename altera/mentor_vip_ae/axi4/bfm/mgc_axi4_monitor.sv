...
`define call_for_axi4_bfm(XXX) axi4.XXX
    ...
    begin
        ...
        `call_for_axi4_bfm(set_config_max_outstanding_rd)(READ_ACCEPTANCE_CAPABILITY);
        `call_for_axi4_bfm(set_config_max_outstanding_wr)(WRITE_ACCEPTANCE_CAPABILITY);
        `call_for_axi4_bfm(set_config_max_outstanding_rw)(COMBINED_ACCEPTANCE_CAPABILITY);
        `call_for_axi4_bfm(set_config_is_issuing)(0);
    end
...
`endif    // _MGC_VIP_VHDL_INTERFACE
    ...
    task automatic get_rw_transaction(axi4_transaction trans);
        ...
        int unsigned lwrite_data_beats_delay[];
    ...
    task automatic get_read_data_phase(axi4_transaction trans, int index = 0);
        ...
        if (trans.resp_user.size() <= index)
          trans.resp_user = new[(index + 1)](trans.resp_user);
    ...
    task automatic get_write_data_phase(axi4_transaction trans, int index = 0, output bit last);
        ...
        if (trans.data_user.size() <= index)
          trans.data_user = new[(index + 1)](trans.data_user);
    ...
    task automatic get_write_response_phase(axi4_transaction trans);
        ...
        if (trans.resp_user.size() == 0)
          trans.resp_user = new[1];

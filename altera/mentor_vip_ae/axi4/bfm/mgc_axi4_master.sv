...
// *****************************************************************************
//
// Title: axi4_master_module
//

// import package for the axi4 interface
import mgc_axi4_pkg::*;

interface mgc_axi4_master #(
                            int AXI4_ADDRESS_WIDTH = 64,
...
(
    ...
    input  ARESETn,
    output AWVALID,
    output [((AXI4_ADDRESS_WIDTH) - 1):0]  AWADDR,
    output [2:0] AWPROT,
    output [3:0] AWREGION,
    output [7:0] AWLEN,
    output [2:0] AWSIZE,
    ...
    output [3:0] AWCACHE,
    output [3:0] AWQOS,
    output [((AXI4_ID_WIDTH) - 1):0]  AWID,
    output [((AXI4_USER_WIDTH) - 1):0]  AWUSER,
    input  AWREADY,
    output ARVALID,
    output [((AXI4_ADDRESS_WIDTH) - 1):0]  ARADDR,
    output [2:0] ARPROT,
    output [3:0] ARREGION,
    output [7:0] ARLEN,
    output [2:0] ARSIZE,
    ...
    output [3:0] ARCACHE,
    output [3:0] ARQOS,
    output [((AXI4_ID_WIDTH) - 1):0]  ARID,
    output [((AXI4_USER_WIDTH) - 1):0]  ARUSER,
    input  ARREADY,
    input  RVALID,
    ...
    input  [((AXI4_USER_WIDTH) - 1):0]  RUSER,
    output RREADY,
    output WVALID,
    output [((AXI4_WDATA_WIDTH) - 1):0]  WDATA,
    output [(((AXI4_WDATA_WIDTH / 8)) - 1):0]  WSTRB,
    output WLAST,
    output [((AXI4_USER_WIDTH) - 1):0]  WUSER,
    input  WREADY,
    input  BVALID,
    input  [1:0] BRESP,
    input  [((AXI4_ID_WIDTH) - 1):0]  BID,
    input  [((AXI4_USER_WIDTH) - 1):0]  BUSER,
    output BREADY
...
`ifdef MODEL_TECH
  `ifdef _MGC_VIP_VHDL_INTERFACE
    `include "mgc_axi4_master.mti.svp"
  `endif
`endif
...
`define call_for_axi4_bfm(XXX) axi4.XXX
    ...
    assign axi4.BID = BID;
    assign axi4.BUSER = BUSER;
    assign BREADY = axi4.BREADY;

    // Set this end to be TLM connected
    initial
    begin
        `call_for_axi4_bfm(axi4_set_master_abstraction_level)(0, 1);
        `call_for_axi4_bfm(set_config_enable_rlast)(USE_RLAST);
        `call_for_axi4_bfm(set_config_max_outstanding_rd)(READ_ISSUING_CAPABILITY);
        `call_for_axi4_bfm(set_config_max_outstanding_wr)(WRITE_ISSUING_CAPABILITY);
        `call_for_axi4_bfm(set_config_max_outstanding_rw)(COMBINED_ISSUING_CAPABILITY);
        `call_for_axi4_bfm(set_config_is_issuing)(1);
    end
...
`ifdef _MGC_VIP_VHDL_INTERFACE
   ...
   generate
    ...
    //------------------------------------------------------------------------------
    //
    // set_data_user hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_SET_DATA_USER]);
        ack_p[gg][AXI4_VHD_SET_DATA_USER] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
          transaction_array[transaction_id_in_p[gg]].set_data_user(value_in_max_p[gg], value_in0_p[gg]);
        end
        else
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_SET_DATA_USER", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_SET_DATA_USER] <= 0;
    end

    //------------------------------------------------------------------------------
    //
    // get_data_user hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_GET_DATA_USER]);
        ack_p[gg][AXI4_VHD_GET_DATA_USER] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
          value_out_max_p[gg] = transaction_array[transaction_id_in_p[gg]].get_data_user(value_in0_p[gg]);
        end
        else
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_GET_DATA_USER", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_GET_DATA_USER] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    //
    // set_resp_user hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_SET_RESP_USER]);
        ack_p[gg][AXI4_VHD_SET_RESP_USER] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
          transaction_array[transaction_id_in_p[gg]].set_resp_user(value_in_max_p[gg], value_in0_p[gg]);
        end
        else
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_SET_RESP_USER", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_SET_RESP_USER] <= 0;
    end

    //------------------------------------------------------------------------------
    //
    // get_resp_user hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_GET_RESP_USER]);
        ack_p[gg][AXI4_VHD_GET_RESP_USER] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
          value_out_max_p[gg] = transaction_array[transaction_id_in_p[gg]].get_resp_user(value_in0_p[gg]);
        end
        else
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_GET_RESP_USER", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_GET_RESP_USER] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_CREATE_WRITE_TRANSACTION]);
        ack_p[gg][AXI4_VHD_CREATE_WRITE_TRANSACTION] = 1;
        ...
        ++circular_id;
        ack_p[gg][AXI4_VHD_CREATE_WRITE_TRANSACTION] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_CREATE_READ_TRANSACTION]);
        ack_p[gg][AXI4_VHD_CREATE_READ_TRANSACTION] = 1;
        if((transaction_array[circular_id] != null) && (transaction_array[circular_id].transaction_done == 0))
        begin
        ...
        transaction_id_out_p[gg] = circular_id;
        ++circular_id;
        ack_p[gg][AXI4_VHD_CREATE_READ_TRANSACTION] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_TRANSACTION]);
        ack_p[gg][AXI4_VHD_EXECUTE_TRANSACTION] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
        ...
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_EXECUTE_TRANSACTION", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_EXECUTE_TRANSACTION] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_BURST]);
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_BURST] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
        ...
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_EXECUTE_WRITE_DATA_BURST", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_BURST] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_READ_ADDR_PHASE]);
        ack_p[gg][AXI4_VHD_EXECUTE_READ_ADDR_PHASE] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
        ...
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_EXECUTE_READ_ADDR_PHASE", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_EXECUTE_READ_ADDR_PHASE] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_WRITE_ADDR_PHASE]);
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_ADDR_PHASE] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
        ...
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_EXECUTE_WRITE_ADDR_PHASE", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_ADDR_PHASE] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_PHASE]);
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_PHASE] = 1;
        if(transaction_array[transaction_id_in_p[gg]] != null)
        begin
        ...
        begin
          $display("%0t: %m (%d): Error: Transaction with ID = %d is not created before AXI4_VHD_EXECUTE_WRITE_DATA_PHASE", $time, index, transaction_id_in_p[gg]);
        end
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_DATA_PHASE] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    //
    // get_read_data_cycle hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_GET_READ_DATA_CYCLE]);
        ack_p[gg][AXI4_VHD_GET_READ_DATA_CYCLE] = 1;
        get_read_data_cycle();
        ack_p[gg][AXI4_VHD_GET_READ_DATA_CYCLE] <= 0;
    end

    //------------------------------------------------------------------------------
    //
    // execute_read_data_ready hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_READ_DATA_READY]);
        ack_p[gg][AXI4_VHD_EXECUTE_READ_DATA_READY] = 1;
        execute_read_data_ready(value_in0_p[gg], value_in1_p[gg]);
        ack_p[gg][AXI4_VHD_EXECUTE_READ_DATA_READY] <= 0;
    end
    ...
    //------------------------------------------------------------------------------
    //
    // get_write_response_cycle hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_GET_WRITE_RESPONSE_CYCLE]);
        ack_p[gg][AXI4_VHD_GET_WRITE_RESPONSE_CYCLE] = 1;
        get_write_response_cycle();
        ack_p[gg][AXI4_VHD_GET_WRITE_RESPONSE_CYCLE] <= 0;
    end

    //------------------------------------------------------------------------------
    //
    // execute_write_resp_ready hook for VHDL environment.
    //
    //------------------------------------------------------------------------------
    initial forever begin
    @(posedge req_p[gg][AXI4_VHD_EXECUTE_WRITE_RESP_READY]);
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_RESP_READY] = 1;
        execute_write_resp_ready(value_in0_p[gg], value_in1_p[gg]);
        ack_p[gg][AXI4_VHD_EXECUTE_WRITE_RESP_READY] <= 0;
    end
...
`endif    // _MGC_VIP_VHDL_INTERFACE
    ...
    //------------------------------------------------------------------------------
    function automatic axi4_transaction create_write_transaction(input bit [((AXI4_ADDRESS_WIDTH) - 1):0]  addr, input bit [3:0] burst_length = 0);
      axi4_transaction trans   = new();
      trans.read_or_write                  = AXI4_TRANS_WRITE;
      trans.addr                           = addr;
      trans.prot                           = AXI4_NORM_SEC_DATA;
      trans.region                         = 0;
      trans.size                           = (AXI4_WDATA_WIDTH == 8)? AXI4_BYTES_1 :
                                             ((AXI4_WDATA_WIDTH == 16)? AXI4_BYTES_2 :
                                             ((AXI4_WDATA_WIDTH == 32)? AXI4_BYTES_4 :
                                             ((AXI4_WDATA_WIDTH == 64)? AXI4_BYTES_8 :
                                             ((AXI4_WDATA_WIDTH == 128)? AXI4_BYTES_16 :
                                             ((AXI4_WDATA_WIDTH == 256)? AXI4_BYTES_32 :
                                             ((AXI4_WDATA_WIDTH == 512)? AXI4_BYTES_64 : AXI4_BYTES_128))))));
      trans.burst                          = AXI4_INCR;
      trans.lock                           = AXI4_NORMAL;
      trans.cache                          = AXI4_NONMODIFIABLE_NONBUF;
      trans.qos                            = 0;
      trans.id                             = 0;
      ...
      trans.data_user                      = new[(burst_length + 1)];
      trans.resp                           = new[(burst_length + 1)];
      trans.resp_user                      = new[(burst_length + 1)];
      trans.address_valid_delay            = 0;
      trans.data_valid_delay               = new[(burst_length + 1)];
      trans.write_response_valid_delay     = 0;
      trans.address_ready_delay            = 0;
      trans.data_ready_delay               = new[(burst_length + 1)];
      trans.write_response_ready_delay     = 0;
    ...
    //------------------------------------------------------------------------------
    function automatic axi4_transaction create_read_transaction(input bit [((AXI4_ADDRESS_WIDTH) - 1):0]  addr,input bit [3:0] burst_length = 0);
      axi4_transaction trans   = new();
    ...
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the master transaction with ~axi4_transaction~ class handle as input.
    ...
    //-------------------------------------------------------------------------------------------------
    task automatic execute_transaction(axi4_transaction trans);
      if(trans.gen_write_strobes == 1'b1)
      begin
        bit [((AXI4_ADDRESS_WIDTH) - 1) : 0]     trans_addr;
        bit [(((AXI4_WDATA_WIDTH / 8)) - 1) : 0] trans_write_strobes[];
        int i;
        ...
        initiate_command_gen_write_strobes(
          trans_write_strobes,
          trans.burst,
          trans_addr,
          trans.burst_length,
    ...
    task automatic do_execute_transaction(axi4_transaction trans);
      if(trans.read_or_write == AXI4_TRANS_WRITE)
      begin
        if (trans.write_data_mode == AXI4_DATA_WITH_ADDRESS)
        begin
          fork
            do_execute_write_addr_phase(trans, AXI4_TRANSACTION_BLOCKING);
          join_none
        end
        else  // AXI4_DATA_AFTER_ADDRESS
          do_execute_write_addr_phase(trans, AXI4_TRANSACTION_BLOCKING);
          if(`call_for_axi4_bfm(get_config_axi4lite_interface)() == 1'b0) begin
            do_execute_write_data_burst(trans, AXI4_TRANSACTION_BLOCKING);
          end
          else begin
            do_execute_write_data_phase(trans, 0, AXI4_TRANSACTION_BLOCKING);
          end
        get_write_response_phase(trans);
      end
      else  // AXI4_TRANS_READ
      begin
        do_execute_read_addr_phase(trans, AXI4_TRANSACTION_BLOCKING);
        if(`call_for_axi4_bfm(get_config_axi4lite_interface)() == 1'b0) begin
          get_read_data_burst(trans);
        end
        else begin
          get_read_data_phase(trans,0);
        end
      end
    endtask
    ...
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the write_data_burst transaction with ~axi4_transaction~ class handle as input.
    // Based on operation_mode, this task initiates the transaction in blocking or non blocking mode.
    // Default operation mode is AXI4_TRANSACTION_BLOCKING.
    // On completion of transaction onto bus, this task sets the transaction_done to 1.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_write_data_burst(axi4_transaction trans);
      do_execute_write_data_burst(trans, trans.operation_mode);
    endtask

    task automatic do_execute_write_data_burst(axi4_transaction      trans,
        ...
        fork
          begin
            ...
            for (i = 0; i <  (trans.burst_length + 1); i++)
              do_execute_write_data_phase(trans, i, AXI4_TRANSACTION_BLOCKING);
      ...
      begin
        ...
        for (j = 0; j <  (trans.burst_length + 1); j++)
          do_execute_write_data_phase(trans, j, AXI4_TRANSACTION_BLOCKING);
    ...
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the read_addr_phase transaction with ~axi4_transaction~ class handle as input.
    // Based on operation_mode, this task initiates the transaction in blocking or non blocking mode.
    // Default operation mode is AXI4_TRANSACTION_BLOCKING.
    // On completion of transaction onto bus, this task sets the transaction_done to 1.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_read_addr_phase(axi4_transaction trans);
      do_execute_read_addr_phase(trans, trans.operation_mode);
    endtask

    task automatic do_execute_read_addr_phase(axi4_transaction      trans,
                                              axi4_operation_mode_e operation_mode = AXI4_TRANSACTION_BLOCKING);
      int tmp_address_valid_delay;

      repeat(trans.address_valid_delay) wait_on(AXI4_CLOCK_POSEDGE);

      if(operation_mode == AXI4_TRANSACTION_NON_BLOCKING)
      begin
        fork
          begin
            `call_for_axi4_bfm(dvc_put_read_addr_channel_phase)(
      ...
      begin
        do
          `call_for_axi4_bfm(dvc_get_read_channel_phase)(
            ...
            last,
            trans.id,
            trans.resp_user[index],
            trans.data_ready_delay[index],
            0, 1
          );
        while (trans.id != trans_id);

        trans.transaction_done = (`call_for_axi4_bfm(get_config_enable_rlast)()) ? last : (index == trans.burst_length) ? 1'b1 : 1'b0 ;
    ...
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the write_addr_phase transaction with ~axi4_transaction~ class handle as input.
    // Based on operation_mode, this task initiates the transaction in blocking or non blocking mode.
    // Default operation mode is AXI4_TRANSACTION_BLOCKING.
    // On completion of transaction onto bus, this task sets the transaction_done to 1.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_write_addr_phase(axi4_transaction trans);
      do_execute_write_addr_phase(trans, trans.operation_mode);
    endtask

    task automatic do_execute_write_addr_phase(axi4_transaction      trans,
                                               axi4_operation_mode_e operation_mode = AXI4_TRANSACTION_BLOCKING);
      int tmp_address_valid_delay;
      ...
      begin
        fork
          begin
            `call_for_axi4_bfm(dvc_put_write_addr_channel_phase)(
              QUESTA_MVC::QUESTA_MVC_COMMS_SENT,
              `call_for_axi4_bfm(get_axi4_master_end)(),
              trans.addr,
              trans.prot,
    ...
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the write_data_phase transaction with ~axi4_transaction~ class handle as input.
    // Based on operation_mode, this task initiates the transaction in blocking or non blocking mode.
    // Default operation mode is AXI4_TRANSACTION_BLOCKING.
    // On completion of transaction onto bus, this task sets the transaction_done to 1.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_write_data_phase(axi4_transaction trans, int index = 0, output bit last);
      do_execute_write_data_phase(trans, index, trans.operation_mode);
    endtask

    task automatic do_execute_write_data_phase(axi4_transaction      trans,
                                               int                   index = 0,
                                               axi4_operation_mode_e operation_mode = AXI4_TRANSACTION_BLOCKING
                                              );
      ...
      begin
        fork
          begin
            `call_for_axi4_bfm(dvc_put_write_channel_phase)(
              QUESTA_MVC::QUESTA_MVC_COMMS_SENT,
              `call_for_axi4_bfm(get_axi4_master_end)(),
              trans.data_words[index],
              trans.write_strobes[index],
              (trans.burst_length == index) ? 1'b1 : 1'b0,
              trans.data_user[index],
              trans.data_ready_delay[index]
            );
    ...
    //------------------------------------------------------------------------------
    //
    // Task: get_read_data_cycle()
    //
    //-------------------------------------------------------------------------------------------------
    // This task just waits for a read data/response cycle (1 clock cycle long in
    // which RVALID is asserted).
    //-------------------------------------------------------------------------------------------------
    task automatic get_read_data_cycle();
      bit [((AXI4_RDATA_WIDTH) - 1):0]  tmp_data;
      axi4_response_e tmp_resp;
      bit tmp_last;
      bit [((AXI4_ID_WIDTH) - 1):0]  tmp_id;
      bit [((AXI4_USER_WIDTH) - 1):0]  tmp_resp_user;

      `call_for_axi4_bfm(dvc_get_read_channel_cycle)(
        QUESTA_MVC::QUESTA_MVC_COMMS_RECEIVE,
        `call_for_axi4_bfm(get_axi4_master_end)(),
        tmp_data,
        tmp_resp,
    ...
    //------------------------------------------------------------------------------
    //
    // Task: execute_read_data_ready()
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the read_data_ready transaction with ready bit value as input.
    // This always runs in the blocking mode as it will always be 1 clock cycle long.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_read_data_ready(input bit ready, input bit non_blocking_mode = 0);
      if(non_blocking_mode)
      begin
        fork
        begin
          `call_for_axi4_bfm(dvc_put_read_channel_ready)(
            QUESTA_MVC::QUESTA_MVC_COMMS_SENT,
            `call_for_axi4_bfm(get_axi4_master_end)(),
            ready
          );
    ...
    //------------------------------------------------------------------------------
    //
    // Task: get_write_response_cycle()
    //
    //-------------------------------------------------------------------------------------------------
    // This task just waits for a write response cycle (1 clock cycle long in
    // which BVALID is asserted).
    //-------------------------------------------------------------------------------------------------
    task automatic get_write_response_cycle();
      ...
      `call_for_axi4_bfm(dvc_get_write_resp_channel_cycle)(
        QUESTA_MVC::QUESTA_MVC_COMMS_RECEIVE,
        `call_for_axi4_bfm(get_axi4_master_end)(),
        tmp_resp,
        tmp_id,
    ...
    //------------------------------------------------------------------------------
    //
    // Task: execute_write_resp_ready()
    //
    //-------------------------------------------------------------------------------------------------
    // This task initiates the write_resp_ready transaction with ready bit value as input.
    // This always runs in the blocking mode as it will always be 1 clock cycle long.
    //-------------------------------------------------------------------------------------------------
    task automatic execute_write_resp_ready(input bit ready, input bit non_blocking_mode = 0);
      if(non_blocking_mode)
      begin
        fork
        begin
          `call_for_axi4_bfm(dvc_put_write_resp_channel_ready)(
            QUESTA_MVC::QUESTA_MVC_COMMS_SENT,
            `call_for_axi4_bfm(get_axi4_master_end)(),
            ready
          );
    ...
    //------------------------------------------------------------------------------
    //
    // Function: initiate_command_gen_write_strobes()
    //
    //------------------------------------------------------------------------------
    //  This function is automatically called after randomization to correctly randomize the <write_strobes> parameter. This constraint is difficult to apply in the normal constraint language, as it is expensive for the constraint-solver.
    // 
    //------------------------------------------------------------------------------
    function automatic void initiate_command_gen_write_strobes(
    ...
    begin
        if ( read_or_write == AXI4_TRANS_WRITE )
        begin
            int transfer_count;
            int unsigned j;
            int write_strobes_len = write_strobes.size();
            ...
            int byte_width = ( AXI4_WDATA_WIDTH / 8 );
            int wrap_boundary;
            int unsigned channel_offset;
            bit aligned_transfer;
            bit [(((AXI4_WDATA_WIDTH / 8)) - 1):0]  ws_local;
            wrap_boundary = ( burst_length + 1 ) * size_bytes;
            channel_offset = ( addr % byte_width ) / wrap_boundary;
            aligned_transfer = ( ( addr % size_bytes ) == 0 );
            if ( `call_for_axi4_bfm(get_config_axi4lite_interface)() == 1'b0 )
            begin
                for ( transfer_count = 0 ; ( ( transfer_count < ( burst_length + 1 ) ) && ( transfer_count < write_strobes_len ) ) ; ++transfer_count )
                begin
                    bit [((AXI4_ADDRESS_WIDTH) - 1):0]  current_addr;
                    bit [((AXI4_ADDRESS_WIDTH) - 1):0]  next_addr;
                    ...
                    else if ( ( burst == AXI4_INCR ) || ( burst == AXI4_WRAP ) )
                    begin
                    ...
                    ws_local = write_strobes[transfer_count];
                    write_strobes[transfer_count] = 0;
                    ...
                    begin
                        ...
                        write_strobes[transfer_count][bit_pos] = 1'b1;
                    end
            ...
            begin
                // for axi4lite interface
                int unsigned aligned_address = addr - addr_offset;
                int unsigned lower_byte_lane = addr - ( addr / size_bytes ) * size_bytes;
                int unsigned upper_byte_lane = aligned_address + ( size_bytes - 1 ) - ( addr / size_bytes ) * size_bytes;
                int unsigned byte_lane;
                bit [(((AXI4_WDATA_WIDTH / 8)) - 1):0]  ws_local = write_strobes[0];
                write_strobes[0] = 0;
                for ( byte_lane = lower_byte_lane ; byte_lane <= upper_byte_lane ; ++byte_lane )
                begin
                    write_strobes[0][byte_lane] = 1'b1;
                end
                write_strobes[0] = write_strobes[0] & ws_local;
            end
        end
        return;
    end

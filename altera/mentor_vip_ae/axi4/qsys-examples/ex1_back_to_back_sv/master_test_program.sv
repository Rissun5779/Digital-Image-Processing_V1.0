...
begin
    ...
    ************************/    
    ...
    trans.set_write_strobes(4'b1111, 0);
    for(int i=1; i<8; i++)
        trans.set_write_strobes(4'b1111, i);
    ...
    bfm.execute_transaction(trans);
    if (trans.get_data_words(0) == 'hACE0ACE1)
      $display ( "@ %t, master_test_program: Read correct data (hACE0ACE1) at address (128)", $time);
    else
      $display ( "@ %t, master_test_program: Error: Expected data (hACE0ACE1) at address (128), but got %h", $time, trans.get_data_words(0));

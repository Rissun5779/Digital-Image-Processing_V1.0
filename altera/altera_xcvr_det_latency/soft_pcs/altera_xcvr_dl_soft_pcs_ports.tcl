...
proc detlat_static_native_ports { } {
	...
	custom_add_tagged_conduit_bus tx_bitslipboundaryselect input "lanes*7" {TXBitslip}
...
#  custom_add_tagged_conduit_bus rx_rlv output "lanes" {RXRunLength}
	custom_add_tagged_conduit_bus rx_bitslipboundaryselectout output "lanes*7" {RXAll}

	#conduit for soft PCS:
	custom_add_tagged_clock_bus cdr_ref_clk input "cdr_refclk_cnt" {NoTag}
	custom_add_tagged_clock_bus usr_pma_clk input 1 {NoTag}
	custom_add_tagged_clock_bus usr_clk input 1 {NoTag}
	custom_add_tagged_clock_bus fifo_calc_clk input 1 {NoTag}
	custom_add_tagged_conduit_bus rx_fiforeset input "lanes" {RXAll}
	custom_add_tagged_conduit_bus rx_fifocalreset input "lanes" {RXAll}
	custom_add_tagged_conduit_bus tx_fiforeset input "lanes" {TXAll}
	custom_add_tagged_conduit_bus tx_fifocalreset input "lanes" {TXAll}
	custom_add_tagged_conduit_bus rx_ph_acc_valid output "lanes" {RXAll}
	custom_add_tagged_conduit_bus rx_wr_full output "lanes" {RXAll}
	custom_add_tagged_conduit_bus rx_rd_empty output "lanes" {RXAll}
	custom_add_tagged_conduit_bus tx_ph_acc_valid output "lanes" {TXAll}
	custom_add_tagged_conduit_bus tx_wr_full output "lanes" {TXAll}
	custom_add_tagged_conduit_bus tx_rd_empty output "lanes" {TXAll}
	custom_add_tagged_conduit_bus rx_fifo_sample_size input "lanes*8" {RXAll}
	custom_add_tagged_conduit_bus rx_phase_measure_acc output "lanes*32" {RXAll}
	custom_add_tagged_conduit_bus rx_fifo_latency output "lanes*(rx_fifo_depth+1)" {RXAll}
	custom_add_tagged_conduit_bus tx_fifo_sample_size input "lanes*8" {TXAll}
	custom_add_tagged_conduit_bus tx_phase_measure_acc output "lanes*32" {TXAll}
	custom_add_tagged_conduit_bus tx_fifo_latency output "lanes*(tx_fifo_depth+1)" {TXAll}

	#conduit for auto reconfig:
	custom_add_tagged_conduit_bus data_width_pma input "lanes*data_width_pma_size" {NoTag}
	custom_add_tagged_conduit_bus error output "lanes" {NoTag}
}

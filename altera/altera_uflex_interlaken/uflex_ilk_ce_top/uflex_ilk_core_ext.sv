...
module uflex_ilk_core_ext #(
  ...
  parameter RAW_INTERNAL_WORDS = (INTERNAL_WORDS == 16) ? 12: INTERNAL_WORDS, // number of segment (words) in the raw interface
  parameter LOG_INTERNAL_WORDS = (INTERNAL_WORDS == 4 ) ? 3 :
                                 (INTERNAL_WORDS == 8 ) ? 4 : 5,
  parameter DUAL               = (INTERNAL_WORDS == 4 ) ? 1 : 2,
  parameter RX_DUAL_SEG        = 0,               //
  parameter TX_DUAL_SEG        = 0,               //
  parameter TX_PKTMOD_ONLY     = 1,               //
  parameter RXFIFO_ADDR_WIDTH  = (INTERNAL_WORDS == 4 ) ? 11 :
                                 (INTERNAL_WORDS == 8 ) ? 12 : 13,
  ...
  parameter IDLE_THRESHOLD     = (INTERNAL_WORDS == 4) ? 0 : 1, //0:low 4 high 8;1:low 8 high 16;2:low 16 high 24
  parameter M20K_ADDR_WIDTH    = (INTERNAL_WORDS == 4) ? 5 : 6, //default is 64 entries
  parameter TX_CREDIT_LATENCY  = (INTERNAL_WORDS == 16)? 12+ECC_ENABLE : 11+ECC_ENABLE //latency between tx_credit and tx_valid
)(
  // Clock and reset
  input                                pll_ref_clk,
  input                                reset_n,
  ...
  // TX User interface
  input  [INTERNAL_WORDS*64-1:0]       itx_din_words, // The following is generated with tx_usr_clock
  input  [LOG_INTERNAL_WORDS*DUAL-1:0] itx_num_valid, // [7:4] aligned with MSB. [3:0] aligned with middle
  input  [DUAL-1:0]                    itx_sop,       // tx_sop[0] is for Dual SOB mode
  input  [DUAL-1:0]                    itx_sob,       // tx_sob[0] is for Dual SOB mode. Start of the burst.
  input                                itx_eob,       // end of the burst
  input  [7:0]                         itx_chan,      // Channel number. Only valid during SOP or SOB cycles.
  input  [3:0]                         itx_eopbits,   // Number of the valid bytes.
  output                               itx_ready,
  input  [CALENDAR_PAGES*16-1:0]       itx_calendar,

  // RX User Interface
  output [INTERNAL_WORDS*64-1:0]       irx_dout_words,// The following is generated with rx_usr_clock
  output [LOG_INTERNAL_WORDS*DUAL-1:0] irx_num_valid, // [7:4] aligned with MSB. [3:0] aligned with middle
  output [DUAL-1:0]                    irx_sop,       // rx_sop[0] is for Dual SOB mode
  output [DUAL-1:0]                    irx_sob,       // rx_sob[0] is for Dual SOB mode. Start of the burst.
  ...
  output                               rx_overflow,    //Regroup buffer overflow
  output [1:0]                         eccstatus,      //ECC status ORed from all the M20K when ECC mode is enabled
  ...
  wire   [NUM_LANES-1:0]          tx_dll_lock; //S10
  ...
  wire   [RAW_INTERNAL_WORDS*64-1:0]  rx_raw_data;
  wire   [RAW_INTERNAL_WORDS-1:0]     rx_raw_data_ctrl;
  wire   [RAW_INTERNAL_WORDS-1:0]     rx_raw_crc24_err;

  wire   [1:0]                        itx_eccstatus;

  reg    crc24_err_r;
  assign crc24_err = crc24_err_r;
  always @(posedge rx_clk) crc24_err_r <= |rx_raw_crc24_err;
  ...
  uflex_ilk_dcore_ext #(
    ...
    .STRIPER            (STRIPER           ),
    .NUM_LANES          (NUM_LANES         ),
    .INTERNAL_WORDS     (RAW_INTERNAL_WORDS),
    .EXTERNAL_WORDS     (INTERNAL_WORDS    ),
    .RXFIFO_ADDR_WIDTH  (RXFIFO_ADDR_WIDTH ),
    .ECC_ENABLE         (ECC_ENABLE        ),
    .WRAM_NUM_DUP       (WRAM_NUM_DUP      ),
    .MLAB_ADDR_WIDTH    (MLAB_ADDR_WIDTH   ),
    ...
    .tx_ready_reset        (tx_ready             ), // input S10
    ...
    // TX User interface
    .tx_data               (itx_din_words        ), // input  [INTERNAL_WORDS*64-1:0]     
    .tx_num_valid          (itx_num_valid        ), // input                              
    .tx_sob                (itx_sob              ), // input  [INTERNAL_WORDS-1:0]        
    .tx_sop                (itx_sop              ), // input  [INTERNAL_WORDS-1:0]        
    .tx_eob                (itx_eob              ),
    .tx_chan               (itx_chan             ), // input  [INTERNAL_WORDS*16-1:0]      
    .tx_eopbits            (itx_eopbits          ), // input  [INTERNAL_WORDS*4-1:0]
    .tx_ready              (itx_ready            ), // output                             
    .tx_calendar           (itx_calendar         ), // input  [CALENDAR_PAGES*16-1:0]     

    // RX User Interface
    .rx_data               (irx_dout_words       ), // output [INTERNAL_WORDS*64-1:0]     
    .rx_num_valid          (irx_num_valid        ), // output                             
    .rx_sob                (irx_sob              ), // output [INTERNAL_WORDS-1:0]        
    .rx_sop                (irx_sop              ), // output [INTERNAL_WORDS-1:0]        
    .rx_eob                (irx_eob              ),
    .rx_chan               (irx_chan             ), // output [INTERNAL_WORDS*16-1:0]      
    .rx_eopbits            (irx_eopbits          ), // output [INTERNAL_WORDS*4-1:0]
    .rx_crc24_err          (irx_err              ), // output [INTERNAL_WORDS-1:0]    
    .rx_calendar           (irx_calendar         ), // output [CALENDAR_PAGES*16-1:0]     
    ...
    .rx_crc32_err          (crc32_err            ), // output [NUM_LANES-1:0]             
    .tx_overflow           (tx_overflow          ), // output                             
    .tx_underflow          (tx_underflow         ), // output                             
    .rxfifo_fill_level     (rxfifo_fill_level    ),
    .rg_overflow           (rx_overflow          ),
    .irx_eccstatus         (eccstatus            ),
    .itx_eccstatus         (itx_eccstatus        ),
  ...
  if (DEVICE_FAMILY == "Stratix 10") begin 
    uflex_ilk_reset_control inst_reset_control (
     ...
     assign tx_pll_powerdown = 1'b0;
  end else begin//A10 
    uflex_ilk_reset_control inst_reset_control (
      .clock                     (mm_clk                    ), //  input  wire                       clock.clk
      .reset                     (!reset_n                  ), //  input  wire                       reset.reset
      .pll_powerdown             (tx_pll_powerdown          ), //  output wire [0:0]         pll_powerdown.pll_powerdown
      .tx_analogreset            (tx_analogreset            ), //  output wire [7:0]        tx_analogreset.tx_analogreset
      .tx_digitalreset           (tx_digitalreset           ), //  output wire [7:0]       tx_digitalreset.tx_digitalreset
      .tx_ready                  (tx_ready                  ), //  output wire [7:0]              tx_ready.tx_ready
      .pll_locked                (tx_pll_locked             ), //  input  wire [0:0]            pll_locked.pll_locked
    ...
    */ 
    ...
    // `endif
      ...
      uflex_ilk_xcvr xcvr (
        ...
        .rx_prbs_err_clr          (rx_prbs_err_clr      ), // input  wire [7:0]    rx_prbs_err_clr.rx_prbs_err_clr
        .rx_prbs_done             (rx_prbs_done         ), // output wire [7:0]    rx_prbs_done.rx_prbs_done
        .rx_prbs_err              (rx_prbs_err          ), // output wire [7:0]    rx_prbs_err.rx_prbs_err
    ...
    `endif
      ...
      uflex_ilk_xcvr xcvr (
        .pll_powerdown            ({(NUM_LANES){tx_pll_powerdown}}), // input  wire [7:0]   pll_powerdown.pll_powerdown
        .tx_analogreset           (tx_analogreset                 ), // input  wire [7:0]   tx_analogreset.tx_analogreset
        .tx_digitalreset          (tx_digitalreset                ), // input  wire [7:0]   tx_digitalreset.tx_digitalreset
        .tx_serial_data           (tx_pin                         ), // output wire [7:0]   tx_serial_data.tx_serial_data

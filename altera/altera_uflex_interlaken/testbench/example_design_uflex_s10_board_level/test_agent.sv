...
module test_agent #(
   ...
   output [LOG_INTERNAL_WORDS*DUAL-1:0]   itx_num_valid,
   output                   [7:0]         itx_chan,
   output              [DUAL-1:0]         itx_sop,
   output                   [3:0]         itx_eopbits,
   output                                 itx_sob,
   output              [DUAL-1:0]         itx_eob,
   ...
   input  [LOG_INTERNAL_WORDS*DUAL-1:0]   irx_num_valid,
   input               [DUAL-1:0]         irx_sop,
   input               [DUAL-1:0]         irx_sob,
   input                                  irx_eob,
   input                    [7:0]         irx_chan,
   ...
   ilk_pkt_gen #(
      ...
      .clk                (usr_clk),
      .rx_lanes_aligned   (rx_lanes_aligned & tx_lanes_aligned),
      .itx_ready          (itx_ready),
      .send_data          (send_data),
      .reconfig_done      (reconfig_done_d),
      .tx_usr_srst        (~usr_rst_n),
   ...
   ilk_pkt_checker #(
      ...
      .tx_usr_srst          (~usr_rst_n),
      .rx_usr_srst          (~usr_rst_n),
      .error_clear          (error_clear),
      .rx_lanes_aligned     (rx_lanes_aligned),
      .rx_data              (irx_dout_words),
      .rx_valid             (irx_num_valid),
      .rx_sop               (irx_sop),
      .rx_sob               (irx_sob),
      .rx_chan              (irx_chan),
      .rx_eopbits           (irx_eopbits),
       .rx_calendar          (irx_calendar),
      .rx_crc32_err         ({ crc32_err & {NUM_LANES{rx_lanes_aligned}} & {NUM_LANES{tx_lanes_aligned}} }),
      .rx_crc24_err         (crc24_err & rx_lanes_aligned & tx_lanes_aligned),

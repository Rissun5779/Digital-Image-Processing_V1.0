...
module ilk_pkt_gen #(
   ...
   output  reg [LOG_INTERNAL_WORDS*DUAL-1:0]   tx_valid,
   output  reg [7:0]                           tx_chan,
   output  reg [DUAL-1:0]                      tx_sop,
   output  reg [3:0]                           tx_eopbits,
   output  reg [DUAL-1:0]                      tx_sob,
   output  reg                                 tx_eob, 
  ...
  reg [LOG_INTERNAL_WORDS*DUAL-1:0] tx_valid_nxt;
  // localparam [2:0]  SHIFT_MOUNT  = (INTERNAL_WORDS == 4 ) ? 3 : 7;

  generate 
    if (INTERNAL_WORDS ==4) begin : gen_4_word
	         ////////// begin of  4 word stage machine ///////////
          always @* begin
             nxt_state         = state;
             tx_sop_nxt        = tx_sop; 
		 ...
		 ////////// end of  4 word stage machine ///////////
	end else if (INTERNAL_WORDS ==8) begin : gen_8_word
	     //////////  begin of  8 word stage machine  ///////		  
            always @* begin
               nxt_state         = state;
               tx_sop_nxt        = tx_sop; 
               tx_eopbits_nxt    = {4{1'b0}}; 

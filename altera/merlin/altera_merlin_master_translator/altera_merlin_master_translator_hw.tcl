...
# $Id: //acds/rel/18.1std/ip/merlin/altera_merlin_master_translator/altera_merlin_master_translator_hw.tcl#1 $
# $Revision: #1 $
# $Date: 2018/07/18 $
# $Author: psgswbuild $
...
proc synth_callback_procedure { entity_name } {
   ...
   if { [ get_parameter_value SYNC_RESET ] == 0 } {
      add_fileset_file altera_merlin_master_translator.sv SYSTEM_VERILOG PATH "altera_merlin_master_translator.sv"
   } else {
      add_fileset_file altera_merlin_master_translator.sv SYSTEM_VERILOG PATH "altera_merlin_master_translator_sync.sv"
   }
}
...
add_parameter SYNC_RESET INTEGER 0
set_parameter_property SYNC_RESET AFFECTS_ELABORATION true
set_parameter_property SYNC_RESET AFFECTS_GENERATION true
set_parameter_property SYNC_RESET HDL_PARAMETER false
set_parameter_property SYNC_RESET DISPLAY_HINT BOOLEAN
set_parameter_property SYNC_RESET DESCRIPTION {0 means asynchronous resets, 1 means synchronous resets}
set_parameter_property SYNC_RESET DISPLAY_NAME {Use synchronous resets}

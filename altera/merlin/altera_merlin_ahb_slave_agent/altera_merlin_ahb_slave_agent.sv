...
#(
    ...
    parameter BURST_SIZE_W                = PKT_BURST_SIZE_H - PKT_BURST_SIZE_L + 1
...
(
    ...
    reg                     hwrite_reg;
    reg [BURST_SIZE_W-1:0]  hsize_reg;
    reg [3:0]               hprot_reg;
    ...
    wire[BURST_SIZE_W-1:0]  cmd_hsize;
    wire[3:0]               cmd_hprot;
    wire                    cmd_hwrite;
    ...
    wire [ADDR_WIDTH+BURST_SIZE_W-1:0]   addr_alignment_in;
    ...
    assign m0_HSIZE             = (accept_new_command)? cmd_hsize[2:0] : hsize_reg[2:0]; // Packet format has changeable burst size, but AHB does not.
    ...
    always @(posedge aclk, negedge aresetn) begin
        if (!aresetn) begin
            haddr_reg <= 'b0;
            hwrite_reg <= 'b0;
            hsize_reg <= HSIZE[BURST_SIZE_W-1:0];  //default to the common 4 byte
            hprot_reg <= 'b1001;
            be_asserted_reg <= '0;
        end 

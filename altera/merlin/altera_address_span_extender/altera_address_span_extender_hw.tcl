...
set_module_property DISPLAY_NAME "Address Span Extender"
...
set_parameter_property MASTER_ADDRESS_DEF DISPLAY_NAME "Sub-window offset"
...
set_parameter_property MASTER_ADDRESS_DEF ENABLED true
set_parameter_property MASTER_ADDRESS_DEF GROUP "Control Slave Properties"

add_parameter ENABLE_SLAVE_PORT BOOLEAN true
set_parameter_property ENABLE_SLAVE_PORT DISPLAY_NAME "Enable Slave Control Port"
set_parameter_property ENABLE_SLAVE_PORT DESCRIPTION  "MASTER_WINDOW set by parameter"
set_parameter_property ENABLE_SLAVE_PORT ALLOWED_RANGES "true,false"
set_parameter_property ENABLE_SLAVE_PORT GROUP "Control Slave Properties"
...
#
    ...
    set en_slave_port [get_parameter_value ENABLE_SLAVE_PORT]
    if { [get_parameter_value SUB_WINDOW_COUNT] > 1 && $en_slave_port == "false" } {
      send_message Error "Enable Slave Control Port must be set to true when the Number of sub-windows > 1."
    }
    if { [get_parameter_value SUB_WINDOW_COUNT] > 2 } {
       set_parameter_property MASTER_ADDRESS_DEF ENABLED false 
    } else {
       set_parameter_property MASTER_ADDRESS_DEF ENABLED true 
    }
#
...
#
    if { [ get_parameter_value ENABLE_SLAVE_PORT ] == "false" } {
       set_port_property avs_cntl_address    termination true
       set_port_property avs_cntl_address    termination_value 0
...
#
    ...
    set slave_var_span [ expr 1 << [expr $slave_byte_address_width_var-2 ] ]
#   Add check to verify Slave Address Span is multiple of Sub-window Offset
    set mod_val [expr [get_parameter_value MASTER_ADDRESS_DEF]  % $slave_var_span ]
    if { $mod_val != 0} {
      send_message Error "Sub-window offset must be a multiple of Slave Address Span. i.e. If Slave Address Span is set to 2 kilobytes, Sub-window offset must be a multiple 0x400."
    } 
#
...
# get ACSD version from QSYS file 
# use ACDS Version Short
# find terminate Parameter and set it to a new parameter name and set to opposite value 

# Set the Parameter List for Updates for ip_upgrade callback
# VERSION should be set to the ACDS release with update
set parameter_upgrade_map {
  {VERSION   OLD_PARAMETER            PARAMETER         VAL_MAP }\
  {15.0      TERMINATE_SLAVE_PORT     ENABLE_SLAVE_PORT {"true" "false"}  }\
  {15.0      TERMINATE_SLAVE_PORT     ENABLE_SLAVE_PORT {"false" "true"}  }\

}

# upgrade function 
# takes in parameter_upgrade_map as input, updates parameters
# and parameter values
# import some functions from alt_xcvr::ip_tcl::ip_module
# upgrade function is implemented as ip_upgrade function in ip_module 
# hw_tcl but need to update from all older ACDS releases < VERSION 
proc upgrade {ip_name ip_version old_params param_map } {
  ...
  if { [expr {$version_index == -1 || $parameter_index == -1 }] } {
  ...
  foreach {param_name param_value} $old_params {
    ...
    for {set i 1 } { $i < [llength $param_map] } { incr i } {
      ...
      # Check that IP version is older than current ACDS version, then upgrade
          ...
          if { $val_map != "NOVAL" } {
            foreach {old_map_val new_map_val} $val_map {
              if {[string compare $old_map_val $param_value] == 0} {
                ...
                send_message info "Upgrade from ${ip_name} v${ip_version} ${param_name}(${param_value}) -> ${new_param_name}(${new_param_value})."
              }
            }
          }
        }
    ...
    if { [lsearch $declared_param_list $new_param_name] == -1 } {
      send_message info "Upgrade from ${ip_name} v${ip_version}. Removing parameter ${new_param_name}"
      set new_param_name NOVAL
    }
    # Set the new parameter name and value unless it was removed
    if {$new_param_name != "NOVAL"} {
       ::alt_xcvr::ip_tcl::ip_module::ip_set "parameter.${new_param_name}.value" $new_param_value
    }
  }
}

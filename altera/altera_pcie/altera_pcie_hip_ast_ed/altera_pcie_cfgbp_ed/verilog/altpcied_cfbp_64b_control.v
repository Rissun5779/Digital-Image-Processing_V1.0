...
#(
  ...
  localparam RXAVL_WRITE      = 4'h2;
  localparam RXAVL_READ       = 4'h4;
  localparam RXAVL_WRITE_DONE = 4'h8;
 ...
 wire             is_cfgwr0, is_cfgrd0 , is_invalid_cfg0;
 wire             is_read;
 wire             is_write;
 wire             is_flush;
 wire             is_msg;
...
wire              rx_check_header;
 ...
 reg              rxcfg_pndg_reg;
 wire             rxcfg_req;
 wire             rx_get_header1, rx_get_header2;
...
always @(posedge Clk_i) begin
  ...
  assign addr_bit2     = is_rx_4dw_hdr ?   rx_st_data_reg[34] : rx_st_data_reg[2];
  assign addr_64b_msb  = {rx_st_data_reg[31: 0], rx_st_data_reg[63:AVALON_WADDR+32]};
  assign addr_32b_msb  =  rx_st_data_reg[31:AVALON_WADDR];
  assign rx_dwlen_raw  =  rx_st_data_reg[9:0];

  assign addr_64b_msb_reg  = {rx_header2_reg[31: 0], rx_header2_reg[63:AVALON_WADDR+32]};
  ...
  begin
    case(rx_state)
     ...
     RX_PENDING:
       ...
       RX_CLEAR_BUF:
         if ((rx_qword_counter == 1) & !(rx_st_eop_reg & rx_st_valid_reg))
            rx_nxt_state <= RX_WAIT_EOP;
         else if ((rx_qword_counter == 1) & rx_st_eop_reg & rx_st_valid_reg)
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i) begin
      ...
      rx_header2_reg    <= rx_st_data_reg[63: 0];
    end
end
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i) begin
      rx_bar_hit_reg    <= 1'b0;
      f1_hit_reg        <= 1'b0;
    end else if(latch_rx_header2_reg) begin
 ...
 always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i)
      ...
      rx_writedata_reg <= rx_st_data_reg[63:32];
...
always @(tx_state, cpl_data_available, tx_cpl_req, tx_st_ready_reg, addr_bit2_reg,
  ...
  begin
    case(tx_state)
      TX_IDLE :
        if( (cpl_data_available | tx_cpl_req )& tx_st_ready_reg)
            tx_nxt_state <= TX_SOP;
        else

...
wire  [63:AVALON_WADDR] addr_64b_msb_reg;
wire  [31:AVALON_WADDR] addr_32b_msb_reg;
wire                    is_rx_4dw_hdr_reg;

 reg                     rx_st_sop_reg;
 reg                     rx_st_eop_reg;
 reg [AVALON_WDATA-1:0]  rx_st_data_reg;
 reg                     rx_st_valid_reg;
 reg                     tx_st_ready_reg;

reg tx_st_sop_reg;
reg tx_st_eop_reg;
reg [1:0] tx_st_empty_reg;
reg [AVALON_WDATA-1:0] tx_st_data_reg;
reg tx_st_valid_reg;
wire tx_eop_sig;
  ...
  assign is_rx_4dw_hdr =  rx_st_data_reg[29];
  assign addr_bit2     = is_rx_4dw_hdr ?  rx_st_data_reg[98] : rx_st_data_reg[66];
  assign rx_dwlen_raw  = rx_st_data_reg[9:0];
  ...
  assign is_rx_4dw_hdr_reg =  rx_header1_reg[29];
  assign addr_64b_msb_reg  = {rx_header2_reg[31:0], rx_header2_reg[63:(32+AVALON_WADDR)]};
  assign addr_32b_msb_reg  = {rx_header2_reg[31:AVALON_WADDR]};


always @(posedge Clk_i) begin
   rx_st_sop_reg  <= RxStSop_i;
   rx_st_eop_reg  <= RxStEop_i;
   rx_st_data_reg <= RxStData_i;
   rx_st_valid_reg <= RxStValid_i;
  ...
  begin
    case(rx_state)
     ...
     RX_LATCH_DATA:
       ...
       if(rx_st_valid_reg & rx_st_eop_reg & (is_msg_wd & rx_dwlen <= 2 & ~addr_bit2_reg| is_flush_wr32 |is_flush_wr64) )
         rx_nxt_state <= RX_IDLE;
       else if(rx_st_valid_reg & rx_st_eop_reg & is_valid_write)
         rx_nxt_state <= RX_PENDING;
       else if(rx_st_valid_reg & rx_st_eop_reg & is_valid_cfgwr0)
         rx_nxt_state <= RX_CFG_PENDING;
       else if (~is_valid_write & rx_st_valid_reg & rx_st_eop_reg)
         rx_nxt_state <= RX_IDLE;
       else if((~is_valid_write & is_write & rx_st_valid_reg)| (is_msg_wd & rx_dwlen > 2) |(is_msg_wd & rx_dwlen == 2 & addr_bit2_reg) )
     ...
     RX_PENDING:
          if((wrena & ~rxm_wait_request) | tx_eop_sig)
           rx_nxt_state <= RX_IDLE;
          else
            rx_nxt_state <= RX_PENDING;
       ...
       RX_CLEAR_BUF:
         if ((rx_2qword_counter == 1) & !(rx_st_eop_reg & rx_st_valid_reg))
            rx_nxt_state <= RX_WAIT_EOP;
         else if ((rx_2qword_counter == 1) & rx_st_eop_reg & rx_st_valid_reg)
            rx_nxt_state <= RX_NXT_VALID1;
         else
            rx_nxt_state <= RX_CLEAR_BUF;
       RX_WAIT_EOP:
         if(rx_st_eop_reg & rx_st_valid_reg)
          rx_nxt_state <= RX_NXT_VALID1;
         else
           rx_nxt_state <= RX_WAIT_EOP;
        RX_NXT_VALID1:
          if (rx_st_sop_reg & rx_st_valid_reg)
            rx_nxt_state <= RX_CHECK_HEADER;
          else
            rx_nxt_state <= RX_IDLE;
       default:
...
assign latch_rx_header1     =  rx_state[2] & rx_st_valid_reg;   // RX_LATCH_HEADER1

assign rxcfg_pndg           =  rx_state[3];                 // RX_CFG_PENDING
//assign latch_rx_header2   =  rx_state[4] & rx_st_valid_reg;   // RX_LATCH_HEADER2
assign rx_check_header      =  rx_state[5];                 // RX_CHECK_HEADER
assign rx_get_write_data    =  rx_state[6];                 // RX_RD_DATA
assign rx_latch_write_data  =  rx_state[7] & rx_st_valid_reg;   // RX_LATCH_DATA
...
assign is_bar0_hit = is_rx_4dw_hdr_reg ? (f0_mem_en & (addr_64b_msb_reg ==  {f0_bar1,f0_bar0_msb})) :
                                         (f0_mem_en & (addr_32b_msb_reg ==   f0_bar0_msb)) ;

assign is_bar1_hit = is_rx_4dw_hdr_reg ? (f1_mem_en & (addr_64b_msb_reg ==  {f1_bar1,f1_bar0_msb})) :
                                         (f1_mem_en & (addr_32b_msb_reg ==   f1_bar0_msb)) ;
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i) begin
      ...
      rx_header1_reg    <= rx_st_data_reg[ 63: 0];
      rx_header2_reg    <= rx_st_data_reg[127:64];
    end
end
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i) begin
      rx_bar_hit_reg    <= 1'b0;
      f1_hit_reg        <= 1'b0;
    end else if(ld_header_reg) begin
 ...
 always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i)
      ...
      rx_writedata_reg <= (addr_bit2 & !is_rx_4dw_hdr ) ? rx_st_data_reg[127:96] : 32'h0;
    else if (rx_latch_write_data & addr_bit2_reg)
      rx_writedata_reg <= rx_3dw_header ? rx_st_data_reg[127:96] :  rx_st_data_reg[63:32];
    else if(rx_latch_write_data )
      rx_writedata_reg <= rx_st_data_reg[31:0];
    end
...
always @(posedge Clk_i or negedge Rstn_i)
  begin
    ...
    else if (ld_header) begin
       casex ({adjusted_amount_dec_dw, adjusted_amount_inc_dw})
         2'b10:   adjusted_data_dw_reg <= rx_dwlen_raw - 10'h1;
         2'b01:   adjusted_data_dw_reg <= rx_dwlen_raw + 10'h1;
         default: adjusted_data_dw_reg <= rx_dwlen_raw;
       endcase
    end
  end
...
always @(tx_state, cpl_data_available, tx_cpl_req, tx_st_ready_reg, addr_bit2_reg,
         cpl_with_data, is_valid_cfgrd0)
  begin
    case(tx_state)
      TX_IDLE :
        if( (cpl_data_available | tx_cpl_req )& tx_st_ready_reg) begin
...
assign tx_eop_sig = tx_state[3] | tx_state[2];


always @(posedge Clk_i)
begin
  tx_st_sop_reg <= tx_state[1] | tx_state[2];
  tx_st_eop_reg <= tx_state[3] | tx_state[2];
  tx_st_empty_reg[1:0] <= {1'b0,tx_eop_sig & (!addr_bit2_reg | is_cfgrd0) & cpl_with_data} ;
  tx_st_data_reg <= (tx_eop_sig & (!addr_bit2_reg | is_cfgrd0) & ~tx_cpl_req) ? {tx_st_data[63:0], tx_st_data[63:0]} : {tx_st_header2[63:0], tx_st_header1[63:0]} ;
  tx_st_valid_reg <= !tx_idle_st;
end


 assign TxStSop_o  =  tx_st_sop_reg;
 assign TxStEop_o  =  tx_st_eop_reg;
 assign TxStEmpty_o[1]  =  1'b0;
 assign TxStEmpty_o[0]  =  tx_st_empty_reg[0] ;
 assign TxStData_o      = tx_st_data_reg;
 assign TxStValid_o     = tx_st_valid_reg;

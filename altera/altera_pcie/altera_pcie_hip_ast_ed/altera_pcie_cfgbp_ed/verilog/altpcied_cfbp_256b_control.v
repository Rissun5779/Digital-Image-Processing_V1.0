  ...
  begin
    case(rx_state)
     ...
     RX_PENDING:
       ...
       RX_CLEAR_BUF:
         //if ((rx_4qword_counter == 1) & !(rx_st_eop_reg & rx_st_valid_reg))
         if ((data_counter == 1) & !(rx_st_eop_reg & rx_st_valid_reg))
            rx_nxt_state <= RX_WAIT_EOP;
         else if ((data_counter == 1) & rx_st_eop_reg & rx_st_valid_reg)
 ...
 always @(posedge Clk_i or negedge Rstn_i)
  begin
    if(~Rstn_i)
      ...
      rx_writedata_reg <= is_rx_4dw_hdr ? rx_st_data_reg[191:160] : rx_st_data_reg[127:96];
    else if(ld_header )
      rx_writedata_reg <= rx_st_data_reg[159:128];
    end

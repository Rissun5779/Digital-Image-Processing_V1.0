...
module pio_avmm_write_controller #(
  parameter         WIDTH =       128,
...
output reg  [3:0]            avmm_be_first_o, // byte enables for 1st Dword
output      [31:0]           avmm_data_out_o,
output                       avmm_write_o,
output      [13:0]           avmm_waddress_o
...
assign avmm_be_first_o    = be_first_i_r;
assign avmm_waddress_o    = waddress_r_o;
assign avmm_write_o       = (WIDTH == 64) ? ((dw3_i_r && !qword_alignment_i_r) ? mm_write_header_r : mm_write_data_r) : ((dw3_i_r && !qword_alignmentdw3_i_r) ? mm_write_header_r : mm_write_data_r); 
assign avmm_data_out_o    = (WIDTH == 64) ? ((qword_alignment_i_r) ? mem_data_i_r[31:0] : mem_data_i_r[63:32]) : ((dw3_i_r && !qword_alignmentdw3_i_r) ? mem_data_i_r[127:96] : ((dw4_i_r && !qword_alignmentdw4_i_r) ? mem_data_i_r[63:32] : mem_data_i_r[31:0] ));
  ...
  always_comb begin
    if (WIDTH == 64) begin
      case (state_64)        
        ...
        HEAD2 : begin          
          mm_write_data = 1'b0; 
          if (mem_eop_i && mem_valid_i) begin
            next_state_64 = HEAD1;
          end
    ...
    if (WIDTH == 128) begin
       case (state_128)
        ...
        HEAD : begin
          ...
          if (write_pkt_i && mem_sop_i && mem_valid_i && rx_st_bar_hit_i && mem_type_i) begin
            mm_write_header =1'b1;        
          end
          else begin
            mm_write_header =1'b0;

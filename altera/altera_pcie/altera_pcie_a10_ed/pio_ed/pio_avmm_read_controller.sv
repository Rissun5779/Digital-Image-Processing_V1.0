...
module pio_avmm_read_controller #(
  parameter         WIDTH =       64
...
output                                avmm_read_o,                            
output reg  [13:0]                    avmm_raddress_o,          // memory Interface
...
logic                                           sel_h1;
logic                                           compltion_qword_align_in;  /// given to the parser to help packet creation.
...
//assign avmm_raddress_o               = (WIDTH == 64) ? ((rx_st_sop_reg && rx_st_valid_i && dw3) ? {encode_bar,rx_st_data_i[12:2]} : {encode_bar,rx_st_data_i[44:34]}) :((!rx_st_data_i[29] && rx_st_sop_i && rx_st_valid_i) ? {encode_bar,rx_st_data_i[76:66]}: {encode_bar,rx_st_data_i[108:98]}) ; 
    ...
    always_ff @(posedge pld_clk_i) begin
      if (reset_i) begin
        ...
        if (mem_sop_i && mem_valid_i) begin
          ...
          avmm_read   <= (read_pkt_i & rx_st_bar_hit_i & mem_type_i);
        end

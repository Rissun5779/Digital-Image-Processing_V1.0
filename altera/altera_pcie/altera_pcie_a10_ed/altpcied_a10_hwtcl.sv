...
module altpcied_a10_hwtcl # (
   parameter MAX_NUM_FUNC_SUPPORT             = 1,
   parameter num_of_func_hwtcl                = 1,
   parameter device_family_hwtcl              = "Arria 10",
...
wire [1:0]  tx_st_empty_int;
   ...
   altpcie_reset_delay_sync #(
      .ACTIVE_RESET           (1),
      .WIDTH_RST              (1),
      .NODENAME               ("reset_status_altpcie_reset_delay_sync_altpcied_a10_hwtcl"),
      .LOCK_TIME_CNT_WIDTH    (1)
   ) reset_status_altpcie_reset_delay_sync_altpcied_a10_hwtcl(
      .clk         (pld_clk),
   ...
   altpcie_reset_delay_sync #(
      .ACTIVE_RESET           (0),
      .WIDTH_RST              (2),
      .NODENAME               ("app_rstn_altpcie_reset_delay_sync_altpcied_a10_hwtcl"),
      .LOCK_TIME_CNT_WIDTH    (1)
   ) app_rstn_altpcie_reset_delay_sync_altpcied_a10_hwtcl(
      .clk         (pld_clk),
...
generate begin : g_target
   wire [1:0] tx_st_empty_t;
   wire [1:0] tx_st_eop_t;

   if ((ast_width_hwtcl=="Avalon-ST 256-bit") && (IS_ROOTPORT == 0)) begin

   assign  tx_st_empty_int = (multiple_packets_per_cycle_hwtcl == 1'b0) ? {1'b0, tx_st_empty_t[1]} : tx_st_empty_t;
   assign  tx_st_eop   = (multiple_packets_per_cycle_hwtcl == 1'b0) ? {1'b0, tx_st_eop_t[1]} : tx_st_eop_t;
      ...
      altpcierd_ast256_downstream # (
         ...
         .tx_st_empty0           (tx_st_empty_t),
         .tx_st_eop0             (tx_st_eop_t),

...
proc add_rdpcie_port_clk {} {
   ...
   set enable_avst_reset_hwtcl  [ get_parameter_value enable_avst_reset_hwtcl ]
   if { $enable_avst_reset_hwtcl>0 } {
      add_interface           clr_st reset end coreclkout_hip
      add_interface_port      clr_st clr_st reset Input 1
      set_interface_property  clr_st synchronousEdges both
   } else {
      add_to_no_connect clr_st     1     In
   }
}
...
proc add_rdpcie_port_rst {} {
   ...
   if  { [ regexp Root $port_type_hwtcl ] } {
      ...
      add_to_no_connect sim_pipe_pclk_out     1     In
   }
...
proc add_rdpcie_port_ast_rx {} {
   ...
   set enable_avst_reset_hwtcl  [ get_parameter_value enable_avst_reset_hwtcl ]
   if { $enable_avst_reset_hwtcl>0 } {
      set_interface_property rx_st associatedReset clr_st
   }
...
proc add_rdpcie_port_ast_tx {} {
   ...
   if { $dataWidth == 128 } {
      ...
      add_to_no_connect tx_st_empty 2 Output
   }
   ...
   set enable_avst_reset_hwtcl  [ get_parameter_value enable_avst_reset_hwtcl ]
   if { $enable_avst_reset_hwtcl>0 } {
      set_interface_property tx_st associatedReset clr_st
   }
   ...
   if { $use_tx_cons_cred_sel_hwtcl==1} {
      add_interface_port tx_cred tx_cred_cons_select tx_cons_cred_sel Output 1
   } else {
      add_to_no_connect tx_cred_cons_select 1 Output
   }
...
proc add_rdpcie_port_lmi {} {
   ...
   if { $enable_lmi_hwtcl == 1 } {
      ...
      add_to_no_connect lmi_din  32 Output
      add_to_no_connect lmi_rden 1  Output
      add_to_no_connect lmi_wren 1  Output
      add_to_no_connect lmi_ack  1  Input
      add_to_no_connect lmi_dout 32 Input
      if { $number_of_func_hwtcl == 1 } {
         add_to_no_connect lmi_addr 12 Output
      } else {
         add_to_no_connect lmi_addr 15 Output
      }
...
####################################################################################################
#
# FPGA Dev kit specific IO
#
proc add_fpgadevkitboard {} {
   set enable_fpga_devkit_board [ get_parameter_value enable_fpga_devkit_board_hwtcl ]
   if { $enable_fpga_devkit_board == 1 } {
      add_interface devkit_pins conduit end
      add_interface_port devkit_pins free_clk free_clk                          Input 1
      add_interface_port devkit_pins req_compliance_pb req_compliance_pb        Input 1
      add_interface_port devkit_pins set_compliance_mode set_compliance_mode    Input 1
      add_interface_port devkit_pins lane_active_led lane_active_led            Output 4
      add_interface_port devkit_pins L0_led L0_led                              Output 1
      add_interface_port devkit_pins alive_led alive_led                        Output 1
      add_interface_port devkit_pins comp_led comp_led                          Output 1
      add_interface_port devkit_pins gen2_led gen2_led                          Output 1
      add_interface_port devkit_pins gen3_led gen3_led                          Output 1

      add_interface hip_currentspeed conduit end
      add_interface_port hip_currentspeed hip_currentspeed currentspeed  input 2

      add_interface hip_testin conduit end
      add_interface_port hip_testin hip_test_in test_in                          Output 32
      add_interface_port hip_testin hip_simu_mode_pipe simu_mode_pipe            Output 1

      add_interface hip_npor conduit end
      add_interface_port hip_npor hip_pin_perst  pin_perst  Output 1
      add_interface_port hip_npor hip_npor       npor            Output 1

      add_interface local_rstn conduit end
      add_interface_port local_rstn local_rstn local_rstn  input 1

      add_interface perstn conduit end
      add_interface_port perstn perstn perstn  input 1
   } else {
      add_to_no_connect free_clk 1 In
      add_to_no_connect req_compliance_pb 1 In
      add_to_no_connect set_compliance_mode 1 In
      add_to_no_connect hip_currentspeed 2 In
      add_to_no_connect local_rstn 1 In
      add_to_no_connect perstn 1 In
   }
}

  ...
  begin
    case(txavl_state)
      TXAVL_IDLE :
       ...
       else if((TxChipSelect_i & TxWrite_i & wr_fifos_ok & ~rx_only  & ~rxm_irq_sreg & MasterEnable_i & ~AddrTransDone_i))        // write cycle detected and fifo ok (cmd and wr_dat fifo)
          txavl_nxt_state <= TXAVL_WAIT_WRADDR;
        else if(TxChipSelect_i & TxRead_i & ~rx_only & reads_cntr < 8  & ~rxm_irq_sreg & MasterEnable_i & ~AddrTransDone_i)   // read cycle
          txavl_nxt_state <= TXAVL_WAIT_RDADDR;
        else
          txavl_nxt_state <= TXAVL_IDLE;

      TXAVL_WAIT_WRADDR :   // wait for address translation done
        if(AddrTransDone_i & wrdat_fifo_ok  & TxWrite_i & TxChipSelect_i) // address translation done signaled
      ...
      TXAVL_WAIT_RDADDR:
         if(AddrTransDone_i & cmd_fifo_ok) // addr translaled and fifo is ok
...
assign txready = sm_burst_data | (sm_idle & TxChipSelect_i & TxRead_i & reads_cntr < 8 & ~rxm_irq_sreg & MasterEnable_i & ~AddrTransDone_i) ; // do not assert ready when IRQ pending
...
assign burstdata_rise = AddrTransDone_i & sm_wait_wraddr;
...
assign lbe[3:0] = sm_msi? 4'b0000: (last_avl_hi_be_zero ^ last_avl_low_be_zero) & (dw_size == 2)? 4'h0 : last_avl_hi_be_zero? last_avlbe_reg[3:0] : last_avlbe_reg[7:4] ;
 ...
 // write header format
   // config write (always 32-bit in length)
      ...
      assign is_rd32 =  sm_rdheader & ~PCIeAddrSpace_i[0];
      assign is_rd64 =  sm_rdheader & PCIeAddrSpace_i[0];
      assign is_wr32 =  (sm_wrheader & ~PCIeAddrSpace_i[0]) | (sm_msi & MsiAddr_i[63:32] == 32'h0);
      assign is_wr64 =  (sm_wrheader & PCIeAddrSpace_i[0]) | (sm_msi & MsiAddr_i[63:32] != 32'h0);

      assign addr_low = sm_msi? MsiAddr_i[31:0] : PCIeAddr_i[31:0];
      assign addr_hi  = sm_msi? MsiAddr_i[63:32] : PCIeAddr_i[63:32];
...
assign pendingrd_fifo_wrreq = (sm_idle & TxChipSelect_i & TxRead_i & ~TxWaitRequest_o & reads_cntr < 8 & ~rxm_irq_sreg & ~AddrTransDone_i); // no read accepted when IRQ pending
...
assign reads_up       = (sm_idle & TxChipSelect_i & TxRead_i &  ~TxWaitRequest_o & reads_cntr < 8 & ~rxm_irq_sreg & ~AddrTransDone_i); // no read accepted when IRQ pending
...
assign rxm_irq_set  =( rxm_irq_rise|
                     ...
                     //& MsiCsr_i[0];  // Intx and Msi go through Cmd FiFO

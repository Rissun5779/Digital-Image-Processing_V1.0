...
// agreement for further details.
   ...
   task ebfm_cfg_rp_basic;
      ...
      begin  // ebfm_cfg_rp_basic
         ...
         tmp_slv[31:24] = 8'h0;
         // Wait so we know we did this (and previous item) successfully
         ebfm_cfgwr_imm_wait(bnm, dev, 0, 24, 4, tmp_slv,compl_status);
   ...
   task ebfm_display_link_status_reg;
      ...
      begin
       ...
       // retrain the link
       ebfm_cfgwr_imm_wait(RP_PRI_BUS_NUM,RP_PRI_DEV_NUM,0,144,2, 32'h0000_0060, compl_status);
         end
         else
         begin
         ...
         // Check EP link capability (Assume bnm=1, dev=1). Do this after link is up
         ebfm_cfgrd_wait(1, 1, fnc, PCIE_CAP_PTR + 12, 4, CFG_SCRATCH_SPACE, compl_status);
         ep_link_cap = shmem_read(CFG_SCRATCH_SPACE ,2);
         min_link_cap = (ep_link_cap < rp_link_cap)? ep_link_cap : rp_link_cap;

         if (link_sts[3:0] != min_link_cap[3:0]) // link is not at its full speed
         begin
            //tell BFM to retrain
         ...
         if (link_sts[3:0] != min_link_cap[3:0])
...
`endif
         ...
         // Try retraining the link before accessing the EP
         ebfm_cfgwr_imm_wait(RP_PRI_BUS_NUM,RP_PRI_DEV_NUM,0,144,2, 32'h0000_0020, compl_status);

...
// agreement for further details.
  ...
  //============================================
   ...
   `define      BENCH                         top_tb.dut_pcie_tb
   `define      ARI_SUPPORT                  `DUT.ari_support_hwtcl
   `define      SR_IOV_SUPPORT               `DUT.sr_iov_support_hwtcl
   `define      TOTAL_PF_COUNT               `DUT.total_pf_count_hwtcl
   `define      PF0_VF_COUNT                 `DUT.pf0_vf_count_hwtcl
   `define      PF1_VF_COUNT                 `DUT.pf1_vf_count_hwtcl
   `define      TOTAL_VF_COUNT               `DUT.total_vf_count_hwtcl
   `define      PF0_VF_MSIX_TBL_SIZE         `DUT.pf0_vf_msix_tbl_size_hwtcl
   `define      PF0_VF_MSIX_TBL_OFFSET       `DUT.pf0_vf_msix_tbl_offset_hwtcl
   `define      PF0_VF_MSIX_TBL_BIR          `DUT.pf0_vf_msix_tbl_bir_hwtcl
   `define      PF0_VF_MSIX_PBA_OFFSET       `DUT.pf0_vf_msix_pba_offset_hwtcl
   `define      PF0_VF_MSIX_PBA_BIR          `DUT.pf0_vf_msix_pba_bir_hwtcl
   `define      PF1_VF_MSIX_TBL_SIZE         `DUT.pf1_vf_msix_tbl_size_hwtcl
   `define      PF1_VF_MSIX_TBL_OFFSET       `DUT.pf1_vf_msix_tbl_offset_hwtcl
   `define      PF1_VF_MSIX_TBL_BIR          `DUT.pf1_vf_msix_tbl_bir_hwtcl
   `define      PF1_VF_MSIX_PBA_OFFSET       `DUT.pf1_vf_msix_pba_offset_hwtcl
   `define      PF1_VF_MSIX_PBA_BIR          `DUT.pf1_vf_msix_pba_bir_hwtcl
   `define      PF0_VF_DEVICE_ID             `DUT.pf0_vf_device_id_hwtcl
   `define      PF1_VF_DEVICE_ID             `DUT.pf1_vf_device_id_hwtcl
   `define      SYSTEM_PAGE_SIZES_SUPPORTED  `DUT.system_page_sizes_supported_hwtcl    
   `define      SRIOV2_EN                    `DUT.sriov2_en
  //=========================================
  // SRIOV defines for A10

  `define      gen123_lane_rate_mode_hwtcl        `DUT.lane_rate_hwtcl
  `define      msi_multi_message_capable_hwtcl    `DUT.pf0_msi_multi_message_capable_hwtcl
  `define      msi_64bit_addressing_capable_hwtcl `DUT.msi_64bit_addressing_capable_hwtcl
  `define      msi_support_hwtcl                  `DUT.pf_msi_support_hwtcl
  `define      enable_function_msix_support_hwtcl `DUT.pf_enable_function_msix_support_hwtcl
  `define      ep_ltssmstate                      `DUT.ltssmstate
  `define      ep_link_ctrl2_currentspeed         `DUT.hip_cfgbp_link2csr[3:0]
  `define      test_cseb_switch_hwtcl             `DUT.test_cseb_switch_hwtcl
  `define      current_speed                      `DUT.altpcie_a10_hip_pipen1b.currentspeed
   ...
   localparam PCIE_2EXT_CAP_BASE_ADDR     = 12'h300;
   localparam PCIE_LINK_CTL3_REG_ADDR     = PCIE_2EXT_CAP_BASE_ADDR + 4;

   localparam ARI_CAP_BASE_ADDR = `SRIOV2_EN ? 10'h160 : 10'h200;
   localparam ARI_CAP_REG_ADDR  = `SRIOV2_EN ? 10'h164 : 10'h204;

   localparam SR_IOV_CAP_BASE_ADDR = `SRIOV2_EN ? 10'h200 : 10'h240;
...
//=============================================
   ...
   task ebfm_display_rc_link_status_reg;
      ...
      begin
         ...
         exp_speed = TEST_AUTONOMOUS ? GEN1 : `ep_link_ctrl2_currentspeed;
         while (link_sts[3:0] != exp_speed)
         begin
             if (~TL_BFM_MODE) begin
         ...
         dummy = ebfm_display(EBFM_MSG_INFO, {"Wait for Link to enter L0 after negotiated to the expected speed of EP Target Link Speed (", himage4(`ep_link_ctrl2_currentspeed), "):"}) ;
...
task check_ari_cap;
   ...
   begin
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
     if (`SRIOV2_EN)
	    unused_result = ebfm_display(EBFM_MSG_INFO, {"Read ARI Enhanced Cap register at DW address 0x58h  = ", himage8(rdata)});
	 else
     ...
     if (rdata[19:16] != 4'h1)
        ...
        next_cap = `SRIOV2_EN ? 16'h200 : 16'h240; // SRIOV
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
     if (`SRIOV2_EN)
	    unused_result = ebfm_display(EBFM_MSG_INFO, {"Read ARI Cap and Control register at DW address 0x59h  = ", himage8(rdata)});
     else
...
task enum_sriov_cap;
   ...
   begin
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR-IOV Base register at DW address 0x80h  = ", himage8(rdata)});
     else
     ...
     if ((`SRIOV2_EN == 0) && (rdata[31:20] != 12'h300) && (ep_func == 0) && (`gen123_lane_rate_mode_hwtcl == "Gen3 (8.0 Gbps)"))
     ...
     else if ((`SRIOV2_EN == 1) && (rdata[31:20] != 12'h280) && (ep_func == 0) && (`gen123_lane_rate_mode_hwtcl == "Gen3 (8.0 Gbps)"))
         unused_result = ebfm_display(EBFM_MSG_ERROR_FATAL, {"ERROR : Expected SR-IOV Next Cap Pointer to point to Secondary PCIe Capability at byte address 0x280  -- Actual = 0x", himage8(rdata[31:20])});
    ...
    if (check_all) begin
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR-IOV Cap Register at DW address 0x81h  = ", himage8(rdata)});
     else
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR-IOV Cap Register at DW address 0x91h  = ", himage8(rdata)});
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_INITIAL_VF_COUNT_REG_ADDR at DW address 0x83h  = ", himage8(rdata)});
     else
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
     if (`SRIOV2_EN)
	   unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_NUM_VFS_REG_ADDR at DW address 0x84h  = ", himage8(rdata)});
	 else
    ...
    if (check_all) begin
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_VF_OFFSET_STRIDE_REG_ADDR at DW address 0x85h  = ", himage8(rdata)});
	 else
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_VF_DEVICE_ID_REG_ADDR at DW address 0x86h  = ", himage8(rdata)});
	 else
	   unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_VF_DEVICE_ID_REG_ADDR at DW address 0x96h  = ", himage8(rdata)});
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
       unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_SUPPORTED_PAGE_SIZES_REG_ADDR at DW address 0x87h  = ", himage8(rdata)});
	 else
	   unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_SUPPORTED_PAGE_SIZES_REG_ADDR at DW address 0x97h  = ", himage8(rdata)});
     ...
     rdata = shmem_read(CFG_SHARE_RD, dw_byte_length) ;
	 if (`SRIOV2_EN)
	    unused_result = ebfm_display(EBFM_MSG_INFO, {"Read SR_IOV_SYSTEM_PAGE_SIZE_REG_ADDR at DW address 0x88h  = ", himage8(rdata)});
     else
...
//============================================
   ...
   task cfg_wr_vf0_bars;
      ...
      begin
         ...
         dw_addr = `SRIOV2_EN ? 12'h89 : 12'h99;
         min_bar = 0;
...
//                                             himage8(exp_data), "  -----, ", "Actual = ", himage8(rdata)});
//            end else begin
...
//         //     unused_result = ebfm_display(EBFM_MSG_INFO, {"------> Read PCIE_LINK_CTL_STATUS_ADDR at addr = ", himage8(config_address), " with cur_link_speed = 0x", himage8(rdata[3:0])});
//         //  end

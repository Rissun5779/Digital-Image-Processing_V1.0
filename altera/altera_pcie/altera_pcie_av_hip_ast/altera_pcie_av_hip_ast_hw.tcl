...
source pcie_av_parameters.tcl
source pcie_av_port.tcl
...
# | module PCI Express
# |
set_module_property NAME altera_pcie_av_hip_ast
set_module_property DESCRIPTION "Arria V Hard IP for PCI Express Intel FPGA IP"
set_module_property VERSION 18.1
set_module_property GROUP "Interface Protocols/PCI Express"
set_module_property AUTHOR "Altera Corporation"
set_module_property DISPLAY_NAME "Arria V Hard IP for PCI Express Intel FPGA IP"
...
set_fileset_property synth TOP_LEVEL altpcie_av_hip_ast_hwtcl

# # +-----------------------------------
# # | SIM_ design files
#   |
add_fileset sim_vhdl SIM_VHDL proc_sim_vhdl
set_fileset_property sim_vhdl TOP_LEVEL altpcie_av_hip_ast_hwtcl

add_fileset sim_verilog SIM_VERILOG proc_sim_verilog
set_fileset_property sim_verilog TOP_LEVEL altpcie_av_hip_ast_hwtcl
...
proc elaboration_callback {} {
   global MAX_FUNC
   send_message info "Arria V Hard IP for PCI Express Intel FPGA IP v18.1"
...
set_parameter_property gen12_lane_rate_mode_hwtcl DESCRIPTION "Selects the maximum data rate of the link.Gen1 (2.5 Gbps) and Gen2 (5.0 Gbps) are supported."
...
set_parameter_property ast_width_hwtcl DESCRIPTION "Selects the width of the data interface between the transaction layer and the application layer implemented in the PLD fabric. The IP core supports interfaces of 64 or 128 bits."
...
proc proc_quartus_synth {name} {
   ...
   add_fileset_file altpcie_av_hip_ast_hwtcl.v                     VERILOG PATH        altpcie_av_hip_ast_hwtcl.v
   ...
   add_fileset_file altpcie_av_hip_128bit_atom.v                   VERILOG PATH        altpcie_av_hip_128bit_atom.v
...
proc proc_sim_vhdl {name} {
   ...
   set verilog_file { "altpcie_av_hip_ast_hwtcl.v" }
...
#  set CurrentDir      [pwd]
   ...
   if { [ regexp endpoint $port_type_hwtcl ] } {
      ...
      if { [ regexp x8 $lane_mask_hwtcl ] && [ regexp Gen1 $gen12_lane_rate_mode_hwtcl ] } {
         set qsysprj  "pcie_de_gen1_x8_ast128"
         add_fileset_file pcie_de_gen1_x8_ast128.qsys OTHER PATH ../altera_pcie_hip_ast_ed/example_design/av/pcie_de_gen1_x8_ast128.qsys
      } 
      
       if { [ regexp x4 $lane_mask_hwtcl ] && [ regexp Gen1 $gen12_lane_rate_mode_hwtcl ] } {
         set qsysprj  "pcie_de_gen1_x4_ast64"
         add_fileset_file pcie_de_gen1_x4_ast64.qsys OTHER PATH ../altera_pcie_hip_ast_ed/example_design/av/pcie_de_gen1_x4_ast64.qsys
      }
      
        if { [ regexp x4 $lane_mask_hwtcl ] && [ regexp Gen2 $gen12_lane_rate_mode_hwtcl ] } {
         set qsysprj  "pcie_de_gen2_x4_ast128"
         add_fileset_file pcie_de_gen2_x4_ast128.qsys OTHER PATH ../altera_pcie_hip_ast_ed/example_design/av/pcie_de_gen2_x4_ast128.qsys
      }
      ...
      if { [ regexp 128 $ast_width_hwtcl ] } {
         ...
         add_fileset_file pcie_de_rp_gen1_x8_ast128.qsys OTHER PATH ../altera_pcie_hip_ast_ed/example_design/av/pcie_de_rp_gen1_x8_ast128.qsys
      } else {
         set qsysprj  "pcie_de_rp_gen1_x4_ast64"
         add_fileset_file pcie_de_rp_gen1_x4_ast64.qsys OTHER PATH ../altera_pcie_hip_ast_ed/example_design/av/pcie_de_rp_gen1_x4_ast64.qsys
      }
   }
...
#      set qsyslib         pcielib
#
#      file copy ${CurrentDir}/../altera_pcie_hip_ast_ed/example_design/av/${qsysfile} ${tmpdir}${qsysfile}
...
add_documentation_link "User Guide" https://documentation.altera.com/#/link/lbl1415574515926/nik1410564766721

...
module altpcieav_256_rp_rxm_rdwr # (

      parameter AVMM_WIDTH                       = 128,
      parameter HPRXM_BAR_TYPE                   = 64,   //AVMM Address Width
      parameter BAR_NUMBER                       = 0,
      parameter DMA_BRST_CNT_W                   = 6,
      parameter BAR2_SIZE_MASK                   = 20,
      parameter RXF_DWIDTH                       = 274,
      parameter PNDG_HDR_SIZE                    = 70,   //65 for NON-SRIOV,  
      parameter SRIOV_EN                         = 1,    //SRIO ON
      parameter ROOT_PORT                        = 0,    //Root Port On then accept all Memory read and write transactions 
  ...
  (
     ...
     // Rx fifo Interface
      ...
      output logic  [PNDG_HDR_SIZE-1:0]            PndgRdHeader_o,
  ...
  localparam  RXM_RDSPLIT_WAIT_COUNT    = 3'h5;
   ...
   logic                                  is_wr_reg;
   logic   [RXF_DWIDTH-1:0]                        tlp_reg;
   logic   [RXF_DWIDTH-1:0]                        tlp_fifo;
   logic   [RXF_DWIDTH-1:0]                        tlp_hold_reg;
   logic   [(AVMM_WIDTH/8)-1:0]           avmm_fbe;
   logic   [9:0]                          first_dw_holes;
   logic   [(AVMM_WIDTH/8)-1:0]           avmm_fbe_reg;
   logic   [(AVMM_WIDTH/8)-1:0]           avmm_fbe_raw;
   logic   [(AVMM_WIDTH/8)-1:0]           avmm_fbe_raw_reg;
   ...
   logic   [HPRXM_BAR_TYPE+39:0]          rxm_cmd;                
   logic   [HPRXM_BAR_TYPE+39:0]          rxm_cmd_q;                 
   ...
   logic   [3:0]                          first_dw_ben;     
   logic   [3:0]                          last_dw_ben;  
   logic   [3:0]                          last_dw_ben_reg;      
   logic   [3:0]                          first_dw_ben_reg;      
   logic   [3:0]                          adjusted_last_dw_ben;     
   ...
   //SRIOV Support
     ...
     logic  [2:0]                           bar_number;
     logic  [PFCNT_WD-1:0]                  pf_number,pf_number_reg;
     logic  [VFCNT_WD-1:0]                  vf_number,vf_number_reg;
     logic                                  vf_active,vf_active_reg;  
     logic  [MFN_AWD-1:0]                   function_offset,function_offset_reg; 

 /// AVMM Read BE
    logic [31:0]             avmm_read_be; 
    logic [31:0]             avmm_read_be_reg;  
    logic [31:0]             avmm_read_be_delayed;  
    logic [31:0]             rxm_avmm_read_be;
    logic [31:0]             avmm_rd_be_addr0;
    logic [31:0]             avmm_rd_be_addr1; 
    logic [31:0]             avmm_rd_be_addr2; 
    logic [31:0]             avmm_rd_be_addr3;
    logic [31:0]             avmm_rd_be_addr4;
    logic [31:0]             avmm_rd_be_addr5;
    logic [31:0]             avmm_rd_be_addr6;
    logic [31:0]             avmm_rd_be_addr7;
    logic [31:0]             avmm_rd_be_addr0_reg2;
    logic [31:0]             avmm_rd_be_addr1_reg2; 
    logic [31:0]             avmm_rd_be_addr2_reg2; 
    logic [31:0]             avmm_rd_be_addr3_reg2;
    logic [31:0]             avmm_rd_be_addr4_reg2;
    logic [31:0]             avmm_rd_be_addr5_reg2;
    logic [31:0]             avmm_rd_be_addr6_reg2;
    logic [31:0]             avmm_rd_be_addr7_reg2;       
  ...
  assign rx_tlp_addr    = tlp_4dw_header? {RxFifoDataq_i[95:64], RxFifoDataq_i[127:98],2'b00} : {32'h0, RxFifoDataq_i[95:66], 2'b00};
  ...
  assign  last_dw_ben   = RxFifoDataq_i[39:36];
  assign  first_dw_ben  = RxFifoDataq_i[35:32];
  ...
  assign rd_tc        = RxFifoDataq_i[22:20];
   ...
   generate if(SRIOV_EN) begin
     assign bar_number = RxFifoDataq_i[RXF_DWIDTH-1-VFCNT_WD-PFCNT_WD-1:RXF_DWIDTH-1-VFCNT_WD-PFCNT_WD-1-3+1];
     assign pf_number  = RxFifoDataq_i[RXF_DWIDTH-1-VFCNT_WD-1:RXF_DWIDTH-1-VFCNT_WD-1-PFCNT_WD+1];
     assign vf_number  = RxFifoDataq_i[RXF_DWIDTH-1-1:RXF_DWIDTH-1-1-VFCNT_WD+1];
     assign vf_active  = RxFifoDataq_i[RXF_DWIDTH-1]; 
     assign function_offset = (MFN_AWD > PFCNT_WD) ? (vf_active ? (vf_number + PFCNT) : {{MFN_AWD-PFCNT_WD{1'b0}},pf_number}) : ( vf_active ? (vf_number + PFCNT) : pf_number);
     
     always_ff @ (posedge Clk_i)
      begin
       if(latch_header)
         begin
            pf_number_reg   <= pf_number ;
            vf_number_reg   <= vf_number ;
            vf_active_reg   <= vf_active ;
            function_offset_reg <= function_offset;
         end
      end     
      assign rxm_cmd          = {is_wr_reg, avmm_bcnt[6:0], function_offset_reg,rx_addr_reg[HPRXM_BAR_TYPE-MFN_AWD-1:0]};
      assign PndgRdHeader_o   = {vf_active_reg,vf_number_reg,pf_number_reg,1'b0,   4'hF,   rd_tc_reg,   rd_attr_reg,   4'hF,   rx_dwlen_reg,  req_id_reg[15:0],   1'b0,    rx_addr_reg[6:0],  rd_tag_reg, last_dw_ben_reg, first_dw_ben_reg};
      assign valid_bar_hit    = (bar_number == BAR_NUMBER);
      assign HPRxmAddress_o   = { {(HPRXM_BAR_TYPE-BAR2_SIZE_MASK ){1'b0}}, rxm_address_reg[HPRXM_BAR_TYPE-1:HPRXM_BAR_TYPE-MFN_AWD],rxm_address_reg[BAR2_SIZE_MASK - MFN_AWD - 1:0] };
    end 
    else begin
//      assign rxm_cmd          = {is_wr_reg, avmm_bcnt[6:0], rx_addr_reg[HPRXM_BAR_TYPE-1:0]};
      assign rxm_cmd = {avmm_read_be_reg, is_wr_reg, avmm_bcnt[6:0], rx_addr_reg[HPRXM_BAR_TYPE-1:0]};

      /// RXM Pending Read Interface
     //                              [51]   {50:47]   [50:48]      [47:46]      [45:42]   [41:32]           [31:16]         [15]     [14:8]                [7:0]
      assign PndgRdHeader_o   = {1'b0,   4'hF,   rd_tc_reg,   rd_attr_reg,   4'hF,   rx_dwlen_reg,  req_id_reg[15:0],   1'b0,    rx_addr_reg[6:0],  rd_tag_reg, last_dw_ben_reg, first_dw_ben_reg};
      assign valid_bar_hit    = (ROOT_PORT) ? 1'b1 : (bar_decode[BAR_NUMBER] & HPRXM_BAR_TYPE != 1);
     ...
     begin
       if(latch_header)
         begin
            ...
            rd_tc_reg           <= rd_tc;
            req_id_reg          <= req_id;
            last_dw_ben_reg     <= last_dw_ben;
            first_dw_ben_reg    <= first_dw_ben;
         end
      end
...
/// pipeline adder
        lpm_add_sub     LPM_DEST_ADD_SUB_component (
                                ...
                                .datab ({{HPRXM_BAR_TYPE-12{1'b0}},avmm_bcnt, 5'h0}),
  ...
  assign tlp_fifo[RXF_DWIDTH-1:0] = RxFifoDataq_i[RXF_DWIDTH-1:0];

 // pipe register
   always_ff @ (posedge Clk_i)
     begin
            tlp_reg[RXF_DWIDTH-1:0] <= tlp_fifo;
      end

  always_ff @ (posedge Clk_i)
     begin
          tlp_hold_reg[RXF_DWIDTH-1:0] <= tlp_reg;
     end
...
//Read BE: 
       /// BE mux for address starts at DW0
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr0[31:0] = {28'h0, first_dw_ben[3:0]};    
             3'h2:    avmm_rd_be_addr0[31:0] = {24'h0, last_dw_ben[3:0], first_dw_ben[3:0]};
             3'h3:    avmm_rd_be_addr0[31:0] = {20'h0, last_dw_ben[3:0], 4'hF, first_dw_ben[3:0]};
             3'h4:    avmm_rd_be_addr0[31:0] = {16'h0, last_dw_ben[3:0], 8'hFF, first_dw_ben[3:0]};
             3'h5:    avmm_rd_be_addr0[31:0] = {12'h0, last_dw_ben[3:0],12'hFFF, first_dw_ben[3:0]};
             3'h6:    avmm_rd_be_addr0[31:0] = {8'h0, last_dw_ben[3:0], 16'hFFFF, first_dw_ben[3:0]};
             3'h7:    avmm_rd_be_addr0[31:0] = {4'h0, last_dw_ben[3:0] ,20'hFFFFF, first_dw_ben[3:0]};
             default: avmm_rd_be_addr0[31:0] = {last_dw_ben[3:0] ,24'hFFFFFF, first_dw_ben[3:0]};  
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr0_reg2 <= avmm_rd_be_addr0;
          
       /// BE mux for address starts at DW1
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr1[31:0] = {24'h0, first_dw_ben[3:0], 4'h0};    
             3'h2:    avmm_rd_be_addr1[31:0] = {20'h0, last_dw_ben[3:0], first_dw_ben[3:0], 4'h0};
             3'h3:    avmm_rd_be_addr1[31:0] = {16'h0, last_dw_ben[3:0], 4'hF, first_dw_ben[3:0], 4'h0};
             3'h4:    avmm_rd_be_addr1[31:0] = {12'h0, last_dw_ben[3:0], 8'hFF, first_dw_ben[3:0], 4'h0};
             3'h5:    avmm_rd_be_addr1[31:0] = {8'h0, last_dw_ben[3:0],12'hFFF, first_dw_ben[3:0], 4'h0};
             3'h6:    avmm_rd_be_addr1[31:0] = {4'h0, last_dw_ben[3:0], 16'hFFFFF, first_dw_ben[3:0], 4'h0};
             default: avmm_rd_be_addr1[31:0] = {last_dw_ben[3:0] ,20'hFFFFF, first_dw_ben[3:0], 4'h0};    
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr1_reg2 <= avmm_rd_be_addr1;
           
       /// BE mux for address starts at DW2
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr2[31:0] = {20'h0, first_dw_ben[3:0], 8'h0};    
             3'h2:    avmm_rd_be_addr2[31:0] = {16'h0, last_dw_ben[3:0], first_dw_ben[3:0], 8'h0};
             3'h3:    avmm_rd_be_addr2[31:0] = {12'h0, last_dw_ben[3:0], 4'hF, first_dw_ben[3:0], 8'h0};
             3'h4:    avmm_rd_be_addr2[31:0] = {8'h0, last_dw_ben[3:0], 8'hFF, first_dw_ben[3:0], 8'h0};
             3'h5:    avmm_rd_be_addr2[31:0] = {4'h0, last_dw_ben[3:0],12'hFFF, first_dw_ben[3:0], 8'h0};
             default: avmm_rd_be_addr2[31:0] = {last_dw_ben[3:0] ,16'hFFFF, first_dw_ben[3:0], 8'h0};  
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr2_reg2 <= avmm_rd_be_addr2;
       
       
       /// BE mux for address starts at DW3
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr3[31:0] = {16'h0, first_dw_ben[3:0], 12'h0};    
             3'h2:    avmm_rd_be_addr3[31:0] = {12'h0, last_dw_ben[3:0], first_dw_ben[3:0], 12'h0};
             3'h3:    avmm_rd_be_addr3[31:0] = {8'h0, last_dw_ben[3:0], 4'hF, first_dw_ben[3:0], 12'h0};
             3'h4:    avmm_rd_be_addr3[31:0] = {4'h0, last_dw_ben[3:0], 8'hFF, first_dw_ben[3:0], 12'h0};
             default: avmm_rd_be_addr3[31:0] = {last_dw_ben[3:0] ,12'hFFF, first_dw_ben[3:0], 12'h0};  
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr3_reg2 <= avmm_rd_be_addr3;
              
       /// BE mux for address starts at DW4
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr4[31:0] = {12'h0, first_dw_ben[3:0], 16'h0};    
             3'h2:    avmm_rd_be_addr4[31:0] = {8'h0, last_dw_ben[3:0], first_dw_ben[3:0], 16'h0};
             3'h3:    avmm_rd_be_addr4[31:0] = {4'h0, last_dw_ben[3:0], 4'hF, first_dw_ben[3:0], 16'h0};
             default: avmm_rd_be_addr4[31:0] = {last_dw_ben[3:0] ,8'hFF, first_dw_ben[3:0], 16'h0};
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr4_reg2 <= avmm_rd_be_addr4;       
       
       
       /// BE mux for address starts at DW5
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr5[31:0] = {8'h0, first_dw_ben[3:0], 20'h0};    
             3'h2:    avmm_rd_be_addr5[31:0] = {4'h0, last_dw_ben[3:0], first_dw_ben[3:0], 20'h0};
             default: avmm_rd_be_addr5[31:0] = {last_dw_ben[3:0] ,4'hF, first_dw_ben[3:0], 20'h0};  
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr5_reg2 <= avmm_rd_be_addr5;                   
                    
       
       /// BE mux for address starts at DW6
         always_comb
           case(rx_tlp_dwlen[2:0])
             3'h1:    avmm_rd_be_addr6[31:0] = {4'h0, first_dw_ben[3:0], 24'h0};    
             default: avmm_rd_be_addr6[31:0] = {last_dw_ben[3:0], first_dw_ben[3:0], 24'h0};   
           endcase
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr6_reg2 <= avmm_rd_be_addr6;        
       
  /// BE mux for address starts at DW7
         
         assign avmm_rd_be_addr7[31:0] = {first_dw_ben[3:0], 28'h0};    
        
//           always_ff @ (posedge Clk_i)   
//                 avmm_rd_be_addr7_reg2 <= avmm_rd_be_addr7;      
                
                
    // BE mux based on starting address
//       always_comb
//            case(rx_tlp_addr[4:2])
//              3'h1 : avmm_read_be[31:0]   = avmm_rd_be_addr1_reg2;
//              3'h2 : avmm_read_be[31:0]   = avmm_rd_be_addr2_reg2;
//              3'h3 : avmm_read_be[31:0]   = avmm_rd_be_addr3_reg2;
//              3'h4 : avmm_read_be[31:0]   = avmm_rd_be_addr4_reg2;
//              3'h5 : avmm_read_be[31:0]   = avmm_rd_be_addr5_reg2;
//              3'h6 : avmm_read_be[31:0]   = avmm_rd_be_addr6_reg2;
//              3'h7 : avmm_read_be[31:0]   = avmm_rd_be_addr7_reg2;
//              default: avmm_read_be[31:0] = avmm_rd_be_addr0_reg2;   
//            endcase
      always_comb
           case(rx_tlp_addr[4:2])
             3'h1 : avmm_read_be[31:0]   = avmm_rd_be_addr1;
             3'h2 : avmm_read_be[31:0]   = avmm_rd_be_addr2;
             3'h3 : avmm_read_be[31:0]   = avmm_rd_be_addr3;
             3'h4 : avmm_read_be[31:0]   = avmm_rd_be_addr4;
             3'h5 : avmm_read_be[31:0]   = avmm_rd_be_addr5;
             3'h6 : avmm_read_be[31:0]   = avmm_rd_be_addr6;
             3'h7 : avmm_read_be[31:0]   = avmm_rd_be_addr7;
             default: avmm_read_be[31:0] = avmm_rd_be_addr0;   
           endcase

       always_ff @ (posedge Clk_i)                   
	 begin     
            avmm_read_be_reg <= (avmm_burst_cnt[7:0] > 8'h1)? 32'hFFFF_FFFF : avmm_read_be_delayed;  
	    avmm_read_be_delayed <= avmm_read_be; 
      end

//        always_ff @ (posedge Clk_i)                   
// 	begin     
//             avmm_read_be_reg <= (avmm_burst_cnt[7:0] > 8'h1)? 32'hFFFF_FFFF : avmm_read_be;
// 	    avmm_read_be_delayed <= avmm_read_be;
// 	end
       
    //Forming the data written to AVMM CMD FIFO
    //assign curr_burst_cnt = ((rxm_wr_state & wr_xfer_cnt == MAX_BCNT) | rxm_rd_split_state)? MAX_BCNT:
    //                        tlp_is_wr_reg ? wr_xfer_cnt : avmm_burst_cnt[4:0];
    
    //assign avmm_cmd_fifo_wrreq_d  = ((rxm_wr_state & wr_xfer_cnt == MAX_BCNT) | rxm_rd_split_state | rxm_cmd_wr_state);
    //assign avmm_cmd_fifo_wrdata_d = {avmm_read_be_reg,tlp_bar_reg, tlp_is_wr_reg, curr_burst_cnt, tlp_addr_reg[HPRXM_BAR_TYPE-1:5]};
  ...
  begin
    case(rx_tlp_addr[4:0])
        5'h0:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {28'h0000000, first_dw_ben} ;
                  10'h2 :  avmm_fbe[31:0] = {24'h000000, last_dw_ben, first_dw_ben};
                  10'h3 :  avmm_fbe[31:0] = {20'h00000, last_dw_ben, 4'hF, first_dw_ben};
                  10'h4 :  avmm_fbe[31:0] = {16'h0000, last_dw_ben, 8'hFF, first_dw_ben};
                  10'h5 :  avmm_fbe[31:0] = {12'h000, last_dw_ben, 12'hFFF, first_dw_ben};

                  10'h6 :  avmm_fbe[31:0] = {8'h00, last_dw_ben, 16'hFFFF, first_dw_ben};
                  10'h7 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, 20'hFFFFF, first_dw_ben};
                  10'h8 :  avmm_fbe[31:0] = {last_dw_ben, 24'hFFFFFF, first_dw_ben};
                  default: avmm_fbe[31:0] = {28'hFFFFFFF, first_dw_ben};
                endcase
        ...
        5'h4:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {24'h000000, first_dw_ben, 4'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {20'h00000, last_dw_ben, first_dw_ben, 4'h0};
                  10'h3 :  avmm_fbe[31:0] = {16'h0000, last_dw_ben, 4'hF, first_dw_ben, 4'h0};
                  10'h4 :  avmm_fbe[31:0] = {12'h000, last_dw_ben, 8'hFF, first_dw_ben, 4'h0};
                  10'h5 :  avmm_fbe[31:0] = {8'h00, last_dw_ben, 12'hFFF, first_dw_ben, 4'h0};
                  10'h6 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, 16'hFFFF, first_dw_ben, 4'h0};
                  10'h7 :  avmm_fbe[31:0] = {last_dw_ben, 20'hFFFFF, first_dw_ben, 4'h0};
                  default: avmm_fbe[31:0] = {24'hFFFFFF, first_dw_ben, 4'h0};
                endcase
         ...
         5'h8:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {20'h00000, first_dw_ben, 8'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {16'h0000, last_dw_ben, first_dw_ben, 8'h0};
                  10'h3 :  avmm_fbe[31:0] = {12'h000, last_dw_ben, 4'hF, first_dw_ben, 8'h0};
                  10'h4 :  avmm_fbe[31:0] = {8'h00, last_dw_ben, 8'hFF, first_dw_ben, 8'h0};
                  10'h5 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, 12'hFFF, first_dw_ben, 8'h0};
                  10'h6 :  avmm_fbe[31:0] = {last_dw_ben, 16'hFFFF, first_dw_ben, 8'h0};
                  default: avmm_fbe[31:0] = {20'hFFFFF, first_dw_ben, 8'h0};
                endcase
          ...
          5'hC:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {16'h0, first_dw_ben, 12'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {12'h0, last_dw_ben, first_dw_ben, 12'h0};
                  10'h3 :  avmm_fbe[31:0] = {8'h0, last_dw_ben, 4'hF, first_dw_ben, 12'h0};
                  10'h4 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, 8'hFF, first_dw_ben, 12'h0};
                  10'h5 :  avmm_fbe[31:0] = {last_dw_ben, 12'hFFF, first_dw_ben, 12'h0};
                  default: avmm_fbe[31:0] = {16'hFFFF, first_dw_ben, 12'h0};
                endcase
          ...
          5'h10:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {12'h0, first_dw_ben, 16'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {8'h0, last_dw_ben, first_dw_ben, 16'h0};
                  10'h3 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, 4'hF, first_dw_ben, 16'h0};
                  10'h4 :  avmm_fbe[31:0] = {last_dw_ben, 8'hFF, first_dw_ben, 16'h0};
                  default: avmm_fbe[31:0] = {12'hFFF, first_dw_ben, 16'h0};
                endcase
          ...
          5'h14:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {8'h0, first_dw_ben, 20'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {4'h0, last_dw_ben, first_dw_ben, 20'h0};
                  10'h3 :  avmm_fbe[31:0] = {last_dw_ben, 4'hF, first_dw_ben, 20'h0};
                  default: avmm_fbe[31:0] = {8'hFF, first_dw_ben, 20'h0};
                endcase
          ...
          5'h18:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {4'h0, first_dw_ben, 24'h0} ;
                  10'h2 :  avmm_fbe[31:0] = {last_dw_ben, first_dw_ben, 24'h0};
                  default: avmm_fbe[31:0] = {4'hF, first_dw_ben, 24'h0};
                endcase
         ...
         5'h1C:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[31:0] = {first_dw_ben, 28'h0} ;
                  default: avmm_fbe[31:0] = {first_dw_ben, 28'h0};
                endcase
  ...
  begin
    case(rx_tlp_addr[3:0])
        4'h0:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[15:0] = {12'h0, first_dw_ben} ;
                  10'h2 :  avmm_fbe[15:0] = {8'h0, last_dw_ben, first_dw_ben};
                  10'h3 :  avmm_fbe[15:0] = {4'h0, last_dw_ben, 4'hF, first_dw_ben};
                  10'h4 :  avmm_fbe[15:0] = {last_dw_ben, 8'hFF, first_dw_ben};
                  default: avmm_fbe[15:0] = {12'hF, first_dw_ben};
                endcase
                first_dw_holes <= 10'h0;
            end
        4'h4:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[15:0] = {8'h0, first_dw_ben, 4'h0} ;
                  10'h2 :  avmm_fbe[15:0] = {4'h0, last_dw_ben, first_dw_ben, 4'h0};
                  10'h3 :  avmm_fbe[15:0] = {last_dw_ben, 4'hF, first_dw_ben, 4'h0};
                  default: avmm_fbe[15:0] = {8'hF, first_dw_ben, 4'h0};
                endcase
                first_dw_holes <= 10'h1;
            end
       4'h8:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[15:0] = {4'h0, first_dw_ben, 8'h0} ;
                  10'h2 :  avmm_fbe[15:0] = {last_dw_ben, first_dw_ben, 8'h0};
                  default: avmm_fbe[15:0] = {4'hF, first_dw_ben, 8'h0};
                endcase
                first_dw_holes <= 10'h2;
            end
       4'hC:
            begin
                case (rx_tlp_dwlen)
                  10'h1 :  avmm_fbe[15:0] = {first_dw_ben, 12'h0} ;
                  default: avmm_fbe[15:0] = {first_dw_ben, 12'h0};
                endcase
                first_dw_holes <= 10'h3;
            end
   ...
   always_ff @ (posedge Clk_i)
     if(latch_header)
       begin
        ...
        avmm_fbe_raw_reg <= avmm_fbe_raw;
        adjusted_dw_count_reg <=  rx_tlp_dwlen[9:0] + first_dw_holes[7:0]; 
        adjusted_last_dw_ben <= last_dw_ben; 
       end
...
generate if(AVMM_WIDTH ==256)
  begin
      always_comb
       begin
        case(adjusted_dw_count_reg[2:0])
          3'h1 : adjusted_avmm_lbe <= {28'h0, adjusted_last_dw_ben};
          3'h2 : adjusted_avmm_lbe <= {24'h0, adjusted_last_dw_ben, 4'hF};
          3'h3 : adjusted_avmm_lbe <= {20'h0, adjusted_last_dw_ben, 8'hFF};
          3'h4 : adjusted_avmm_lbe <= {16'h0, adjusted_last_dw_ben, 12'hFFF};
          3'h5 : adjusted_avmm_lbe <= {12'h0, adjusted_last_dw_ben, 16'hFFFF};
          3'h6 : adjusted_avmm_lbe <= {8'h0, adjusted_last_dw_ben, 20'hFFFFF};
          3'h7 : adjusted_avmm_lbe <= {4'h0, adjusted_last_dw_ben, 24'hFFFFFF};
          default:adjusted_avmm_lbe <= {adjusted_last_dw_ben, 28'hFFFFFFF};
        endcase 
       end  
  end
else
  begin
      always_comb
       begin
        case(adjusted_dw_count_reg[1:0])
          2'h1 : adjusted_avmm_lbe <= {12'h0, adjusted_last_dw_ben};
          2'h2 : adjusted_avmm_lbe <= {8'h0, adjusted_last_dw_ben, 4'hF};
          2'h3 : adjusted_avmm_lbe <= {4'h0, adjusted_last_dw_ben, 8'hFF};
          default:adjusted_avmm_lbe <= {adjusted_last_dw_ben, 12'hFFF};
        endcase 
       end      
  end           
endgenerate        
...
always_comb
  begin
    case(rx_tlp_addr[4:0])
        5'h0:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h0000_000F;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h0000_00FF;
              10'h3 :  avmm_fbe_raw[31:0] = 32'h0000_0FFF;
              10'h4 :  avmm_fbe_raw[31:0] = 32'h0000_FFFF;
              10'h5 :  avmm_fbe_raw[31:0] = 32'h000F_FFFF;
              10'h6 :  avmm_fbe_raw[31:0] = 32'h00FF_FFFF;
              10'h7 :  avmm_fbe_raw[31:0] = 32'h0FFF_FFFF;
              default: avmm_fbe_raw[31:0] = 32'hFFFF_FFFF;
            endcase
        5'h4:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h0000_00F0;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h0000_0FF0;
              10'h3 :  avmm_fbe_raw[31:0] = 32'h0000_FFF0;
              10'h4 :  avmm_fbe_raw[31:0] = 32'h000F_FFF0;
              10'h5 :  avmm_fbe_raw[31:0] = 32'h00FF_FFF0;
              10'h6 :  avmm_fbe_raw[31:0] = 32'h0FFF_FFF0;
              10'h7 :  avmm_fbe_raw[31:0] = 32'hFFFF_FFF0;
              default: avmm_fbe_raw[31:0] = 32'hFFFF_FFF0;
            endcase
         5'h8:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h0000_0F00;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h0000_FF00;
              10'h3 :  avmm_fbe_raw[31:0] = 32'h000F_FF00;
              10'h4 :  avmm_fbe_raw[31:0] = 32'h00FF_FF00;
              10'h5 :  avmm_fbe_raw[31:0] = 32'h0FFF_FF00;
              10'h6 :  avmm_fbe_raw[31:0] = 32'hFFFF_FF00;
              default: avmm_fbe_raw[31:0] = 32'hFFFF_FF00;
            endcase
          5'hC:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h0000_F000;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h000F_F000;
              10'h3 :  avmm_fbe_raw[31:0] = 32'h00FF_F000;
              10'h4 :  avmm_fbe_raw[31:0] = 32'h0FFF_F000;
              10'h5 :  avmm_fbe_raw[31:0] = 32'hFFFF_F000;
              default: avmm_fbe_raw[31:0] = 32'hFFFF_F000;
            endcase
          5'h10:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h000F_0000;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h00FF_0000;
              10'h3 :  avmm_fbe_raw[31:0] = 32'h0FFF_0000;
              10'h4 :  avmm_fbe_raw[31:0] = 32'hFFFF_0000;
              default: avmm_fbe_raw[31:0] = 32'hFFFF_0000;
            endcase
          5'h14:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h00F0_0000;
              10'h2 :  avmm_fbe_raw[31:0] = 32'h0FF0_0000;
              10'h3 :  avmm_fbe_raw[31:0] = 32'hFFF0_0000;
              default: avmm_fbe_raw[31:0] = 32'hFFF0_0000;
            endcase
          5'h18:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'h0F00_0000;
              10'h2 :  avmm_fbe_raw[31:0] = 32'hFF00_0000;
              default: avmm_fbe_raw[31:0] = 32'hFF00_0000;
            endcase
         5'h1C:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[31:0] = 32'hF000_0000;
              default: avmm_fbe_raw[31:0] = 32'hF000_0000;
            endcase
       default: avmm_fbe_raw[31:0] = 32'hFFFF_FFFF;
      endcase
  end
  ...
  begin
    case(rx_tlp_addr[3:0])
        4'h0:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[15:0] = 16'h000F;
              10'h2 :  avmm_fbe_raw[15:0] = 16'h00FF;
              10'h3 :  avmm_fbe_raw[15:0] = 16'h0FFF;
              default: avmm_fbe_raw[15:0] = 16'hFFFF;
            endcase
        4'h4:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[15:0] = 16'h00F0;
              10'h2 :  avmm_fbe_raw[15:0] = 16'h0FF0;
              default: avmm_fbe_raw[15:0] = 16'hFFF0;
            endcase
       4'h8:
            case (rx_tlp_dwlen)
              10'h1 :  avmm_fbe_raw[15:0] = 16'h0F00;
              default: avmm_fbe_raw[15:0] = 16'hFF00;
            endcase

       default: avmm_fbe_raw[15:0] = 16'hF000;

    endcase
  end
end
endgenerate // avmm_fbe_raw logic
...
altpcie_256_rp_fifo
   #(
    .FIFO_DEPTH(16),    
    .DATA_WIDTH(32+8+HPRXM_BAR_TYPE)   /// read_be, address, burst count, write/read 
    )
 rxm_cmd_fifo
(
      .clk(Clk_i),
      .rstn(Rstn_i),
      .srst(1'b0),
      .wrreq(hprxm_wr_pipe_state_reg | hprxm_rd_pipe_state_reg & (rx_dwlen_reg != 0 & avmm_burst_cnt_reg <= 16) | rxm_rdsplit_send_first | rxm_rdsplit_send_max | rxm_rdsplit_send_last),
...
assign rxm_avmm_burst_count = rxm_cmd_q[HPRXM_BAR_TYPE+DMA_BRST_CNT_W-1:HPRXM_BAR_TYPE];
     ...
     begin
       if(rxm_cmd_fifo_rdreq)
         begin
             ...
             rxm_avmm_read_be <= rxm_cmd_q[HPRXM_BAR_TYPE+39:HPRXM_BAR_TYPE+8];
         end
      end
...
assign HPRxmByteEnable_o  = rxm_avmm_write_state? rxm_write_data[(AVMM_WIDTH+AVMM_WIDTH/8)-1:AVMM_WIDTH] : rxm_avmm_read_be; 
...
always_comb
  begin
    ...
    always_ff @ (posedge Clk_i)
     begin
      if(hprxm_pipe_state)
        rd_split_remain_cntr <=  avmm_burst_cnt[6:0];
      else if(rxm_rdsplit_send_first)
         rd_split_remain_cntr <=  rd_split_remain_cntr - 1'b1;
  ...
  assign rdcpl_cred_used = (rx_dwlen_reg == 10'h0)? 4'h8 : {1'b0,rx_dwlen_reg[9:7]};

    always_ff @ (posedge Clk_i )
     begin
      if(~Rstn_i)
        cpl_space_credit <=  6'd32;

...
input  logic  [3:0]                            LaneAct_i
  ...
  localparam ROOT_PORT                         = (port_type_hwtcl == "Root port" ) ? 1 : 0;     
  ...
  if(enable_cra_hwtcl == 1) begin
   ...
   // Currently this interface is not used
    ...
    // Rp mode interface
        ...
        .AvalonIrqReq_i          ({rx_fifo_rdreq_from_cra,RxIntStatus_i}),
   ...
   //send rd_request when rx fifo is not empty and TLP  tag is between 16-31 & completion packet only
   
   assign is_compl = rx_fifo_dataq[256] & (rx_fifo_dataq[27:24] == 4'hA);
   assign rp_rx_tlp_rdreq = ((rx_fifo_count != 0) & (rx_fifo_dataq[77:76] == 2'b01)) & (~rx_fifo_rdreq_from_cra) & is_compl;

 ...
 # (
  ...
  (
    ...
    // interface to the Rx pending read FIFO
      ...
      input  logic   [PNDG_HDR_SIZE-1:0]           PndgRdFifoDato_i,
      ...
      output logic   [CMDF_DWIDTH-1:0]             CmdFifoDatin_o,
...
localparam      TXRESP_IDLE          = 4'h0;
localparam      TXRESP_RD_FIFO       = 4'h1;
localparam      TXRESP_LD_BCNT       = 4'h2;
localparam      TXRESP_WAIT_DATA     = 4'h3;
localparam      TXRESP_SEND_FIRST    = 4'h4;
localparam      TXRESP_SEND_LAST     = 4'h5;
localparam      TXRESP_SEND_MAX      = 4'h6;
localparam      TXRESP_DONE          = 4'h7;
localparam      TXRESP_WAIT_FIRST    = 4'h8;
localparam      TXRESP_WAIT_MAX      = 4'h9;
localparam      TXRESP_WAIT_LAST     = 4'hA;    
localparam      TXRESP_PIPE_FIRST    = 4'hB;
localparam      TXRESP_PIPE_MAX      = 4'hC;
localparam      TXRESP_PIPE_LAST     = 4'hD;
localparam      TXRESP_LD_OVRD_BYTES = 4'hE;  
...
logic   [12:0]   dwlen_last_bytes_sent_reg;
logic  [12:0]   dwlen_bytes_sent;
logic   [12:0]  dwlen_last_bytes_sent;
logic   [7:0]    tag;        
...
logic   [3:0]  txresp_state;
logic   [3:0]  txresp_nxt_state;
logic           first_cpl_sreg;
logic           first_cpl;
logic  [8:0]    bytes_to_RCB_reg;
logic  [12:0]   curr_bcnt_reg;
logic  [12:0]   dwlen_bcnt_reg;
...
logic           sm_ld_ovrd;
logic [3:0] num_of_masked_bytes;
logic [1:0] first_dw_invalid_bytes;
logic [1:0] first_dw_invalid_bytes_1dw;
logic [1:0] last_dw_invalid_bytes;
logic [2:0] total_invalid_bytes_BE;
    
//SRIOV Support

  logic  [PFCNT_WD-1:0]                  pf_number;
  logic  [VFCNT_WD-1:0]                  vf_number;
  logic                                  vf_active;  
...
endgenerate
   ...
   generate if(SRIOV_EN) begin
     assign pf_number  = PndgRdFifoDato_i[PFCNT_WD+65-1:65];
     assign vf_number  = PndgRdFifoDato_i[VFCNT_WD+PFCNT_WD+65-1:PFCNT_WD+65];
     assign vf_active  = PndgRdFifoDato_i[VFCNT_WD+PFCNT_WD+65]; 
     assign CmdFifoDatin_o[CMDF_DWIDTH-1:0] = { vf_active,vf_number,pf_number,3'b000 ,attr[1:0], dw_len[8:0], tc[2:0], remain_bytes_reg[11:0], is_flush, first_cpl_sreg, 4'h0,       // 3+2+9+3+12+1+5 = 3
                                              32'h0, is_uns_rd_size, requester_id[15:0], tag[7:0], lower_addr_reg[6:0]}; // 32+1+16+8+7 = 64     
    end 
    else begin
     assign CmdFifoDatin_o[CMDF_DWIDTH-1:0] = { 3'b000 ,attr[1:0], dw_len[8:0], tc[2:0], remain_bytes_reg[11:0], is_flush, first_cpl_sreg, 4'h0,       // 3+2+9+3+12+1+5 = 3
                                              32'h0, is_uns_rd_size, requester_id[15:0], tag[7:0], lower_addr_reg[6:0]}; // 32+1+16+8+7 = 64
       
    end    
   endgenerate
   
assign tag            = PndgRdFifoDato_i[15:8];
assign requester_id   = PndgRdFifoDato_i[39:24];
assign rd_addr[7:0]   = {PndgRdFifoDato_i[23:18], 2'b00};
assign rd_dwlen       = (PndgRdFifoDato_i[49:40] == 10'h0)? 11'b100_0000_0000 : {1'b0,PndgRdFifoDato_i[49:40]};
assign attr           = PndgRdFifoDato_i[55:54];
assign tc             = PndgRdFifoDato_i[58:56];
assign is_flush       = PndgRdFifoDato_i[23];
assign is_uns_rd_size = PndgRdFifoDato_i[64];
assign first_byte_mask = PndgRdFifoDato_i[3:0];
assign last_byte_mask = PndgRdFifoDato_i[7:4];

assign dw_len[8:0] = ({last_byte_mask, first_byte_mask}>0)?(is_uns_rd_size? 9'h0 :  dwlen_bytes_sent[10:2]):9'h001;
...
always @*
  ...
  begin
    case(txresp_state)
      ...
      TXRESP_LD_BCNT:  // load byte count reg and calculate the first byte 7 bit of address
           txresp_nxt_state <= TXRESP_LD_OVRD_BYTES;
      
      TXRESP_LD_OVRD_BYTES:
 ...
 assign   sm_ld_ovrd    = (txresp_state == TXRESP_LD_OVRD_BYTES); 
...
// assign num_of_masked_bytes = ~first_byte_mask[0] + ~first_byte_mask[1] + ~first_byte_mask[2] + ~first_byte_mask[3] + ~last_byte_mask[0] + ~last_byte_mask[1] + ~last_byte_mask[2] + ~last_byte_mask[3];

 assign total_invalid_bytes_BE = (rd_dwlen == 11'd1)? first_dw_invalid_bytes_1dw : (first_dw_invalid_bytes + last_dw_invalid_bytes);

 always @(posedge Clk_i)
  begin
    if(sm_ld_bcnt) begin 
      if (last_byte_mask == 0 &&first_byte_mask==0 )
	begin
          curr_bcnt_reg <= 3'b001;
	  dwlen_bcnt_reg <= 3'b001;
	end
      else
	begin
          curr_bcnt_reg <= {rd_dwlen, 2'b00} - total_invalid_bytes_BE;
	  dwlen_bcnt_reg <= {rd_dwlen, 2'b00};
	end
    end
    else if(sm_send_first)
	begin
      	  curr_bcnt_reg <= curr_bcnt_reg - bytes_to_RCB_reg + first_dw_invalid_bytes;
	  dwlen_bcnt_reg <= dwlen_bcnt_reg - bytes_to_RCB_reg; 
	end
    else if(sm_send_max)
	begin
          curr_bcnt_reg <= (txresp_nxt_state == TXRESP_SEND_LAST)? (curr_bcnt_reg - max_payload + last_dw_invalid_bytes): curr_bcnt_reg - max_payload;
	  dwlen_bcnt_reg <= dwlen_bcnt_reg - max_payload;
	end
...
/// Credit Consumed Counter
   always @(posedge Clk_i )
      begin
        if(~Rstn_i)
          payload_consumed_cntr <= 15'h0; /// start at non-zero to prevent underflow tx cpl buffer
        else if(sm_ld_ovrd & ~is_flush & ~is_uns_rd_size)
 ...
 always @(posedge Clk_i )
  ...
  assign payload_ok = payload_available_sub <= 16384 & ~sm_idle & ~sm_rd_fifo & ~sm_ld_bcnt & ~sm_wait_data & ~sm_wait_first & ~sm_wait_max & ~sm_wait_last & ~sm_ld_ovrd;
...
assign dwlen_last_bytes_sent = dwlen_bcnt_reg;
...
always @(posedge Clk_i)
  begin
     begin
      ...
      dwlen_last_bytes_sent_reg <= dwlen_last_bytes_sent;
     end
  end  
...
always @*
  begin
    case({sm_send_first, sm_send_max, sm_send_last, is_flush, is_uns_rd_size})
      5'b00100 : dwlen_bytes_sent = dwlen_last_bytes_sent_reg;
      5'b01000 : dwlen_bytes_sent = max_bytes_sent_reg;
      5'b10000 : dwlen_bytes_sent = first_bytes_sent_reg;
      5'b00110 : dwlen_bytes_sent = 4;
      default : dwlen_bytes_sent = 0;
    endcase
  end
...
always @*
   casex(first_byte_mask[3:0])
      4'bxx10 : first_dw_invalid_bytes[1:0] <= 2'h1;
      4'bx100 : first_dw_invalid_bytes[1:0] <= 2'h2;
      4'b1000 : first_dw_invalid_bytes[1:0] <= 2'h3;
      default:  first_dw_invalid_bytes[1:0] <= 2'h0;
    endcase  

always @* 
   casex(last_byte_mask[3:0])
      4'b01xx : last_dw_invalid_bytes[1:0] <= 2'h1;
      4'b001x : last_dw_invalid_bytes[1:0] <= 2'h2;
      4'b0001 : last_dw_invalid_bytes[1:0] <= 2'h3;
      default:  last_dw_invalid_bytes[1:0] <= 2'h0;
    endcase  

always @*
   casex(first_byte_mask[3:0])
      4'b1xx1 : first_dw_invalid_bytes_1dw[1:0] <= 2'h0;
      4'b01x1 : first_dw_invalid_bytes_1dw[1:0] <= 2'h1;
      4'b1x10 : first_dw_invalid_bytes_1dw[1:0] <= 2'h1;
      4'b0011 : first_dw_invalid_bytes_1dw[1:0] <= 2'h2;
      4'b0110 : first_dw_invalid_bytes_1dw[1:0] <= 2'h2;
      4'b1100 : first_dw_invalid_bytes_1dw[1:0] <= 2'h2;
      default:  first_dw_invalid_bytes_1dw[1:0] <= 2'h3;
    endcase  
...
always @(posedge Clk_i)
  begin
    if(sm_send_first)
      ...
      lower_addr_reg <= rd_addr[6:0] + first_dw_invalid_bytes;
    end      

...
logic [255:0] DTMReadData;
logic [127:0] DTMReadData_reg;
logic readvalid_128;
...
always_comb   begin
   case (DCSAddress_i)
     ...
     default : reg_decode[15:0] = 16'b0000_0000_0000_0000;
   endcase
end
...
always_ff @(posedge Clk_i or negedge Rstn_i) begin
   ...
   else if ((DMA_WIDTH == 256) ? DTMReadDataValid_i : (DTMReadDataValid_i && ~readvalid_128)) begin
...
always_ff @(posedge Clk_i or negedge Rstn_i) begin
   ...
   else if ((DTMReadDataValid_i == 1'b1) && (DMA_WIDTH !=256)) begin
...
always_ff @(posedge Clk_i or negedge Rstn_i) begin
   if(~Rstn_i) begin
      DTMReadData_reg <= 128'h0;
   end
   else begin
      DTMReadData_reg <=  DTMReadData_i;
   end
end

assign DTMReadData = (DMA_WIDTH == 256) ? DTMReadData_i : {DTMReadData_i, DTMReadData_reg};
always_ff @(posedge Clk_i) begin
   if((DTMReadDataValid_i==1'b1) && (dtm_rd_state==DTMRDCTL_COPY_TABLE) &&  ((DMA_WIDTH == 256) ? 1'b1 : (readvalid_128 == 1'b0)))begin
      local_rd_dt_mem[dt_copy_readvalid_addr_incr_reg] <= DTMReadData[159:0];
   end
end

always_ff @(posedge Clk_i) begin
   if((DTMReadDataValid_i==1'b1) && (dtm_wr_state==DTMWRCTL_COPY_TABLE))begin
      local_wr_dt_mem[dt_copy_readvalid_addr_incr_reg] <= DTMReadData[159:0];
   end
end
...
assign rd_num_descriptor = (DMA_WIDTH == 256) ? rd_num_descriptor_reg : 2*rd_num_descriptor_reg;
assign wr_num_descriptor = (DMA_WIDTH == 256) ? wr_num_descriptor_reg : 2*wr_num_descriptor_reg;
...
always_ff @(posedge Clk_i or negedge Rstn_i) begin
   if(~Rstn_i) begin
      ...
      if ((dtm_rd_state==DTMRDCTL_COPY_TABLE) && (dt_copy_addr_incr_reg_lookahead < rd_num_descriptor)) begin
         DTMAddress_o <= (DMA_WIDTH == 256) ? {rd_epdt_address_hi_reg, rd_epdt_address_low_reg} + {dt_copy_addr_incr_reg_lookahead, 5'h0} :
                                              {rd_epdt_address_hi_reg, rd_epdt_address_low_reg} + {dt_copy_addr_incr_reg_lookahead, 4'h0};
         DTMRead_o <= 1'b1;
      end
      else if ((dtm_wr_state==DTMWRCTL_COPY_TABLE) && (dt_copy_addr_incr_reg_lookahead < wr_num_descriptor_reg)) begin
         DTMAddress_o <= (DMA_WIDTH == 256) ? {wr_epdt_address_hi_reg, wr_epdt_address_low_reg} + {dt_copy_addr_incr_reg_lookahead, 5'h0} :
                                              {wr_epdt_address_hi_reg, wr_epdt_address_low_reg} + {dt_copy_addr_incr_reg_lookahead, 4'h0};
         DTMRead_o <= 1'b1;
      end
      else begin
         DTMAddress_o <= 64'h0;

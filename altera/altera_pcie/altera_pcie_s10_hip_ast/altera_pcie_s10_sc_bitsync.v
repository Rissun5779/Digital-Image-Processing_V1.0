...
module altera_pcie_s10_sc_bitsync
  #(
    parameter DWIDTH    = 1, // Sync Data input
    parameter NODENAME  = "altera_pcie_s10_sc_bitsync", // Expecting Instance name
   ...
   localparam MULTI_2      = {"-name SDC_STATEMENT \"set_multicycle_path -to [get_keepers *" , NODENAME , "*altera_pcie_s10_sc_bitsync_meta_dff\[*\]] 2\" "};
   localparam MULTI_3      = {"-name SDC_STATEMENT \"set_multicycle_path -to [get_keepers *" , NODENAME , "*altera_pcie_s10_sc_bitsync_meta_dff\[*\]] 3\" "};
   localparam FPATH        = {"-name SDC_STATEMENT \"set_false_path -to [get_keepers *"      , NODENAME , "*altera_pcie_s10_sc_bitsync_meta_dff\[*\]]\" "};
   localparam FHOLD        = {"-name SDC_STATEMENT \"set_false_path -hold -to [get_keepers *", NODENAME , "*altera_pcie_s10_sc_bitsync_meta_dff\[*\]]\" "};
   localparam SDC          = (SDC_TYPE==0)?{MULTI_3,";",FHOLD}:(SDC_TYPE==1)?{MULTI_2,";",FHOLD}:{FPATH  ,";",FHOLD};

// synthesis translate_off
   initial begin
      $display("INFO:         altera_pcie_s10_sc_bitsync::---------------------------------------------------------------------------------------------");
      $display("INFO:         altera_pcie_s10_sc_bitsync:: NODENAME is %s", NODENAME);
      $display("INFO:         altera_pcie_s10_sc_bitsync:: SDC is %s", SDC);
   end
// synthesis translate_on

   // Define wires/regs
   (* altera_attribute = SDC *)
   reg [DWIDTH-1:0]                 altera_pcie_s10_sc_bitsync_meta_dff;
   wire                             reset_value;
      ...
      if (SYNCSTAGE==2) begin : g_bitsync2
         ...
         always @(negedge rst_n or posedge clk) begin
            if (rst_n == 1'b0) begin
               ...
               sync_regs[DWIDTH-1:0] <= altera_pcie_s10_sc_bitsync_meta_dff;
   ...
   always @(posedge clk) begin
      altera_pcie_s10_sc_bitsync_meta_dff[DWIDTH-1:0] <= data_in;
   end


endmodule // altera_pcie_s10_sc_bitsync

// synthesis translate_off
`timescale 1ns / 1ps
// synthesis translate_on
module altera_pcie_s10_sc_bitsync_node
  #(
    parameter DWIDTH    = 1, // Sync Data input
    parameter NODENAME  = "altera_pcie_s10_sc_bitsync_node", // Instance name
    parameter SYNCSTAGE = 2, // Sync stages
    parameter SDC_TYPE  = 0, // 0: Multi Cycle=3, 1:Multi Cycle=2, 2: Set False Path
    parameter RESET_VAL = 0  // Reset value
    )
...
endmodule //altera_pcie_s10_sc_bitsync_node

...
proc add_pcie_hip_parameters_ui_system_settings {} {
   ...
   set_parameter_property use_config_bypass_hwtcl DESCRIPTION "When On, the Stratix V Hard IP for PCI Express Intel FPGA IP bypasses the Transaction Layer Configuration Space registers included as part of the Hard IP, allowing you to substitute a custom Configuration Space implemented in soft logic."

   # Enable TL-Direct simulation
   ...
   set_parameter_property hip_tag_checking_hwtcl VISIBLE true
...
proc add_pcie_hip_hidden_rtl_parameters {} {
   ...
   #add_parameter          pcie_inspector_enable_hwtcl integer 0
   #set_parameter_property pcie_inspector_enable_hwtcl VISIBLE false
   #set_parameter_property pcie_inspector_enable_hwtcl HDL_PARAMETER true
...
proc validation_parameter_system_setting {} {
   ...
   set hip_tag_checking_hwtcl            [ get_parameter_value hip_tag_checking_hwtcl ]
   ...
   if {$use_config_bypass_hwtcl == 1 } {
      ...
      if { $hip_tag_checking_hwtcl == 1 } {
         send_message warning "The Config Bypass Support for this design is enabled. The settings for Enable Hard IP completion tag checking would not have any effect, the application can issue up to 256 tags and the completion tag checking circuit is expected to be built in the application logic"
      }
   }

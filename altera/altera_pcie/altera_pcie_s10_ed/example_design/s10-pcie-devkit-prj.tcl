...
#--------------------------------------------------------------#
#
# s10-pcie-devkit-prj:  Create quartus project from a QIP/Qsys file
#
#
...
#        : default    : None"
#
#  Arg 1 : $GenerateQPF : Update QII project generated by qsys-script
#        : optional   : yes
#        : default    : 1"
#
...
if { [ string eq "" $DeviceQSF] } {
   set DeviceQSF "10SX280N3F43I3VG"
}
...
set deviceFamily_qsf "Stratix 10"
set deviceFamily_short "s10"
...
puts "--------- ::---------------------------------------------------------------------------------------------------------------- "
puts "--------- ::"
puts "--------- :: Running : s10-pcie-devkit-prj.tcl"
puts "--------- ::"
...
if { $GenerateQPF > 0 && $PassSimGeneration>0 && $PassSynthGeneration>0 } {
   set devkit_qsys_script "${IP_ROOTDIR}/altera/altera_pcie/altera_pcie_s10_ed/example_design/s10-revd-devkit-pinout-qsys-script.tcl"
   puts "--------- ::---------------------------------------------------------------------------------------------------------------- "
   puts "--------- ::"
   puts "--------- :: qsys-script --pro --cmd='set argstring \"${project_name},${DeviceQSF}\"' --script=${devkit_qsys_script}"
   puts "--------- ::"
   puts "--------- ::"
   if { [ catch {exec ${QSYS_ROOTDIR}qsys-script --pro --quartus-project=${project_name} --cmd='set argstring \"${project_name},${DeviceQSF}\"' --script=${devkit_qsys_script} } msg ] } {
      puts "--------- :: $msg"
   }
   puts "--------- ::"
   #--------------------------------------------------------------#
...
#TODO Update this after getting the S10 DEV KIT
   # Pinout created with s10-revd-devkit-pinout-qsys-script.tcl
   if { [ file exist $s10_devkit_tcl ] == 1 } {
      source $s10_devkit_tcl
   }
   # SDC created with s10-revd-devkit-pinout-qsys-script.tcl
   if { [ file exist $s10_devkit_sdc ] == 1 } {
      set_global_assignment -name SDC_FILE $s10_devkit_sdc
   }

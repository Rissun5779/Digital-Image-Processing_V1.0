...
module altpcieav_256_rp_rxm_rdwr # (
      ...
      parameter PNDG_HDR_SIZE                    = 62,   //57 for NON-SRIOV,  
  ...
  assign rd_tc        = RxFifoDataq_i[22:20];
   ...
   generate if(SRIOV_EN) begin
     ...
     always_ff @ (posedge Clk_i)
      ...
      assign PndgRdHeader_o   = {vf_active_reg,vf_number_reg,pf_number_reg,1'b0,   4'hF,   rd_tc_reg,   rd_attr_reg,   4'hF,   rx_dwlen_reg,  req_id_reg[15:0],   1'b0,    rx_addr_reg[6:0],  rd_tag_reg};
         ...
         // calculate the first valid address based on FBE
           always_comb
             begin
              ...
              if (avmm_fbe_reg[0])
                 first_valid_addr[7:0] <= 8'h00;
              else if (avmm_fbe_reg[4])
                first_valid_addr[7:0] <= 8'h04;
              else if (avmm_fbe_reg[8])
                first_valid_addr[7:0] <= 8'h08;
              else if (avmm_fbe_reg[12])
                first_valid_addr[7:0] <= 8'h0C;
              else if (avmm_fbe_reg[16])
                first_valid_addr[7:0] <= 8'h10;
              else if (avmm_fbe_reg[20])
                first_valid_addr[7:0] <= 8'h14;
              else if (avmm_fbe_reg[24])
                first_valid_addr[7:0] <= 8'h18;
              else 
                first_valid_addr[7:0] <= 8'h1C;  

...
(
    ...
    input   logic  [4:0]                             CfgAddr_i,
    input   logic  [1:0]                             CfgFunc_i,
    input   logic  [31:0]                            CfgCtl_i,
    output  logic  [12:0]                            CfgBusDev_o,
    output  logic  [31:0]                            DevCsr_o,
    output  logic  [31:0]                            PciCmd_o,

    output  logic  [15:0]                            MsixIntfc_o, //Encoding diff in S10
    output  logic  [31:0]                            MsiDataCrl_o, //Encoding diff in S10
   ...
   input  logic [VFCNT_WD-1:0]                       bus_master_en_vf_i  // Bus Master Enable for VFs
    ...
    logic                                            extended_tag         ;
    logic                                            bus_master_enable    ;
    logic                                            msi_enable           ;
    logic  [2:0]                                     multiple_msi_enable  ;
    logic                                            msi64bit_enable     ;
    logic                                            msix_enable          ;
    logic                                            msix_func_mask        ;
    logic                                            rstn_reg;

    logic                                            rstn_rr;

    logic                                            rstn_r;
    logic  [4:0]                                     cfg_addr_reg;
    logic  [1:0]                                     cfg_func_reg;
...
always @ (posedge Clk_i or negedge Rstn_i)
  begin
     if (~Rstn_i)
       ...
       begin
         cfg_addr_reg                  <= CfgAddr_i[4:0];
         cfg_func_reg                  <= CfgFunc_i[1:0];
      ...
      begin
        if (rstn_reg == 0)
          begin
            ...
            msi_data             <= 16'h0;
            msi_addr             <= 64'h0;
            max_payload_size     <= 3'h0;
            max_read_req_size    <= 3'h0;
            extended_tag         <= 1'b0;
            bus_master_enable    <= 1'b0;
            msi_enable           <= 1'b0;
            multiple_msi_enable  <= 3'h0;
            msi64bit_enable     <= 1'b0;
            msix_enable          <= 1'b0;
            msix_func_mask        <= 1'b0;
          end
        else
          begin
            ...
            cfg_busdev          <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h00)  ) ? {cfg_data_reg[23:16],cfg_data_reg[28:24]}   : cfg_busdev;
            msi_addr[31:0]      <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h03)  ) ? cfg_data_reg[31:0]   :  msi_addr[43:32];
            msi_addr[63:32]     <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h04)  ) ? cfg_data_reg[31:0]   :  msi_addr[63:44];
            msi_data[15:0]      <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[31:16]  :  msi_data[15:0];
            max_payload_size    <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h00)  ) ? cfg_data_reg[2:0]      :  max_payload_size   ;
            max_read_req_size   <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h00)  ) ? cfg_data_reg[5:3]      : max_read_req_size   ;
            extended_tag        <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h00)  ) ? cfg_data_reg[6]        : extended_tag    ;
            bus_master_enable   <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h00)  ) ? cfg_data_reg[7]      :  bus_master_enable  ; // cfg_prmcsr[2] -  //Bus Master Enable
            msi_enable          <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[0]        : msi_enable ;
            multiple_msi_enable <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[4:2]        : multiple_msi_enable  ;
            msi64bit_enable    <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[2]        : msi64bit_enable    ;
            msix_enable         <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[5]        : msi_enable    ;
            msix_func_mask      <= ( (cfg_func_reg == 2'h0 ) && (cfg_addr_reg[4:0]==5'h06)  ) ? cfg_data_reg[6]        : msix_func_mask    ;
...
generate if (SRIOV_EN == 1) begin
      ...
      assign MsiDataCrl_o       = 32'h0;
       ...
       assign MsiIntfc_o[80]       = msi_enable   ;
       assign MsiIntfc_o[81]       = bus_master_enable    ;  // Master Enable
       assign CfgBusDev_o[12:0]    = cfg_busdev       ;
       assign DevCsr_o[31:0]       = {16'h0,1'b0,max_read_req_size,3'b0,extended_tag,max_payload_size,5'h0} ;
       assign PciCmd_o             = {29'h0, bus_master_enable, 2'h0};
       assign MsiDataCrl_o         = {msi_data_reg,8'h00,msi64bit_enable ,multiple_msi_enable,3'b000,msi_enable};
       assign MsixIntfc_o[15:0]    = {msix_enable,msix_func_mask,14'h0000 } ;
   end
endgenerate

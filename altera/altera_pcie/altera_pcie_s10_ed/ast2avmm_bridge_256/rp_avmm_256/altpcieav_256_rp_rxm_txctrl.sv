...
module altpcieav_256_rp_rxm_txctrl

# (
           parameter AVMM_WIDTH       = 256,     
           parameter TX_FIFO_WIDTH    = 265,   //Data+Sop+Eop+Empty    
  ...
  (
      ...
      input   logic [CMDF_DWIDTH-1:0]              CmdFifoDat_i,
...
//SRIOV Support
localparam tlp_dwidth  = SRIOV_EN ? AVMM_WIDTH+CMDF_DWIDTH-99 : AVMM_WIDTH;

  logic  [CMDF_DWIDTH-99-1:0]    sriov_cmpl_sideband,sriov_cmpl_sideband_reg;  
  logic  [tlp_dwidth-1:0] tlp_data;  
...
generate if(SRIOV_EN) begin
  assign sriov_cmpl_sideband  = CmdFifoDat_i[CMDF_DWIDTH-1:99];
  always @(posedge Clk_i)
     if(CmdFifoRdReq_o)
       sriov_cmpl_sideband_reg <= sriov_cmpl_sideband;
  
  assign tlp_data     = sm_cpl_hdr? {sriov_cmpl_sideband_reg,cpl_header_qw3, cpl_header_qw2,cpl_header_qw1, cpl_header_qw0} : {{CMDF_DWIDTH-99{1'b0}},tx_data};  
  assign output_fifo_data_in[TX_FIFO_WIDTH-1:0]  = {tlp_data[AVMM_WIDTH+CMDF_DWIDTH-99-1:AVMM_WIDTH],tlp_empty, tlp_eop, tlp_sop, tlp_data[AVMM_WIDTH-1:0]}; 
 end 
 else begin
...
assign tx_modlen_sel = {tx_address_lsb_reg[4:2], dw_len_reg[2:0]};
  ...
  /*        6'b000_000:  tx_modlen_lines[7:0] <= dw_len_reg[9:3];       // data is 256-bit aligned and modulo-256, quad quadwords
    ...
    begin            
 logic dw_len_lt_4; 
 assign dw_len_lt_4  = dw_len_reg[5:0] < 4 ;
   ...
   begin
      case(txcpl_state)
        ...
        TXCPL_ARB_PIPE:
         ...
         TXCPL_DATA:
            if((cpl_dat_cntr == 1) & cmd_fifo_empty)
               txcpl_nxt_state <= TXCPL_IDLE;
...
assign data_2_arb_req = sm_cpl_data & ~((cpl_dat_cntr == 1) & cmd_fifo_empty) & (cpl_dat_cntr == 1 & ~cmd_fifo_empty);

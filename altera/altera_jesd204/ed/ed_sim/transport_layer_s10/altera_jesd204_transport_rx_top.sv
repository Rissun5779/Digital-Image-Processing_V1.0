...
module altera_jesd204_transport_rx_top #(
   ...
   parameter OUTPUT_BUS_WIDTH  = (N_PRIME==12) ? 480 : (F==8)? (8*8*L*N/N_PRIME) : (F==4)? (8*4*L*N/N_PRIME) : (F==2) ? (F2_FRAMECLK_DIV*8*2*L*N/N_PRIME) : (F==1) ? (F1_FRAMECLK_DIV*8*1*L*N/N_PRIME) : 1,
   ...
   generate
   if (L==8 && F==8 && N_PRIME==12)
   begin

      altera_jesd204_deassembler_nprime12 #(
         .L(L),
         .F(F),
         .N(N),
         .CS(CS),

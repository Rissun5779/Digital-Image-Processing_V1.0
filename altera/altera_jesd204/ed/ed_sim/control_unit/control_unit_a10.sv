...
module control_unit #(
   ...
   parameter XCVR_PLL_PER_LINK = 1,   // Number of transceiver PLL per LINK
...
( 
   ...
   output [LINK*XCVR_PLL_PER_LINK-1:0][9:0]        reconfig_mgmt_pll_address,
   output [LINK*XCVR_PLL_PER_LINK-1:0]             reconfig_mgmt_pll_read,
   input  [LINK*XCVR_PLL_PER_LINK-1:0][31:0]       reconfig_mgmt_pll_readdata,
   input  [LINK*XCVR_PLL_PER_LINK-1:0]             reconfig_mgmt_pll_waitrequest,
   output [LINK*XCVR_PLL_PER_LINK-1:0]             reconfig_mgmt_pll_write, 
   output [LINK*XCVR_PLL_PER_LINK-1:0][31:0]       reconfig_mgmt_pll_writedata,
   output [LINK-1:0][2+9:0]      reconfig_mgmt_xcvr_address, // additional 2-bit for up to 4-channels, need to revise this
   output [LINK-1:0]             reconfig_mgmt_xcvr_read,
   input  [LINK-1:0][31:0]       reconfig_mgmt_xcvr_readdata,
   input  [LINK-1:0]             reconfig_mgmt_xcvr_waitrequest,
   output [LINK-1:0]             reconfig_mgmt_xcvr_write,
   output [LINK-1:0][31:0]       reconfig_mgmt_xcvr_writedata,
   output              cu_busy
);
   ...
   localparam USE_COMBINED_MIF      = 1;  // Use single MIF file, or use separate MIF files for ATX PLL and transceiver reconfiguration
   localparam BONDED_MODE           = "non_bonded"; // valid value "non_bonded", "bonded"
//   localparam NUM_CHANNEL           = LINK*L; 
   localparam NUM_CHANNEL           = L;  // This NUM_CHANNEL is used to determined number of channels to be reconfigured. Changed to the number of channel in one LINK only as
                                          // different LINK will be reconfigured parallelly by different XCVR_RCFG_MASTER
...
//   reg  [31:0]  pll_writedata;
   ...
   wire         wire_tx_ready_and;  
   wire         wire_rx_ready_and;  
   wire         frame_rst_sync;
   wire         link_rst_sync;
   ...
   reg          set_reconfig_atx_pll;
   wire [8:0]   pll_address;
   wire [31:0]  pll_writedata;
   wire         wire_tx_waitrequest_and; 
   wire         wire_rx_waitrequest_and; 

   wire         reconfig_atx_pll_start;
   wire [LINK*XCVR_PLL_PER_LINK-1:0]  reconfig_atx_pll_busy;
   wire         reconfig_atx_pll_busy_all;
   wire         reconfig_xcvr_start;
   wire [LINK-1:0]  reconfig_xcvr_busy;
   wire         reconfig_xcvr_busy_all;
...
//   assign runtime_f = runtime_lmf_reg ? F-1 : F-1;   // TBD
   ...
   assign pll_address        = set_pll_mif_base ? 9'b000010000 : 9'b000000000;  // Start MIF streaming instruction : default
   assign pll_writedata      = runtime_datarate_reg ? 32'b00000000000000000000000000000000 : 32'b00000000000000000000000000101110; // Set MIF base address to 46 for downscaling, 0 for compile time configuration
   ...
   localparam ATX_PLL_RECONFIG    = 25; // data rate reconfig
   localparam ATX_PLL_RECONFIG_POLL = 26; // data rate reconfig
   localparam SELECT_XCVR_CHANNEL = 27; // data rate reconfig
   localparam XCVR_RECONFIG       = 28; // data rate reconfig
   localparam XCVR_RECONFIG_POLL  = 29; // data rate reconfig
   //Insert new state ERROR to capture FSM errors
   //
   localparam ERROR               = 30; // FSM error
   ...
   // Next state logic
   //  	
   always @ (current_state or wire_tx_ready_and or wire_rx_ready_and or spi_trdy or delay_cnt or
             ...
             or init_done or channel_number or reconfig_atx_pll_busy_all or reconfig_xcvr_busy_all
             or init_reset or reconfig_in_progress or runtime_lmf_reg or runtime_datarate_reg
             or csr_scr_en_constant or csr_k_constant or avs_address or last_jesd_rom_addr
             or pd_channel_num or last_pd_channel or runtime_l_prev)				 
   begin
      ...
      case (current_state)
         ...
         LMF_RECONFIG: begin
            ...
            if ((csr_scr_en_constant == 1'b0) && (csr_k_constant == 5'b0)) begin
               ...
               //Delay by one cycle to account for delayed avs_address setting
               dec_delay_cnt = 1'b1;
         ...
         // This PLL MIF base address setup state is skipped in Arria 10 PLL reconfiguraiton.
         ...
         PLL_RECONFIG_WRITE: begin
            set_reconfig_pll = 1'b1;
            set_pll_mif_base = 1'b1;  // In Arria 10 PLL reconfiguration, don't need to setup MIF base address before start MIF write. Both happen at the same cycle.
            dec_delay_cnt = 1'b1;
         ...
         PLL_RECONFIG_DONE: begin
            if (~pll_mgmt_waitrequest) begin
               next_state = ATX_PLL_RECONFIG;
            end
         end
 
         ATX_PLL_RECONFIG: begin
            set_reconfig_atx_pll = 1'b1;
            next_state = ATX_PLL_RECONFIG_POLL;
         end
         
         ATX_PLL_RECONFIG_POLL: begin
            if (reconfig_atx_pll_busy_all) begin
               next_state = ATX_PLL_RECONFIG_POLL;
            end
            else begin
         ...
         XCVR_RECONFIG_POLL: begin
            if (reconfig_xcvr_busy_all) begin
               next_state = XCVR_RECONFIG_POLL;
            end
            else begin
               next_state = SELECT_XCVR_CHANNEL;
   ...
   begin
      if (~rst_n) begin  
         ...
         if (set_lmf_read) begin
            ...
            csr_scr_en_constant <= jesd204_tx_avs_readdata[LINK-1][7];
            csr_k_constant <= jesd204_tx_avs_readdata [LINK-1][20:16];
         end
      end       
   end
         ...
         if (set_runtime_lmf_read) begin
            case(rom3_addr_ptr[2:0])
               3'b000: begin
                  ...
                  runtime_l <= rom3_data_out[4:0];
               end

               3'b001: begin
                  runtime_m <= rom3_data_out[7:0];
               end

               3'b010: begin
                  runtime_f_prev <= runtime_f;
                  runtime_f <= rom3_data_out[7:0];
               end
            endcase        
         end
      end       
   ...
   //   
   // Output register for channel number
   //  
   always @ (posedge clk or negedge rst_n)
   begin
      if (~rst_n) begin  
   ...
   rom_1port_16 #(
    ...
    // 
    // XCVR PLL reconfiguration master with embedded ROM
    // 
    assign reconfig_atx_pll_start = set_reconfig_atx_pll;
    assign reconfig_atx_pll_busy_all = (&reconfig_atx_pll_busy);

    generate 
    for (i = 0; i < (LINK * XCVR_PLL_PER_LINK); i = i + 1) begin: GEN_ATX_PLL_RCFG_MASTER
   
    xcvr_reconfig_mif_master #(
        // ROM initialization MIF file
        .COMBINED_INIT_FILE         ("./xcvr_atx_pll_combined.mif"),
//        .MODE_0_INIT_FILE           ("./xcvr_atx_pll_mode_0.mif"),
//        .MODE_1_INIT_FILE           ("./xcvr_atx_pll_mode_1.mif"),
        
        // Use combined MIF file
        .USE_COMBINED_MIF           (USE_COMBINED_MIF),
        
        // MIF data format
        .MIF_DATA_OFFSET_LSB        (0),
        .MIF_DATA_OFFSET_MSB        (7),
        .MIF_BIT_MASK_OFFSET_LSB    (8),
        .MIF_BIT_MASK_OFFSET_MSB    (15),
        .MIF_DPRIO_ADDR_OFFSET_LSB  (16),
        .MIF_DPRIO_ADDR_OFFSET_MSB  (25),
        
        // ROM size
        .MIF_PER_CONFIG_SIZE        (20),
        .MIF_ADDR_WIDTH             (USE_COMBINED_MIF ? 6 : 5),
        
        // Index of MIF to be read from ROM and write to registers (per config MIF)
        .MIF_READ_START_OFFSET      (5'h0),
        .MIF_READ_NUM               (5'h12 - 5'h0 + 5'h1),
        .DEVICE_FAMILY (DEVICE_FAMILY)
    ) u_atx_pll_reconfig_mif_master (
        .clk                        (clk),
        .rst_n                      (rst_n),
        
        .reconfig_start             (reconfig_atx_pll_start),
        .reconfig_busy              (reconfig_atx_pll_busy[i]),
        
        .mif_select                 (~runtime_datarate_reg),
        
        .reconfig_mgmt_address      (reconfig_mgmt_pll_address[i]),
        .reconfig_mgmt_read         (reconfig_mgmt_pll_read[i]),
        .reconfig_mgmt_readdata     (reconfig_mgmt_pll_readdata[i]),
        .reconfig_mgmt_waitrequest  (reconfig_mgmt_pll_waitrequest[i]),
        .reconfig_mgmt_write        (reconfig_mgmt_pll_write[i]),
        .reconfig_mgmt_writedata    (reconfig_mgmt_pll_writedata[i])
    );
    ...
    // 
    // XCVR reconfiguration master with embedded ROM
    // 
    assign reconfig_xcvr_start = set_reconfig_xcvr;
    assign reconfig_xcvr_busy_all = (&reconfig_xcvr_busy);
    
    generate 
    for (i = 0; i < LINK; i = i + 1) begin: GEN_XCVR_RCFG_MASTER

    assign reconfig_mgmt_xcvr_address[i][11:10] = channel_number[1:0];

    xcvr_reconfig_mif_master #(
        // ROM initialization MIF file
        .COMBINED_INIT_FILE         ("./xcvr_cdr_combined.mif"),
//        .MODE_0_INIT_FILE           ("./xcvr_cdr_mode_0.mif"),
//        .MODE_1_INIT_FILE           ("./xcvr_cdr_mode_1.mif"),
        ...
        // ROM size
        .MIF_PER_CONFIG_SIZE        (121),
        .MIF_ADDR_WIDTH             (USE_COMBINED_MIF ? 8 : 7),
        
        // Index of MIF to be read from ROM and write to registers (per config MIF)
        .MIF_READ_START_OFFSET      (7'h63),
        .MIF_READ_NUM               (7'h6f - 7'h63 + 7'h1),
        .DEVICE_FAMILY (DEVICE_FAMILY)
    ) u_xcvr_reconfig_mif_master (
        .clk                        (clk),
        .rst_n                      (rst_n),
        
        .reconfig_start             (reconfig_xcvr_start),
        .reconfig_busy              (reconfig_xcvr_busy[i]),
        
        .mif_select                 (~runtime_datarate_reg),
        
        .reconfig_mgmt_address      (reconfig_mgmt_xcvr_address[i][9:0]),
        .reconfig_mgmt_read         (reconfig_mgmt_xcvr_read[i]),
        .reconfig_mgmt_readdata     (reconfig_mgmt_xcvr_readdata[i]),
        .reconfig_mgmt_waitrequest  (reconfig_mgmt_xcvr_waitrequest[i]),
        .reconfig_mgmt_write        (reconfig_mgmt_xcvr_write[i]),
        .reconfig_mgmt_writedata    (reconfig_mgmt_xcvr_writedata[i])
    );

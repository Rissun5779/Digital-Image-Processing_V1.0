...
if {$is_qsys_pro} {
    ...
    catch {eval [list exec "qsys-generate" --synthesis "${output_dir}/${variant_name}.ip"]} temp
    puts $temp
} else {
...
if {$is_qsys_pro} {
    ...
    catch {eval [list exec "qsys-generate" --synthesis "${output_dir_rst_ctrl}/${rst_ctrl_name}.ip"]} temp
    puts $temp
} else {
...
lappend arg_list_pll "--synthesis=VERILOG"
lappend arg_list_pll "--part=$dut_device_part"
lappend arg_list_pll "--output-directory=$output_dir_pll"

set core_pll_qsys_file "./example_design/core_pll.qsys"

if {$is_qsys_pro} {
    set qsys_file_name [file tail $core_pll_qsys_file]
	
    set script_cmd [list qsys-script --package-version=16.1 --new-quartus-project=$qsys_file_name --system-file=$core_pll_qsys_file --cmd='save_system']
    set status [catch {exec {*}$script_cmd} result]
} 
set qdir $::env(QUARTUS_ROOTDIR)
catch {eval [concat [list exec "$qdir/sopc_builder/bin/qsys-generate" $core_pll_qsys_file ] $arg_list_pll ]} temp
puts $temp

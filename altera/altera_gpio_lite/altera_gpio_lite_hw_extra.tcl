...
proc set_clock_interface {} {
	...
	set enable_hr_clock [get_parameter_value gui_enable_hr_clock]
	set enable_phase_invert_ctrl_port [get_parameter_value gui_enable_invert_hr_clock_port]
	set enable_phase_detector_for_ck [ get_parameter_value gui_enable_phase_detector_for_ck]
	...
	add_interface mimic_clock conduit end
	add_interface_port mimic_clock mimic_clock export Output $data_size
	set_interface_property mimic_clock ENABLED false
	set_port_property mimic_clock VHDL_TYPE STD_LOGIC_VECTOR
	...
	if {[safe_string_compare $data_dir input] || [safe_string_compare $data_dir bidir]} {
		...
		if {[safe_string_compare $io_reg_mode ddr] && [safe_string_compare $clock_enable true]} {
		...
		if {[ safe_string_compare $use_advanced_ddr_features true] && [safe_string_compare $io_reg_mode ddr]} {
			...
			if {[safe_string_compare $enable_hr_clock true]} {
				set_interface_property hr_clock ENABLED true
			}

			if {[safe_string_compare $enable_phase_invert_ctrl_port true]} {
	...
	if {[safe_string_compare $io_reg_mode ddr]
		&& [safe_string_compare $data_dir bidir]
		&& [ safe_string_compare $enable_phase_detector_for_ck true]} {
		set_interface_property mimic_clock ENABLED true
	}
}
...
proc set_pad_interface {} {
	set data_size [get_parameter_value SIZE]
	set data_dir [get_parameter_value PIN_TYPE]
	set pseudo_diff_buf "false"
	set true_diff_buf "false"

	if {[get_parameter_property gui_true_diff_buf ENABLED]} {
		set true_diff_buf [get_parameter_value gui_true_diff_buf]
	} elseif {[get_parameter_property gui_pseudo_diff_buf ENABLED]} {
...
proc set_async_mode_interface {} {
	set enable_aclr_port [get_parameter_value gui_enable_aclr_port]
	set enable_aset_port [get_parameter_value gui_enable_aset_port]
	set enable_sclr_port [get_parameter_value gui_enable_sclr_port]
	...
	add_interface_port sclr sclr export Input 1
	set_interface_property sclr ENABLED false
		
	if {[safe_string_compare $enable_aset_port true]} {
	    set_interface_property aset ENABLED true
	}
	
	if {[safe_string_compare $enable_aclr_port true]} {
	    set_interface_property aclr ENABLED true
	}

	if {[safe_string_compare $enable_sclr_port true]} {
...
proc set_enable_buffer_interface {} {
	...
	set data_dir [get_parameter_value PIN_TYPE]
	set io_reg_mode [get_parameter_value gui_io_reg_mode]
	set enable_oe_port [get_parameter_value gui_enable_oe_port]
	
	set enable_nsleep_port [get_parameter_value gui_enable_nsleep_port]
	add_interface nsleep conduit end
	add_interface_port nsleep nsleep export Input $data_size
	set_interface_property nsleep ENABLED false
	set_port_property nsleep VHDL_TYPE STD_LOGIC_VECTOR
	if {([safe_string_compare $data_dir input] || [safe_string_compare $data_dir bidir]) && [safe_string_compare $enable_nsleep_port true]} {
	    set_interface_property nsleep ENABLED true
	}
	...
	if { [safe_string_compare $data_dir bidir] || ([safe_string_compare $data_dir output] && [safe_string_compare $enable_oe_port true])} {
...
proc ip_validate {} {
	...
	set enable_aset_port [get_parameter_value gui_enable_aset_port]
	set enable_sclr_port [get_parameter_value gui_enable_sclr_port]
	set set_registers_to_power_up_high [get_parameter_value gui_set_registers_to_power_up_high]
	...
	set invert_output_clock [get_parameter_value gui_invert_output_clock]
	set invert_oe_inclock [get_parameter_value gui_invert_oe_inclock]	
	...
	set enable_hr_clock [ get_parameter_value gui_enable_hr_clock]
	set enable_phase_invert_ctrl_port [ get_parameter_value gui_enable_invert_hr_clock_port]
	set enable_oe_half_cycle_delay [ get_parameter_value gui_enable_oe_half_cycle_delay]
	set use_hardened_ddio_input_registers [get_parameter_value gui_use_hardened_ddio_input_registers]
	set enable_phase_detector_for_ck [ get_parameter_value gui_enable_phase_detector_for_ck]
    ...
    if [get_quartus_ini "ip_altgpio_zb_use_advanced_ddr_features"] {
		...
		set_parameter_property gui_enable_hr_clock VISIBLE true
		set_parameter_property gui_enable_invert_hr_clock_port VISIBLE true
		set_parameter_property gui_invert_clkdiv_input_clock VISIBLE true
		set_parameter_property gui_enable_oe_half_cycle_delay VISIBLE true
		set_parameter_property gui_enable_phase_detector_for_ck VISIBLE true
		...
		if {($use_advanced_ddr_features == "true") && ($use_ddio_reg_to_drive_oe == "true") && ($data_dir == "output" || $data_dir == "bidir")} {
				set_parameter_property gui_enable_oe_half_cycle_delay ENABLED true
	   	} else {	   			
				set_parameter_property gui_enable_oe_half_cycle_delay ENABLED false				
				set enable_oe_half_cycle_delay "true"		
	   	}

		if {($use_advanced_ddr_features == "true") && ($data_dir == "input" || $data_dir == "bidir")} {
				set_parameter_property gui_enable_hr_clock ENABLED true				
	   	} else {	   			
				set_parameter_property gui_enable_hr_clock ENABLED false				
				set enable_hr_clock "false"
	   	}
		...
		if {$enable_hr_clock == "true"} {
			...
			set_parameter_property gui_enable_invert_hr_clock_port ENABLED false
			set_parameter_property gui_invert_clkdiv_input_clock ENABLED false
			set enable_phase_invert_ctrl_port "false"
			set invert_clkdiv_input_clock "false"		
		}
		
		if {$io_reg_mode == "ddr" && (($data_dir == "output") || ($data_dir == "bidir"))} {
		set_parameter_property gui_invert_output_clock ENABLED true
		if {[safe_string_compare $invert_output_clock true]} {
			set_parameter_value INVERT_OUTPUT_CLOCK "true"
		} else {
			set_parameter_value INVERT_OUTPUT_CLOCK "false"
		}
		} else {
			set_parameter_property gui_invert_output_clock ENABLED false
			set_parameter_value INVERT_OUTPUT_CLOCK "false"
		}
		
		if {($io_reg_mode == "ddr") || ($io_reg_mode == "single-register")} { 
			if {($use_register_to_drive_obuf_oe == "true") || ($use_ddio_reg_to_drive_oe == "true")} {
				set_parameter_property gui_invert_oe_inclock ENABLED true
				if {[safe_string_compare $invert_oe_inclock true]} {
					set_parameter_value INVERT_OE_INCLOCK "true"
				} else {
					set_parameter_value INVERT_OE_INCLOCK "false"
				}
			} else {
				set_parameter_property gui_invert_oe_inclock ENABLED false
				set_parameter_value INVERT_OE_INCLOCK "false"
			}
		} else {
			set_parameter_property gui_invert_oe_inclock ENABLED false
		...
		if {($io_reg_mode == "ddr") && ($data_dir == "bidir")} {
			set_parameter_property gui_enable_phase_detector_for_ck VISIBLE true
			...
			set_parameter_property gui_enable_phase_detector_for_ck ENABLED false
			set_parameter_property gui_enable_phase_detector_for_ck VISIBLE false
			set_parameter_property gui_enable_phase_detector_for_ck_off_shadow VISIBLE true
			set enable_phase_detector_for_ck "false"
		}
	...
	if {[safe_string_compare $enable_oe_half_cycle_delay true]} {
		set_parameter_value ENABLE_OE_HALF_CYCLE_DELAY "true"
	} else {
		set_parameter_value ENABLE_OE_HALF_CYCLE_DELAY "false"
	}

	if {[safe_string_compare $enable_hr_clock true]} {
		set_parameter_value ENABLE_HR_CLOCK "true"
	} else {
		set_parameter_value ENABLE_HR_CLOCK "false"
	}

	if {[safe_string_compare $enable_phase_invert_ctrl_port true]} {
		set_parameter_value ENABLE_PHASE_INVERT_CTRL_PORT "true"
	} else {
		set_parameter_value ENABLE_PHASE_INVERT_CTRL_PORT "false"
	}
	...
	if {[safe_string_compare $enable_phase_detector_for_ck true]} {
		set_parameter_value ENABLE_PHASE_DETECTOR_FOR_CK "true"
	} else {
		set_parameter_value ENABLE_PHASE_DETECTOR_FOR_CK "false"
	}
    ...
    if {($data_dir == "input")} {
		...
		set_parameter_property gui_true_diff_buf ENABLED false
                ...
                set_parameter_property gui_pseudo_diff_buf ENABLED true
	        if {[safe_string_compare $pseudo_diff_buf true]} {
         		set_parameter_value BUFFER_TYPE "pseudo_differential"			
        	 } else {
			set_parameter_value BUFFER_TYPE "single-ended"
		 }
	...
	if {$io_reg_mode == "bypass" || $io_reg_mode == "single-register"} {
		...
		set_parameter_value ENABLE_CLOCK_ENA_PORT false
		
		set_parameter_property gui_enable_aclr_port ENABLED false	
		set_parameter_property gui_enable_aset_port ENABLED false
		set_parameter_property gui_enable_sclr_port ENABLED false	
		set_parameter_property gui_set_registers_to_power_up_high ENABLED false		

	} elseif { $io_reg_mode == "ddr" } {	
		set_parameter_property gui_clock_enable ENABLED true
		set_parameter_value ENABLE_CLOCK_ENA_PORT [get_parameter_value gui_clock_enable]		
		...
		if {($data_dir != "input") || [safe_string_compare $use_hardened_ddio_input_registers true]} {
		...
		if { [safe_string_compare $enable_sclr_port true] } {
			...
			set_parameter_value SYNC_MODE "clear"
			set_parameter_value SET_REGISTER_OUTPUTS_HIGH "false"
			set_parameter_property gui_enable_aclr_port ENABLED false
			set_parameter_property gui_enable_aset_port ENABLED false
		    set_parameter_property gui_set_registers_to_power_up_high ENABLED false
		} elseif { [safe_string_compare $enable_aset_port true] } {
			set_parameter_value ASYNC_MODE "preset"
			set_parameter_value SYNC_MODE "none"
			set_parameter_value SET_REGISTER_OUTPUTS_HIGH "false"
			set_parameter_property gui_enable_aclr_port ENABLED false
			set_parameter_property gui_enable_sclr_port ENABLED false
		    set_parameter_property gui_set_registers_to_power_up_high ENABLED false
		} elseif {[safe_string_compare $enable_aclr_port true] } {
			...
			set_parameter_property gui_enable_sclr_port ENABLED false
			set_parameter_property gui_set_registers_to_power_up_high ENABLED false
		} elseif { [safe_string_compare $set_registers_to_power_up_high true]} {
			set_parameter_value ASYNC_MODE "none"
			set_parameter_value SYNC_MODE "none"
	...
	if {$io_reg_mode == "ddr" && ($data_dir == "input" || $data_dir == "bidir")} {
	...
	if {($io_reg_mode != "bypass" ) && (($data_dir == "output") ||($data_dir == "bidir"))} {
		...
		if {$io_reg_mode == "single-register" } {
                        if {[safe_string_compare $true_diff_buf false]} {
				...
				set_parameter_property gui_use_ddio_reg_to_drive_oe VISIBLE false
                                set_parameter_property gui_use_ddio_reg_to_drive_oe_off_shadow VISIBLE true
				...
				set_parameter_property gui_use_register_to_drive_obuf_oe VISIBLE false
                                set_parameter_property gui_use_register_to_drive_obuf_oe_off_shadow VISIBLE true 
				set_parameter_property gui_use_ddio_reg_to_drive_oe VISIBLE false
                                set_parameter_property gui_use_ddio_reg_to_drive_oe_off_shadow VISIBLE true
				set_parameter_value USE_DDIO_REG_TO_DRIVE_OE "false"
                                set_parameter_value USE_ONE_REG_TO_DRIVE_OE "false"
	...
	if {( $data_dir == "input" || $data_dir == "output" )} {
		...
		set_parameter_property gui_bus_hold ENABLED false
		set_parameter_value BUS_HOLD "false"
	} 
	
	if { ($io_reg_mode == "ddr") &&($data_dir == "input" || $data_dir == "bidir" )}  {
		set_parameter_property gui_use_hardened_ddio_input_registers ENABLED true
		if {[safe_string_compare $use_hardened_ddio_input_registers true]} {
			set_parameter_value USE_ADVANCED_DDR_FEATURES_FOR_INPUT_ONLY "true"
		} else {
			set_parameter_value USE_ADVANCED_DDR_FEATURES_FOR_INPUT_ONLY "false"
		}
	} else {
		set_parameter_property gui_use_hardened_ddio_input_registers ENABLED false
		set_parameter_value USE_ADVANCED_DDR_FEATURES_FOR_INPUT_ONLY "false"
	}
	...
	if {$data_dir == "output"} {
			...
			set_parameter_value ENABLE_OE_PORT [get_parameter_value gui_enable_oe_port]
                } else {
            		set_parameter_property gui_enable_oe_port VISIBLE false
			set_parameter_property gui_enable_oe_port_off_shadow VISIBLE true
			set_parameter_property gui_enable_oe_port_on_shadow VISIBLE false
                }
		...
		set_parameter_property gui_enable_oe_port_off_shadow VISIBLE true
		set_parameter_property gui_enable_oe_port_on_shadow VISIBLE false
		set_parameter_value ENABLE_OE_PORT "false"
	} elseif {$data_dir == "bidir"} {
                if {[safe_string_compare $true_diff_buf false]} {
			set_parameter_property gui_enable_oe_port VISIBLE false
			set_parameter_property gui_enable_oe_port_off_shadow VISIBLE false
			set_parameter_property gui_enable_oe_port_on_shadow VISIBLE true
			set_parameter_value ENABLE_OE_PORT "true"
		 } else {
			set_parameter_property gui_enable_oe_port VISIBLE false
			set_parameter_property gui_enable_oe_port_off_shadow VISIBLE true
			set_parameter_property gui_enable_oe_port_on_shadow VISIBLE false
	...
	if {$data_dir == "output"} {
		set_parameter_property gui_enable_nsleep_port ENABLED false
		set_parameter_value ENABLE_NSLEEP_PORT false
	} elseif {$data_dir == "input"} {
		set_parameter_property gui_enable_nsleep_port ENABLED true
		set_parameter_value ENABLE_NSLEEP_PORT [get_parameter_value gui_enable_nsleep_port]
	} else {
		set_parameter_property gui_enable_nsleep_port ENABLED true
		set_parameter_value ENABLE_NSLEEP_PORT [get_parameter_value gui_enable_nsleep_port]
	}

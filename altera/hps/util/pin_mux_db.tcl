...
###############################################################################
#
#   $Header: //acds/rel/18.1std/ip/hps/util/pin_mux_db.tcl#1 $
#
...
namespace eval ::pin_mux_db {
	...
	set customer_pin_names_tmp           [lindex $db 8]	
	set hlgpi_pins                       [lindex $db 9]	
	# case 376322
	foreach name $customer_pin_names_tmp {
	    if { $name == "SDMMC_FB_CLK_IN" } {
	        set name "HPS_GPIO44" 
	    }
	    lappend customer_pin_names $name 
	}

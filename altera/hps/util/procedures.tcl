...
proc load_peripherals_pin_muxing_model {pin_muxing_peripherals_ref} {
	...
	foreach option_name [pin_mux_db::get_option_names $peripheral_name] {
		...
		if { $key != "SDIO.CLK_IN" } {
		    ...
		    lappend signals_uppercase "HPS_GPIO44"
		    lappend signal_parts "HPS_GPIO44"
		}
	    }
	    ...
	    # only add the pin set which has valid mode
	    if {[llength $valid_modes]} {
...
proc get_bfm_parameters {bfm { iname ""}} {
   ...
   set bfmTypeParamsList   [list  mgc_axi_slave {{AXI_ID_WIDTH "id_width" 8} {AXI_ADDRESS_WIDTH "addr_width" 32} {AXI_WDATA_WIDTH data_width 32} {AXI_RDATA_WIDTH data_width 32} {index "" 0} } ]
   if { [string compare $iname "h2f_lw_axi_master"] != 0} {
		lappend bfmTypeParamsList mgc_axi_master {{AXI_ID_WIDTH "id_width" 8} {AXI_ADDRESS_WIDTH "addr_width" 32} {AXI_WDATA_WIDTH data_width 32} {AXI_RDATA_WIDTH data_width 32} {index "" 1} }
	} else {
		lappend bfmTypeParamsList mgc_axi_master {{AXI_ID_WIDTH "id_width" 8} {AXI_ADDRESS_WIDTH "addr_width" 32} {AXI_WDATA_WIDTH data_width 32} {AXI_RDATA_WIDTH data_width 32} {index "" 2} }
	}
   ...
   lappend bfmTypeParamsList   altera_avalon_reset_source {{ASSERT_HIGH_RESET "reset_polarity" 0} {INITIAL_RESET_CYCLES "" 1}}
    
   array set bfmParamsTable $bfmTypeParamsList
   return $bfmParamsTable($bfm)
}

proc expose_border {instance_name interfaces_str {export_all ""}} {
    array set interfaces $interfaces_str
    foreach name $interfaces([ORDERED_NAMES]) {
	funset interface
	array set interface $interfaces($name)
	set export_interface 1
	if {[string compare $export_all "EXPORT_ALL"] != 0} {
		if {[info exists interface([NO_EXPORT])] == 1 && $interface([NO_EXPORT]) == 1} {
			set export_interface 0
		}
	}
	if { [info exists interface([HDL_ONLY])] == 0 && $export_interface == 1} {
...
################################################################
	if {$freq != 0} {
		if { [string first "phy_txclk_o" $pin_pattern ] != -1  }  {
			set i [string range $pin_pattern 33 33]
			set clock_name "emac${i}_tx_clk"
			fpga_interfaces::add_raw_sdc_constraint [format [SDC_CREATE_EMAC_CLOCK] $clock_name [frequency_to_period $freq] $i]
		} else {
...
#	    fpga_interfaces::set_interface_property f2h_sdram_bon_out associatedClock f2h_sdram_bon_out
    ...
    proc add_sdc { type_id } {
    #              type_id
    #                0 if axi, 1 if avalon-mm bidir,
    #                2 if avalon-mm write only, 3 if avalon-mm read-only
        
        fpga_interfaces::add_raw_sdc_constraint "# These false paths fix a problem with the HPS timing library for the F2SDRAM bridge."
        fpga_interfaces::add_raw_sdc_constraint "# This issue only affects timing analysis. There are no functionality problems with the bridge"
        foreach reg_index [ list 3768 3769 3770 3771 3773 3774 3775 3776 3778 3779 3780 3781 3783 3784 3785 3786 3790 3792 3793 3795 3796 3797 3798 3799 3800 3802 3803 3805 3806 3808 3809 3811 3812 3813 3815 3816 3817 3818 3820 3821 3822 3823 3830 3831 3832 3834 3835 3837 4494 4495 4496 4497 4498 4499 4500 4501 4502 4503 4504 4505 4506  ] {
            fpga_interfaces::add_raw_sdc_constraint "set_false_path -from \[get_registers {*fpga_interfaces|f2sdram~FF_$reg_index}\]"
        }

        foreach reg_index [ list 1054 1055 1056 1057 1118 1119 1120 1121 3405 3408 3409 3410 3414 3417 3418 3419 798 799 800 801 862 863 864 865 926 927 928 929 990 991 992 993  ] {
            fpga_interfaces::add_raw_sdc_constraint "set_false_path -to \[get_registers {*fpga_interfaces|f2sdram~FF_$reg_index}\]"
        }
        fpga_interfaces::add_raw_sdc_constraint "# ------ End of F2SDRAM bridge fix ------"

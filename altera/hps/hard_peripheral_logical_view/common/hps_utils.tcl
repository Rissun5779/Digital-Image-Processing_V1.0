...
proc hps_utils_add_component_boiler_plate {name display_name} {
	set_module_property NAME [string map { "-" "_"} ${name}]
...
proc hps_utils_add_ram_component {cname name slave_bits} {
	hps_utils_add_component_boiler_plate $cname $name
	#
	# file sets
	#
	#
	...
	set_module_assignment embeddedsw.CMacro.SIZE_VALUE 1<<$slave_bits
	set_module_assignment embeddedsw.CMacro.WRITABLE {1}
	set_module_assignment embeddedsw.memoryInfo.MEM_INIT_DATA_WIDTH $slave_bits
	...
	# 
	hps_utils_add_axi_slave axi_slave0 axi_sig0 $slave_bits

	set_interface_assignment axi_slave0 embeddedsw.configuration.isMemoryDevice {1}
	set_interface_assignment axi_slave0 embeddedsw.configuration.isNonVolatileStorage {0}
}

proc hps_utils_add_clock_out {cname} {
	add_interface $cname clock start
	set_interface_property $cname clockRate 0
	set_interface_property $cname ENABLED true
	add_interface_port $cname "${cname}_clk" clk Output 1
}
proc hps_utils_add_clock {cname} {
	add_interface $cname clock end
	set_interface_property $cname clockRate 0
	set_interface_property $cname ENABLED true
	add_interface_port $cname "${cname}_clk" clk Input 1
}
proc hps_utils_add_reset {rname cname} {
	add_interface $rname reset end
	set_interface_property $rname associatedClock $cname
	set_interface_property $rname synchronousEdges DEASSERT
	set_interface_property $rname ENABLED true
	add_interface_port $rname "${rname}_rst" reset Input 1
}
proc hps_utils_add_clock_reset {} {
	hps_utils_add_clock clock_sink

	hps_utils_add_reset reset_sink clock_sink
}
...
proc hps_utils_add_axi_slave_with_clk {int_name sig_name bits clk {reordering 1}} {
	add_interface $int_name axi end
	set_interface_property $int_name associatedClock $clk
	...
	set_interface_property $int_name readDataReorderingDepth $reordering
...
proc hps_utils_add_axi_slave {int_name sig_name bits { reordering 1 }} {
	hps_utils_add_axi_slave_with_clk $int_name $sig_name $bits clock_sink $reordering
}
...
proc hps_utils_add_instance_reset_clk {rst clk name type} {
	set type [string map { "-" "_"} ${type}]
	add_instance $name $type 18.1
	add_connection "${rst}.clk_reset" "${name}.reset_sink"
	add_connection "${clk}" "${name}.clock_sink" clock
}
proc hps_utils_add_instance_clk_reset {clk name type} {
	set type [string map { "-" "_"} ${type}]
	add_instance $name $type 18.1
	add_connection "${clk}.clk_reset" "${name}.reset_sink"
	add_connection "${clk}.clk" "${name}.clock_sink" clock
}

proc hps_utils_setup_swEnabled {} {
	add_parameter swEnabled boolean true
	set_parameter_property swEnabled affects_elaboration true
}
proc hps_utils_set_swEnabled {inst_name pinmux_name} {
	if { [get_parameter_value $pinmux_name] == "Unused" } {
		set_instance_parameter_value $inst_name swEnabled false
	}
}

proc hps_utils_elab_swEnabled {} {
	if { [get_parameter_value swEnabled] } {
		set val "okay"
	} else {
		set val "disabled"
	}
	set_module_assignment embeddedsw.dts.params.status $val
}

proc hps_utils_float2mhz {mhz_float} {
	return [expr int($mhz_float) * 1000000]
}

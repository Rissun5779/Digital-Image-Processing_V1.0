...
set_module_property NAME altera_hps
...
set_module_property DISPLAY_NAME "Arria V/Cyclone V Hard Processor System"
...
add_display_item "" "SDRAM"  "group" "tab" 
...
proc load_periph_ifaces_db {} {
    ...
    set bfm_types_file "fpga_bfm_types.csv"
...
namespace eval ::fpga_interfaces {
    source ../altera_interface_generator/api.tcl
}
...
proc compose {} {
    ...
    set_parameter_value hps_device_family [get_parameter_value DEVICE_FAMILY]
    ...
    #~~~~~~~~~~~~~~~~~~~~~ START OF BLOCK ADDED FOR EMIF ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    foreach param_name [get_instance_parameters fpga_interfaces] {
	if {$param_name != "interfaceDefinition" &&
	    $param_name != "qipEntries" &&
	    $param_name != "ignoreSimulation" &&
	    $param_name != "hps_parameter_map"} {
            set_instance_parameter fpga_interfaces $param_name [get_parameter_value $param_name]
	}
    }
    # this version of emif is for simulation only
    set_instance_parameter fpga_interfaces HHP_HPS_SIMULATION "TRUE"
    #~~~~~~~~~~~~~~~~~~~~~ END OF BLOCK ADDED FOR EMIF ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    add_instance hps_io altera_hps_io
    set_instance_parameter_value hps_io border_description [array get hps_io_border]
    # TODO: expose more than just soc_io if the EMIF starts elaborating interfaces
    ...
    # connect gpio and loanio atoms
    array set interfaces $fpga_interfaces_data(interfaces)
    foreach name $interfaces([ORDERED_NAMES]) {
        if {[string compare $name "loanio_gpio"] == 0} {
            add_connection fpga_interfaces.loanio_gpio hps_io.gpio_loanio
        }
    }

    #~~~~~~~~~~~~~~~~~~~~~ START OF BLOCK ADDED FOR EMIF ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if {!([get_parameter_value quartus_ini_hps_ip_suppress_sdram_synth] == "true")} {
    ...
    foreach param_name [get_instance_parameters hps_io] {
	if {$param_name != "border_description" &&
	    $param_name != "hps_parameter_map"} {
	    set_instance_parameter hps_io $param_name [get_parameter_value $param_name]
	}
    }
    #~~~~~~~~~~~~~~~~~~~~~ END OF BLOCK ADDED FOR EMIF ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
proc logicalview_dtg {} {
    ...
    set hard_peripheral_logical_view_dir ../hard_peripheral_logical_view
...
proc elab_clocks_resets {device_family} {
    ...
    set sdram_ref_clk [get_parameter_value REF_CLK_FREQ]
    set sdram_pll_mult [get_parameter_value PLL_MEM_CLK_MULT]
    set sdram_pll_div [get_parameter_value PLL_MEM_CLK_DIV]

    if {$sdram_pll_div > 0 } {
        set Fref [expr $sdram_pll_div / 2 ]
        set VCO  [expr $sdram_ref_clk * $sdram_pll_mult / $Fref]
    	set valid_freq [list]

    	for {set c 0} {$c <= 15} {incr c} {
    	    set Fout [expr $VCO / ($c + 1) ]
    	    lappend valid_freq "$c:$Fout"
    	}
    	set_parameter_property  S2FCLK_USER2CLK allowed_ranges $valid_freq
    }
    ...
    if [is_enabled S2FCLK_USER2CLK_Enable] {
        set_parameter_property S2FCLK_USER2CLK enabled true
	fpga_interfaces::add_interface          h2f_user2_clock                 clock  Output
	fpga_interfaces::add_interface_port     h2f_user2_clock  h2f_user2_clk  clk    Output  1     $instance_name
	set frequency      [get_parameter_value S2FCLK_USER2CLK]
	set sdram_ref_clk  [get_parameter_value REF_CLK_FREQ]
	set sdram_pll_mult [get_parameter_value PLL_MEM_CLK_MULT]
	set sdram_pll_div  [get_parameter_value PLL_MEM_CLK_DIV]
	...
	if {$sdram_pll_div > 0 } {
	    set Fref [expr $sdram_pll_div / 2 ]
	    set VCO  [expr $sdram_ref_clk * $sdram_pll_mult / $Fref]
	    set frequency [expr $VCO / ($frequency + 1) ]
	}
	set frequency [expr {$frequency * [MHZ_TO_HZ]}]
	
	fpga_interfaces::set_interface_property h2f_user2_clock clockRateKnown true
	fpga_interfaces::set_interface_property h2f_user2_clock clockRate      $frequency
	add_clock_constraint_if_valid $frequency "*|fpga_interfaces|${instance_name}|h2f_user2_clk"
    } else {
...
proc define_memory_conduit_ddr2 {instance_name} {
    ::hps_io::internal::add_interface_port memory mem_a mem_a Output           [get_parameter_value MEM_IF_ADDR_WIDTH]      $instance_name mem_a
    ::hps_io::internal::add_interface_port memory mem_ba mem_ba Output         [get_parameter_value MEM_IF_BANKADDR_WIDTH]  $instance_name mem_ba
    ::hps_io::internal::add_interface_port memory mem_ck mem_ck Output         [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck
    ::hps_io::internal::add_interface_port memory mem_ck_n mem_ck_n Output     [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck_n
    ::hps_io::internal::add_interface_port memory mem_cke mem_cke Output       [get_parameter_value MEM_IF_CLK_EN_WIDTH]    $instance_name mem_cke
    ::hps_io::internal::add_interface_port memory mem_cs_n mem_cs_n Output     [get_parameter_value MEM_IF_CS_WIDTH]        $instance_name mem_cs_n
    ::hps_io::internal::add_interface_port memory mem_ras_n mem_ras_n Output   [get_parameter_value MEM_IF_CONTROL_WIDTH]   $instance_name mem_ras_n
    ::hps_io::internal::add_interface_port memory mem_cas_n mem_cas_n Output   [get_parameter_value MEM_IF_CONTROL_WIDTH]   $instance_name mem_cas_n
    ::hps_io::internal::add_interface_port memory mem_we_n mem_we_n Output     [get_parameter_value MEM_IF_CONTROL_WIDTH]   $instance_name mem_we_n
    ::hps_io::internal::add_interface_port memory mem_dq mem_dq Bidir          [get_parameter_value MEM_IF_DQ_WIDTH]        $instance_name mem_dq
    ::hps_io::internal::add_interface_port memory mem_dqs mem_dqs Bidir        [get_parameter_value MEM_IF_DQS_WIDTH]       $instance_name mem_dqs
    ::hps_io::internal::add_interface_port memory mem_odt mem_odt Output       [get_parameter_value MEM_IF_ODT_WIDTH]       $instance_name mem_odt
    if {[string compare -nocase [get_parameter_value MEM_IF_DQSN_EN] "true"] == 0} {
	::hps_io::internal::add_interface_port memory mem_dqs_n mem_dqs_n Bidir [get_parameter_value MEM_IF_DQS_WIDTH]       $instance_name mem_dqs_n
    }
    if {[string compare -nocase [get_parameter_value MEM_IF_DM_PINS_EN] "true"] == 0} {
        ::hps_io::internal::add_interface_port memory mem_dm mem_dm Output     [get_parameter_value MEM_IF_DM_WIDTH]        $instance_name mem_dm
    }
    ::hps_io::internal::add_interface_port memory oct_rzqin oct_rzqin Input    1                                            $instance_name oct_rzqin
}

proc define_memory_conduit_ddr3 {instance_name} {
    ::hps_io::internal::add_interface_port memory mem_a mem_a Output           [get_parameter_value MEM_IF_ADDR_WIDTH]      $instance_name mem_a
    ::hps_io::internal::add_interface_port memory mem_ba mem_ba Output         [get_parameter_value MEM_IF_BANKADDR_WIDTH]  $instance_name mem_ba
    ::hps_io::internal::add_interface_port memory mem_ck mem_ck Output         [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck
    ::hps_io::internal::add_interface_port memory mem_ck_n mem_ck_n Output     [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck_n
    ...
    ::hps_io::internal::add_interface_port memory mem_cas_n mem_cas_n Output   [get_parameter_value MEM_IF_CONTROL_WIDTH]   $instance_name mem_cas_n
    ::hps_io::internal::add_interface_port memory mem_we_n mem_we_n Output     [get_parameter_value MEM_IF_CONTROL_WIDTH]   $instance_name mem_we_n
    ::hps_io::internal::add_interface_port memory mem_reset_n mem_reset_n Output 1                                          $instance_name mem_reset_n
    ::hps_io::internal::add_interface_port memory mem_dq mem_dq Bidir          [get_parameter_value MEM_IF_DQ_WIDTH]        $instance_name mem_dq
    ::hps_io::internal::add_interface_port memory mem_dqs mem_dqs Bidir        [get_parameter_value MEM_IF_DQS_WIDTH]       $instance_name mem_dqs
    ::hps_io::internal::add_interface_port memory mem_dqs_n mem_dqs_n Bidir    [get_parameter_value MEM_IF_DQS_WIDTH]       $instance_name mem_dqs_n
    ::hps_io::internal::add_interface_port memory mem_odt mem_odt Output       [get_parameter_value MEM_IF_ODT_WIDTH]       $instance_name mem_odt
...
proc define_memory_conduit_lpddr2 {instance_name} {
    ::hps_io::internal::add_interface_port memory mem_ca mem_a Output          [get_parameter_value MEM_IF_ADDR_WIDTH]      $instance_name mem_ca
    ::hps_io::internal::add_interface_port memory mem_ck mem_ck Output         [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck
    ::hps_io::internal::add_interface_port memory mem_ck_n mem_ck_n Output     [get_parameter_value MEM_IF_CK_WIDTH]        $instance_name mem_ck_n
    ::hps_io::internal::add_interface_port memory mem_cke mem_cke Output       [get_parameter_value MEM_IF_CLK_EN_WIDTH]    $instance_name mem_cke
    ::hps_io::internal::add_interface_port memory mem_cs_n mem_cs_n Output     [get_parameter_value MEM_IF_CS_WIDTH]        $instance_name mem_cs_n
...
proc elab_EMIF {device_family} {

    set suppress_emif_synth [expr {[get_parameter_value quartus_ini_hps_ip_suppress_sdram_synth] == "true"}]

    validate_emif_component

    ::hps_io::internal::set_property SUPPRESS_SDRAM_SYNTH $suppress_emif_synth

    if {!$suppress_emif_synth} {

	set location {}
	set instance_name "hps_sdram_inst"
	set entity_name  "hps_sdram"
	::hps_io::internal::add_module_instance $instance_name $entity_name $location
	
        ::hps_io::internal::add_interface memory conduit end

	set protocol [get_parameter_value HPS_PROTOCOL]

	if {[string compare -nocase $protocol "DDR2"] == 0} {
		define_memory_conduit_ddr2 $instance_name
	} elseif {[string compare -nocase $protocol "DDR3"] == 0} {
		define_memory_conduit_ddr3 $instance_name
	} elseif {[string compare -nocase $protocol "LPDDR2"] == 0} {
		define_memory_conduit_lpddr2 $instance_name
	} else {
		send_message error "Unsupported protocol $protocol for SDRAM on HPS"
        }
    }
}
#~~~~~~~~~~~~~~~~~~~~~ END OF BLOCK ADDED FOR EMIF ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

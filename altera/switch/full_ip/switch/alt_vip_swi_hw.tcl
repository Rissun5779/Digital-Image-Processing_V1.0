...
proc swi_elaboration_callback {} {
    ...
    send_message info "The Switch will no longer be available after 16.0, please upgrade to Switch II."
}
...
proc swi_generation_callback {} {
    ...
    # Depending on the platform type and bitness, find the correct bin and java path:
    # For windows, the path will contain backslashes, so replace all of these to be platform independent
    ...
    # Try to get the BINDIR from the env vars, but if not, work it out manually
    ...
    if {$POINTERSIZE == 8} {
        #64 bit
        set java_path "${QUARTUS_BINDIR}/jre64/bin/java"
    } else {
        #32 bit
        set java_path "${QUARTUS_BINDIR}/jre/bin/java"
    }
    
    send_message info "Using java executable: '$java_path'"

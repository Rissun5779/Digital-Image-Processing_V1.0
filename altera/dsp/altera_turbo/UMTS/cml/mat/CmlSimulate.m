...
for i = 1:length(sim_param)
    if (sim_param(i).rtl_simulation == 1)
        ...
        if (sim_param(i).umts == 1)
            files(1).name = strcat(dump_dir, '/ctc_umts_dec_input_info.txt');
            files(2).name = strcat(dump_dir, '/ctc_umts_dec_input_data.txt');
            files(3).name = strcat(dump_dir, '/ctc_umts_dec_output_gold.txt');
            files(4).name = strcat(dump_dir, '/ctc_umts_dec_output_u_gold.txt');
            files(5).name = strcat(dump_dir, '/ctc_umts_dec_output_info_gold.txt');
            files(6).name = strcat(dump_dir, '/ctc_umts_enc_input_data.txt');
            files(7).name = strcat(dump_dir, '/ctc_umts_enc_input_info.txt');
            files(8).name = strcat(dump_dir, '/ctc_umts_enc_output_gold.txt');
        else
            files(1).name = strcat(dump_dir, '/ctc_input_info.txt');
            files(2).name = strcat(dump_dir, '/ctc_input_data.txt');
            files(3).name = strcat(dump_dir, '/ctc_decoded_output_gold.txt');
            files(4).name = strcat(dump_dir, '/ctc_output_u_gold.txt');
            files(5).name = strcat(dump_dir, '/ctc_output_et_info_gold.txt');

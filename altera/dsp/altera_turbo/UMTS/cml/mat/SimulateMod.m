...
for snrpoint = 1:length(EsNo)
    ...
    while ( ( sim_state.trials( code_param.max_iterations, snrpoint ) < sim_param.max_trials( snrpoint ) )&( sim_state.frame_errors(code_param.max_iterations, snrpoint) < sim_param.max_frame_errors(snrpoint) ) )
        ...
        % Append CRC to the code block if crc_et is on
        if (sim_param.crc_et == 1)
        	crc_type = sim_param.crc_type;
        	crc_reflect_out = sim_param.crc_reflect_out;
            crc_input = data(1:end-24);
        	crc_value = crcCheck(crc_input, crc_type, crc_reflect_out);
            crc_bin = str2num(reshape(dec2bin(crc_value, 24), 24,1))';
            
            data = [crc_input crc_bin];
            crc_value = crcCheck(data, crc_type, crc_reflect_out);
            
            if (crc_value ~= 0)
                error('CRC is appended incorrectly!');
            end;
        end;
        ...
        if 	(sim_param.rtl_simulation == 1)
            if (sim_param.umts == 1)
                cml_save_to_file(data', [sim_param.dump_dir '/ctc_umts_enc_input_data.txt'], 0);
                cml_save_to_file([sim_param.framesize], [sim_param.dump_dir '/ctc_umts_enc_input_info.txt'], 0);
                cml_save_to_file(codeword', [sim_param.dump_dir '/ctc_umts_enc_output_gold.txt'], 0);
            else

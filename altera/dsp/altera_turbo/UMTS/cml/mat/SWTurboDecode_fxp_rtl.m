function [detected_data, errors, output_decoder_c, input_upper_u] = SWTurboDecode_fxp_rtl( input_decoder_c, data, turbo_iterations, decoder_type, code_interleaver, pun_pattern, tail_pattern, g1, nsc_flag1, g2, nsc_flag2, num_engines, sldwin_size, fxp_data_width, varargin, sim_param)
...
%assign debug stuff
dump_input = sim_param.dump_input;
dump_iter = sim_param.dump_iter;
dump_output = sim_param.dump_output;
dump_dir = sim_param.dump_dir;
crc_type = sim_param.crc_type;
crc_et = sim_param.crc_et;
crc_reflect_out = sim_param.crc_reflect_out;
...
%        input_decoder_c = reshape(eval(sprintf('ctc_input%d',K_interleaver))',1,[]) ./ (2^bwr);
    ...
    input_decoder_c = fix(input_decoder_c .* (2^bwr)) ./ (2^bwr);
    ...
    % dump the input data. This will be used for the rtl simulation input
    
    if (sim_param.rtl_simulation == 1) 
        reshaped_input = reshape(input_decoder_c,3,length(input_decoder_c)/3);
        if (sim_param.umts == 1)
            cml_save_to_file(reshaped_input', [dump_dir '/ctc_umts_dec_input_data.txt'], bwr);
            cml_save_to_file([K_interleaver + 4, turbo_iterations*2], [dump_dir '/ctc_umts_dec_input_info.txt'], 0);
        else
            cml_save_to_file(reshaped_input', [dump_dir '/ctc_input_data.txt'], bwr);
            cml_save_to_file([K_interleaver + 4, turbo_iterations*2, crc_type], [dump_dir '/ctc_input_info.txt'], 0);
        end    
%	    save 'input.mat' data input_upper_u input_decoder_c;
    end
    ...
    for turbo_iter=1:turbo_iterations
        ...
        if (sim_param.code_configuration ~= 4)
	        [output_upper_u output_upper_c output_beta1 output_llr_upper_u] = SWSisoDecode_fxp( input_upper_u(codeword_index,:), input_upper_c, g1, nsc_flag1, decoder_type, ...
    	    	num_engines, sldwin_size, input_beta1, 2*turbo_iter - 1, fxp_data_width );
		else % UMTS
	        [output_upper_u output_upper_c output_beta1 output_llr_upper_u] = SWSisoDecode_umts_fxp( input_upper_u(codeword_index,:), input_upper_c, g1, nsc_flag1, decoder_type, ...
    	    	num_engines, sldwin_size, input_beta1, 2*turbo_iter - 1, fxp_data_width );
		end
        ...
        % dump the output data. This will be used for the rtl simulation golden
        if (dump_iter == 1)
            len = ceil(K_interleaver/num_engines)*num_engines;
            out_upper_u = output_upper_u;
            if (len > length(output_upper_u))
                out_upper_u(len) = 0;
            end
            out_u =reshape(out_upper_u,len/num_engines,num_engines);b=out_u';c=b(end:-1:1,:);
            if (sim_param.umts == 1)
                %cml_save_to_file(c', [dump_dir '/ctc_umts_dec_output_u_gold.txt'], bwr);
            else
                cml_save_to_file(c', [dump_dir '/ctc_output_u_gold.txt'], bwr);
            end
        end

        detected_data(codeword_index,:) = (sign(output_llr_upper_u)+1)/2;
        error_positions = xor( detected_data(codeword_index,:), data(codeword_index,:) );

        % Stop when CRC check passes!
        if (sim_param.rtl_simulation == 1 && crc_et == 1)
            crc_value = crcCheck(ceil(detected_data), crc_type, crc_reflect_out);
            %fprintf('The data %s returned %d for the %d th iteration.\n',num2str(ceil(detected_data)), crc_value, 2*turbo_iter-1);
            if(crc_value == 0)
                cml_save_to_file([K_interleaver, 2*turbo_iter-2, 1, crc_type], [dump_dir '/ctc_output_et_info_gold.txt'],0);
                fprintf('CRC type is %d ---',crc_type);
                fprintf('Terminating at %d half iteration(s).', 2*turbo_iter - 1);

                % adding the following just to avoid error
                output_lower_c = output_upper_c;
                break;
            end;
        end;
        ...
        if (sim_param.code_configuration ~= 4)
	        [output_lower_u output_lower_c output_beta2 output_llr_lower_u] = SWSisoDecode_fxp( input_lower_u, input_lower_c, g2, nsc_flag2, decoder_type, ...
    	    		num_engines, sldwin_size, input_beta2, 2*turbo_iter, fxp_data_width);
        else % UMTS
	        [output_lower_u output_lower_c output_beta2 output_llr_lower_u] = SWSisoDecode_umts_fxp( input_lower_u, input_lower_c, g2, nsc_flag2, decoder_type, ...
    	    		num_engines, sldwin_size, input_beta2, 2*turbo_iter, fxp_data_width);
        end
        input_beta2 = output_beta2;
          
        % dump the output data. This will be used for the rtl simulation golden
        if (sim_param.rtl_simulation == 1 && dump_iter == 1)
            len = ceil(K_interleaver/num_engines)*num_engines;
            out_lower_u = output_lower_u;
            if (len > length(output_lower_u))
                out_lower_u(len) = 0;
            end
            out_u = reshape(out_lower_u,len/num_engines,num_engines);b=out_u';c=b(end:-1:1,:);
            if (sim_param.umts == 1)
                %cml_save_to_file(c', [dump_dir '/ctc_umts_dec_output_u_gold.txt'], bwr);
            else
                cml_save_to_file(c', [dump_dir '/ctc_output_u_gold.txt'], bwr);
        ...
        % count errors
        detected_data(codeword_index,:) = Deinterleave( (sign(output_llr_lower_u)+1)/2, code_interleaver );
        error_positions = xor( detected_data(codeword_index,:), data(codeword_index,:) );       
        ...
        if (sim_param.rtl_simulation == 1 && crc_et == 1)
            if (sim_param.umts == 1)
                cml_save_to_file([K_interleaver, 2*turbo_iter-1], [dump_dir '/ctc_umts_dec_output_info_gold.txt'], 0);
            else
                crc_value = crcCheck(ceil(detected_data), crc_type, crc_reflect_out);
                %fprintf('The data %s returned %d for the %d th iteration.\n',num2str(ceil(detected_data)), crc_value, 2*turbo_iter);
                if(crc_value == 0)
                    cml_save_to_file([K_interleaver, 2*turbo_iter-1, 1, crc_type], [dump_dir '/ctc_output_et_info_gold.txt'],0);
                    fprintf('CRC type is %d ---',crc_type);
                    fprintf('Terminating at %d half iteration(s).', 2*turbo_iter);
                    break; 
                end;
            end;
        end;
    ...
    if (sim_param.rtl_simulation == 1) 
        if (sim_param.umts == 1)
            cml_save_to_file(ceil(detected_data'), [dump_dir '/ctc_umts_dec_output_gold.txt'], 0);
        else
            cml_save_to_file(ceil(detected_data'), [dump_dir '/ctc_decoded_output_gold.txt'], 0);
        end
    end
    
    if (sim_param.rtl_simulation == 1)
        if (sim_param.umts == 1)
            cml_save_to_file([K_interleaver, 2*turbo_iterations-1], [dump_dir '/ctc_umts_dec_output_info_gold.txt'], 0);
        elseif (crc_et == 1 && crc_value ~= 0)
            cml_save_to_file([K_interleaver, 2*turbo_iterations-1, 0, crc_type], [dump_dir '/ctc_output_et_info_gold.txt'], 0);
            fprintf('CRC type is %d ---',crc_type);
            fprintf('Reached full iterations. No early termination happens!');
        end;
    end;

...
proc generate_enc_ROM_rtl {entity} {
    ...
    if {$isvarrate} {
        ...
        set nbRAM1 $nbcheckgroup
        set nbvar1 $nbvargroup
    }
    
    set nbparitygroup [list $nbRAM1 10 15 20] 
    set nbparityvar   [list $nbvar1 30 25 20]
    ...
    if {$islongframe} {
        set nbparitygroup_temp [list ]
        for {set ii 0} {$ii<[llength $nbparitygroup]} {incr ii} {
            lappend nbparitygroup_temp [expr [lindex $nbparitygroup $ii]+4]
        }
        set nbparitygroup $nbparitygroup_temp
    }
    set nbparitygroup [concat [list 0] $nbparitygroup]
    ...
    append text     "module altera_ldpc_wimedia_enc_ROMs(\n"
    append text     "    clk, \n"
    append text     "    rst, \n"
    append text     "    in_addr, \n"
    ...
    append text     "localparam  Z              = 30;\n"
    if {$isvarrate} {
        append text     "localparam  ADDR_WIDTH     =  7;\n"
        append text     "localparam  OUTDATA_WIDTH  = [expr 20+4*$islongframe];\n\n"
    } else {
        append text     "localparam  ADDR_WIDTH     =  5;\n"
        append text     "localparam  OUTDATA_WIDTH  = [expr $nbcheckgroup+4*$islongframe];\n\n"
    }
    ...
    for {set ii 0} {$ii < [expr 1+3*$isvarrate]} {incr ii} {

        set nb_addr [expr $nb_addr_per_code*((3-$ii)*$isvarrate+1)]
        set nb_addr_pow2 [expr 1<<[clog2 $nb_addr]]
        set addr_width [clog2 [expr $nb_addr_pow2]]
    
        for {set jj [lindex $nbparitygroup $ii]} {$jj < [lindex $nbparitygroup $ii+1]} {incr jj} {
            ...
            append text     "            .ram_block_type(\"M20K\"),\n"
    ...
    add_fileset_file "altera_auto_wimedia_enc_ROM.sv" SYSTEM_VERILOG TEXT $text


    
    set grouped_data [list ]
    for {set ff 0} {$ff<[expr 1+3*$isvarrate]} {incr ff} {
        if {$isvarrate} {
            set cc [expr $ff+1]
        } else {
            set cc $code_is
        }
        set fp [open "$cwd/code${cc}_ROM.hex" r]
    ...
    for {set ii 0} {$ii < [expr 1+3*$isvarrate]} {incr ii} {
        ...
        for {set jj [lindex $nbparitygroup $ii]} {$jj < [lindex $nbparitygroup $ii+1]} {incr jj} {
            ...
            for {set kk 0} {$kk < [expr (3-$ii)*$isvarrate+1]} {incr kk} {
                set code_number [expr (3-$kk)*$isvarrate]
                set offset [expr $jj*[lindex $nbparityvar $code_number]]

                # writing parity for code (4-kk) in parity group parity_group
                for {set ll 0} {$ll < [lindex $nbparityvar $code_number]} {incr ll} {
                    lappend parity_list [lindex [lindex $grouped_data $code_number]  [expr $ll+$offset] ]
                }
    
                # filing up with zeros
                if {[lindex $nbparityvar $code_number]!=$nb_addr_per_code} {
                    for {set ll 0} {$ll < $nb_addr_per_code-[lindex $nbparityvar $code_number]} {incr ll} {

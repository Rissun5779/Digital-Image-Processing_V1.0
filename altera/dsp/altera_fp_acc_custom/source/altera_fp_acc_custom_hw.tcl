...
set_module_property DISPLAY_NAME ALTERA_FP_ACC_CUSTOM
set_module_property DESCRIPTION "An application-specific floating point accumulator. A floating point accumulator that can be customized to the required range of input and output values. The accumulator will be built to operate at the target frequency on the target device family." 
...
set_parameter_property MSBA DESCRIPTION "An upper bound on the order of magnitude of the accumulated result. The accumulator cannot exceed <i>2<sup>MSBA</sup>\u00d71.m </i> where MSBA is the exponent, and m is the mantissa"
...
set_parameter_property maxMSBX DESCRIPTION "An upper bound on the order of magnitude of the inputs to the accumulator. Any input X where<i> X=2<sup>e</sup>\u00d71.m </i> with exponent e greater than maxMSBX, and any mantissa m, will cause the accumulator to overflow."
...
set_parameter_property LSBA DESCRIPTION "A parameter which controls the accuracy of the accumulation. Any input<i> X=2<sup>e</sup>\u00d71.m </i>with exponent e less than lsbA, and any mantissa m, will be shifted out of the accumulator."
...
proc elaboration_callback {} {
	...
	if { bool([get_parameter_value validation_failed]) == bool(false) } {
		...
		set output [call_cmdPolyEval "none" $quartus_dir "." "report"]
...
proc call_cmdPolyEval { entity_name quartus_dir tmp_dir call_type } {
	...
	lappend args [get_parameter_value frequency]
	lappend args "-name"
	...
	lappend args [get_parameter_value LSBA]
	lappend args [get_parameter_value MSBA]
	lappend args [get_parameter_value maxMSBX]

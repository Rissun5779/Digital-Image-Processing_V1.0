...
architecture rtl of auk_dspip_fifo_pfc is
  ...
  signal first_data_reg : std_logic_vector(DATA_WIDTH_g-1 downto 0);
  signal first_wrreq : std_logic;
  signal first_rdreq : std_logic;
...
-- if (write_section = read_section) then
    ...
    if reset = '1' then                 -- asynchronous reset (active high)
      ...
      if enable = '1' then
        if sclr = '1' then
          ...
          if wrreq = '1' and first_wrreq = '0' then
            datao_alt <= datai;
          elsif rdreq_del = '1' then
  ...
  datao <= datao_early when rdreq_del = '1' and first_rdreq='1' else datao_alt;

  --first data needs to be handled seperately
  first_dat : process (clk, reset)
  begin  -- process first_dat
    if reset = '1' then
--      first_data_reg <= (others => '0');
      first_wrreq <= '0';
      first_rdreq <= '0';
    elsif rising_edge(clk) then
      if enable = '1' then
        first_wrreq <= wrreq or first_wrreq;
        first_rdreq <= rdreq_del or first_rdreq;
--        if wrreq = '1' and first_wrreq = '0' then
--          first_data_reg <= datai;
--        end if;
      end if;
    end if;
  end process first_dat;

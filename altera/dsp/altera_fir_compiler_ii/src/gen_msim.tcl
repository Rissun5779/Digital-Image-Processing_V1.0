...
set current_year  [clock format [clock seconds] -format "%Y"]
	...
	puts $out_file "## ================================================================================
## Legal Notice: Copyright (C) 1991-$current_year Altera Corporation. All rights reserved.
...
#set tag "$tag"
	...
	puts $out_file "        altera_mf    {altera_mf_components altera_mf} {} {} \"\$q_sim_lib\" \\"
	puts $out_file "        lpm          {220pack 220model}               {220model} {}  \"\$q_sim_lib\" \\"
	puts $out_file "        sgate        {sgate_pack sgate}               {sgate} {}     \"\$q_sim_lib\" \\"
	puts $out_file "        altera_lnsim {altera_lnsim_components}     {}   {mentor/altera_lnsim_for_vhdl} \"\$q_sim_lib\" \\"
	puts $out_file "        twentynm        {twentynm_atoms twentynm_components}               {} {}     \"\$q_sim_lib\" \\"

	puts $out_file "    \]"
	
	puts $out_file "	foreach {lib file_vhdl_list file_verilog_list file_sysverilog_list src_files_loc} \$quartus_libs {"
	puts $out_file "        if {\[file exist \$lib\]} {"
	puts $out_file "            catch {eval \"file delete -force -- \$lib\"} fid"
	puts $out_file "            puts \"file delete command returned \$fid\\n\""
	puts $out_file "        }"
	...
	puts $out_file "		  catch {vcom -quiet -explicit -93 -work \$lib \[file join \$src_files_loc \${file_item}.vhd\]} err_msg"
	puts $out_file "		  if {!\[string match \"\" \$err_msg\]} {return \$err_msg}"
	puts $out_file "		}"
	puts $out_file "		foreach file_item \$file_verilog_list {"
	puts $out_file "		  catch {vlog -work \$lib \[file join \$src_files_loc \${file_item}.v\]} err_msg"
	puts $out_file "		  if {!\[string match \"\" \$err_msg\]} {return \$err_msg}"
	puts $out_file "		}"
	puts $out_file "		foreach file_item \$file_sysverilog_list {"
	puts $out_file "		  catch {vlog -work \$lib \[file join \$src_files_loc \${file_item}.sv\]} err_msg"
	puts $out_file "		  if {!\[string match \"\" \$err_msg\]} {return \$err_msg}"
	puts $out_file "		}"
	puts $out_file "	}"
	puts $out_file ""
	...
	puts $out_file "    vcom -work work dspba_library_package.vhd"
	puts $out_file "    vcom -work work dspba_library.vhd"
	puts $out_file "    vcom -work work auk_dspip_roundsat_hpfir.vhd"
	puts $out_file "    vcom -work work auk_dspip_math_pkg_hpfir.vhd"
	puts $out_file "    vcom -work work auk_dspip_lib_pkg_hpfir.vhd"
	puts $out_file "    vcom -work work auk_dspip_avalon_streaming_controller_hpfir.vhd"
	puts $out_file "    vcom -work work auk_dspip_avalon_streaming_sink_hpfir.vhd"
	puts $out_file "    vcom -work work auk_dspip_avalon_streaming_source_hpfir.vhd"
puts $out_file {
	...
	puts $out_file "    vcom -work work \${top_entity}_ast.vhd"
	puts $out_file "    vcom -work work \${top_entity}.vhd"
	puts $out_file ""
	...
	puts $out_file "    lappend vsim_cmd \"-L\" \"\$device_lib_name\" \"-L\" \"altera_mf\" \"-L\" \"lpm\" \"-L\" \"sgate\" \"-L\" \"altera\" \"-L\" \"altera_lnsim\" \"-L\" \"work\""
	puts $out_file "} else {"

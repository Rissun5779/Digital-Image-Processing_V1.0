...
set readfile_name ${module_dir}/ast_components/hpfir_ast_temp${extension}.template
...
set ModeWidth [lindex $argv 24]
set ComplexData [lindex $argv 25]
...
set log2_outputfifodepth [expr {log($outputfifodepth)/log(2)}]
if {$log2_outputfifodepth > 1} {
    set log2_outputfifodepth [expr {ceil($log2_outputfifodepth)}]
} else {
    set log2_outputfifodepth 1
}


set antilog2_outputfifodepth [expr {int(2 ** $log2_outputfifodepth)}]
...
set ComplexConstant 1
if { $ComplexData eq "true" } {
    set ComplexConstant 2
}
...
while {-1 != [gets $in line]} {
    if {[regexp -nocase "update generic here" $line]} {
        ...
        puts $out "  MODE_WIDTH         : integer := $ModeWidth;"
        ...
        puts $out "  COMPLEX_CONST       : integer := $ComplexConstant"
        ...
        set component_name [lindex [split $line] end]
        puts $out "component ${entity_name}_${component_name} is"
        puts $out "  port ("
        ...
        if { $ModeWidth > 0 } {
            puts $out "    xIn_mode              : in std_logic_vector(MODE_WIDTH-1 downto 0);"
        }
        if {$bankcount > 1} {
          for {set i 0} {$i < $PhysChanIn} {incr i} {
        ...
        if {$Read_Write_Mode != "none"} {
            puts $out "    bus_clk            : in std_logic;"
            puts $out "    h_areset           : in std_logic;"
            puts $out "    busIn_writedata            : in std_logic_vector($busDataWidth_msb downto 0);"
            puts $out "    busIn_address            : in std_logic_vector(11 downto 0);"
            puts $out "    busIn_write            : in std_logic_vector(0 downto 0);"
            if {$Read_Write_Mode != "write"} {
                puts $out "    busOut_readdata           : out std_logic_vector($busDataWidth_msb downto 0);"
                puts $out "    busOut_readdatavalid           : out std_logic_vector(0 downto 0);"
            }
        }
        ...
        puts $out ");"
        puts $out "end component ${entity_name}_${component_name};"
    } elseif {[regexp -nocase "update instantiation here" $line]} {
        set component_name [lindex [split $line] end]
        puts $out "hpfircore_${component_name}: ${entity_name}_${component_name}"
        puts $out "   port map ("
        puts $out "     xIn_v     => data_valid_${component_name},"
        puts $out "     xIn_c     => \"00000000\","
        for {set i 0} {$i < $PhysChanIn} {incr i} {
            puts $out "     xIn_$i     => data_in_${component_name}(($bankInWidth + $InWidth) * $i + $InWidth - 1 downto ($bankInWidth + $InWidth) * $i),"
        }
        if {$bankcount > 1} {
          for {set i 0} {$i < $PhysChanIn} {incr i} {
            puts $out "     bankIn_$i  => data_in_${component_name}(($bankInWidth + $InWidth) * $i + ($bankInWidth + $InWidth) - 1 downto ($bankInWidth + $InWidth) * $i + $InWidth),"
          }
        }
        if { $ModeWidth > 0 } {
            puts $out "     xIn_mode  => data_in_${component_name}((INWIDTH + BANKINWIDTH) * PHYSCHANIN  + MODE_WIDTH - 1  downto (INWIDTH + BANKINWIDTH) * PHYSCHANIN),"
        }
        puts $out "     xOut_v    => core_out_valid_${component_name},"
        puts $out "     xOut_c    => core_out_channel_${component_name},"
        for {set j 0} {$j < $PhysChanOut} {incr j} {
            puts $out "     xOut_$j   => core_out_${component_name}($FullWidth * $j + $FullWidth - 1 downto $FullWidth * $j),"
        }
        if {$useClkEnable == "true"} {
        ...
        if {$Read_Write_Mode != "none"} {
            ...
            puts $out "     busIn_writedata   => busIn_d_${component_name},"
            puts $out "     busIn_address   => busIn_a_${component_name},"
            puts $out "     busIn_write   => busIn_w_${component_name},"
            if {$Read_Write_Mode != "write"} {
                puts $out "     busOut_readdata  => busOut_r_${component_name},"
                puts $out "     busOut_readdatavalid  => busOut_v_${component_name},"
            }
        }
        puts $out "     clk       => clk,"
        ...
        puts $out "   );"
        if {$Read_Write_Mode != "none"} {
            puts $out "    busIn_d_${component_name}            : in std_logic_vector($busDataWidth_msb downto 0);"
            puts $out "    busIn_a_${component_name}            : in std_logic_vector(11 downto 0);"
            puts $out "    busIn_w_${component_name}            : in std_logic_vector(0 downto 0);"
            if {$Read_Write_Mode != "write"} {
                puts $out "    busOut_r_${component_name}           : out std_logic_vector($busDataWidth_msb downto 0);"
                puts $out "    busOut_v_${component_name}           : out std_logic_vector(0 downto 0);"
            }
        }
    } else {

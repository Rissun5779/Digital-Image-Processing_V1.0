...
set current_year  [clock format [clock seconds] -format "%Y"]
	...
	puts $out_file "## ================================================================================"
	puts $out_file "## Legal Notice: Copyright (C) $current_year Intel Corporation. All rights reserved."
	...
	puts $out_file "## associated documentation or information provided by Intel or a partner"
	puts $out_file "## under Intel's Megafunction Partnership Program may be used only to"
	puts $out_file "## program PLD devices (but not masked PLD devices) from Intel.  Any other"
	...
	puts $out_file "## a separate agreement with Intel or a megafunction partner.  Title to"
	...
	puts $out_file "## any other related documentation or information provided by Intel or a"
	puts $out_file "## megafunction partner, remains with Intel, the megafunction partner, or"
	...
	puts $out_file "set file_dir \[file dirname \[info script\]\]"
...
#	puts $out_file ""		
	...
	puts $out_file "if {\[file exists \$file_dir/${entity_name}_coef_reload.txt\]} {"
	puts $out_file "  set_global_assignment -name EDA_TEST_BENCH_FILE \$file_dir/${entity_name}_coef_reload.txt -section_id tb -library work"
	puts $out_file "}"

	puts $out_file "if {\[file exists \$file_dir/${entity_name}_coef_reload_rtl.txt\]} {"
	puts $out_file "  set_global_assignment -name EDA_TEST_BENCH_FILE \$file_dir/${entity_name}_coef_reload_rtl.txt -section_id tb -library work"
	puts $out_file "}"

	puts $out_file "set_global_assignment -name EDA_TEST_BENCH_FILE \$file_dir/${entity_name}_input.txt -section_id tb -library work"
	puts $out_file ""	
	puts $out_file "set_global_assignment -name EDA_TEST_BENCH_FILE \$file_dir/${entity_name}_tb.vhd -section_id tb -library work"
	puts $out_file ""	

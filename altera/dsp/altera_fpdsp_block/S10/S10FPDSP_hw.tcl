...
# | module 
# |
set_module_property NAME altera_s10fpdsp_block
...
set_module_property DISPLAY_NAME "Stratix 10 Native Floating Point DSP"
...
add_parameter clear_type string "SCLR"
set_parameter_property clear_type VISIBLE true
set_parameter_property clear_type DISPLAY_NAME "Clear Type"
set_parameter_property clear_type ALLOWED_RANGES [list "NONE:None" "SCLR:Synchronous" "ACLR:Asynchronous"]
set_parameter_property clear_type DESCRIPTION "Choose between a synchronous, asynchronous or no clear"

add_parameter single_clear boolean false
set_parameter_property single_clear VISIBLE true
set_parameter_property single_clear DISPLAY_NAME "Single Clear"
...
add_parameter exception_flags boolean false
set_parameter_property exception_flags VISIBLE true
set_parameter_property exception_flags DISPLAY_NAME "Connect Exception Flags"
set_parameter_property exception_flags DESCRIPTION "Choose whether to export the exception flags, or to allow them to hang"

add_parameter adder_exceptions boolean false
set_parameter_property adder_exceptions VISIBLE false
set_parameter_property adder_exceptions DERIVED true

add_parameter mult_exceptions boolean false
set_parameter_property mult_exceptions VISIBLE false
set_parameter_property mult_exceptions DERIVED true


add_display_item "DSP Block View" widget_group group tab
add_display_item widget_group arria_widget parameter
set jar_path [file join $env(QUARTUS_ROOTDIR) .. ip altera dsp altera_fpdsp_block S10 com.altera.dsp.fpdsp.ui.jar]
...
proc get_clocks {} {
    return  [list accumulate_clock ax_clock ay_clock az_clock accum_pipeline_clock ax_chainin_pl_clock mult_pipeline_clock accum_2nd_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock accum_adder_clock adder_input_clock adder_input_clock_2 output_clock output_clock_2 ]
}
#Set of clocks that actually show up in the HDL
proc get_hdl_clocks {} {
    return  [list accumulate_clock ax_clock ay_clock az_clock accum_pipeline_clock ax_chainin_pl_clock mult_pipeline_clock accum_2nd_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock accum_adder_clock adder_input_clock output_clock ]
}


proc set_exception_flags {} {
        set_parameter_value adder_exceptions false
        set_parameter_value mult_exceptions false
    if {[get_parameter_value exception_flags] } {
        if { [string equal [get_parameter_value OPERATION] "sp_mult"] } {
            set_parameter_value mult_exceptions true
        } elseif  { [string equal [get_parameter_value OPERATION] "sp_add"] } {
            set_parameter_value adder_exceptions true
...
proc get_used_registers {} {
   if { [string equal [get_parameter_value OPERATION] "sp_mult"] } {
        return  [list ay_clock az_clock mult_pipeline_clock mult_2nd_pipeline_clock output_clock output_clock_2]
   } elseif  { [string equal [get_parameter_value OPERATION] "sp_add"] } {
        return  [list   ax_clock  ay_clock  ax_chainin_pl_clock  ax_chainin_2nd_pl_clock  adder_input_clock  adder_input_clock_2  output_clock output_clock_2]
   } elseif  { [string equal [get_parameter_value OPERATION] "sp_mult_add"] } {
        if { [get_parameter_value chain_mux] } {
            return  [list  ay_clock az_clock ax_chainin_pl_clock mult_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock adder_input_clock adder_input_clock_2 output_clock output_clock_2]
        } else {
            return  [list  ax_clock ay_clock az_clock ax_chainin_pl_clock mult_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock adder_input_clock adder_input_clock_2 output_clock output_clock_2]
        }
   } elseif  { [string equal [get_parameter_value OPERATION] "sp_mult_acc"] } {
        return  [list  accumulate_clock ay_clock az_clock accum_pipeline_clock mult_pipeline_clock accum_2nd_pipeline_clock mult_2nd_pipeline_clock accum_adder_clock adder_input_clock_2 output_clock output_clock_2]
   } elseif  { [string equal [get_parameter_value OPERATION] "sp_vector1"] } {
        if { [get_parameter_value chain_mux] } {
            return  [list  ax_clock ay_clock az_clock ax_chainin_pl_clock ax_chainin_2nd_pl_clock mult_pipeline_clock mult_2nd_pipeline_clock adder_input_clock adder_input_clock_2 output_clock]
        } else {
            return  [list  ax_clock ay_clock az_clock  mult_pipeline_clock mult_2nd_pipeline_clock  adder_input_clock_2 output_clock]
        }
   } elseif  { [string equal [get_parameter_value OPERATION] "sp_vector2"] } {
        if { [get_parameter_value chain_mux] } {
            return  [list  ax_clock ay_clock az_clock ax_chainin_pl_clock mult_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock adder_input_clock adder_input_clock_2 output_clock output_clock_2]
        } else {
            return  [list  ax_clock ay_clock az_clock mult_pipeline_clock mult_2nd_pipeline_clock adder_input_clock_2 output_clock output_clock_2]
        }
   } else {
        return  [list  accumulate_clock ax_clock ay_clock az_clock accum_pipeline_clock ax_chainin_pl_clock mult_pipeline_clock accum_2nd_pipeline_clock ax_chainin_2nd_pl_clock mult_2nd_pipeline_clock accum_adder_clock adder_input_clock adder_input_clock_2 output_clock output_clock_2]
   }
    ...
    ax_chainin_2nd_pl_clock     ax_chainin_2nd_pl_clock                 
    mult_2nd_pipeline_clock     mult_2nd_pipeline_clock                 
    accum_adder_clock           accum_adder_clock           
    adder_input_clock           adder_input_clock           
    adder_input_clock_2         adder_input_clock_2             
    output_clock                output_clock      
    output_clock_2              output_clock_2        
...
proc render_terp {output_name terp_file} {
    ...
    set params(used_clocks) [get_used_registers] 
...
proc elab {} {
    ...
    foreach clock [get_clocks] {
        if { [string equal [get_parameter_value $clock]  "0"] && ([lsearch -nocase [get_used_registers]  $clock  ] != -1)  } {
            set_parameter_value clk0_used true
        }        
        if { [string equal [get_parameter_value $clock]  "1"] && ([lsearch -nocase [get_used_registers]  $clock  ] != -1) } {
            set_parameter_value clk1_used true
        }         
        if { [string equal [get_parameter_value $clock]  "2"] && ([lsearch -nocase [get_used_registers]  $clock  ] != -1) } {
    ...
    if { [get_parameter_value clear_type] eq "SCLR"} {
        if { [get_parameter_value single_clear]  } {
            add_interface_input clr 1
        } else {
            add_interface_input clr 2
        }
    } elseif { [get_parameter_value clear_type] eq "ACLR"} {
        if { [get_parameter_value single_clear]  } {
    ...
    if {$operation eq "sp_mult"} {
        ...
        add_interface_input "ay" 32
        add_interface_input "az" 32
    } elseif {$operation eq "sp_vector1"} {
        if {[get_parameter_value chain_mux]} {
            add_interface_input "chainin" 32
            set_parameter_value use_chainin true
        } else {
            set_parameter_value use_chainin false
        }
    ...
    if {[get_parameter_value mult_exceptions] } {
        add_interface_output "mult_overflow" 1
        add_interface_output "mult_underflow" 1
        add_interface_output "mult_invalid" 1
        add_interface_output "mult_inexact" 1
    }
    if {[get_parameter_value adder_exceptions] } {
        add_interface_output "adder_overflow" 1
        add_interface_output "adder_underflow" 1
        add_interface_output "adder_invalid" 1
        add_interface_output "adder_inexact" 1
    }
    ...
    set_parameter_value possible_registers [join [get_used_registers] ","]
}

# There are a set of constaints which essentially boil down to "if you're going to use both these registers, they must use the same clock"
# This process sends a message ERRORing if the registers presented as arguments fail that test.
proc check_matching_register_bank registers {
    foreach register $registers {
        set only_clock "NONE"
        if { [lsearch -nocase [get_used_registers]  $register  ] != -1 && [get_parameter_value "${register}"] != "NONE" } {
            if { $only_clock == "NONE" } {
                set only_clock [get_parameter_value $register]
            } else {
                send_message ERROR "If you use the following registers, please ensure they share the same clock: $registers" 
                break
            }
        }
    }
...
proc validate {} {
    if { [get_parameter_value clear_type] eq "NONE" } {
        set_parameter_property single_clear enabled false
    } else {
        set_parameter_property single_clear enabled true
    }
    set operation [get_parameter_value OPERATION]
    set list_of_fixed_clock_modes [list sp_vector1 sp_vector2 sp_add sp_mult_add ]
    if { ![string equal [get_parameter_value adder_input_clock_2] [get_parameter_value adder_input_clock] ] &&   ([lsearch -nocase $list_of_fixed_clock_modes  $operation  ] != -1) } {
        send_message ERROR "Adder input registers must share the same clock"
    } 
    if { ![string equal [get_parameter_value adder_input_clock_2] [get_parameter_value accum_adder_clock] ] &&   ($operation == "sp_mult_acc") } {
        send_message ERROR "Adder and accumulator input registers must share the same clock"
    } 
     
    set register_bank_shares [list ax_chainin_pl_clock accum_pipeline_clock]
    check_matching_register_bank $register_bank_shares
    set register_bank_shares [list ax_chainin_2nd_pl_clock accum_2nd_pipeline_clock]
    check_matching_register_bank $register_bank_shares
    set register_bank_shares [list adder_input_clock accum_adder_clock]
    check_matching_register_bank $register_bank_shares

    if {[lsearch -nocase [get_used_registers]  "output_clock"  ] != -1 && [lsearch -nocase [get_used_registers]  "output_clock_2"  ] != -1 && [get_parameter_value "output_clock"] != [get_parameter_value "output_clock_2"] } {
        send_message error "Both output registers must use the same clock"
    }
    ...
    foreach cur_clock [get_hdl_clocks] {
        if { [string equal $cur_clock "adder_input_clock"] } {
            if { [lsearch -nocase [get_used_registers]  "adder_input_clock"  ] != -1 } {
                set_parameter_value "${cur_clock}_derived" [get_parameter_value adder_input_clock]
            } elseif { [lsearch -nocase [get_used_registers]  "adder_input_clock_2"  ] != -1 } {
                set_parameter_value "${cur_clock}_derived" [get_parameter_value adder_input_clock_2]
            }
        } elseif { [lsearch -nocase [get_used_registers]  $cur_clock  ] != -1 } {

...
set_parameter_property STREAM_CONFIG DISPLAY_HINT "rows:5 columns:80"
...
set_interface_assignment h2t debug.typeName configrom
...
proc compose {} {
   ...
   set_instance_parameter_value demux useHighBitsOfChannel false
   ...
   set_instance_parameter_value mux useHighBitsOfChannel false
   ...
   set_instance_parameter_value mgmt_demux useHighBitsOfChannel false
   add_connection clock.out_clk mgmt_demux.clk
   add_connection reset.out_reset mgmt_demux.reset

   set agents [expr {1 + $streams}]
   if {$agents < 2} {
   ...
   # $maxAgentChannelWidth is width of channel signal to support agentChannelSpan, i.e. 3 bits to support 4-7 channels
   ...
   set channelShift [log2ceil $agents]
   set maxAgentChannelWidth [log2ceil $agentChannelSpan]
   set channelBits [expr {$channelShift + $maxAgentChannelWidth}]
   ...
   # TODO: Now we have a transparent bridge of the correct shape and automatic adapter
   # insertion transforms we should not need the manual adapter insertion below.  Remove
   # it.
   ...
   add_instance bridge altera_avalon_st_debug_host_endpoint_bridge
   set_instance_parameter_value bridge DATA_WIDTH 8
   set_instance_parameter_value bridge CHANNEL_WIDTH $phyChannelWidth
   add_connection clock.out_clk bridge.clk
   add_connection reset.out_reset bridge.reset
   add_connection bridge.int_h2t $h2tInternal
   add_connection bridge.int_mgmt $mgmtInternal
   add_connection $t2hInternal bridge.int_t2h

   # export the muxed IO streams, i.e the inputs signals that go to the demuxes
   # and the output signals that come from the mux
   set_interface_property h2t      export_of bridge.ext_h2t
   set_interface_property mgmt     export_of bridge.ext_mgmt
   set_interface_property t2h      export_of bridge.ext_t2h
   ...
   set debug "<debug channel_shift='$channelShift'>"
   ...
   for { set i 0 } { $i < $streams } { incr i } {
      ...
      if {$async} {
         ...
         set aclk cbrdg_$agent_clk.out_clk
         set areset mgmt_rst_synch_$i.reset_out
      } else {
         set aclk clock.out_clk
         set areset mgmt_reset_$i.reset_source
      }
      #send_message {text info} "exported reset_out connection port $i"

      # Use a transparent bridge to make an exportable reset signal
      set rbridge reset_bridge_$i
      add_instance $rbridge altera_reset_bridge
      set_instance_parameter_value $rbridge NUM_RESET_OUTPUTS 1
      add_connection $aclk $rbridge.clk
      add_connection $areset $rbridge.in_reset

      add_interface $reset_intf reset start
      set_interface_property $reset_intf export_of $rbridge.out_reset

      set bridge bridge_$i
      add_instance $bridge altera_avalon_st_debug_agent_endpoint_bridge
      set_instance_parameter_value $bridge DATA_WIDTH    $conf(data_width)
      set_instance_parameter_value $bridge CHANNEL_WIDTH $conf(channel_width)
      set_instance_parameter_value $bridge HAS_MGMT      $conf(has_mgmt)
      set_instance_parameter_value $bridge READY_LATENCY $conf(ready_latency)
      add_connection $aclk   $bridge.clk
      add_connection $areset $bridge.reset
      ...
      set_interface_assignment $h2t debug.interfaceGroup [list associatedT2h $t2h]
      set_interface_assignment $h2t debug.channelOffset $id
      set_interface_assignment $h2t debug.channelShift $channelShift
      ...
      add_connection $inner $bridge.int_h2t
      ...
      add_connection $bridge.int_t2h $inner
      # ---------------------------------------------------------------------------------------------

      if {$conf(has_mgmt)} {
         set_interface_assignment $h2t debug.interfaceGroup [list associatedT2h $t2h associatedMgmt $mgmt]
         
         # TODO: Test this mode and check we exported the right interface
         add_connection mgmt_demux.out$id $bridge.int_mgmt
      ...
      set debug "$debug<agent base='$id' span='$channelSpan' type_id='$mfr_code:$type_code' />"

...
# $Id: //acds/rel/18.1std/ip/sld/core/altera_connection_identification_hub/altera_connection_identification_hub_hw.tcl#1 $
# $Revision: #1 $
# $Date: 2018/07/18 $
# $Author: psgswbuild $
...
proc elaborate {} {
	...
	for {set i 0} {$i < $count} {incr i} {
		...
		add_interface ident_$i conduit end
		add_interface_port ident_$i address_$i address Input $aw
		add_interface_port ident_$i contrib_$i writedata Input 4
		add_interface_port ident_$i rdata_$i readdata Output $dw

		if {$l > 0} {
			set_interface_property ident_$i associatedClock clock_$i
		}
	}
}

...
#    set num_syms_err         [expr ([get_parameter_value MON_ERR_WIDTH]     + 7) / 8]
    ...
    if {$width_dat} {
        add_interface_port tap_input iut_st_data  data    Input $width_dat
    } else {
        add_interface_port tap_input iut_st_data  data    Input 1
        set_port_property iut_st_data  TERMINATION TRUE
        set_port_property iut_st_data  TERMINATION_VALUE 0
    }

    if {$width_chn} {
        add_interface_port tap_input iut_st_ch    channel Input $width_chn
    } else {
        add_interface_port tap_input iut_st_ch    channel Input 1
        set_port_property iut_st_ch    TERMINATION TRUE
        set_port_property iut_st_ch    TERMINATION_VALUE 0
    }

    if {$width_mty} {
        add_interface_port tap_input iut_st_empty empty   Input $width_mty
    } else {
        add_interface_port tap_input iut_st_empty empty   Input 1
        set_port_property iut_st_empty TERMINATION TRUE
        set_port_property iut_st_empty TERMINATION_VALUE 0
    }

    if {$width_err} {
        add_interface_port tap_input iut_st_err   error   Input $width_err
    } else {
        add_interface_port tap_input iut_st_err   error   Input 1
        set_port_property iut_st_err   TERMINATION TRUE
        set_port_property iut_st_err   TERMINATION_VALUE 0
    }
}

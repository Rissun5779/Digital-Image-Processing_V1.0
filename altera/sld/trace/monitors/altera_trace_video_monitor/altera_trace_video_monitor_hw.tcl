...
add_instance    tap_splitter         altera_trace_video_tap_bridge
...
# compose_alt_trace_tap
# \param    monitor_name,       name of the altera_trace_video_tap_bridge instance
...
proc compose_alt_trace_tap {tap_name msg_clock msg_reset symbol_width data_width empty_width } {
    ...
    set_instance_parameter_value   $tap_name      MON_EMPTY_WIDTH    $empty_width
...
# \param    monitor_name,       name of the altera_trace_av_st_monitor instance
# \param    msg_clock,          the clock interface for the Avalon-ST connection to the distiller
# \param    msg_reset,          the reset interface (associated to msg_clock)
...
proc compose_alt_vip_monitor {monitor_name msg_clock msg_reset symbol_width data_width empty_width capture_sym_width} {
    set_instance_parameter_value   $monitor_name      MON_DATA_WIDTH          $data_width
    set_instance_parameter_value   $monitor_name      MON_EMPTY_WIDTH         $empty_width
     #[expr $data_width / $symbol_width]                                      
    ...
    set_instance_parameter_value   $monitor_name      TRACE_OUT_SYMBOL_WIDTH  [expr $capture_sym_width / 8]   
...
proc snoop_composition_callback {} {
    ...
    set  data_width          [expr $bps * $channels_par * $pixels_in_parallel]
    if { $pixels_in_parallel > 1 } {
        ...
        set empty_width      [clogb2_pure [expr $channels_par * $pixels_in_parallel]]
    } else {
        set empty_width 0
    }
    ...
    set_interface_property   tap             export_of            tap_splitter.din
	...
	set_interface_assignment capture         debug.monitoredInterfaces          tap_splitter
    ...
    set_interface_assignment capture		 debug.param.virtual_register(-6)	$pixels_in_parallel
    set_interface_assignment capture		 debug.param.virtual_register(-7)	$empty_width
    ...
    compose_alt_trace_tap    tap_splitter    $avst_clock  $avst_reset  $bps  $data_width  $empty_width
    compose_alt_vip_monitor  monitor         $avst_clock  $avst_reset  $bps  $data_width  $empty_width  $capture_width

    # Connect the monitor to the output of the snoop_tap
    add_connection    tap_splitter.dout0          monitor.tap_input
    ...
    if {$tb_support} {
       ...
       # }
        ...
        add_connection           tap_splitter.dout1                            pixel_monitor.tap_input

...
set_parameter_property ENABLE_UNIDIRECTIONAL DISPLAY_NAME "Enable unidirectional mode"
set_parameter_property ENABLE_UNIDIRECTIONAL DISPLAY_HINT BOOLEAN
set_parameter_property ENABLE_UNIDIRECTIONAL DESCRIPTION "Enable unidirectional mode on transmit path for 10G"
...
proc ethernet_10g_mac_update_gui {} {
    ...
    set preamble_ena [get_parameter_value PREAMBLE_PASSTHROUGH]
    ...
    if {$mac_1g10g_ena != 0} {
        ...
        if { $unidirectional_ena == 1 || $ptp_1step_ena == 1} {
    ...
    if {[_get_device_type_1588_sv] || ($mac_1g10g_ena == 0 && [_get_device_type_1588])} {
    ...
    #Grey out speed selection button when one of the following is selected
    if { $unidirectional_ena == 1 || $pfc_ena == 1 || $preamble_ena == 1 || $datapath_option != 3 || ($timestamp_ena == 1 && [_get_device_type_1588])} {
...
proc update_option {arg3} {
    set datapath_option [get_parameter_value $arg3]
    if {$datapath_option == 1} {
        set_parameter_value ENABLE_SUPP_ADDR 0
    }
    if {$datapath_option == 2} {
        set_parameter_value INSTANTIATE_TX_CRC 0
    }
}

    ...
    always @( posedge clk or posedge reset ) begin
        if(reset) begin
            ...
            if(decode_valid_out) begin
				...
				if (msg_valid & is_ptp_port & is_udp) begin
				...
				end else if (msg_valid & (!is_ptp_port | !is_udp) ) begin
				        tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg <= tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg;
						offset_location_valid <= 1'b1;
				end else begin
						if (ethertype_valid && data_sink_frame_end) begin
							tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg <= tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg;
							...
							tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg <= tx_etstamp_ins_ctrl_out_offset_checksum_correction_reg;
							offset_location_valid <= offset_location_valid;	
                        end							
				end

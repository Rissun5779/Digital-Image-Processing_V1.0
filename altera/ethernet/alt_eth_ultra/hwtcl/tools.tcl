...
proc ::ethernet::tools::is_pro { } {
    return [is_software_edition QUARTUS_PRIME_PRO]
}


proc ::ethernet::tools::create_temp_dir { {dir_name temp_qsys_dir} } {
    file mkdir $dir_name
    return ./${dir_name}
}

proc ::ethernet::tools::remove_temp_dir { {dir_name temp_qsys_dir} } {
    file delete -force $dir_name
}

# Recursively call add_fileset_file. Used to copy an
# entire directory. Src must be a relative path
proc ::ethernet::tools::copy_folder {src dst {option ""}} {
    ...
    foreach file $files {
        ...
        if {$is_dir} {
            if { [string compare $option recursive] == 0 } {
                ::ethernet::tools::copy_folder $file $dst_path $option
            }
        } else {
...
proc ::ethernet::tools::copy_folder_abs {src dst} {
...
proc ::ethernet::tools::generate_ip_core {ip_name ip_core_name family device path gen_rtl {parameter_list {}}} {
    set ip_inst_name ${ip_core_name}_inst; # IP instantiation name
    ...
    puts $fileId "set_instance_property ${ip_inst_name} autoexport 1"
    ...
    ::ethernet::tools::qsys_file_generate ${path} ${ip_core_name}

    if {$gen_rtl == true} {
        ::ethernet::tools::local_qsysgenerate ${path} ${ip_core_name} "./${ip_core_name}"
    }

    file delete -force $qsysfile_generation_script
}

proc ::ethernet::tools::qsys_file_generate { filepath qsysname} {
...
proc ::ethernet::tools::local_qsysgenerate { filepath qsysname subdir} {
    set ip_generation_script ${filepath}/${qsysname}_gen.tcl
    set fh [open ${ip_generation_script} w]
    set qdir $::env(QUARTUS_ROOTDIR)
...
proc ::ethernet::tools::generate_parameter_value_pairs {parameters} {
...
#set ip alt_e2550
#set top_level_name ex_50g
#set params {ADME_ENABLE=1}
#set temp_dir [::ethernet::tools::create_temp_dir]
#set device_family "Arria 10"
#set part_num 10AX115S3F45I2SG

#::ethernet::tools::generate_ip_core $ip $top_level_name $device_family $part_num $temp_dir $params

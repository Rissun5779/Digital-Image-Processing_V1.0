...
// $Id: //acds/rel/18.1std/ip/ethernet/alt_eth_ultra/40g/rtl/pma/e40_io_frame_40.v#1 $
// $Revision: #1 $
// $Date: 2018/07/18 $
// $Author: psgswbuild $
// ______________________________________________________________________________
...
if (TARGET_CHIP == 5 && ENA_KR4) begin // Arria 10 KR4
    ...
    wire [40*4-1:0] tx_launch_delay1 /* synthesis keep */;
    wire [40*4-1:0] tx_launch_delay2 /* synthesis keep */;
    wire [40*4-1:0] tx_launch_delay /* synthesis keep */;
    wire [3:0]   tx_10g_data_valid_delay1, tx_10g_data_valid_delay2 /* synthesis keep */;
    wire [3:0]   tx_10g_data_valid_delay /* synthesis keep */;

    assign tx_launch_delay1 = tx_launch;
    assign tx_launch_delay2 = tx_launch_delay1;
    assign tx_launch_delay = tx_launch_delay2;
    assign tx_10g_data_valid_delay1 = tx_10g_data_valid;
    assign tx_10g_data_valid_delay2 = tx_10g_data_valid_delay1;
    assign tx_10g_data_valid_delay = tx_10g_data_valid_delay2;
    ...
    for (i=0; i<4; i=i+1) begin : POST_MUX_PIPE
        always @(posedge tx_10g_coreclkin[i]) begin
            tx_launch_2[i*40 +: 40] <= tx_launch_delay[i*40 +: 40];
            tx_10g_data_valid_2[i] <= tx_10g_data_valid_delay[i];
        end
        if(SYNTH_FEC) begin
            //always @(posedge tx_10g_coreclkin[i]) begin
            always @(posedge din_clk) begin

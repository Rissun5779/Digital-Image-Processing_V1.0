...
module alt_aeu_40_csr_kra10lt #(
  parameter ES_DEVICE     = 1,  // select ES or PROD device, 1 is ES-DEVICE, 0 is device
  parameter MAINTAPWIDTH  = 5,  // Width of Main Tap.  If set >7, need RTL fix
...
`ifdef ALTERA_RESERVED_XCVR_FULL_KR_TIMERS
  localparam BER_TIME_DEFAULT = 30'h000_03_C00;     // HW mode, 1 K frames
  localparam TAP_OVRD_DEFAULT = 30'h00_00_00_00;    // HW mode, no tap override
  localparam LT_BASE_DEFAULT  = (ES_DEVICE) ? 32'h8140_5111 : 32'hE114_5111 ;     // Enable LT, step=1/1 eq=5 (32), freeze DFE, enable VOD
  localparam LT_CDR_TWEAKS    = 25'h00_0000;       // Enable Ipd/Rpd update
`else // not FULL_KR_TIMERS
  localparam LT_CDR_TWEAKS    = 25'h00_0000;       // Enable Ipd/Rpd update 
  localparam LT_BASE_DEFAULT  = 32'hE148_5111;     // Enable LT, step=1/1 eq=5 (32), manual VGA=7 CTLE=1, bypass DFE, no VOD
  localparam BER_TIME_DEFAULT = 30'h000_00_003;     // sim mode, 3 frames
  localparam TAP_OVRD_DEFAULT = 30'h24_46_39_3c;    // sim mode, Vmax = 28,
`endif // FULL_KR_TIMERS                            // Vmin=25, Vpost=6, Vpre=4
...
//**************************************************************************** 
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      ...
      if (lcl_stat_sync[0] & ~lcl_stat_dly[0]) lcl_stat_ltch[7*0 +: 6] <= lcl_txi_status[7*0 +: 6];
      if (lcl_stat_sync[1] & ~lcl_stat_dly[1]) lcl_stat_ltch[7*1 +: 6] <= lcl_txi_status[7*1 +: 6];
      if (lcl_stat_sync[2] & ~lcl_stat_dly[2]) lcl_stat_ltch[7*2 +: 6] <= lcl_txi_status[7*2 +: 6];
      if (lcl_stat_sync[3] & ~lcl_stat_dly[3]) lcl_stat_ltch[7*3 +: 6] <= lcl_txi_status[7*3 +: 6];
      if (pcs_mode_rc[1])                      lcl_stat_ltch[7*1-1]    <= lcl_txi_status[7*1-1];
      if (pcs_mode_rc[1])                      lcl_stat_ltch[7*2-1]    <= lcl_txi_status[7*2-1];
      if (pcs_mode_rc[1])                      lcl_stat_ltch[7*3-1]    <= lcl_txi_status[7*3-1];
      if (pcs_mode_rc[1])                      lcl_stat_ltch[7*4-1]    <= lcl_txi_status[7*4-1];
...
//****************************************************************************
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      reg_krlt_base   <= LT_BASE_DEFAULT; 
      reg_krlt_reset  <= LT_CDR_TWEAKS; 
      ...
      ADDR_KRLT_TAPVAL: begin
        ...
        readdata[27:24] <= last_ctle_rc[0*4 +: 4];
        readdata[29:28] <= last_ctle_mode[0*2 +: 2];
        readdata[31:30] <= last_dfe_mode[0*2 +:2];
      end  // Tap values
      ADDR_KRLT_TAPVAL1: begin
        ...
        readdata[27:24] <= last_ctle_rc[1*4 +: 4];
        readdata[29:28] <= last_ctle_mode[1*2 +: 2];
        readdata[31:30] <= last_dfe_mode[1*2 +:2];
      end  // Tap values
      ADDR_KRLT_TAPVAL2: begin
        ...
        readdata[27:24] <= last_ctle_rc[2*4 +: 4];
        readdata[29:28] <= last_ctle_mode[2*2 +: 2];
        readdata[31:30] <= last_dfe_mode[2*2 +:2];
      end  // Tap values
      ADDR_KRLT_TAPVAL3: begin
        ...
        readdata[27:24] <= last_ctle_rc[3*4 +: 4];
        readdata[29:28] <= last_ctle_mode[3*2 +: 2];
        readdata[31:30] <= last_dfe_mode[3*2 +:2];
      end  // Tap values
...
//****************************************************************************
// Drive the outputs
// Note that some of the following are redifined for different purpose
// but wire/port names are kept same to minimize RTL changes
//****************************************************************************
  ...
  assign lt_d1           = reg_krlt_reset[23:12];
  ...
  // output bus is compacted where register bits are byte oriented
  // order is {enpre,pretap,enpost,posttap,envodmin,vodmin,envodmax,vodmax}
...
endmodule // alt_aeu_40_csr_kra10lt

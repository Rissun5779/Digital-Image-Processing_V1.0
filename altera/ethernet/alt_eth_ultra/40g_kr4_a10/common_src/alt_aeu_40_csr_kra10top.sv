...
module alt_aeu_40_csr_kra10top
  #(
...
`ifdef ALTERA_RESERVED_XCVR_FULL_KR_TIMERS
  localparam KR_BASE_DEFAULT  = 16'h0102;   // Disable AN timer, enable rcfg_cal
`else // not FULL_KR_TIMERS
  localparam KR_BASE_DEFAULT  = 16'h1102;   // Disable AN timer, enable rcfg_cal, LT timeout to data mode
...
//****************************************************************************
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      reg_top_base[15:0]  <= KR_BASE_DEFAULT;
...
//****************************************************************************
  ...
  assign force_mode       = reg_top_base[7:4];
  assign en_rcfg_cal      = reg_top_base[8];
  assign lt_fail_response = reg_top_base[12];
  assign fec_enable       = reg_top_base[16];
  assign fec_request      = reg_top_base[18];
  assign fec_err_ind      = reg_top_base[17];


endmodule // alt_aeu_40_csr_kra10top

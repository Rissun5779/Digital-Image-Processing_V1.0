...
module alt_aeu_40_csr_krfec
  ...
  always @(posedge clk or posedge reset) begin
    if (reset == 1'b1) begin
      ...
      (ADDR_KRFEC_STATUS + REG_OFFS): begin //0xB2
        readdata[12:0] <= reg_add_b2;
        if (write) begin
          reg_add_b2      <= writedata[12:0];
        end  // if write
      end  // base
      ...
      endcase
       ...
       reg_add_b2_11   <= reg_add_b2[11];
    if(reg_add_b2_11) reg_add_b2[11] <= 1'b0; // clear after write
 ...
 /// drive outputs
  ...
  assign clear_counters         = reg_add_b2[12];

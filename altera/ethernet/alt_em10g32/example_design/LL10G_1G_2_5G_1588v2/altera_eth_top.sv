...
module altera_eth_top # (
    ...
    wire                                sampling_pll_locked;
    
    wire [NUM_OF_CHANNEL/2-1:0]        error_n;
    ...
    wire                   [31:0]       csr_master_tod_readdata;
     ...
     // Timestamp request
    wire    [NUM_OF_CHANNEL-1:0]        tx_egress_timestamp_request_valid;
    wire    [NUM_OF_CHANNEL-1:0][3:0]   tx_egress_timestamp_request_fingerprint;
    
    // TX Ingress Timestamp correction - temporary not use
    wire    [NUM_OF_CHANNEL-1:0]        tx_ingress_timestamp_valid      = 'h0;
    wire    [NUM_OF_CHANNEL-1:0][95:0]  tx_ingress_timestamp_96b_data   = 'h0;
    wire    [NUM_OF_CHANNEL-1:0][63:0]  tx_ingress_timestamp_64b_data   = 'h0;
    wire    [NUM_OF_CHANNEL-1:0]        tx_ingress_timestamp_format     = 'h0;
    

    // output RX timestamp 
    wire    [NUM_OF_CHANNEL-1:0]        rx_ingress_timestamp_96b_valid;
    wire    [NUM_OF_CHANNEL-1:0][95:0]  rx_ingress_timestamp_96b_data;
    wire    [NUM_OF_CHANNEL-1:0]        rx_ingress_timestamp_64b_valid;
    wire    [NUM_OF_CHANNEL-1:0][63:0]  rx_ingress_timestamp_64b_data;
    ...
    wire    [NUM_OF_CHANNEL-1:0][3:0]   tx_egress_timestamp_96b_fingerprint;
    wire    [NUM_OF_CHANNEL-1:0]        tx_egress_timestamp_64b_valid;
    wire    [NUM_OF_CHANNEL-1:0][63:0]  tx_egress_timestamp_64b_data;
    wire    [NUM_OF_CHANNEL-1:0][3:0]   tx_egress_timestamp_64b_fingerprint;
    ...
    // Channel Ready
    assign channel_ready_n = ~(channel_tx_ready & channel_rx_ready & {NUM_OF_CHANNEL{core_pll_locked}} & {NUM_OF_CHANNEL{sampling_pll_locked}});
    ...
    alt_mge_rd #(
        ...
        // 1588
        .tx_egress_timestamp_request_in_valid           (tx_egress_timestamp_request_valid),
        ...
        .sampling_pll_locked                            (sampling_pll_locked),
        
        // Configuration
        .clock_operation_mode_mode                      ({NUM_OF_CHANNEL{ORDINARY_CLOCK}}),
    ...
    generate for (i =0; i <(NUM_OF_CHANNEL/2); i = i + 1)
        begin: ETH_TRAFFIC_CTRL_PAIR_CHANNEL
            eth_traffic_controller_32b_top eth_traffic_controller (
                ...
                .tx_egress_timestamp_96b_valid            (tx_egress_timestamp_96b_valid           [2*(i+1)-1: 2*i]), 
                .tx_egress_timestamp_96b_data             (tx_egress_timestamp_96b_data            [2*(i+1)-1: 2*i]), 
                .tx_egress_timestamp_96b_fingerprint      (tx_egress_timestamp_96b_fingerprint     [2*(i+1)-1: 2*i]), 

                .tx_egress_timestamp_request_fingerprint  (tx_egress_timestamp_request_fingerprint [2*(i+1)-1: 2*i]), 
                .tx_egress_timestamp_request_valid        (tx_egress_timestamp_request_valid       [2*(i+1)-1: 2*i]),

                .rx_ingress_timestamp_96b_valid           (rx_ingress_timestamp_96b_valid          [2*(i+1)-1: 2*i]), 
                .rx_ingress_timestamp_96b_data            (rx_ingress_timestamp_96b_data           [2*(i+1)-1: 2*i]),
            
                .eth_1588_wait_limit                      (1'b1),
                .eth_1588_start_tod_sync                  (start_tod_sync                          [2*(i+1)-1: 2*i]),
                .eth_1588_channel_ready                   (~channel_ready_n                        [2*(i+1)-1: 2*i]),
                .eth_1588_traffic_controller_error_n      (error_n[i])
            );    
        end
    endgenerate 

...
module alt_mge_rd #(
    ...
    // TOD Master CSR
    output                                  csr_master_tod_waitrequest,
    input                      [31:0]       csr_master_tod_writedata,
    input                      [3:0]        csr_master_tod_address,
    input                                   csr_master_tod_write,
    input                                   csr_master_tod_read,
    output                     [31:0]       csr_master_tod_readdata,

    // 1588
    output                                  master_pulse_per_second,
    input  [NUM_OF_CHANNEL-1:0]             tx_egress_timestamp_request_in_valid,
    input  [NUM_OF_CHANNEL-1:0][3:0]        tx_egress_timestamp_request_in_fingerprint,
    input  [NUM_OF_CHANNEL-1:0]             tx_ingress_timestamp_valid,
    input  [NUM_OF_CHANNEL-1:0][95:0]       tx_ingress_timestamp_96b_data,
    input  [NUM_OF_CHANNEL-1:0][63:0]       tx_ingress_timestamp_64b_data,
    input  [NUM_OF_CHANNEL-1:0]             tx_ingress_timestamp_format,
    input  [NUM_OF_CHANNEL-1:0][1:0]        clock_operation_mode_mode,
    input  [NUM_OF_CHANNEL-1:0]             pkt_with_crc_mode,
    output [NUM_OF_CHANNEL-1:0]             tx_egress_timestamp_96b_valid,
    output [NUM_OF_CHANNEL-1:0][95:0]       tx_egress_timestamp_96b_data,
    output [NUM_OF_CHANNEL-1:0][3:0]        tx_egress_timestamp_96b_fingerprint,
    output [NUM_OF_CHANNEL-1:0]             tx_egress_timestamp_64b_valid,
    output [NUM_OF_CHANNEL-1:0][63:0]       tx_egress_timestamp_64b_data,
    output [NUM_OF_CHANNEL-1:0][3:0]        tx_egress_timestamp_64b_fingerprint,
    output [NUM_OF_CHANNEL-1:0]             rx_ingress_timestamp_96b_valid,
    output [NUM_OF_CHANNEL-1:0][95:0]       rx_ingress_timestamp_96b_data,
    output [NUM_OF_CHANNEL-1:0]             rx_ingress_timestamp_64b_valid,
    output [NUM_OF_CHANNEL-1:0][63:0]       rx_ingress_timestamp_64b_data,
    input  [NUM_OF_CHANNEL-1:0]             start_tod_sync,
    output [NUM_OF_CHANNEL-1:0]             pulse_per_second,
    output                                  sampling_pll_locked
    
);
    
    // Local parameter
    localparam NUM_OF_CH_WIDTH = log2ceil(NUM_OF_CHANNEL);
    localparam PPS_PULSE_ASSERT_CYCLE_MASTER   = 1250000;                          //Pulse Per Second for 1G - Assert for 10ms (10ms / 8.0ns = 1250000)    
    ...
    assign  master_tod_clk = refclk;
    assign  calc_clk = tod_sync_1g_sampling_clk;
    ...
    // IOPLL for 1588 TOD Sync sampling
    alt_mge_1588_sampling_pll u_1588_sampling_iopll (
        .refclk          (refclk),
        .rst             (reset),    
        .outclk_0        (tod_sync_1g_sampling_clk),   // 80 MHz
        .outclk_1        (tod_sync_2p5g_sampling_clk), // 53.333333 MHz
        .locked          (sampling_pll_locked)
    );
    ...
    begin : CHANNEL_GEN
        ...
        alt_mge_channel u_channel (
            ...
            .latency_measure_clk        (calc_clk),                                            //derive from pll 2                              
            
            // Sampling Clock
            .tod_sync_2p5g_sampling_clk (tod_sync_2p5g_sampling_clk),                                            //derive from pll 2                              
            ...
            // 1588
            .tx_egress_timestamp_request_in_valid                       (tx_egress_timestamp_request_in_valid[i]),                         //export
            .tx_egress_timestamp_request_in_fingerprint                 (tx_egress_timestamp_request_in_fingerprint[i]),                   //export
            .tx_etstamp_ins_ctrl_in_residence_time_update               (tx_ingress_timestamp_valid[i]),                                   //export
            .tx_etstamp_ins_ctrl_in_ingress_timestamp_96b               (tx_ingress_timestamp_96b_data[i]),                                //export
            .tx_etstamp_ins_ctrl_in_ingress_timestamp_64b               (tx_ingress_timestamp_64b_data[i]),    
            .tx_etstamp_ins_ctrl_in_residence_time_calc_format          (tx_ingress_timestamp_format[i]),                                  //export
            .clock_operation_mode_mode                                  (clock_operation_mode_mode[i]),                                    //export    
            .pkt_with_crc_mode                                          (pkt_with_crc_mode[i]),                                            //export
            .tx_egress_timestamp_96b_valid                              (tx_egress_timestamp_96b_valid[i]),                                //export
            .tx_egress_timestamp_96b_data                               (tx_egress_timestamp_96b_data[i]),                                 //export
            .tx_egress_timestamp_96b_fingerprint                        (tx_egress_timestamp_96b_fingerprint[i]),                          //export
            .tx_egress_timestamp_64b_valid                              (tx_egress_timestamp_64b_valid[i]),                                //export
            .tx_egress_timestamp_64b_data                               (tx_egress_timestamp_64b_data[i]),                                 //export
            .tx_egress_timestamp_64b_fingerprint                        (tx_egress_timestamp_64b_fingerprint[i]),                          //export
            .rx_ingress_timestamp_96b_valid                             (rx_ingress_timestamp_96b_valid[i]),                               //export    
            .rx_ingress_timestamp_96b_data                              (rx_ingress_timestamp_96b_data[i]),                                //export    
            .rx_ingress_timestamp_64b_valid                             (rx_ingress_timestamp_64b_valid[i]),                               //export
            .rx_ingress_timestamp_64b_data                              (rx_ingress_timestamp_64b_data[i]),                                //export
            ...
            .start_tod_sync                                             (start_tod_sync[i]),                                               //export

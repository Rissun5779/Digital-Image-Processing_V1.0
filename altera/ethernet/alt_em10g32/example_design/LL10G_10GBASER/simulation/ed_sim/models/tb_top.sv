...
module tb_top;
	...
	*/
    ...
    wire 	[NUM_CHANNELS-1:0] atx_pll_locked;
...
//            U_AVALON_DRIVER.avalon_mm_csr_wr(32'h10000*id + TX_CRC_INSERT_CONTROL_ADDR, {1'b0, 1'b1});
            ...
            // configure rx fifo
            // drop on error
            U_AVALON_DRIVER.avalon_mm_csr_wr(32'h10000*id + 32'h9414, 32'h01);
            U_AVALON_DRIVER.avalon_mm_csr_rd(32'h10000*id + 32'h9414, readdata);
            $display("rx drop on error   = %0d", readdata);
            
            // configure almost full threshold
            U_AVALON_DRIVER.avalon_mm_csr_rd(32'h10000*id + 32'h9408, readdata);
            $display("rx fifo almost full   = %0d", readdata);
            U_AVALON_DRIVER.avalon_mm_csr_wr(32'h10000*id + 32'h9408, 32'h0370);
            U_AVALON_DRIVER.avalon_mm_csr_rd(32'h10000*id + 32'h9408, readdata);
            $display("rx fifo almost full  = %0d", readdata);
            
            // configure almost empty threshold
            U_AVALON_DRIVER.avalon_mm_csr_rd(32'h10000*id + 32'h940C, readdata);
            $display("rx fifo almost empty   = %0d", readdata);
            U_AVALON_DRIVER.avalon_mm_csr_wr(32'h10000*id + 32'h940C, 32'h03);
            U_AVALON_DRIVER.avalon_mm_csr_rd(32'h10000*id + 32'h940C, readdata);
            $display("rx fifo almost empty  = %0d", readdata);

...
module tb_testcase;
    ...
    static reg [1:0] speed_array[3] = {ETH_SPEED_10G, ETH_SPEED_2P5G, ETH_SPEED_1G};
    
    // Test procedures
    initial begin
        
        while(tb_top.reset !== 1'b0) begin
            @(posedge tb_top.refclk_1g2p5g);
        end
        ...
        foreach(speed_array[speed]) begin
            ...
            repeat(50) @(posedge tb_top.refclk_1g2p5g);
            ...
            // Wait until all packets are loopback completely
            repeat(4000) @(posedge tb_top.refclk_1g2p5g);
        ...
        $display("------------------------");
        tb_top.U_AVALON_DRIVER.display_eth_statistics(RD_CHANNEL_0_BASE_ADDR + RD_CHANNEL_MAC_OFFSET + TX_STATISTICS_ADDR);
        
        $display("------------------------");
        $display("Channel 0: RX Statistics");
        $display("------------------------");
        tb_top.U_AVALON_DRIVER.display_eth_statistics(RD_CHANNEL_0_BASE_ADDR + RD_CHANNEL_MAC_OFFSET + RX_STATISTICS_ADDR);

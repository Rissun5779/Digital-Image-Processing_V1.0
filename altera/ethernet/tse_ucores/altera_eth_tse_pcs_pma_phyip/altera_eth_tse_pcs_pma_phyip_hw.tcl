...
# |
set_module_property NAME altera_eth_tse_pcs_pma_phyip
...
array set pcs_phyip_connection_ports {
   ...
   "rx_frame"              {Input   8}
   "rx_kchar"              {Input   1}
   "rx_runlengthviolation" {Input   1}
   "rx_patterndetect"      {Input   1}
   "rx_runningdisp"        {Input   1}
   "rx_syncstatus"         {Input   1}
   "rx_char_err_gx"        {Input   1}
   "rx_disp_err"           {Input   1}
   "sd_loopback"           {Output  1}
}
...
proc fileset_cb {entityname} {
   ...
   set top_level_file altera_eth_tse_pcs_pma_phyip.v
...
proc sdc_file_gen {} {
   set sdc_template "altera_eth_tse_pcs_pma_phyip.sdc"
   set sdc_out_file [create_temp_file altera_eth_tse_pcs_pma_phyip.sdc]
   ...
   close $in
   close $out
   add_fileset_file altera_eth_tse_pcs_pma_phyip.sdc SDC PATH $sdc_out_file {SYNTHESIS}
}
...
proc elaborate {} {
   ...
   set_fileset_property synthesis_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_phyip"
   set_fileset_property sim_ver_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_phyip"
   set_fileset_property sim_vhd_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_phyip"
...
proc build_interfaces {} {
   ...
   foreach {port} [array names pcs_phyip_connection_ports] {
      set direction [lindex $pcs_phyip_connection_ports($port) 0]
      set width [lindex $pcs_phyip_connection_ports($port) 1]
   ...
   if {$ENABLE_SGMII} {
      ...
      add_interface_port UNUSED_RMFIFO_CP rx_rmfifodatadeleted "export" Input 1
      add_interface_port UNUSED_RMFIFO_CP rx_rmfifodatainserted "export" Input 1
      set_interface_property UNUSED_RMFIFO_CP ENABLED 0
   } else {
      add_interface rx_rmfifodatadeleted conduit end
      add_interface_port rx_rmfifodatadeleted rx_rmfifodatadeleted "export" Input 1

      add_interface rx_rmfifodatainserted conduit end
      add_interface_port rx_rmfifodatainserted rx_rmfifodatainserted "export" Input 1
   }
...
proc build_phy_timestamp_interface {args} {
   ...
   add_interface_port $WA_BOUNDARY wa_boundary      export      Input    5 

...
# +-----------------------------------
# | module altera_eth_tse_pcs
# |
set_module_property NAME altera_eth_tse_pcs_pma_nf_phyip
...
# PHYIP connection ports
array set pcs_phyip_connection_ports {
   "rx_pcs_clk"            {"rx_pcs_clk"              Input   1}
   "tx_pcs_clk"            {"tx_pcs_clk"              Input   1}
   "tx_frame"              {"tx_parallel_data"        Output  8}
   "tx_kchar"              {"tx_datak"                Output  1}
   "rx_frame"              {"rx_parallel_data"        Input   8}
   "rx_kchar"              {"rx_datak"                Input   1}
   "rx_runlengthviolation" {"rx_runlengthviolation"   Input   1}
   "rx_patterndetect"      {"rx_patterndetect"        Input   1}
   "rx_runningdisp"        {"rx_runningdisp"          Input   1}
   "rx_syncstatus"         {"rx_syncstatus"           Input   1}
   "rx_char_err_gx"        {"rx_errdetect"            Input   1}
   "rx_disp_err"           {"rx_disperr"              Input   1}
   "sd_loopback"           {"sd_loopback"             Output  1}
}
...
proc fileset_cb {entityname} {
   ...
   global gxb_rtl_files
   global common_rtl_files
   global pcs_ocp_files

   set top_level_file altera_eth_tse_pcs_pma_nf_phyip.v
   ...
   foreach {file_name filetype} $gxb_rtl_files {
      add_fileset_file $file_name $filetype PATH ../../altera_eth_tse/$file_name {SYNTHESIS}
   }
...
proc sdc_file_gen {} {
   set sdc_template "altera_eth_tse_pcs_pma_nf_phyip.sdc"
   set sdc_out_file [create_temp_file altera_eth_tse_pcs_pma_nf_phyip.sdc]
   ...
   close $in
   close $out
   add_fileset_file altera_eth_tse_pcs_pma_nf_phyip.sdc SDC PATH $sdc_out_file {SYNTHESIS}
}
...
proc elaborate {} {
   ...
   set_fileset_property synthesis_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_nf_phyip"
   set_fileset_property sim_ver_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_nf_phyip"
   set_fileset_property sim_vhd_fileset TOP_LEVEL "altera_eth_tse_pcs_pma_nf_phyip"
...
proc build_interfaces {} {
   ...
   global pcs_phyip_connection_ports
   ...
   # Build interface to PHYIP
   foreach {port} [array names pcs_phyip_connection_ports] {
      set role [lindex $pcs_phyip_connection_ports($port) 0]
      set direction [lindex $pcs_phyip_connection_ports($port) 1]
      set width [lindex $pcs_phyip_connection_ports($port) 2]

      add_interface $port conduit end
      add_interface_port $port $port $role $direction $width
   }
   if {$ENABLE_SGMII} {
      add_interface UNUSED_RMFIFO_CP conduit end
      add_interface_port UNUSED_RMFIFO_CP rx_rmfifodatadeleted    rx_rmfifodatadeleted    Input 1
      add_interface_port UNUSED_RMFIFO_CP rx_rmfifodatainserted   rx_rmfifodatainserted   Input 1
      set_interface_property UNUSED_RMFIFO_CP ENABLED 0
   } else {
      add_interface rx_rmfifodatadeleted conduit end
      add_interface_port rx_rmfifodatadeleted rx_rmfifodatadeleted   rx_rmfifodatadeleted Input 1

      add_interface rx_rmfifodatainserted conduit end
      add_interface_port rx_rmfifodatainserted rx_rmfifodatainserted rx_rmfifodatainserted Input 1
   }
   ...
   build_phy_timestamp_interface $PCS_PHASE_MEASURE_CLK $RX_LATENCY_ADJ $TX_LATENCY_ADJ $WA_BOUNDARY $TX_PTP_ALIGNMENT
...
proc build_phy_timestamp_interface {args} {
   ...
   set WA_BOUNDARY [lindex $args 3]
   set TX_PTP_ALIGNMENT [lindex $args 4]
   ...
   # word aligner boundary
   add_interface $WA_BOUNDARY conduit end
   add_interface_port $WA_BOUNDARY wa_boundary rx_std_bitslipboundarysel Input    5 

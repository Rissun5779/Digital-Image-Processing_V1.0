...
# |
set_module_property NAME altera_eth_tse_pcs
...
proc fileset_cb {entityname} {
   ...
   set top_level_file altera_eth_tse_pcs.v
...
proc sdc_file_gen {} {
   set sdc_template "altera_eth_tse_pcs.sdc"
   set sdc_out_file [create_temp_file altera_eth_tse_pcs.sdc]
   ...
   close $in
   close $out
   add_fileset_file altera_eth_tse_pcs.sdc SDC PATH $sdc_out_file {SYNTHESIS}
}
...
proc elaborate {} {
   ...
   set_fileset_property synthesis_fileset TOP_LEVEL "altera_eth_tse_pcs"
   set_fileset_property sim_ver_fileset TOP_LEVEL "altera_eth_tse_pcs"
   set_fileset_property sim_vhd_fileset TOP_LEVEL "altera_eth_tse_pcs"
...
proc build_interfaces {} {
   ...
   set_interface_property $PCS_REF_CLK_CP_NAME ENABLED $ENABLE_CLK_SHARING
   ...
   # Serdes control
   build_pcs_serdes_interface $PCS_SERDES_CONTROL_CP_NAME

   # Ten Bit interface
   build_pcs_tbi_interface $PCS_TBI_CP_NAME
...
proc build_avalon_reset_interface {args} {
   ...
   add_interface_port $AVALON_RESET_CP_NAME reset_reg_clk reset Input 1
}
...
proc build_pcs_misc_interfaces {args} {
   ...
   set PCS_STATUS_LED_CP_NAME [lindex $args 4]
   set PCS_SERDES_CONTROL_CP_NAME [lindex $args 5]
...
proc build_pcs_serdes_interface {args} {
   set PCS_SERDES_CONTROL_CP_NAME [lindex $args 0]

   # Serdes control
   add_interface $PCS_SERDES_CONTROL_CP_NAME conduit end
   add_interface_port $PCS_SERDES_CONTROL_CP_NAME sd_loopback  export   Output   1
   add_interface_port $PCS_SERDES_CONTROL_CP_NAME powerdown    export   Output   1

}

proc build_pcs_tbi_interface {args} {
   set PCS_TBI_CP_NAME [lindex $args 0]

   add_interface $PCS_TBI_CP_NAME conduit end
   add_interface_port $PCS_TBI_CP_NAME tbi_rx_clk  export   Input    1
   add_interface_port $PCS_TBI_CP_NAME tbi_tx_clk  export   Input    1
   add_interface_port $PCS_TBI_CP_NAME tbi_rx_d    export   Input    10
   add_interface_port $PCS_TBI_CP_NAME tbi_tx_d    export   Output   10
}

...
wire [79:0] tx_core_out;       // 4*20
wire        tx_data_valid;
wire        tx_data_valid_r;
wire [79:0] tx_fifo_out;       // 4*20
wire [79:0] tx_oversample_out; // 4*20
wire        tx_fifo_rdreq;
...
generate
    for (i=0; i<4; i=i+1) begin: TX_DCFIFO   
        mr_clock_sync #(
            ...
            .wrreq (1'b1),
            .data (tx_core_out[i*20+19:i*20]),
            .rdreq (tx_fifo_rdreq),
            .q (tx_fifo_out[i*20+19:i*20])		      
        );
    end
endgenerate
...
generate
    for (i=0; i<4; i=i+1) begin: TX_OVERSAMPLE   
        mr_tx_oversample #(
            ...
            .din (tx_fifo_out[i*20+19:i*20]),
            .din_valid (tx_data_valid_r),
            .dout (tx_oversample_out[i*20+19:i*20])		       
        );
    end
endgenerate

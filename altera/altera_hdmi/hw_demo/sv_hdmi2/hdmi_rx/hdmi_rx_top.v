...
//
// Generate the HPD for source
//
//reg hdmi_rx_hpd_r1;
//reg [23:0] rx_hdmi_count;

//always @(posedge reconfig_mgmt_clk or posedge sys_reset)
//begin
//  if(sys_reset) begin
//    rx_hdmi_count <= 24'd0;
//  end else begin
//    if(hdmi_rx_hpd_r1 == 1'd0) begin
//      rx_hdmi_count <= rx_hdmi_count + 24'd1;
//    end
//  end 
//end
...
//    hdmi_rx_hpd_r1 <= 1'd0;
//  end else begin
//    if(rx_hdmi_count < 34'd1000000) begin //upon reset. hold HPD signal for 100ms
//      hdmi_rx_hpd_r1 <= 1'b0;
//    end else begin
//      hdmi_rx_hpd_r1 <= 1'b1;
//    end
...
//assign hdmi_rx_hpd = hdmi_rx_hpd_r1;
...
mr_reconfig_mgmt_sv #(
   ...
   .RX_DEFAULT_RANGE (6),
   .PLL_DEFAULT_RANGE (4),
   .CYC_MEASURE_CLK_IN_10_MSEC (24'hF4240)
) u_reconfig_mgmt_sv (
   /* I */ .sys_clk (mgmt_clk),

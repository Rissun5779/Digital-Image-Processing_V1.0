...
module mr_rx_oversample #(   
   parameter [6:0]                 DIN_WIDTH       = 7'd40,   // Width of din
   parameter [5:0]                 DOUT_WIDTH      = 6'd40,   // Width of dout
...
begin
   if (rst) begin
      ...
      if (total_accumulated_samples >= DOUT_WIDTH) begin
         total_accumulated_samples <= (total_accumulated_samples - DOUT_WIDTH) + samples_per_word[ADDER_WIDTH-1:0];
      end else begin
         total_accumulated_samples <= total_accumulated_samples + samples_per_word[ADDER_WIDTH-1:0];
      end
   end
end
...
begin
   if (rst) begin
      ...
      case (samples_per_word)
         ...
         6      : begin dout_sr <= {samples_data[0+:6],  dout_sr[DOUT_WIDTH+MAX_SAMPLES_PER_WORD-1:6]}; end
         7      : begin dout_sr <= {samples_data[0+:7],  dout_sr[DOUT_WIDTH+MAX_SAMPLES_PER_WORD-1:7]}; end
         8      : begin dout_sr <= {samples_data[0+:8],  dout_sr[DOUT_WIDTH+MAX_SAMPLES_PER_WORD-1:8]}; end
         9      : begin dout_sr <= {samples_data[0+:9],  dout_sr[DOUT_WIDTH+MAX_SAMPLES_PER_WORD-1:9]}; end
         default: begin dout_sr <= dout_sr;                                                             end	   
      endcase
   end
end
...
begin
   if (rst) begin
      ...
      case (total_accumulated_samples)
         ...
         DOUT_WIDTH+5 : begin dout <= dout_sr[(MAX_SAMPLES_PER_WORD-5) +:DOUT_WIDTH]; end 
         DOUT_WIDTH+6 : begin dout <= dout_sr[(MAX_SAMPLES_PER_WORD-6) +:DOUT_WIDTH]; end 
         DOUT_WIDTH+7 : begin dout <= dout_sr[(MAX_SAMPLES_PER_WORD-7) +:DOUT_WIDTH]; end 
         DOUT_WIDTH+8 : begin dout <= dout_sr[(MAX_SAMPLES_PER_WORD-8) +:DOUT_WIDTH]; end
         default      : begin dout <= dout;                                           end
      endcase
   end
end

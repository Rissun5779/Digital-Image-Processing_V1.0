...
## Department     : 
## Date           : $Date: 2016-06-21 10:01:07 +0200 (Tue, 21 Jun 2016) $
## Revision       : $Revision: 981 $
## URL            : $URL: svn://10.9.0.1/hdmi/trunk/bitec_hdmi.sdc $
## *********************************************************************
## Description
##
## Timing constraints
...
foreach each_inst $inst_list {
      ...
      regexp ".*${top_level}.*dcfifo.*component" "$each_inst" inst_path
...
apply_sdc_pre_dcfifo dcfifo bitec_hdmi_rx
apply_sdc_pre_dcfifo dcfifo bitec_hdmi_tx

proc apply_set_mcp {path_name} {
  set path_list [get_keepers -nowarn  $path_name]
  if {[get_collection_size $path_list] > 0} {
     foreach_in_collection path  $path_list {
        set_multicycle_path -end -setup -to $path 10
        set_multicycle_path -end -hold -to $path 9
     }
   }
}

apply_set_mcp *auxiliary_encoder.audio.audio|phase_wrap
apply_set_mcp *auxiliary_encoder.audio.audio|cntr_1m[*]
apply_set_mcp *auxiliary_encoder.audio.audio|CTS_valid

...
#======================================
#
# module altera_hmcc_mtaps
#

send_message PROGRESS "Reading TCL"

set_module_property NAME "altera_hmcc_mtaps"
...
set_module_property UPGRADEABLE_FROM { altera_hmcc 15.1 {Arria 10} altera_hmcc 15.0 {Arria 10}  } 
...
add_display_item "IP" "Dynamic Reconfiguration" GROUP
...
set_parameter_property DEVICE_FAMILY ALLOWED_RANGES {"Arria 10" "Stratix 10"}
set_parameter_property DEVICE_FAMILY DESCRIPTION "Supports Arria 10 and Stratix 10"
...
set_parameter_property CHANNELS DESCRIPTION "Selects full width (16 channels) fro Stratix 10 and Arria 10\
or half width (8 channels) for Arria 10 only."
...
set_parameter_property DATA_RATE ALLOWED_RANGES {"10000 Mbps" "12500 Mbps" "15000 Mbps"}
...
set_parameter_property CDR_REFCLK ALLOWED_RANGES {"125" "156.25" "166.67"}
set_parameter_property CDR_REFCLK UNITS Megahertz
add_display_item "General" CDR_REFCLK parameter

set_parameter_update_callback CDR_REFCLK choose_CDR_REFCLK_callback

add_parameter PORTS INTEGER 1
set_parameter_property PORTS DEFAULT_VALUE 1
set_parameter_property PORTS DISPLAY_NAME "Ports"
set_parameter_property PORTS DESCRIPTION "Selects the number of data path interfaces."
set_parameter_property PORTS ALLOWED_RANGES {1 2 3 4}
set_parameter_property PORTS HDL_PARAMETER true
add_display_item "General" PORTS parameter

add_parameter RESPONSE_REORDERING_GUI BOOLEAN
set_parameter_property RESPONSE_REORDERING_GUI DEFAULT_VALUE 0
set_parameter_property RESPONSE_REORDERING_GUI DISPLAY_NAME "Enable Response Re-ordering"
set_parameter_property RESPONSE_REORDERING_GUI DESCRIPTION "Enables automatic response re-ordering (and tag management)."
add_display_item "General" RESPONSE_REORDERING_GUI parameter

add_parameter RESPONSE_REORDERING INTEGER 0
set_parameter_property RESPONSE_REORDERING DEFAULT_VALUE 0
set_parameter_property RESPONSE_REORDERING ALLOWED_RANGES {0 1}
set_parameter_property RESPONSE_REORDERING VISIBLE false
set_parameter_property RESPONSE_REORDERING HDL_PARAMETER true
set_parameter_property RESPONSE_REORDERING DERIVED true
...
set_parameter_property ENABLE_ADME DESCRIPTION "Please refer to the Transceiver PHY User Guide for more information."
...
# add_parameter IRTRY_INSERT_COUNT INTEGER 32
# set_parameter_property IRTRY_INSERT_COUNT DEFAULT_VALUE 32
# set_parameter_property IRTRY_INSERT_COUNT DISPLAY_NAME "Retry Insert Count"
# set_parameter_property IRTRY_INSERT_COUNT ALLOWED_RANGES {32}
...
set_parameter_property EXAMPLE_DESIGN_SELECT DEFAULT_VALUE "HMCC Daughter Board"
set_parameter_property EXAMPLE_DESIGN_SELECT DISPLAY_NAME "Select Design"
set_parameter_property EXAMPLE_DESIGN_SELECT ALLOWED_RANGES {"HMCC Daughter Board"}
...
set_parameter_property EXAMPLE_DESIGN_SELECT DESCRIPTION "<p>The HMCC daughter board example design demonstrates simple sequential transactions for the selected number of ports.<br><br>Note: This design only supports 125 Mhz CDR and default values for Rx and Tx mappings.</p>"
add_display_item "Available Example Design" EXAMPLE_DESIGN_SELECT PARAMETER

# EXAMPLE_DESIGN_SIM_FILES 
add_parameter EXAMPLE_DESIGN_SIM_FILES BOOLEAN 0
...
set_parameter_property EXAMPLE_DESIGN_SYNTH_FILES DEFAULT_VALUE 1
...
set_parameter_property GENERATED_HDL DISPLAY_NAME "Generated File Format"
set_parameter_property GENERATED_HDL ALLOWED_RANGES {"Verilog"}
set_parameter_property GENERATED_HDL DISPLAY_HINT ""
set_parameter_property GENERATED_HDL ENABLED true
...
set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"Arria 10 GX FPGA Development Kit (Production Silicon)" "Arria 10 GX FPGA Development Kit (Engineering Sample 2)" "None"}
...
set_parameter_property BOARD_SELECTION DESCRIPTION "<p>This option provides support for various development kits listed. The details for Altera development kits can be found on the Altera website. If this menu is greyed out then unsupported IP parameter values have been selected preventing example design generation. The example design will be generated for the target device shown.<\p>"
...
set_display_item_property "HW_BOARD_TEXT" text "<html>Family: Arria 10 Device: 10AX115S2F45I1SG<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is provided through selected Development kit(s) with a specific device.<br>To exclude hardware aspects of the example design, select none from the Target Development Kit pull down menu.</html>"
...
proc set_BOARD_SELECTION_text { } {
if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
    if { [string match [get_parameter_value BOARD_SELECTION] "Arria 10 GX FPGA Development Kit (Production Silicon)"] == 1 } { 
        set_display_item_property "HW_BOARD_TEXT" text "<html>Family: Arria 10 Device: 10AX115S2F45I1SG<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is provided through selected Development kit(s) with a specific device.<br>To exclude hardware aspects of the example design, select none from the Target Development Kit pull down menu.</html>"
    } elseif { [string match [get_parameter_value BOARD_SELECTION] "Arria 10 GX FPGA Development Kit (Engineering Sample 2)"] == 1 } { 
        set_display_item_property "HW_BOARD_TEXT" text "<html>Family: Arria 10 Device: 10AX115S2F45I1SGE2<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is provided through selected Development kit(s) with a specific device.<br>To exclude hardware aspects of the example design, select none from the Target Development Kit pull down menu.</html>"
    } else {
	    set_display_item_property "HW_BOARD_TEXT" text "<html>Family: [get_parameter_value DEVICE_FAMILY ] Device: [get_parameter_value part_trait_device]<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is provided through selected Development kit(s) with a specific device.<br>To exclude hardware aspects of the example design, select none from the Target Development Kit pull down menu.</html>"
    }
} else {
  set_display_item_property "HW_BOARD_TEXT" text "<html>Family: [get_parameter_value DEVICE_FAMILY ] Device: [get_parameter_value part_trait_device]<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is currently not available for Stratix 10.<br></html>"
} 
}

# Callback to force options in the GUI
proc choose_CDR_REFCLK_callback { arg } {    
if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
    #Example Design Tab
    if {([string match [get_parameter_value CDR_REFCLK] "125"] == 1) && ([string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "true"] == 1) == 1} {
        set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"Arria 10 GX FPGA Development Kit (Production Silicon)" "Arria 10 GX FPGA Development Kit (Engineering Sample 2)" "None"}
        set_parameter_value BOARD_SELECTION "Arria 10 GX FPGA Development Kit (Production Silicon)"
        set_BOARD_SELECTION_text
    } else {
        set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"None"}
        set_parameter_value BOARD_SELECTION "None"
        send_message WARNING "Example design hardware board support not provided for CDR reference clocks other then 125 Mhz."
        set_BOARD_SELECTION_text
    }
    #Example Design Tab END
} else {
  set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"None"}
  set_parameter_value BOARD_SELECTION "None"
  send_message WARNING "Example design hardware board support not provided for Stratix 10."
  set_BOARD_SELECTION_text
}
}
# Callback to force options in the GUI
proc choose_EXAMPLE_DESIGN_SYNTH_FILES_callback { arg } {    
if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
    #Example Design Tab
    if {([string match [get_parameter_value CDR_REFCLK] "125"] == 1) && ([string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "true"] == 1) == 1} {
        set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"Arria 10 GX FPGA Development Kit (Production Silicon)" "Arria 10 GX FPGA Development Kit (Engineering Sample 2)" "None"}
...
proc validate_proc {} {
#If the device family is S10 we need to change the example design board selection to none
if {[string match [get_parameter_value DEVICE_FAMILY ] "Stratix 10"] == 1} {
	set_parameter_property BOARD_SELECTION ALLOWED_RANGES {"None"}
	set_display_item_property "HW_BOARD_TEXT" text "<html>Family: [get_parameter_value DEVICE_FAMILY ] Device: [get_parameter_value part_trait_device]<br><br> Example design supports generation, simulation, and Quartus compile flows for any selected device.<br> The hardware support is provided through selected Development kit(s) with a specific device.<br>To exclude hardware aspects of the example design, select none from the Target Development Kit pull down menu.</html>"
}
...
if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
    ...
    if {[string match [get_parameter_value CDR_REFCLK] "125"] != 1} {
        send_message WARNING "Example design hardware board support not provided for CDR reference clocks other then 125 Mhz. Example design hardware board selection set to NONE"
    }
    ...
    if { [string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "false"] == 1 } {
        if { [string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "false"] == 1 } {
            send_message WARNING "At least one of the Simulation or Synthesis check boxes from Example Design Files must be selected to allow generation of Example Design Files."
            return
        }
    }
    #Example Design Tab END
}    
    if {[string is true [get_parameter_value RESPONSE_REORDERING_GUI]]} {
        set_parameter_value RESPONSE_REORDERING 1
    } else {
        set_parameter_value RESPONSE_REORDERING 0
    }
    
    #General Tab
    if {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        set_parameter_property PORTS ALLOWED_RANGES {"1"}
        set_parameter_property PORTS HDL_PARAMETER false
        
        set_parameter_property RESPONSE_REORDERING ALLOWED_RANGES {0}
    } else {
        set_parameter_property PORTS ALLOWED_RANGES {1 2 3 4}
        set_parameter_property PORTS HDL_PARAMETER true
...
proc elaborate {} {
    ...
    upvar altera_hmcc_rx_xcvr_reset_name altera_hmcc_rx_xcvr_reset_name
    upvar altera_hmcc_tx_xcvr_reset_name altera_hmcc_tx_xcvr_reset_name
    upvar altera_hmcc_xcvr_reset_name    altera_hmcc_xcvr_reset_name
    
    set    altera_xcvr_name               "altera_xcvr_"
    set    altera_hmcc_rx_xcvr_reset_name "altera_hmcc_rx_xcvr_reset_"
    set    altera_hmcc_tx_xcvr_reset_name "altera_hmcc_tx_xcvr_reset_"
    set    altera_hmcc_xcvr_reset_name    "altera_hmcc_xcvr_reset_"
    
    set data_rate_is_10g "0"
    
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Stratix 10"] == 1} {
      set_parameter_property DATA_RATE_GUI ALLOWED_RANGES {"10" "12.5" "15"}
      set_parameter_property CHANNELS ALLOWED_RANGES {16}
      set_parameter_property ENABLE_ADME ENABLED false
    } else {
      set_parameter_property DATA_RATE_GUI ALLOWED_RANGES {"10" "12.5"}
      set_parameter_property CHANNELS ALLOWED_RANGES {8 16}
    }
    ...
    if {[string match [get_parameter_value DATA_RATE_GUI] "10"] == 1} {
        ...
        if {[string match [get_parameter_value CDR_REFCLK] "390.625"]} {
            send_message ERROR "CDR reference clock must be driven at 125, 156.25, or 166.67 Mhz"
        }
        set_parameter_property CDR_REFCLK ALLOWED_RANGES {"125" "156.25" "166.67"}
	...
	send_message INFO "An external PLL with an output frequency of 6250MHz must be connected to the tx_bonding_clocks inputs. See User guide for more information."
        if {[string match [get_parameter_value CDR_REFCLK] "312.5"]} {
            send_message ERROR "CDR reference clock must be driven at 125, or 156.25 Mhz"
        }
        set_parameter_property CDR_REFCLK ALLOWED_RANGES {"125" "156.25"}
        set pll_ref_num_int "390"
        set data_rate_num "12500"
        set data_rate_is_10g "0"
    }  elseif {[string match [get_parameter_value DATA_RATE_GUI] "15"] == 1} {
        set_parameter_value DATA_RATE "15000 Mbps"
	send_message INFO "An external PLL with an output frequency of 7500MHz must be connected to the tx_bonding_clocks inputs. See User guide for more information."
        set_parameter_property CDR_REFCLK ALLOWED_RANGES {"125" "156.25" "166.67"}
        set pll_ref_num_int "390"
        set data_rate_num "15000"
        set data_rate_is_10g "0"
    }
    ...
    append altera_xcvr_name "[expr {[get_parameter_value CHANNELS]/4}]f[get_parameter_value CHANNELS]l_[expr {${data_rate_num}/1000}]g_${cdr_ref_num_int}cdr"
    
    if { [get_parameter_value ENABLE_ADME] } {
        append altera_xcvr_name "_ADME_reconfig"
    }
    
    # !--- YOU MUST CHANGE THE BOARD SELECTION IN THE EXAMPLE DESIGN TAB TO NONE OR DEVICE FAMILY WILL GET FORCED TO ARRIA 10 ---!

    ########## add XCVR    ###############
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
    #add_hdl_instance altera_hmcc_xcvr altera_xcvr_native_a10  18.1
    ...
    add_hdl_instance $altera_xcvr_name altera_xcvr_native_s10  18.1
    #bonding mode defaulting to non-bonding for testing
    set altera_hmcc_xcvr_param_val_list [list  \
        protocol_mode                       "basic_enh" \
        channels                            "[get_parameter_value CHANNELS]" \
        set_data_rate                       "$data_rate_num" \
        ...
        tx_clkout2_sel                      "pcs_x2_clkout" \
        rx_fifo_mode                        "Register-Basic" \
        rx_fifo_pempty                      "4" \
        rx_fifo_pfull                       "20" \
    ...
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        append altera_hmcc_rx_xcvr_reset_name "[expr {[get_parameter_value CHANNELS]/4}]f[get_parameter_value CHANNELS]l_${pll_ref_num_int}pll"
        if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
        add_hdl_instance $altera_hmcc_rx_xcvr_reset_name altera_xcvr_reset_control  18.1
        set altera_hmcc_rx_xcvr_reset_param_val_list [list \
            RX_ENABLE                       "1" \
            TX_ENABLE                       "0" \
            TX_PLL_ENABLE                   "0" \
            CHANNELS                        "[get_parameter_value CHANNELS]" \
        ...
        add_hdl_instance $altera_hmcc_rx_xcvr_reset_name altera_xcvr_reset_control_s10  18.1
        set altera_hmcc_rx_xcvr_reset_param_val_list [list \
            RX_ENABLE                       "1" \
            TX_ENABLE                       "0" \
            TX_PLL_ENABLE                   "0" \
        ...
        foreach {param val} $altera_hmcc_rx_xcvr_reset_param_val_list {
            set_instance_parameter_value $altera_hmcc_rx_xcvr_reset_name $param $val
        }

        append altera_hmcc_tx_xcvr_reset_name "[expr {[get_parameter_value CHANNELS]/4}]f[get_parameter_value CHANNELS]l_${pll_ref_num_int}pll"

        ########## add TX XCVR reset    ###############
        if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
        add_hdl_instance $altera_hmcc_tx_xcvr_reset_name altera_xcvr_reset_control  18.1
        set altera_hmcc_tx_xcvr_reset_param_val_list [list \
            RX_ENABLE                       "0" \
            TX_ENABLE                       "1" \
            TX_PLL_ENABLE                   "1" \
            CHANNELS                        "[get_parameter_value CHANNELS]" \
        ...
        add_hdl_instance $altera_hmcc_tx_xcvr_reset_name altera_xcvr_reset_control_s10  18.1
        set altera_hmcc_tx_xcvr_reset_param_val_list [list \
            RX_ENABLE                       "0" \
            TX_ENABLE                       "1" \
            CHANNELS                        "[get_parameter_value CHANNELS]" \
        ...
        foreach {param val} $altera_hmcc_tx_xcvr_reset_param_val_list {
            set_instance_parameter_value $altera_hmcc_tx_xcvr_reset_name $param $val
        }
    } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        append altera_hmcc_xcvr_reset_name "[expr {[get_parameter_value CHANNELS]/4}]f[get_parameter_value CHANNELS]l_${pll_ref_num_int}pll"
        if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
         ########## add XCVR reset    ###############
        ...
        add_hdl_instance $altera_hmcc_xcvr_reset_name altera_xcvr_reset_control_s10  18.1
        set altera_hmcc_xcvr_reset_param_val_list [list \
            RX_ENABLE                       "0" \
            TX_ENABLE                       "1" \
            CHANNELS                        "[get_parameter_value CHANNELS]" \
    ...
    #set_instance_property altera_hmcc_rx_xcvr_reset HDLINSTANCE_USE_GENERATED_NAME true 
    #set_instance_property altera_hmcc_tx_xcvr_reset HDLINSTANCE_USE_GENERATED_NAME true 
...
#   add_conduit pll_powerdown       end     Output  1
    ...
    if {[string match [get_parameter_value PORTS] "1"] == 1} {
    # User Request Interface
    add_conduit dp_req_ready    end     Output  1
    if {[string match [get_parameter_value RESPONSE_REORDERING] "0"] == 1} {
    ...
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
    add_conduit dp_rsp_errstat  end     Output  7
    }
	...
	set ports [get_parameter_value PORTS]
	for {set i 0} {$i<$ports} {set i [expr $i+1] } {
	    ## User Request Interface
	    add_conduit dp${i}_req_ready    end     Output  1
	    if {[string match [get_parameter_value RESPONSE_REORDERING] "0"] == 1} {
		add_conduit dp${i}_req_tag      end     Input   9
	    }
	    add_conduit dp${i}_req_cube     end     Input   3
	    add_conduit dp${i}_req_addr     end     Input   34
	    add_conduit dp${i}_req_cmd      end     Input   6
	    add_conduit dp${i}_req_data     end     Input   [expr {[get_parameter_value CHANNELS] * 32}]
	    add_conduit dp${i}_req_valid    end     Input   1
	    add_conduit dp${i}_req_sop      end     Input   1
	    add_conduit dp${i}_req_eop      end     Input   1

	    # User Response Interface
	    if {[string match [get_parameter_value RESPONSE_REORDERING] "0"] == 1} {
		add_conduit dp${i}_rsp_tag      end     Output  9
	    }
	    add_conduit dp${i}_rsp_size     end     Output  3
	    add_conduit dp${i}_rsp_cmd      end     Output  6
	    add_conduit dp${i}_rsp_error    end     Output  1
	    add_conduit dp${i}_rsp_errstat  end     Output  7
	    add_conduit dp${i}_rsp_data     end     Output  [expr {[get_parameter_value CHANNELS] * 32}]
	    add_conduit dp${i}_rsp_valid    end     Output  1
	    add_conduit dp${i}_rsp_sop      end     Output  1
	    add_conduit dp${i}_rsp_eop      end     Output  1
	}
    }
...
proc synth_proc {outputName} {  
    ...
    global altera_hmcc_rx_xcvr_reset_name
    global altera_hmcc_tx_xcvr_reset_name
    global altera_hmcc_xcvr_reset_name
    
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        set 4f16l_src_files     "altera_hmcc_crc_1 altera_hmcc_crc_2 altera_hmcc_crc_3 altera_hmcc_crc_9 altera_hmcc_delimiter altera_hmcc_error_response_queue altera_hmcc_irtry_counter altera_hmcc_reordering_response_queue altera_hmcc_response_queue altera_hmcc_tx_retry_buffer altera_hmcc_retry_counter altera_hmcc_rx altera_hmcc_seq_checker altera_hmcc_token_extractor altera_hmcc_tx altera_hmcc_tx_crc32k altera_hmcc_tx_mtaps altera_hmcc_tx_port_fifo altera_hmcc_tx_scheduler"
    ...
    foreach vfile $common_folder_v_files {
        add_fileset_file src/${vfile}.v VERILOG PATH ${search_path}/common/${vfile}.v
    }

    foreach svfile $common_folder_sv_files {
        add_fileset_file src/${svfile}.sv SYSTEM_VERILOG PATH ${search_path}/common/${svfile}.sv
    }
    
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        add_fileset_file src/altera_hmcc_mtaps_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_mtaps_wys_lut_inc.iv
    } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        add_fileset_file src/altera_hmcc_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_wys_lut_inc.iv
    }

    
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        # Files in the src folder
        set src_folder_sv_files [split "$4f16l_src_files" " "]

        foreach svfile $src_folder_sv_files {
            add_fileset_file src/${svfile}_4f16l.sv SYSTEM_VERILOG PATH ${search_path}/4f16l/${svfile}_4f16l.sv
        }
        # Add Full-Width OCP file
        add_fileset_file src/altera_hmcc_tx_4f16l.ocp OTHER PATH ${search_path}/4f16l/altera_hmcc_tx_4f16l.ocp
    } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        # Files in the src folder
        set src_folder_sv_files [split "$2f8l_src_files" " "]

        foreach svfile $src_folder_sv_files {
            add_fileset_file src/${svfile}_2f8l.sv SYSTEM_VERILOG PATH ${search_path}/2f8l/${svfile}_2f8l.sv
        }
        # Add Half-Width OCP file
        add_fileset_file src/altera_hmcc_tx_2f8l.ocp OTHER PATH ${search_path}/2f8l/altera_hmcc_tx_2f8l.ocp
    }
    ...
    } elseif {[string match [get_parameter_value DATA_RATE] "15000 Mbps"] == 1} {
        set terp_data_rate "15"
    }

    set params(device_family)                   [get_parameter_value DEVICE_FAMILY]
    set params(channels)                        [get_parameter_value CHANNELS]
    set params(response_reordering)             [get_parameter_value RESPONSE_REORDERING]
    set params(ports)                           [get_parameter_value PORTS]
    set params(data_rate)                       $terp_data_rate
    set params(top_level_name)                  $outputName 
    set params(altera_xcvr_name)                $altera_xcvr_name              
    set params(altera_hmcc_rx_xcvr_reset_name)  $altera_hmcc_rx_xcvr_reset_name
    set params(altera_hmcc_tx_xcvr_reset_name)  $altera_hmcc_tx_xcvr_reset_name
    set params(altera_hmcc_xcvr_reset_name)     $altera_hmcc_xcvr_reset_name
    interpret_terp src/${outputName}.sdc ${search_path}/common/altera_hmcc.sdc.terp [array get params] "SDC"
    interpret_terp src/${outputName}.sv  ${search_path}/common/altera_hmcc.sv.terp  [array get params]
}
...
#   add_fileset_file altera_std_synchronizer_nocut.v VERILOG PATH $std_sync_path 
    if {1} {
	add_fileset_file src/synopsys/altera_std_synchronizer_nocut.v VERILOG_ENCRYPT PATH $std_sync_path SYNOPSYS_SPECIFIC
    }
    if {1} {
	add_fileset_file src/mentor/altera_std_synchronizer_nocut.v VERILOG_ENCRYPT PATH $std_sync_path MENTOR_SPECIFIC
    }
    if {1} {
	add_fileset_file src/cadence/altera_std_synchronizer_nocut.v VERILOG_ENCRYPT PATH $std_sync_path CADENCE_SPECIFIC
    }
    ...
    foreach vfile $common_folder_v_files {
        if {1} {
            add_fileset_file src/synopsys/${vfile}.v VERILOG_ENCRYPT PATH ${search_path}/common/synopsys/${vfile}.v SYNOPSYS_SPECIFIC
        }
        if {1} {
            add_fileset_file src/mentor/${vfile}.v VERILOG_ENCRYPT PATH ${search_path}/common/mentor/${vfile}.v MENTOR_SPECIFIC
        }
        if {1} {
            add_fileset_file src/cadence/${vfile}.v VERILOG_ENCRYPT PATH ${search_path}/common/cadence/${vfile}.v CADENCE_SPECIFIC
        }
    }
    
    foreach svfile $common_folder_sv_files {
        if {1} {
            add_fileset_file src/synopsys/${svfile}.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/common/synopsys/${svfile}.sv SYNOPSYS_SPECIFIC
        }
        if {1} {
            add_fileset_file src/mentor/${svfile}.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/common/mentor/${svfile}.sv MENTOR_SPECIFIC
        }
        if {1} {
            add_fileset_file src/cadence/${svfile}.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/common/cadence/${svfile}.sv CADENCE_SPECIFIC
        }
    }
    #If the .iv file for Quartus is not encrypted use that .iv file for synopsys and mentor
    if {[file exists "../common/altera_hmcc_mtaps_wys_lut_inc.iv"]} { 
        set fp [open "../common/altera_hmcc_mtaps_wys_lut_inc.iv" r]
        set filecontent [read $fp]
        set input_list [split $filecontent "\n"]
        set searchTextList [lsearch -all -inline $input_list "*alterathisfilemustbeencryptedaltera*"]
        set encrypted 1
        foreach elem $searchTextList {
            set encrypted 0
        }
        close $fp
        ...
        if {$encrypted == 0} {
            if {1} {
                add_fileset_file src/synopsys/altera_hmcc_mtaps_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_mtaps_wys_lut_inc.iv SYNOPSYS_SPECIFIC
            }
            add_fileset_file src/mentor/altera_hmcc_mtaps_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_mtaps_wys_lut_inc.iv MENTOR_SPECIFIC
        } else {
            if {1} {
                add_fileset_file src/synopsys/altera_hmcc_mtaps_wys_lut_inc.ivp SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/synopsys/altera_hmcc_mtaps_wys_lut_inc.ivp SYNOPSYS_SPECIFIC
            }
        }
        } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        if {$encrypted == 0} {
            if {1} {
                add_fileset_file src/synopsys/altera_hmcc_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_wys_lut_inc.iv SYNOPSYS_SPECIFIC
            }
            add_fileset_file src/mentor/altera_hmcc_wys_lut_inc.iv SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/altera_hmcc_wys_lut_inc.iv MENTOR_SPECIFIC
        } else {
            if {1} {
                add_fileset_file src/synopsys/altera_hmcc_wys_lut_inc.ivp SYSTEM_VERILOG_INCLUDE PATH ${search_path}/common/synopsys/altera_hmcc_wys_lut_inc.ivp SYNOPSYS_SPECIFIC
            }
        }
        }
    }
    ...
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        ...
        foreach svfile $src_folder_sv_files {
            if {1} {
                add_fileset_file src/synopsys/${svfile}_4f16l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/4f16l/synopsys/${svfile}_4f16l.sv SYNOPSYS_SPECIFIC
            }
            if {1} {
                add_fileset_file src/mentor/${svfile}_4f16l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/4f16l/mentor/${svfile}_4f16l.sv MENTOR_SPECIFIC
            }
            if {1} {
                add_fileset_file src/cadence/${svfile}_4f16l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/4f16l/cadence/${svfile}_4f16l.sv CADENCE_SPECIFIC
            }
        }
    } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        # Files in the src folder
        ...
        foreach svfile $src_folder_sv_files {
            if {1} {
                add_fileset_file src/synopsys/${svfile}_2f8l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/2f8l/synopsys/${svfile}_2f8l.sv SYNOPSYS_SPECIFIC
            }
            if {1} {
                add_fileset_file src/mentor/${svfile}_2f8l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/2f8l/mentor/${svfile}_2f8l.sv MENTOR_SPECIFIC
            }
            if {1} {
                add_fileset_file src/cadence/${svfile}_2f8l.sv SYSTEM_VERILOG_ENCRYPT PATH ${search_path}/2f8l/cadence/${svfile}_2f8l.sv CADENCE_SPECIFIC
            }
        }
    }
...
proc example_design {outputName} {
    ...
    if { [string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "false"] == 1 } {
        if { [string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "false"] == 1 } {
            send_message ERROR "At least one of the Simulation or Synthesis check boxes from Example Design Files must be selected to allow generation of Example Design Files."
            return
        }
    }
    ...
    generate_related_ip_files $outputName $device $edition
    ...
    if {[string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "true"] == 1} {
        append files_in_example_design_folder { \
            par/hmcc_example.qpf \
            par/hmcc_example.sdc \
            par/sysconsole_testbench.tcl \
        }
    }
    
    if {[string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "true"] == 1} {
        append files_in_example_design_folder { \
    ...
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
      foreach file $files_in_example_design_folder {
         add_fileset_file example_design/${file} OTHER PATH ${search_path}/a10_example_design/${file}
      }
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "true"] == 1} {
         add_fileset_file example_design/sim/Makefile OTHER PATH ${search_path}/a10_example_design/sim/Makefile.mk
         add_fileset_file example_design/sim/make.bat OTHER PATH ${search_path}/a10_example_design/sim/Makefile_windows.mk
      }
      
      set num_lanes [get_parameter_value CHANNELS]
      
      add_fileset_file example_design/src/m20k_2port.v            OTHER PATH  "${search_path}/a10_example_design/src/m20k_2port_${num_lanes}l.v"
      add_fileset_file example_design/src/request_generator.sv    OTHER PATH  "${search_path}/a10_example_design/src/request_generator_${num_lanes}l.sv"
      add_fileset_file example_design/src/response_monitor.sv     OTHER PATH  "${search_path}/a10_example_design/src/response_monitor_${num_lanes}l.sv" 
    } else {
      foreach file $files_in_example_design_folder {
         add_fileset_file example_design/${file} OTHER PATH ${search_path}/s10_example_design/${file}
      }
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "true"] == 1} {
         add_fileset_file example_design/sim/Makefile OTHER PATH ${search_path}/s10_example_design/sim/Makefile.mk
         add_fileset_file example_design/sim/make.bat OTHER PATH ${search_path}/s10_example_design/sim/Makefile_windows.mk
      }
      
      set num_lanes [get_parameter_value CHANNELS]
      
      add_fileset_file example_design/src/m20k_2port.v            OTHER PATH  "${search_path}/s10_example_design/src/m20k_2port_${num_lanes}l.v"
      add_fileset_file example_design/src/request_generator.sv    OTHER PATH  "${search_path}/s10_example_design/src/request_generator_${num_lanes}l.sv"
      add_fileset_file example_design/src/response_monitor.sv     OTHER PATH  "${search_path}/s10_example_design/src/response_monitor_${num_lanes}l.sv" 
    ...
    set params(num_ports)                       [get_parameter_value PORTS]
    ...
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "true"] == 1} {
          interpret_terp ../example_design/sim/hmc_cfg.sv         ${search_path}/a10_example_design/sim/hmc_cfg.sv.terp       [array get params]
          interpret_terp ../example_design/sim/hmcc_tb.sv         ${search_path}/a10_example_design/sim/hmcc_tb.sv.terp       [array get params]
      }
      
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "true"] == 1} {
          interpret_terp ../example_design/par/hmcc_example.qsf   ${search_path}/a10_example_design/par/hmcc_example.qsf.terp [array get params] "OTHER"
      }
      
      interpret_terp ../example_design/src/hmcc_example.sv    ${search_path}/a10_example_design/src/hmcc_example.sv.terp  [array get params]
    } else {
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SIM_FILES] "true"] == 1} {
          interpret_terp ../example_design/sim/hmc_cfg.sv         ${search_path}/s10_example_design/sim/hmc_cfg.sv.terp       [array get params]
          interpret_terp ../example_design/sim/hmcc_tb.sv         ${search_path}/s10_example_design/sim/hmcc_tb.sv.terp       [array get params]
      }
      
      if {[string match [get_parameter_value EXAMPLE_DESIGN_SYNTH_FILES] "true"] == 1} {
          interpret_terp ../example_design/par/hmcc_example.qsf   ${search_path}/s10_example_design/par/hmcc_example.qsf.terp [array get params] "OTHER"
      }
      
      interpret_terp ../example_design/src/hmcc_example.sv    ${search_path}/s10_example_design/src/hmcc_example.sv.terp  [array get params]
    }
...
# Taken from Ethernet! :)
#
proc generate_related_ip_files {outputName device edition} {
    if {[string match [get_parameter_value CHANNELS] "16"] == 1} {
        set search_path ".."
    } elseif {[string match [get_parameter_value CHANNELS] "8"] == 1} {
        set search_path "../../altera_hmcc"
    ...
    if {[string match $edition "std"] == 1} {
      set package_require "package require -exact qsys 14.1"
    } else {
      set package_require "package require -exact qsys 16.0"
    }   
    
    puts $fileId $package_require
    puts $fileId "create_system ${hmcc_name}_system"

    puts $fileId "add_instance $hmcc_name altera_hmcc_mtaps  18.1"
    ...
    if {[string match $edition "std"] == 1} {
      ...
      puts $fileId "save_system [file dirname $templocation]/$subdir/$hmcc_name.ip"
    }  
    ...
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
        ...
        puts $fileId "set_project_property DEVICE        $device"
        puts $fileId "add_instance $pll_name altera_xcvr_atx_pll_s10  18.1"
    }
    ...
    if {[string match [get_parameter_value DEVICE_FAMILY ] "Arria 10"] == 1} {
      ...
      puts $fileId "set_instance_parameter_value $pll_name rcfg_enable    \"1\""
    }
    
    if {[string match $edition "std"] == 1} {
      puts $fileId "save_system [file dirname $templocation]/$subdir/$pll_name.qsys"
    } else {
      puts $fileId "save_system [file dirname $templocation]/$subdir/$pll_name.ip"
    }
    ...
    if {[string match $edition "std"] == 1} {
      ...
      puts $fileId "save_system [file dirname $templocation]/$subdir/$gpio_name.ip"
    }
    
    close $fileId

    send_message INFO "Generating Transceiver ATX PLL IP core."
    local_qsysgenerate [file dirname $templocation] ${pll_name}_gen.tcl  $pll_name $subdir $device $edition
    send_message INFO "Done generating Transceiver ATX PLL IP core."

    send_message INFO "Generating HMC Controller IP core."
    local_qsysgenerate [file dirname $templocation] ${hmcc_name}_gen.tcl $hmcc_name $subdir $device $edition
    send_message INFO "Done generating HMC Controller IP core."
    
    send_message INFO "Generating GPIO IP core."
    local_qsysgenerate [file dirname $templocation] ${gpio_name}_gen.tcl $gpio_name $subdir $device $edition
    send_message INFO "Done generating GPIO IP core."
    
    set qdir $::env(QUARTUS_ROOTDIR)
    send_message INFO "Generating AVMM system."
    exec -ignorestderr "${qdir}/sopc_builder/bin/qsys-generate" "${search_path}/a10_example_design/src/AVMM.qsys" "--synthesis=VERILOG" "--output-directory=[file dirname $templocation]/$subdir/AVMM" "--part=$device" "--family=[get_parameter_value DEVICE_FAMILY]"
...
#
proc local_qsysgenerate { filepath filename qsysname subdir device edition} { 
...
add_documentation_link "Example Design" https://www.altera.com/en_US/pdfs/literature/ug/deug_hmcc.pdf

...
set_clock_groups -asynchronous -group  [get_clocks {usr_clk}] -to [get_clocks {*test_env*dut*ilk_50g*g_xcvr_native*tx_pma_clk}]

set_clock_groups -asynchronous -group  {mm_clk} -group  {usr_clk}
set_clock_groups -asynchronous -group  {clk50} -group   {usr_clk}

set_false_path -from [get_clocks {~ALTERA_CLKUSR~}] -to   [get_clocks {test_env|test_dut|dut|ilk_50g*|np_inst|np_inst|g_xcvr_native_insts[*]|tx_pma_clk}]
set_false_path -to   [get_clocks {~ALTERA_CLKUSR~}] -from [get_clocks {test_env|test_dut|dut|ilk_50g*|np_inst|np_inst|g_xcvr_native_insts[*]|tx_pma_clk}]
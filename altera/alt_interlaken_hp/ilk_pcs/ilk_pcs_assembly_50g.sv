...
assign rx_digital_rst = !rd4_ready && !ignore_rx_digital;
   ...
   generate
      for (i=0; i<MAX_NUM_SIXPACKS; i=i+1) begin : gxlp
         ...
         if (((LANE_PROFILE >> (i*MAX_LANES_PER_SIXPACK)) & 6'h3f) != 6'h00) begin
            ...
            reg  [MAX_LANES_PER_SIXPACK*20-1:0] tmp_pcs_testbus_r  = {MAX_LANES_PER_SIXPACK*20{1'b0}};
            reg  [MAX_LANES_PER_SIXPACK*20-1:0] tmp_pcs_testbus_rr = {MAX_LANES_PER_SIXPACK*20{1'b0}};
            ...
            defparam gx .PMA_PCS_WIDTH      = 32;
            defparam gx .TX_ODWIDTH         = "width_32";
            defparam gx .RX_IDWIDTH         = "width_32";
            ...
            always @(posedge mm_clk) begin
              tmp_pcs_testbus_r  <= tmp_pcs_testbus;
              tmp_pcs_testbus_rr <= tmp_pcs_testbus_r;
            end

            if( DIAG_ON != 0 ) begin
               assign gated_pcs_testbus [(i+1)*MAX_LANES_PER_SIXPACK*20-1 : i*MAX_LANES_PER_SIXPACK*20] =
                      {(MAX_LANES_PER_SIXPACK*20){pcs_testbus_6sel[i]}} & tmp_pcs_testbus_rr & exp_pcs_testbus_lnsel;
            end
   ...
   generate
     if (BYPASS_RDC) begin   //This is related to how striper behave. For 8 lane 6g only usr clock 250~300MHz
       always @(posedge clk_rx_common) begin
          if (srst_rx_common) begin
             rx_cadence      <= 1'b0;
          end
          else begin
             rx_cadence      <= (~|rx_pempty_r) && (!rx_cadence);   //Generate case
          end
       end
     end else begin
       always @(posedge clk_rx_common) begin
          if (srst_rx_common) begin
             ...
             rx_cadence      <= (~|rx_pempty_r) && (!last_rx_cadence || !rx_cadence);   //Generate case
          end
       end
     end
   endgenerate
   ...
   reg  [NUM_LANES-1:0] tx_pempty_r = {NUM_LANES{1'b0}};
   reg  [NUM_LANES-1:0] tx_pempty_rr = {NUM_LANES{1'b0}};
   
   always @(posedge tx_usr_clk) begin
        tx_pempty_r   <= tx_pempty;
        tx_pempty_rr  <= tx_pempty_r;
    end   
   ...
   assign tx_pcsfifo_pempty = |tx_pempty_rr;

...
module ilk_rx_aligner #(
        ...
        output reg                                rx_fifo_clr  /* synthesis preserve dont_replicate dont_retime */,
   ...
   generate
     if (BYPASS_RDC && (NUM_LANES == 8)) begin   //This is related to how striper behave. For 8 lane 6g only usr clock 250~300MHz
       always @(posedge clk_rx_common) begin
          if (srst_rx_common) begin
             rx_cadence      <= 1'b0;
          end
       ...
       always @(posedge clk_rx_common) begin
          if (srst_rx_common) begin
             ...
             rx_cadence      <= (~|rx_pempty_r) && (!last_rx_cadence || !rx_cadence);   //General case
          end
       end
     end
   endgenerate

...
`ifdef ALTERA_RESERVED_QIS
  localparam  CNTR_BITS = 20;
`else
  localparam  CNTR_BITS = 6;
`endif
...
// localparam BYPASS_RBF = (DATA_RATE == "6250.0 Mbps") ? 0:1;
   ...
   ******************************************************************************************/
   ...
   ECC status 
   ******************************************************************************************/
    wire                         [1:0] eccstatus_rx_mmclk;
    wire                         [1:0] eccstatus_tx_mmclk;
    wire                         [1:0] eccstatus_tx_macclk;
    wire                         [1:0] eccstatus_rg0_usrclk;
    
   ilk_pulse_stretch_sync ss_ecc_rx ( .aclr_fast_clk(1'b0), .fast_clk(rx_usr_clk), .aclr_slow_clk(1'b0), .slow_clk(mm_clk),
                                      .din(eccstatus_rg0_usrclk), .dout(eccstatus_rx_mmclk));
   defparam  ss_ecc_rx.WIDTH = 2;

   ilk_pulse_stretch_sync ss_ecc_tx ( .aclr_fast_clk(1'b0), .fast_clk(tx_mac_clk), .aclr_slow_clk(1'b0), .slow_clk(mm_clk),
                                      .din(eccstatus_tx_macclk), .dout(eccstatus_tx_mmclk));
   defparam  ss_ecc_tx.WIDTH = 2;
   ...
   ****************************************************************************************/
   ...
   ilk_iw8_mac_ctl_status #(
      ...
      .DIAG_ON    (DIAG_ON)
        ...
        .eccstatus_rx        (eccstatus_rx_mmclk),
        .eccstatus_tx        (eccstatus_tx_mmclk),
...
generate
   if ( FAMILY == "Arria 10" ) begin : pcs_assembly_a10_gen
    ilk_hard_pcs_assembly_a10 #(
      ...
      .mm_clk             (mm_clk),
      .mm_clk_locked      (reset_n),   // fires whole analog reset sequence + CPU
      .mm_read            (mm_read_pcs),
...
//             lane_txd [(i+1)*64-1:i*64] <= 64'h0;
   ...
   reg [NUM_LANES-1:0] irx_lanewords_valid /* synthesis preserve */;
   always @(posedge clk_rx_common) begin
      if (srst_rx_common) irx_lanewords_valid <= {NUM_LANES{1'b0}};
      else irx_lanewords_valid <= {NUM_LANES{rx_valid[0] && !rx_framing_decision}};
   end
   ...
   generate
     if (BYPASS_RDC) begin
        ...
        assign lanewords_buf_valid = irx_lanewords_valid[0];
        ...
        defparam rdc .TARGET_CHIP = TARGET_CHIP;
     end
   endgenerate
   ...
   ilk_packet_regroup_8 #(
      ...
      .eccstatus                (eccstatus_rg0_usrclk),

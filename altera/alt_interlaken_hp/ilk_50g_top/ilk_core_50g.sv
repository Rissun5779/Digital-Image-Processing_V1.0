...
module ilk_core_50g #(
  ...
  //Bypass domain crossing smooth FIFO at the receiver side.
  parameter BYPASS_RDC          = 1,
  // Can be set to 1 in designs which share MAC and PCS clock.
  parameter BYPASS_LOOSEFIFO    = 0,
  //Bypass small fifo at receiver side.
  parameter BYPASS_RBF           = 1,
  //TX user clock period round in MHZ. 250MHz in default.
  parameter TX_USRCLK_MHZ        = 250,
  ...
  //Enable Error handler block
  parameter ERR_HANDLER_ON      = 1
)(
 ...
 //The following parameter is used to control the flow control to TX hard pcs fifo.
 //At 300MHz, the max rate TX MAC sends to TX PCS is 330/512 = 64.5%. The minimum
 //rate is at 200/511 = 39.1%. The normal rate is toggle between these. For other
 //Clock frequency, it is pro-rated based on clock period.
 
  localparam  TX_USRCLK_PERIOD = 1000000 / TX_USRCLK_MHZ;
  localparam  TX_MAX_DUTY      = ((330 * TX_USRCLK_PERIOD / 3333) > 511) ? 511 : (
                                  330 * TX_USRCLK_PERIOD / 3333);
  localparam  TX_MIN_DUTY      = ((200 * TX_USRCLK_PERIOD / 3333) > 511) ? 511 : (
                                   200 * TX_USRCLK_PERIOD / 3333);
...
//For Arria 10
wire         tx_pll_powerdown_a10;
wire [31: 0] reconfig_readdata_a10;
wire         reconfig_waitrequest_a10;
wire [W_BUNDLE_FROM_XCVR*(NUM_LANES+NUM_SIXPACKS)-1:0] reconfig_from_xcvr_sv;
   generate
     if (FAMILY == "Arria 10") begin
         ...
         assign reconfig_readdata     = reconfig_readdata_a10;
         assign reconfig_waitrequest  = reconfig_waitrequest_a10;
     end
     else  begin
         assign tx_pll_powerdown      = 1'b0;
         assign reconfig_from_xcvr    = reconfig_from_xcvr_sv;
         assign reconfig_readdata     = 32'b0;
         assign reconfig_waitrequest  = 32'b0;
...
ilk_status_sync ss_tx_all_locked_mac (.clk(tx_usr_clk),.din(tx_lanes_aligned_pcsclk),.dout(tx_lanes_aligned_macclk));
...
// PCS instanciation
//////////////////////////////////////////////////////////////////
wire pre_crc_words_valid;
generate
  wire striper_wd_req;
  if ( FAMILY == "Arria 10" ) begin : pcs_assembly_a10_gen
    ilk_hard_pcs_assembly_a10_50g #(
      .NUM_LANES          (NUM_LANES),
      ...
      .reconfig_readdata  (reconfig_readdata_a10 ),
      .reconfig_waitrequest(reconfig_waitrequest_a10 )

    );
  end else if ( ( FAMILY == "Stratix V" ) || ( FAMILY == "Arria V GZ" ) ) begin : hard_pcs
    assign striper_wd_req = 1'b0;
    ilk_pcs_assembly_50g #(
      .USE_ATX            (USE_ATX),
      .DATA_RATE          (DATA_RATE),
      ...
      .reconfig_from_xcvr (reconfig_from_xcvr_sv)
    );
  end
endgenerate
...
always@ (posedge rx_usr_clk) begin
  sop_cntr_inc <= |irx_sop & (|irx_num_valid);
  eop_cntr_inc <= |irx_eopbits & (|irx_num_valid);
end
...
ilk_iw4_iw2_rx_packet_regroup rg0 (
  ...
  .dout_muse      (core_irx_muse),
  .dout_crc24_err (core_irx_crc24_err),
  .holding        (rxfifo_fill_level_macclk),
  .overflow       (rg_overflow_macclk)
);
...
//      assign irx_multi_use  = core_irx_muse;

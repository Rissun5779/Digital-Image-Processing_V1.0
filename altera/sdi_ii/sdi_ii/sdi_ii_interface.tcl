...
# +-----------------------------------
# | proc : add_connect_fanout
# | 
# | Add a fanout instance and connect the 2nd output to instance 2.
# | Only connecting fanout1 now.
#
proc add_connect_fanout { instance1 interface1 instance2 interface2  fanout_width } {
     add_instance    ${interface1}_fanout               altera_fanout
     set_instance_parameter     ${interface1}_fanout    WIDTH   $fanout_width
     add_connection  ${instance1}.${interface1}         ${interface1}_fanout.sig_input
     add_connection  ${interface1}_fanout.sig_fanout1   ${instance2}.${interface2}
}
...
# +---------------------------------------------------------------------------
# proc: add_export_fanout_interface
#
# Add and export interface of fanout without renaming
#
proc add_export_fanout_interface { interface type port_dir} {
    add_interface              $interface   $type                       $port_dir
    set_interface_property     $interface   export_of                   ${interface}_fanout.sig_fanout0
    set_interface_property     $interface   PORT_NAME_MAP              "${interface} sig_fanout0"
    set_interface_assignment   $interface   ui.blockdiagram.direction   $port_dir
}

# +---------------------------------------------------------------------------
# proc: terminate_port
#
# Terminate any unnecessary port
#
proc terminate_port {port_name port_dir} {
   if {$port_dir == "input"} {
...
proc add_phy_xcvr_connection {instance1 instance2 dir device} {
  ...
  if { $dir == "rx" || $dir == "du" } {
     ...
     add_connection       ${instance1}.xcvr_rx_is_lockedtoref       ${instance2}.rx_is_lockedtoref
...
proc add_phy_xcvr_b_connection {instance1 instance2 dir device} {
  ...
  if { $dir == "rx" || $dir == "du" } {
     ...
     add_connection       ${instance1}.xcvr_rx_is_lockedtoref_b     ${instance2}.rx_is_lockedtoref
...
proc add_phy__rx_phy_mgmt_connection {instance1 instance2 std device}  {
  if { $device == "Stratix IV" } {
     ...
     # add_connect_fanout   $instance1   rx_pll_locked    $instance2   rx_pll_locked  1
     ...
     if { $std == "dl" } {
        ...
        # add_connect_fanout   $instance1   rx_pll_locked_b    $instance2   rx_pll_locked_b   1
...
proc add_rx_phy_mgmt__rx_protocol_connection {instance1 instance2 std} {
  ...
  add_connect_fanout   $instance1   rx_rst_proto_out    $instance2   rx_rst_proto_in    1
  ...
  if { $std == "mr" } {
    add_connect_fanout   $instance2   rx_trs              $instance1   rx_trs_in    4
  } else {
    add_connect_fanout   $instance2   rx_trs              $instance1   rx_trs_in    1
  }
...
proc tx_protocol__add_export_interface {instance config insert_vpid std} {
  ...
  if {$config == "proto"} {
    ...
    if { ($std == "threeg" & [get_parameter_value FAMILY] != "Arria 10") | $std == "ds" | $std == "tr" } {
...
proc tx_phy_mgmt__add_export_interface {instance config std device} {
  if { $config == "xcvr" } {
  ...
  if { $std == "dl" } {
   ...
   if {$device != "Stratix V" && $device != "Arria V" && $device != "Arria V GZ" && $device != "Cyclone V"} {
      add_interface tx_dataout conduit output
      set_interface_property tx_dataout export_of ${instance}.xcvr_tx_dataout
      set_interface_property tx_dataout PORT_NAME_MAP "tx_dataout xcvr_tx_dataout"

      if { $std == "dl" } {
         add_interface tx_dataout_b conduit output
         set_interface_property tx_dataout_b export_of ${instance}.xcvr_tx_dataout_b
         set_interface_property tx_dataout_b PORT_NAME_MAP "tx_dataout_b xcvr_tx_dataout_b"
      }
   }
}

proc phy__add_export_interface {instance phy_name phy_rst_ctrl dir std device xcvr_tx_pll_sel} {
  if { $dir == "du" | $dir == "tx" } {
  ...
  if { $dir == "du" | $dir == "rx" } {
    ...
    # add_export_fanout_interface    rx_pll_locked      conduit output

    if { $std == "dl" } {
      add_export_interface $instance sdi_rx_b              conduit input
      # add_export_fanout_interface    rx_pll_locked_b       conduit output
    }
  }
  ...
  if { $device != "Arria 10"} {
    ...
    # Case:294867 - Providing flexibility to user to control pll_powerdown signal
     if { $dir == "du" | $dir == "tx" } {
         if { $std == "dl" } {
             add_instance            pll_powerdown_fanout                altera_fanout
             set_instance_parameter  pll_powerdown_fanout                SPECIFY_SIGNAL_TYPE    1
             set_instance_parameter  pll_powerdown_fanout                SIGNAL_TYPE            pll_powerdown
             if { $xcvr_tx_pll_sel == "1" } {
                set_instance_parameter  pll_powerdown_fanout             WIDTH    2
             }

             add_interface pll_powerdown_in conduit input
             set_interface_property pll_powerdown_in export_of pll_powerdown_fanout.sig_input
             set_interface_property pll_powerdown_in PORT_NAME_MAP "pll_powerdown_in sig_input"

             add_connection          pll_powerdown_fanout.sig_fanout0    ${phy_name}.pll_powerdown
             add_connection          pll_powerdown_fanout.sig_fanout1    ${phy_name}_b.pll_powerdown
             add_connection          ${phy_rst_ctrl}_b.pll_powerdown     ${instance}.pll_powerdown_in

         } else {
             add_interface pll_powerdown_in conduit input
             set_interface_property pll_powerdown_in export_of ${phy_name}.pll_powerdown
             set_interface_property pll_powerdown_in PORT_NAME_MAP "pll_powerdown_in pll_powerdown"
         }
         add_interface pll_powerdown_out conduit output
         set_interface_property pll_powerdown_out export_of ${phy_rst_ctrl}.pll_powerdown
         set_interface_property pll_powerdown_out PORT_NAME_MAP "pll_powerdown_out pll_powerdown"
     }
  }
...
proc rx_phy_mgmt__add_export_interface {instance config std device} {
  ...
  if {$config == "xcvr"} {
    add_export_interface            $instance   rx_trs_in               conduit input
    ...
    add_export_fanout_interface    rx_rst_proto_out       conduit output
  }
  
   if {$device == "Arria 10"} {
      # add_export_interface $instance rx_pll_locked   conduit input
      add_export_interface $instance rx_datain       conduit input
      add_export_interface $instance rx_ready        conduit input
      add_export_interface $instance gxb_ltr         conduit output
      add_export_interface $instance gxb_ltd         conduit output
      # add_export_interface $instance trig_rst_ctrl   conduit output
      if { $std == "dl" } {
      # add_export_interface $instance rx_pll_locked_b   conduit input
      add_export_interface $instance rx_datain_b       conduit input
      add_export_interface $instance rx_ready_b        conduit input
      add_export_interface $instance gxb_ltr_b         conduit output
      add_export_interface $instance gxb_ltd_b         conduit output
      # add_export_interface $instance trig_rst_ctrl_b   conduit output
      }
   }
}

proc rx_protocol__add_export_interface {instance config extract_vpid std crc_err a2b b2a device} {
  if { $a2b | $b2a } {
  ...
  if { $config == "proto" } {
     ...
     add_export_fanout_interface            rx_trs              conduit output
  }
...
proc add_tx_clk_rst_connection {config dir std device hd_frequency} {
   ...
   if {$config == "xcvr_proto" | $config == "xcvr"} {
      ...
      if {$device == "Stratix V" || $device == "Arria V" || $device == "Arria V GZ" || $device == "Cyclone V"} {
         add_connection           tx_pclk_bridge.out_clk                      u_phy_adapter.tx_pclk
...
proc add_rx_clk_rst_connection {std device hd_frequency} {
   ...
   if {$device == "Stratix V" || $device == "Arria V" || $device == "Arria V GZ" || $device == "Cyclone V"} {
      ...
      add_clk_bridge         xcvr_rxclk
      add_connection         xcvr_rxclk_bridge.out_clk             u_rx_phy_mgmt.xcvr_rxclk
      add_export_interface   u_rx_phy_mgmt      trig_rst_ctrl      reset         output
      # add_interface          trig_rst_ctrl        reset                 output
      # set_interface_property trig_rst_ctrl        export_of             u_rx_phy_mgmt.trig_rst_ctrl   
  
      if { $std == "dl" } {
         add_clk_bridge         xcvr_rxclk_b
         add_connection         xcvr_rxclk_b_bridge.out_clk        u_rx_phy_mgmt.xcvr_rxclk_b
         if { $device != "Arria 10" } {
            add_export_interface   u_rx_phy_mgmt    trig_rst_ctrl_b   reset         output
         }
      }
   }

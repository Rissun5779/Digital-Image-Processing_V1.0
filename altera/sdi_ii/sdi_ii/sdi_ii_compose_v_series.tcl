...
# |
   if { $gen_xcvr } {
      ...
      set_instance_parameter            u_phy_adapter  VIDEO_STANDARD        $video_std
      set_instance_parameter            u_phy_adapter  DIRECTION             $dir
      set_instance_parameter            u_phy_adapter  XCVR_TX_PLL_SEL       $xcvr_tx_pll_sel
...
# |
   if { $tx_component } {
      ...
      if { $gen_xcvr } {
         ...
         tx_phy_mgmt__add_export_interface u_tx_phy_mgmt $config $video_std $device
...
# |
   if { $rx_component } {
      if { $gen_proto } {
         ...
         rx_protocol__add_export_interface u_rx_protocol $config $extract_vpid $video_std $crc_err $a2b $b2a $device
      }

      if { $gen_xcvr } {
         add_instance       u_rx_phy_mgmt     sdi_ii_rx_phy_mgmt
         propagate_params   u_rx_phy_mgmt
         rx_phy_mgmt__add_export_interface u_rx_phy_mgmt $config $video_std $device
...
# |
   if { $gen_xcvr } {
      ...
      phy__add_export_interface      u_phy_adapter  u_phy   u_tx_phy_rst_ctrl   $dir    $video_std  $device   $xcvr_tx_pll_sel
...
proc add_native_v_phy {family phy_name tx_xcvr_rst_name rx_xcvr_rst_name dir video_std pll_type pll0_data_rate pll1_data_rate pll_refclk_freq0 pll_refclk_freq1 xcvr_tx_pll_sel txpll data_rate pma_clk_div} {
   ...
   set_instance_parameter_value   $phy_name   pll_refclk_cnt                        $refclk_cnt
   ...
   if { $dir == "du" || $dir == "tx" } {
      if { $video_std == "ds" || $video_std == "tr" || $xcvr_tx_pll_sel != "0" } {
      ...
      if { $pll_refclk_freq0 == "148.5 MHz" } {
        set_instance_parameter_value   $tx_xcvr_rst_name   SYS_CLK_IN_MHZ            149
      } elseif { $pll_refclk_freq0 == "74.25 MHz" } {
        set_instance_parameter_value   $tx_xcvr_rst_name   SYS_CLK_IN_MHZ            75
      }
   ...
   if { $dir == "du" || $dir == "rx" } {
      ...
      if { $pll_refclk_freq0 == "148.5 MHz" } {
        set_instance_parameter_value   $rx_xcvr_rst_name   SYS_CLK_IN_MHZ            149
      } elseif { $pll_refclk_freq0 == "74.25 MHz" } {
        set_instance_parameter_value   $rx_xcvr_rst_name   SYS_CLK_IN_MHZ            75
      }

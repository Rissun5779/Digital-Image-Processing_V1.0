...
set fifo_wraclr_collection [get_pins -nowarn -nocase -compatibility_mode *loopback*u_fifo*wraclr|dff*|clrn]
foreach_in_collection pin $fifo_wraclr_collection {
   set fifo_wraclr_name [get_object_info -name $pin]
   set_false_path -to [get_pins -nocase -compatibility_mode $fifo_wraclr_name]
}

set fifo_rdaclr_collection [get_pins -nowarn -nocase -compatibility_mode *loopback*u_fifo*rdaclr|dff*|clrn]
foreach_in_collection pin $fifo_rdaclr_collection {
   set fifo_rdaclr_name [get_object_info -name $pin]
   set_false_path -to [get_pins -nocase -compatibility_mode $fifo_rdaclr_name]
}

if { [llength [query_collection -report -all [get_keepers -nowarn -nocase *loopback*u_fifo_b|*]]] > 0} {
    set_false_path -from [get_keepers -no_case *rx_prealign_dl_linkB|*u_align|data_a_valid] -to [get_keepers -no_case *loopback|*u_fifo_b|*]
}
...
if { [llength [query_collection -report -all [get_keepers -nowarn -nocase *loopback*u_fifo_a|*]]] > 0} {
    apply_sdc_dcfifo *loopback*u_fifo_a
}

if { [llength [query_collection -report -all [get_keepers -nowarn -nocase *loopback*u_fifo_b|*]]] > 0} {
    apply_sdc_dcfifo *loopback*u_fifo_b
}

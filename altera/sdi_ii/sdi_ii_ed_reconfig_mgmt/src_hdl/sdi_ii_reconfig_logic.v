...
//---------------------------------------------------------------------------------------------------------------------------------------------------------
// Module Name: SDI II reconfig logic
//
// Parameter: NUM_CHS - Number of xcvr channels required to do reconfiguration
//            FAMILY - Device family (V-series only)
//            VIDEO_STANDARD - Please specify "dl" for HD dual-link or "tr" for otherwise.
//            DIRECTION - This parameter affects the logical channel number being assigned in generated example design.
//                        If you are making any changes to the design, please ignore this parameter and assign the logical channel number
//                        correctly below. Please refer to SDI II User Guide (Chapter on Expanding to multiple channels) to know how to 
//                        assign the logical channel number. 
//
//---------------------------------------------------------------------------------------------------------------------------------------------------------
module sdi_ii_reconfig_logic # (
  ...
  (
   ...
   input       [NUM_CHS*3-1:0] sdi_rx_std,
  ...
  wire [NUM_CHS*3-1:0] sdi_rx_std_sync;
  ...
  genvar i, j, k, m, n;
   ...
   // 
   //===========================================================================================
   localparam TX_LOG_CH_RCFG = (XCVR_TX_PLL_SEL == 2) ? 8'd1 : 8'd0;
   ...
   assign tx_log_ch_num[0] = TX_LOG_CH_RCFG; // Duplex Tx channel share same logical channel number with Rx
   assign tx_log_ch_num_b[0] = TX_LOG_CH_RCFG; // Duplex Tx channel share same logical channel number with Rx

   generate if (NUM_CHS > 1)
   begin : log_ch1_num_gen
      assign rx_log_ch_num[1] = 8'd2; // Rx channel
      assign tx_log_ch_num[1] = (VIDEO_STANDARD=="dl")? (DIRECTION=="du" ? 8'd4 + TX_LOG_CH_RCFG: 8'd6 + TX_LOG_CH_RCFG) :
                                (DIRECTION=="du" ? 8'd2 + TX_LOG_CH_RCFG : 8'd3 + TX_LOG_CH_RCFG); // Tx channel
      assign tx_log_ch_num_b[1] = (VIDEO_STANDARD=="dl")? (DIRECTION=="du" ? 8'd7 + TX_LOG_CH_RCFG : 8'd9 + TX_LOG_CH_RCFG) : 8'd0 + TX_LOG_CH_RCFG;
   end
   endgenerate
   ...
   begin
      if (rst) begin
         ...
         std_select[1] <= sdi_rx_std_sync[ch_loops/2*3+1];
         std_select[0] <= sdi_rx_std_sync[ch_loops/2*3];
   ...
   localparam [3:0] READ             = 4'b1001;  // Read or verify the data contains in the register which offset had been specified
   localparam [3:0] READ_WAIT_REQUEST = 4'b1010;  // Read or verify the data contains in the register which offset had been specified
   localparam [3:0] READ_BUSY_DELAY  = 4'b1011;  // Wait for reconfig_busy deassertion (Read/Write process is done)
   localparam [3:0] FINISH_RECONFIG  = 4'b1100;  // Check if all read/write operations on a channel is done

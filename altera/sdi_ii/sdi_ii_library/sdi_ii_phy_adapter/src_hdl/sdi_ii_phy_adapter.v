...
//--------------Common ports used for all families  ----------------
  ...
  rx_std,                   // rx_std from core for data mapping to Native PHY in different config
...
//-------12G mode is using pma_div_clk instead-------------------
  unused_tx_clkout,
  unused_rx_clkout,

//-------Additional ports for Enhanced PCS-------------------
  tx_control,
  rx_control,
  tx_enh_data_valid
   ...
   localparam TX_REFCLK_CNT     = (XCVR_TX_PLL_SEL != 0)   ? 2 : 1; // For V-series only
   localparam TXPLL_LCK_CNT     = (XCVR_TX_PLL_SEL == 1) ? 2 : 1;   // For V-series only
   localparam TXPLL_PWRDWN_CNT  = (XCVR_TX_PLL_SEL == 1 || ED_TXPLL_SWITCH == 1)   ? 2 : 1; // For V-series & Arria 10
   ...
   input  [20*NUM_STREAMS-1:0]      xcvr_tx_datain;
   ...
   input  [TXPLL_LCK_CNT - 1:0]    tx_pll_locked_from_xcvr;
   ...
   input  [TXPLL_PWRDWN_CNT - 1:0] pll_powerdown_in;
   ...
   output [20*NUM_STREAMS-1:0]     xcvr_rx_dataout;
   ...
   input                           unused_tx_clkout;
   input                           unused_rx_clkout;
   output                          tx_enh_data_valid;
   output [17:0]                   tx_control;
   input  [19:0]                   rx_control;
  ...
  begin : tx_gen
     ...
     assign tx_pll_select_to_xcvr_rst   = (XCVR_TX_PLL_SEL == 1 || ED_TXPLL_SWITCH == 1) ? xcvr_refclk_sel : 1'b0;
     assign tx_serial_clk_out           = tx_serial_clk_in;

     if (VIDEO_STANDARD == "mr") begin
         assign tx_datain_to_xcvr   = {24'd0, xcvr_tx_datain[79:40], 24'd0, xcvr_tx_datain[39:0]};
         assign tx_enh_data_valid   = 1'b1;
         assign tx_control          = 18'd0;
     end else begin
         assign tx_datain_to_xcvr   = {12'd0, xcvr_tx_datain[19:10], 12'd0, xcvr_tx_datain[9:0]};
         assign tx_enh_data_valid   = 1'b0;
         assign tx_control          = 18'd0;
     end
     ...
     if (XCVR_TX_PLL_SEL != 0) begin 
        assign tx_pll_refclk[1]             = xcvr_refclk_alt;
     end

    if (XCVR_RST_CTRL_CHS == 1) begin
  ...
  begin : rx_gen
     ...
     // Rxdata assignment due to the active data placement in PHY which is not in simplified mode
     if (VIDEO_STANDARD == "mr") begin
        assign xcvr_rx_dataout          = rx_std[2:1] == 2'b11 ? {rx_dataout_from_xcvr[103:64], rx_dataout_from_xcvr[39:0]} : 
                                          rx_std[2:1] == 2'b10 ? {40'd0, rx_dataout_from_xcvr[57:48], rx_dataout_from_xcvr[41:32], rx_dataout_from_xcvr[25:16], rx_dataout_from_xcvr[9:0]} :
                                                                 {60'd0, rx_dataout_from_xcvr[41:32], rx_dataout_from_xcvr[9:0]};
     end else begin

...
begin
   if (enable) begin
     ...
     if (check_vpid_once) begin
       if (rx_b2a | rx_a2b) begin
         repeat (2) @(posedge anc_detected);
       end else begin
         @(posedge anc_detected);
       end
       $display ("Payload ID packet detected. Checking Payload ID...");
       check_vpid_ins = 1'b1;
       wait (check_done);
       ...
       if (interlaced) begin
         $display("Receiving format is interlaced format. Waiting for the next Payload ID packet...");
         @(posedge anc_detected);
         $display ("Payload ID packet detected. Checking Payload ID...");
         check_vpid_ins = 1'b1;
         wait (check_done);
     ...
     if (check_vpid_error) begin
       ...
       $display ("Payload ID packet detected, however vpid_valid signal should be low.");
       ...
       $display ("Test proceeds to receiving a valid Payload ID packet. Waiting for the Payload ID packet...");
       @(posedge anc_detected);
       $display ("Payload ID packet detected. Checking Payload ID...");
       check_vpid_ins = 1'b1;
       wait (check_done);
       ...
       $display ("Test proceeds to receiving a few non-valid Payload ID packet. Expecting vpid_valid signal to be deasserted after a few frames.");
       if (mode_dl) begin
   ...
   // Check other ANC packet, if vpid is inserted here then assert error signal
     ...
     if (anc_detected_a) begin
        if (rxdata_tocheck[7:0] != 8'h41) begin
           ...
           $display("Other anc packet should be present before Payload ID packet, current packet detected rxdata_tocheck[7:0] = %h", rxdata_tocheck[7:0]);
           vpid_error = 1'b1;
        end
     end
     if (anc_detected_b) begin
        if (rxdata_tocheck_b[7:0] != 8'h41) begin
           ...
           $display("Other anc packet should be present before Payload ID packet, current packet detected rxdata_tocheck_b[7:0] = %h", rxdata_tocheck_b[7:0]);
           vpid_error = 1'b1;
        end
     end
     check_done = 1'b1;
